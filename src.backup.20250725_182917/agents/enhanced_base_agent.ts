...
import { EnhancedMemorySystem, type Memory, type MemorySearchOptions } from '../memory/enhanced_memory_system';
...
export abstract class EnhancedBaseAgent extends BaseAgent {
  ...
  protected async process(_context: AgentContext): Promise<AgentResponse> {
    ...
    try {
      ...
      const relevantMemories = await this.searchRelevantMemories(_context.userRequest);
      ...
      if (this.enhancedConfig.autoLearn && response.success) {
        await this.storeInteractionMemory(_context.userRequest, response);
      }
