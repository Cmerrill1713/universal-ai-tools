...
export class EnhancedPlannerAgent extends EnhancedMemoryAgent {
  ...
  protected async executeWithMemory(_context: AgentContext): Promise<PartialAgentResponse> {
    ...
    try {
      ...
      if (requestAnalysis.requiresExternalData) {
        await this.gatherExternalContext(requestAnalysis, _context);
      }
      ...
      if (_context.userRequest.toLowerCase().includes('execute') || 
          _context.userRequest.toLowerCase().includes('implement')) {
        const executionResults = await this.executePlanSteps(optimizedPlan, _context);
  ...
  private async analyzeRequestWithMemory(_context: AgentContext): Promise<any> {
    ...
    const requiresExternalData = 
      ...
      memoryInsights.similarRequests.length < 3 || // Few similar past requests;
      _context.userRequest.toLowerCase().includes('latest') || // Needs current information;
      _context.userRequest.toLowerCase().includes('current') ||;
      _context.userRequest.toLowerCase().includes('search');
...
This memory-integrated approach ensures each plan builds upon accumulated wisdom while adapting to specific requirements.`;`
  ...
  private adjustStepsWithPattern(steps: PlanStep[], _pattern: PlanningPattern): PlanStep[] {
    return steps.map((step) => ({
      ...step,
      confidence: Math.min(1.0, step.confidence + _pattern.successRate * 0.2),
      estimatedTime: this.adjustTimeWithPattern(step.estimatedTime, _pattern.averageTime),
    }));
  }
  ...
  private getAppliedPatterns(domain: string): string[] {
    const _pattern = this.planningPatterns.get(domain);
    return _pattern ? _pattern.commonSteps.slice(0, 3) : [];
  }
   ...
   */
  private async gatherExternalContext(analysis: any, _context: AgentContext): Promise<void> {
    ...
    try {
      ...
      if (this.isToolAvailable('WEB_SEARCH')) {
        ...
        if (searchResult.success && searchResult.data) {
          analysis.externalResources = searchResult.data.results;
          this.logger.info(`Found ${searchResult.data.results.length} external resources for ${domain}`);
        }
      }

      // Analyze local project structure if applicable
      if (this.isToolAvailable('LIST_FILES') && _context.workingDirectory) {
        const filesResult = await this.executeTool({
          toolName: 'LIST_FILES',
          parameters: { path: _context.workingDirectory, recursive: true },
          requestId: _context.requestId,
        });

        if (filesResult.success && filesResult.data) {
          analysis.projectStructure = filesResult.data;
          
          // Analyze key files if found
          if (domain === 'web_development' && filesResult.data.includes('package.json')) {
            ...
            if (packageResult.success) {
              analysis.projectConfig = JSON.parse(packageResult.data);
            }
          }
        }
      }
    } catch (error) {
      this.logger.warn('Failed to gather external context:', error);
   ...
   */
  private async executePlanSteps(plan: Plan, _context: AgentContext): Promise<any[]> {
      ...
      if (step.tools && step.tools.length > 0) {
        ...
        for (const toolName of step.tools) {
          if (this.isToolAvailable(toolName)) {
            try {
              const toolParams = this.getToolParametersForStep(step, toolName, _context);
              ...
              if (!plan.toolsUsed.includes(toolName)) {
                plan.toolsUsed.push(toolName);
              }
              ...
              if (result.success) {
                ...
                step.confidence = Math.max(0.1, step.confidence - 0.2);
                this.logger.warn(`Tool ${toolName} failed for step ${step.id}: ${result.error}`);
              }
            } catch (error) {
        ...
        executionResults.push(stepResults);
      }
    }
   ...
   */
  private getToolParametersForStep(step: PlanStep, toolName: string, _context: AgentContext): any {
    // Map tool names to appropriate parameters based on step context
    const params: any = {};

    switch (toolName) {
      case 'CREATE_DIRECTORY':;
        params.path = this.extractPathFromStep(step, 'directory');
        params.recursive = true;
        break;

      case 'CREATE_FILE':;
      case 'WRITE_FILE':;
        params.path = this.extractPathFromStep(step, 'file');
        params.content = this.generateFileContent(step, _context);
        break;

      case 'EXECUTE_COMMAND':;
        params.command = this.extractCommandFromStep(step);
        params.cwd = _context.workingDirectory || process.cwd();
        break;

      case 'ANALYZE_CODE':;
        params.path = this.extractPathFromStep(step, 'code');
        break;

      case 'WEB_SEARCH':;
        params.query = this.extractSearchQueryFromStep(step);
        params.limit = 10;
        break;

      default:;
        // Default parameters
        params.path = _context.workingDirectory || '.';
    }
  ...
  private extractPathFromStep(step: PlanStep, type: string): string {
    ...
    if (type === 'directory' && step.description.includes('directory')) {
      const match = step.description.match(/directory\s+['"]?([^'"]+)['"]?/i);
      return match ? match[1] : 'project';
    }
    
    if (type === 'file' && step.description.includes('file')) {
      const match = step.description.match(/file\s+['"]?([^'"]+)['"]?/i);
      return match ? match[1] : 'README.md';
    }

    return type === 'directory' ? 'src' : 'index.js';
  }

  private generateFileContent(step: PlanStep, _context: AgentContext): string {
    // Generate appropriate file content based on step context
    if (step.description.includes('README')) {
      return `# ${_context.userRequest}\n\nGenerated by Enhanced Planner Agent\n\n## Overview\n\nThis project was set up according to the strategic plan.\n`;
    }
    
    if (step.description.includes('config')) {
  ...
  private extractCommandFromStep(step: PlanStep): string {
    ...
    for (const pattern of commandPatterns) {
      const match = step.description.match(pattern);
      if (match) return match[1];
    }

    // Default commands based on context
    if (step.description.includes('install')) return 'npm install';
    if (step.description.includes('test')) return 'npm test';
    if (step.description.includes('build')) return 'npm run build';
  ...
  private extractSearchQueryFromStep(step: PlanStep): string {
    ...
    const keywords = step.description.toLowerCase().split(' ')
      .filter(word => word.length > 3 && !['with', 'from', 'using', 'based'].includes(word));
