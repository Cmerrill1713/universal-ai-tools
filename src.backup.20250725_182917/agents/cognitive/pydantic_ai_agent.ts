...
export class PydanticAIAgent extends RealCognitiveAgent {
  ...
  protected setupCognitiveCapabilities(): void {
    ...
    this.cognitiveCapabilities.set('structured_response', {
      ...
      execute: async (input any, _context: AgentContext) => {
        return this.generateStructuredResponse(inputcontext);
      },
    });
    ...
    this.cognitiveCapabilities.set('validate', {
      name: 'validate',
      execute: async (input any, _context: AgentContext) => {
        return this.validateData(inputcontext);
      },
    });
    ...
    this.cognitiveCapabilities.set('analyze', {
      name: 'analyze',
      execute: async (input any, _context: AgentContext) => {
        return this.performCognitiveAnalysis(inputcontext);
      },
    });
    ...
    this.cognitiveCapabilities.set('plan', {
      name: 'plan',
      execute: async (input any, _context: AgentContext) => {
        return this.createTaskPlan(inputcontext);
      },
    });
    ...
    this.cognitiveCapabilities.set('generate_code', {
      name: 'generate_code',
      execute: async (input any, _context: AgentContext) => {
        return this.generateCode(inputcontext);
      },
    });
  }
  ...
  private async generateStructuredResponse(;
    ...
    try {
      ...
      const response = schema
        ? await this.pydanticService.requestWithSchema(requestschema);
   ...
   */
  private async validateData(inputany, _context: AgentContext): Promise<PartialAgentResponse> {
    try {
      ...
      return {
        ...
        reasoning: result.success;
          ...
          : `Data validation failed with ${result.error.errors.length} errors`,
   ...
   */
  private async createTaskPlan(inputany, _context: AgentContext): Promise<PartialAgentResponse> {
    try {
   ...
   */
  private async generateCode(inputany, _context: AgentContext): Promise<PartialAgentResponse> {
    try {
  ...
  protected async process(;
    ...
    try {
      ...
      const result = await capability.execute(
        { contentcontext.userRequest, ...context.metadata },
        context;
      );
