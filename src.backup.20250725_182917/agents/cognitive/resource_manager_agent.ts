...
export class ResourceManagerAgent extends EnhancedMemoryAgent {
  ...
  async processInput(inputstring, _context: AgentContext): Promise<PartialAgentResponse> {
    try {
      ...
      switch (requestType) {
        case 'allocate':;
          return await this.handleAllocationRequest(input_context);
        case 'release':;
          return await this.handleReleaseRequest(input_context);
        case 'optimize':;
          return await this.handleOptimizationRequest(input_context);
        case 'status':;
          return await this.handleStatusRequest(input_context);
        case 'forecast':;
          return await this.handleForecastRequest(input_context);
        default:;
          return await this.handleGeneralResourceQuery(input_context);
      }
    } catch (error) {
      return this.handleResourceError(error: input_context);
    }
  }
  ...
  private async handleAllocationRequest(;
    ...
    const request this.parseAllocationRequest(input_context);
    ...
    // Create allocation
    const allocation = this.createAllocation(requestavailableResource);
  ...
  private parseAllocationRequest(inputstring, _context: AgentContext): ResourceRequest {
  ...
  private async handleReleaseRequest(;
    ...
    if (!allocationId) {
      ...
      const consumerId = _context.requestId || this.extractConsumerId(input
  ...
  private processPendingRequests(): void {
    ...
    for (const _requestof this.pendingRequests) {
      ...
      if (resource) {
        const allocation = this.createAllocation(requestresource);
