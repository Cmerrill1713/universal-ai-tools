...
export class UniversalAgentRegistry extends EventEmitter {
    ...
    supabase;
    logger = console;
    constructor(memoryCoordinator, supabase) {
        super();
        this.memoryCoordinator = memoryCoordinator;
        this.supabase = supabase;
        this.registerCognitiveAgents();
        this.registerPersonalAgents();
        this.logger.info(`✅ Universal Agent Registry initialized with ${this.agentDefinitions.size} agent definitions`);
    }
    /**
     * Register the 10 cognitive agents for Universal AI Tools
     */
    registerCognitiveAgents() {
     ...
     */
    registerPersonalAgents() {
        const personalAgents = [
            {
                name: 'personal_assistant',
                category: AgentCategory.PERSONAL,
                description: 'High-level personal AI assistant with vector memory for intelligent coordination',
                priority: 1,
     ...
     */
    async importAgentClass(definition) {
        try {
            // Import based on category and module path
            if (definition.category === AgentCategory.PERSONAL) {
                switch (definition.name) {
                    case 'personal_assistant': {
                        const { default: PersonalAssistantAgent } = await import('./personal/personal_assistant_agent');
                    ...
                    case 'photo_organizer': {
                        const { default: PhotoOrganizerAgent } = await import('./personal/photo_organizer_agent');
    ...
    async processRequest(agentName, context) {
        ...
        if (definition) {
            agentCollaborationWS.updateAgentStatus({
                agentId: agentName,
                agentName: definition.className.replace('Agent', ' Agent'),
                status: 'thinking',
                currentTask: `Processing: ${context.task || 'request'}`,
                timestamp: new Date(),
                metadata: {
                    participatingIn: context.requestId || 'direct',
                },
            });
        }
        try {
            ...
            // Notify completion
            agentCollaborationWS.completeAgentTask(agentName, result);
            return result;
        }
        catch (error) {
            // Notify error
            agentCollaborationWS.updateAgentStatus({
                agentId: agentName,
                agentName: definition?.className.replace('Agent', ' Agent') || agentName,
                status: 'error',
     ...
     */
    async unloadUnusedAgents(maxIdleMinutes = 30) {
    ...
    async shutdown() {
        ...
        const shutdownPromises = Array.from(this.loadedAgents.values()).map((agent) => agent.shutdown().catch((error) => this.logger.error(`Error shutting down agent:`, error)));
