...
export class AgentCoordinator extends EventEmitter {
  ...
  private async handleResourceRequest(;
    ...
    logger.info(`Handling resource _requestfrom ${fromAgent}`, request;
    await this.messageBroker.sendMessage({
      sessionId: session.id,
      fromAgent: 'coordinator',
      toAgent: fromAgent,
  ...
  private async handleCoordinationRequest(;
    ...
    logger.info(`Handling coordination _requestfrom ${fromAgent}`, request;
    await this.messageBroker.sendMessage({
      sessionId: session.id,
      fromAgent: 'coordinator',
      toAgent: fromAgent,
  ...
  private async initiateErrorRecovery(agentId: string, error: any): Promise<void> {
    logger.error`🚨 Initiating _errorrecovery for agent ${agentId}`, error:

    // Create an _errorrecovery plan
    const recoveryPlan = await this.coordinateGroupFix(
      `Error recovery for agent ${agentId}: ${error.message || error:,`
      { agentId, error:;
    );
