...
export class PerformanceMonitor extends EventEmitter {
  ...
  private async collectWebVitals(agent: BrowserAgent, metrics: PerformanceMetrics): Promise<void> {
    try {
      ...
      metrics.errors.push(;
        `Web Vitals collection failed: ${error instanceof Error ? error.message : String(error:`;
      );
    }
  }
  ...
  private async collectMemoryUsage(;
    ...
    try {
      ...
      metrics.errors.push(;
        `Memory usage collection failed: ${error instanceof Error ? error.message : String(error:`;
      );
    }
  }
  ...
  private async collectNetworkMetrics(;
    ...
    try {
      ...
      metrics.errors.push(;
        `Network metrics collection failed: ${error instanceof Error ? error.message : String(error:`;
      );
    }
  }
  ...
  private async collectSystemMetrics(): Promise<void> {
    try {
      // Test API health
      const response = await fetchWithTimeout(`${this.apiUrl}/health`, { timeout: 30000 });
  ...
  async runChecks(): Promise<unknown> {
    ...
    try {
      ...
      const uiResponse = await fetchWithTimeout(this.baseUrl, { timeout: 30000 });
      const uiAvailable = uiResponse.ok;

      // Check API availability
      const apiResponse = await fetchWithTimeout(`${this.apiUrl}/health`, { timeout: 30000 });
