...
export class PydanticAIService {
  ...
  async requestrequest Partial<AIRequest>): Promise<AIResponse> {
    ...
    try {
      ...
      return this.createErrorResponse(_requestid || uuidv4(), error: Date.now() - startTime);
    }
  }
  ...
  private async processResponse(;
    ...
    const response: AIResponse = {
      ...
      contentthis.extractTextContent(dspyResponse.result),
      structuredData,
    ...
    if (_requestcontext.memoryEnabled) {
      await this.storeInteractionMemory(requestresponse);
    }
  ...
  convertAgentResponse(agentResponse: AgentResponse, requestAIRequest): AIResponse {
    return {
      ...
      contentagentResponse.data ? String(agentResponse.data) : agentResponse.message || '',
      ...
      validation: {
        ...
        errors: agentResponse.error:  [agentResponse.error:: undefined,
      },
      metadata: {
...
export const pydanticAI = {
  ...
  requestWithSchema: <T>(requestPartial<AIRequest>, schema: z.ZodSchema<T>) =>;
