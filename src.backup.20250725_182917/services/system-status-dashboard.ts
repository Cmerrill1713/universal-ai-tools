...
import { getErrorTrackingService } from './_errortracking-service';
...
export class SystemStatusDashboard extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to start system status dashboard', LogContext.SYSTEM, { error:);
      throw error:;
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      logger.error('Error stopping system status dashboard', LogContext.SYSTEM, { error:);
      throw error:;
    }
  }
  ...
  private async startWebSocketServer(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.wss.on('connection', (ws: WebSocket, requestIncomingMessage) => {
        ...
        this.wss.on('error:  (error:=> {
          logger.error('WebSocket server error:  LogContext.SYSTEM, { error:);
          reject(error:;
        });
      } catch (error) {
        reject(error:;
  ...
  private handleNewConnection(ws: WebSocket, requestIncomingMessage): void {
    ...
    const clientIp = _requestsocket.remoteAddress || 'unknown';
    const userAgent = _requestheaders['user-agent'];
    ...
    ws.on('error:  (error:=> {
      logger.error('WebSocket client error:  LogContext.SYSTEM, {
        client_id: clientId,
        _error;
      });
    });
  ...
  private async startDataCollection(): Promise<void> {
    ...
    this.updateInterval = setInterval(async () => {
      try {
        ...
        logger.error('Error collecting dashboard data', LogContext.SYSTEM, { error:);
      }
  ...
  private collectApplicationMetrics(apmMetrics: any, dbHealth: any): DashboardData['application'] {
    return {
      ...
      errors: {
        total: 0, // Would get from _errortracking;
        perMinute: 0, // Would get from _errortracking;
        topErrors: [], // Would get from _errortracking;
      },
    };
  }
  ...
  private collectAlerts(healthData: any, errorStats: any): DashboardData['alerts'] {
    ...
    // Add _erroralerts (would get from _errortracking service)
