...
export class MLXFineTuningService {
  ...
  async prepareTrainingData(agentInteractions: any[], taskType: string): Promise<string> {
    ...
    for (const interaction of agentInteractions) {
      if (interaction.success && interaction.quality_score > 0.7) {
        trainingData.push({
          inputinteraction.user__request;
    ...
    logger.info(`📊 Prepared ${trainingData.length} training examples for ${taskType}`);
    return datasetPath;
  }
  ...
  async fineTuneModel(config: FineTuningConfig): Promise<string> {
    ...
    logger.info(`🚀 Starting fine-tuning job ${jobId} for ${config.task_type}`);
    ...
    process.stdout?.on('data', (data) => {
      logger.info(`[${jobId}] ${data.toString()}`);
    });
    ...
    process.on('close', (code) => {
      logger.info(`Fine-tuning job ${jobId} finished with code ${code}`);
      ...
      if (code === 0) {
        ...
        logger.info(`✅ Fine-tuned model registered as ${modelKey}`);
      }
    });
  ...
  async createSpecializedAgents(agentInteractionData: any[]) {
    ...
    for (const taskType of tasks) {
      ...
      if (taskData.length >= 50) {
        ...
        logger.info(`🎯 Creating specialized ${taskType} agent...`);
        ...
        logger.info(`⚠️ Insufficient data for ${taskType} agent (${taskData.length}/50)`);
      }
    }
