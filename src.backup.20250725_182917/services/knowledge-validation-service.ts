...
export class KnowledgeValidationService {
  ...
  async validateScrapedKnowledge(;
    ...
    try {
      ...
      const qualityResult = await this.validateContentQuality(contentmetadata);
      ...
      if (source.priority === 'high' && metadata.contentType !== 'code') {
        const factCheckResult = await this.validateFactChecking(contentmetadata);
        validationResults.push(factCheckResult);
      }

      // 4. Deprecation detection
      const deprecationResult = await this.detectDeprecation(contentmetadata);
      ...
      if (metadata.hasCodeExamples || metadata.contentType === 'code') {
        const technicalResult = await this.validateTechnicalAccuracy(contentmetadata);
  ...
  private async validateContentQuality(;
    ...
    const structureScore = this.validateContentStructure(contentmetadata);
  ...
  private async validateFactChecking(;
    ...
    try {
      ...
      const factCheckResult = await dspyService.requestvalidate_facts', {
        claims,
        context: metadata,
        requireSources: true,
      });
  ...
  private async detectDeprecation(;
    ...
    const deprecationDatabase = await this.checkDeprecationDatabase(contentmetadata);
  ...
  private async validateTechnicalAccuracy(;
    ...
    for (const codeBlock of codeBlocks) {
      ...
      if (!syntaxValidation.isValid) {
        issues.push(`Syntax _errorin ${codeBlock.language} code: ${syntaxValidation.error:);`;
  ...
  private countTokens(contentstring): number {
    if (this.encoding) {
      return this.encoding.encode(contentlength;
    }
