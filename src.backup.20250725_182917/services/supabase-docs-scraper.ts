...
COMMENT ON COLUMN posts._contentIS E'@graphql({"description": "Post content)';
  ...
  private async storeDocumentation(): Promise<void> {
    ...
    for (const [key, doc] of this.docsCache) {
      try {
        ...
        const { error: kbError } = await this.supabase.from('ai_knowledge_base').upsert(
          {
        ...
        for (const snippet of validatedDoc.code_snippets) {
          const { error: snippetError } = await this.supabase.from('ai_code_snippets').insert({
        ...
        if (validatedDoc.examples) {
          for (const example of validatedDoc.examples) {
            const { error: exampleError } = await this.supabase.from('ai_code_examples').insert({
              title: example.title,
