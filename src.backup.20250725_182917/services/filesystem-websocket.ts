...
export interface FSWebSocketMessage {
  type: 'auth' | 'watch' | 'unwatch' | 'event' | 'error: | 'ping' | 'pong';
  path?: string;
...
export class FileSystemWebSocketService extends EventEmitter {
  ...
  async start(server?: any): Promise<void> {
    try {
      ...
      logger.error('Failed to start File System WebSocket service:', error:;
      throw error:;
    }
  }
  ...
  async stop(): Promise<void> {
    try {
      ...
      logger.error('Error stopping File System WebSocket service:', error:;
      throw error:;
    }
  }
  ...
  private async handleWatch(;
    ...
    if (!watcher) {
      ...
      watcher.on('error:  (error:=> {
        logger.error('File watcher error: , error:{ path: sanitizedPath });
      });
  ...
  private sanitizePath(inputPath: string): string | null {
    try {
      ...
      logger.error('Path sanitization _errorin WebSocket:', error:;
      return null;
    }
  }
