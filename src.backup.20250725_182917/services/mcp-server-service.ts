...
export class MCPServerService extends EventEmitter {
  ...
  private async handleConnection(ws: WebSocket, req: any): Promise<void> {
    ...
    ws.on('message', async (data) => {
      try {
        ...
        logger.error('Failed to handle MCP message', LogContext.SYSTEM, { error });
        ws.send(;
          JSON.stringify({
            type: 'error',
    ...
    ws.on('error', (error) => {
      logger.error('MCP WebSocket error', LogContext.SYSTEM, { connectionId, error });
    });
  ...
  private async handleAgentRegistration(;
    ...
    if (error) {
      logger.error('Failed to register MCP agent', LogContext.SYSTEM, { error });
      ws.send(;
        JSON.stringify({
          type: 'error',
  ...
  private async loadAgentConfigurations(): Promise<void> {
    ...
    if (error) {
      logger.error('Failed to load MCP agents', LogContext.SYSTEM, { error });
      return;
    }
  ...
  async getAgentKeys(agentId: string): Promise<Record<string, string>> {
    ...
    if (error || !keys) {
      logger.error('Failed to retrieve agent keys', LogContext.SECURITY, { error });
      return {};
    }
    ...
    for (const key of keys) {
      try {
        ...
        logger.error('Failed to decrypt key', LogContext.SECURITY, {
          agentId,
