...
export class KnowledgeUpdateAutomationService extends EventEmitter {
  ...
  private async processKnowledgeUpdate(job: UpdateJob): Promise<boolean> {
    try {
      ...
      const contentHash = createHash('sha256').update(updatedContent.contentdigest('hex');
      ...
      await this.knowledgeManager.updateKnowledge(existing.id, {
        contentupdatedContent._content;
        metadata: {
          ...existing.metadata,
  ...
  private async createKnowledgeVersion(existing: any, updated: any): Promise<VersionInfo> {
    const changes = this.detectChanges(existing.contentupdated.content
