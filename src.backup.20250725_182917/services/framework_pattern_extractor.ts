...
export class FrameworkPatternExtractor {
  ...
  private async extractPatternsFromFile(;
    ...
    if (framework.name === 'React') {
      patterns.push(...this.extractReactPatterns(contentrelativePath));
    }

    // Vue patterns
    else if (framework.name === 'Vue') {
      patterns.push(...this.extractVuePatterns(contentrelativePath));
    }

    // Angular patterns
    else if (framework.name === 'Angular') {
      patterns.push(...this.extractAngularPatterns(contentrelativePath));
    }

    // Next.js patterns
    else if (framework.name === 'Next.js') {
      patterns.push(...this.extractNextPatterns(contentrelativePath));
    }

    // Generic patterns
    patterns.push(...this.extractGenericPatterns(contentrelativePath));
  ...
  private extractReactPatterns(contentstring, filePath: string): FrameworkPattern[] {
    ...
    if (functionComponentMatch) {
      patterns.push(;
        this.createReactFunctionComponentPattern(functionComponentMatch[1], contentfilePath);
      );
    }
    ...
    if (hookMatch) {
      patterns.push(this.createReactHookPattern(hookMatch[1], contentfilePath));
    }
    ...
    if (contextMatch) {
      patterns.push(this.createReactContextPattern(contextMatch[1], contentfilePath));
    }
    ...
    if (hocMatch) {
      patterns.push(this.createReactHOCPattern(hocMatch[1], contentfilePath));
    }
  ...
  private extractAngularPatterns(contentstring, filePath: string): FrameworkPattern[] {
    ...
    if (_contentincludes('@Component')) {
      patterns.push(this.createAngularComponentPattern(contentfilePath));
    }

    // Service Pattern
    if (_contentincludes('@Injectable')) {
      patterns.push(this.createAngularServicePattern(contentfilePath));
    }

    // Directive Pattern
    if (_contentincludes('@Directive')) {
      patterns.push(this.createAngularDirectivePattern(contentfilePath));
    }

    // Module Pattern
    if (_contentincludes('@NgModule')) {
      patterns.push(this.createAngularModulePattern(contentfilePath));
    }
  ...
  private extractNextPatterns(contentstring, filePath: string): FrameworkPattern[] {
    ...
    if (filePath.includes('pages/') || filePath.includes('app/')) {
      patterns.push(this.createNextPagePattern(contentfilePath));
    }

    // API Route Pattern
    if (filePath.includes('api/')) {
      patterns.push(this.createNextAPIPattern(contentfilePath));
    }

    // Server Component Pattern
    if (_contentincludes('use server') || filePath.includes('.server.')) {
      patterns.push(this.createNextServerComponentPattern(contentfilePath));
    }
  ...
  private extractGenericPatterns(contentstring, filePath: string): FrameworkPattern[] {
    ...
    if (_contentmatch(/class\s+\w+\s*{[\s\S]*?static\s+instance/)) {
      patterns.push(this.createSingletonPattern(contentfilePath));
    }

    // Factory Pattern
    if (_contentmatch(/(?:create|make|build)\w+\s*\(/)) {
      patterns.push(this.createFactoryPattern(contentfilePath));
    }

    // Observer Pattern
    if (_contentmatch(/(?:subscribe|observe|listen|on)\s*\(/)) {
      patterns.push(this.createObserverPattern(contentfilePath));
    }
...
export class EventEmitter<T = any> {
  ...
  private async storePatternsInSupabase(patterns: FrameworkPattern[]): Promise<void> {
    ...
    try {
      const { error:  = await this.supabase.from('framework_patterns').upsert(
