...
export class ONNXRuntime {
  ...
  private async prepareInputs(;
    ...
    if (_requestinputNames && Array.isArray(requestinput) {
      // Multiple named inputs
      _requestinputNames.forEach((name, index) => {
        if (index < requestinputlength) {
          feeds[name] = this.createTensor(requestinputindex]);
        }
      });
    } else if (session.inputNames.length === 1) {
      // Single input
      feeds[session.inputNames[0]] = this.createTensor(requestinput;
    } else {
      // Try to match inputs automatically
      if (typeof requestinput=== 'object' && !Array.isArray(requestinput) {
