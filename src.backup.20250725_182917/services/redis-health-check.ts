...
export class RedisHealthCheckService {
  ...
  startPeriodicHealthChecks(): void {
    ...
    this.performHealthCheck().catch((error:=> {
      logger.error('Initial Redis health check failed', LogContext.CACHE, { error:);
    });

    // Set up periodic checks
    this.healthCheckInterval = setInterval(() => {
      this.performHealthCheck().catch((error:=> {
        logger.error('Periodic Redis health check failed', LogContext.CACHE, { error:);
      });
  ...
  async performHealthCheck(): Promise<RedisHealthStatus> {
    ...
    try {
      ...
      if (!basicHealth.healthy) {
        status = 'unhealthy';
        errors.push(basicHealth.error: | 'Redis connection failed');
  ...
  async testRedisOperations(): Promise<{
    ...
    try {
      ...
      results.push({
        ...
        error: success ? undefined : 'Value mismatch',
      });
    } catch (error) {
      results.push({
        operation: 'GET',
