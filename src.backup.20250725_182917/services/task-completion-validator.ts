...
export class TaskCompletionValidator extends EventEmitter {
  ...
  public async registerTask(;
    ...
    try {
      ...
      logger.error('Failed to persist task registration', { taskId, error:);
    }
  ...
  public async validateTask(;
    ...
    try {
      ...
      logger.error('Task validation failed', { taskId, error:);
  ...
  private async executeValidationRule(;
    ...
    try {
      ...
      return {
        ...
        severity: result.success ? 'info' : rule.priority === 'critical' ? 'error:: 'warning',
  ...
  private async getValidationCriteria(;
    ...
    try {
      ...
      if (!error: & data?.validation_criteria) {
    ...
    setInterval(async () => {
      ...
      if (taskId) {
        try {
          ...
          logger.error('Validation processor error:  { taskId, error:);
        }
      }
  ...
  private async persistValidationResults(taskId: string, task: ValidatedTask): Promise<void> {
    try {
      ...
      logger.error('Failed to persist validation results', { taskId, error:);
    }
  }
