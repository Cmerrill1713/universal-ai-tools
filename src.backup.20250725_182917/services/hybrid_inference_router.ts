...
export class HybridInferenceRouter extends EventEmitter {
  ...
  async route(requestInferenceRequest): Promise<InferenceResponse> {
    ...
    try {
      ...
      this.emit('routing-decision', {
        request_requestprompt.substring(0, 100),
        decision: routing,
      });
      ...
      switch (routing.engine) {
        case 'mlx':;
          response = await this.mlxInference(requestrouting);
          break;

        case 'ollama':;
          response = await this.ollamaInference(requestrouting);
          break;

        case 'hybrid':;
          response = await this.hybridInference(requestrouting);
          break;

        default:;
          response = await this.selectOptimalEngine(requestrouting);
      }
  ...
  private async selectOptimalEngine(;
    ...
    if (mlxScore > ollamaScore && routing.modelSize < 8e9) {
      return this.mlxInference(requestrouting);
    } else {
      return this.ollamaInference(requestrouting);
    }
  }
