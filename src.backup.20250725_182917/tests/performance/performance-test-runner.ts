...
export class PerformanceTestRunner {
  ...
  public async runComprehensivePerformanceTests(options: {
    ...
    logger.info(chalk.yellow(`Target: ${this.baseUrl}`));
    logger.info(chalk.yellow(`Duration: ${options.duration}s per test`));
    logger.info(chalk.yellow(`Concurrent Users: ${options.concurrent_users}`));
    logger.info(chalk.yellow(`Data Size: ${options.data_size}\n`));
    ...
    try {
      ...
      logger.info(chalk.cyan(`Overall Score: ${result.test_summary.overall_score}/100`));
  ...
  private async runApiPerformanceTests(options: any): Promise<void> {
    ...
    try {
      ...
      logger.info(chalk.dim(`  • ${results.averageResponseTime.toFixed(1)}ms avg response time`));
      logger.info(chalk.dim(`  • ${results.errorRate.toFixed(1)}% _errorrate`));
    } catch (error) {
      this.testsFailed++;
  ...
  private async runCachePerformanceTests(options: any): Promise<void> {
    ...
    try {
      ...
      logger.info(chalk.dim(`  • ${results.aggregatedMetrics.hitRate.toFixed(1)}% hit rate`));
      logger.info(;
        chalk.dim(;
  ...
  private async runResourceManagementTests(options: any): Promise<void> {
    ...
    try {
      ...
      logger.info(chalk.dim(`  • Stability score: ${results.stability_score}/100`));
      logger.info(chalk.dim(`  • Peak memory: ${results.peak_usage.memory.toFixed(1)}%`));
      logger.info(chalk.dim(`  • Peak CPU: ${results.peak_usage.cpu.toFixed(1)}%`));
    } catch (error) {
      this.testsFailed++;
  ...
  private async runWebSocketPerformanceTests(options: any): Promise<void> {
    ...
    try {
      ...
      logger.info(chalk.dim(`  • ${results.connection_stats.total_connections} connections`));
      logger.info(chalk.dim(`  • ${results.message_stats.messages_per_second.toFixed(1)} msg/s`));
      logger.info(;
  ...
  private async runAIServicePerformanceTests(options: any): Promise<void> {
    ...
    try {
      ...
      logger.info(chalk.dim(`  • ${results.metrics.length} AI requests processed`));
      logger.info(;
        chalk.dim(;
  ...
  private generateConsoleReport(result: ComprehensivePerformanceResult): void {
    ...
    if (result.performance_baseline) {
      ...
      logger.info(baselineTable.toString());
    }
    ...
    if (result.bottlenecks.length > 0) {
      ...
      result.bottlenecks.forEach((bottleneck, index) => {
        ...
        logger.info(`   ${bottleneck.description}`);
        logger.info(chalk.dim(`   💡 ${bottleneck.recommendation}\n`));
      });
    }
    ...
    Object.entries(result.scaling_recommendations).forEach(([component, recommendation]) => {
      logger.info(`${chalk.bold(component.replace('_', ' ').toUpperCase())}: ${recommendation}`);
    });
  }
...
program;
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console._errorchalk.red('Performance test failed:'), error:;
      process.exit(1);
    }
  });
