...
interface WriteBehindOptions {
  ...
  onWriteError?: (error: Error, batch: WriteOperation[]) => void;
}
...
export class WriteBehindCache<T = any> extends EventEmitter {
  ...
  private onWriteError?: (error: Error, batch: WriteOperation[]) => void;
  ...
  private startFlushTimer(): void {
    ...
    this.flushTimer = setInterval(() => {
      this.flushBatch().catch((error:=> {
        logger.error('Write-behind cache flush error: , error:;
  ...
  private queueWrite(key: string, value: T, ttl: number): void {
    ...
    if (this.writeQueue.size >= this.batchSize) {
      this.flushBatch().catch((error:=> {
        logger.error('Write-behind cache immediate flush error: , error:;
        this.emit('error:  error:;
      });
    }
  }
  ...
  async get(key: string): Promise<T | undefined> {
    ...
    try {
      ...
      logger.error`Write-behind cache get _errorfor key ${key}:`, error:;
      this.emit('error:  error:;
    }
  ...
  async delete(key: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error`Write-behind cache delete _errorfor key ${key}:`, error:;
      this.emit('error:  error:;
  ...
  async has(key: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error`Write-behind cache has _errorfor key ${key}:`, error:;
      return false;
    }
  }
  ...
  private async flushBatch(): Promise<void> {
    ...
    try {
      ...
      logger.error('Write-behind cache batch write error: , error:;
  ...
  private async handleBatchError(batch: WriteOperation[], error: Error): Promise<void> {
    ...
    if (failedBatch.length > 0) {
      if (this.onWriteError) {
        this.onWriteError(error: failedBatch);
      }
    ...
    if (retryBatch.length > 0) {
      setTimeout(() => {
        this.flushBatch().catch((err) => {
          logger.error('Write-behind cache retry error: , err);
        });
      }, this.retryDelay);
    }
  }
  ...
  async clear(): Promise<void> {
    try {
      ...
      logger.error('Write-behind cache clear error: , error:;
      this.emit('error:  error:;
    }
  }
  ...
  async warmup(keys: string[]): Promise<void> {
    ...
    try {
      ...
      logger.error('Write-behind cache warmup error: , error:;
      this.emit('error:  error:;
    }
  }
