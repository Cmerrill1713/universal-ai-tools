...
export class WriteThroughCache<T = any> extends EventEmitter {
  ...
  async get(key: string): Promise<T | undefined> {
    ...
    try {
      ...
      logger.error`Write-through cache get _errorfor key ${key}:`, error:;
  ...
  private async performWrite(key: string, value: T, ttl: number): Promise<void> {
    try {
      ...
      logger.error`Write-through cache set _errorfor key ${key}:`, error:;
  ...
  async delete(key: string): Promise<boolean> {
    try {
      ...
      logger.error`Write-through cache delete _errorfor key ${key}:`, error:;
      this.emit('error:  error:;
      return false;
    }
  }
  ...
  async has(key: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error`Write-through cache has _errorfor key ${key}:`, error:;
      return false;
    }
  }
