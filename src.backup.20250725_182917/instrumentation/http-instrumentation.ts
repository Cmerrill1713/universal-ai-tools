...
export class HttpInstrumentation {
  ...
  async withHttpSpan<T>(operation: HttpOperation, fn: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      if (axios.isAxiosError(error: {
        ...
        span.setStatus({
          code: SpanStatusCode.ERROR,
          message: error instanceof Error ? error.message : 'HTTP _requestfailed',
        });
      }
  ...
  private handleRequest(config: InternalAxiosRequestConfig): InternalAxiosRequestConfig {
    ...
    if (config.data) {
      ...
      span.setAttribute('http.requestcontentlength', bodySize);
    }
   ...
   */
  private handleRequestError(error: any): Promise<unknown> {
    const { config } = _error
  ...
  private handleResponse(response: AxiosResponse): AxiosResponse {
    ...
    if (span) {
      ...
      span.setAttributes({
        ...
        'http.responsecontentlength': response.headers['content-length'] || 0,
        'http.responsecontenttype': response.headers['content-type'],
   ...
   */
  private handleResponseError(error: AxiosError): Promise<unknown> {
    ...
    if (span) {
      ...
      if (_errorresponse) {
        span.setAttributes({
          ...
          'http.responsecontentlength': _errorresponse.headers['content-length'] || 0,
          'http.responsecontenttype': _errorresponse.headers['content-type'],
        });
      ...
      if (error: request&& !_errorresponse) {
