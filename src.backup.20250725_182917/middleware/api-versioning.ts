...
export class ApiVersioningMiddleware {
  ...
  private initializeVersions() {
    ...
    const activeVersions = Array.from(this.versions.values())
      .filter((v) => v.active)
      .sort((a, b) => {
        const aNum = parseInt(a.version.slice(1, 10), 10);
        const bNum = parseInt(b.version.slice(1, 10), 10);
        return bNum - aNum;
      });
  ...
  versionRouter() {
    ...
    router.get('/versions', (req, res) => {
      ...
      res.json({
        success: true,
        currentVersion: (req as any).apiVersion || this.defaultVersion,
  ...
  addVersion(version: ApiVersion): void {
    ...
    if (validated.active) {
      const currentLatestNum = parseInt(this.latestVersion.slice(1, 10), 10);
      const newVersionNum = parseInt(validated.version.slice(1, 10), 10);
