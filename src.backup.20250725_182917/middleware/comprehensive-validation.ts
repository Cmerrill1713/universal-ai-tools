...
export class ComprehensiveValidationMiddleware {
  ...
  public validate(options: ComprehensiveValidationOptions = {}) {
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        this.handleValidationError(error: req, res, next);
      }
    };
  }
  ...
  private async applyRequestSizeLimit(req: Request, res: Response): Promise<void> {
    return new Promise((resolve, reject) => {
      requestSizeLimit(req, res, (error:=> {
        if (error:{
  ...
  private async applySQLProtection(req: Request, res: Response): Promise<void> {
    return new Promise((resolve, reject) => {
      this.sqlProtection.middleware()(req, res, (error:=> {
        if (error:{
          reject(;
  ...
  private async applySchemaValidation(;
    ...
    if (
      ...
      return new Promise((resolve, reject) => {
        ValidationMiddleware.validate(validationOptions)(req, res, (error:=> {
          if (error:{
            reject(error:;
          } else {
            resolve();
   ...
   */
  private handleValidationError(error: any, req: Request, res: Response, next: NextFunction): void {
    let statusCode = 400;
