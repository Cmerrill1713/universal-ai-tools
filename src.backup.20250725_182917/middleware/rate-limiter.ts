...
export class SupabaseRateLimitStore implements RateLimitStore {
  ...
  async get(key: string): Promise<RateLimitInfo | null> {
    try {
      ...
      return {
        ...
        firstRequest: new Date(data.first_requestgetTime(),
        blocked: data.blocked,
        tier: data.tier,
      };
    } catch (error) {
      logger.error('Rate limit store get error: , LogContext.SECURITY, {
  ...
  async set(key: string, value: RateLimitInfo, ttl: number): Promise<void> {
    try {
      await this.supabase.from('rate_limits').upsert({
        ...
        first_requestnew Date(value.firstRequest),
        blocked: value.blocked,
        tier: value.tier,
        updated_at: new Date(),
      });
    } catch (error) {
      logger.error('Rate limit store set error: , LogContext.SECURITY, {
        error: error instanceof Error ? error.message : String(error:;
      });
    }
  }
  ...
  async increment(key: string): Promise<number> {
    try {
      ...
      logger.error('Rate limit store increment error: , LogContext.SECURITY, {
        error: error instanceof Error ? error.message : String(error:;
      });
      return 1;
    }
  ...
  async reset(key: string): Promise<void> {
    try {
      ...
      logger.error('Rate limit store reset error: , LogContext.SECURITY, {
        error: error instanceof Error ? error.message : String(error:;
      });
    }
  }
  ...
  async cleanup(): Promise<void> {
    try {
      ...
      logger.error('Rate limit store cleanup error: , LogContext.SECURITY, {
        error: error instanceof Error ? error.message : String(error:;
      });
    }
  }
  ...
  public limit(;
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        logger.error('Rate limiting error: , LogContext.SECURITY, {
          error: error instanceof Error ? error.message : String(error:;
        });
