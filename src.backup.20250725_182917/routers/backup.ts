...
export function BackupRouter(supabase: SupabaseClient) {
  ...
  router.post('/create', validateRequest(CreateBackupSchema), async (req: any, res) => {
    try {
      ...
      logger.error('logger.error('Backup creation error:', error);
      res.status(500).json({
        success: false,
        error: {
          code: 'BACKUP_ERROR',
  ...
  router.post('/restore', validateRequest(RestoreBackupSchema), async (req: any, res) => {
    try {
      ...
      logger.error('logger.error('Restore error:', error);
      res.status(500).json({
        success: false,
        error: {
          code: 'RESTORE_ERROR',
  ...
  router.delete('/:backupId', async (req, res) => {
    try {
      ...
      logger.error('logger.error('Delete backup error:', error);
      res.status(500).json({
        success: false,
        error: {
          code: 'DELETE_ERROR',
  ...
  router.post('/:backupId/verify', async (req, res) => {
    try {
      ...
      logger.error('logger.error('Verify backup error:', error);
      res.status(500).json({
        success: false,
        error: {
          code: 'VERIFY_ERROR',
  ...
  router.post('/cleanup', async (req, res) => {
    try {
      ...
      logger.error('logger.error('Cleanup error:', error);
      res.status(500).json({
        success: false,
        error: {
          code: 'CLEANUP_ERROR',
