...
class FileSystemRouterClass {
  ...
  private sanitizePath(inputPath: string): string | null {
    try {
      ...
      logger.error('logger.error('Path sanitization error: , error;
      return null;
    }
  }
    ...
    try {
      ...
      if (!validation.success) {
        res.status(400).json({
          ...
          details: validation._error: errors,
        });
        return;
      }
      ...
      logger.error('logger.error('Browse error: , error;
  ...
  private async handleRead(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        const stats = await promises.stat(sanitizedPath);
        if (!stats.isFile()) {
      ...
      if (range && (range.start !== undefined || range.end !== undefined)) {
        ...
        stream.on('error', (error) => {
          logger.error('logger.error('Read stream error: , error;
          res.status(500).json({ error: 'Failed to read file' });
        });
      } else {
        // Read entire file
        const content await promises.readFile(sanitizedPath, encoding as BufferEncoding);
      ...
      logger.error('logger.error('Read error: , error;
      res.status(500).json({ error: 'Internal server error:});
    }
  }
  ...
  private async handleWrite(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { path: requestPath, contentencoding, mode, permissions } = validation.data;
      const userId = (req as any).user.id;
      ...
      let exists = false;
      try {
        await promises.stat(sanitizedPath);
        exists = true;
      } catch (error) {
        // File doesn't exist
      }
      ...
      // Write file
      if (mode === 'append') {
        await promises.appendFile(sanitizedPath, data);
      } else {
        await promises.writeFile(sanitizedPath, data);
      }

      // Set permissions if specified
      if (permissions) {
        await promises.chmod(sanitizedPath, parseInt(permissions, 8, 10));
      }
      ...
      logger.error('logger.error('Write error: , error;
      res.status(500).json({ error: 'Internal server error:});
    }
  }
    ...
    try {
      ...
      try {
        ...
        res.json({
          ...
          args,
          stdout: _errorstdout || '',
          stderr: _errorstderr || error.message,
      ...
      logger.error('logger.error('Execute error: , error;
      res.status(500).json({ error: 'Internal server error:});
    }
  }
  ...
  private async handleMove(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        await promises.stat(sanitizedSource);
      } catch (error) {
      ...
      try {
        await promises.stat(sanitizedDest);
        destExists = true;
      } catch (error) {
      ...
      // Move file/directory
      await promises.rename(sanitizedSource, sanitizedDest);
      ...
      logger.error('logger.error('Move error: , error;
      res.status(500).json({ error: 'Internal server error:});
    }
  }
  ...
  private async handleCopy(req: Request, res: Response): Promise<void> {
    try {
      ...
      let sourceStats;
      try {
        sourceStats = await promises.stat(sanitizedSource);
      } catch (error) {
        res.status(404).json({ error: 'Source not found' });
        return;
      }
      ...
      logger.error('logger.error('Copy error: , error;
      res.status(500).json({ error: 'Internal server error:});
    }
  }
  ...
  private async handleDelete(req: Request, res: Response): Promise<void> {
    try {
      ...
      let stats;
      try {
        stats = await promises.stat(sanitizedPath);
      } catch (error) {
      ...
      if (stats.isDirectory()) {
        ...
        await promises.rm(sanitizedPath, { recursive: true, force });
      } else {
        await promises.unlink(sanitizedPath);
      }
      ...
      logger.error('logger.error('Delete error: , error;
      res.status(500).json({ error: 'Internal server error:});
    }
  }
    ...
    try {
      ...
      const searchDirectory = async (dirPath: string): Promise<void> => {
        ...
        try {
          ...
          logger.error('logger.error('Search directory error: , error;
        }
      };
      ...
      logger.error('logger.error('Search error: , error;
      res.status(500).json({ error: 'Internal server error:});
    }
  }
  ...
  private async handleDownload(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.error('logger.error('Download error: , error;
      res.status(500).json({ error: 'Internal server error:});
    }
  }
  ...
  private handleWebSocket(ws: WebSocket, req: Request): void {
    ...
    ws.on('message', async (message: string) => {
      try {
        ...
        logger.error('logger.error('WebSocket message error: , error;
        ws.send()
          JSON.stringify({
            type: 'error;
  ...
  private async handleSystemInfo(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      logger.error('logger.error('System info error: , error;
      res.status(500).json({ error: 'Internal server error:});
    }
  }
