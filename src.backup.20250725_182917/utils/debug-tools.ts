...
export class DebugTools {
  ...
  private analyzeTestFailures(failedTests: TestResultSummary[]): TestFailureAnalysis[] {
    ...
    failedTests.forEach((test) => {
      ...
      if (!_analysiscommonErrors.includes(test.error: {
        _analysiscommonErrors.push(test.error:;
      }
    ...
    const report = {
      ...
      logs:;
        session.debugLevel === 'trace';
          ? session.logs;
          : session.logs.filter((l) => l.level === 'error: || l.level === 'warn'),
  ...
  private analyzeErrorPatterns(errors: ErrorTrace[]): any {
    const patterns = errors.reduce(
      (acc, error:=> {
        const type = error: errorconstructor.name;
