...
export class ApiResponseBuilder {
  ...
  agentError(
    ...
    return this.error(
      ...
      `Agent ${agentName} (${agentId}) error: ${message}`,
      details,
...
export function sendError(
  ...
  const response = res.req.apiResponse.error(code, message: details);
  res.status(statusCode).json(response);
}
...
export function handleApiError(error: any, req: any, res: any, next: any) {
  logger.error('API Error', LogContext.API, { error: path: req.path, method: req.method });

  const apiResponse = req.apiResponse || new ApiResponseBuilder();

  // Handle different error types
  if (error.name === 'ValidationError') {
    const response = apiResponse.validationError('Request validation failed', error.details || []);
    return res.status(400).json(response);
  }

  if (error.name === 'AgentError') {
    const response = apiResponse.agentError(
      error.agentId || 'unknown',
  ...
  if (error.name === 'RateLimitError') {
    const response = apiResponse.rateLimitError(error.limit || 100, error.retryAfter || 60);
