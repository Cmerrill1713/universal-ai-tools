...
export class AdaptiveToolManager {
  ...
  async executeAdaptiveTool(;
    ...
    const transformedInput = this.transformInput(inputpreference);
    ...
    await this.learnFromExecution(toolName, modelUsed, inputtransformedOutput);
  ...
  private transformInput(inputany, preference: FormatPreference): any {
    switch (preference.input_format) {
      ...
      case 'structured':;
        return this.convertToStructuredFormat(inputpreference.example);
      case 'json':;
      default:;
        return _input;
    }
  ...
  private async executeToolLogic(toolName: string, inputany, context?: any): Promise<unknown> {
    ...
    switch (toolName) {
      case 'adaptive_file_operation':;
        return this.executeFileOperation(inputcontext);
      case 'adaptive_search':;
        return this.executeSearch(inputcontext);
      case 'adaptive_code__analysis:;
        return this.executeCodeAnalysis(inputcontext);
      case 'adaptive_data_processing':;
        return this.executeDataProcessing(inputcontext);
      case 'adaptive_web_interaction':;
        return this.executeWebInteraction(inputcontext);
      default:;
  ...
  private convertToStructuredFormat(inputany, example: any): any {
    ...
    for (const key of Object.keys(example)) {
      ...
      result[key] = this.findMatchingField(inputkey);
    }
    return result;
  }
