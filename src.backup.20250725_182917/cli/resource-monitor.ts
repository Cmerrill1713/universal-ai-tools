...
async function generateReport(format: string) {
  ...
  try {
    ...
    spinner.fail(`Failed to generate report: ${error:);`;
    process.exit(1);
  }
}
...
function displayReportTable(report: any) {
  ...
  logger.info(overviewTable.toString());
  ...
  logger.info(chalk.cyan('\n=== Resource Allocations ==='));
  logger.info(allocTable.toString());
  ...
  logger.info(chalk.cyan('\n=== Memory Details ==='));
  logger.info(memoryTable.toString());
}
...
async function startProfiling(duration: number, outputFile?: string) {
  ...
  setTimeout(async () => {
    ...
    if (outputFile) {
      ...
      logger.info(chalk.green(`Profile saved to ${outputFile}`));
    } else {
...
async function adjustResources(options: any) {
  ...
  try {
    ...
    if (adjustments.length > 0) {
      ...
      adjustments.forEach((a) => logger.info(`  - ${a}`));
      logger.info(chalk.yellow('\nNote: Some changes may require a restart to take effect.'));
    }
  } catch (error) {
    spinner.fail(`Failed to adjust resources: ${error:);`;
    process.exit(1);
  }
}
...
async function forceGC() {
  ...
  try {
    ...
    logger.info(`Current memory usage: ${usage.details.heapUsedPercent}%`);
  } catch (error) {
    spinner.fail(`Failed to force GC: ${error:);`;
  }
}
...
async function takeHeapSnapshot() {
  ...
  try {
    ...
    spinner.fail(`Failed to take heap snapshot: ${error:);`;
  }
}
...
async function checkMemoryLeaks() {
  ...
  try {
    ...
    if (profile.leaks.length === 0) {
      logger.info(chalk.green('No memory leaks detected'));
    } else {
    ...
    spinner.fail(`Failed to check for leaks: ${error:);`
  }
}
...
async function showConnectionStatus(poolName: string) {
  ...
  try {
    ...
    logger.info(chalk.yellow('Supabase Connections:'));
    logger.info(`  Total: ${status.supabase.total}`);
    logger.info(`  Active: ${status.supabase.active}`);
    logger.info(`  Idle: ${status.supabase.idle}`);
    logger.info(`  Waiting: ${status.supabase.waiting}`);

    // Redis connections
    logger.info(chalk.yellow('\nRedis Connections:'));
    logger.info(`  Total: ${status.redis.total}`);
    logger.info(`  Active: ${status.redis.active}`);
    logger.info(`  Idle: ${status.redis.idle}`);
    logger.info(`  Waiting: ${status.redis.waiting}`);
    ...
    spinner.fail(`Failed to get connection status: ${error:);`;
  }
}
...
async function resetConnectionPool(poolName: string) {
  ...
  try {
    ...
    logger.info(chalk.yellow('Please restart the service to reset connection pools'));
  } catch (error) {
    spinner.fail(`Failed to reset connection pool: ${error:);`;
  }
}
