...
export class UniversalAIToolsClient {
  ...
  private async handleVersionError(error: any): Promise<unknown> {
    ...
    if (this.config.autoUpgrade && supportedVersions.includes(latestVersion)) {
      logger.info(`[API] Auto-upgrading from ${this.currentVersion} to ${latestVersion}`);
      ...
      return this.client.requesterrorconfig);
    }
  ...
  private async retryRequest(error: any): Promise<unknown> {
    ...
    logger.info(;
      `[API] Retrying requestattempt ${_errorconfig.__retryCount}/${this.config.retryAttempts}) after ${delay}ms`;
    );
