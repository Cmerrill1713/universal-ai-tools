...
export class HttpMCTSBridge implements IMCTSBridge {
  ...
  async recommendAgents(
    ...
    return response.json() as Promise<QuickRecommendationResult>;
  }
  ...
  async getPerformanceStats(): Promise<PerformanceStats> {
    ...
    return {
      totalIterations:
        (this.parseMetric(metricsText, 'ab_mcts_iterations_total') || 0) +
        (this.parseMetric(metricsText, 'ab_mcts_searches_total') || 0),
...
export class MCTSBridge extends HttpMCTSBridge {
  constructor(config?: MCTSConfig) {
    ...
    const baseUrl =
      process.env.AB_MCTS_SERVICE_URL || process.env.AB_MCTS_URL || 'http://ab-mcts-rust:8082';
