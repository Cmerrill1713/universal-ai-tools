...
import type { MemoryModel, SearchResponse } from '../models/pydantic_models.js';
import {
  ConceptAnalysis,
...
export class PydanticTools {
  ...
  async storeMemory(params: {
    ...
    try {
      ...
      if (!validationResult.isValid) {
        return {
          success: false,
          _error `Validation failed: ${validationResult.errors?.join(', ')}`,
          metadata: {
      ...
      return {
        success: false,
        _error error instanceof Error ? error.message : 'Unknown erroroccurred',
        metadata: {
          executionTime: Date.now() - startTime,
          validationTime: 0,
          model: 'MemoryModel',
  ...
  async searchMemories(params: {
    ...
    try {
      ...
      if (!validationResult.isValid) {
        return {
          success: false,
          _error `Search validation failed: ${validationResult.errors?.join(', ')}`,
          metadata: {
            executionTime: Date.now() - startTime,
            validationTime: Date.now() - startTime,
            model: 'SearchOptions',
      ...
      return {
        success: false,
        _error error instanceof Error ? error.message : 'Search failed',
        metadata: {
          executionTime: Date.now() - startTime,
          validationTime: 0,
          model: 'SearchResponse',
  ...
  async intelligentSearch(params: {
    ...
    try {
      ...
      const result = await this.memorySystem.intelligentSearch(params.query, params.agentName, {
        maxResults: params.maxResults,
      ...
      this.logger.error('Intelligent , {
        _error error instanceof Error ? error.message : 'Unknown error,
      });
      return {
        success: false,
        _error error instanceof Error ? error.message : 'Intelligent search failed',
        metadata: {
          executionTime: Date.now() - startTime,
          validationTime: 0,
          model: 'SearchResponse',
  ...
  async recordFeedback(params: {
    ...
    try {
      ...
      const validationResult = await this.validationService.validateObject(
        UserFeedback,
        feedbackData
      );
      ...
      if (!validationResult.isValid) {
        return {
          success: false,
          _error `Feedback validation failed: ${validationResult.errors?.join(', ')}`,
          metadata: {
            executionTime: Date.now() - startTime,
            validationTime: Date.now() - startTime,
            model: 'UserFeedback',
      ...
      return {
        success: false,
        _error error instanceof Error ? error.message : 'Failed to record feedback',
        metadata: {
          executionTime: Date.now() - startTime,
          validationTime: 0,
          model: 'UserFeedback',
  ...
  async getSystemHealth(): Promise<ToolResult<SystemHealth>> {
    ...
    try {
      ...
      this.logger.error('Failed to get sy, {
        _error error instanceof Error ? error.message : 'Unknown error,
      });
      return {
        success: false,
        _error error instanceof Error ? error.message : 'Failed to get system health',
        metadata: {
          executionTime: Date.now() - startTime,
          validationTime: 0,
          model: 'SystemHealth',
  ...
  async getLearningInsights(params: { agentName: string, }): Promise<ToolResult<object>> {
    ...
    try {
      ...
      this.logger.error('Failed to get learning insight, {
        _error error instanceof Error ? error.message : 'Unknown error,
      });
      return {
        success: false,
        _error error instanceof Error ? error.message : 'Failed to get learning insights',
        metadata: {
          executionTime: Date.now() - startTime,
          validationTime: 0,
          model: 'LearningInsights',
    ...
    try {
      ...
      switch (params.modelType.toLowerCase()) {
        ...
        default:
          return {
            success: false,
            _error `Unknown model type: ${params.modelType}`,
            metadata: {
              executionTime: Date.now() - startTime,
              validationTime: 0,
      ...
      return {
        ...
        data: validationResult.data,
        _error validationResult.isValid ? undefined : validationResult.errors?.join(', '),
        warnings: validationResult.warnings,
        metadata: {
          executionTime: Date.now() - startTime,
          validationTime: Date.now() - startTime,
      ...
      return {
        success: false,
        _error error instanceof Error ? error.message : 'Validation failed',
        metadata: {
          executionTime: Date.now() - startTime,
          validationTime: 0,
          model: params.modelType,
  ...
  serializeData(params: {
    ...
    try {
      ...
      return {
        success: false,
        _error error instanceof Error ? error.message : 'Serialization failed',
        metadata: {
          executionTime: Date.now() - startTime,
          validationTime: 0,
          model: 'Serialization',
