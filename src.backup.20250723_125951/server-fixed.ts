...
console.log(`[OK] [STARTUP] Completed: Initial imports (${Date.now() - startTime}ms)`);
...
try {
  ...
  console.log(
    `[ERROR] [STARTUP] Failed: Security middleware application - ${error instanceof Error ? error.message : String(error)}`
  );
  logger.error(
...
app.post('/api/assistant/create-tool', async (req, res) => {
  try {
    ...
    if (req.body.save) {
      const { data, error } = await supabase.from('ai_custom_tools').insert(tool).select().single();
    ...
    try {
      ...
      if (health.healthy) {
        ...
        try {
          ...
          logger.error(
            '⚠️ Redis performance middleware upgrade failed, keeping fallback',
...
server.listen(port, async () => {
  ...
  logger.info(`[OK] Universal AI Tools Service running on port ${port}`);
