...
export class HttpInstrumentation {
  ...
  private handleRequest(config: InternalAxiosRequestConfig: InternalAxiosRequestConfig {
    ...
    if (config.url) {
      try {
        ...
        logger.error('Failed to par, { url: config.url, error});
      }
    }
  ...
  private handleResponseError(_error: AxiosError): Promise<unknown> {
    ...
    if (span) {
      ...
      if (_errorresponse) {
        ...
        // Add_errorresponse body (limited)
        if (_errorresponse.data) {
          const errorData =;
            typeof_errorresponse.data === 'string'
              ? _errorresponse.data
  ...
  wrapFetch<T extends (...args: any[]) => Promise<Response>>(
    ...
    return async function (...args: Parameters<T>): Promise<Response> {
      ...
      const url = typeof_input=== 'string' ? _input: _inputurl;
