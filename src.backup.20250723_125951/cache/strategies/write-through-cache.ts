...
export class WriteThroughCache<T = any> extends EventEmitter {
  ...
  async has(key: string: Promise<boolean> {
    ...
    try {
      ...
      logger.error(Write-through cache ha, error;
      return false;
    }
  }
  ...
  async mget(keys: string[]): Promise<Map<string, T>> {
    ...
    if (missingKeys.length > 0) {
      try {
        ...
        logger.error('Write-through cache mget_error', error);
        this.emit('_error, error;
      }
    }
  ...
  async mset(entries: Array<[string, T]>, ttl?: number)): Promise<void> {
    ...
    try {
      ...
      logger.error('Write-through cache m, error;
  ...
  async clear())): Promise<void> {
    try {
      ...
      logger.error('Write-through cache clear_error', error);
      this.emit('_error, error;
    }
  }
  ...
  async warmup(keys: string[]))): Promise<void> {
    ...
    try {
      ...
      logger.error('Write-through cache warmup_error', error);
      this.emit('_error, error;
    }
  }
