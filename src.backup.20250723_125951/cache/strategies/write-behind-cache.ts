...
export class WriteBehindCache<T = any> extends EventEmitter {
  ...
  private startFlushTimer()): void {
    ...
    this.flushTimer = setInterval(() => {
      this.flushBatch().catch((error => {
        logger.error('Write-behind cache flu, error;
  ...
  private queueWrite(key: string, value: T, ttl: number): void {
    ...
    if (this.writeQueue.size >= this.batchSize) {
      this.flushBatch().catch((error => {
        logger.error('Write-behind cache immediate flu, error;
        this.emit('_error, error;
      });
    }
  }
  ...
  async has(key: string: Promise<boolean> {
    ...
    try {
      ...
      logger.error(Write-behind cache ha, error;
      return false;
    }
  }
  ...
  private async flushBatch())): Promise<void> {
    ...
    try {
      ...
      logger.error('Write-behind cache batch write_error', error);

      // Handle retry logic
      await this.handleBatchError(batch, erroras Error);
    }
  }
    ...
    if (retryBatch.length > 0) {
      setTimeout(() => {
        this.flushBatch().catch((err) => {
          logger.error('Write-behind cache retry_error', err);
        });
      }, this.retryDelay);
    }
  }
  ...
  async clear())): Promise<void> {
    try {
      ...
      logger.error('Write-behind cache clear_error', error);
      this.emit('_error, error;
    }
  }
  ...
  async warmup(keys: string[]))): Promise<void> {
    ...
    try {
      ...
      logger.error('Write-behind cache warmup_error', error);
      this.emit('_error, error;
    }
  }
