...
export function createMCPRouter(supabase: SupabaseClient, mcpService: MCPServerService {
  ...
  router.get('/agents', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get MCP agent, LogContext.API, { error });
      res.status(500).json({
        success: false,
  ...
  router.get('/agents/:agentId', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get MCP agent', {
        error,
  ...
  router.post('/agents/:agentId/keys', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to store MCP agent key, LogContext.SECURITY, {
        error,
        agentId: req.params.agentId,
      });
      res.status(500).json({
  ...
  router.post('/agents/:agentId/execute', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to execute MCP agent action', {
        error,
        agentId: req.params.agentId,
  ...
  router.get('/status', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get MCP statu, LogContext.API, { error });
      res.status(500).json({
        success: false,
  ...
  router.put('/agents/:agentId', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to update MCP agent', {
        error,
        agentId: req.params.agentId,
      });
      res.status(500).json({
  ...
  router.delete('/agents/:agentId', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to delete MCP agent', {
        error,
        agentId: req.params.agentId,
      });
      res.status(500).json({
