...
export function MemoryRouter(supabase: SupabaseClient) {
  ...
  router.post('/', validateMemoryStore, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (error) {
        logger.error('Failed to , LogContext.API, {
          error: error.message,
          memoryData,
        });
        return sendError(
      ...
      logger.error('Store memory error', {
      ...
      sendError(
        ...
        'An unexpected_erroroccurred while storing memory',
        500,
        error.message
      );
    }
  ...
  router.get('/', async (req: AuthenticatedRequest, res => {
    try {
      ...
      if (error) {
        logger.error('Failed to retrieve memorie, LogContext.API, { error: error.message });
        return sendError(
          res,
          'MEMORY_STORAGE_ERROR' as ErrorCode,
      ...
      logger.error('Retrieve memorie, LogContext.API, {
        error: error.message,
        stack: error.stack,
      });
      sendError(
        res,
        'INTERNAL_SERVER_ERROR' as ErrorCode,
        'An unexpected_erroroccurred while retrieving memories',
        500,
        error.message
      );
    }
  ...
  router.post('/search', validateMemorySearch, async (req: AuthenticatedRequest, res => {
    ...
    try {
      ...
      logger.error('Search memorie, LogContext.API, {
        error: error instanceof Error ? error.message : String(error),
      });
      res.status(400).json({
        success: false,
  ...
  router.put('/:id/importance', async (req: AuthenticatedRequest, res => {
    try {
      ...
      logger.error('Update memory importance error', {
        error: error instanceof Error ? error.message : String(error),
      });
      res.status(400).json({ error: error instanceof Error ? error.message : 'Failed to update memory importance' });
    }
  });
