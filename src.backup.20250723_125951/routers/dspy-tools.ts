...
export function createDSPyToolsRouter() {
  ...
  router.get('/tools', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get DSPy tool, LogContext.API, { error });
      res.status(500).json({
        success: false,
    ...
    try {
      ...
      if (!tool_name || !input) {
        return res.status(400).json({
          success: false,
          error: 'tool_name and input are required',
        });
      }
      ...
      logger.error('Failed to execute DSPy tool', { error, tool: req.body.tool_name });
      res.status(500).json({
        success: false,
  ...
  router.post('/pipeline', async (req: Request, res: Response) => {
    try {
      ...
      if (!Array.isArray(tools) || !input) {
        return res.status(400).json({
          success: false,
          error: 'tools array and input are required',
        });
      }
      ...
      logger.error('Failed to execute DSPy pipeline', { error, tools: req.body.tools });
      res.status(500).json({
        success: false,
  ...
  router.post('/recommend', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get tool recommendation, LogContext.DSPY, { error, task: req.body.task });
      res.status(500).json({
        success: false,
  ...
  router.get('/categories', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get DSPy categorie, LogContext.DSPY, { error });
      res.status(500).json({
        success: false,
