...
class FileSystemRouterClass {
  ...
  private sanitizePath(inputPath: string | null {
    try {
      ...
      for (const_patternof this.blockedPatterns) {
        if (_patterntest(resolvedPath)) {
          logger.warn('Access to blocked_patternattempted', {
            path: resolvedPath,
            _pattern_patterntoString(),
          });
          return null;
        }
      }
  ...
  private async logOperation(
    ...
    try {
      ...
      logger.error('Failed to log file sy, error;
    }
  }
  ...
  private handleWebSocket(ws: WebSocket, req: Request): void {
    ...
    ws.on('message', async (message): string => {
      try {
        ...
        logger.error('WebSocket mes, error;
        ws.send(
          JSON.stringify({
            type: '_error,
