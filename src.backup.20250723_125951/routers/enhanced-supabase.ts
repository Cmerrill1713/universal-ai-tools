...
router.post('/process/:fileId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('File proces, LogContext.API, { error });
...
router.get('/process/status/:jobId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to get job statu, LogContext.API, { error });
...
router.post('/search/semantic', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Semantic , LogContext.DATABASE, { error });
...
router.post('/search/hybrid', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Hybrid , LogContext.DATABASE, { error });
...
router.post('/memory', async (req: Request, res: Response) => {
  try {
    ...
    if (!type || !content {)
      return res.status(400).json({ error: 'Type and contentrequired' });
    }

    // Generate embedding
    const embedding = await generateEmbedding(content;

    const result = await enhancedSupabase.storeEmbedding('memory', content embedding, {
      ...metadata,
      type,
      importance,
      userId: req.user?.id,
    ...
    logger.error('Failed to , LogContext.MEMORY, { error });
...
router.post('/memory/search', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Memory , LogContext.MEMORY, { error });
...
router.post('/realtime/subscribe', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to setup realtime sub, LogContext.WEBSOCKET, { error });
...
router.post('/realtime/broadcast', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to broadcast mes, LogContext.WEBSOCKET, { error });
