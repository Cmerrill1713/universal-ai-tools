...
export function DSPyWidgetsRouter(supabase: SupabaseClient) {
   ...
   */
  router.post('/improve', async (req: any, res: Response) => {
    try {
      ...
      logger.error('Widget improvement error', {
        error: error instanceof Error ? error.message : String(error),
      });
   ...
   */
  router.get('/progress/:widgetId', async (req: any, res: Response) => {
    try {
      ...
      logger.error('Progress query error', LogContext.API, {
        error: error instanceof Error ? error.message : String(error),
      });
   ...
   */
  router.get('/:widgetId', async (req: any, res: Response) => {
    try {
      const { widgetId } = req.params;
      ...
      logger.error('Widget retrieval error', {
        error: error instanceof Error ? error.message : String(error),
      });
   ...
   */
  router.delete('/:widgetId', async (req: any, res: Response) => {
    try {
      const { widgetId } = req.params;
   ...
   */
  router.get('/status/active', async (req: any, res: Response) => {
    try {
      ...
      logger.error('Active generation error', LogContext.API, {
        error: error instanceof Error ? error.message : String(error),
      });
   ...
   */
  router.get('/status/health', async (req: any, res: Response) => {
    try {
