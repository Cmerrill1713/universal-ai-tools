...
router.get('/summary', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to fetch agent performance , LogContext.API, { error });
    res.status(500).json({
      success: false,
...
router.get('/metrics', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      logger.error('Failed to fetch performance metric, LogContext.API, { error });
      res.status(500).json({
        success: false,
...
router.get('/trends', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      logger.error('Failed to fetch performance trend, LogContext.API, { error });
      res.status(500).json({
        success: false,
...
router.get('/alerts', async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      logger.error('Failed to fetch performance alert, LogContext.API, { error });
      res.status(500).json({
        success: false,
...
router.post('/alerts/:alertId/resolve', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to re, LogContext.API, { error });
    res.status(500).json({
      success: false,
...
router.get('/compare', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to compare agent, LogContext.API, { error });
    res.status(500).json({
      success: false,
...
router.get('/benchmarks', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to fetch benchmark, LogContext.API, { error });
    res.status(500).json({
      success: false,
...
router.put('/benchmarks', async (req: Request, res: Response) => {
  try {
    ...
    if (!agent_type || !task_type || complexity_level === undefined || !expected_execution_time) {
      return res.status(400).json({
        success: false,
        error: 'Missing required fields: agent_type, task_type, complexity_level, expected_execution_time',
      });
    }
...
router.post('/aggregate', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to aggregate metric, LogContext.API, { error });
    res.status(500).json({
      success: false,
