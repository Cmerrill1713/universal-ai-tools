...
export function SpeechRouter(supabase: SupabaseClient) {
  ...
  router.post('/synthesize', async (req: any, res: Response) => {
    try {
  ...
  router.get('/voices', async (req: any, res: Response) => {
    try {
      ...
      logger.error('Error fetching voice, error;
      res.status(500).json({
  ...
  router.post('/configure-voice', async (req: any, res: Response) => {
    try {
      const schema = z.object({
  ...
  router.get('/history/:conversation_id', async (req: any, res: Response) => {
    try {
      ...
      const { data: history, error } = await supabase
        ...
        .or('content.ilike.User (voice):%,content.ilike.Assistant (voice):%')
      ...
      logger.error('Error fetching voice hi, error;
      res.status(500).json({
  ...
  router.get('/health', async (req: any, res: Response) => {
    try {
      ...
      logger.error('Error checking speech , error);
      res.status(500).json({
        success: false,
        status: 'unhealthy',
        error: {
  ...
  router.post('/test/kokoro/:voiceId', async (req: any, res: Response) => {
    try {
      ...
      logger.error('Kokoro voice te, error;
      res.status(500).json({
        success: false,
        error: {
  ...
  router.post('/admin/clear-cache', async (req: any, res: Response) => {
    try {
  ...
  router.post('/synthesize/retry', async (req: any, res: Response) => {
    try {
      const schema = z.object({
        text: z.string().min(1).max(5000),
        personality: z.enum(['sweet', 'shy', 'confident', 'caring', 'playful']).default('sweet'),
  ...
  router.post('/estimate-duration', async (req: any, res: Response) => {
    try {
      const schema = z.object({
        text: z.string().min(1).max(5000),
