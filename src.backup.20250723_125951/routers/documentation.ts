...
export function DocumentationRouter(supabase: SupabaseClient) {
  ...
  router.post('/search/snippets', validateRequest(SearchDocsSchema), async (req: any, res: Response) => {
    try {
  ...
  router.get('/supabase/features', validateRequest(GetFeatureDocsSchema), async (req: any, res: Response) => {
    try {
  ...
  router.post('/snippets', async (req: any, res: Response) => {
    try {
  ...
  router.get('/quickstart/:feature', async (req, res) => {
    try {
      ...
      logger.error('Error fetching quick, error;
      res.status(500).json({
        success: false,
        error: {
          code: 'FETCH_ERROR',
