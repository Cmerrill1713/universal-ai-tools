...
export function BackupRouter(supabase: SupabaseClient) {
  ...
  router.get('/help/aws-sdk', async (req: any, res: Response) => {
    try {
  ...
  // Create a new backup
  router.post('/create', validateRequest(CreateBackupSchema), async (req: any, res: Response) => {
    try {
  ...
  // List backups
  router.get('/list', validateRequest(ListBackupsSchema), async (req: any, res: Response) => {
    try {
      ...
      logger.error('Error listing backup', error);
      res.status(500).json({
        success: false,
        error: {
          code: 'LIST_ERROR',
  ...
  // Restore from backup
  router.post('/restore', validateRequest(RestoreBackupSchema), async (req: any, res: Response) => {
    try {
  ...
  // Create schedule
  router.post('/schedules', validateRequest(ScheduleBackupSchema), async (req: any, res: Response) => {
    try {
  ...
  router.post('/estimate', async (req, res) => {
    try {
      ...
      logger.error('Error estimating backup , error);
      res.status(500).json({
        success: false,
        error: {
          code: 'ESTIMATE_ERROR',
