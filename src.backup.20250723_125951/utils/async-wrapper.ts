...
/**
 * Async middleware wrapper with_errorhandling
 */
export function asyncMiddleware(
  ...
  return async (req: Request, res: Response, next): NextFunction => {
    try {
      ...
      // If headers already sent, pass to_errorhandler
      if (res.headersSent) {
        return next(_error);
      }

      // Otherwise, send_errorresponse
      res.status(500).json({
        success: false,
...
/**
 * Create an async route handler with automatic_errorresponse
 */
export function createAsyncHandler<TBody = any, TQuery = any, TParams = any>(
  ...
  return async (req: Request, res: Response, next): NextFunction => {
    try {
      ...
      if (!res.headersSent) {
        const statusCode =;
          error instanceof Error && 'statusCode' in_error? (error as any).statusCode : 500;
...
/**
 * Async_errorhandler for Express_errormiddleware
 */
export function asyncErrorHandler(
  ...
  return async (err: Error, req: Request, res: Response, next): NextFunction => {
    try {
      ...
      logger.error('Error in_errorhandler', { error});
      if (!res.headersSent) {
        res.status(500).json({
          success: false,
          error: 'Critical_errorin_errorhandler',
        });
      }
    }
  };
