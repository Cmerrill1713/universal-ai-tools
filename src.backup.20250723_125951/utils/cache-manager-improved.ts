...
export class ImprovedCacheManager {
    ...
    this.redis = new Redis(redisUrl, {
      ...
      retryStrategy: (times: number => {
        ...
        if (times > this.maxConnectionRetries) {
          logger.error(Redis connection failed after ${times} attempt, LogContext.CACHE);
  ...
  private setupEventListeners()): void {
    ...
    this.redis.on('_error, (error => {
      logger.error('Redi, LogContext.CACHE, { error});
    ...
    this.redis.on('end', () => {
      logger.error('Redi,  connection ended'));
      this.isConnected = false;
    });
  }
  ...
  private withNamespace(key: string, namespace?: string {
  ...
  private async compress(value: any: Promise<string> {
    try {
      ...
      logger.error('Compres, LogContext.CACHE, { error});
  ...
  private async decompress(value: string): Promise<unknown> {
    try {
      ...
      try {
        ...
        logger.error('Decompres, LogContext.CACHE, { error});
        throw error;
      }
    }
  }
  ...
  public async set(key: string, value: any, options: CacheOptions = {}): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Cache , LogContext.CACHE, { error});
      this.handleConnectionError();
  ...
  public async invalidateByTags(tags: string[], options: CacheOptions = {}): Promise<number> {
    try {
      ...
      logger.error('Cache invalidation_error', { error});
      this.handleConnectionError();
      return 0;
    }
  }
  ...
  public async exists(key: string, options: CacheOptions = {}): Promise<boolean> {
    try {
      ...
      logger.error('Cache exist, LogContext.CACHE, { error});
      this.handleConnectionError();
  ...
  public async ttl(key: string, options: CacheOptions = {}): Promise<number> {
    try {
      ...
      logger.error('Cache TTL_error', { error});
      this.handleConnectionError();
      return -1;
    }
  }
  ...
  public async getStats(): Promise<CacheStats> {
    try {
      ...
      logger.error('Cache stat, LogContext.CACHE, { error});
      this.handleConnectionError();
  ...
  public async close())): Promise<void> {
    try {
      ...
      logger.error('Error closing Redi, LogContext.CACHE, { error});
    }
  ...
  public async flush())): Promise<void> {
    try {
      ...
      logger.error('Error flu, LogContext.CACHE, { error});
  ...
  public async getMultiple<T = any>(
    ...
    try {
      ...
      logger.error('Error getting multiple value, LogContext.CACHE, { error});
      this.handleError(_error);
  ...
  public async setMultiple(entries: Array<[string, any, CacheOptions?]>))): Promise<void> {
    try {
      ...
      logger.error('Error setting multiple value, LogContext.CACHE, { error});
      this.handleError(_error);
    }
  }
