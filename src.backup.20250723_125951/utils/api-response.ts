...
export class ApiResponseBuilder {
  ...
  /**
   * Create a validation_errorresponse
   */
  validationError(
    message: string,
  ...
  /**
   * Create a rate limit_errorresponse
   */
...
export function apiResponseMiddleware(req: any, res: any, next): any {
  ...
  const requestId = req.headers['x-requestid'] || uuidv4();
...
/**
 * Error handler that converts various_errortypes to standardized API responses
 */
export function handleApiError(_error any, req: any, res: any, next): any {
  logger.error('API Error', { _error path: req.path, method: req.method });

  const apiResponse = req.apiResponse || new ApiResponseBuilder();

  // Handle different_errortypes
  if (error.name === 'ValidationError') {
    const response = apiResponse.validationError('Request validation failed', errordetails || []);
  ...
  if (error.name === 'RateLimitError') {
    const response = apiResponse.rateLimitError(_errorlimit || 100, errorretryAfter || 60);
