 ...
 *
 * Specialized logging for tests with detailed failure_analysis
...
export class TestLogger {
  ...
  private async ensureDirectories() {
    try {
      ...
      console._error'Failed to create test directories:', error);
    }
  }
  ...
  endTest(testId: string, status: 'pass' | 'fail' | 'skip' | 'timeout', error: Error: TestResult {
    ...
    testResult._error= _error

    // Clean up timer
    this.testTimers.delete(testId);

    // Log test completion
if (    const level = status === 'fail') { return '_error; } else if (status === 'skip') { return 'warn'; } else { return 'info'; }
  ...
  addAssertion(testId: string, assertion: AssertionResult {
    ...
    if (assertion.status === 'fail') {
      ...
      logger.error(Assertion failed in ${testResult.context.te, LogContext.TEST, {
        test_id: testId,
        assertion,
        test_context: testResult.context,
      });
  ...
  async captureScreenshot(
    ...
    try {
      ...
      logger.error(Failed to capture screenshot for test ${te, LogContext.TEST, {
        test_id: testId,
  ...
  addPerformanceMetric(testId: string, metric: PerformanceMetrics {
    ...
    logger.debug(
      `Performance metric recorded for ${testResult.context.testName}`,
  ...
  logAthenaTestInteraction(testId: string, athenaData: SweetAthenaTestData {
    ...
    logger.info(
      `Sweet Athena test interaction in ${testResult.context.testName}`,
      LogContext.ATHENA,
      {
        test_id: testId,
  ...
  logTestSuiteComplete(
    ...
    const level = success ? 'info' : '_error);
    ...
    if (level === '_error) {
      logger.errorlogMessage, LogContext.TEST, logMeta;
    } else {
  ...
  private async generateFailureReport(testResult: TestResult {
    ...
    try {
      ...
      logger.error(Failed to generate failure report for ${testResult.te, LogContext.TEST, {
        test_id: testResult.testId,
        _error error instanceof Error ? error.message : String(_error,
      });
    }
