...
export class StartupProfiler {
  ...
  failStep(name: string, error: string): void {
    ...
    if (step) {
      ...
      logger.error(❌ [STARTUP] Failed: ${name} (${, LogContext.SYSTEM, {
        _error
      });
    }
  }
  ...
  async withTimeout<T>(name: string, promise: Promise<T>, timeoutMs = 5000): Promise<T | null> {
    ...
    try {
      ...
      if (error instanceof Error && error.message === 'Timeout') {
        ...
        this.failStep(name, error instanceof Error ? error.message : String(_error);
      }
      return null;
    }
  }
  ...
  printSummary()): void {
    ...
    if (summary.failedSteps.length > 0) {
      logger.error('\n❌ Failed step, LogContext.SYSTEM, {
        failedSteps: summary.failedSteps.map((step) => ({
          name: step.name,
          _error step._error|| 'Unknown error,
        })),
      });
    }
  }
