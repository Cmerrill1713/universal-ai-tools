...
export async function withTimeout<T>(promise: Promise<T>, options: TimeoutOptions: Promise<T> {
  ...
  const timeoutPromise = new Promise<T>((_, reject => {
    setTimeout(() => {
      const_error= new Error(`${name} timed out after ${timeout}ms`);
      if (throwOnTimeout) {
        reject(error);
      } else {
...
export async function initializeWithTimeout<T>(
  ...
  try {
    ...
    if (options.critical) {
      logger.error(❌ Critical service ${, LogContext.SYSTEM, {
        _error errorMessage,
        duration,
      });
      throw error;
...
export async function initializeWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error instanceof Error ? error: new Error(String(_error);
