...
export class SafeMemorySystemWrapper {
  ...
  async initialize(): Promise<boolean> {
    ...
    this.config.logger.error('Memory system initialization failed after all retrie, '));
    return false;
  }
  ...
  async storeMemory(
    ...
    try {
      ...
      this.config.logger.error('Failed to , error);

      // If it's a connection_error try to reinitialize
  ...
  async searchMemories(options: any: Promise<any[]> {
    ...
    try {
      ...
      this.config.logger.error('Failed to search memorie, error;
