...
/**
 * Advanced access_patternlearning system
 */
export class AccessPatternLearner {
  ...
  async recordAccess(
    ...
    try {
      ...
      const { error} = await this.supabase.from('memory_access_patterns').insert(accessPattern);
      ...
      this.logger.error('Failed to record acces, error;
    }
  }
  ...
  async recordUserFeedback(
    ...
    try {
      // Find the most recent access_patternfor this memory and agent
      ...
      // Update the access_patternwith feedback
      await this.supabase
        .from('memory_access_patterns')
        .update({
  ...
  async calculateUtilityScore(
    ...
    try {
      ...
      if (hourlyAccesses > 0) {
        ...
        explanation.push(`Time_pattern +${(timeBoost * 100).toFixed(1)}% (active at this: hour`);
      }
  ...
  > {
    try {
      ...
      this.logger.error('Failed to re-rank result, error;
  ...
  private async updateAdaptiveWeights(
    ...
    try {
      ...
      this.logger.error('Failed to update adaptive weight, error;
    }
  }
  ...
  private async generateLearningInsights(agentName: string: Promise<LearningInsights> {
    try {
      ...
      this.logger.error('Failed to generate learning insight, error;
      return this.getDefaultInsights();
    }
  }
