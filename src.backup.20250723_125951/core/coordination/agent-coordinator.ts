...
export class AgentCoordinator extends EventEmitter {
  ...
  private async createPrimaryStrategy(
    ...
    strategy.agentRoles = [
      ...
      },
      {
        ...
        capabilities: ['Online research', 'Problem_analysis, 'Information gathering'],
      },
    ];
  ...
  private setupEventHandlers()): void {
    ...
    this.taskManager.on('task_failed', (task) => {
      logger.error(❌ Task failed: ${ta, k.id}`));`
  ...
  private async handleResourceRequest(
    ...
    logger.info(`Handling resource requestfrom ${fromAgent}`, request;`
    await this.messageBroker.sendMessage({
      sessionId: session.id,
      fromAgent: 'coordinator',
      toAgent: fromAgent,
  ...
  private async handleCoordinationRequest(
    ...
    logger.info(`Handling coordination requestfrom ${fromAgent}`, request;`
    await this.messageBroker.sendMessage({
      sessionId: session.id,
      fromAgent: 'coordinator',
      toAgent: fromAgent,
  ...
  private async initiateErrorRecovery(agentId: string, error: any)): Promise<void> {
    ...
    const recoveryPlan = await this.coordinateGroupFix(
      `Error recovery for agent ${agentId}: ${error.message || _error`,
      { agentId, error}
    );
  ...
  private async gatherPageInformation(agent: BrowserAgent): Promise<unknown> {
    ...
    try {
      ...
      return { error: error instanceof Error ? error.message : String(_error };
    }
  }
