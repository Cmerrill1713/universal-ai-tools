...
export class OnlineResearchAgent {
    ...
    try {
      ...
      const solutions = results.slice(0, 3).map((result) => {
        ...
        return {
          ...
          url: result.url,
          title: result.title,
          solution: result.contentsubstring(0, 1000), // First 1000 chars
    ...
    try {
      ...
      const solutions = results.slice(0, 3).map((result) => {
        ...
        const hasSolutionIndicator = solutionIndicators.some((indicator) =>;
          result.contenttoLowerCase().includes(indicator)
        );
        ...
        return {
          ...
          url: result.url,
          title: result.title,
          solution: result.contentsubstring(0, 800), // First 800 chars
    ...
    try {
      ...
      const solutions = results.slice(0, 3).map((result) => {
        ...
        return {
          ...
          url: result.url,
          title: result.title,
          solution: result.contentsubstring(0, 600), // First 600 chars
          confidence: Math.round(confidence),
        };
      });
      ...
      logger.error('Documentation , error);
      return [];
    }
  }
  ...
  private async searchDevCommunity(query: ResearchQuery: Promise<any[]> {
    try {
      ...
      logger.error('Dev community , error);
      return [];
    }
  }
  ...
  private async fallbackSearch(query: ResearchQuery: Promise<ResearchResult | null> {
    try {
      ...
      const solutions = results.slice(0, 5).map((result) => ({
        ...
        url: result.url,
        title: result.title,
        solution: result.contentsubstring(0, 800),
  ...
  async updateSuccessRate(solutionId: string, successful: boolean)): Promise<void> {
    try {
      ...
      logger.error('Failed to update succes, error;
    }
  }
