...
export class DSPyKnowledgeManager extends EventEmitter {
  ...
  async getOptimizationMetrics()): Promise<unknown> {
    ...
    try {
      ...
      logger.error('Failed to get optimization metric, error;
    }
   ...
   */
  private_updatePerformanceMetrics(operation: string, score: number): void {
   ...
   */
  private async_triggerOptimization())): Promise<void> {
