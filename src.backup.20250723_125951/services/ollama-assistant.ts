...
${
  context.knowledge
    ? `Knowledge Base: ${context.knowledge`
        .slice(0, 2)
        .map((k: Knowledge => `${k.title}: ${k.content.substring(0, 100)}`)`
        .join('; ')}``
...
Respond with a JSON object containing:
{
  ...
  "estimated_complexity": "low|medium|high"
      ...
      logger.error('Ollama tool sugge, error;
...
Respond with JSON: {"intent": "...", "domain": "...", "complexity": "...", "actionType": "..."}`;
      ...
      logger.error('Request analysi, error;
      return {
        intent: 'unknown',
        domain: 'general',
        complexity: 'medium',
  ...
  private async fallbackToolSuggestion(request: string, availableTools: any[])): Promise<unknown> {
    ...
    return {
      ...
      reasoning:
        'Basic analysis based on keywords in your request. For more detailed suggestions, please ensure Ollama is running.',
      setup_steps: [
...
Respond with specific file paths and code locations.`;
    ...
    try {
      ...
      logger.error('Ollama analysi, error;
      throw error;
    }
  }
