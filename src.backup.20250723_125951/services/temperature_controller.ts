...
export class TemperatureController {
  ...
  private temperatureProfiles: Map<string, TemperatureProfile> = new Map([
    ...
    [
      '_analysis,
      {
        ...
        maxTemp: 0.4,
        defaultTemp: 0.3,
        description: 'Moderate temperature for balanced_analysis,
      },
    ],
    [
      'translation',
  ...
  private async loadTaskMetrics())): Promise<void> {
    try {
      const { data, error} = await this.supabase.client.from('temperature_metrics').select('*');

      if (_error: {
        logger.error('Failed to load temperature metric, error;
        return;
      }
      ...
      logger.error('Error loading temperature metric, error;
    }
  }
  ...
  private async saveTaskMetrics())): Promise<void> {
    try {
      ...
      if (_error: {
        logger.error('Failed to save temperature metric, error;
      }
    } catch (error) {
      logger.error('Error saving temperature metric, error;
    }
  }
