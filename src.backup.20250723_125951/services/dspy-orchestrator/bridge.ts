...
export class DSPyBridge extends EventEmitter {
  ...
  constructor() {
    ...
    this.startupPromise = this.startPythonService().catch((error => {
      logger.error('Failed to start DSPy , LogContext.DSPY, {
    ...
    try {
      ...
      this.ws.on('message', (data): string => {
        try {
          ...
          logger.error('Failed to parse DSPy respon, LogContext.DSPY, {
            _error error instanceof Error ? error.message : String(_error,
          });
        }
      });
      ...
      this.ws.on('_error, (error => {
        logger.error('DSPy WebSocket_error', {
          _error error instanceof Error ? error.message : String(_error,
        });
      });
      ...
      logger.error('Failed to connect to DSPy , LogContext.DSPY, {
        _error error instanceof Error ? error.message : String(_error,
      });
