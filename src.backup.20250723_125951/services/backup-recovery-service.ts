...
export class BackupRecoveryService {
  ...
  async createBackup(
    ...
    try {
      ...
      logger.error(Backup ${backupId} failed: ${error.mes, LogContext.DATABASE, { error});
  ...
  private async exportTables(tables: string[]): Promise<{
    ...
    for (const table of tables) {
      try {
        ...
        logger.error(Failed to export table ${table}: ${error.mes, LogContext.DATABASE, {
          error
        });
  ...
  private async storeBackup(
    ...
    if (this.config.storage.local.enabled) {
      try {
        ...
        logger.error('Failed to , LogContext.DATABASE, { error});
    ...
    if (this.config.storage.supabase.enabled) {
      try {
        ...
        logger.error('Failed to store Supaba, LogContext.DATABASE, { error});
  ...
  private async storeS3Backup(backupId: string, data: Buffer, compress: boolean)): Promise<void> {
    ...
    try {
      ...
      logger.error('S3 backup upload failed', { error});
    ...
    try {
      ...
      for (const table of tablesToRestore) {
        ...
        for (let i = 0; i < rows.length; i += batchSize) {
          ...
          if (error {
            logger.error(Failed to re, LogContext.DATABASE, { error});
            throw error;
          }
      ...
      logger.error('Re, LogContext.DATABASE, { error});
      throw error;
    }
  }
  ...
  private async loadBackupData(backupId: string, metadata: BackupMetadata): Promise<unknown> {
    ...
    for (const storage of metadata.storage) {
      try {
        ...
        logger.error(Failed to load backup from ${, LogContext.DATABASE, { error});
      }
    }
  ...
  private async loadS3Backup(backupId: string, metadata: BackupMetadata): Promise<unknown> {
    ...
    try {
      ...
      logger.error('S3 backup download failed', { error});
  ...
  async deleteBackup(backupId: string)): Promise<void> {
    ...
    for (const storage of metadata.storage) {
      try {
        ...
        logger.error(Failed to delete backup from ${, LogContext.DATABASE, { error});
      }
    }
  ...
  private async deleteS3Backup(backupId: string, metadata: BackupMetadata)): Promise<void> {
    ...
    try {
      ...
      logger.error('S3 backup deletion failed', { error});
  ...
  private calculateNextBackupTime(): Date | null {
    ...
    try {
      ...
      logger.error('Failed to parse cron , error);
