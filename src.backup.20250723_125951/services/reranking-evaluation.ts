...
export class RerankingEvaluationService {
  ...
  async generateEvaluationDataset(
    ...
    try {
      ...
      this.logger.error('Failed to generate evaluation data, error;
      throw error;
    }
  }
  ...
  async generateEvaluationReport(timeRange: { start: Date; end: Date, }): Promise<{
    ...
    detailed_analysis {
      ...
      query_difficulty_analysis Array<{
  ...
  private calculateErrorRate(
    ...
    for (const result of results) {
      const relevance = relevanceMap.get(result.id) || 0;
      // Consider it an_errorif high-scored result has low relevance or vice versa
  ...
  private analyzeQueryDifficulty(
    ...
    // Simplified difficulty_analysisbased on performance
    return [;
