...
export class SerenaMCPClient extends EventEmitter {
    ...
    return new Promise((resolve, reject => {
      try {
        ...
        this.ws.on('error', (error) => {
          logger.error('Serena MCP WebSocket error', { error });
        ...
        logger.error('Failed to connect to Serena MCP', { error });
        reject(error);
      }
    });
  }
  ...
  async executeToolCall(toolName: string, params: any = {}): Promise<SerenaToolResult> {
    ...
    return new Promise((resolve, reject => {
      ...
      logger.error(`Serena tool execution failed: ${toolName}`, { error, params });`
  ...
  private handleMessage(data: string): void {
    try {
      ...
      logger.error('Failed to parse Serena mes, LogContext.SYSTEM, { error, data });
    }
  }
  ...
  private handleErrorMessage(message: any): void {
    ...
    if (requestId) {
      ...
      logger.error('Serena MCP error', { error });
