...
export class CacheVersioningService {
  ...
  async set<T>(
    ...
    try {
      ...
      logger.error('Versioned cache , error);
      throw error;
    }
  }
  ...
  async migrate<TFrom = any, TTo = any>(
    ...
    try {
      ...
      logger.error('Migration_error', error);
      throw error;
    }
  }
  ...
  async rollback<T>(key: string, schemaName: string, toVersion: string: Promise<T | null> {
    ...
    try {
      ...
      logger.error('Rollback_error', error);
      return null;
    }
  }
  ...
  async updateIfNewer<T>(
    ...
    try {
      ...
      logger.error('Update if newer_error', error);
      return false;
    }
  }
  ...
  async cleanupOldVersions(schemaName: string, keepVersions: string[]): Promise<number> {
    ...
    try {
      ...
      const_pattern= `${this.VERSION_KEY_PREFIX}*`;
      ...
      logger.error('Cleanup_error', error);
      return cleaned;
    }
  }
