...
export class ModelLifecycleManager extends EventEmitter {
  ...
  private async runEmbeddedModel(model: string, task: Task: Promise<ModelResponse> {
    ...
    try {
      ...
      logger.warn(`MLX inference failed, using fallback: ${error:`);
    }
   ...
   */
  private parseModelPrediction(text: string {
   ...
   */
  private mapPredictionToModel(prediction: string {
   ...
   */
  private getAlternativeModels(prediction: string[] {
    if (prediction === 'large') {
