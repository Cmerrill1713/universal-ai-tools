...
export class DatabasePerformanceMonitor extends EventEmitter {
  ...
  async start(): Promise<void> {
    if (this.isStarted) {
      logger.warn('Database performance monitor already started', LogContext.DATABASE);
      return;
    }

    if (!this.config.enabled) {
      logger.info('Database performance monitoring disabled', LogContext.DATABASE);
      return;
    }

    try {
      logger.info('Starting database performance monitor', LogContext.DATABASE, { config: this.config });
      ...
      logger.info('Database performance monitor started successfully', LogContext.DATABASE);

    } catch (error) {
      logger.error('Failed to start database performance monitor', LogContext.DATABASE, { error });
      throw error;
    }
  }
  ...
  async stop(): Promise<void> {
    if (!this.isStarted) {
      logger.warn('Database performance monitor not started', LogContext.DATABASE);
      return;
    }

    try {
      logger.info('Stopping database performance monitor', LogContext.DATABASE);
      ...
      logger.info('Database performance monitor stopped successfully', LogContext.DATABASE);

    } catch (error) {
      logger.error('Error stopping database performance monitor', LogContext.DATABASE, { error });
      throw error;
    }
  }
  ...
  trackQuery(
    ...
    logger.debug('Query tracked', LogContext.DATABASE, {
      query_id: queryId,
  ...
  startTransaction(options: {
    ...
    logger.debug('Transaction started', LogContext.DATABASE, {
  ...
  endTransaction(
    ...
    if (!transaction) {
      logger.warn('Transaction not found', LogContext.DATABASE, { transaction_id: transactionId });
      return;
    }
    ...
    logger.debug('Transaction ended', LogContext.DATABASE, {
      transaction_id: transactionId,
      status,
  ...
  async getDatabaseHealth(): Promise<DatabaseHealth> {
    ...
    const slowQueries = recentQueries.filter(q => q.executionTime > this.config.slowQueryThreshold).length;
    ...
    const cacheHitRatio = recentQueries.length > 0
  ...
  generateReport(durationMinutes = 60): DatabaseReport {
    ...
    const averageQueryTime = queries.length > 0
      ...
      : 0;
    const slowQueries = queries.filter(q => q.executionTime > this.config.slowQueryThreshold).length;
    ...
    const connectionMetrics = {
      averageUtilization: recentConnections.length > 0
        ? recentConnections.reduce((sum, c) => sum + c.connectionUtilization, 0) / recentConnections.length
        : 0,
      peakConnections: recentConnections.length > 0
    ...
    const transactionMetrics = {
      averageDuration: completedTransactions.length > 0
        ...
        : 0,
      abortRate: transactions.length > 0
  ...
  private setupQueryInterception(): void {
    ...
    logger.info('Query interception setup completed', LogContext.DATABASE);
  }
  ...
  private async collectMetrics(): Promise<void> {
    try {
      ...
      logger.error('Error collecting database metrics', LogContext.DATABASE, { error });
    }
  }
  ...
  private handleSlowQuery(query: QueryMetrics): void {
    logger.warn('Slow query detected', LogContext.DATABASE, {
      query_id: query.id,
    ...
    // Check if this query pattern is frequently slow
    const recentSimilarQueries = this.queryMetrics
      .filter(q => 
        q.queryHash === query.queryHash && 
        q.timestamp > new Date(Date.now() - 3600000) // Last hour
      );

    const slowCount = recentSimilarQueries.filter(q => q.executionTime > this.config.slowQueryThreshold).length;
    ...
    if (slowPercentage > 50 && recentSimilarQueries.length > 5) {
      this.emit('slowQueryPattern', {
        ...
        averageTime: recentSimilarQueries.reduce((sum, q) => sum + q.executionTime, 0) / recentSimilarQueries.length
      });
    }
  }
  ...
  private calculateHealthScore(metrics: {
    ...
    const connectionScore = Math.max(0, 100 - (metrics.connectionUtilization / 100) * 100 - metrics.connectionErrors * 5);
    
    // Resource utilization score (0-100)
    const resourceScore = metrics.cacheHitRatio;
    
    // Concurrency score (0-100)
    const concurrencyScore = Math.max(0, 100 - (metrics.activeTransactions / thresholds.activeTransactions) * 50 - metrics.lockContention * 10);
  ...
  private generateRecommendations(data: {
    ...
    const slowQueries = data.queries.filter(q => q.executionTime > this.config.slowQueryThreshold);
    if (slowQueries.length > 0) {
      ...
      if (tablesWithSlowQueries.length > 0) {
        recommendations.push(`Consider adding indexes to tables: ${tablesWithSlowQueries.join(', ')}`);
      }
    }

    // Connection pool recommendations
    const avgConnectionUtil = data.connectionMetrics.length > 0
      ? data.connectionMetrics.reduce((sum, c) => sum + c.connectionUtilization, 0) / data.connectionMetrics.length
      : 0;
    ...
    if (longTransactions.length > 0) {
      recommendations.push(`Review ${longTransactions.length} long-running transactions for optimization`);
    }
