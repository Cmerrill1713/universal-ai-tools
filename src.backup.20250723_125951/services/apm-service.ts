...
export class APMService extends EventEmitter {
  ...
  async start())): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to start APM , LogContext.PERFORMANCE, { error});
      throw error;
    }
  }
  ...
  async stop())): Promise<void> {
    ...
    try {
      ...
      logger.error('Error stopping APM , LogContext.PERFORMANCE, { error});
      throw error;
    }
  }
  ...
  recordError(
    ...
    logger.error('APM_errorrecorded', {
      error_id: errorId,
      transaction_id: transactionId,
      span_id: spanId,
      fingerprint,
  ...
  private async flushMetrics())): Promise<void> {
    try {
      ...
      logger.error('Failed to flush APM metric, LogContext.PERFORMANCE, { error});
    }
  }
  ...
  private generateErrorFingerprint(_error: Error: string {
    // Create a fingerprint based on_errortype and stack trace
    const stack = error.stack || '';
    const lines = stack.split('\n').slice(0, 3); // First 3 lines
    return Buffer.from(`${error.name}:${lines.join('')}`)`
      .toString('base64')
      .substring(0, 16);
  }
