...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    logger.info('OpenAI → Ollama request, LogContext.SYSTEM, {
      model,
    ...
    const response = {
      ...
      usage: {
        ...
        total_tokens: (prompt.split(' ').length + contentsplit(' ').length) * 2,
      },
    };

    res.json(response);
  } catch (error) {
    logger.error('OpenAI proxy_error, {
...
app.post('/v1/completions', async (req, res) => {
  try {
    ...
    logger.error('OpenAI completion, LogContext.API, {
      _error error instanceof Error ? error.message : String(_error,
    });
    res.status(500).json({
      _error: {
