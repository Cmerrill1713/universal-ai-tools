...
export class MemoryManager extends EventEmitter {
    ...
    if (level === 'critical') {
      ...
      this.memoryPressureCallbacks.forEach((callback) => {
        try {
          ...
          logger.error('Error in memory pres, LogContext.PERFORMANCE, { error});
        }
      });
  ...
  public async takeHeapSnapshot(): Promise<string> {
    ...
    try {
      ...
      logger.error('Failed to take heap snap, LogContext.PERFORMANCE, { error});
      throw error;
    }
  }
  ...
  public async storeAIMemory(context: string, response: any, metadata: any = {}))): Promise<void> {
    try {
      ...
      logger.error('Failed to , LogContext.PERFORMANCE, { error});
    }
  }
  ...
  public retrieveAIMemory(context: string: any | null {
    try {
      ...
      logger.error('Failed to retrieve AI memory', { error});
      return null;
    }
  }

  private generateMemoryKey(context: string {
