...
export interface ConversationContext {
  ...
  messageHistory: any[];
  relationshipDepth: 'new' | 'familiar' | 'trusted';
...
export class AthenaConversationEngine {
  ...
  async processConversation(request: ConversationRequest): Promise<AthenaResponse> {
    try {
      ...
      if (intent.confidence < 0.6) {
        ...
        // Return a simple response since personality is disabled
        return {
          content: "I'm here to help you build amazing things! What would you like to create today?",
          personalityMood: 'helpful',
   ...
   */
  private generateCreateTableSQL(tableName: string, columns: string[], purpose?: string {
