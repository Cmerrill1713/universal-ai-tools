...
export class ProductionCacheManager {
  ...
  async set<T = any>(key: string, value: T, options: CacheOptions = {}): Promise<boolean> {
    try {
      ...
      logger.error('Cache , LogContext.CACHE, {
        key,
        error error instanceof Error ? error.message : String(error,
      });
      return false;
  ...
  async invalidateByTags(tags: string[]): Promise<number> {
    try {
      ...
      logger.error('Cache invalidate by tag, LogContext.CACHE, {
        tags,
        error error instanceof Error ? error.message : String(error,
      });
      return 0;
  ...
  async getStats(): Promise<CacheStats> {
    try {
      ...
      logger.error('Cache stat, LogContext.CACHE, {
        error error instanceof Error ? error.message : String(error,
      });
      return { ...this.stats };
    }
...
// Lazy initialization
let_cacheManager: ProductionCacheManager | null = null;
