...
export class CircuitBreakerService extends EventEmitter {
  ...
  private setupEventListeners(breaker: OpossumCircuitBreaker<any, any>, name: string): void {
    ...
    breaker.on('failure', (error => {
      ...
      metrics.failures++;
      metrics.requests++;
      logger.warn(`Circuit breaker ${name}: Failure`, LogContext.SYSTEM, { _error error.message });`
    });
  ...
  /**
   * Wrap an HTTP requestwith circuit breaker
   */
  async httpRequest(
    name: string,
    ...
    const breaker = this.getBreaker(name, {
      fallback: () => {
        logger.warn(`HTTP requestfallback for ${name}`, LogContext.API);`
