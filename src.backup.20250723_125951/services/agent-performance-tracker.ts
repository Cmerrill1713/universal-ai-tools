...
export class AgentPerformanceTracker extends EventEmitter {
  ...
  private initializeBufferFlush()): void {
    ...
    this.bufferFlushInterval = setInterval(() => {
      this.flushMetricsBuffer().catch((error => {
        logger.error('Failed to flush metric, LogContext.PERFORMANCE, { error});
      });
    }, 5000);
  }
  ...
  private async flushMetricsBuffer())): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to flush metrics to databa, LogContext.PERFORMANCE, { error});
  ...
  private async aggregateMetrics(
    ...
    try {
      ...
      logger.error('Failed to aggregate metric, LogContext.PERFORMANCE, { period, error});
    }
  }
  ...
  async cleanupOldMetrics())): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to cleanup old metric, LogContext.PERFORMANCE, { error});
    }
  }
