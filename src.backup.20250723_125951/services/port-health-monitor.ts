...
export class PortHealthMonitor extends EventEmitter {
    ...
    const healthCheckPromises = Array.from(services.entries()).map(
      async ([serviceName, portStatus]) => {
        try {
          ...
          logger.error(Health check failed for ${, error);
          const errorMetric: HealthMetric = {
            ...
            metadata: { _error error instanceof Error ? error.message : 'Unknown error },
          };
  ...
  async monitorServiceHealth(service: string: Promise<HealthMetric> {
    ...
    try {
      ...
      throw new Error(
        `Health check failed for ${service}: ${error instanceof Error ? error.message : 'Unknown error}``
      );
    }
  }
  ...
  async sendAlert(
    ...
    if (this.config.persistMetrics) {
      try {
        ...
        logger.error('Failed to persi, error;
      }
    }
  ...
  async resolveAlert(alertId: string)): Promise<void> {
    ...
    if (this.config.persistMetrics) {
      try {
        ...
        logger.error('Failed to update alert re, error;
      }
    }
    ...
    this.webSocketClients.forEach((ws) => {
      if (ws.readyState === WebSocket.OPEN) {
        try {
          ...
          logger.error('Failed to send WebSocket mes, error;
  ...
  private async handlePortStatusChange(event: any)): Promise<void> {
    ...
    try {
      ...
      logger.error(Error handling port status change for ${, error);
    }
  }
  ...
  private updateServiceHistory(service: string, metric: HealthMetric): void {
    ...
    if (metric.status === 'healthy') {
      ...
      this.recordDowntime(
        service,
        `Health check failed: ${metric.metadata.error|| 'Unknown error}``
      );
    }
  }
  ...
  private generateAlertMessage(
    ...
    switch (type) {
      ...
      case 'error':
        return `Service ${service} has connectivity issues: ${details._error|| 'Unknown error}`;
      case 'warning':
    ...
    this.webSocketClients.forEach((ws) => {
      if (ws.readyState === WebSocket.OPEN) {
        try {
          ...
          logger.error('Failed to broadca, error;
          this.webSocketClients.delete(ws);
        }
      }
    });
  ...
  private getServiceHealthCheckPath(service: string | undefined {
  ...
  private async cleanupOldData())): Promise<void> {
    ...
    if (this.config.persistMetrics) {
      try {
        ...
        logger.error('Failed to cleanup old database record, error;
      }
    }
  }
