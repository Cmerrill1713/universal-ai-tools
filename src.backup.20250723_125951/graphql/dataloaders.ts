  ...
  const agentLoader = new DataLoader<UUID, Agent | null>(;
    async (agentIds: readonly UUID[]) => {
      try {
        ...
        if (_error: {
          logger.error('Agent loader_error, {
        ...
        logger.error('Agent loader batch_error, {
  ...
  const memoryLoader = new DataLoader<UUID, Memory | null>(;
    async (memoryIds: readonly UUID[]) => {
      try {
        ...
        if (_error: {
          logger.error('Memory loader_error, {
            _error error.message,
            details: error
          });
        ...
        logger.error('Memory loader batch_error, {
          _error error instanceof Error ? error.message : String(_error,
          details: error
        });
        return memoryIds.map(() => null);
  ...
  const knowledgeEntityLoader = new DataLoader<UUID, KnowledgeEntity | null>(;
    async (entityIds: readonly UUID[]) => {
      try {
        ...
        if (_error: {
          logger.error('Knowledge entity loader_error, {
            _error error.message,
            details: error
          });
        ...
        logger.error('Knowledge entity loader batch_error, {
          _error error instanceof Error ? error.message : String(_error,
          details: error
        });
        return entityIds.map(() => null);
  ...
  const knowledgeRelationshipLoader = new DataLoader<UUID, KnowledgeRelationship | null>(;
    async (relationshipIds: readonly UUID[]) => {
      try {
        ...
        if (_error: {
          logger.error('Knowledge relation, LogContext.DATABASE, {
            _error error.message,
            details: error
          });
  ...
  const agentMemoriesLoader = new DataLoader<UUID, Memory[]>(;
    async (agentIds: readonly UUID[]) => {
      try {
        ...
        if (_error: {
          logger.error('Agent memorie, LogContext.DATABASE, {
            _error error.message,
            details: error
          });
          return agentIds.map(() => []);
  ...
  const entityRelationshipsLoader = new DataLoader<UUID, KnowledgeRelationship[]>(;
    async (entityIds: readonly UUID[]) => {
      try {
        ...
        if (_error: {
          logger.error('Entity relationship, LogContext.DATABASE, {
            _error error.message,
            details: error
          });
...
export function primeDataLoaders(
  ...
  try {
    ...
    logger.error('Error priming DataLoader, LogContext.DATABASE, {
      _error error instanceof Error ? error.message : String(_error,
      details: error
    });
  }
