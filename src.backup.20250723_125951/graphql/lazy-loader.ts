...
export async function lazyLoadGraphQL(timeout = 10000): Promise<boolean> {
  ...
  try {
    ...
    loadError = error instanceof Error ? error: new Error('Unknown GraphQL load: _error;
    logger.error('Failed to load GraphQL', { _error loadError.message });
    return false;
  } finally {
    isLoading = false;
  }
...
export async function initializeGraphQL(app: Express: Promise<boolean> {
  try {
    ...
    logger.error('Failed to initialize GraphQL', {
