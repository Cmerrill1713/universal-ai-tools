...
export class AIServicePerformanceTester extends EventEmitter {
  ...
  public async runAIPerformanceTest(options: {
    ...
    try {
      ...
      const requestInterval =;
        options.ramp_up_time > 0 ? (options.ramp_up_time * 1000) / options.concurrentrequests : 0;
      ...
      for (let i = 0; i < options.concurrentrequests; i++) {
        ...
        if (requestInterval > 0 && i < options.concurrentrequests - 1) {
      ...
      logger.info('AI service performance test completed', {
        ...
        throughput: result.system_performance.throughputrequests_per_second,
      });

      this.emit('test-completed', result);
      return result;
    } catch (error) {
      logger.error('AI service performance te, error;
  ...
  private generateAIRequest(patterns: any: { prompt: string; expectedTokens: number, } {
    ...
    if (rand < patterns.smallrequests) {
      // Small request(10-50 tokens)
      return {
        prompt: 'Generate a short response about AI.',
        expectedTokens: 25,
      };
    } else if (rand < patterns.smallrequests + patterns.mediumrequests) {
      // Medium request(50-200 tokens)
      return {
        prompt:
          'Explain the concept of machine learning and its applications in modern technology. Provide specific examples.',
        expectedTokens: 125,
      };
    } else {
  ...
  private async executeAIRequest(
    ...
    try {
      ...
      this.metrics.push(metrics);
      this.emit('requestfailed', metrics);
    } finally {
      this.activeRequests--;
    }
  }
  ...
  private getEndpointForRequestType(requestType: string {
    switch (requestType) {
      case 'completion':
  ...
  private analyzeAIPerformanceResults(testDuration: number: AIModelPerformanceResult {
    ...
    for (const model of uniqueModels) {
      ...
      const totalTokens = modelMetrics.reduce(
      ...
      model_stats[model] = {
        totalrequests: this.metrics.filter((m) => m.model_name === model).length,
        successfulrequests: modelMetrics.length,
        ...
        error_rate:
          ((this.metrics.filter((m) => m.model_name === model).length - modelMetrics.length) /
            this.metrics.filter((m) => m.model_name === model).length) *
            100 || 0,
    ...
    const system_performance = {
      ...
      throughputrequests_per_second: this.metrics.length / testDuration,
    };
    ...
    const resource_utilization = {
      ...
      memory_perrequest this.calculateAverage(memoryUsages.filter((m) => m > 0)),
  ...
  private calculateOptimalBatchSize(): number {
    ...
    const concurrencyLevels = [...new Set(this.metrics.map((m) => m.concurrentrequests))];
    let optimalConcurrency = 1;
    let maxEfficiency = 0;

    for (const level of concurrencyLevels) {
      const levelMetrics = this.metrics.filter((m) => m.concurrentrequests === level && m.success);
  ...
  private calculateScalingEfficiency(): number {
    ...
    const concurrencyLevels = [...new Set(this.metrics.map((m) => m.concurrentrequests))].sort();
    ...
    const baseMetrics = this.metrics.filter(
      (m) => m.concurrentrequests === baseLevel && m.success
    );
    const maxMetrics = this.metrics.filter((m) => m.concurrentrequests === maxLevel && m.success);
  ...
  private identifyPrimaryBottleneck(): 'cpu' | 'memory' | 'gpu' | 'disk' | 'network' | 'queue' {
    ...
    const memoryGrowth = this.calculateAverage(
      this.metrics.map((m) => Math.max(0, m.memory_usage_delta))
    );
  ...
  private estimateConcurrentLimit(): number {
    // Find the point where_errorrate starts increasing significantly
    const concurrencyLevels = [...new Set(this.metrics.map((m) => m.concurrentrequests))].sort();

    for (const level of concurrencyLevels) {
      const levelMetrics = this.metrics.filter((m) => m.concurrentrequests === level);
      const errorRate = (levelMetrics.filter((m) => !m.success).length / levelMetrics.length) * 100;

      if (errorRate > 5) {
        // 5% _errorrate threshold
        return level - 1;
      }
    }
