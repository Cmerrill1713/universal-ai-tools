...
export abstract class EnhancedBaseAgent extends BaseAgent {
  ...
  protected async process(context: AgentContext: Promise<AgentResponse> {
    ...
    try {
      ...
      this.logger.error(Enhanced agent processing failed: ${(error as Error).mes, age}`));`
      return {
        success: false,
        data: null,
   ...
   * Helper methods
   */
  private categorizeRequest(request: string {
  ...
  private extractKeywordsFromRequest(request: string[] {
    return request;
      .toLowerCase()
      .split(/\W+/)
