...
export class ResourceManagerAgent extends EnhancedMemoryAgent {
  ...
  async processInput(input string, _context: AgentContext: Promise<PartialAgentResponse> {
    try {
      // Save_inputfor strategy selection
  ...
  private parseRequestType(input: string {
  ...
  private extractAllocationId(input: string | null {
    const match = _inputmatch(/alloc-[\w-]+/);
    return match ? match[0] : null;
  }

  private extractConsumerId(input: string {
  ...
  private extractQueryTopics(input: string[] {
    const topics = [];
  ...
  private extractTimeframe(input: string {
  ...
  private async generateQueryResponse(
    ...
    return {
      ...
      message: 'Resource_analysiscompleted',
      confidence: 0.9,
  ...
  private handleResourceError(
    ...
    console._error'Resource management_error', error);

    return {
      success: false,
      data: null,
      message: `Resource management_error ${error.message}`,
      confidence: 0,
