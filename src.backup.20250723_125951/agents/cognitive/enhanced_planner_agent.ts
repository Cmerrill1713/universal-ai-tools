...
export class EnhancedPlannerAgent extends EnhancedMemoryAgent {
  ...
  constructor(config?: Partial<AgentConfig>) {
    super({
      ...
      memoryConfig: {
        ...
        episodicMemoryLimit: 2000, // More episodes for_patternlearning
        enableLearning: true,
  ...
  private getBaseStepsForDomain(domain: string: PlanStep[] {
    const_pattern = this.planningPatterns.get(domain);
...
This memory-integrated approach ensures each plan builds upon accumulated wisdom while adapting to specific requirements.`;
  ...
  private extractDomain(userRequest: string {
  ...
  private getAppliedPatterns(domain: string[] {
    const_pattern= this.planningPatterns.get(domain);
    return_pattern? _patterncommonSteps.slice(0, 3) : [];
  }
