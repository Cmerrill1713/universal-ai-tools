 ...
 * Provides smart organization, duplicate detection, content_analysis and automated workflows
 */
...
export class FileManagerAgent extends BaseAgent {
  ...
  protected async onInitialize())): Promise<void> {
    ...
    // Initialize content_analysiscapabilities
  ...
  protected async process(_context: AgentContext & { memoryContext?: any }): Promise<AgentResponse> {
    ...
    try {
      ...
      this.logger.error('FileManagerAgent proces, error;
      return {
        success: false,
        data: null,
   ...
   */
  private async parseFileIntent(request: string): Promise<unknown> {
...
Respond with JSON: {
    ...
    try {
      ...
      for (const operation of organizationPlan) {
        try {
          ...
          errors.push(
            `Failed to ${operation.type} ${operation.source}: ${(error as Error).message}``
          );
        }
      }
    ...
    for (const filePath of files) {
      try {
        ...
        if (_analysis {
          ...
          if (_analysistopics) {
            for (const topic of_analysistopics) {
              insights.keyTopics.set(topic, (insights.keyTopics.get(topic) || 0) + 1);
            }
          }
        }
      } catch (error) {
        this.logger.error(Analysi, (error as Error).message);
      }
    }
  ...
  private async loadOrganizationRules())): Promise<void> {
    try {
      ...
      if (data && data.length > 0) {
        this.organizationRules = data[0].contentrules || [];
      } else {
        // Set default organization rules
        this.organizationRules = this.getDefaultOrganizationRules();
      }
    } catch (error) {
      this.logger.error('Failed to load organization rule, error;
      this.organizationRules = this.getDefaultOrganizationRules();
    }
  }
  ...
  private async handleGeneralFileQuery(request: string): Promise<unknown> {
