...
export class WebScraperAgent extends BaseAgent {
  ...
  constructor(supabase: SupabaseClient) {
    const config: AgentConfig = {
      ...
      capabilities: [
        ...
        {
          name: 'api_request',
          description: 'Make intelligent API requests with authentication and_errorhandling',
          inputSchema: {
            type: 'object',
            properties: {
              url: { type: 'string' },
   ...
   */
  private async parseScrapingIntent(request: string): Promise<unknown> {
...
Return as JSON with clear structure.`;
  ...
  private extractText(html: string, selector?: string {
  ...
  private extractHTML(html: string, selector?: string {
  ...
  private async storeAPIResponse(
    ...
    try {
      ...
      this.logger.error('Failed to store API respon, error;
    }
  }
  ...
  private async handleGeneralWebQuery(request: string): Promise<unknown> {
