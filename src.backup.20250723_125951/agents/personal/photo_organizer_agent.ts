...
export class PhotoOrganizerAgent extends BaseAgent {
    ...
    const config: AgentConfig = {
      ...
      capabilities: [
        ...
        },
        {
          ...
          description: 'Create smart photo albums based on ML_analysis,
          inputSchema: {
            type: 'object',
            properties: {
  ...
  protected async process(_context: AgentContext & { memoryContext?: any }): Promise<AgentResponse> {
    ...
    try {
      ...
      this.logger.error('PhotoOrganizerAgent proces, error;
      return {
        success: false,
        data: null,
...
`;
   ...
   */
  private async parsePhotoIntent(request: string): Promise<unknown> {
...
`;
      ...
      this.logger.error('Face detection , error);
      return null;
    }
  }
  ...
  private async handleGeneralPhotoQuery(request: string): Promise<unknown> {
