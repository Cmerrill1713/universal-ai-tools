...
export class CalendarAgent extends BaseAgent {
  ...
  protected async process(_context: AgentContext & { memoryContext?: any }): Promise<AgentResponse> {
    ...
    try {
      ...
      this.logger.error('CalendarAgent proces, error;
      return {
        success: false,
        data: null,
   ...
   */
  private async parseCalendarIntent(request: string): Promise<unknown> {
...
Respond with JSON: {
   ...
   */
  private async createEventFromNaturalLanguage(request string, intent: any): Promise<unknown> {
  ...
  private async createCalendarEvent(event: CalendarEvent: Promise<CalendarEvent> {
    try {
      ...
      if (result.includes('_error')) {
        throw new Error(`AppleScript_error ${result}`);
      }
  ...
  private parseAppleScriptEventResult(scriptResult: string: CalendarEvent[] {
    ...
    try {
      ...
      this.logger.error('Failed to parse AppleScript re, error;
    }
  ...
  private parseIndividualEvent(eventStr: string: CalendarEvent | null {
    try {
      ...
      this.logger.error('Error par, error;
      return null;
    }
  }
...
Respond with JSON: {
  ...
  private async loadCalendarPreferences())): Promise<void> {
    try {
      ...
      this.logger.error('Failed to load calendar preference, error;
    }
  }
  ...
  private extractEventTitle(request: string {
  ...
  private buildCalendarReasoning(intent: any, result: any: string {
    return `Processed calendar ${intent.action} requestwith ${result.conflicts?.length || 0} conflicts found.`;
  }
  ...
  private async handleGeneralCalendarQuery(request: string): Promise<unknown> {
