...
const httpRequestDuration = new prometheus.Histogram({
  name: 'httprequest_duration_seconds',
...
export class ProductionPerformanceMiddleware extends EventEmitter {
  ...
  private cleanupMetrics()): void {
    ...
    if (removed > 0) {
      logger.debug(`Cleaned up ${removed} requestmetrics`, LogContext.PERFORMANCE);`
    }
  }
    ...
    return (req: Request, res: Response, next): NextFunction => {
      ...
      res.end = function (this: Response, ...args: any[]) {
        ...
        if (responseTime > self.options.slowRequestThreshold) {
          logger.warn('Slow requestdetected', LogContext.PERFORMANCE, {
            ...metric,
            threshold: self.options.slowRequestThreshold,
          });
          self.emit('slow-request, metric;
        }

        // Log errors
        if (res.statusCode >= 400) {
          logger.error('Reque, LogContext.PERFORMANCE, metric;
        }
...
• Slow requestdetection (>${this.options.slowRequestThreshold}ms)
