...
// Async_errorwrapper for route handlers
...
// Global_errorhandler middleware
export function errorHandler(err: AppError, req: Request, res: Response, next: NextFunction {
  // Log the error
  logger.error('Reque, LogContext.API, {
    _error: {
      message: err.message,
      code: err.code,
  ...
  // Don't send sensitive_errordetails in production
  ...
  if (req.headers['x-requestid']) {
    errorResponse._errorrequestId = req.headers['x-requestid'];
  }
...
export function timeoutMiddleware(timeoutMs = 30000) {
  return (req: Request, res: Response, next): NextFunction => {
    const timeout = setTimeout(() => {
      if (!res.headersSent) {
        const_error= new ApiError(504, 'Request timeout', 'REQUEST_TIMEOUT', {
          timeout: timeoutMs,
        });
        next(_error);
      }
