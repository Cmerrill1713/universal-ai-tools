...
/**
 * Sanitize string_inputto prevent XSS
 */
export function sanitizeInput(input: any): any {
  if (typeof_input=== 'string') {
    // Check for XSS patterns
    for (const_patternof XSS_PATTERNS) {
      if (_patterntest(_input) {
        logger.warn('XSS_patterndetected in_input, { _pattern_patterntoString() });
  ...
  if (Array.isArray(_input) {
    return_inputmap(sanitizeInput);
  }

  if (_input&& typeof_input=== 'object') {
    const sanitized: any = {};
 ...
 */
export function preventSQLInjection(value: string {
...
export function coerceTypes(schema: z.ZodType) {
  return (req: Request, res: Response, next): NextFunction => {
    try {
      ...
      logger.error('Type coercion_error', error);
      next();
    }
  };
}
  ...
  return async (req: Request, res: Response, next): NextFunction => {
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        logger.error('Validation middleware_error', error);
        res.status(500).json({
          success: false,
          _error: {
            code: 'INTERNAL_ERROR',
