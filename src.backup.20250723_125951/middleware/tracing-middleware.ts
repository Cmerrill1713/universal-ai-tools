  ...
  return (req: TracedRequest, res: Response, next): NextFunction => {
    ...
    const span = tracer.startSpan(
      spanName,
      {
        ...
        attributes: {
          ...
          'http.request_id': req.get('x-requestid') || `req-${Date.now()}`,
    ...
    if (mergedOptions.customAttributesExtractor) {
      try {
        ...
        logger.error('Error extracting custom attribute, LogContext.SYSTEM, {
    ...
    if (mergedOptions.recordRequestBody && req.body) {
      try {
        ...
        span.setAttribute('http.requestbody', bodyStr.substring(0, 1000)); // Limit size
        span.setAttribute('http.requestbody.size', bodyStr.length);
      } catch (error) {
        span.setAttribute('http.requestbody._error, 'Failed to serialize requestbody');
      }
    }
    ...
    const captureResponse = (body: any => {
      ...
      if (mergedOptions.recordResponseBody && body) {
        try {
          ...
          span.setAttribute('http.response.body._error, 'Failed to serialize response body');
        }
      }
      ...
      if (res.statusCode >= 400) {
        ...
        // Add_errordetails if available
        if (res.locals._error: {
          span.recordException(res.locals._error);
          span.setAttribute('_errortype', res.locals.error.name || 'Error');
