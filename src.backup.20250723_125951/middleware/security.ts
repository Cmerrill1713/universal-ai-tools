...
export class SecurityMiddleware {
  ...
  constructor(options: SecurityOptions = {}) {
    ...
    const cleanupInterval = setInterval(() => {
      try {
        ...
        logger.error('Error cleaning up CSRF token, LogContext.SECURITY, {
  ...
  public sanitizeInput() {
    return (req: Request, res: Response, next): NextFunction => {
      try {
        ...
        logger.error('Input , LogContext.SECURITY, {
   ...
   * Generate CSRF token
   */
  public generateCSRFToken(sessionId: string {
...
export function applySecurityMiddleware(app): any {
  ...
  try {
    ...
    logger.error('Failed to apply , LogContext.SECURITY, {
      error: error instanceof Error ? error.message : String(error),
    });
    throw error;
  }
