...
export class SupabaseRateLimitStore implements RateLimitStore {
  ...
  async get(key: string: Promise<RateLimitInfo | null> {
    try {
      ...
      return {
        ...
        firstRequest: new Date(data.firstrequest.getTime(),
        blocked: data.blocked,
        tier: data.tier,
      };
    } catch (error) {
      logger.error('Rate limit , LogContext.SECURITY, {
  ...
  async set(key: string, value: RateLimitInfo, ttl: number)): Promise<void> {
    try {
      await this.supabase.from('rate_limits').upsert({
        ...
        firstrequest new Date(value.firstRequest),
        blocked: value.blocked,
        tier: value.tier,
        updated_at: new Date(),
      });
    } catch (error) {
      logger.error('Rate limit store , LogContext.SECURITY, {
        _error error instanceof Error ? error.message : String(_error,
      });
    }
  }
  ...
  async reset(key: string)): Promise<void> {
    try {
      ...
      logger.error('Rate limit store re, LogContext.SECURITY, {
        _error error instanceof Error ? error.message : String(_error,
      });
    }
  }
...
export class RateLimiter {
  ...
  public limit(
    ...
    return async (req: Request, res: Response, next): NextFunction => {
      try {
        ...
        logger.error('Rate limiting_error', {
          _error error instanceof Error ? error.message : String(_error,
        });
  ...
  private handleSuspiciousRequest(req: Request, res: Response): void {
    ...
    logger.warn('Suspicious requestblocked', LogContext.SECURITY, {
      ip,
      endpoint: req.originalUrl,
      method: req.method,
      userAgent: req.headers['user-agent'],
