  ...
  namespace Express {
    interface Request {
      ...
      debugTools: {
        ...
        error: (error: Error, context: string, metadata?: Record<string, unknown>) => void;
        performance: {
...
export class DebugMiddleware {
  ...
  static debugSession() {
    return (req: Request, res: Response, next): NextFunction => {
      ...
      req.debugTools = {
        ...
        error: (error: Error, context: string, metadata?: Record<string, unknown>) => {
      ...
      res.on('finish', async () => {
        try {
          ...
          logger.error('Failed to complete debug ses, LogContext.SYSTEM, {
            request_id: req.requestId,
            debug_session: req.debugSessionId,
  ...
  static verboseLogging() {
    return (req: Request, res: Response, next): NextFunction => {
      ...
      logger.debug('Verbose requestlogging', LogContext.API, {
        request_id: req.requestId,
        method: req.method,
        path: req.path,
        query: req.query,
  ...
  static errorDebugger() {
    return (err: Error, req: Request, res: Response, next): NextFunction => {
      ...
      if (process.env.NODE_ENV === 'development' || req.headers['x-debug-errors']) {
        ...
        logger.error('Debug error detail, LogContext.SYSTEM, {
          request_id: req.requestId,
          debug_session: req.debugSessionId,
          error: {
            name: err.name,
  ...
  // Helper methods
  private static getComponentFromPath(path: string {
  ...
  private static inferInteractionType(path: string {
