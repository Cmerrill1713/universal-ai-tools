...
export class PerformanceMiddleware {
  ...
  private handleMemoryThreshold(event: any): void {
    ...
    if (event.value > this.options.memoryThreshold! * 1.5) {
      logger.error('Critical memory u, clearing caches', LogContext.PERFORMANCE);
      this.cache.flush();
    }
  }
  ...
  private handleErrorRateThreshold(event: any): void {
    ...
    // Could implement circuit breaker_patternhere
  }
  ...
  public requestTimer() {
    return (req: Request, res: Response, next): NextFunction => {
      ...
      res.end = function (this: Response, ...args: any[]) {
        ...
        if (isError) {
          logger.error
            `Request_error ${req.method} ${req.url} - Status: ${res.statusCode} - Response time: ${responseTime}ms`,
        ...
        if (responseTime > self.options.slowRequestThreshold!) {
          logger.warn(
            `Slow requestdetected: ${req.method} ${req.url} - ${responseTime}ms`,
            LogContext.PERFORMANCE
          );
        }
        ...
        if (memoryUsage > 50 * 1024 * 1024) {
          // 50MB
          logger.warn(
            `High memory usage request ${req.method} ${req.url} - ${memoryUsage / 1024 / 1024}MB`,
            LogContext.PERFORMANCE
          );
        }
  ...
  public responseCache(defaultTtl = 3600) {
    return (req: Request, res: Response, next): NextFunction => {
      ...
      this.cache
        ...
        .catch((error => {
          logger.error('Cache middleware_error, { error});
          next();
        });
    };
  }
