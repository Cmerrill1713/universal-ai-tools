      ...
      if (PrometheusMiddleware.isAthenaRequest(req)) {
        ...
        req.recordAthenaInteraction = (
          ...
          const actualResponseTime =;
            responseTimeMs || Date.now() - (req.prometheusStartTime || Date.now());
  ...
  static metricsEndpoint() {
    return async (req: Request, res: Response) => {
      try {
        ...
        logger.error('Failed to generate Prometheus metric, LogContext.PERFORMANCE, {
  ...
  private static recordAthenaMetrics(req: Request, res: Response, duration: number {
    try {
      ...
      const personalityMood =;
        req.body?.personality_mood || (req.query.personality_mood as string) || 'sweet';
      ...
      logger.error('Failed to record Athena metric, LogContext.ATHENA, {
