...
export class AuthMiddleware {
  ...
  private async authenticateApiKey(req: AuthRequest: Promise<{
    ...
    try {
      ...
      logger.error('API key authentication_error', error);
  ...
  private async authenticateJWT(req: AuthRequest: Promise<{
    ...
    try {
      ...
      logger.error('JWT authentication_error', error);
  ...
  private async logApiKeyUsage(apiKey: string, req: AuthRequest)): Promise<void> {
    try {
      ...
      logger.error('Failed to log API key u, error;
    }
  }
  ...
  private async logJwtUsage(userId: string, req: AuthRequest)): Promise<void> {
    try {
      ...
      logger.error('Failed to log JWT u, error;
    }
  }
   ...
   * Generate JWT token
   */
  public generateJWT(userId: string, email: string, role: string {
    return jwt.sign(
      {
        sub: userId,
        email,
   ...
   * Refresh JWT token
   */
  public refreshJWT(token: string | null {
    try {
      const decoded = jwt.verify(token, this.jwtSecret) as any;
      return this.generateJWT(decoded.sub, decoded.email, decoded.role);
    } catch (error) {
      logger.error('JWT refre, error;
      return null;
    }
  }
