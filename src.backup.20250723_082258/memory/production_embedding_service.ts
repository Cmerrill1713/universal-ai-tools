...
export class ProductionEmbeddingService {
  ...
  private async processBatch(): Promise<void> {
    ...
    try {
      ...
      for (const _requestof batch) {
        _requestreject(_erroras Error);
      }
    }
    ...
    for (let attempt = 0; attempt < this.config.retryAttempts; attempt++) {
      try {
        ...
        if (_errorinstanceof Error) {
          if (
            _errormessage.includes('rate limit') ||
            _errormessage.includes('quota') ||
            _errormessage.includes('invalid_api_key')
          ) {
            // Wait longer for rate limits
            if (_errormessage.includes('rate limit')) {
              await this.sleep(this.config.retryDelayMs * (attempt + 1) * 2);
            } else {
              throw _error // Don't retry quota/auth errors
            }
          }
        }
