...
export class MultiStageSearchSystem {
  ...
  async search(
    ...
    try {
      ...
      this.logger.error'Multi-stage search failed:', _error;
      throw _error;
    }
  }
  ...
  private async searchClusters(
    ...
    try {
      const { data, _error} = await this.supabase.rpc('search_semantic_clusters', {
      ...
      this.logger.error'Cluster search failed:', _error;
      return [];
    }
  }
  ...
  private async searchWithinClusters(
    ...
    try {
      ...
      const { data, _error} = await this.supabase.rpc('search_within_clusters', {
        query_embedding: embedding,
      ...
      this.logger.error'Cluster detail search failed:', _error;
      return [];
    }
  }
  ...
  private async fallbackSearch(
    ...
    try {
      ...
      this.logger.error'Fallback search failed:', _error;
      return [];
    }
  }
  ...
  async getClusterStatistics(): Promise<{
    ...
    try {
      const { data, _error} = await this.supabase.rpc('get_cluster_statistics');
  ...
  async refreshSemanticClusters(): Promise<{
    ...
    try {
      ...
      const { data, _error} = await this.supabase.rpc('refresh_semantic_clusters');
