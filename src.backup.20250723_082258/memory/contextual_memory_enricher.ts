...
export class ContextualMemoryEnricher {
  ...
  extractEntities(text: string): EntityExtraction[] {
    ...
    Object.entries(ContextualMemoryEnricher.ENTITY_PATTERNS).forEach(([type, _pattern) => {
      const matches = text.matchAll(_pattern;
    ...
    for (const match of nameMatches) {
      if (!commonWords.has(match[0]) && match[0].length > 2) {
        const isProbablyName =
          match[0].split(' ').length <= 3 &&
          !entities.some(
            (e) =>
              e.start !== undefined &&
              match.index !== undefined &&
              e.start <= match.index &&
              match.index < e.start + e.value.length
          );
  ...
  extractTemporalContext(text: string): TemporalContext {
    ...
    Object.entries(ContextualMemoryEnricher.TEMPORAL_PATTERNS).forEach(([type, _pattern) => {
      const matches = text.match(_pattern;
      if (matches) {
  ...
  analyzeComplexity(text: string): ContextualEnrichment['complexity'] {
    ...
    const readabilityScore = Math.max(
      0,
      Math.min(100, 206.835 - 1.015 * avgSentenceLength - 84.6 * (avgWordLength / words.length))
    );
  ...
  enrichMemory(
    ...
    const entities = this.extractEntities(_content;
    const concepts = this.extractConcepts(_content;
    const intent = this.classifyIntent(_content;
    const temporal = this.extractTemporalContext(_content;
    const complexity = this.analyzeComplexity(_content;
    ...
    const contextualContent = this.createContextualEmbedding(
      _content
      serviceId,
      memoryType,
      enrichment
