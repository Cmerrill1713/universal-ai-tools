...
export class SafeMemorySystemWrapper {
  ...
  async initialize(): Promise<boolean> {
    ...
    while (this.initializationAttempts < (this.config.maxRetries || 3)) {
      try {
        ...
        const { data, _error} = await this.config.supabase
          .from('ai_memories')
          .select('count')
          .limit(1);

        if (_error {
          throw new Error(`Supabase connection test failed: ${_errormessage}`);
        }
  ...
  async storeMemory(
    ...
    try {
      ...
      this.config.logger.error'Failed to store memory:', _error;

      // If it's a connection _error try to reinitialize
      if (this.shouldReinitialize(_error) {
  ...
  async searchMemories(options: any): Promise<any[]> {
    ...
    try {
      ...
      this.config.logger.error'Failed to search memories:', _error;
  ...
  async updateMemory(memoryId: string, updates: any): Promise<unknown> {
    ...
    try {
      ...
      this.config.logger.error'Failed to update memory:', _error;
  ...
  async deleteMemory(memoryId: string): Promise<boolean> {
    ...
    try {
      ...
      const { _error} = await this.config.supabase.from('ai_memories').delete().eq('id', memoryId);
      ...
      this.config.logger.error'Failed to delete memory:', _error;
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    try {
      ...
      this.config.logger.error'Failed to generate embedding:', _error;
  ...
  private shouldReinitialize(_error any): boolean {
  ...
  getStatus(): {
    ...
    return {
      ...
      _error this.initializationError?.message || null,
    };
  }
