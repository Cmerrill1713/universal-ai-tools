...
export class OllamaEmbeddingService {
  ...
  async preWarmCache(commonTexts: string[]): Promise<void> {
    console.log(`Pre-warming Ollama embedding cache with ${commonTexts.length} texts...`);
    await this.generateEmbeddings(commonTexts);
    console.log(`Cache pre-warmed with ${this.cache.size} embeddings`);
  }
  ...
  async pullModel(model?: string): Promise<void> {
    ...
    console.log(`Pulling Ollama model: ${modelToPull}...`);

    try {
      await this.makeRequest('/api/pull', 'POST', {
        name: modelToPull,
      });
      console.log(`Successfully pulled model: ${modelToPull}`);
    } catch (_error) {
      console._error`Failed to pull model ${modelToPull}:`, _error;
      throw _error;
    }
  }
  ...
  async listModels(): Promise<Array<{ name: string; size: number; modified_at: string }>> {
    try {
      ...
      console._error'Failed to list Ollama models:', _error;
      return [];
    }
  }
    ...
    try {
      ...
      for (const item of currentBatch) {
        try {
          ...
          item.reject(_errorinstanceof Error ? _error: new Error('Unknown _error));
        }
      }
    } catch (_error) {
      // If batch processing fails, reject all items
      currentBatch.forEach((item) => {
        item.reject(_errorinstanceof Error ? _error: new Error('Batch processing failed'));
      });
    }
  }
  ...
  private async generateSingleEmbedding(text: string): Promise<number[]> {
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        console.warn(`Ollama embedding attempt ${attempt} failed:`, _error;

        if (attempt === this.config.maxRetries) {
          this.stats.failedRequests++;
          throw new Error(
            `Failed to generate embedding after ${this.config.maxRetries} attempts: ${_error`
          );
        }
  ...
  private async makeRequest(endpoint: string, method: 'GET' | 'POST', data?: any): Promise<unknown> {
    ...
    try {
      ...
      if (_errorinstanceof Error && _errorname === 'AbortError') {
