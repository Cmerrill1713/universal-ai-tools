...
export class AIInstrumentation {
  ...
  async withAISpan<T>(operation: AIOperation, fn: () => Promise<T>): Promise<T> {
    ...
    const span = this.tracer.startSpan(spanName, {
      ...
      attributes: {
        ...
        'ai._requestmax_tokens': operation.maxTokens,
        'ai._requesttemperature': operation.temperature,
        'ai._requeststream': operation.stream || false,
        'ai._requestprompt_preview': operation.prompt?.substring(0, 100),
        'ai._requestprompt_length': operation.prompt?.length || 0,
      },
    });
    ...
    try {
      ...
      span.setStatus({
        ...
        message: _errorinstanceof Error ? _errormessage : 'AI operation failed',
      });

      // Record _errordetails
      if (_errorinstanceof Error) {
        span.setAttribute('_errortype', _errorname);
        ...
        } else if (_errormessage.includes('context length')) {
          span.setAttribute('ai._errortype', 'context_length_exceeded');
        } else if (_errormessage.includes('timeout')) {
          span.setAttribute('ai._errortype', 'timeout');
        } else if (_errormessage.includes('authentication')) {
  ...
  private recordAIResponse(span: Span, response: any): void {
    ...
    if (response.choices?.[0]) {
      ...
      if (choice.message?._content {
        span.setAttribute('ai.response.content_length', choice.message._contentlength);
        span.setAttribute('ai.response.content_preview', choice.message._contentsubstring(0, 100));
      }
    ...
    return async function* (params: any) {
      ...
      try {
        ...
        for await (const chunk of stream) {
          ...
          if (chunk.choices?.[0]?.delta?._content {
            _content+= chunk.choices[0].delta._content
          }
          if (chunk.usage) {
        ...
        span.setAttribute('ai.response.content_length', _contentlength);
        ...
        span.setStatus({
          code: SpanStatusCode.ERROR,
          message: _errorinstanceof Error ? _errormessage : 'Streaming failed',
        });
        throw _error;
      } finally {
        span.end();
  ...
  private extractPrompt(params: any): string | undefined {
    ...
    if (params.messages && Array.isArray(params.messages)) {
      return params.messages.map((m: any) => `${m.role}: ${m._content`).join('\n');
    }
