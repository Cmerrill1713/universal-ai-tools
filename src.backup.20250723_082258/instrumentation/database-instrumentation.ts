...
export class DatabaseInstrumentation {
  ...
  async withDatabaseSpan<T>(operation: DatabaseOperation, fn: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      span.setStatus({
        ...
        message: _errorinstanceof Error ? _errormessage : 'Database operation failed',
      });
      ...
      if (_errorinstanceof Error) {
        ...
        span.setAttribute('_errorstack', _errorstack?.substring(0, 1000) || '');
      }
  ...
  async withTransaction<T>(
    ...
    try {
      ...
      span.setStatus({
        code: SpanStatusCode.ERROR,
        message: _errorinstanceof Error ? _errormessage : 'Transaction failed',
      });
