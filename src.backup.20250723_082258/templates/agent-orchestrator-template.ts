...
export class AgentOrchestrator extends EventEmitter {
  ...
  private async assignTaskToAgent(task: Task): Promise<void> {
    ...
    if (circuitBreaker) {
      try {
        ...
        await this.handleTaskFailure(task, _erroras Error);
      }
    }
  }
  ...
  private async handleTaskFailure(task: Task, _error Error): Promise<void> {
    task.retryCount++;
    ...
    if (task.retryCount < maxRetries) {
      ...
      this.emit('taskRetrying', { task, _error});
      ...
      this.emit('taskFailed', { task, _error});
      console._error`Task failed permanently: ${task.type} (${task.id}):`, _errormessage);
    }
  }
