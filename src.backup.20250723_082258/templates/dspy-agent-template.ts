...
export abstract class BaseDSPyAgent extends EventEmitter {
  ...
  abstract zeroShotQuery(_request AgentRequest): Promise<AgentMessage>;

  // Compiled query _pattern(post-compilation)
  abstract compiledQuery(_request AgentRequest): Promise<AgentMessage>;
  ...
  protected async handleError(_error Error, context: any): Promise<void> {
    this.isHealthy = false;
    this.emit('_error, { _error context, agent: this.id });
...
export class DSPyRAGAgent extends BaseDSPyAgent {
  ...
  async zeroShotQuery(_request AgentRequest): Promise<AgentMessage> {
    try {
      // Retrieve relevant documents
      const passages = await this.retrieve(_requestquery);

      // Generate answer using DSPy
      const answer = await this.generate(_requestquery, passages);
      ...
      await this.handleError(_erroras Error, _request;
      throw _error;
    }
  }

  async compiledQuery(_request AgentRequest): Promise<AgentMessage> {
    try {
      ...
      const passages = await this.retrieveCompiled(_requestquery);
      const answer = await this.generateCompiled(_requestquery, passages);
  ...
  async compileModules(trainingData: any[]): Promise<void> {
    ...
    try {
      ...
      this.emit('compilationFailed', { agent: this.id, _error});
      throw _error;
    }
  }
