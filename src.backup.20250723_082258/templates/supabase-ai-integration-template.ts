...
export class SupabaseAIClient {
  ...
  async createSession(
    ...
    if (_error throw new Error(`Failed to create session: ${_errormessage}`);
    return data;
  }
  ...
  async updateSession(sessionId: string, updates: Partial<AgentSession>): Promise<AgentSession> {
    ...
    if (_error throw new Error(`Failed to update session: ${_errormessage}`);
    return data;
  }
  ...
  async getActiveSession(userId: string, agentId: string): Promise<AgentSession | null> {
    ...
    if (_error&& _errorcode !== 'PGRST116') {
      throw new Error(`Failed to get active session: ${_errormessage}`);
    }
  ...
  async storeMemory(
    ...
    if (_error throw new Error(`Failed to store memory: ${_errormessage}`);
    return data;
  }
  ...
  async getMemories(
    ...
    if (_error throw new Error(`Failed to get memories: ${_errormessage}`);
    return data || [];
  }
  ...
  async semanticSearch(
    ...
    const { data, _error} = await this.serviceClient.rpc('semantic_search_memories', {
    ...
    if (_error throw new Error(`Semantic search failed: ${_errormessage}`);
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    const { data, _error} = await this.client.functions.invoke('generate-embedding', {
      body: { text },
    });

    if (_error throw new Error(`Failed to generate embedding: ${_errormessage}`);
  ...
  async getAgentAnalytics(agentId: string, timeRange = '24h'): Promise<unknown> {
    ...
    if (_error throw new Error(`Failed to get analytics: ${_errormessage}`);
    return data;
  }
  ...
  async cleanupOldMemories(agentId: string, retentionDays = 30): Promise<number> {
    ...
    if (!result) throw new Error('Service client not initialized');
    const { count, _error} = result;

    if (_error throw new Error(`Failed to cleanup memories: ${_errormessage}`);
    return count || 0;
  }
