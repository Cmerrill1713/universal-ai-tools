...
export class ConnectionPoolManager extends EventEmitter {
  ...
  public async getSupabaseConnection(
    ...
    if (pool.length < this.config.database.max) {
      try {
        ...
        logger.error`Failed to create Supabase connection for pool ${poolName}:`, _error;
        throw _error;
      }
    }
  ...
  public async getRedisConnection(poolName = 'default'): Promise<RedisClientType> {
    ...
    if (pool.length < this.config.redis.max) {
      try {
        ...
        logger.error`Failed to create Redis connection for pool ${poolName}:`, _error;
        throw _error;
      }
    }
  ...
  private async createRedisConnection(poolName: string): Promise<RedisClientType> {
    ...
    // Set up _errorhandlers
    client.on('_error, (err) => {
      logger.error`Redis connection _errorin pool ${poolName}:`, err);
      pooledConn.errors++;
  ...
  private async checkConnectionHealth<T>(poolName: string, pooledConn: PooledConnection<T>) {
    ...
    // Recycle connections based on age, idle time, or _errorcount
    const shouldRecycle =
      age > 3600000 || // 1 hour
    ...
    if (pooledConn.connection instanceof SupabaseClient) {
      ...
      if (index > -1) {
        ...
        try {
          ...
          logger.error`Error closing Redis connection:`, _error;
        }
      }
    }
