...
export class MLXFineTuningService extends EventEmitter {
  ...
  async createFineTuningPipeline(
    ...
    try {
      ...
      job.status = 'failed';
      job._error= _errorinstanceof Error ? _errormessage : String(_error;
      this.emit('job-failed', { jobId, _error job._error});
      throw _error;
    }
  }
  ...
  private async prepareDatasetFromMemories(
    ...
    const { data: memories, _error} = await query;
...
`;
    ...
    const child = exec(`python3 ${scriptPath}`, (_error stdout, stderr) => {
      if (_error {
        job.status = 'failed';
        job._error= _errormessage;
        this.emit('job-failed', { jobId: job.id, _error _errormessage });
      } else {
        try {
          ...
          job.status = 'failed';
          job._error= 'Failed to parse training output';
        }
      }
    });
  ...
  private parseTrainingMetrics(job: FineTuningJob, log: string): void {
    ...
    if (epochMatch) {
      job.currentEpoch = parseInt(epochMatch[1], 10);
    }
  ...
  private async convertToOllama(job: FineTuningJob): Promise<void> {
    try {
      ...
      job.status = 'failed';
      job._error= `Conversion failed: ${_error`;
      this.emit('job-failed', { jobId: job.id, _error job._error});
    }
  }
...
`;
  ...
  private extractOutput(memory: any): string {
    ...
    return memory.metadata?.summary || memory._content
  }
