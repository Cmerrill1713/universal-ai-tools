 ...
 * Provides connection pooling, health monitoring, _errorhandling,
 * circuit breaker _pattern and in-memory fallback cache
 */
...
export class RedisService {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      const errorMessage = _errorinstanceof Error ? _errormessage : String(_error;

      logger.error'❌ Redis connection failed', LogContext.CACHE, {
        _error errorMessage,
        attempts: this.connectionAttempts,
  ...
  private setupEventListeners(client?: Redis): void {
    ...
    targetClient.on('_error, (_error => {
      if (!client || client === this.client) {
        this.isConnected = false;
      }
      logger.error'❌ Redis _error, LogContext.CACHE, {
  ...
  private setupClusterEventListeners(): void {
    ...
    this.clusterClient.on('_error, (_error => {
      this.isConnected = false;
      logger.error'❌ Redis cluster _error, LogContext.CACHE, {
        _error _errormessage,
      });
    });
    ...
    this.clusterClient.on('node _error, (_error address) => {
      logger.error'❌ Redis cluster node _error, LogContext.CACHE, {
        _error _errormessage,
        address,
      });
    });
    ...
    try {
      ...
      for (const poolClient of this.connectionPool) {
        try {
          ...
          logger.error'❌ Error disconnecting pool client', LogContext.CACHE, {
            _error _errorinstanceof Error ? _errormessage : String(_error,
          });
        }
      }
      ...
      for (const replica of this.readReplicas) {
        try {
          ...
          logger.error'❌ Error disconnecting read replica', LogContext.CACHE, {
            _error _errorinstanceof Error ? _errormessage : String(_error,
          });
        }
      }
      ...
      logger.error'❌ Error during Redis disconnect', LogContext.CACHE, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });
    } finally {
      this.client = null;
  ...
  async healthCheck(): Promise<{ healthy: boolean; latency?: number; _error: string }> {
    if (!this.client || !this.isConnected) {
      return { healthy: false, _error 'Not connected' };
    }
