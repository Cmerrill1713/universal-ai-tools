...
export class AutofixMemoryService {
  ...
  async storeFix(fix: Omit<AutofixMemory, 'session_id'>): Promise<void> {
    try {
      ...
      // Generate embedding for the fix _contentfor similarity search
      const _content= `${fix.fix_type}: ${fix.reasoning} | ${fix.original_code} -> ${fix.fixed_code}`;
      ...
      logger.error'Error storing autofix memory:', _error;
    }
  }
  ...
  async getSimilarFixes(currentFix: string, filePath: string, limit = 5): Promise<AutofixMemory[]> {
    try {
      ...
      logger.error'Error retrieving similar fixes:', _error;
      return [];
    }
  }
  ...
  async getFixPatternsForFileType(fileExtension: string): Promise<FixPattern[]> {
    try {
      ...
      logger.error'Error getting fix patterns:', _error;
      return [];
    }
  }
  ...
  async storeSessionSummary(summary: {
    ...
    try {
      const _content= `Autofix session completed: ${summary.total_fixes} fixes across ${summary.files_modified.length} files`;
      ...
      logger.error'Error storing session summary:', _error;
    }
  }
  ...
  async getAutofixInsights(): Promise<{
    ...
    try {
      ...
      logger.error'Error getting autofix insights:', _error;
      return {
        most_common_fixes: [],
        highest_success_patterns: [],
        recent_learnings: [],
  ...
  private async storeAutofixRecord(fix: AutofixMemory): Promise<void> {
    try {
      ...
      const { _error} = await this.supabase.from('autofix_history').insert(fix);

      if (_error&& !_errormessage.includes('does not exist')) {
        logger.warn('Failed to store in autofix_history table:', _error;
      }
    } catch (_error) {
