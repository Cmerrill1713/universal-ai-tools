...
export class ErrorTrackingService extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      logger.error'Failed to start _errortracking service', LogContext.ERROR, { _error});
      throw _error;
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      logger.error'Error stopping _errortracking service', LogContext.ERROR, { _error});
      throw _error;
    }
  }
  ...
  trackError(
    ...
    const errorMessage = _errorinstanceof Error ? _errormessage : _error
  ...
  private createErrorEvent(
    ...
    const isErrorObject = _errorinstanceof Error;
    const message = isErrorObject ? _errormessage : _error
    const stackTrace = isErrorObject ? _errorstack || '' : '';
  ...
  private shouldIgnoreError(message: string, level: ErrorEvent['level']): boolean {
    // Check minimum level
    const levelPriority = { debug: 0, info: 1, warn: 2, _error 3, fatal: 4 };
  ...
  private isMoreSevere(level1: ErrorEvent['level'], level2: ErrorEvent['level']): boolean {
    const severity = { debug: 0, info: 1, warn: 2, _error 3, fatal: 4 };
  ...
  private cleanupOldErrors(): void {
    ...
    for (const [id, _error of this.errors.entries()) {
