...
export class TemperatureController {
  ...
  private temperatureProfiles: Map<string, TemperatureProfile> = new Map([
    [
      'creative_writing',
      {
        ...
        maxTemp: 1.0,
        defaultTemp: 0.85,
        description: 'High creativity for storytelling and creative _content,
      },
    ],
    [
      'code_generation',
  ...
  public async getOptimalParams(
    ...
    if (context?.userPreference !== undefined) {
      const userAdjustment = this.getUserPreferenceAdjustment(context.userPreference, profile);
    ...
    if (context?.qualityRequirement) {
      const qualityAdjustment = this.getQualityAdjustment(context.qualityRequirement, taskType);
  ...
  private getUserPreferenceAdjustment(
    ...
    return {
      adjustment: Math.max(profile.minTemp, Math.min(profile.maxTemp, userPreference)),
    };
  }
  ...
  public async recordResult(
    ...
    if (qualityScore !== undefined) {
      ...
      metrics.avgQualityScore = alpha * qualityScore + (1 - alpha) * metrics.avgQualityScore;
    }
    ...
    if (success && qualityScore !== undefined) {
      ...
      const gradient =
        (qualityScore - metrics.avgQualityScore) * (temperature - metrics.optimalTemp);
  ...
  private async loadTaskMetrics(): Promise<void> {
    try {
      const { data, _error} = await this.supabase.client.from('temperature_metrics').select('*');

      if (_error {
        logger.error'Failed to load temperature metrics:', _error;
        return;
      }
      ...
      logger.error'Error loading temperature metrics:', _error;
    }
  }
  ...
  private async saveTaskMetrics(): Promise<void> {
    try {
      ...
      const { _error} = await this.supabase.client
        .from('temperature_metrics')
        .upsert(records, { onConflict: 'task_type' });

      if (_error {
        logger.error'Failed to save temperature metrics:', _error;
      }
    } catch (_error) {
      logger.error'Error saving temperature metrics:', _error;
    }
  }
