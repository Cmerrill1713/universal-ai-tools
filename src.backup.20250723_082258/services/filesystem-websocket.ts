...
export class FileSystemWebSocketService extends EventEmitter {
  ...
  async start(server?: any): Promise<void> {
    try {
      ...
      logger.error'Failed to start File System WebSocket service:', _error;
      throw _error;
    }
  }
  ...
  async stop(): Promise<void> {
    try {
      ...
      logger.error'Error stopping File System WebSocket service:', _error;
      throw _error;
    }
  }
  ...
  private handleConnection(ws: AuthenticatedFSWebSocket, req: any): void {
    ...
    // Handle errors
    ws.on('_error, (_error => {
      logger.error'WebSocket _error', _error { clientId });
    });
  ...
  private async handleWatch(
    ...
    if (!watcher) {
      ...
      watcher.on('_error, (_error => {
        logger.error'File watcher _error', _error { path: sanitizedPath });
      });
      ...
      for (const path of ws.watchedPaths) {
        ...
        if (clients) {
          ...
          if (clients.size === 0) {
            ...
            if (watcher) {
              watcher.close().catch((_error => {
                logger.error'Error closing watcher:', _error;
              });
  ...
  /**
   * Send _errormessage to WebSocket client
   */
  private sendError(ws: WebSocket, _error string): void {
    this.sendMessage(ws, {
      type: '_error,
      data: { _error},
      timestamp: Date.now(),
  ...
  private sanitizePath(inputPath: string): string | null {
    try {
      ...
      logger.error'Path sanitization _errorin WebSocket:', _error;
      return null;
    }
  }
