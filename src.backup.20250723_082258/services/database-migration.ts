...
export class DatabaseMigrationService {
  ...
  async initialize(): Promise<void> {
    try {
      const { _error} = await this.supabase.rpc('create_migration_table', {
        sql: `
      ...
      if (_error&& !_errormessage.includes('already exists')) {
        throw _error;
      }

      logger.info('Migration table initialized');
    } catch (_error) {
      logger.error'Failed to initialize migration table:', _error;
      throw _error;
    }
  }
  ...
  async getMigrationFiles(): Promise<Migration[]> {
    try {
      ...
      for (const file of files) {
        ...
        const [, id, name] = match;
        const checksum = this.generateChecksum(_content;
      ...
      logger.error'Failed to read migration files:', _error;
      throw _error;
    }
  }
  ...
  async getAppliedMigrations(): Promise<Migration[]> {
    try {
      ...
      logger.error'Failed to get applied migrations:', _error;
      throw _error;
    }
  }
  ...
  async runMigration(migration: Migration): Promise<void> {
    ...
    try {
      ...
      logger.error`Migration failed: ${migration.name}`, _error;
      throw _error;
    }
  }
  ...
  async runPendingMigrations(): Promise<number> {
    try {
      ...
      logger.error'Failed to run migrations:', _error;
      throw _error;
    }
  }
  ...
  async rollbackLast(): Promise<void> {
    try {
      ...
      logger.error'Rollback failed:', _error;
      throw _error;
    }
  }
  ...
  async validate(): Promise<boolean> {
    try {
      ...
      if (status.conflicts.length > 0) {
        logger.error'Migration conflicts found:', status.conflicts);
        return false;
      }
      ...
      logger.error'Migration validation failed:', _error;
      return false;
    }
  }
   ...
   */
  private generateChecksum(_content string): string {
    return crypto.createHash('sha256').update(_contenttrim()).digest('hex');
  }
