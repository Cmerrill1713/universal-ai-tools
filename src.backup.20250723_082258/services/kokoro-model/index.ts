...
`;
      ...
      logger.error'Failed to load Kokoro model:', _error;
      throw _error;
    }
  }
...
try:
    ...
    print(json.dumps({'success': False, '_error: str(e)}))
`;
    ...
    try {
      ...
      if (response.success) {
        ...
        throw new Error(`Synthesis failed: ${response._error`);
      }
    } catch (_error) {
  ...
  static async getAvailableVoices(voicesDir: string): Promise<string[]> {
    try {
      ...
      logger.error'Failed to list voices:', _error;
      return [];
    }
  }
