...
export class SwarmOrchestrator extends EventEmitter {
  ...
  async completeTask(
    taskId: string,
    result: { success: boolean; result?: any; _error: string }
  ): Promise<void> {
  ...
  async failTask(taskId: string, _error string): Promise<void> {
    const task = this.tasks.get(taskId);
    if (!task) return;
    ...
    if (this.config.autoRetry && task.attempts < task.maxAttempts) {
      ...
      await this.completeTask(taskId, { success: false, _error});
    }
  }
