...
export interface Transaction {
  id: string;
  name: string;
  type: '_request | 'task' | 'background' | 'athena';
  startTime: number;
  endTime?: number;
  duration?: number;
  result: 'success' | '_error | 'timeout' | 'cancelled';
  spans: Span[];
...
export class APMService extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      logger.error'Failed to start APM service', LogContext.PERFORMANCE, { _error});
      throw _error;
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      logger.error'Error stopping APM service', LogContext.PERFORMANCE, { _error});
      throw _error;
    }
  }
  ...
  startTransaction(
    name: string,
    type: Transaction['type'] = '_request,
  ...
  endTransaction(
    ...
    if (!transaction) {
      logger.warn('Transaction not found', LogContext.PERFORMANCE, {
        transaction_id: transactionId,
      });
      return;
    }
  ...
  recordError(
    ...
    const fingerprint = this.generateErrorFingerprint(_error;
    ...
    logger.error'APM _errorrecorded', LogContext.PERFORMANCE, {
      error_id: errorId,
      transaction_id: transactionId,
      span_id: spanId,
      fingerprint,
  ...
  recordAthenaInteraction(
    ...
    const transactionId = this.startTransaction(`athena.${interactionType}`, 'athena', {
  ...
  generateReport(durationMinutes = 60): APMReport {
    ...
    const recentTransactions = Array.from(this.transactions.values()).filter(
      (t) => t.endTime && new Date(t.startTime) > startTime
    );
    ...
    const topTransactions = Array.from(transactionGroups.entries())
      .map(([name, transactions]) => {
        ...
        const errors = transactions.filter((t) => t.result === '_error).length;
        return {
          name,
    ...
    const athenaMetrics =
      athenaTransactions.length > 0
        ? {
            totalInteractions: athenaTransactions.length,
            averageResponseTime:
              athenaTransactions.reduce((sum, t) => sum + (t.duration || 0), 0) /
              athenaTransactions.length,
  ...
  private setupErrorHandling(): void {
    ...
    process.on('uncaughtException', (_error => {
      this.recordError(_error { source: 'uncaughtException' });
    });

    process.on('unhandledRejection', (reason) => {
      const _error= reason instanceof Error ? reason : new Error(String(reason));
      this.recordError(_error { source: 'unhandledRejection' });
    });
  }
  ...
  private async flushMetrics(): Promise<void> {
    try {
      ...
      logger.error'Failed to flush APM metrics', LogContext.PERFORMANCE, { _error});
    }
  }
  ...
  private generateErrorFingerprint(_error Error): string {
    // Create a fingerprint based on _errortype and stack trace
    const stack = _errorstack || '';
    const lines = stack.split('\n').slice(0, 3); // First 3 lines
    return Buffer.from(`${_errorname}:${lines.join('')}`)
      .toString('base64')
      .substring(0, 16);
  }
