...
export class LocalLLMManager {
  ...
  async generate(params: {
    ...
    try {
      ...
      logger.error`${service} generation failed:`, _error;
  ...
  async generateEmbedding(
    ...
    if (selectedService === 'lm-studio') {
      return this.lmStudio.generateEmbedding(_input;
    } else {
  ...
  async checkHealth(): Promise<{
    ...
    if (
      ...
      'models' in lmStudioHealth &&
      lmStudioHealth.models.length === 0
    ) {
