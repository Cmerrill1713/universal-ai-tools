...
export class AthenaWidgetCreationService {
   ...
   */
  async createWidget(_request WidgetRequest): Promise<WidgetCreationResult> {
    try {
      this.logger.info(`Creating widget from description: ${_requestdescription}`);

      // Parse the natural language description
      const parsed = await this.parseWidgetDescription(_request;

      // Generate the React component
      const component = await this.generateReactComponent(parsed, _request;
      ...
      if (!validation.valid) {
        return {
          success: false,
          _error 'Generated component failed validation',
      ...
      await this.storeWidget(component, _requestuserId);
      ...
      this.logger.error'Widget creation failed:', _error;
      return {
        success: false,
   ...
   */
  private async parseWidgetDescription(_request WidgetRequest): Promise<ParsedWidgetRequirements> {
    const prompt = `Parse this widget description into structured requirements:

Description: "${_requestdescription}"
Additional Requirements: ${JSON.stringify(_requestrequirements || {})}
Examples: ${JSON.stringify(_requestexamples || [])}
...
Respond with JSON matching this structure:
{
  ...
  "features": ["string"]
    ...
    try {
      ...
      // Fallback parsing
      return this.fallbackParsing(_request;
    }
  }
  ...
  private async generateReactComponent(
    ...
    _request WidgetRequest
  ): Promise<WidgetComponent> {
    const template = this.getTemplate(parsed.componentType);
    const stylingFramework = _requestrequirements?.style || parsed.styling.framework;
...
Respond with JSON:
{
  ...
  "usage": "Example usage code"
    ...
    try {
      ...
      throw new Error(`Component generation failed: ${(_erroras Error).message}`);
    }
  }
  ...
  private extractDependencies(deps: string[], stylingFramework: string): string[] {
    ...
    return allDeps.filter((dep) => dep && !dep.startsWith('./') && !dep.startsWith('../'));
  }
  ...
  private async validateComponent(component: WidgetComponent): Promise<{
    ...
    try {
      ...
      if (
        ...
        !component.code.includes(component.propInterface.split(' ')[1])
      ) {
      ...
      errors.push(`Validation _error ${(_erroras Error).message}`);
    }
...
describe('${component.name}', () => {
  ...
  private async generateDocumentation(
    ...
    const events = parsed.events.map((e) => `- **${e.name}**: ${e.handler}`).join('\n');
...
Generated with Sweet Athena Widget Creator 🌸`;
  ...
  private async storeWidget(component: WidgetComponent, userId: string): Promise<void> {
    try {
      ...
      this.logger.error'Failed to store widget:', _error;
    }
  }
  ...
  async getWidget(widgetId: string): Promise<WidgetComponent | null> {
    ...
    try {
      ...
      this.logger.error'Failed to get widget:', _error;
      return null;
    }
  }
...
<body>
  ...
  async exportWidget(widgetId: string): Promise<string | null> {
    ...
    try {
      ...
      await fs.writeFile(path.join(exportDir, `${widget.name}.tsx`), widget.code);

      // Write styles if separate
      if (widget.styles && !widget.code.includes('styled-components')) {
        await fs.writeFile(path.join(exportDir, `${widget.name}.css`), widget.styles);
      }

      // Write tests
      if (widget.tests) {
        await fs.writeFile(path.join(exportDir, `${widget.name}.test.tsx`), widget.tests);
      }

      // Write documentation
      await fs.writeFile(path.join(exportDir, 'README.md'), widget.documentation);
      ...
      const packageJson = {
        ...
        dependencies: widget.dependencies.reduce(
          (acc, dep) => {
            acc[dep] = 'latest';
            return acc;
          },
      ...
      this.logger.error'Failed to export widget:', _error;
      return null;
    }
  }
...
const ChartComponent: FC<ChartProps> = ({ data, type, title }) => {
  ...
  private fallbackParsing(_request WidgetRequest): ParsedWidgetRequirements {
    const words = _requestdescription.toLowerCase().split(' ');
    let componentType: ParsedWidgetRequirements['componentType'] = 'custom';

    if (words.some((w) => ['form', '_input, 'submit'].includes(w))) {
      componentType = 'form';
    ...
    const componentName = this.generateComponentName(_requestdescription);
    ...
    return {
      ...
      events: [],
      styling: {
        framework: _requestrequirements?.style || 'styled-components',
        theme: _requestrequirements?.theme || 'light',
        responsive: _requestrequirements?.responsive !== false,
      },
      features: _requestrequirements?.features || [],
    };
  }
