...
export class KnowledgeValidationService {
  ...
  async validateScrapedKnowledge(
    ...
    try {
      ...
      const qualityResult = await this.validateContentQuality(_content metadata);
      ...
      if (source.priority === 'high' && metadata.contentType !== 'code') {
        const factCheckResult = await this.validateFactChecking(_content metadata);
        validationResults.push(factCheckResult);
      }

      // 4. Deprecation detection
      const deprecationResult = await this.detectDeprecation(_content metadata);
      ...
      if (metadata.hasCodeExamples || metadata.contentType === 'code') {
        const technicalResult = await this.validateTechnicalAccuracy(_content metadata);
      ...
      logger.error`Failed to validate knowledge ${scrapedId}:`, _error;
    }
  ...
  private async validateContentQuality(
    ...
    const tokenCount = this.countTokens(_content;
    if (tokenCount > 8000) {
      issues.push('Content exceeds optimal token limit for processing');
      suggestions.push('Summarize or split _contentfor better processing');
      score *= 0.7;
    }

    // Readability analysis
    const readabilityScores = this.analyzeReadability(_content;
    ...
    const grammarIssues = await this.checkGrammarAndSpelling(_content;
    ...
    const structureScore = this.validateContentStructure(_content metadata);
    ...
    // Duplicate _contentcheck
    const duplicateScore = await this.checkDuplicateContent(_content;
    if (duplicateScore > 0.8) {
      issues.push('Content appears to be duplicate or very similar to existing knowledge');
      suggestions.push('Focus on unique insights or merge with existing _content);
      score *= 0.5;
    }
  ...
  private async validateFactChecking(
    ...
    try {
      ...
      const claims = await this.extractFactualClaims(_content;
      ...
      const factCheckResult = await dspyService._request'validate_facts', {
        claims,
        context: metadata,
        requireSources: true,
      });
      ...
      logger.error'Fact checking failed:', _error;
  ...
  private async detectDeprecation(
    ...
    const lowerContent = _contenttoLowerCase();
    ...
    const versionMatches = _contentmatch(versionPattern);
    ...
    const yearMatches = _contentmatch(datePattern);
    ...
    const deprecationDatabase = await this.checkDeprecationDatabase(_content metadata);
  ...
  private async validateTechnicalAccuracy(
    ...
    // Extract code blocks
    const codeBlocks = this.extractCodeBlocks(_content;
    ...
    for (const codeBlock of codeBlocks) {
      ...
      if (!syntaxValidation.isValid) {
        issues.push(`Syntax _errorin ${codeBlock.language} code: ${syntaxValidation._error`);
    ...
    if (metadata.category === 'api' || _contentincludes('endpoint')) {
      const apiValidation = await this.validateAPIUsage(_content;
  ...
  private countTokens(_content string): number {
    if (this.encoding) {
      return this.encoding.encode(_content.length;
    }
    // Fallback: approximate token count
    return Math.ceil(_contentsplit(/\s+/).length * 1.3);
  }

  private analyzeReadability(_content string): Record<string, number> {
    const sentences = this.sentenceTokenizer.tokenize(_content;
    const words = this.tokenizer.tokenize(_content;
  ...
  private async checkGrammarAndSpelling(_content string): Promise<string[]> {
    const issues: string[] = [];
    const words = this.tokenizer.tokenize(_content;
    ...
    for (const word of words) {
      if (word.length > 2 && !this.isValidWord(word)) {
        issues.push(`Possible spelling _error ${word}`);
      }
    }
  ...
  private validateContentStructure(_content string, metadata: Record<string, unknown>): number {
    let score = 1.0;

    // Check for headings
    const headingPattern = /^#{1,6}\s+.+$/gm;
    const headings = _contentmatch(headingPattern);
    ...
    if (metadata.hasCodeExamples) {
      ...
      const codeBlocks = _contentmatch(codeBlockPattern);
    ...
    const lists = _contentmatch(listPattern);
  ...
  private async checkDuplicateContent(_content string): Promise<number> {
    try {
      // Generate _contenthash
      const contentHash = createHash('sha256').update(_content.digest('hex');
      ...
      // Check for similar _contentusing text similarity
      // This would use vector embeddings in production
      return 0.0; // No duplicates
    } catch (_error) {
      logger.error'Duplicate check failed:', _error;
      return 0.0;
    }
  }

  private async extractFactualClaims(_content string): Promise<string[]> {
    const claims: string[] = [];
    const sentences = this.sentenceTokenizer.tokenize(_content;
  ...
  private extractCodeBlocks(_content string): Array<{ code: string; language: string }> {
    const codeBlockPattern = /```(\w+)?\n([\s\S]*?)```/g;
    const blocks: Array<{ code: string; language: string }> = [];

    let match;
    while ((match = codeBlockPattern.exec(_content) !== null) {
      blocks.push({
  ...
  private async validateCodeSyntax(codeBlock: {
    code: string;
    language: string;
  }): Promise<{ isValid: boolean; _error: string }> {
    ...
    try {
      if (codeBlock.language === 'javascript' || codeBlock.language === 'typescript') {
        ...
        if (codeBlock.code.split('{').length !== codeBlock.code.split('}').length) {
          return { isValid: false, _error 'Unmatched braces' };
        }
      }

      return { isValid: true };
    } catch (_error) {
      return { isValid: false, _error String(_error };
    }
  }
  ...
  private async validateAPIUsage(_content string): Promise<{
    isValid: boolean;
    score: number;
    issues: string[];
    suggestions: string[];
    ...
    const mentionedMethods = httpMethods.filter((method) => _contentincludes(method));

    if (mentionedMethods.length === 0 && _contentincludes('endpoint')) {
    ...
    if (!_contentincludes('response') && !_contentincludes('returns')) {
  ...
  private async storeValidationResults(
    ...
    const { _error} = await supabase.from('knowledge_validation').insert(entries);

    if (_error {
      logger.error'Failed to store validation results:', _error;
    }
  }
...
if (      overallScore >= 0.7) { return 'validated'; } else if (overallScore >= 0.5) { return 'needs_review'; } else { return 'rejected'; }
    ...
    if (_error {
      logger.error'Failed to update scraped knowledge status:', _error;
    }
  }
