...
export class KnowledgeFeedbackService extends EventEmitter {
  ...
  async trackUsage(analytics: UsageAnalytics): Promise<void> {
    try {
      // Store in database
      const { _error} = await supabase.from('knowledge_usage_analytics').insert({
      ...
      if (_error {
        logger.error'Failed to track usage:', _error;
        return;
      }
      ...
      logger.error'Error tracking usage:', _error;
    }
  }
  ...
  private async processUsageAnalytics(): Promise<void> {
    try {
      ...
      const { data: recentAnalytics, _error} = await supabase
      ...
      if (_error {
        logger.error'Failed to fetch recent analytics:', _error;
        return;
      }
      ...
      logger.error'Error processing usage analytics:', _error;
    }
  }
  ...
  private async detectUsagePatterns(): Promise<void> {
    try {
      ...
      const { data: analytics, _error} = await supabase
        .from('knowledge_usage_analytics')
        .select('*')
        .gte('created_at', oneHourAgo.toISOString());

      if (_error|| !analytics) return;
      ...
      logger.error'Error detecting usage patterns:', _error;
    }
  }
  ...
  private async evaluatePerformance(): Promise<void> {
    try {
      ...
      logger.error'Error evaluating performance:', _error;
    }
  }
  ...
  private async updateKnowledgeRanking(): Promise<void> {
    try {
      ...
      const { data: knowledgeItems, _error} = await supabase
        .from('knowledge_usage_analytics')
        .select(
          `
          knowledge_id,
      ...
      if (_error|| !knowledgeItems) return;
      ...
      logger.error'Error updating knowledge ranking:', _error;
    }
  }
  ...
  private async updateRelationship(
    ...
    try {
      const { _error} = await supabase.rpc('update_learned_relationship', {
      ...
      logger.error'Failed to update relationship:', _error;
    }
  }
  ...
  private async storePerformanceMetrics(metrics?: PerformanceMetric[]): Promise<void> {
    ...
    const { _error} = await supabase.from('knowledge_performance_metrics').insert(
      metrics.map((m) => ({
    ...
    if (_error {
      logger.error'Failed to store performance metrics:', _error;
    }
  }
  ...
  private async calculateUsageEffectiveness(since: Date): Promise<number> {
    ...
    if (_error|| !data || data.length === 0) return 0.5;
  ...
  private async calculateUpdateFrequency(since: Date): Promise<number> {
    ...
    if (_error|| !data || data.length < 2) return 0.5;
