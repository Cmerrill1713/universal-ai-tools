...
export class PixelStreamingBridge extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.error'Failed to initialize Pixel Streaming Bridge:', undefined, _error;
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      logger.error'Failed to connect to Pixel Streaming server:', undefined, _error;
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      logger.error'Error during disconnect:', undefined, _error;
      this.emit('_error, _error;
    }
  }
  ...
  async sendAvatarCommand(command: Omit<AvatarCommand, 'timestamp'>): Promise<void> {
    ...
    try {
      ...
      logger.error'Failed to send avatar command:', undefined, _error;
      this.emit('commandError', { command: fullCommand, _error});
      throw _error;
    }
  }
  ...
  private setupSignallingServer(): void {
    ...
    this.signallingServer.addEventListener('_error, (_error any) => {
      logger.error'Signalling server _error', undefined, _error;
      this.emit('_error, _error;
    });
  }
    ...
    while (this.commandQueue.length > 0 && this.isConnected) {
      ...
      if (command) {
        try {
          ...
          logger.error'Failed to process queued command:', undefined, { command, _error});
        }
      }
    }
  }
  ...
  private handleUE5Message(data: any): void {
    try {
      ...
      logger.error'Failed to process UE5 message:', undefined, { data, _error});
    }
  }
