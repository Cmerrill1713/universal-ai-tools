...
export class DynamicContextManager {
  ...
  private async applySlidingWindow(
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      const msgTokens = msg.tokens || (await this.estimateTokens(msg._content);
  ...
  private async scoreMessageImportance(messages: Message[]): Promise<Message[]> {
    return messages.map((msg) => {
      ...
      const lengthBonus = Math.min(1, msg._contentlength / 1000) * 0.1;
  ...
  private async compressMessages(
    ...
    return messages.map((msg) => {
      if (msg.role === 'system' || msg._contentlength < 200) {
        return msg; // Don't compress system messages or short messages
      }

      const compressed = this.compressText(msg._content;
      if (compressed.length < msg._contentlength * 0.8) {
        return {
          ...msg,
          _content compressed,
  ...
  private async countTokens(messages: Message[]): Promise<number> {
    ...
    for (const msg of messages) {
      if (msg.tokens) {
        ...
        total += await this.estimateTokens(msg._content;
      }
    }
    return total;
  }
