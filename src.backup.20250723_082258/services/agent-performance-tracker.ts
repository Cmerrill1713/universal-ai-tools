...
export class AgentPerformanceTracker extends EventEmitter {
  ...
  private initializeBufferFlush(): void {
    ...
    this.bufferFlushInterval = setInterval(() => {
      this.flushMetricsBuffer().catch((_error => {
        logger.error'Failed to flush metrics buffer', LogContext.PERFORMANCE, { _error});
      });
    }, 5000);
  }
  ...
  private initializeAggregation(): void {
    ...
    this.aggregationInterval = setInterval(
      () => {
        this.runAggregation().catch((_error => {
          logger.error'Failed to run aggregation', LogContext.PERFORMANCE, { _error});
        });
      },
      5 * 60 * 1000
    );
  ...
  private async flushMetricsBuffer(): Promise<void> {
    ...
    try {
      ...
      logger.error'Failed to flush metrics to database', LogContext.PERFORMANCE, { _error});
  ...
  private async aggregateMetrics(
    ...
    try {
      const { _error} = await this.supabase.rpc('aggregate_performance_metrics', {
        p_period: period,
      });
      ...
      logger.error'Failed to aggregate metrics', LogContext.PERFORMANCE, { period, _error});
    }
  }
  ...
  async cleanupOldMetrics(): Promise<void> {
    ...
    try {
      ...
      logger.error'Failed to cleanup old metrics', LogContext.PERFORMANCE, { _error});
    }
  }
