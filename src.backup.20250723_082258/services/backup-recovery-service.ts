...
// Dynamic AWS SDK loader with helpful _errormessages
async function loadAwsSdk(): Promise<boolean> {
  ...
  try {
    ...
    awsSdkError = _errorinstanceof Error ? _errormessage : 'Unknown _errorloading AWS SDK';
...
export class BackupRecoveryService {
  ...
  async createBackup(
    ...
    try {
      ...
      logger.error`Backup ${backupId} failed: ${_errormessage}`, LogContext.DATABASE, { _error});
      metadata.status = 'failed';
      metadata._error= _errormessage;
      metadata.duration = Date.now() - startTime;
  ...
  private async exportTables(tables: string[]): Promise<{
    ...
    for (const table of tables) {
      try {
        ...
        const { data: tableData, _error} = await this.supabase.from(table).select('*');
        ...
        logger.error`Failed to export table ${table}: ${_errormessage}`, LogContext.DATABASE, {
          _error
        });
        throw new Error(`Export failed for table ${table}: ${_errormessage}`);
      }
    }
  ...
  private async storeS3Backup(backupId: string, data: Buffer, compress: boolean): Promise<void> {
    ...
    try {
      ...
      throw new Error(`S3 upload failed: ${_errormessage}`);
    }
  }
  ...
  private async loadSupabaseBackup(backupId: string, metadata: BackupMetadata): Promise<unknown> {
    ...
    const { data: fileData, _error} = await this.supabase.storage
      .from(this.config.storage.supabase.bucket)
      .download(filename);
  ...
  private async loadS3Backup(backupId: string, metadata: BackupMetadata): Promise<unknown> {
    ...
    try {
      ...
      throw new Error(`S3 download failed: ${_errormessage}`);
    }
  }
  ...
  async cleanupOldBackups(): Promise<number> {
    ...
    try {
      // Get all backups
      const { data: backups, _error} = await this.supabase
        .from('backup_metadata')
        .select('*')
        .eq('status', 'completed')
  ...
  private async deleteLocalBackup(backupId: string, metadata: BackupMetadata): Promise<void> {
    ...
    try {
      ...
      if (_errorcode !== 'ENOENT') throw _error;
    }
  }
  ...
  private async deleteS3Backup(backupId: string, metadata: BackupMetadata): Promise<void> {
    ...
    try {
      ...
      throw new Error(`S3 deletion failed: ${_errormessage}`);
    }
  }
  ...
  private calculateNextBackupTime(): Date | null {
    ...
    try {
      ...
      logger.error'Failed to parse cron schedule:', _error;
  ...
  async verifyBackup(backupId: string): Promise<{
    ...
    try {
      ...
      errors.push(`Verification failed: ${_errormessage}`);
      return { valid: false, errors };
    }
  }
