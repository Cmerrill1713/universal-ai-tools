...
export class AntiHallucinationService {
  ...
  async groundResponse(prompt: string): Promise<{ response: string; citations: string[] }> {
    ...
    const { data: memories, _error} = await this.supabase
      .from('ai_memories')
      .select('*')
      .textSearch('_content, prompt)
  ...
  private async searchMemories(response: string): Promise<MemoryModel[]> {
    ...
    const { data: memories, _error} = await this.supabase
      .from('ai_memories')
      .select('*')
      .or(keywords.map((k) => `_contentilike.%${k}%`).join(','))
      .limit(20);

    if (_error|| !memories) {
      console._error'Error searching memories:', _error;
      return [];
    }
  ...
  private async checkCitations(
    ...
    const citedFacts = facts.filter((fact) => {
      ...
      return memories.some((memory) => {
        ...
        const contentLower = memory._contenttoLowerCase();
        ...
        return (
          ...
          this.semanticSimilarity(fact.claim, memory._content > 0.7
        );
      });
    });

    const citations = memories
      .filter((m) => facts.some((f) => m._contenttoLowerCase().includes(f.claim.toLowerCase())))
      .map((m) => `Memory ${m.id}: ${m._contentsubstring(0, 100)}...`);
  ...
  private calculateTruthScore(
    ...
    const groundedFacts = facts.filter((f) => {
      return memories.some((m) => {
        ...
        const contentLower = m._contenttoLowerCase();
        ...
        // General matching
        return (
          contentLower.includes(claimLower) || this.semanticSimilarity(f.claim, m._content > 0.7
        );
      });
    }).length;
  ...
  private async generateWithCitations(
    ...
    const context = memories.map((m, i) => `[${i + 1}] ${m._content`).join('\n\n');

    // In real implementation, this would call an LLM
    // For now, we'll create a simple response that includes all memory content
    const facts = memories.map((m) => m._content.join('\n\n');
    ...
    const citations = memories.map(
      (m, i) => `[${i + 1}] Memory ${m.id}: ${m._contentsubstring(0, 50)}...`
    );
