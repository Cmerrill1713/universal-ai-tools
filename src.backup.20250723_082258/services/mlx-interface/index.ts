...
`;
      ...
      logger.warn('MLX availability check failed:', _error;
...
try:
    ...
    print(json.dumps({"loaded": False, "_error: str(e)}))
    sys.exit(1)
`;
      ...
      logger.error`Failed to load MLX model ${modelId}:`, _error;
      throw _error;
    }
  }
...
`;
      ...
      if (response.success) {
        ...
        throw new Error(`MLX generation failed: ${response._error`);
      }
    } catch (_error) {
      logger.error`MLX generation _errorfor ${modelId}:`, _error;
      throw _error;
    }
  }
...
`;
      ...
      try {
        ...
        logger.warn('MLX memory cleanup failed:', _error;
      }
