...
export class ModelLifecycleManager extends EventEmitter {
  private generateWithOllama = async (model: string, task: Task): Promise<ModelResponse> => {
    try {
      ...
      logger.error'Ollama generation failed:', _error;
      return {
  ...
  private async initializeInterfaces(): Promise<void> {
    ...
    try {
      ...
      logger.error'Failed to initialize MLX interface:', _error;
  ...
  async predictAndWarm(context: any): Promise<ModelPrediction> {
    ...
    try {
      ...
      logger.warn('Failed to list Ollama models:', _error;
  ...
  private async warmModel(modelName: string): Promise<void> {
    ...
    try {
      ...
      this.emit('model-_error, { model: modelName, _error});
      throw _error;
    }
  }
  ...
  private async processWarmingQueue(): Promise<void> {
    ...
    try {
      ...
      console._error`Failed to warm model ${task.model}:`, _error;
    }
  ...
  async autoManageMemory(): Promise<void> {
    try {
      ...
      if (totalSize > 0.8 * this.memoryLimit) {
        ...
        for (const [name, model] of lruModels) {
          ...
          try {
            ...
            logger.warn(`Failed to unload model ${name}:`, _error;
          }
      ...
      logger.error'Failed to manage memory:', _error;
    }
  }
  ...
  private async loadModel(modelName: string): Promise<void> {
    ...
    if (modelName.includes('mlx')) {
      ...
      try {
        ...
        logger.error`Failed to load MLX model ${modelName}:`, _error;
        throw _error;
      }
      ...
      try {
        ...
        logger.warn(`Failed to check Ollama model ${modelName}:`, _error;
      }
    }
  }
  ...
  private async getMemoryUsage(): Promise<number> {
    try {
      ...
      logger.debug('Failed to get system memory usage, using model size estimation:', _error;
