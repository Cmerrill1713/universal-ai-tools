...
export class TelemetryService extends EventEmitter {
  ...
  constructor() {
    ...
    // Setup _errorhandling
    this.on('_error, (_error => {
      logger.error'Telemetry service _error, LogContext.TELEMETRY, { _error});
    });
  }
  ...
  async initialize(options: TelemetryServiceOptions = {}): Promise<void> {
    ...
    try {
          ...
          getNodeAutoInstrumentations({
            ...
            '@opentelemetry/instrumentation-http': {
              ...
              requestHook: (span, _request => {
                if ('headers' in _request {
                  span.setAttributes({
                    'http._requestbody.size': _requestheaders['_contentlength'] || 0,
                    'http.user_agent': _requestheaders['user-agent'] || '',
                    'ai.service': _requestheaders['x-ai-service'] || 'unknown',
                  });
                }
              },
              responseHook: (span, response) => {
                if ('headers' in response) {
                  span.setAttributes({
                    'http.response.body.size': response.headers['_contentlength'] || 0,
                    'http.response.content_type': response.headers['_contenttype'] || '',
                  });
                }
              },
            },
      ...
      logger.error'Failed to initialize telemetry service', LogContext.SYSTEM, { _error});
      throw _error;
    }
  }
  ...
  async shutdown(): Promise<void> {
    if (this.sdk) {
      try {
        ...
        logger.error'Error shutting down telemetry service', LogContext.SYSTEM, { _error});
      }
    }
  }
  ...
  createSpan(spanData: CustomSpanData): Span | null {
    ...
    try {
      ...
      logger.error'Failed to create custom span', LogContext.TELEMETRY, {
        _error
  ...
  endSpan(
    span: Span,
    metadata?: { success?: boolean; _error: Error; attributes?: Record<string, unknown> }
  ): void {
    if (!span) return;
    ...
    try {
      ...
      if (metadata) {
        const { success = true, _error attributes = {} } = metadata;

        // Set status
        if (_error {
          span.setStatus({ code: SpanStatusCode.ERROR, message: _errormessage });
          span.setAttributes({
            _error true,
      ...
      logger.error'Failed to end span', LogContext.TELEMETRY, { _error});
    }
  }
  ...
  async traceOperation<T>(
    ...
    try {
      ...
      this.recordPerformanceTrace({
        ...
        duration,
        success: false,
        metadata: { _error _errorinstanceof Error ? _errormessage : String(_error },
        traceId: span.spanContext().traceId,
        timestamp: new Date(),
      });
      ...
      this.endSpan(span, {
        success: false,
        _error _errorinstanceof Error ? _error: new Error(String(_error),
