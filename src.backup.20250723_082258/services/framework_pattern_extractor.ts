...
export class FrameworkPatternExtractor {
  ...
  private async extractPatternsFromFile(
    ...
    if (framework.name === 'React') {
      patterns.push(...this.extractReactPatterns(_content relativePath));
    }

    // Vue patterns
    else if (framework.name === 'Vue') {
      patterns.push(...this.extractVuePatterns(_content relativePath));
    }

    // Angular patterns
    else if (framework.name === 'Angular') {
      patterns.push(...this.extractAngularPatterns(_content relativePath));
    }

    // Next.js patterns
    else if (framework.name === 'Next.js') {
      patterns.push(...this.extractNextPatterns(_content relativePath));
    }

    // Generic patterns
    patterns.push(...this.extractGenericPatterns(_content relativePath));
   ...
   */
  private extractReactPatterns(_content string, filePath: string): FrameworkPattern[] {
    const patterns: FrameworkPattern[] = [];

    // Function Component Pattern
    const functionComponentMatch = _contentmatch(
      /(?:export\s+)?(?:const|function)\s+(\w+).*?(?::\s*(?:React\.)?FC|=.*?=>.*?<)/
    );
    if (functionComponentMatch) {
      patterns.push(
        this.createReactFunctionComponentPattern(functionComponentMatch[1], _content filePath)
      );
    }

    // Custom Hook Pattern
    const hookMatch = _contentmatch(/(?:export\s+)?(?:const|function)\s+(use\w+)/);
    if (hookMatch) {
      patterns.push(this.createReactHookPattern(hookMatch[1], _content filePath));
    }

    // Context Pattern
    const contextMatch = _contentmatch(
      /(?:const|let)\s+(\w+Context)\s*=\s*(?:React\.)?createContext/
    );
    if (contextMatch) {
      patterns.push(this.createReactContextPattern(contextMatch[1], _content filePath));
    }

    // HOC Pattern
    const hocMatch = _contentmatch(
      /(?:const|function)\s+(with\w+).*?=.*?(?:Component|WrappedComponent)/
    );
    if (hocMatch) {
      patterns.push(this.createReactHOCPattern(hocMatch[1], _content filePath));
    }
   ...
   */
  private extractVuePatterns(_content string, filePath: string): FrameworkPattern[] {
    const patterns: FrameworkPattern[] = [];

    // Composition API Pattern
    if (_contentincludes('setup()') || _contentincludes('<script setup>')) {
      patterns.push(this.createVueCompositionPattern(_content filePath));
    }

    // Composable Pattern
    const composableMatch = _contentmatch(/(?:export\s+)?(?:const|function)\s+(use\w+)/);
    if (composableMatch && filePath.includes('composables')) {
      patterns.push(this.createVueComposablePattern(composableMatch[1], _content filePath));
    }

    // Single File Component Pattern
    if (_contentincludes('<template>') && _contentincludes('<script>')) {
      patterns.push(this.createVueSFCPattern(_content filePath));
    }
   ...
   */
  private extractAngularPatterns(_content string, filePath: string): FrameworkPattern[] {
    const patterns: FrameworkPattern[] = [];

    // Component Pattern
    if (_contentincludes('@Component')) {
      patterns.push(this.createAngularComponentPattern(_content filePath));
    }

    // Service Pattern
    if (_contentincludes('@Injectable')) {
      patterns.push(this.createAngularServicePattern(_content filePath));
    }

    // Directive Pattern
    if (_contentincludes('@Directive')) {
      patterns.push(this.createAngularDirectivePattern(_content filePath));
    }

    // Module Pattern
    if (_contentincludes('@NgModule')) {
      patterns.push(this.createAngularModulePattern(_content filePath));
    }
   ...
   */
  private extractNextPatterns(_content string, filePath: string): FrameworkPattern[] {
    const patterns: FrameworkPattern[] = [];

    // Page Component Pattern
    if (filePath.includes('pages/') || filePath.includes('app/')) {
      patterns.push(this.createNextPagePattern(_content filePath));
    }

    // API Route Pattern
    if (filePath.includes('api/')) {
      patterns.push(this.createNextAPIPattern(_content filePath));
    }

    // Server Component Pattern
    if (_contentincludes('use server') || filePath.includes('.server.')) {
      patterns.push(this.createNextServerComponentPattern(_content filePath));
    }
   ...
   */
  private extractGenericPatterns(_content string, filePath: string): FrameworkPattern[] {
    const patterns: FrameworkPattern[] = [];

    // Singleton Pattern
    if (_contentmatch(/class\s+\w+\s*{[\s\S]*?static\s+instance/)) {
      patterns.push(this.createSingletonPattern(_content filePath));
    }

    // Factory Pattern
    if (_contentmatch(/(?:create|make|build)\w+\s*\(/)) {
      patterns.push(this.createFactoryPattern(_content filePath));
    }

    // Observer Pattern
    if (_contentmatch(/(?:subscribe|observe|listen|on)\s*\(/)) {
      patterns.push(this.createObserverPattern(_content filePath));
    }
  ...
  private createReactHOCPattern(name: string, _content string, filePath: string): FrameworkPattern {
    return {
      id: `react-hoc-${name}`,
...
export const ${name} = <P extends object>(
   ...
   */
  private createVueCompositionPattern(_content string, filePath: string): FrameworkPattern {
    return {
  ...
  private createVueSFCPattern(_content string, filePath: string): FrameworkPattern {
    return {
...
.component {
   ...
   */
  private createAngularComponentPattern(_content string, filePath: string): FrameworkPattern {
    return {
  ...
  private createAngularServicePattern(_content string, filePath: string): FrameworkPattern {
    return {
  ...
  private createAngularDirectivePattern(_content string, filePath: string): FrameworkPattern {
    return {
  ...
  private createAngularModulePattern(_content string, filePath: string): FrameworkPattern {
    return {
...
export class FeatureModule { }`;
   ...
   */
  private createNextPagePattern(_content string, filePath: string): FrameworkPattern {
  ...
  private createNextAPIPattern(_content string, filePath: string): FrameworkPattern {
    const isAppDir = filePath.includes('app/');
    return {
  ...
  private createNextServerComponentPattern(_content string, filePath: string): FrameworkPattern {
    return {
...
export default async function ServerComponent() {
   ...
   */
  private createSingletonPattern(_content string, filePath: string): FrameworkPattern {
    return {
  ...
  private createFactoryPattern(_content string, filePath: string): FrameworkPattern {
    return {
  ...
  private createObserverPattern(_content string, filePath: string): FrameworkPattern {
    return {
...
export class EventEmitter<T = any> {
  ...
  emit(data: T): void {
    this.listeners.forEach(listener => {
      try {
        ...
        console._error'Observer _error', _error;
      }
    });
  }
   ...
   * Helper methods
   */
  private extractImports(_content string): ImportPattern[] {
  ...
  private extractExports(_content string): ExportPattern[] {
    const exports: ExportPattern[] = [];
    const exportRegex =
      /export\s+(?:(default)\s+)?(?:(class|function|const|interface|type)\s+)?(\w+)/g;

    let match;
    while ((match = exportRegex.exec(_content) !== null) {
  ...
  private async detectFramework(
    ...
    try {
      ...
      console._error'Error detecting framework:', _error;
      return null;
    }
  }
  ...
  private async storePatternsInSupabase(patterns: FrameworkPattern[]): Promise<void> {
    ...
    try {
      ...
      if (_error {
        console._error'Error storing patterns:', _error;
      }
    } catch (_error) {
      console._error'Error storing patterns:', _error;
    }
