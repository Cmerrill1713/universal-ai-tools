...
export class RerankingPipeline {
  ...
  async search(
    ...
    try {
      ...
      this.logger.error'Pipeline search failed:', _error;
      throw _error;
    }
  }
  ...
  async searchKnowledgeEntities(
    ...
    try {
      ...
      const { data, _error} = await this.supabase.rpc('search_knowledge_entities', {
      ...
      const results = data.map((item: any) => ({
        id: item.id,
        _content `${item.name}: ${item.description || ''}`,
      ...
      this.logger.error'Knowledge entity search failed:', _error;
      throw _error;
    }
  }
  ...
  private async performTextBasedSearch(query: string, options: MultiStageSearchOptions) {
    ...
    const { data, _error} = await this.supabase.rpc('search_memories_with_context', {
      query_text: query,
  ...
  private applyQualityFilters(
    ...
    return results.filter((result) => {
      ...
      if (result._contentlength < 10) {
        return false; // Too short
      }

      if (result._contentlength > 5000) {
