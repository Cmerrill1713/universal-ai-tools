...
export class ONNXRuntime {
  ...
  async loadModel(modelId: string, config: ONNXModelConfig): Promise<void> {
    try {
      ...
      logger.error`Failed to load ONNX model ${modelId}:`, _error;
      throw _error;
    }
  }
   ...
   */
  async runInference(modelId: string, _request ONNXInferenceRequest): Promise<ONNXInferenceResult> {
    ...
    try {
      // Prepare inputs
      const feeds = await this.prepareInputs(session, _request;

      // Run inference
      const results = await session.run(feeds);

      // Process outputs
      const output = await this.processOutputs(results, _requestoutputNames);
      ...
      logger.error`ONNX inference failed for ${modelId}:`, _error;
      throw _error;
    }
  }
  ...
  private async prepareInputs(
    ...
    _request ONNXInferenceRequest
  ): Promise<ort.InferenceSession.OnnxValueMapType> {
    const feeds: ort.InferenceSession.OnnxValueMapType = {};

    if (_requestinputNames && Array.isArray(_request_input) {
      // Multiple named inputs
      _requestinputNames.forEach((name, index) => {
        if (index < _request_inputlength) {
          feeds[name] = this.createTensor(_request_inputindex]);
        }
      });
    } else if (session.inputNames.length === 1) {
      // Single input
      feeds[session.inputNames[0]] = this.createTensor(_request_input;
    } else {
      // Try to match inputs automatically
      if (typeof _request_input=== 'object' && !Array.isArray(_request_input) {
        // Input is an object with named fields
        for (const [key, value] of Object.entries(_request_input) {
