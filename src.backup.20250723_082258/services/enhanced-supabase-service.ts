...
export class EnhancedSupabaseService {
  ...
  public async uploadFile(options: FileUploadOptions): Promise<{
    ...
    try {
      ...
      // Upload file
      const { data, _error} = await this.client.storage.from(bucket).upload(path, file, {
        contentType,
        upsert: false,
        cacheControl: '3600',
      });
  ...
  public async downloadFile(bucket: string, path: string): Promise<Buffer> {
    try {
      const { data, _error} = await this.client.storage.from(bucket).download(path);
  ...
  public async listFiles(
    ...
    try {
      const { data, _error} = await this.client.storage.from(bucket).list(options?.path, {
  ...
  public async callEdgeFunction(options: EdgeFunctionCall): Promise<unknown> {
    try {
      ...
      const { data, _error} = await this.client.functions.invoke(functionName, {
        body: payload,
        headers: {
  ...
  public async semanticSearch(options: VectorSearchOptions): Promise<any[]> {
    try {
      ...
      const { data, _error} = await this.client.rpc(`search_${collection}_semantic`, {
  ...
  public async hybridSearch(
    ...
    try {
      ...
      const { data, _error} = await this.client.rpc(`hybrid_search_${collection}`, {
        text_query: query,
        query_embedding: embedding,
        match_count: limit,
  ...
  public async createSignedUploadUrl(
    ...
    try {
      const { data, _error} = await this.client.storage
        .from(bucket)
  ...
  public async verifyResourceAccess(
    ...
    try {
      const { data, _error} = await this.client.rpc('check_resource_permission', {
        user_id: userId,
        ...
        if (payload.new?.queue_name === queueName && payload.new?.status === 'pending') {
          try {
            ...
            await this.client
              ...
              .update({
                status: 'failed',
                error_message: _errorinstanceof Error ? _errormessage : String(_error,
