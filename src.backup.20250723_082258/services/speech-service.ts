...
export class SpeechService {
  ...
  async transcribeAudio(
    ...
    try {
      ...
      logger.error'Transcription _error, LogContext.AVATAR, { _error});
  ...
  private async transcribeWithOpenAI(
    ...
    try {
      ...
      logger.error'OpenAI Whisper _error, LogContext.AVATAR, { _error});
  ...
  private async transcribeWithLocalWhisper(
    ...
    try {
      ...
      logger.error'Local Whisper _error, LogContext.AVATAR, { _error});
  ...
  async synthesizeSpeech(options: SynthesisOptions): Promise<AudioResult> {
    try {
      ...
      logger.error'Synthesis _error, LogContext.AVATAR, { _error});
  ...
  private async synthesizeWithElevenLabs(options: SynthesisOptions): Promise<AudioResult> {
    ...
    try {
      ...
      logger.error'ElevenLabs synthesis _error, LogContext.AVATAR, { _error});
  ...
  private async synthesizeWithOpenAI(options: SynthesisOptions): Promise<AudioResult> {
    ...
    try {
      ...
      logger.error'OpenAI TTS _error, LogContext.AVATAR, { _error});
  ...
  private async synthesizeWithLocalTTS(options: SynthesisOptions): Promise<AudioResult> {
    try {
      ...
      logger.error'Kokoro TTS failed', LogContext.AVATAR, { _error});
  ...
  private async synthesizeWithKokoro(options: SynthesisOptions): Promise<AudioResult> {
    try {
      ...
      // Process audio with comprehensive _errorhandling
      ...
      logger.error'Kokoro TTS synthesis _error, LogContext.AVATAR, { _error});
      throw new Error(
        `Kokoro TTS failed: ${_errorinstanceof Error ? _errormessage : 'Unknown _error}`
      );
    }
  }
  ...
  async getAvailableVoices(): Promise<any[]> {
    ...
    try {
      ...
      logger.error'Failed to fetch Kokoro voices', LogContext.AVATAR, { _error});
    }
    ...
    if (this.elevenLabsApiKey) {
      try {
        ...
        logger.error'Failed to fetch ElevenLabs voices', LogContext.AVATAR, { _error});
      }
    }
  ...
  async testKokoroVoice(voiceId: string, sampleText?: string): Promise<Buffer> {
    try {
      ...
      logger.error'Kokoro voice test failed', LogContext.AVATAR, { _error});
      throw _error;
    }
  }
  ...
  async clearAllCaches(): Promise<void> {
    try {
      ...
      logger.error'Error clearing speech service caches', LogContext.AVATAR, { _error});
    }
  }
  ...
  async synthesizeSpeechWithRetry(options: SynthesisOptions, maxRetries = 2): Promise<AudioResult> {
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      try {
        ...
        lastError = _errorinstanceof Error ? _error: new Error('Unknown synthesis _error);
        logger.warn(
  ...
  async estimateAudioDuration(text: string, voiceProfile?: VoiceProfile): Promise<number> {
    try {
      ...
      logger.error'Error estimating audio duration', LogContext.AVATAR, { _error});
