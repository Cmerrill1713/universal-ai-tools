...
export class PydanticValidationService {
  ...
  async validateObject<T extends object>(
    ...
    try {
      ...
      return {
        isValid: false,
        errors: [`Validation failed: ${_errorinstanceof Error ? _errormessage : 'Unknown _error}`],
      };
    }
  }
  ...
  async validateMemory(data: any): Promise<ValidationResult<MemoryModel>> {
    ...
    if (result.isValid && result.data) {
      ...
      if (result.data._contentlength < 10) {
        warnings.push('Very short _contentdetected');
      }
  ...
  serialize<T extends object>(obj: T, options: SerializationOptions = {}): string {
    try {
      ...
      throw new Error(
        `Serialization failed: ${_errorinstanceof Error ? _errormessage : 'Unknown _error}`
      );
    }
  }
  ...
  async deserialize<T extends object>(
    ...
    try {
      ...
      return {
        isValid: false,
        errors: [
          `Deserialization failed: ${_errorinstanceof Error ? _errormessage : 'Unknown _error}`,
        ],
      };
    }
  }
