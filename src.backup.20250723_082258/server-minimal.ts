...
app.post('/api/assistant/chat', async (req, res) => {
  try {
    ...
    logger.info(`Chat _requestprocessed: ${message.substring(0, 50)}...`);
    ...
    logger.error'Chat endpoint _error', _error;

    // More detailed _errorresponse
    const errorResponse = {
      _error 'Internal server _error,
      message: 'Failed to process chat _request,
      details: _errorinstanceof Error ? _errormessage : 'Unknown _error,
      timestamp: new Date().toISOString(),
    };
...
app.post('/api/speech/synthesize/retry', (req, res) => {
  res.status(503).json({
    _error 'TTS service not available',
...
app.post('/api/speech/synthesize/kokoro', (req, res) => {
  res.status(503).json({
    _error 'Kokoro TTS not available',
