...
export class WebScraperAgent extends BaseAgent {
  ...
  protected async process(_context: AgentContext & { memoryContext?: any }): Promise<AgentResponse> {
    ...
    try {
      ...
      return {
        success: true,
        data: result,
        reasoning: `Successfully processed web ${intent.action} _request,
        confidence: 0.8,
   ...
   */
  private async parseScrapingIntent(_request string): Promise<unknown> {
...
Respond with JSON: {
  ...
  "auth": {...}
    ...
    try {
      ...
      return this.fallbackScrapingIntentParsing(_request;
    }
  }
  ...
  private async scrapeWebsite(intent: any): Promise<WebData> {
    ...
    try {
      ...
      switch (extractType) {
        case 'text':
          extractedData = this.extractText(_content selector);
          break;
        case 'html':
          extractedData = this.extractHTML(_content selector);
          break;
        case 'links':
          extractedData = this.extractLinks(_content url);
          break;
        case 'images':
          extractedData = this.extractImages(_content url);
          break;
        case 'structured':
          extractedData = await this.extractStructuredDataFromHTML(_content url);
          break;
        default:
      ...
      const webData: WebData = {
        url,
        title: this.extractTitle(_content,
        _content extractedData,
        metadata: {
          timestamp: new Date(),
          responseTime,
          statusCode: response.status,
          contentType: response.headers['_contenttype'],
          size: _contentlength,
        },
      };
      ...
      this.logger.error`Scraping failed for ${url}:`, _error;
      throw _error;
    }
  }
  ...
  private async scrapeWithBrowser(
    ...
    try {
      ...
      let _content any;
      if (selector) {
        _content= await page.locator(selector).textContent();
      } else {
        _content= await page._content);
      }
...
Return as JSON with clear structure.`;
    ...
    try {
      ...
      return {
        ...
        _content this.extractText(html),
  ...
  private fallbackScrapingIntentParsing(_request string): any {
    ...
    if (requestLower.includes('api') || requestLower.includes('_request)) {
  ...
  private async storeScrapedData(data: WebData): Promise<void> {
    try {
      await this.supabase.from('ai_memories').insert({
        ...
        _content `Scraped ${data.url}: ${data.title}`,
        metadata: data,
        timestamp: new Date().toISOString(),
      });
    } catch (_error) {
      this.logger.error'Failed to store scraped data:', _error;
    }
  }
  ...
  private async storeAPIResponse(
    ...
    try {
      await this.supabase.from('ai_memories').insert({
        ...
        _content `${method} ${url} -> ${status}`,
        metadata: { url, method, data, status },
        timestamp: new Date().toISOString(),
      });
    } catch (_error) {
      this.logger.error'Failed to store API response:', _error;
    }
  }
  ...
  private async handleGeneralWebQuery(_request string): Promise<unknown> {
