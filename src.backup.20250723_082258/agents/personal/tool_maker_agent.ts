...
export class ToolMakerAgent extends BaseAgent {
  ...
  protected async process(_context: AgentContext & { memoryContext?: any }): Promise<AgentResponse> {
    ...
    try {
      ...
      return {
        success: true,
        data: result,
        reasoning: `Successfully processed tool ${intent.action} _request,
        confidence: 0.85,
      ...
      return {
        success: false,
        data: null,
        reasoning: `Tool creation failed: ${(_erroras Error).message}`,
        confidence: 0.1,
        latencyMs: Date.now() - startTime,
        agentId: this.config.name,
   ...
   */
  private async parseToolMakingIntent(_request string): Promise<unknown> {
    const prompt = `Parse this tool creation _request
...
Respond with JSON: {
  ...
  "security": {...}
    ...
    try {
      ...
      return this.fallbackToolIntentParsing(_request;
    }
  }
...
2. Proper _errorhandling and validation
...
- Include proper _inputvalidation
- Return structured results with success/_errorstatus
...
async function scrapeWebsite(params) {
  ...
  try {
    ...
    if (selector) {
      ...
      const _content= extractWithSelector(html, selector);
      return { success: true, data: _content url };
    }
    
    return { success: true, data: html, url };
  } catch (_error) {
    return { success: false, _error _errormessage, url };
  }
}`,
...
async function customTool(params) {
  ...
  private async testTool(tool: CustomTool): Promise<unknown> {
    try {
      ...
      if (this.hasUnsafeCodePatterns(tool.implementation)) {
        return { success: false, _error 'Tool contains unsafe code patterns' };
      }
      ...
      if (!tool.implementation || typeof tool.implementation !== 'string') {
        return { success: false, _error 'Tool implementation must be a non-empty string' };
      }

      return { success: true, message: 'Tool validation passed (execution disabled for security)' };
    } catch (_error) {
      return { success: false, _error (_erroras Error).message };
    }
  }
  ...
  private async loadCustomTools(): Promise<void> {
    try {
      const { data: tools, _error} = await this.supabase
        .from('ai_custom_tools')
        .select('*')
        .eq('created_by', 'tool_maker_agent');

      if (_error {
        this.logger.error'Failed to load custom tools:', _error;
        return;
      }
      ...
      this.logger.error'Error loading custom tools:', _error;
    }
  }
  ...
  private async loadToolTemplates(): Promise<void> {
    try {
      const { data: templates, _error} = await this.supabase.from('ai_tool_templates').select('*');

      if (_error {
        this.logger.warn('Failed to load tool templates from database:', _error;
        return;
      }
  ...
  private async saveCustomTools(): Promise<void> {
    try {
      ...
      this.logger.error'Failed to save custom tools:', _error;
    }
  }
  ...
  private async saveToolToDatabase(tool: CustomTool): Promise<void> {
    try {
      ...
      this.logger.error'Failed to save tool to database:', _error;
    }
  }

  private fallbackToolIntentParsing(_request string): any {
  ...
  private async deployTool(intent: any): Promise<unknown> {
    ...
    if (!tool.metadata.tested) {
      ...
      if (!testResults.success) {
        throw new Error(`Tool ${toolId} failed testing: ${testResults._error`);
      }
   ...
   */
  private async handleGeneralToolQuery(_request string): Promise<unknown> {
    const requestLower = _requesttoLowerCase();
    ...
    try {
      const response = await axios.post('http://localhost:11434/api/generate', {
        model: 'llama3.2:3b',
        prompt: `As a tool creation assistant, respond to this query: "${_request"
