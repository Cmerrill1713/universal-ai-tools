...
export class SystemControlAgent extends BaseAgent {
  ...
  protected async process(_context: AgentContext & { memoryContext?: any }): Promise<AgentResponse> {
    ...
    try {
      // Parse the user _requestto determine system operation
      ...
      this.logger.error'SystemControlAgent processing _error', _error;
      return {
        success: false,
        data: null,
        reasoning: `System operation failed: ${(_erroras Error).message}`,
        confidence: 0.1,
        latencyMs: Date.now() - startTime,
        agentId: this.config.name,
   ...
   */
  private async parseSystemIntent(_request string): Promise<unknown> {
    const prompt = `Parse this macOS system control _request
...
Respond with JSON: {
  ...
  "context": "..."
    ...
    try {
      ...
      return this.fallbackSystemIntentParsing(_request;
    }
  }
  ...
  private async controlApplication(intent: any): Promise<unknown> {
    ...
    try {
      ...
      message = `Application control failed: ${(_erroras Error).message}`;
    }
  ...
  private async getCPUMetrics(): Promise<SystemMetrics['cpu']> {
    try {
      ...
      this.logger.error'Failed to get CPU metrics:', (_erroras Error).message);
      return {
        usage: 0,
        cores: os.cpus().length,
        model: 'Unknown',
  ...
  private async getMemoryMetrics(): Promise<SystemMetrics['memory']> {
    try {
      ...
      this.logger.error'Failed to get memory metrics:', _error;
      const total = os.totalmem();
      const free = os.freemem();
  ...
  private async getDiskMetrics(): Promise<SystemMetrics['disk']> {
    try {
      ...
      this.logger.error'Failed to get disk metrics:', _error;
      return {
        total: 0,
        used: 0,
        available: 0,
  ...
  private async launchApplication(appName: string, args: string[] = []): Promise<unknown> {
    try {
      ...
      return {
        success: false,
        message: `Failed to launch ${appName}: ${(_erroras Error).message}`,
      };
    }
  }
  ...
  private async quitApplication(appName: string): Promise<boolean> {
    try {
      ...
      this.logger.error`Failed to quit ${appName}:`, _error;
      return false;
    }
  }
  ...
  private async focusApplication(appName: string): Promise<boolean> {
    try {
      ...
      this.logger.error`Failed to focus ${appName}:`, _error;
      return false;
    }
  }
  ...
  private async hideApplication(appName: string): Promise<boolean> {
    try {
      ...
      this.logger.error`Failed to hide ${appName}:`, _error;
      return false;
    }
  }
  ...
  private async getRunningApplications(): Promise<ApplicationInfo[]> {
    try {
      ...
      this.logger.error'Failed to get running applications:', _error;
      return [];
    }
  }
  ...
  private fallbackSystemIntentParsing(_request string): any {
  ...
  private async storeSystemMetrics(metrics: SystemMetrics): Promise<void> {
    try {
      await this.supabase.from('ai_memories').insert({
        ...
        _content `System metrics: CPU ${metrics.cpu.usage}%, Memory ${Math.round((metrics.memory.used / metrics.memory.total) * 100)}%`,
        metadata: metrics,
        timestamp: new Date().toISOString(),
      });
    } catch (_error) {
      this.logger.error'Failed to store system metrics:', (_erroras Error).message);
    }
  }
  ...
  private async handleGeneralSystemQuery(_request string): Promise<unknown> {
