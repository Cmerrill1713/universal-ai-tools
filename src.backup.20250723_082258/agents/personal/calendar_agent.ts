...
export class CalendarAgent extends BaseAgent {
  ...
  protected async process(_context: AgentContext & { memoryContext?: any }): Promise<AgentResponse> {
    ...
    try {
      // Parse the user _requestto determine intent
      ...
      this.logger.error'CalendarAgent processing _error', _error;
      return {
        success: false,
        data: null,
        reasoning: `Calendar operation failed: ${(_erroras Error).message}`,
        confidence: 0.1,
        latencyMs: Date.now() - startTime,
        agentId: this.config.name,
   ...
   */
  private async parseCalendarIntent(_request string): Promise<unknown> {
    const prompt = `Parse this calendar _requestand extract the intent:
...
Respond with JSON: {
  ...
  "context": {...}
    ...
    try {
      ...
      return this.fallbackIntentParsing(_request;
    }
  }
   ...
   */
  private async createEventFromNaturalLanguage(_request string, intent: any): Promise<unknown> {
    const eventDetails = intent.eventDetails || {};

    // Parse date and time
    const dateTime = await this.parseDateTime(_request;

    const event: CalendarEvent = {
      title: eventDetails.title || this.extractEventTitle(_request,
    ...
    await this.storeEventMemory(createdEvent, _request;
  ...
  private async analyzeSchedule(intent: any): Promise<unknown> {
    ...
    return {
      timeframe,
      _analysis
      insights: await this.generateScheduleInsights(_analysis,
    };
  }
  ...
  private async createCalendarEvent(event: CalendarEvent): Promise<CalendarEvent> {
    try {
      ...
      if (result.includes('_error')) {
        throw new Error(`AppleScript _error ${result}`);
      }
      ...
      this.logger.error'Failed to create macOS calendar event:', _error;
      throw new Error(`Failed to create calendar event: ${(_erroras Error).message}`);
    }
  }
  ...
  private async checkForConflicts(newEvent: CalendarEvent): Promise<ScheduleConflict[]> {
    try {
      ...
      this.logger.error'Conflict checking failed:', _error;
      return [];
    }
  }
  ...
  private async getEventsInRange(startDate: Date, endDate: Date): Promise<CalendarEvent[]> {
    try {
      ...
      this.logger.error'Failed to get calendar events:', _error;
      return [];
    }
  }
    ...
    try {
      ...
      for (const eventStr of eventStrings) {
        try {
          ...
          this.logger.warn('Failed to parse individual event:', eventStr, _error;
        }
      }
    } catch (_error) {
      this.logger.error'Failed to parse AppleScript result:', _error;
    }
  ...
  private parseIndividualEvent(eventStr: string): CalendarEvent | null {
    try {
      ...
      for (const [field, _pattern of Object.entries(patterns)) {
        const match = eventStr.match(_pattern;
        if (match) {
      ...
      this.logger.error'Error parsing individual event:', _error;
      return null;
    }
  }
  ...
  private isAllDayEvent(startDate: Date, endDate: Date): boolean {
    ...
    return (
      (start === 0 && end === 0) || endDate.getTime() - startDate.getTime() >= 24 * 60 * 60 * 1000
    );
  }
...
Respond with JSON: {
  ...
  private async loadCalendarPreferences(): Promise<void> {
    try {
      ...
      if (data) {
        this.calendarPreferences = data._content
      } else {
      ...
      this.logger.error'Failed to load calendar preferences:', _error;
    }
  }
  ...
  private async checkCalendarAccess(): Promise<boolean> {
    try {
      ...
      this.logger.warn('Calendar access check failed:', _error;
      return false;
    }
  }
  ...
  private extractEventTitle(_request string): string {
    // Extract likely event title from request
    return _requestreplace(/create|schedule|book/gi, '').trim() || 'New Event';
  }
  ...
  private buildCalendarReasoning(intent: any, result: any): string {
    return `Processed calendar ${intent.action} _requestwith ${result.conflicts?.length || 0} conflicts found.`;
  }
  ...
  private async storeEventMemory(event: CalendarEvent, originalRequest: string): Promise<void> {
    try {
      await this.supabase.from('ai_memories').insert({
        ...
        _content `Created event: ${event.title} at ${event.startDate.toISOString()}`,
        metadata: { event, originalRequest },
        timestamp: new Date().toISOString(),
      });
    } catch (_error) {
      this.logger.error'Failed to store event memory:', _error;
    }
  }
  ...
  private async generateScheduleInsights(_analysis any): Promise<string> {
    const insights = [
      ...
      `${_analysistotalHours.toFixed(1)} hours of meetings`,
      `${_analysisbusyDays.length} busy days identified`,
      `${_analysisfreeTime.toFixed(1)} hours of free time remaining`,
    ];

    if (_analysispatterns.peakHours) {
      const peakHour = Object.keys(_analysispatterns.peakHours)[0];
  ...
  private async handleGeneralCalendarQuery(_request string): Promise<unknown> {
