...
export abstract class BaseAgent extends EventEmitter {
  ...
  async initialize(memoryCoordinator?: unknown): Promise<void> {
    try {
      ...
      this.logger.error(`❌ Failed to initialize agent ${this.config.name}:`, _error);
      throw _error;
    }
  }
  ...
  async execute(context: AgentContext): Promise<AgentResponse> {
    ...
    try {
      ...
      const errorResponse: AgentResponse = {
        success: false,
        data: null,
        reasoning: `Agent execution failed: ${_error instanceof Error ? _error.message : String(_error)`,
        confidence: 0,
        latencyMs,
        agentId: this.config.name,
  ...
  async shutdown(): Promise<void> {
    try {
      ...
      this.logger.error(`❌ Error during agent shutdown:`, _error);
    }
  }
  ...
  protected async loadMemory(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(`⚠️ Failed to load memory for agent ${this.config.name}:`, __error);
    }
  }
  ...
  protected async retrieveMemory(context: AgentContext): Promise<unknown> {
    ...
    try {
      ...
      this.logger.warn(`⚠️ Failed to retrieve memory:`, _error);
      return null;
    }
  }
  ...
  protected async storeMemory(context: AgentContext, result: PartialAgentResponse): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(`⚠️ Failed to store memory:`, _error);
    }
  }
