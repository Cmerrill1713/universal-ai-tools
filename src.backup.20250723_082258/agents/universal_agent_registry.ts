...
export class UniversalAgentRegistry extends EventEmitter {
  ...
  private registerPersonalAgents(): void {
    const personalAgents: AgentDefinition[] = [
      ...
      },
      {
        ...
        dependencies: ['ollama_assistant'],
        capabilities: ['organize_files', 'find_duplicates', 'analyze__content, 'smart_search'],
        memoryEnabled: true,
        maxLatencyMs: 5000,
        retryAttempts: 2,
      },
      ...
      },
      {
        ...
        dependencies: ['ollama_assistant'],
        capabilities: ['scrape_website', 'monitor_website', 'api__request],
        memoryEnabled: true,
        maxLatencyMs: 10000,
        retryAttempts: 3,
      },
  ...
  private async loadAgent(agentName: string): Promise<BaseAgent | null> {
    try {
      ...
      this.logger.error`❌ Failed to load agent '${agentName}':`, _error;
      return null;
    }
  }
  ...
  private async importAgentClass(definition: AgentDefinition): Promise<unknown> {
    try {
      ...
      this.logger.error`❌ Failed to import agent class ${definition.className}:`, _error;
      return null;
    }
  }
  ...
  /**
   * Process a _requestthrough the appropriate agent
   */
  ...
  async shutdown(): Promise<void> {
    ...
    const shutdownPromises = Array.from(this.loadedAgents.values()).map((agent) =>
      agent.shutdown().catch((_error => this.logger.error`Error shutting down agent:`, _error)
    );
