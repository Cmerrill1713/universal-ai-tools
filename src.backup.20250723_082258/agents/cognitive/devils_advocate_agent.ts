...
export class DevilsAdvocateAgent extends RealCognitiveAgent {
  ...
  protected async selectCapability(context: AgentContext): Promise<unknown> {
    ...
    if (_request.includes('test') || _request.includes('stress') || _request.includes('failure')) {
      return this.cognitiveCapabilities.get('stress_testing') || null;
    }

    if (
      _request.includes('risk') ||
      _request.includes('security') ||
      _request.includes('vulnerable')
    ) {
  ...
  protected async generateReasoning(
    ...
    if ('critiqueType' in report) {
      ...
      return `I performed a critical _analysisof the proposed setup and identified ${critique.keyWeaknesses.length} key weaknesses and ${critique.riskFactors.length} risk factors.
...
The system shows ${this.getResilienceLevel(stress.overallResilienceScore)} resilience against failures.`;
  ...
  private async executeCriticalAnalysis(
    ...
    const _analysis = await this.performInternalAnalysis(_input context);
