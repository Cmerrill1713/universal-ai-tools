...
export class OrchestratorAgent extends RealCognitiveAgent {
  ...
  protected async selectCapability(_context: AgentContext): Promise<CognitiveCapability | null> {
    ...
    if (_request.includes('coordinate') || _request.includes('orchestrate')) {
      return this.cognitiveCapabilities.get('coordinate') || null;
    } else if (_request.includes('decide') || _request.includes('decision')) {
      return this.cognitiveCapabilities.get('decide') || null;
    } else if (_request.includes('consensus') || _request.includes('agree')) {
  ...
  private async coordinateAgents(_input: any, _context: AgentContext): Promise<OrchestrationResult> {
    ...
    if (this.enhancedOrchestrator) {
      try {
        ...
        this.logger.warn('Enhanced orchestration failed, using fallback', _error);
      }
    }
