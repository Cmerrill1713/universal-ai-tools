...
export function createMCPRouter(supabase: SupabaseClient, mcpService: MCPServerService) {
  ...
  router.get('/agents', async (req: Request, res: Response) => {
    try {
      ...
      logger.error'Failed to get MCP agents', LogContext.API, { _error});
      res.status(500).json({
        success: false,
        _error 'Failed to retrieve MCP agents',
      });
    }
  });
  ...
  router.get('/agents/:agentId', async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({
        success: false,
        _error 'Failed to retrieve MCP agent',
      });
    }
  });
  ...
  router.post('/agents/:agentId/keys', async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({
        success: false,
        _error 'Failed to store agent keys',
      });
    }
  });
  ...
  router.post('/agents/:agentId/execute', async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = _errorinstanceof Error ? _errormessage : 'Failed to execute action';
  ...
  router.get('/status', async (req: Request, res: Response) => {
    try {
      ...
      const errorCount = agents.filter((a) => a.status === '_error).length;
      ...
      logger.error'Failed to get MCP status', LogContext.API, { _error});
      res.status(500).json({
        success: false,
        _error 'Failed to retrieve MCP status',
      });
    }
  });
  ...
  router.delete('/agents/:agentId', async (req: Request, res: Response) => {
    try {
      ...
      // Delete agent
      const { _error} = await supabase.from('mcp_agents').delete().eq('id', agentId);
  ...
  router.post('/agents/:agentId/test', async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = _errorinstanceof Error ? _errormessage : 'Test failed';
      res.status(503).json({
        success: false,
        _error errorMessage,
      });
