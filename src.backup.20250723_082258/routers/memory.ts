...
export function MemoryRouter(supabase: SupabaseClient) {
  ...
  // Enhanced validation middleware with proper _errorresponses
  const validateMemoryStore = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    if (typeof _content!== 'string' || _contentlength === 0) {
      return sendError(
        res,
  ...
  router.post('/', validateMemoryStore, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // Generate embedding if _contentis provided
      let embedding: number[] | null = null;
      try {
        const embeddingResult = await supabase.rpc('ai_generate_embedding', {
          _content memoryData._content
        });
        embedding = embeddingResult.data;
      } catch (embeddingError) {
        logger.warn('Failed to generate embedding, storing without it', LogContext.API, {
          _error embeddingError instanceof Error ? embeddingError.message : String(embeddingError),
          _content memoryData._contentsubstring(0, 100),
        });
      }
      ...
      logger.info('Memory stored successfully', LogContext.API, {
        memoryId: memory.id,
        contentLength: memory._contentlength,
        hasEmbedding: !!embedding,
      });

      sendSuccess(res, memory, 201);
    } catch (_error Error | unknown) {
      logger.error'Store memory _error, LogContext.API, {
        _error _errormessage,
        stack: _errorstack,
      });
      sendError(
        ...
        'An unexpected _erroroccurred while storing memory',
        500,
        _errormessage
      );
    }
  ...
  router.get('/', async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (_error {
        logger.error'Failed to retrieve memories', LogContext.API, { _error _errormessage });
        return sendError(
          res,
          'MEMORY_STORAGE_ERROR' as ErrorCode,
      ...
      logger.error'Retrieve memories _error, LogContext.API, {
        _error _errormessage,
        stack: _errorstack,
      });
      sendError(
        res,
        'INTERNAL_SERVER_ERROR' as ErrorCode,
        'An unexpected _erroroccurred while retrieving memories',
        500,
        _errormessage
      );
    }
  ...
  router.post('/search', validateMemorySearch, async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      const { data: embedding } = await supabase.rpc('ai_generate_embedding', {
        _content searchParams.query,
      });

      // Perform vector search
      const { data, _error} = await supabase.rpc('search_memories', {
      ...
      logger.error'Search memories _error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });
      res.status(400).json({
        success: false,
        _error {
          ...
          message: _error.message || 'Unknown _error,
          details: _error
        },
        metadata: {
          requestId: req.id,
  ...
  router.put('/:id/importance', async (req: AuthenticatedRequest, res) => {
    try {
      ...
      logger.error'Update memory importance _error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });
      res.status(400).json({ _error _error.message || 'Failed to update memory importance' });
    }
  });
