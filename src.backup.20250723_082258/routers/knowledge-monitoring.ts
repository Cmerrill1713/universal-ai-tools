...
export default function createKnowledgeMonitoringRouter(supabase: SupabaseClient) {
  ...
  router.get('/dashboard', async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ _error 'Failed to fetch dashboard data' });
    }
  });
  ...
  router.get('/sources', async (_req, res) => {
    try {
      ...
      logger.error'Error fetching source status:', _error;
      res.status(500).json({ _error 'Failed to fetch source status' });
    }
  });
  ...
  router.get('/alerts', async (req, res) => {
    try {
      ...
      const { data: alerts, _error} = await query;
      ...
      logger.error'Error fetching alerts:', _error;
      res.status(500).json({ _error 'Failed to fetch alerts' });
    }
  });
  ...
  router.put('/alerts/:id', async (req, res) => {
    try {
      ...
      logger.error'Error updating alert:', _error;
      res.status(500).json({ _error 'Failed to update alert' });
    }
  });
  ...
  router.get('/performance', async (req, res) => {
    try {
      ...
      const { data: metrics, _error} = await supabase
      ...
      logger.error'Error fetching performance metrics:', _error;
      res.status(500).json({ _error 'Failed to fetch performance metrics' });
    }
  });
  ...
  router.get('/usage-patterns', async (_req, res) => {
    try {
      ...
      logger.error'Error fetching usage patterns:', _error;
      res.status(500).json({ _error 'Failed to fetch usage patterns' });
    }
  });
  ...
  router.get('/update-status', async (_req, res) => {
    try {
      ...
      logger.error'Error fetching update status:', _error;
      res.status(500).json({ _error 'Failed to fetch update status' });
    }
  });
  ...
  router.post('/manual-update', async (req, res) => {
    try {
      ...
      if (!sourceId || !url) {
        return res.status(400).json({ _error 'sourceId and url are required' });
      }
      ...
      logger.error'Error queuing manual update:', _error;
      res.status(500).json({ _error 'Failed to queue update' });
    }
  });
  ...
  router.get('/quality-trends', async (req, res) => {
    try {
      ...
      const { data: knowledge, _error} = await query.limit(1000);
      ...
      logger.error'Error fetching quality trends:', _error;
      res.status(500).json({ _error 'Failed to fetch quality trends' });
    }
  });
  ...
  router.get('/relationships', async (req, res) => {
    try {
      ...
      const { data: relationships, _error} = await supabase
      ...
      logger.error'Error fetching relationships:', _error;
      res.status(500).json({ _error 'Failed to fetch relationships' });
    }
  });
  ...
  function determineSourceStatus(
    ...
    issueCount: number,
    enabled: boolean
  ): 'healthy' | 'warning' | '_error | 'disabled' {
