...
export function createDSPyToolsRouter() {
  ...
  router.get('/tools', async (req: Request, res: Response) => {
    try {
      ...
      logger.error'Failed to get DSPy tools', LogContext.API, { _error});
      res.status(500).json({
        success: false,
        _error 'Failed to retrieve DSPy tools',
      });
    }
  });
  ...
  router.get('/categories', async (req: Request, res: Response) => {
    try {
      ...
      logger.error'Failed to get DSPy categories', LogContext.DSPY, { _error});
      res.status(500).json({
        success: false,
        _error 'Failed to retrieve categories',
      });
    }
  });
