...
router.post('/upload', upload.single('file'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
      return res.status(400).json({ _error 'No file provided' });
    }

    const validation = FileUploadSchema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({ _error validation._error});
    }
    ...
    logger.error'File upload failed:', LogContext.API, { _error});
    res.status(500).json({ _error 'File upload failed' });
  }
});
...
router.post('/upload-multiple', upload.array('files', 10), async (req: Request, res: Response) => {
  try {
    ...
    if (!files || files.length === 0) {
      return res.status(400).json({ _error 'No files provided' });
    }
    ...
    logger.error'Multiple file upload failed:', LogContext.API, { _error});
    res.status(500).json({ _error 'Multiple file upload failed' });
  }
});
...
router.post('/upload-url', async (req: Request, res: Response) => {
  try {
    ...
    if (!bucket || !path) {
      return res.status(400).json({ _error 'Bucket and path required' });
    }
    ...
    logger.error'Failed to create upload URL:', LogContext.API, { _error});
    res.status(500).json({ _error 'Failed to create upload URL' });
  }
});
...
router.post('/process/:fileId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error'File processing failed:', LogContext.API, { _error});
    res.status(500).json({ _error 'File processing failed' });
  }
});
...
router.get('/process/status/:jobId', async (req: Request, res: Response) => {
  try {
    ...
    const { data, _error} = await enhancedSupabase.client
      .from('job_queue')
      .select('*')
      .eq('id', jobId)
      .single();
    ...
    if (_error|| !data) {
      return res.status(404).json({ _error 'Job not found' });
    }
    ...
    res.json({
      ...
      data: {
        status: data.status,
        result: data.result,
        _error data.error_message,
        startedAt: data.started_at,
        completedAt: data.completed_at,
      },
    });
  } catch (_error) {
    logger.error'Failed to get job status:', LogContext.API, { _error});
    res.status(500).json({ _error 'Failed to get job status' });
  }
});
...
router.post('/search/semantic', async (req: Request, res: Response) => {
  try {
    ...
    logger.error'Semantic search failed:', LogContext.DATABASE, { _error});
    res.status(500).json({ _error 'Semantic search failed' });
  }
});
...
router.post('/search/hybrid', async (req: Request, res: Response) => {
  try {
    ...
    if (!collection || !query) {
      return res.status(400).json({ _error 'Collection and query required' });
    }
    ...
    logger.error'Hybrid search failed:', LogContext.DATABASE, { _error});
    res.status(500).json({ _error 'Hybrid search failed' });
  }
});
...
router.post('/memory', async (req: Request, res: Response) => {
  try {
    ...
    if (!type || !_content {
      return res.status(400).json({ _error 'Type and _contentrequired' });
    }
    ...
    logger.error'Failed to store memory:', LogContext.MEMORY, { _error});
    res.status(500).json({ _error 'Failed to store memory' });
  }
});
...
router.post('/memory/search', async (req: Request, res: Response) => {
  try {
    ...
    if (!query) {
      return res.status(400).json({ _error 'Query required' });
    }
    ...
    logger.error'Memory search failed:', LogContext.MEMORY, { _error});
    res.status(500).json({ _error 'Memory search failed' });
  }
});
...
router.post('/realtime/subscribe', async (req: Request, res: Response) => {
  try {
    ...
    if (!channel) {
      return res.status(400).json({ _error 'Channel required' });
    }
    ...
    logger.error'Failed to setup realtime subscription:', LogContext.WEBSOCKET, { _error});
    res.status(500).json({ _error 'Failed to setup realtime subscription' });
  }
});
...
router.post('/realtime/broadcast', async (req: Request, res: Response) => {
  try {
    ...
    if (!channel || !event) {
      return res.status(400).json({ _error 'Channel and event required' });
    }
    ...
    logger.error'Failed to broadcast message:', LogContext.WEBSOCKET, { _error});
    res.status(500).json({ _error 'Failed to broadcast message' });
  }
});
...
router.use((_error Error, req: Request, res: Response, next: NextFunction) => {
  logger.error'Router _error', LogContext.ERROR, { _error _errormessage, stack: _errorstack });
  res.status(500).json({
