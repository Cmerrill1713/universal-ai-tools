...
export function ToolRouter(supabase: SupabaseClient) {
  ...
  router.post(
    ...
    async (req: any, res) => {
      ...
      try {
        ...
        // Get tool definition
        const { data: tool, _error toolError } = await supabase
        ...
        switch (tool.implementation_type) {
          case 'sql':
            // Execute SQL query
            const { data, _error} = await supabase.rpc('execute_dynamic_sql', {
          ...
          case 'api':
            ...
            try {
              ...
              throw new Error(`API call failed: ${_errormessage}`);
            }
            break;
        ...
        logger.error'Tool execution _error', _error;
  ...
  router.get('/', CommonValidators.pagination, async (req: any, res) => {
    try {
      ...
      logger.error'List tools _error', _error;
      res.status(500).json({ _error 'Failed to list tools' });
    }
  });
  ...
  router.post('/', async (req: any, res) => {
    try {
      ...
      logger.error'Create tool _error', _error;
      res.status(400).json({ _error _errormessage });
    }
  });
  ...
  router.post(
    ...
    async (req: any, res) => {
      try {
        ...
        logger.error'Built-in tool _error', _error;
        res.status(400).json({ _error _errormessage });
      }
    }
  );
...
async function storeContext(supabase: SupabaseClient, serviceId: string, params: any) {
  const { context_type, context_key, _content metadata } = params;
...
async function searchKnowledge(supabase: SupabaseClient, params: any) {
  ...
  if (query) {
    queryBuilder = queryBuilder.textSearch('_content, query);
  }

  const { data, _error} = await queryBuilder.limit(limit);
...
async function sendCommunication(supabase: SupabaseClient, fromServiceId: string, params: any) {
  const { to_service, message_type, _content thread_id } = params;

  // Find target service
  const { data: targetService, _error serviceError } = await supabase
    .from('ai_services')
    .select('id')
