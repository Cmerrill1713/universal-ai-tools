...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!agentId || !taskType || !context) {
      return res.status(400).json({ _error 'Missing required fields: agentId, taskType, context' });
    }
    ...
    try {
      ...
      logger.error'Failed to submit task:', _error;
      return res.status(500).json({ _error 'Failed to submit task' });
    }
  })
);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!status) {
        return res.status(404).json({ _error 'Task not found' });
      }

      return res.status(200).json( status);
    } catch (_error) {
      logger.error'Failed to get task status:', _error;
      return res.status(500).json({ _error 'Failed to get task status' });
    }
  })
);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      logger.error'Failed to get global status:', _error;
      return res.status(500).json({ _error 'Failed to get global status' });
    }
  })
);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!evolution) {
        return res.status(404).json({ _error 'Agent not found' });
      }

      return res.status(200).json( evolution);
    } catch (_error) {
      logger.error'Failed to get agent evolution:', _error;
      return res.status(500).json({ _error 'Failed to get agent evolution' });
    }
  })
);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error'Failed to get cross-learning history:', _error;
      return res.status(500).json({ _error 'Failed to get cross-learning history' });
    }
  })
);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error'Failed to trigger evolution:', _error;
      return res.status(500).json({ _error 'Failed to trigger evolution' });
    }
  })
);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      logger.error'Failed to get insights:', _error;
      return res.status(500).json({ _error 'Failed to get insights' });
    }
  })
);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!Array.isArray(tasks)) {
      return res.status(400).json({ _error 'Tasks must be an array' });
    }
    ...
    try {
      ...
      logger.error'Failed to submit batch tasks:', _error;
      return res.status(500).json({ _error 'Failed to submit batch tasks' });
    }
  })
);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { data: patterns, _error} = await supabase
      ...
      logger.error'Failed to get patterns:', _error;
      return res.status(500).json({ _error 'Failed to get patterns' });
    }
  })
);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { data: metrics, _error} = await query
      ...
      return res.status(500).json({ _error 'Failed to get metrics' });
    }
  })
);
...
function aggregateMetrics(metrics: any[]): any {
  ...
  const errorCount = metrics.filter((m) => m._error.length;
