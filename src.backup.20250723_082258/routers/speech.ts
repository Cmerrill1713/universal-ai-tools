...
export function SpeechRouter(supabase: SupabaseClient) {
  ...
  router.post('/transcribe', upload.single('audio'), async (req: any, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ _error 'No audio file provided' });
      }
      ...
      logger.error'Transcription _error', _error;
      ...
      res.status(500).json({
        _error 'Failed to transcribe audio',
  ...
  router.post('/synthesize', async (req: any, res) => {
    try {
      ...
      logger.error'Synthesis _error', _error;
      res.status(500).json({
        _error 'Failed to synthesize speech',
        details: _errormessage,
      });
    }
  });
  ...
  router.post(
    ...
    async (req: any, res) => {
      ...
      try {
        ...
        logger.error'Kokoro synthesis _error', _error;
        res.status(500).json({
          success: false,
          _error {
  ...
  router.get('/voices', async (req: any, res) => {
    try {
      ...
      logger.error'Error fetching voices:', _error;
      res.status(500).json({
        _error 'Failed to fetch available voices',
        details: _errormessage,
      });
    }
  });
  ...
  router.post('/configure-voice', async (req: any, res) => {
    try {
      ...
      logger.error'Voice configuration _error', _error;
      res.status(500).json({
        _error 'Failed to configure voice',
        details: _errormessage,
      });
    }
  });
  ...
  router.get('/history/:conversation_id', async (req: any, res) => {
    try {
      ...
      logger.error'Error fetching voice history:', _error;
      res.status(500).json({
        _error 'Failed to fetch voice history',
        details: _errormessage,
      });
    }
  });
  ...
  router.get('/health', async (req: any, res) => {
    try {
      ...
      logger.error'Error checking speech service health:', _error;
      res.status(500).json({
        success: false,
        status: 'unhealthy',
        _error {
  ...
  router.post('/test/kokoro/:voiceId', async (req: any, res) => {
    try {
      ...
      logger.error'Kokoro voice test _error', _error;
      res.status(500).json({
        success: false,
        _error {
  ...
  router.post('/admin/clear-cache', async (req: any, res) => {
    try {
      ...
      logger.error'Error clearing caches:', _error;
      res.status(500).json({
        success: false,
        _error {
  ...
  router.post('/synthesize/retry', async (req: any, res) => {
    try {
      ...
      logger.error'Synthesis with retry _error', _error;
      res.status(500).json({
        _error 'Failed to synthesize speech with retry',
        details: _errormessage,
      });
    }
  });
  ...
  router.post('/estimate-duration', async (req: any, res) => {
    try {
      ...
      logger.error'Duration estimation _error', _error;
      res.status(500).json({
        success: false,
        _error {
