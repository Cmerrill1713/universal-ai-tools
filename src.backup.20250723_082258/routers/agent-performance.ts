...
router.get('/summary', async (req: Request, res: Response) => {
  try {
    ...
    logger.error'Failed to fetch agent performance summary', LogContext.API, { _error});
    res.status(500).json({
      success: false,
      _error 'Failed to fetch performance summary',
    });
  }
});
...
router.get('/metrics', async (req: Request, res: Response) => {
  try {
    ...
    const { data, _error} = await supabaseQuery
    ...
    if (_errorinstanceof z.ZodError) {
      res.status(400).json({
        success: false,
        _error 'Invalid query parameters',
        details: _errorerrors,
      });
    } else {
      logger.error'Failed to fetch performance metrics', LogContext.API, { _error});
      res.status(500).json({
        success: false,
...
router.get('/trends', async (req: Request, res: Response) => {
  try {
    ...
    if (_errorinstanceof z.ZodError) {
      ...
      logger.error'Failed to fetch performance trends', LogContext.API, { _error});
      res.status(500).json({
        success: false,
        _error 'Failed to fetch performance trends',
      });
    }
  }
});
...
router.get('/alerts', async (req: Request, res: Response) => {
  try {
    ...
    if (_errorinstanceof z.ZodError) {
      ...
      logger.error'Failed to fetch performance alerts', LogContext.API, { _error});
      res.status(500).json({
        success: false,
...
router.post('/alerts/:alertId/resolve', async (req: Request, res: Response) => {
  try {
    ...
    logger.error'Failed to resolve alert', LogContext.API, { _error});
    res.status(500).json({
      success: false,
  ...
  try {
    ...
    if (!agentIds) {
      return res.status(400).json({
        success: false,
        _error 'agentIds query parameter is required',
      });
    }
    ...
    logger.error'Failed to compare agents', LogContext.API, { _error});
    res.status(500).json({
      success: false,
      _error 'Failed to compare agents',
    });
  }
});
...
router.get('/benchmarks', async (req: Request, res: Response) => {
  try {
    ...
    const { data, _error} = await query.order('complexity_level', { ascending: true });
    ...
    logger.error'Failed to fetch benchmarks', LogContext.API, { _error});
    res.status(500).json({
      success: false,
      _error 'Failed to fetch benchmarks',
    });
  }
});
...
router.put('/benchmarks', async (req: Request, res: Response) => {
  try {
    ...
    const { _error} = await supabase.from('agent_performance_benchmarks').upsert({
      agent_type,
      task_type,
      complexity_level,
      expected_execution_time,
    ...
    logger.error'Failed to update benchmark', LogContext.API, { _error});
    res.status(500).json({
      success: false,
      _error 'Failed to update benchmark',
    });
  }
});
...
router.post('/aggregate', async (req: Request, res: Response) => {
  try {
    ...
    if (!period || !['minute', 'hour', 'day', 'week', 'month'].includes(period)) {
      return res.status(400).json({
        success: false,
        _error 'Invalid period. Must be one of: minute, hour, day, week, month',
      });
    }

    const { _error} = await supabase.rpc('aggregate_performance_metrics', {
      p_period: period,
    });
    ...
    logger.error'Failed to aggregate metrics', LogContext.API, { _error});
    res.status(500).json({
      success: false,
      _error 'Failed to aggregate metrics',
    });
  }
});
