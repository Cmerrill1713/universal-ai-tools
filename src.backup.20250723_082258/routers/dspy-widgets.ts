...
export function DSPyWidgetsRouter(supabase: SupabaseClient) {
  ...
  router.post('/generate', async (req: any, res) => {
    try {
      ...
      logger.info(`🎯 Widget generation _request ${requestId}`, LogContext.API, {
      ...
      logger.error'Widget generation _request_error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });
      ...
      if (_errorinstanceof z.ZodError) {
        res.status(400).json({
          success: false,
          _error 'Invalid _requestformat',
          details: _errorerrors,
        });
      } else {
        res.status(500).json({
          success: false,
          _error 'Widget generation failed',
  ...
  router.post('/improve', async (req: any, res) => {
    try {
      ...
      logger.info(`🔄 Widget improvement _request ${requestId}`, LogContext.API, {
      ...
      logger.error'Widget improvement _error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });
      ...
      if (_errorinstanceof z.ZodError) {
        ...
        res.status(500).json({
          success: false,
          _error 'Widget improvement failed',
          message: _errorinstanceof Error ? _errormessage : 'Unknown _error,
        });
      }
    }
  ...
  router.get('/progress/:widgetId', async (req: any, res) => {
    try {
      ...
      // Check if this is a generation _requestID
      const { data: generation, _error genError } = await supabase
        .from('ai_widget_generations')
        .select('*')
        .eq('id', widgetId)
        .single();
      ...
      if (progress) {
        ...
        if (widget) {
          ...
          res.status(404).json({
            success: false,
            _error 'Widget generation not found',
          });
        }
      }
    } catch (_error) {
      logger.error'Progress check _error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });

      res.status(500).json({
        success: false,
        _error 'Failed to get widget progress',
      });
    }
  });
  ...
  router.get('/', async (req: any, res) => {
    try {
      ...
      logger.error'Widget list _error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });

      res.status(500).json({
        success: false,
        _error 'Failed to retrieve widgets',
      });
    }
  });
  ...
  router.get('/:widgetId', async (req: any, res) => {
    try {
      ...
      logger.error'Widget retrieval _error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });

      res.status(500).json({
        success: false,
        _error 'Failed to retrieve widget',
      });
    }
  });
  ...
  router.delete('/:widgetId', async (req: any, res) => {
    try {
      ...
      logger.error'Widget deletion _error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });
  ...
  router.get('/status/active', async (req: any, res) => {
    try {
      ...
      logger.error'Active generations _error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });

      res.status(500).json({
        success: false,
        _error 'Failed to get active generations',
      });
    }
  });
  ...
  router.get('/status/health', async (req: any, res) => {
    try {
      ...
      logger.error'Health check _error', LogContext.API, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });
