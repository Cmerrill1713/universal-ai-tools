...
describe('Local LLM Services', () => {
  ...
  beforeEach(() => {
    ...
    mockFetch.mockImplementation((url: any) => {
      ...
      if (
        ...
        (urlStr.includes('v1/completions') || urlStr.includes('v1/chat/completions'))
      ) {
        return Promise.resolve(
          createMockResponse(
            {
  ...
  describe('Ollama Service', () => {
    ...
    it('should handle generation _request, async () => {
  ...
  describe('LM Studio Service', () => {
              ...
              choices: [
                {
                  ...
                  message: { _content 'const result = a + b;' },
                },
              ],
    ...
    it('should handle completion _request, async () => {
      ...
      expect(result).toHaveProperty('_content);
    ...
    it('should handle streaming', async () => {
      const mockStream = new ReadableStream({
        start(controller) {
          controller.enqueue(
            new TextEncoder().encode('data: {"choices":[{"delta":{"_content:"Hello"}}]}\n\n')
          );
          controller.enqueue(
            new TextEncoder().encode('data: {"choices":[{"delta":{"_content:" world"}}]}\n\n')
          );
  ...
  describe('Local LLM Manager', () => {
    ...
    it('should respect service preference', async () => {
      ...
      expect(result._content.toBe('LM Studio response');
    });
    ...
    it('should handle model prefix in model name', async () => {
      ...
      expect(result._content.toBe('Generated response');
    });
  ...
  describe('Error Handling', () => {
    it('should handle network errors gracefully', async () => {
      mockFetch.mockRejectedValue(new Error('Network _error));
    ...
    it('should throw when no service is available', async () => {
      ...
      await expect(manager.generate({ prompt: 'Test' })).rejects.toThrow(
  ...
  describe('Performance', () => {
    ...
    it('should handle concurrent requests', async () => {
      const promises = Array(5)
        .fill(null)
        .map((_, i) => localLLMManager.generate({ prompt: `Test ${i}` }));

      const results = await Promise.all(promises);
      expect(results).toHaveLength(5);
      expect(results.every((r: any) => r._content).toBe(true);
    });
  });
});
