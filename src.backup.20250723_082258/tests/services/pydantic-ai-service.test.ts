...
describe('PydanticAIService', () => {
  ...
  describe('Request validation', () => {
    it('should validate AI _requeststructure', () => {
      const validRequest = {
        prompt: 'Test prompt',
        context: {
          userId: 'user123',
  ...
  describe('Basic AI requests', () => {
    it('should process a simple AI _request, async () => {
      const mockResponse = {
        success: true,
      ...
      expect(response._content.toBe('Test response');
    ...
    it('should handle _requesterrors gracefully', async () => {
      ...
      expect(response._content.toContain('Request failed');
    });
  });
  ...
  describe('Caching', () => {
    it('should cache successful responses', async () => {
      ...
      const _request= { prompt: 'Cache test' };

      // First request
      const response1 = await service._request_request;
      expect(response1.metadata.cacheHit).toBe(false);

      // Second _requestshould be cached
      const response2 = await service._request_request;
      expect(response2.metadata.cacheHit).toBe(true);
      expect(response2._content.toBe(response1._content;
    ...
    it('should not cache failed responses', async () => {
      ...
      const _request= { prompt: 'Error then success' };

      // First _requestfails
      const response1 = await service._request_request;
      expect(response1.success).toBe(false);

      // Second _requestshould not use cache
      const response2 = await service._request_request;
    ...
    it('should clear cache on demand', async () => {
      ...
      const _request= { prompt: 'Clear cache test' };

      // Cache a response
      await service._request_request;
      ...
      expect(mockDSPyService.manageKnowledge).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          memory: expect.objectContaining({
            _content expect.stringContaining('Store this in memory'),
