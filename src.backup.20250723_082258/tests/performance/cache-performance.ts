...
export class CachePerformanceTester extends EventEmitter {
  ...
  public async runPerformanceTest(options: {
    ...
    try {
      ...
      logger.error'Cache performance test failed:', _error;
    ...
    try {
      ...
      const averageKeySize =
        setOperations.length > 0
          ? setOperations.reduce((sum, m) => sum + m.keySize, 0) / setOperations.length
          : 0;
      const averageValueSize =
        setOperations.length > 0
          ? setOperations.reduce((sum, m) => sum + m.valueSize, 0) / setOperations.length
          : 0;
      ...
      logger.error'Failed to get memory usage:', _error;
      return {
        used: 0,
        peak: 0,
        keyCount: 0,
  ...
  private async getEvictionMetrics(): Promise<{
    ...
    try {
      ...
      logger.error'Failed to get eviction metrics:', _error;
      return {
        evictedKeys: 0,
        evictionRate: 0,
        memoryPressure: 0,
  ...
  private calculateAggregatedMetrics(testDuration: number) {
    ...
    const hitRate =
      getOperations.length > 0
        ? (getOperations.filter((op) => op.hit).length / getOperations.length) * 100
        : 0;
  ...
  private async cleanupTestData(): Promise<void> {
    try {
      ...
      logger.error'Failed to cleanup test data:', _error;
    }
  }
