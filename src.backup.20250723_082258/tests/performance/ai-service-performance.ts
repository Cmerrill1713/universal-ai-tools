...
export class AIServicePerformanceTester extends EventEmitter {
  ...
  public async runAIPerformanceTest(options: {
    ...
    try {
      ...
      const requestInterval =
        options.ramp_up_time > 0 ? (options.ramp_up_time * 1000) / options.concurrent_requests : 0;
      ...
      logger.error'AI service performance test failed:', _error;
  ...
  private async preloadModels(models: string[]): Promise<void> {
    ...
    for (const model of models) {
      ...
      try {
        ...
        logger.warn(`Failed to preload model ${model}:`, _error;
  ...
  private async runConcurrentAIRequests(
    ...
    while (Date.now() < endTime && this.isRunning) {
      ...
      // Select random model and _requesttype
      const model = models[Math.floor(Math.random() * models.length)];
      const requestType = requestTypes[Math.floor(Math.random() * requestTypes.length)];

      // Generate _requestbased on pattern
  ...
  private generateAIRequest(patterns: any): { prompt: string; expectedTokens: number } {
    ...
    if (rand < patterns.small_requests) {
      ...
      // Large _request(200-1000 tokens)
      return {
  ...
  private buildPayload(
    ...
    switch (requestType) {
      ...
      case 'chat':
        return {
          ...basePayload,
          messages: [{ role: 'user', _content prompt }],
          options: {
            num_predict: expectedTokens,
            temperature: 0.7,
          },
  ...
  private analyzeAIPerformanceResults(testDuration: number): AIModelPerformanceResult {
    ...
    const resource_utilization = {
      ...
      memory_per__request this.calculateAverage(memoryUsages.filter((m) => m > 0)),
  ...
  private calculateScalingEfficiency(): number {
    ...
    const baseMetrics = this.metrics.filter(
      (m) => m.concurrent_requests === baseLevel && m.success
    );
