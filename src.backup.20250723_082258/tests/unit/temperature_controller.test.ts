...
mockSelect.mockResolvedValue({ data: [], _error null });
mockUpsert.mockResolvedValue({ _error null });
...
describe('TemperatureController', () => {
  ...
  describe('Context-Based Adjustments', () => {
    it('should adjust temperature based on complexity', async () => {
      const baseParams = await temperatureController.getOptimalParams('_analysis);
      const lowComplexity = await temperatureController.getOptimalParams('_analysis, {
        complexity: 'low',
      });
      const highComplexity = await temperatureController.getOptimalParams('_analysis, {
        complexity: 'high',
      });
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle null quality scores in recordResult', async () => {
      // Should not throw
      await expect(temperatureController.recordResult('general', 0.5, true)).resolves.not.toThrow();
    ...
    it('should handle database errors gracefully', async () => {
      mockFrom.mockReturnValueOnce({
        select: jest.fn().mockResolvedValue({ data: null, _error new Error('DB Error') }),
      });
  ...
  describe('Persistence and Loading', () => {
    it('should load existing metrics on initialization', async () => {
      ...
      mockSelect.mockResolvedValueOnce({ data: mockData, _error null });
    ...
    it('should handle save errors gracefully', async () => {
      mockUpsert.mockResolvedValueOnce({ _error new Error('Save failed') });
  ...
  describe('Complex Context Handling', () => {
    it('should handle multiple context factors simultaneously', async () => {
      const params = await temperatureController.getOptimalParams('_analysis, {
        complexity: 'high',
        userPreference: 0.35,
        previousAttempts: 2,
        qualityRequirement: 'quality',
  ...
  describe('Learning and Optimization', () => {
    it('should update optimal temperature using gradient descent', async () => {
      ...
      const rec1 = temperatureController.getRecommendations().find((r) => r.taskType === 'general');
      ...
      const rec2 = temperatureController.getRecommendations().find((r) => r.taskType === 'general');
    ...
    it('should maintain quality score with exponential moving average', async () => {
      ...
      const rec = temperatureController.getRecommendations().find((r) => r.taskType === 'general');
