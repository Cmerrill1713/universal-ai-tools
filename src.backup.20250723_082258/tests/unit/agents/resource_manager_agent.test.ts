...
describe('ResourceManagerAgent', () => {
  ...
  describe('resource allocation', () => {
    it('should allocate resources for valid requests', async () => {
      const _input= 'allocate 100 cores of compute for data processing';
    ...
    it('should handle priority allocations', async () => {
      const _input= 'urgently need 500MB memory for critical task';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should enforce resource limits', async () => {
      const _input= 'allocate 50000 cores of compute'; // Exceeds capacity
      const response = await agent.processInput(_input mockContext);
  ...
  describe('resource release', () => {
    it('should release allocated resources', async () => {
      // First allocate
      const allocResponse = await agent.processInput('allocate 200 cores of compute', mockContext);
    ...
    it('should process pending requests after release', async () => {
      ...
      // Queue a request
      const pendingResponse = await agent.processInput('allocate 200 cores', mockContext);
      ...
      // Check if pending _requestwas processed
      await waitFor(50);
  ...
  describe('resource status and monitoring', () => {
    ...
    it('should track resource metrics', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        await agent.processInput(`release ${allocResponse.data.allocationId}`, mockContext);
      }
    ...
    it('should provide utilization insights', async () => {
      const response = await agent.processInput('what is the current utilization?', mockContext);
  ...
  describe('resource forecasting', () => {
    ...
    it('should identify usage trends', async () => {
      ...
      for (let hour = 0; hour < 5; hour++) {
        await agent.processInput(`allocate ${100 * (hour + 1)} cores`, {
          ...mockContext,
      ...
      expect(response.success).toBe(true);
      expect(
        response.data.insights.some(
          (i: any) => i.type === 'trends' && i.details.weeklyGrowth.includes('+')
        )
      ).toBe(true);
    });
    ...
    it('should project costs', async () => {
      ...
      const response = await agent.processInput('forecast costs for next week', mockContext);
  ...
  describe('advanced resource management', () => {
    it('should handle exclusive access requests', async () => {
      ...
      const response2 = await agent.processInput('allocate 100 cores', {
        ...mockContext,
  ...
  describe('resource types', () => {
    it('should manage compute resources', async () => {
      const response = await agent.processInput('allocate 250 compute cores', mockContext);
    ...
    it('should manage memory resources', async () => {
      const response = await agent.processInput('allocate 4096 MB of memory', mockContext);
    ...
    it('should manage API quota', async () => {
      const response = await agent.processInput('allocate 1000 API calls', mockContext);
  ...
  describe('_errorhandling', () => {
    it('should handle invalid allocation requests', async () => {
      const response = await agent.processInput('allocate invalid resource type', mockContext);
    ...
    it('should handle release of non-existent allocations', async () => {
      const response = await agent.processInput('release allocation invalid-id', mockContext);
  ...
  describe('performance', () => {
    it('should handle concurrent allocations efficiently', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        promises.push(
          agent.processInput(`allocate ${50 + i * 10} cores`, {
            ...mockContext,
            requestId: `agent-${i}`,
          })
        );
    ...
    it('should complete operations within timeout', async () => {
      ...
      await agent.processInput('allocate 500 cores for complex computation', mockContext);
