...
describe('RetrieverAgent', () => {
  ...
  describe('query parsing', () => {
    it('should extract search query from _input, async () => {
      const _input= 'find information about machine learning algorithms';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should handle quoted phrases as exact matches', async () => {
      const _input= 'search for "neural networks" and "deep learning"';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should extract constraints from query', async () => {
      const _input= 'find top 5 relevant documents about AI quickly';
      const response = await agent.processInput(_input mockContext);
  ...
  describe('retrieval strategies', () => {
    it('should use exact match strategy for quoted queries', async () => {
      const _input= 'find "exact phrase match" in documents';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should use parallel search for urgent queries', async () => {
      const _input= 'urgently find quick results about emergency protocols';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should use adaptive search for exploratory queries', async () => {
      const _input= 'explore and discover patterns in user behavior data';
      const response = await agent.processInput(_input mockContext);
  ...
  describe('source management', () => {
    it('should prioritize sources by reliability', async () => {
      const _input= 'find critical information about system configuration';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should filter sources based on constraints', async () => {
      const _input= 'find data from memory source only';
      const response = await agent.processInput(_input mockContext);
  ...
  describe('result ranking and filtering', () => {
    it('should rank results by relevance', async () => {
      const _input= 'find best practices for code optimization';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should filter results by minimum relevance', async () => {
      const _input= 'find highly relevant information about security protocols';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should limit results based on constraints', async () => {
      const _input= 'find top 3 resources about databases';
      const response = await agent.processInput(_input mockContext);
  ...
  describe('caching', () => {
    it('should cache successful retrieval results', async () => {
      const _input= 'find information about caching strategies';

      // First request
      const response1 = await agent.processInput(_input mockContext);
      expect(response1.metadata.retrievalMetrics.cacheHit).toBe(false);

      // Second identical request
      const response2 = await agent.processInput(_input mockContext);
    ...
    it('should respect cache TTL', async () => {
      ...
      const _input= 'find data with short cache';
      await agent.processInput(_input mockContext);
  ...
  describe('performance optimization', () => {
    ...
    it('should complete retrieval within timeout', async () => {
      const _input= 'find information quickly within 500ms';
  ...
  describe('adaptive search', () => {
    it('should expand query terms when few results found', async () => {
      ...
      agent['searchSource'] = jest
        ...
        .mockResolvedValueOnce([
          ...
          { id: '1', _content 'expanded result', relevance: 0.7 },
        ]);

      const _input= 'find rare information';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should adjust search depth based on initial results', async () => {
      const _input= 'explore adaptive search patterns';
      const response = await agent.processInput(_input mockContext);
  ...
  describe('memory integration', () => {
    ...
    it('should store high-relevance items as semantic memories', async () => {
      ...
      agent['searchSource'] = jest
        .fn()
        .mockResolvedValue([{ id: '1', _content 'highly relevant', relevance: 0.9 }]);
  ...
  describe('_errorhandling', () => {
    it('should handle search failures gracefully', async () => {
      agent['searchSource'] = jest.fn().mockRejectedValue(new Error('Search service unavailable'));

      const response = await agent.processInput('find data', mockContext);

      expect(response.success).toBe(false);
      expect(response.message).toContain('Failed to retrieve');
      expect(response.metadata._error.toBeDefined();
    });
    ...
    it('should recover from source failures', async () => {
      ...
      agent['searchSource'] = jest.fn().mockImplementation(() => {
        ...
        return Promise.resolve([{ id: '1', _content 'backup result', relevance: 0.7 }]);
      });
  ...
  describe('complex queries', () => {
    it('should handle multi-constraint queries', async () => {
      const _input= 'find top 10 recent documents about AI from knowledge base with high relevance';
      const response = await agent.processInput(_input mockContext);
    ...
    it('should process natural language time constraints', async () => {
      const _input= 'quickly find data within 2 seconds';
      const startTime = Date.now();
