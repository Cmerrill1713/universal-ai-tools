...
describe('AntiHallucinationService', () => {
  ...
  describe('searchMemories', () => {
    it('should search memories for relevant facts', async () => {
      const mockMemories = [
        createMockMemory({ _content 'Paris is the capital of France' }),
        createMockMemory({ _content 'London is the capital of UK' }),
      ];

      mockLimit.mockResolvedValueOnce({ data: mockMemories, _error null });

      const results = await service['searchMemories']('capital of France');

      expect(results).toHaveLength(2);
      expect(results[0]._content.toContain('Paris');
    });

    it('should handle search errors gracefully', async () => {
      mockLimit.mockResolvedValueOnce({ data: null, _error new Error('Search failed') });
  ...
  describe('verifyWithMemory', () => {
    it('should verify truthful statements with high confidence', async () => {
      ...
      mockLimit.mockResolvedValue({
        data: [createMockMemory({ _content 'Paris is the capital city of France' })],
        _error null,
      });
    ...
    it('should handle statements with no memory support', async () => {
      ...
      mockLimit.mockResolvedValue({ data: [], _error null });
  ...
  describe('groundResponse', () => {
    it('should generate grounded response with citations', async () => {
      const mockMemories = [
        createMockMemory({
          _content 'The Eiffel Tower is 330 meters tall',
          metadata: { source: 'Wikipedia' },
        }),
        createMockMemory({
          _content 'The Eiffel Tower was built in 1889',
    ...
    it('should indicate low confidence when no memories found', async () => {
      mockLimit.mockResolvedValueOnce({ data: [], _error null });
