...
jest.mock('../../services/supabase_service.js', () => ({
  SupabaseService: {
    getInstance: jest.fn(() => ({
      client: {
        from: jest.fn(() => ({
          select: jest.fn(() => Promise.resolve({ data: [], _error null })),
          upsert: jest.fn(() => Promise.resolve({ _error null })),
          insert: jest.fn(() => Promise.resolve({ data: [], _error null })),
        })),
      },
    })),
  },
...
describe('DynamicContextManager', () => {
  ...
  describe('Context Optimization', () => {
    const createMessage = (_content string, role: 'user' | 'assistant' | 'system' = 'user') => ({
      role,
      _content
      timestamp: Date.now(),
      tokens: Math.ceil(_contentlength / 4),
    });
    ...
    it('should optimize context based on strategy', async () => {
      ...
      expect(optimized[optimized.length - 1]._content.toContain('Response 29');
    });
  });
  ...
  describe('Context Statistics', () => {
    it('should track token usage statistics', async () => {
      const messages = [
        { role: 'user' as const, _content 'Test message', timestamp: Date.now(), tokens: 10 },
        { role: 'assistant' as const, _content 'Response', timestamp: Date.now(), tokens: 8 },
      ];
