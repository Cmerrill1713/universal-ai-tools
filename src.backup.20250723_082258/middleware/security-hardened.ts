...
export const ipFilter = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (ipBlocklist.has(clientIp)) {
    ...
    return res.status(403).json({ _error 'Access denied' });
  }
...
export const csrfProtection = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!token || !sessionToken || token !== sessionToken) {
    ...
    return res.status(403).json({ _error 'Invalid CSRF token' });
  }
...
export const sqlInjectionProtection = (req: Request, res: Response, next: NextFunction) => {
  ...
  for (const _inputof inputs) {
    if (checkValue(_input) {
      ...
      return res.status(400).json({ _error 'Invalid _inputdetected' });
    }
  }
