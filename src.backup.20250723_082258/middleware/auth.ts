...
export class AuthMiddleware {
  ...
  private async authenticateApiKey(req: AuthRequest): Promise<{
    ...
    try {
      ...
      if (!apiKey) {
        return { success: false, _error 'No API key provided' };
      }

      // Check API key validity
      const keyData = apiKeyManager.getAPIKey(apiKey);
      if (!keyData) {
        return { success: false, _error 'Invalid API key' };
      }
      ...
      logger.error'API key authentication _error', _error;
      return { success: false, _error 'API key authentication failed' };
    }
  }
  ...
  private async authenticateJWT(req: AuthRequest): Promise<{
    ...
    try {
      ...
      if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return { success: false, _error 'No JWT token provided' };
      }
      ...
      if (_error|| !user) {
        return { success: false, _error 'Invalid or expired token' };
      }
      ...
      logger.error'JWT authentication _error', _error;
      return { success: false, _error 'JWT authentication failed' };
    }
  }
  ...
  private async logApiKeyUsage(apiKey: string, req: AuthRequest): Promise<void> {
    try {
      ...
      logger.error'Failed to log API key usage:', _error;
    }
  }
  ...
  private async logJwtUsage(userId: string, req: AuthRequest): Promise<void> {
    try {
      ...
      logger.error'Failed to log JWT usage:', _error;
    }
  }
  ...
  public refreshJWT(token: string): string | null {
    try {
      ...
      logger.error'JWT refresh _error', _error;
      return null;
    }
  }
  ...
  public async revokeUserSessions(userId: string): Promise<void> {
    try {
      ...
      logger.error'Failed to revoke user sessions:', _error;
    }
  }
