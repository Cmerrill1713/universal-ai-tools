  ...
  return (req: TracedRequest, res: Response, next: NextFunction) => {
    ...
    if (mergedOptions.recordRequestBody && req.body) {
      try {
        ...
        span.setAttribute('http._requestbody._error, 'Failed to serialize _requestbody');
      }
    }
    ...
    const captureResponse = (body: any) => {
      ...
      span.setAttribute('http.response.size', res.get('_contentlength') || 0);
      ...
      if (res.statusCode >= 400) {
        ...
        if (res.locals._error {
          ...
          span.setAttribute('_errormessage', res.locals._errormessage);
          span.setAttribute('_errorstack', res.locals._errorstack?.substring(0, 1000));
        }
      } else {
...
export function createTracingErrorMiddleware(
  ...
  return (err: Error, req: TracedRequest, res: Response, next: NextFunction) => {
    ...
    if (span && options.errorFilter?.(err) !== false) {
      ...
      span.setAttribute('_errormessage', err.message);
      span.setAttribute('_errorstack', err.stack?.substring(0, 1000) || 'No stack trace');
