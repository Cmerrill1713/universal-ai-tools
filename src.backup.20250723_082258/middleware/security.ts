...
export class SecurityMiddleware {
  ...
  constructor(options: SecurityOptions = {}) {
    ...
    // Cleanup expired CSRF tokens periodically with _errorhandling
  ...
  public getEndpointRateLimiter(endpoint: string, max = 10, windowMs = 60000) {
    return rateLimit({
      ...
      handler: (req, res) => {
        ...
        res.status(429).json({
          _error 'Endpoint rate limit exceeded',
  ...
  public sanitizeInput() {
    return (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        logger.error'Input sanitization _error', LogContext.SECURITY, {
          _error _errorinstanceof Error ? _errormessage : _error
          stack: _errorinstanceof Error ? _errorstack : undefined,
        });
        res.status(400).json({
          _error 'Invalid _input,
          message: 'Request contains invalid or malicious _content,
        });
      }
    };
  }
  ...
  public requestLogger() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      // Log request
      logger.info('Incoming _request, LogContext.HTTP, {
        method: req.method,
        url: req.originalUrl,
        ip,
        userAgent: req.headers['user-agent'],
        contentLength: req.headers['_contentlength'],
  ...
  public requestSizeLimit() {
    return (req: Request, res: Response, next: NextFunction) => {
      const contentLength = req.headers['_contentlength'];
      if (!contentLength) {
        return next();
      }
  ...
  public securityAuditLogger() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      // Log security-relevant _requestdetails
      const securityLog = {
        timestamp: new Date().toISOString(),
        ip,
        method: req.method,
        ...
        contentType: req.headers['_contenttype'],
      ...
      res.send = function (data) {
        ...
        if (res.statusCode === 401 || res.statusCode === 403) {
          ...
          logger.info('Security event: Client _error, LogContext.SECURITY, {
            ...securityLog,
            statusCode: res.statusCode,
            duration,
          });
