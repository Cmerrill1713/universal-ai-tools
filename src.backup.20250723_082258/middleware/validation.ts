    ...
    return (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (options.body && req.body) {
          ...
          if (!result.success) {
            errors.push(...this.formatZodErrors(result._error 'body'));
          } else {
        ...
        if (options.query && req.query) {
          ...
          if (!result.success) {
            errors.push(...this.formatZodErrors(result._error 'query'));
          } else {
        ...
        if (options.params && req.params) {
          ...
          if (!result.success) {
            errors.push(...this.formatZodErrors(result._error 'params'));
          } else {
        ...
        if (options.headers && req.headers) {
          ...
          if (!result.success) {
            errors.push(...this.formatZodErrors(result._error 'headers'));
          }
        }
