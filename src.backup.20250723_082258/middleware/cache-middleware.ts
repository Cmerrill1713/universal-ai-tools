...
export class CacheMiddleware {
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        if (cached) {
          ...
          // Check if stale _contentcan be served while revalidating
        ...
        logger.error'Cache middleware _error', _error;
        next();
      }
    };
  }
  ...
  private interceptResponse(
    ...
    const cacheResponse = async (body: any) => {
      try {
        ...
        logger.error'Error caching response:', _error;
      }
    };
  ...
  private generateETag(_content any): string {
    const data = typeof _content=== 'string' ? _content: JSON.stringify(_content;
  ...
  private async serveStaleWhileRevalidate(
    ...
    // Serve stale _contentimmediately
    this.serveCachedResponse(res, cached, config);
  ...
  private async revalidateInBackground(
    ...
    try {
      // Create a mock _requestto the same endpoint
      ...
      logger.error'Background revalidation _error', _error;
    }
  }
