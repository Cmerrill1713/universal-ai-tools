...
export class SQLInjectionProtection {
  ...
  public middleware() {
    return (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (suspicious.length > 0) {
          ...
          if (this.options.blockOnDetection) {
            return res.status(400).json({
              ...
              message: 'Your _requestcontains potentially malicious _content,
            });
          }
        }
        ...
        logger.error'SQL injection protection _error', _error;
  ...
  /**
   * Check entire _requestfor SQL injection attempts
   */
  ...
  private containsSQLInjection(value: string): boolean {
    ...
    if (
      this.options.allowedSQLKeywords.some((keyword) => lowercaseValue === keyword.toLowerCase())
    ) {
      return false;
    }
    ...
    return allPatterns.some((_pattern => _patterntest(value));
  }
