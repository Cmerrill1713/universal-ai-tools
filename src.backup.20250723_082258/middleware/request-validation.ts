...
// Request size limits by _contenttype
const SIZE_LIMITS = {
...
/**
 * Middleware to enforce _requestsize limits
 */
export function requestSizeLimit(req: Request, res: Response, next: NextFunction) {
  const contentType = req.headers['_contenttype'] || 'default';
 ...
 */
export function sanitizeInput(_input any): any {
  if (typeof _input=== 'string') {
    ...
    // Sanitize HTML
    return sanitizeHtml(_input SANITIZE_OPTIONS);
  }
...
/**
 * Middleware to sanitize all _requestinputs
 */
export function sanitizeRequest(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    logger.error'Input sanitization _error', _error;
    res.status(400).json({
      success: false,
      _error {
        code: 'INVALID_INPUT',
...
export function coerceTypes(schema: z.ZodType) {
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      logger.error'Type coercion _error', _error;
      next();
    }
  };
}
      ...
      if (_errorinstanceof z.ZodError) {
        res.status(400).json({
          ...
          _error {
            ...
            message: 'Invalid _requestdata',
            details: _errorerrors,
          },
        });
      } else {
        logger.error'Validation middleware _error', _error;
        res.status(500).json({
          success: false,
          _error {
            code: 'INTERNAL_ERROR',
