 ...
 *
 * Specialized logging for tests with detailed failure _analysis
...
export class TestLogger {
  ...
  private async ensureDirectories() {
    try {
      ...
      console._error'Failed to create test directories:', _error;
    }
  }
  ...
  // End a test run
  endTest(testId: string, status: 'pass' | 'fail' | 'skip' | 'timeout', _error: Error): TestResult {
  ...
  private async generateFailureReport(testResult: TestResult) {
    const reportData = {
      ...
      _error testResult.error
        ? {
            message: testResult._errormessage,
            stack: testResult._errorstack,
            name: testResult._errorname,
          }
        : null,
...
export const endTest = (
  ...
  status: 'pass' | 'fail' | 'skip' | 'timeout',
  _error: Error
) => testLogger.endTest(testId, status, _error;
