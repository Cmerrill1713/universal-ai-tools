...
export class DatabaseOptimizer {
  ...
  private updateStats(
    ...
    performanceMonitor.recordRequest(responseTime, _error;
  }
    ...
    try {
      ...
      if (options.cache !== false) {
        ...
        if (cached !== null) {
          ...
          return { data: cached, _error null, fromCache: true };
        }
      }
      ...
      const { data, _error} = await result;
      ...
      return { data: data as T[], _error fromCache };
    } catch (_error) {
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('select', responseTime, false, true);

      logger.error`Database select _erroron ${table}:`, LogContext.DATABASE, { _error});
      return { data: null, _error fromCache };
    }
  }
  ...
  public async insert<T = any>(
    ...
    try {
      ...
      const { data: insertedData, _error} = await result;
      ...
      return { data: insertedData, _error};
    } catch (_error) {
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('insert', responseTime, false, true);

      logger.error`Database insert _erroron ${table}:`, LogContext.DATABASE, { _error});
  ...
  public async update<T = any>(
    ...
    try {
      ...
      const { data: updatedData, _error} = await result;
      ...
      return { data: updatedData, _error};
    } catch (_error) {
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('update', responseTime, false, true);

      logger.error`Database update _erroron ${table}:`, LogContext.DATABASE, { _error});
      return { data: null, _error};
    }
  }
  ...
  public async delete<T = any>(
    ...
    try {
      ...
      const { data: deletedData, _error} = await result;
      ...
      return { data: deletedData, _error};
    } catch (_error) {
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('delete', responseTime, false, true);

      logger.error`Database delete _erroron ${table}:`, LogContext.DATABASE, { _error});
      return { data: null, _error};
    }
  }
  ...
  public async upsert<T = any>(
    ...
    try {
      ...
      const { data: upsertedData, _error} = await result;
      ...
      return { data: upsertedData, _error};
    } catch (_error) {
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('insert', responseTime, false, true);

      logger.error`Database upsert _erroron ${table}:`, LogContext.DATABASE, { _error});
      return { data: null, _error};
    }
  }
    ...
    try {
      ...
      if (options.cache !== false) {
        ...
        if (cached !== null) {
          ...
          const responseTime = seconds * 1000 + nanoseconds / 1000000;
          this.updateStats('select', responseTime, true);
          return { data: cached, _error null };
        }
      }
      ...
      logger.error`Database RPC _errorfor ${functionName}:`, LogContext.DATABASE, { _error});
      return { data: null, _error};
    }
  }
  ...
  public async healthCheck(): Promise<DatabaseHealth> {
    ...
    try {
      ...
      const { data, _error} = await this.supabase.from('ai_services').select('id').limit(1);
