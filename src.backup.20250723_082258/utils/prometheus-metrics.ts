...
export class PrometheusMetricsCollector {
  ...
  constructor() {
    ...
    // Use lazy initialization _patterninstead
  }
  ...
  recordDatabaseOperation(table: string, operation: string, durationMs: number, _error: string) {
    // Initialize lazily if not already done
    if (!this.initialized) {
      this.initialize().catch(() => {});
    }
