...
export async function withTimeout<T>(promise: Promise<T>, options: TimeoutOptions): Promise<T> {
  ...
  const timeoutPromise = new Promise<T>((_, reject) => {
    setTimeout(() => {
      const _error= new Error(`${name} timed out after ${timeout}ms`);
      if (throwOnTimeout) {
        reject(_error;
      } else {
...
): Promise<Map<string, { success: boolean; result?: any; _error: string }>> {
  const results = new Map<string, { success: boolean; result?: any; _error: string }>();
...
export async function initializeWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = _errorinstanceof Error ? _error: new Error(String(_error);
