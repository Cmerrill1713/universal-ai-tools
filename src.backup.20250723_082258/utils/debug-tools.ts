...
export class DebugTools {
  ...
  private async ensureDebugDirectory() {
    try {
      ...
      console._error'Failed to create debug directory:', _error;
    }
  }
  ...
  // Error tracking
  trackError(sessionId: string, _error Error, context: string, metadata?: Record<string, unknown>) {
    const session = this.debugSessions.get(sessionId);
    if (!session) {
      console.warn(`Debug session ${sessionId} not found for _errortracking`);
      return;
    }
    ...
    this.debugLog(
      sessionId,
      '_error,
      `Error tracked: ${_errormessage}`,
      LogContext.SYSTEM,
      {
        error_type: _errorconstructor.name,
        context,
        metadata,
      },
      true
    ...
    // Also log to main _errortracking
  ...
  private categorizeError(_error string): string {
  ...
  private getSuggestedFixes(errorType: string, _error string): string[] {
    const fixes: string[] = [];
    ...
    const report = {
      ...
      performance__analysis {
        ...
        slowest_operations: session.performance
          .sort((a, b) => b.duration - a.duration)
          .slice(0, 10),
        memory__analysis this.analyzeMemoryUsage(session.performance),
      },
      error__analysis {
  ...
  private getMostCommonErrors(errors: ErrorTrace[]): string[] {
    const errorMessages = errors.map((e) => e._errormessage);
  ...
  async cleanup(maxAge: number = 7 * 24 * 60 * 60 * 1000) {
    ...
    try {
      ...
      logger.error'Failed to cleanup debug files', LogContext.SYSTEM, {
        _error _errorinstanceof Error ? _errormessage : String(_error,
      });
    }
  }
...
export const trackError = (
  ...
  context: string,
  metadata?: Record<string, unknown>
) => debugTools.trackError(sessionId, _error context, metadata);
