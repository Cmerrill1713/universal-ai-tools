...
export interface RegisteredAgent {
  ...
  name: string;
  type: string;
  status: 'idle' | 'busy' | '_error | 'offline';
...
export class AgentRegistry extends EventEmitter {
  ...
  private setupIndexes(): void {
    ...
    this.statusIndex.set('_error, new Set());
  ...
  async findAgentsByCapabilities(query: CapabilityQuery): Promise<RegisteredAgent[]> {
    ...
    if (!query.requiredSkills || query.requiredSkills.length === 0) {
      ...
      const skillSets = query.requiredSkills.map(
        (skill) => this.capabilityIndex.get(skill) || new Set()
      );
    ...
    const filteredAgents = Array.from(candidates)
      ...
      .filter((agent) => {
        ...
        if (query.minConfidence) {
          const hasMinConfidence = agent.capabilities.some(
            (cap) => (cap.confidence || 0) >= query.minConfidence!
          );
  ...
  private calculateSuitabilityScore(agent: RegisteredAgent, query: CapabilityQuery): number {
    ...
    const avgConfidence =
      agent.capabilities.reduce((sum, cap) => sum + (cap.confidence || 0), 0) /
  ...
  async cleanup(): Promise<void> {
    ...
    const staleAgents = Array.from(this.agents.values()).filter(
      (agent) => agent.lastSeen < staleThreshold
    );
