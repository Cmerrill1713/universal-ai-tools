    ...
    try {
      ...
      for (const test of tests) {
        try {
          ...
          result.errors.push(
            `Test failed: ${_errorinstanceof Error ? _errormessage : String(_error}`
          );
        }
      }
      ...
      result.errors.push(
        `Validation failed: ${_errorinstanceof Error ? _errormessage : String(_error}`
      );
      result.duration = Date.now() - startTime;
      logger.error`UI validation _errorfor agent ${agent.id}:`, _error;
    }
  ...
  private async testChatPage(agent: BrowserAgent): Promise<TestResult> {
    ...
    try {
      ...
      if (agent.type === 'puppeteer') {
        ...
        const chatElements = await (agent.page as Page).$$('button, .card, _input textarea');
