...
export interface Message {
  ...
  type: 'coordination' | 'task' | 'status' | '_error | 'artifact' | 'heartbeat' | 'discovery';
  _content any;
  timestamp: number;
...
export class MessageBroker extends EventEmitter {
  ...
  async unregisterAgent(agentId: string): Promise<void> {
    ...
    if (queue) {
      ...
      if (queue.messages.length > 0) {
        logger.warn(
          `📬 Agent ${agentId} unregistered with ${queue.messages.length} pending messages`
        );
    ...
    try {
      while (queue.messages.length > 0) {
        ...
        try {
          ...
          logger.error`📬 Failed to deliver message ${message.id}:`, _error;
          await this.handleDeliveryFailure(message, _error;
        }
      }
    } finally {
  ...
  private async handleDeliveryFailure(message: Message, _error any): Promise<void> {
    ...
    if (message.retryCount < maxRetries) {
      ...
      logger.warn(
        `📬 Retrying message ${message.id} (attempt ${message.retryCount}/${maxRetries})`
      );
    } else {
      await this.handleUndeliveredMessage(message);
    }
  ...
  private updateDeliveryStats(deliveryTime: number): void {
    ...
    this.messageStats.averageDeliveryTime =
      totalDelivered === 1
        ? deliveryTime
