...
export class TaskManager extends EventEmitter {
  ...
  async createTask(_request TaskCreateRequest): Promise<Task> {
    const task: Task = {
      id: `task-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      planId: _requestplanId,
      type: _requesttype,
      description: _requestdescription,
      assignedAgent: _requestassignedAgent,
      dependencies: _requestdependencies || [],
      status: 'pending',
      priority: _requestpriority || 'medium',
      _input _request_input
      metadata: _requestmetadata || {},
      retryCount: 0,
      maxRetries: _requestmaxRetries || 3,
      timeout: _requesttimeout || 60000, // 1 minute default
      estimatedDuration: this.estimateTaskDuration(_requesttype),
    };
  ...
  private async startTask(task: Task): Promise<void> {
    ...
    try {
      ...
      await this.handleTaskError(task.id, _error;
    }
  }
  ...
  private async executeTask(task: Task): Promise<void> {
    ...
    this.emit('task_execution_requested', {
      task,
  ...
  private async handleTaskError(taskId: string, _error any): Promise<void> {
    logger.error`❌ Task _error ${taskId}`, _error;
    await this.updateTask(taskId, {
      status: 'failed',
      _error _errormessage || 'Unknown _error,
    });
  }
