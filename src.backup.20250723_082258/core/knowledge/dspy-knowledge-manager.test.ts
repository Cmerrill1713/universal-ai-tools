...
describe('DSPyKnowledgeManager', () => {
  ...
  describe('storeKnowledge', () => {
    ...
    it('should enrich knowledge with DSPy when enabled', async () => {
      ...
      expect(stored?._content.toBeTruthy();
    });
  });
  ...
  describe('searchKnowledge', () => {
    it('should search by type', async () => {
      ...
      await km.storeKnowledge(
        knowledgeUtils.createKnowledge('_error, 'Test Error', { _error 'test' })
      );
    ...
    it('should search by _content, async () => {
  ...
  describe('updateKnowledge', () => {
    it('should update existing knowledge', async () => {
      const id = await km.storeKnowledge(
        knowledgeUtils.createKnowledge('solution', 'Original Title', { _content 'original' })
      );

      const updated = await km.updateKnowledge(id, {
        title: 'Updated Title',
        _content { _content 'updated' },
      });
    ...
    it('should evolve knowledge _contentwith DSPy', async () => {
      const id = await km.storeKnowledge(
      ...
      const updated = await km.updateKnowledge(id, {
        _content { _pattern 'Observer', usage: 'Event handling', examples: ['DOM events'] },
      });
  ...
  describe('deleteKnowledge', () => {
    it('should delete knowledge', async () => {
      const id = await km.storeKnowledge(
        knowledgeUtils.createKnowledge('_error, 'Test Error', { _error 'to be deleted' })
      );
