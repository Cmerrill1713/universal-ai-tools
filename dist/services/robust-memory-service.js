...
export class RobustMemoryService {
    breaker = createCircuitBreaker('memory-db', {
        failureThreshold: 3,
        successThreshold: 2,
        timeout: 10000,
        volumeThreshold: 5,
    ...
    getLruKey(userId, type) {
        return `${userId}::${type}`;
    }
    updateLru(userId, type, embedding, id, importance = 0.5) {
    ...
    isDuplicate(userId, type, emb) {
        const key = this.getLruKey(userId, type);
        const arr = this.lru.get(key) || [];
        for (const e of arr.slice(0, 50)) {
            if (cosineSimilarity(e.embedding, emb) >= this.dedupeThreshold)
                return true;
        }
        return false;
    }
    async save(memory) {
        memWrites.inc();
        try {
            const embedding = (await generateEmbedding(memory.content)) || generateFallbackEmbedding(memory.content);
...
//# sourceMappingURL=robust-memory-service.js.map