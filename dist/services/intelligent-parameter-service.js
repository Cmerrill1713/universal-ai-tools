...
export class IntelligentParameterService {
    ...
    initializeTaskProfiles() {
        ...
        this.taskProfiles.set(TaskType.CODE_REVIEW, {
            ...
            userPromptTemplate: 'Review this code and provide detailed feedback:\n\n```{language}\n{code}\n```\n\nFocus on:\n- Security vulnerabilities\n- Performance issues\n- Code quality and maintainability\n- Best practices\n- Potential bugs\n\nReview:',
            presencePenalty: 0.0,
    ...
    initializeContextTemplates() {
        this.contextTemplates.set(TaskType.CODE_GENERATION, 'Programming Language: {language}\nFramework: {framework}\nComplexity: {complexity}\nEnvironment: {environment}');
        this.contextTemplates.set(TaskType.DATA_ANALYSIS, 'Data Type: {dataType}\nSize: {size}\nObjective: {objective}\nConstraints: {constraints}');
        this.contextTemplates.set(TaskType.CREATIVE_WRITING, 'Genre: {genre}\nAudience: {audience}\nMood: {mood}\nSetting: {setting}');
    }
    initializeDomainAdjustments() {
    ...
    adjustForComplexity(params, complexity) {
        const adjustments = {
        ...
        const adjustment = adjustments[complexity] || adjustments.medium;
        return {
            ...params,
    ...
    adjustForUserPreferences(params, prefs) {
        ...
        if (prefs.preferredTemperature !== undefined) {
            ...
            adjusted.temperature = Math.max(0.0, Math.min(1.0, adjusted.temperature + creativityAdjustments[prefs.creativity]));
        }
        if (prefs.preferredLength) {
            ...
            adjusted.maxTokens = Math.round(adjusted.maxTokens * lengthMultipliers[prefs.preferredLength]);
        }
    ...
    adjustForOutputLength(params, expectedLength) {
        const lengthMultipliers = {
            short: 0.5,
            medium: 1.0,
            long: 1.8,
        };
        const multiplier = lengthMultipliers[expectedLength] || 1.0;
        return {
            ...params,
            maxTokens: Math.round(params.maxTokens * multiplier),
            contextLength: Math.round(params.contextLength * Math.min(multiplier, 1.2)),
        };
    }
    buildDynamicPrompt(template, context) {
        let prompt = template;
        const replacements = {
    ...
    detectTaskType(userInput, context) {
        ...
        if (this.containsKeywords(input, ['debug', 'fix bug', 'error', 'not working', 'troubleshoot'])) {
        ...
        if (this.containsKeywords(input, ['solve', 'calculate', 'reasoning', 'logic', 'step by step'])) {
    ...
    getModelOptimizedParameters(baseParams, modelName) {
    ...
    createTaskContext(userInput, taskType, additionalContext, userPreferences) {
...
//# sourceMappingURL=intelligent-parameter-service.js.map