...
class VisionBrowserDebugger {
    ...
    analysisResults = [];
    visionServiceUrl = process.env.VISION_SERVICE_URL || 'http://localhost:9999/api/v1/vision';
    constructor() {
    ...
    ALLOWED_AUTO_FIX_COMMANDS = new Set([
        'npm run lint:fix',
        'npm run format',
        'npx tsc --noEmit',
        'npm run type-check',
    ...
    validateAutoFixCommand(command) {
    ...
    async captureAndAnalyzeBrowser() {
        ...
        try {
            ...
            if (screenshotPath) {
                ...
                console.log(`🔍 Analysis complete - Found ${analysis.detectedElements.length} UI elements, ${analysis.consoleErrors.length} console errors`);
            }
        }
        catch (error) {
    ...
    async captureDevToolsScreenshot() {
        try {
    ...
    async callVisionService(imagePath) {
        try {
            const isValidPath = validatePath(imagePath, {
    ...
    generateFallbackAnalysis(imagePath) {
    ...
    generateMockConsoleErrors(timestamp) {
        const errors = [];
        if (Math.random() > 0.3) {
            errors.push({
    ...
    generateMockNetworkIssues(timestamp) {
        const issues = [];
        if (Math.random() > 0.4) {
            issues.push({
    ...
    generateMockUIElements(timestamp) {
        const elements = [];
        if (Math.random() > 0.7) {
            elements.push({
                type: 'error',
    ...
    generateMockPerformanceMetrics(timestamp) {
        return [
            {
    ...
    extractConsoleErrors(visionData) {
        const errors = [];
    ...
    extractNetworkIssues(visionData) {
        const issues = [];
    ...
    extractPerformanceMetrics(visionData) {
        const metrics = [];
    ...
    createConsoleErrorSuggestion(error) {
    ...
    createNetworkIssueSuggestion(issue) {
    ...
    createPerformanceSuggestion(metric) {
        return {
    ...
    createUIElementSuggestion(element) {
        return {
    ...
    async executeAutoFix(suggestion) {
        try {
            ...
            console.log(`❌ Auto-fix failed for: ${suggestion.issue} - ${error instanceof Error ? error.message : String(error)}`);
...
//# sourceMappingURL=vision-browser-debugger.js.map