...
Always provide production-ready code with proper error handling, validation, and documentation.`;
    ...
    detectProgrammingLanguage(request) {
    ...
    identifyCodeRequestType(request) {
        const requestTypes = [
        ...
        try {
            ...
            if (parsed.code_response) {
                ...
                if (parsed.code_response.code_blocks && Array.isArray(parsed.code_response.code_blocks)) {
                    const hasValidCode = parsed.code_response.code_blocks.some((block) => block.code && block.code.length > 10);
                    if (hasValidCode) {
                        confidence += 0.15;
                    }
                }
...
//# sourceMappingURL=enhanced-code-assistant-agent.js.map