...
router.post('/train', authenticate, zodValidate(z.object({
    modelId: z.string(),
...
router.put('/thresholds', authenticate, zodValidate(z.object({
...
router.post('/evolve', authenticate, zodValidate(z.object({
    targetTask: z.string(),
...
router.post('/speculative/test', authenticate, zodValidate(z.object({
    prompt: z.string(),
    ...
    try {
        ...
        const result = await speculativeDecodingService.generateWithSpeculation(prompt, targetModel, { maxTokens });
        return res.json({
            success: true,
            data: {
...
router.post('/mlx/test', authenticate, zodValidate(z.object({
    modelId: z.string(),
    prompt: z.string(),
    maxTokens: z.number().min(1).max(500).optional(),
    ...
    try {
        ...
        const response = await mlxProviderService.generate(modelId, prompt, { maxTokens, temperature });
