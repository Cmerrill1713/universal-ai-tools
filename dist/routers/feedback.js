...
router.post('/submit', async (req, res) => {
    try {
        const { sessionId, feedbackType, category, rating, title, description, context, tags, modelId, providerId, responseTime, attachments, } = req.body;
        const userId = req.user?.id || 'anonymous';
        if (!sessionId || !feedbackType || !description) {
            return sendError(res, 'VALIDATION_ERROR', 'Missing required fields: sessionId, feedbackType, description');
        }
        const validTypes = ['rating', 'suggestion', 'bug_report', 'feature_request', 'general'];
        if (!validTypes.includes(feedbackType)) {
            return sendError(res, 'VALIDATION_ERROR', `feedbackType must be one of: ${validTypes.join(', ')}`);
        }
        if (category) {
            const validCategories = ['model_performance', 'user_interface', 'speed', 'accuracy', 'usability', 'other'];
            if (!validCategories.includes(category)) {
                return sendError(res, 'VALIDATION_ERROR', `category must be one of: ${validCategories.join(', ')}`);
            }
        }
        if (rating !== undefined) {
...
router.put('/:feedbackId/status', async (req, res) => {
    try {
        ...
        if (!validStatuses.includes(status)) {
            return sendError(res, 'VALIDATION_ERROR', `status must be one of: ${validStatuses.join(', ')}`);
        }
...
router.post('/rating', async (req, res) => {
    try {
        const { sessionId, rating, modelId, providerId, responseTime, context, } = req.body;
        const userId = req.user?.id || 'anonymous';
...
router.post('/bug', async (req, res) => {
    try {
        const { sessionId, title, description, steps, expected, actual, context, attachments, } = req.body;
        const userId = req.user?.id || 'anonymous';
        if (!sessionId || !title || !description) {
            return sendError(res, 'VALIDATION_ERROR', 'Missing required fields: sessionId, title, description');
        }
...
router.post('/feature', async (req, res) => {
    try {
        const { sessionId, title, description, useCase, priority, category, } = req.body;
        const userId = req.user?.id || 'anonymous';
        if (!sessionId || !title || !description) {
            return sendError(res, 'VALIDATION_ERROR', 'Missing required fields: sessionId, title, description');
        }
