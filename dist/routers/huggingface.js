...
import { analysisParametersMiddleware, creativeParametersMiddleware, parameterEffectivenessLogger, } from '../middleware/intelligent-parameters';
...
router.post('/generate', creativeParametersMiddleware(), async (req, res) => {
    try {
        if (!huggingFaceService) {
            return res.status(503).json({
                success: false,
        ...
        const optimizedParameters = {
            ...
            do_sample: req.body.temperature > 0.1,
            ...parameters,
        };
...
router.post('/qa', analysisParametersMiddleware(), async (req, res) => {
    try {
        if (!huggingFaceService) {
            return res.status(503).json({
                success: false,
