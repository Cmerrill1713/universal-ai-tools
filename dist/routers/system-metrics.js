...
router.get('/metrics', async (req, res, next) => {
    try {
        ...
        const performanceData = await parameterAnalyticsService.getRecentPerformance(60);
        const avgResponseTime = performanceData.length > 0
            ? Math.round(performanceData.reduce((sum, p) => sum + (p.executionTime || 0), 0) /
...
router.get('/performance', async (req, res, next) => {
    try {
        ...
        const interval = minutes > 1440 ? 60 : minutes > 360 ? 15 : 5;
        const grouped = [];
        const now = Date.now();
        for (let i = 0; i < minutes / interval; i++) {
            const startTime = now - (i + 1) * interval * 60000;
            const endTime = now - i * interval * 60000;
            const intervalData = performanceData.filter((p) => p.timestamp >= startTime && p.timestamp < endTime);
        ...
        sendSuccess(res, {
            ...
            data: grouped,
        }, 200, { message: 'Performance data retrieved successfully' });
    }
    catch (error) {
...
router.get('/agents/performance', async (req, res, next) => {
    try {
        const agentRegistry = new AgentRegistry();
        const loadedAgents = agentRegistry.getLoadedAgents();
        const agentPerformance = await Promise.all(loadedAgents.map(async (agentName) => {
