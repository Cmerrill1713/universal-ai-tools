...
router.post('/semantic-search', async (req, res) => {
    try {
        ...
        contextAnalyticsService.trackRetrievalEvent(searchTime, searchResults.results.length, searchResults.metrics.averageRelevance);
...
router.get('/:userId', async (req, res) => {
    try {
        ...
        const context = await contextStorageService.getContext(userId, category, projectPath, parseInt(limit) || 10);
        res.json({
            success: true,
            data: {
                context,
...
router.post('/:userId/search', async (req, res) => {
    try {
        ...
        const context = await contextStorageService.getContext(userId, category, undefined, limit || 10);
        const filteredContext = context.filter((ctx) => ctx.content.toLowerCase().includes(query.toLowerCase()) ||
...
router.get('/enhanced/:sessionId/:userId', async (req, res) => {
    try {
        ...
        const { maxTokens, relevanceThreshold, includeRecentMessages, includeSummaries, timeWindow } = req.query;
        ...
        const options = {
            ...
            relevanceThreshold: relevanceThreshold ? parseFloat(relevanceThreshold) : undefined,
...
router.get('/analytics/user/:analyticsUserId', async (req, res) => {
    try {
        ...
        const analytics = await contextAnalyticsService.getUserAnalytics(analyticsUserId, timeWindow ? parseInt(timeWindow) : undefined);
...
router.get('/analytics/compression', async (req, res) => {
    try {
        const { timeWindow } = req.query;
        const analytics = await contextAnalyticsService.getCompressionAnalytics(timeWindow ? parseInt(timeWindow) : undefined);
