...
router.get('/events', authenticate, async (req, res) => {
    try {
        ...
        const providerFilter = providers ? providers.split(',') : undefined;
...
router.get('/availability', authenticate, async (req, res) => {
    try {
        ...
        const slots = await calendarIntegrationService.findAvailableSlots(startDate, endDate, durationMinutes);
        res.json({
            success: true,
            data: slots,
            metadata: { requestId: uuidv4() }
...
router.post('/schedule', authenticate, async (req, res) => {
    try {
        ...
        const scheduling = await calendarIntegrationService.smartSchedule(title, duration, participants, preferences);
        return res.json({
            success: true,
            data: scheduling,
            metadata: { requestId: uuidv4() }
