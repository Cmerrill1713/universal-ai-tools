...
router.post('/optimize', authenticate, zodValidate(optimizeSchema), async (req, res) => {
    try {
...
router.get('/presets', authenticate, async (req, res) => {
    try {
        ...
        const filteredPresets = taskType
            ? presets.filter(preset => preset.taskTypes.includes(taskType))
            : presets;
        sendSuccess(res, {
