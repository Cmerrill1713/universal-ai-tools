...
async function processJob(client, jobId) {
    const { rows } = await client.query(`UPDATE public.mcp_jobs
     SET status = 'in_progress', updated_at = now()
     WHERE id = $1 AND status = 'queued'
     RETURNING id, tool_name, tool_args`, [jobId]);
    if (rows.length === 0)
        return;
    const job = rows[0];
    try {
        ...
        await client.query(`UPDATE public.mcp_jobs SET status='completed', result = $2, updated_at = now() WHERE id = $1`, [job.id, JSON.stringify(result ?? null)]);
        log.info('✅ MCP job completed', LogContext.MCP, { jobId: job.id, tool: job.tool_name });
        if (job.tool_name === 'propose_migration') {
            const sql = result?.sql || '';
            if (sql.trim()) {
                await client.query(`INSERT INTO public.migration_suggestions(job_id, request, sql_text) VALUES ($1, $2, $3)`, [job.id, String(job.tool_args?.request ?? ''), sql]);
            }
            else {
        ...
        await client.query(`UPDATE public.mcp_jobs SET status='failed', error = $2, updated_at = now() WHERE id = $1`, [job.id, message]);
...
async function main() {
    ...
    const pending = await client.query(`SELECT id FROM public.mcp_jobs WHERE status='queued' ORDER BY created_at ASC LIMIT 25`);
    for (const row of pending.rows) {
        await processJob(client, String(row.id));
    }
    client.on('notification', async (msg) => {
        if (msg.channel !== 'mcp_jobs_insert' || !msg.payload)
            return;
...
//# sourceMappingURL=mcp-job-worker.js.map