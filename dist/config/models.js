...
export const ModelConfig = {
    ...
    imageGeneration: {
        sdxl: process.env.SDXL_MODEL_PATH ||
            '/Users/christianmerrill/Downloads/stable-diffusion-xl-base-1.0-Q4_1.gguf',
        sdxlRefiner: process.env.SDXL_REFINER_PATH ||
    ...
    lfm2: {
        ...
        modelPath: process.env.LFM2_MODEL_PATH ||
    ...
    switch (taskType) {
        ...
        default:
            if (speed === 'fast')
                return ModelConfig.text.small;
            if (speed === 'quality')
                return ModelConfig.text.large;
            return ModelConfig.text.medium;
    }
}
export async function isModelAvailable(model) {
    try {
        const response = await (globalThis.fetch || fetch)('http://localhost:11434/api/tags');
