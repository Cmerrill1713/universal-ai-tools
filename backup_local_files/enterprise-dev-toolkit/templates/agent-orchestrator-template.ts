...
export class AgentOrchestrator extends EventEmitter {
  ...
  async updateAgentHeartbeat(agentId: string, metrics?: Partial<AgentRegistration['metrics']>): Promise<void> {
    const agent = this.agents.get(agentId);
    if (agent) {
  ...
  private async assignTaskToAgent(task: Task): Promise<void> {
    ...
    if (!selectedAgent) {
      console.log(`No available agent for task ${task.id}, capabilities: ${task.requiredCapabilities.join(', ')}`);
      return;
    }
  ...
  private async executeTask(task: Task, agent: AgentRegistration): Promise<void> {
    ...
    try {
      ...
      this.updateAgentMetrics(agent.id, true, execution.endTime.getTime() - execution.startTime.getTime());
  ...
  private async handleTaskFailure(task: Task, error: Error): Promise<void> {
    ...
    if (task.retryCount < maxRetries) {
      ...
      console.error(`Task failed permanently: ${task.type} (${task.id}):`, error.message);
    }
  }

  private async callAgent(agent: AgentRegistration, task: Task): Promise<any> {
  ...
  private updateAgentMetrics(agentId: string, success: boolean, latency: number): void {
    ...
    if (agent) {
      ...
      if (success) {
        agent.metrics.averageLatency = 
          (agent.metrics.averageLatency + latency) / 2;
      }
...
export function createAgentOrchestrator(config?: Partial<AgentOrchestrator['config']>): AgentOrchestrator {
