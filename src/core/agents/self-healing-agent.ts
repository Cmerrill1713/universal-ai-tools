...
export interface HealingAction {
  id: string;
  type: 'restart_agent' | 'reload_page' | 'clear_cache' | 'restart_service' | 'fix_api_call' | 'online_research';
  description: string;
...
export class SelfHealingAgent extends EventEmitter {
  ...
  private initializeHealingActions(): void {
    const actions: HealingAction[] = [
      ...
      },
      {
        ...
        implementation: async (agent: BrowserAgent, context: any) => {
          try {
            const error = context.error || context.description || 'Unknown error';
            const technology = this.detectTechnology(error);
  ...
  private setupEventListeners(): void {
    ...
    this.agentPool.on('agent-error', (data) => {
      this.reportIssue({
        ...
        type: 'crash',
        description: `Agent error: ${data.error.message}`,
        severity: 'high',
        context: data.error
      });
    });
  ...
  private async performHealthCheck(): Promise<void> {
    try {
      ...
      for (const agent of agents) {
        ...
        if (agent.status === 'error' && agent.errorCount > 0) {
          this.reportIssue({
            agentId: agent.id,
            type: 'crash',
            description: `Agent in error state with ${agent.errorCount} errors`,
            severity: 'high',
        ...
        if (timeSinceLastUse > 300000 && agent.status === 'busy') { // 5 minutes
          this.reportIssue({
            agentId: agent.id,
            type: 'ui',
  ...
  private async verifyIssueResolved(issue: Issue, agent: BrowserAgent): Promise<boolean> {
    try {
      ...
      switch (issue.type) {
        case 'crash':
          // Check if agent is no longer in error state
...
`;
  ...
  private detectTechnology(error: string): string {
  ...
  private async applySolution(agent: BrowserAgent, solution: any, context: any): Promise<boolean> {
    try {
      ...
      if (solutionText.includes('npm run dev') || solutionText.includes('start the development server')) {
      ...
      logger.info(`📝 Complex solution requires manual intervention: ${solution.solution.substring(0, 200)}...`);
