...
export class OnlineResearchAgent {
    ...
    try {
      ...
      const solutions = results.slice(0, 3).map(result => {
        ...
        return {
          ...
          url: result.url,
          title: result.title,
          solution: result.content.substring(0, 1000), // First 1000 chars
    ...
    try {
      ...
      const solutions = results.slice(0, 3).map(result => {
        ...
        return {
          ...
          url: result.url,
          title: result.title,
          solution: result.content.substring(0, 800), // First 800 chars
    ...
    try {
      ...
      const solutions = results.slice(0, 3).map(result => {
        ...
        return {
          ...
          url: result.url,
          title: result.title,
          solution: result.content.substring(0, 600), // First 600 chars
          confidence: Math.round(confidence)
        };
      });
  ...
  private async searchDevCommunity(query: ResearchQuery): Promise<any[]> {
    try {
      ...
      // Convert SearXNG results to our format
      const solutions = results.slice(0, 3).map(result => {
        // Community content varies in quality, so moderate confidence
  ...
  private async fallbackSearch(query: ResearchQuery): Promise<ResearchResult | null> {
    try {
      ...
      const solutions = results.slice(0, 5).map(result => ({
        ...
        url: result.url,
        title: result.title,
        solution: result.content.substring(0, 800),
