...
describe('DSPyKnowledgeManager', () => {
  ...
  describe('searchKnowledge', () => {
    it('should search by type', async () => {
      ...
      await km.storeKnowledge(knowledgeUtils.createKnowledge(
        'solution',
        'Test Solution',
        { solution: 'test' }
      ));
    ...
    it('should search by content', async () => {
      const id = await km.storeKnowledge(knowledgeUtils.createKnowledge(
        'solution',
  ...
  describe('updateKnowledge', () => {
    ...
    it('should evolve knowledge content with DSPy', async () => {
      const id = await km.storeKnowledge(knowledgeUtils.createKnowledge(
        'pattern',
        'Design Pattern',
        { pattern: 'Observer', usage: 'Event handling' }
      ));

      const updated = await km.updateKnowledge(id, {
        content: { pattern: 'Observer', usage: 'Event handling', examples: ['DOM events'] }
      });
  ...
  describe('event emission', () => {
    it('should emit events on knowledge operations', async () => {
      ...
      km.on('knowledge_stored', (event) => { storedEvent = event; });
      km.on('knowledge_updated', (event) => { updatedEvent = event; });
      km.on('knowledge_deleted', (event) => { deletedEvent = event; });
