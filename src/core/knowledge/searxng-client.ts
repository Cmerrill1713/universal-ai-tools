...
export class SearXNGClient {
  ...
  async search(params: SearXNGSearchParams): Promise<SearXNGResponse> {
    ...
    if (params.safesearch !== undefined) searchParams.append('safesearch', params.safesearch.toString());
  ...
  async searchDocumentation(query: string, technology: string): Promise<SearXNGResult[]> {
    ...
    const docResults = response.results.filter(result => 
  ...
  async searchDevCommunity(query: string): Promise<SearXNGResult[]> {
    ...
    return [...response.results, ...devToResponse.results]
      .sort((a, b) => b.score - a.score);
  }

  async multiEngineSearch(query: string, engines: string[] = ['duckduckgo', 'google', 'bing']): Promise<SearXNGResult[]> {
    const response = await this.search({
      q: query,
  ...
  async getEngineStatus(): Promise<{ [engine: string]: boolean }> {
    try {
      ...
      logger.warn('Could not fetch engine status:', error);
      return {};
    }
  }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      logger.warn('SearXNG health check failed:', error);
      return false;
    }
  }
