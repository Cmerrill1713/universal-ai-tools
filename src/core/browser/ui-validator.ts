...
export class UIValidator {
  ...
  private async testPageLoad(agent: BrowserAgent): Promise<TestResult> {
    ...
    try {
      ...
      return {
        ...
        error: success ? undefined : `Invalid page title: ${title}`
      };
    } catch (error) {
      return {
        name: 'Page Load',
  ...
  private async testNavigation(agent: BrowserAgent): Promise<TestResult> {
    ...
    try {
      ...
      for (const route of routes) {
        // Navigate to route
        if (agent.type === 'puppeteer') {
          await (agent.page as Page).goto(`${this.testUrl}${route.path}`, { waitUntil: 'networkidle0' });
          // Wait for page content to load
          await (agent.page as Page).waitForSelector('h2', { timeout: 5000 });
        } else {
          await (agent.page as PlaywrightPage).goto(`${this.testUrl}${route.path}`, { waitUntil: 'networkidle' });
          // Wait for page content to load
  ...
  private async testDashboard(agent: BrowserAgent): Promise<TestResult> {
    ...
    try {
      ...
      if (agent.type === 'puppeteer') {
        ...
        await (agent.page as Page).waitForSelector('[data-testid="dashboard"], h2', { timeout: 5000 });
        ...
        // Check for dashboard elements
        await (agent.page as PlaywrightPage).waitForSelector('[data-testid="dashboard"], h2', { timeout: 5000 });
  ...
  private async testToolsPage(agent: BrowserAgent): Promise<TestResult> {
    ...
    try {
      ...
      if (agent.type === 'puppeteer') {
        ...
        await (agent.page as PlaywrightPage).goto(`${this.testUrl}/tools`, { waitUntil: 'networkidle' });
  ...
  private async testAgentsPage(agent: BrowserAgent): Promise<TestResult> {
    ...
    try {
      ...
      if (agent.type === 'puppeteer') {
        ...
        await (agent.page as PlaywrightPage).goto(`${this.testUrl}/agents`, { waitUntil: 'networkidle' });
  ...
  private async testChatPage(agent: BrowserAgent): Promise<TestResult> {
    ...
    try {
      ...
      if (agent.type === 'puppeteer') {
        ...
        await (agent.page as PlaywrightPage).goto(`${this.testUrl}/chat`, { waitUntil: 'networkidle' });
        
        // Wait for chat page to load
        await (agent.page as PlaywrightPage).waitForSelector('h2', { timeout: 5000 });
        
        // Check for chat-specific elements
        const chatElements = await (agent.page as PlaywrightPage).$$('button, .card, input, textarea');
        if (chatElements.length === 0) {
          throw new Error('No chat elements found');
        }
      }
  ...
  private async testButtonFunctionality(agent: BrowserAgent): Promise<TestResult> {
    ...
    try {
      ...
      const pagesToTest = [
        { path: '/memory', buttonSelectors: ['button[data-testid="create-memory"], button:has-text("Store New Memory"), button:has-text("Search")'] },
        { path: '/tools', buttonSelectors: ['button[data-testid="create-tool"], button:has-text("Create Tool"), button:has-text("Execute")'] },
        { path: '/agents', buttonSelectors: ['button[data-testid="create-agent"], button:has-text("Create Agent"), button:has-text("Start")'] },
        { path: '/chat', buttonSelectors: ['button[data-testid="send-message"], button:has-text("Send"), button:has-text("Clear")'] }
      ];

      for (const page of pagesToTest) {
        if (agent.type === 'puppeteer') {
          await (agent.page as Page).goto(`${this.testUrl}${page.path}`, { waitUntil: 'networkidle0' });
          ...
          await (agent.page as PlaywrightPage).goto(`${this.testUrl}${page.path}`, { waitUntil: 'networkidle' });
