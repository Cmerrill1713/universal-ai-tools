...
export class PerformanceMonitor extends EventEmitter {
  ...
  async measureAgent(agent: BrowserAgent): Promise<PerformanceReport> {
    ...
    try {
      ...
      logger.info(`Performance measurement complete for agent ${agent.id}: Score ${report.benchmarks.performanceScore}`);
      return report;
  ...
  private async navigateAndMeasure(agent: BrowserAgent, metrics: PerformanceMetrics): Promise<void> {
    const startTime = Date.now();
  ...
  private async collectMemoryUsage(agent: BrowserAgent, metrics: PerformanceMetrics): Promise<void> {
    try {
      if (agent.type === 'puppeteer') {
        const page = agent.page as Page;
  ...
  private async collectNetworkMetrics(agent: BrowserAgent, metrics: PerformanceMetrics): Promise<void> {
    try {
      if (agent.type === 'puppeteer') {
        const page = agent.page as Page;
  ...
  private generateBenchmarks(metrics: PerformanceMetrics): PerformanceReport['benchmarks'] {
    ...
    if (metrics.pageLoadTime > 3000) {
      score -= 20;
      recommendations.push('Page load time is too slow (>3s). Consider optimizing bundle size and lazy loading.');
    ...
    if (metrics.firstContentfulPaint > 2000) {
      score -= 15;
      recommendations.push('First Contentful Paint is too slow (>2s). Optimize critical rendering path.');
    ...
    if (metrics.largestContentfulPaint > 4000) {
      score -= 20;
      recommendations.push('Largest Contentful Paint is too slow (>4s). Optimize images and critical resources.');
    ...
    } else if (metrics.memoryUsage.usedJSHeapSize > 25 * 1024 * 1024) { // 25MB
      score -= 5;
    ...
    if (metrics.networkRequests.failed > 0) {
      score -= 25;
      recommendations.push(`${metrics.networkRequests.failed} network requests failed. Check API connectivity.`);
    }
    
    if (metrics.networkRequests.avgResponseTime > 1000) {
      score -= 15;
      recommendations.push('Average API response time is slow (>1s). Optimize backend performance.');
  ...
  private async collectSystemMetrics(): Promise<void> {
    try {
      ...
      logger.debug(`System metrics - API Health: ${healthData.status}, Response Time: ${apiResponseTime}ms`);
  ...
  async runChecks(): Promise<any> {
    ...
    try {
      ...
      logger.info(`Performance checks complete: UI=${uiAvailable}, API=${apiAvailable}, Response Time=${apiResponseTime}ms`);
      return checks;
  ...
  generateReport(): string {
    ...
    const avgPageLoadTime = allMetrics.reduce((sum, m) => sum + m.pageLoadTime, 0) / allMetrics.length;
    const avgMemoryUsage = allMetrics.reduce((sum, m) => sum + m.memoryUsage.usedJSHeapSize, 0) / allMetrics.length;
