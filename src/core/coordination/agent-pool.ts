...
export class BrowserAgentPool extends EventEmitter {
  ...
  private async createPuppeteerAgents(): Promise<void> {
    ...
    for (const config of configs) {
      try {
        ...
        page.on('error', (error) => {
          logger.error(`Puppeteer agent ${config.id} error:`, error);
          this.handleAgentError(config.id, error);
        });

        page.on('pageerror', (error) => {
          logger.error(`Puppeteer agent ${config.id} page error:`, error);
          this.handleAgentError(config.id, error);
        });
  ...
  private async createPlaywrightAgents(): Promise<void> {
    const configs = [
      { id: 'playwright-chrome-desktop-1', browser: 'chromium', viewport: { width: 1920, height: 1080 } },
      { id: 'playwright-chrome-desktop-2', browser: 'chromium', viewport: { width: 1366, height: 768 } },
      { id: 'playwright-chrome-desktop-3', browser: 'chromium', viewport: { width: 1440, height: 900 } },
      { id: 'playwright-chrome-mobile-1', browser: 'chromium', viewport: { width: 375, height: 812 } },
      { id: 'playwright-chrome-mobile-2', browser: 'chromium', viewport: { width: 414, height: 896 } },
      { id: 'playwright-firefox-desktop-1', browser: 'firefox', viewport: { width: 1920, height: 1080 } },
      { id: 'playwright-firefox-desktop-2', browser: 'firefox', viewport: { width: 1366, height: 768 } },
      { id: 'playwright-firefox-mobile-1', browser: 'firefox', viewport: { width: 375, height: 812 } },
      { id: 'playwright-safari-desktop-1', browser: 'webkit', viewport: { width: 1920, height: 1080 } },
      { id: 'playwright-safari-desktop-2', browser: 'webkit', viewport: { width: 1366, height: 768 } },
      { id: 'playwright-safari-mobile-1', browser: 'webkit', viewport: { width: 375, height: 812 } },
      { id: 'playwright-edge-desktop-1', browser: 'chromium', viewport: { width: 1920, height: 1080 } }
    ];
    ...
    for (const config of configs) {
      try {
        ...
        // Set up error handling
        page.on('pageerror', (error) => {
          logger.error(`Playwright agent ${config.id} page error:`, error);
          this.handleAgentError(config.id, error);
        });
  ...
  async executeOnAllAgents<T>(task: (agent: BrowserAgent) => Promise<T>): Promise<T[]> {
    ...
    const promises = agents.map(agent => 
      this.executeOnAgent(agent.id, task)
    );
