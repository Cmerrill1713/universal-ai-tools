...
export class TaskManager extends EventEmitter {
  ...
  async createTask(request: TaskCreateRequest): Promise<Task> {
    const task: Task = {
      ...
      timeout: request.timeout || 60000, // 1 minute default
  ...
  private async handleStatusChange(task: Task, oldStatus: Task['status'], newStatus: Task['status']): Promise<void> {
    // Clean up timeout
    ...
    for (const dependentId of dependents) {
      ...
      if (dependentTask && dependentTask.status === 'pending') {
        ...
        await this.updateTask(dependentId, { 
          status: 'cancelled', 
          error: `Dependency failed: ${failedTaskId}` 
        });
      }
    }
  }
  ...
  async getTaskStats(): Promise<{
    ...
    const byType: Record<Task['type'], number> = {
      research: 0, test: 0, execute: 0, monitor: 0, coordinate: 0
    };
  ...
  async cleanup(): Promise<void> {
    ...
    const tasksToClean = Array.from(this.tasks.values())
      .filter(task => 
