...
export const AgentCollaborateSchema = z.object({
  agents: z.array(z.enum(['analytical', 'creative', 'critical', 'systems', 'research'])).min(2).max(5),
  task: z.string().min(1).max(5000),
...
export const APIRequestSchema = <T extends z.ZodType>(dataSchema: T) => z.object({
  data: dataSchema,
  metadata: z.object({
...
export const BatchOperationSchema = <T extends z.ZodType>(itemSchema: T) => z.object({
...
export const WebSocketMessageSchema = z.object({
  type: z.enum(['chat', 'agent_update', 'memory_sync', 'voice_stream', 'error']),
  data: z.any(),
  timestamp: DateSchema,
