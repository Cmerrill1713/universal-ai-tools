...
export class WriteBehindCache<T = any> extends EventEmitter {
  ...
  async get(key: string): Promise<T | undefined> {
    ...
    try {
      ...
      logger.error(`Write-behind cache get error for key ${key}:`, error);
      this.emit('error', error);
    }
  ...
  async delete(key: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error(`Write-behind cache delete error for key ${key}:`, error);
      this.emit('error', error);
  ...
  async has(key: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error(`Write-behind cache has error for key ${key}:`, error);
      return false;
    }
  }
