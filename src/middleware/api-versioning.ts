...
export class ApiVersioningMiddleware {
  ...
  versionDetection() {
    return (req: VersionedRequest, res: Response, next: NextFunction) => {
      ...
      if (!version && acceptHeader) {
        const versionMatch = acceptHeader.match(/application\/vnd\.universal-ai-tools\.(v\d+)\+json/);
        if (versionMatch) {
  ...
  addVersion(version: ApiVersion): void {
    ...
    if (validated.active) {
      const currentLatestNum = parseInt(this.latestVersion.slice(1));
      const newVersionNum = parseInt(validated.version.slice(1));
  ...
  deprecateVersion(version: string, sunsetDate: Date): void {
    ...
    if (versionInfo) {
      ...
      logger.warn(`API version ${version} has been deprecated. Sunset date: ${sunsetDate.toISOString()}`);
    }
  }
...
export function versionedErrorHandler(version: string) {
  return (err: Error, req: Request, res: Response, next: NextFunction) => {
    logger.error(`API ${version} error:`, err);
