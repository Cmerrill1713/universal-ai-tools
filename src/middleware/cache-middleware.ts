...
export class CacheMiddleware {
  ...
  private revalidationQueue: Map<string, Promise<any>>;
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        if (cached) {
          ...
          if (ifModifiedSince && 
              new Date(ifModifiedSince) >= new Date(cached.lastModified)) {
            res.status(304).end();
            return;
          }

          // Check if stale content can be served while revalidating
      ...
      if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(req.method)) {
        res.on('finish', async () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            const invalidatePattern = req.headers['x-cache-invalidate-pattern'];
            
            if (invalidateTags || invalidatePattern) {
              const tags = invalidateTags 
                ? String(invalidateTags).split(',').map(t => t.trim())
                : undefined;
