...
export class SecurityMiddleware {
  ...
  constructor(options: SecurityOptions = {}) {
    ...
    const cleanupInterval = setInterval(() => {
      try {
        ...
        logger.error('Error cleaning up CSRF tokens', LogContext.SECURITY, { error: error instanceof Error ? error.message : String(error) });
      }
    }, 3600000); // Every hour
  ...
  public securityHeaders() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (config.server.isProduction && this.options.enableCSP) {
        const nonce = createHash('sha256').update(Date.now() + Math.random().toString()).digest('base64').slice(0, 16);
  ...
  private generateCSP(): string {
    logger.warn('generateCSP() is deprecated. Use getHelmetMiddleware() for CSP configuration', LogContext.SECURITY);
    const cspDirectives = [
      "default-src 'self'",
  ...
  public sanitizeInput() {
    return (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        // Sanitize parameters
        if (req.params) {
          req.params = JSON.parse(securityHardeningService.sanitizeInput(JSON.stringify(req.params)));
        }
        ...
        logger.error('Input sanitization error:', LogContext.SECURITY, { error: error instanceof Error ? error.message : error, stack: error instanceof Error ? error.stack : undefined });
        res.status(400).json({
          error: 'Invalid input',
...
export const securityMiddleware = {
  get ipAccessControl() { return getSecurityMiddleware().ipAccessControl.bind(getSecurityMiddleware()); },
  get requestSizeLimit() { return getSecurityMiddleware().requestSizeLimit.bind(getSecurityMiddleware()); },
  get getHelmetMiddleware() { return getSecurityMiddleware().getHelmetMiddleware.bind(getSecurityMiddleware()); },
  get getCorsMiddleware() { return getSecurityMiddleware().getCorsMiddleware.bind(getSecurityMiddleware()); },
  get getExpressRateLimiter() { return getSecurityMiddleware().getExpressRateLimiter.bind(getSecurityMiddleware()); },
  get sanitizeInput() { return getSecurityMiddleware().sanitizeInput.bind(getSecurityMiddleware()); },
  get csrfProtection() { return getSecurityMiddleware().csrfProtection.bind(getSecurityMiddleware()); },
  get securityAuditLogger() { return getSecurityMiddleware().securityAuditLogger.bind(getSecurityMiddleware()); }
};
...
export function applySecurityMiddleware(app: any) {
  ...
  try {
    ...
    logger.error('Failed to apply security middleware', LogContext.SECURITY, { error: error instanceof Error ? error.message : String(error) });
    throw error;
  }
