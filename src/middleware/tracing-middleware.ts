...
import { SpanContext, SpanKind, SpanStatusCode, context, propagation, trace } from '@opentelemetry/api';
...
interface TracingOptions {
  ...
  ignoreRoutes?: string[];
  customAttributesExtractor?: (req: Request) => Record<string, any>;
...
export function createTracingMiddleware(options: TracingOptions = {}): (req: TracedRequest, res: Response, next: NextFunction) => void {
  ...
  return (req: TracedRequest, res: Response, next: NextFunction) => {
    ...
    const span = tracer.startSpan(
      spanName,
      {
        ...
        attributes: {
          ...
          'http.request_id': req.get('x-request-id') || `req-${Date.now()}`,
    ...
    if (mergedOptions.customAttributesExtractor) {
      try {
        ...
        logger.error('Error extracting custom attributes', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      }
    }
    ...
    if (mergedOptions.recordRequestBody && req.body) {
      try {
        ...
        span.setAttribute('http.request.body', bodyStr.substring(0, 1000)); // Limit size
        span.setAttribute('http.request.body.size', bodyStr.length);
      } catch (error) {
        span.setAttribute('http.request.body.error', 'Failed to serialize request body');
      }
    }
    ...
    const captureResponse = (body: any) => {
      ...
      if (res.statusCode >= 400) {
        ...
        if (res.locals.error) {
          ...
          span.setAttribute('error.type', res.locals.error.name || 'Error');
    ...
    // Run the request handler with the span context
...
export function createTracingErrorMiddleware(options: TracingOptions = {}): (err: Error, req: TracedRequest, res: Response, next: NextFunction) => void {
  return (err: Error, req: TracedRequest, res: Response, next: NextFunction) => {
    ...
    if (span && options.errorFilter?.(err) !== false) {
      ...
      if (req.traceId) {
        span.setAttribute('error.trace_id', req.traceId);
      }
      if (req.userId) {
        span.setAttribute('error.user_id', req.userId);
      }
      if (req.aiService) {
        span.setAttribute('error.ai_service', req.aiService);
      }
    }
...
export function createLoggingContextMiddleware(): (req: TracedRequest, res: Response, next: NextFunction) => void {
  return (req: TracedRequest, res: Response, next: NextFunction) => {
    const span = trace.getActiveSpan();
    if (span) {
      const spanContext = span.spanContext();
