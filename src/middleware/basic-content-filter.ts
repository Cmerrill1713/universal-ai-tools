...
export const basicInputFilter = (req: Request, res: Response, next: NextFunction): void => {
  try {
    ...
    if (req.body?.message) {contentToCheck = req.body.message;} else if (req.body?.prompt) {contentToCheck = req.body.prompt;} else if (req.body?.content) {contentToCheck = req.body.content;} else if (req.body?.query) {contentToCheck = req.body.query;}

    if (contentToCheck) {
      const result = checkContent(contentToCheck);
      
      if (!result.allowed) {
        log.warn(`🛡️ Content blocked: ${result.reason}`, LogContext.API);
        res.status(400).json({
          success: false,
          error: {
      ...
      if (result.sanitized) {
        log.warn('🧹 Content sanitized (PII removed)', LogContext.API);
    ...
    log.error('Content filter error', LogContext.API, { error });
    res.status(500).json({
      success: false,
      error: {
        code: 'FILTER_ERROR',
    ...
    try {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300 && data) {
        ...
        if (responseContent) {
          ...
          if (!result.allowed) {
            log.warn(`🛡️ Response blocked: ${result.reason}`, LogContext.API);
            const safeResponse = {
              success: false,
              error: {
          ...
          // Apply sanitization if needed
          if (result.sanitized) {
            log.warn('🧹 Response sanitized (PII removed)', LogContext.API);
            if (typeof data === 'string') {
      ...
      log.error('Output filter error', LogContext.API, { error });
    }
