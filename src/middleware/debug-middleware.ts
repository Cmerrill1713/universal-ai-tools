  ...
  namespace Express {
    interface Request {
      ...
      debugTools: {
        ...
        error: (error: Error, context: string, metadata?: Record<string, any>) => void;
        performance: {
          start: (operation: string, metadata?: Record<string, any>) => string;
          end: (traceId: string, operation: string, metadata?: Record<string, any>) => void;
        };
...
export class DebugMiddleware {
  ...
  static debugSession() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      req.debugTools = {
        ...
        error: (error: Error, context: string, metadata?: Record<string, any>) => {
          trackError(req.debugSessionId!, error, context, {
            request_id: req.requestId,
            method: req.method,
            path: req.path,
            ...metadata
        ...
        performance: {
          ...
          end: (traceId: string, operation: string, metadata?: Record<string, any>) => {
      ...
      res.on('finish', async () => {
        try {
          req.debugTools.log('info', `Request completed: ${req.method} ${req.path}`, LogContext.API, {
  ...
  static verboseLogging() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      logger.debug('Verbose request logging', LogContext.API, {
        request_id: req.requestId,
        method: req.method,
        path: req.path,
        query: req.query,
  ...
  static athenaDebugger() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      const athenaDebugData = {
        interactionType: req.body?.interaction_type || DebugMiddleware.inferInteractionType(req.path),
        ...
        userInput: req.body?.message || req.body?.user_input,
        timestamp: new Date(),
      ...
      res.json = function(body: any) {
        ...
        if (req.debugTools?.athena) {
          req.debugTools.athena({
            ...
            errors: res.statusCode >= 400 ? [body?.error || 'Unknown error'] : undefined
          });
        }
  ...
  static performanceDebugger() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      req.debugPerformance = {
        ...
        addMarker: (name: string, metadata?: Record<string, any>) => {
          ...
          if (req.debugTools?.log) {
            req.debugTools.log('debug', `Performance marker: ${name}`, LogContext.PERFORMANCE, marker);
          }
      ...
      res.on('finish', () => {
        ...
        if (!res.headersSent) {
          res.setHeader('X-Debug-Performance', JSON.stringify({
  ...
  static testResultAggregator() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      req.testAggregator = {
        recordResult: (testSuite: string, testName: string, status: 'pass' | 'fail' | 'skip', duration: number, error?: string) => {
          const testResult = {
            testName,
            status,
            duration,
          ...
          if (req.debugTools?.log) {
            req.debugTools.log('info', `Test result: ${testName} - ${status}`, LogContext.TEST, testResult);
          }

          // Store in test aggregation system
          const existingAggregation = debugTools.getAllTestAggregations().find(a => a.testSuite === testSuite);
  ...
  static errorDebugger() {
    return (err: Error, req: Request, res: Response, next: NextFunction) => {
      // Enhanced error debugging in development
      if (process.env.NODE_ENV === 'development' || req.headers['x-debug-errors']) {
        
        // Track error in debug session if available
        if (req.debugTools?.error) {
          req.debugTools.error(err, 'middleware_error', {
            path: req.path,
            method: req.method,
            query: req.query,
            body: DebugMiddleware.sanitizeBody(req.body)
        ...
        logger.error('Debug error details', LogContext.SYSTEM, {
          request_id: req.requestId,
          debug_session: req.debugSessionId,
          error: {
            name: err.name,
        ...
        // Add debug information to error response in development
  ...
  private static sanitizeHeaders(headers: any): any {
    ...
    const sensitiveHeaders = [
      ...
      'x-secret-key', 'x-private-key', 'password'
    ];
  ...
  private static sanitizeBody(body: any): any {
    ...
    const sensitiveFields = [
      'password', 'secret', 'token', 'key', 'apiKey', 'authToken',
...
declare module 'express-serve-static-core' {
  interface Request {
    debugPerformance?: {
      data: any;
      addMarker: (name: string, metadata?: Record<string, any>) => any;
    };
    testAggregator?: {
      recordResult: (testSuite: string, testName: string, status: 'pass' | 'fail' | 'skip', duration: number, error?: string) => any;
