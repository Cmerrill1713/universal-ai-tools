  ...
  async get(key: string): Promise<RateLimitInfo | null> {
    try {
      ...
      return {
        ...
        firstRequest: new Date(data.first_request).getTime(),
    ...
    try {
      await this.supabase
        ...
        .upsert({
          ...
          first_request: new Date(value.firstRequest),
...
export class RateLimiter {
   ...
   */
  public limit(configOrName: string | RateLimitConfig): (req: Request, res: Response, next: NextFunction) => Promise<void> {
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        // Get configuration
        const rateConfig = typeof configOrName === 'string'
        ...
        // Generate key
        const key = rateConfig.keyGenerator
          ? rateConfig.keyGenerator(req)
  ...
  /**
   * Check if request is suspicious (potential DDoS)
   */
  private isSuspiciousRequest(req: Request): boolean {
    ...
    // Check for request flooding (multiple requests in very short time)
