...
export class UniversalAIToolsClient {
    ...
    this.client.interceptors.response.use(
      (response) => {
        ...
        if (apiVersion && apiVersion !== this.currentVersion) {
          console.log(`[API] Server returned version ${apiVersion}, client using ${this.currentVersion}`);
        }
  ...
  private async handleVersionError(error: any): Promise<any> {
    const { supportedVersions, latestVersion } = error.response.data.error;
    ...
    if (this.config.autoUpgrade && supportedVersions.includes(latestVersion)) {
      ...
      this.client.defaults.headers['Accept'] = `application/vnd.universal-ai-tools.${latestVersion}+json`;
  ...
  private async retryRequest(error: any): Promise<any> {
    ...
    console.log(`[API] Retrying request (attempt ${error.config.__retryCount}/${this.config.retryAttempts}) after ${delay}ms`);
  ...
  async getVersions(): Promise<ApiResponse<{
