...
export class RedisInstrumentation {
  ...
  instrumentRedisClient(client: any): any {
    ...
    const commands = [
      // String operations
      'get', 'set', 'mget', 'mset', 'del', 'exists', 'expire', 'ttl',
      ...
      // Other operations
      'ping', 'flushdb', 'flushall', 'keys', 'scan'
    ];
  ...
  async withCacheSpan<T>(
    ...
    const span = this.tracer.startSpan(spanName, {
      ...
      attributes: {
        ...
        'net.peer.port': parseInt(process.env.REDIS_PORT || '6379'),
      },
    });
  ...
  private wrapPipeline(pipelineFn: any): any {
    ...
    return function() {
      ...
      const commands = Object.getOwnPropertyNames(pipeline)
        .filter(prop => typeof pipeline[prop] === 'function' && prop !== 'exec');
   ...
   */
  wrapCacheFunction<T extends (...args: any[]) => Promise<any>>(
    fn: T,
    options: {
      operation: string;
    ...
    try {
      ...
      if (client.info) {
        ...
        if (stats.used_memory) {
          span.setAttribute('cache.health.memory_used', parseInt(stats.used_memory));
        }
        if (stats.connected_clients) {
          span.setAttribute('cache.health.connected_clients', parseInt(stats.connected_clients));
        }
        if (stats.total_commands_processed) {
          span.setAttribute('cache.health.total_commands', parseInt(stats.total_commands_processed));
        }
        if (stats.evicted_keys) {
          span.setAttribute('cache.health.evicted_keys', parseInt(stats.evicted_keys));
        }
...
export const instrumentRedis = (client: any) => 
  redisInstrumentation.instrumentRedisClient(client);

export const withCacheSpan = <T>(operation: CacheOperation, fn: () => Promise<T>) =>
  redisInstrumentation.withCacheSpan(operation, fn);

export const wrapCacheFunction = <T extends (...args: any[]) => Promise<any>>(
  fn: T,
  options: any
