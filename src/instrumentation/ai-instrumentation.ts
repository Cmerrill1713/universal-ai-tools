...
export class AIInstrumentation {
  ...
  async withAISpan<T>(
    ...
    const span = this.tracer.startSpan(spanName, {
      ...
      attributes: {
        ...
        'ai.request.max_tokens': operation.maxTokens,
        'ai.request.temperature': operation.temperature,
        'ai.request.stream': operation.stream || false,
        'ai.request.prompt_preview': operation.prompt?.substring(0, 100),
        'ai.request.prompt_length': operation.prompt?.length || 0,
      },
    });
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('rate limit')) {
          span.setAttribute('ai.error.type', 'rate_limit');
        } else if (error.message.includes('context length')) {
          span.setAttribute('ai.error.type', 'context_length_exceeded');
        } else if (error.message.includes('timeout')) {
          span.setAttribute('ai.error.type', 'timeout');
        } else if (error.message.includes('authentication')) {
          span.setAttribute('ai.error.type', 'authentication');
        }
      }
  ...
  private recordAIResponse(span: Span, response: any): void {
    ...
    if (response.choices?.[0]) {
      ...
      if (choice.message?.content) {
        ...
        span.setAttribute('ai.response.content_preview', choice.message.content.substring(0, 100));
      }
   ...
   */
  createTracedAIFunction<T extends (...args: any[]) => Promise<any>>(
    fn: T,
    service: string,
    operation: string,
   ...
   */
  async monitorAIServiceHealth(service: string, healthCheckFn: () => Promise<boolean>): Promise<void> {
    ...
    try {
      ...
      span.setAttribute('ai.health.status', 'error');
    } finally {
      span.end();
    }
  }
...
export const instrumentOpenAI = (client: any) =>
  aiInstrumentation.instrumentOpenAI(client);

export const instrumentAnthropic = (client: any) =>
  aiInstrumentation.instrumentAnthropic(client);

export const createTracedAIFunction = <T extends (...args: any[]) => Promise<any>>(
  fn: T,
  service: string,
  operation: string,
  modelExtractor: (...args: Parameters<T>) => string
