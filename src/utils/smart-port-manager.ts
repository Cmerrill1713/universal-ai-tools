...
export class SmartPortManager extends EventEmitter {
   ...
   */
  async findAvailablePort(preferredPort: number, range?: { start: number; end: number }): Promise<number> {
  ...
  async scanPortRange(startPort: number, endPort: number): Promise<number[]> {
    ...
    for (let i = startPort; i <= endPort; i += batchSize) {
      ...
      for (let port = i; port <= batchEnd; port++) {
        batch.push(this.checkPortAvailability(port).then(available => {
  ...
  async resolvePortConflict(service: string, requestedPort: number): Promise<number> {
    ...
    console.log(`Port ${requestedPort} is unavailable for ${service}, finding alternative...`);

    // Try fallback ports
    for (const fallbackPort of serviceConfig.fallbackPorts) {
      if (await this.checkPortAvailability(fallbackPort)) {
        console.log(`Resolved to fallback port ${fallbackPort} for ${service}`);
        this.emit('portConflictResolved', { service, original: requestedPort, resolved: fallbackPort });
        return fallbackPort;
      }
    }
    ...
    console.log(`Resolved to port ${availablePort} for ${service}`);
    this.emit('portConflictResolved', { service, original: requestedPort, resolved: availablePort });
  ...
  monitorPortChanges(interval: number = 30000): void {
    ...
    console.log(`Port monitoring started with ${interval}ms interval`);
  }
  ...
  async generateOptimalPortConfig(): Promise<PortConfiguration> {
    ...
    for (const [serviceName, serviceConfig] of this.services) {
      try {
        ...
        console.error(`Failed to assign port for ${serviceName}:`, error);
  ...
  private async checkServiceHealth(config: ServiceConfig, port: number): Promise<'healthy' | 'unhealthy' | 'unknown'> {
    try {
  ...
  private async identifyPortService(port: number): Promise<{ service: string; pid?: number } | null> {
    try {
      if (this.platform === 'darwin' || this.platform === 'linux') {
