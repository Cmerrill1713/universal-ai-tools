...
export function validatePath(filePath: string, options: PathValidationOptions = {}): boolean {
  ...
  const allowedPattern = new RegExp(`^[${baseAllowedPattern}${additionalAllowedChars}]+$`);
 ...
 */
export function sanitizeFilename(filename: string, options: { maxLength?: number; allowedExtensions?: string[] } = {}): string {
 ...
 */
export function createSecurePath(baseDir: string, filename: string, subdirs: string[] = []): string {
 ...
 */
export function generateSecureTempPath(baseDir: string, prefix = 'temp', extension = '.tmp'): string {
 ...
 */
export function auditPathSecurity(filePaths: string[], options: PathValidationOptions = {}): {
  valid: string[];
