...
export class CacheManager {
  ...
  public async extend(key: string, ttl: number, options: CacheOptions = {}): Promise<boolean> {
    try {
      ...
      logger.error('Cache extend error:', error);
      return false;
    }
  }

  public async getMultiple<T = any>(keys: string[], options: CacheOptions = {}): Promise<(T | null)[]> {
    try {
  ...
  public async setMultiple(pairs: Array<{ key: string; value: any; ttl?: number }>, options: CacheOptions = {}): Promise<boolean> {
    try {
      const multi = this.redis.multi();
