...
export class DatabaseOptimizer {
  ...
  private updateStats(queryType: keyof QueryStats['queryTypes'], responseTime: number, cached = false, error = false): void {
  ...
  public async select<T = any>(
    ...
    try {
      ...
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('select', responseTime, false, !!error);
      
      // Cache successful results
      if (!error && data && options.cache !== false) {
        const cacheKey = this.createCacheKey(table, query, options);
  ...
  public async insert<T = any>(
    ...
    try {
      ...
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('insert', responseTime, false, !!error);
  ...
  public async update<T = any>(
    ...
    try {
      ...
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('update', responseTime, false, !!error);
  ...
  public async delete<T = any>(
    ...
    try {
      ...
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('delete', responseTime, false, !!error);
  ...
  public async upsert<T = any>(
    ...
    try {
      ...
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const responseTime = seconds * 1000 + nanoseconds / 1000000;
      this.updateStats('insert', responseTime, false, !!error); // Treat as insert for stats
  ...
  public async analyzeSlowQueries(): Promise<Array<{ query: string; avgTime: number; count: number }>> {
  ...
  public async optimizeTable(table: string): Promise<{ suggestions: string[]; indexes: string[] }> {
    try {
      ...
      const { data: tableStats } = await this.supabase.rpc('get_table_stats', { table_name: table });
  ...
  public generateReport(): string {
    const cacheHitRate = this.stats.totalQueries > 0 ? (this.stats.cachedQueries / this.stats.totalQueries) * 100 : 0;
    const errorRate = this.stats.totalQueries > 0 ? (this.stats.errors / this.stats.totalQueries) * 100 : 0;
    const slowQueryRate = this.stats.totalQueries > 0 ? (this.stats.slowQueries / this.stats.totalQueries) * 100 : 0;
