...
export class BayesianModel implements BayesianPerformanceModel {
  ...
  private getContextMultiplier(context: Record<string, any>): number {
    ...
    if (context.complexity === 'simple') {multiplier *= 0.8;} else if (context.complexity === 'complex') {multiplier *= 1.2;}

    // Adjust based on urgency
    if (context.urgency === 'high') {multiplier *= 1.1;} else if (context.urgency === 'low') {multiplier *= 0.9;}
