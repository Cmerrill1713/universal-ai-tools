...
export class MetalOptimizer {
  ...
  private detectHardware() {
    try {
      ...
      if (this.isAppleSilicon) {
        ...
        if (memMatch) {
          this.gpuInfo.unifiedMemory = `${Math.round(parseInt(memMatch[1]) / (1024 * 1024 * 1024))  } GB`;
        }
   ...
   */
  getOllamaMetalSettings(): Record<string, any> {
    if (!this.isAppleSilicon) {
      return {};
    }
    ...
    const memSize = parseInt(this.gpuInfo.unifiedMemory) || 8;
    if (memSize >= 32) {
   ...
   */
  getLMStudioMetalSettings(): Record<string, any> {
    if (!this.isAppleSilicon) {
      return {};
    }
  ...
  private getOptimalThreadCount(): number {
    const cpus = os.cpus();
    const performanceCores = cpus.filter(cpu => 
      cpu.model.includes('Apple') && !cpu.model.includes('Efficiency')
    ).length;
   ...
   */
  getModelLoadingParams(modelSize: string): Record<string, any> {
    ...
    if (this.isAppleSilicon) {
      try {
        ...
        if (wiredMatch) {
          ...
          usage.gpuMemoryUsage = Math.round((wiredMemory / parseInt(this.gpuInfo.unifiedMemory)) * 100);
        }
      } catch (error) {
        // Ignore errors
      }
