...
export async function initializeWithTimeout<T>(
  ...
  try {
    ...
    if (options.critical) {
      ...
      logger.warn(`⚠️  ${serviceName} failed to initialize, continuing without it`, LogContext.SYSTEM, {
        error: errorMessage,
        duration
      });
...
export async function initializeWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      logger.info(`🔄 Attempting to initialize ${serviceName} (attempt ${attempt}/${maxRetries})...`);
