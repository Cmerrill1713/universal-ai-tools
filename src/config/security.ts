...
export const securityConfig = {
  ...
  cors: {
    ...
    origins: config.server.isProduction 
  ...
  policies: [
    {
      ...
      rules: [
        {
          type: 'deny',
          pattern: '(union|select|insert|update|delete|drop|create|alter|exec|script|javascript|vbscript|onload|onerror|onclick)',
          conditions: [
            {
              field: 'content-type',
              operator: 'contains',
    ...
    },
    {
      ...
      rules: [
        {
          type: 'deny',
          pattern: '<(script|iframe|object|embed|form|input|link|meta|base)',
          conditions: []
        }
      ]
    },
...
export function getRateLimitForEndpoint(endpoint: string, method: string): RateLimitRule | undefined {
...
export function isIPWhitelisted(ip: string): boolean {
  return securityConfig.rateLimiting.whitelist.includes(ip) ||
