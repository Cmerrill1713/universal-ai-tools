...
export function generateSecureDefaults(): void {
  ...
  // Generate JWT_SECRET if not set or insecure
  if (!process.env.JWT_SECRET || process.env.JWT_SECRET.length < 32 || 
      process.env.JWT_SECRET.includes('example') || process.env.JWT_SECRET === 'your-jwt-secret-here') {
    const jwtSecret = crypto.randomBytes(64).toString('base64');
    updates.push(`JWT_SECRET=${jwtSecret}`);
    process.env.JWT_SECRET = jwtSecret;
  }
  
  // Generate ENCRYPTION_KEY if not set or insecure
  if (!process.env.ENCRYPTION_KEY || process.env.ENCRYPTION_KEY.length < 32 || 
      process.env.ENCRYPTION_KEY.includes('example') || process.env.ENCRYPTION_KEY === 'your-encryption-key-here') {
