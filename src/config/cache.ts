...
export const getCacheConfig = (env: string = process.env.NODE_ENV || 'development'): CacheConfig => {
...
export const CACHE_KEY_PATTERNS = {
  ...
  // Model cache keys
  modelResponse: (modelId: string, hash: string) => 
    `model:response:${modelId}:${hash}`,
  embedding: (text: string, modelId: string) => 
    `embedding:${modelId}:${createHash(text)}`,
