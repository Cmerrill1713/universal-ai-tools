...
export class MultiStageSearchSystem {
  ...
  async search(
    ...
    try {
      ...
      const relevantClusters = await this.searchClusters(
        embedding,
        {
      ...
      this.logger.debug(`Found ${relevantClusters.length} relevant clusters in ${clusterSearchTime}ms`);
      ...
      if (relevantClusters.length > 0) {
        ...
        searchResults = await this.searchWithinClusters(
          embedding,
          relevantClusters,
          {
        ...
        this.logger.debug(`Found ${searchResults.length} memories in clusters in ${detailSearchTime}ms`);
      }

      // Stage 3: Fallback to full search if insufficient results
      if ((options.enableFallbackSearch ?? config.fallbackEnabled) && 
      ...
      this.logger.info(`Multi-stage search completed in ${totalTime}ms: ${clustersEvaluated} clusters, ${memoriesEvaluated} memories evaluated`);
      ...
      this.logger.error('Multi-stage search failed:', error);
      throw error;
    }
  }
  ...
  private async searchClusters(
    ...
    try {
      ...
      this.logger.error('Cluster search failed:', error);
      return [];
    }
  }
  ...
  private async searchWithinClusters(
    ...
    try {
      ...
      this.logger.error('Cluster detail search failed:', error);
      return [];
    }
  }
  ...
  async refreshSemanticClusters(): Promise<{
    ...
    try {
      ...
      this.logger.info(`Semantic clusters refreshed in ${processingTime}ms: ${data.clusters_created} clusters, ${data.memories_processed} memories`);
  ...
  analyzeSearchPerformance(metrics: SearchMetrics[]): {
    ...
    if (metrics.length === 0) {
      return {
        recommendations: ['No search metrics available for analysis'],
        averagePerformance: {
          totalTime: 0,
          clusterEfficiency: 0,
          fallbackRate: 0,
    ...
    const avgClusterTime = metrics.reduce((sum, m) => sum + m.clusterSearchTime, 0) / metrics.length;
    ...
    const clusterEfficiency = avgClusterTime > 0 ? (avgTotalTime - avgClusterTime) / avgTotalTime : 0;

    const recommendations: string[] = [];

    if (avgTotalTime > 500) {
      recommendations.push('Search times are high - consider increasing cluster threshold for faster searches');
    }

    if (fallbackRate > 0.3) {
      recommendations.push('High fallback rate - consider lowering cluster threshold or increasing max clusters');
    }
    ...
    if (clusterEfficiency < 0.5) {
      recommendations.push('Cluster search not providing significant benefit - review clustering parameters');
    }
