...
import type { AccessPatternLearner, LearningInsights, UtilityScore } from './access_pattern_learner';
...
export interface MemorySearchOptions {
  ...
  // Access pattern learning options
...
export class EnhancedMemorySystem {
    ...
    try {
      ...
      const { data, error } = await this.supabase
        ...
        .insert({
          ...
          memory_category: this.categorizeMemoryFromEnrichment(enrichmentResult.enrichment, memoryType)
        })
        .select()
        .single();
      ...
      this.logger.info(`Stored enriched memory with ${enrichmentResult.enrichment.entities.length} entities for ${serviceId}`);
  ...
  async multiStageSearchMemories(options: MemorySearchOptions): Promise<{
    ...
    try {
      ...
      this.logger.info(`Multi-stage search completed: ${metrics.clustersEvaluated} clusters, ${metrics.memoriesEvaluated} memories in ${metrics.totalSearchTime}ms`);
  ...
  private async generateEmbedding(text: string): Promise<number[]> {
    try {
      ...
      this.logger.error(`Failed to generate embedding using ${this.useOllama ? 'Ollama' : 'OpenAI'}:`, error);
      ...
      if (this.useOllama && this.embeddingService instanceof OllamaEmbeddingService) {
        ...
        if (!health.available) {
          this.logger.warn('Ollama is not available. Make sure Ollama is running at http://localhost:11434');
        } else if (!health.modelLoaded) {
          this.logger.warn(`Model ${this.embeddingModel} is not loaded. Try running: ollama pull ${this.embeddingModel}`);
        }
      }
  ...
  async preWarmMemoryCache(limit = 100): Promise<void> {
    try {
      ...
      if (frequentMemories) {
        ...
        this.logger.info(`Pre-warmed cache with ${formattedMemories.length} frequently accessed memories`);
      }
    } catch (error) {
  ...
  private calculateEnrichedImportance(enrichment: any, baseImportance: number): number {
    ...
    if (enrichment.complexity.technicalLevel === 'expert' || enrichment.complexity.technicalLevel === 'advanced') {
      adjustedImportance += 0.1;
    }
  ...
  async contextualSearch(
    ...
    try {
      ...
      this.logger.debug(`Query enrichment found ${queryEnrichment.enrichment.entities.length} entities and ${queryEnrichment.enrichment.concepts.length} concepts`);

      // Use contextual content for embedding generation
  ...
  private extractEntities(content: string): any[] {
    ...
    // Extract URLs
    const urls = content.match(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/g);
    if (urls) {
   ...
   */
  private categorizeMemory(memoryType: string, content: string): string {
    // Simple categorization logic
    if (memoryType === 'consolidated') return 'consolidated';
    if (content.toLowerCase().includes('task') || content.toLowerCase().includes('todo')) return 'task';
    if (content.toLowerCase().includes('meeting') || content.toLowerCase().includes('appointment')) return 'calendar';
    if (content.toLowerCase().includes('code') || content.toLowerCase().includes('function')) return 'technical';
    return 'general';
  }
   ...
   */
  private async createSimilarityConnections(
    memoryId: string,
    embedding: number[]
  ): Promise<void> {
    try {
  ...
  /**
   * Record memory access pattern using the access learner
   */
  async recordMemoryAccess(
    ...
    try {
      await this.accessLearner.recordAccess(
        memoryId,
        agentName,
        accessType,
        {
  ...
  async recordUserFeedback(
    ...
    try {
      await this.accessLearner.recordUserFeedback(
        memoryId,
        agentName,
        feedback,
        followUpQueries
      );
      this.logger.info(`Recorded user feedback for memory ${memoryId}: relevance=${feedback.relevance}`);
    } catch (error) {
  ...
  async refreshSemanticClusters(): Promise<{
    ...
    try {
      ...
      this.logger.info(`Semantic clusters refreshed: ${result.clustersCreated} clusters created, ${result.memoriesProcessed} memories processed`);
  ...
  async checkEmbeddingServiceHealth(): Promise<{
    ...
    if (this.useOllama && this.embeddingService instanceof OllamaEmbeddingService) {
      ...
      if (!health.available) {
        ...
        recommendations.push('Alternative models: ollama pull all-minilm, ollama pull mxbai-embed-large');
      }
      ...
      return {
        ...
        error: process.env.OPENAI_API_KEY ? undefined : 'OPENAI_API_KEY environment variable not set',
        recommendations: process.env.OPENAI_API_KEY ? undefined : [
          'Set OPENAI_API_KEY environment variable',
          'Or switch to Ollama for local embeddings'
        ]
      };
    }
  }
   ...
   */
  switchEmbeddingService(useOllama: boolean, config?: EmbeddingConfig | OllamaEmbeddingConfig): void {
