...
export class OllamaEmbeddingService {
  ...
  async checkHealth(): Promise<{
    ...
    try {
      ...
      const modelLoaded = modelsResponse.models?.some((m: any) => 
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        if (embedding.length !== this.config.dimensions) {
          console.warn(`Expected ${this.config.dimensions} dimensions, got ${embedding.length}. Adjusting config.`);
...
export function getOllamaEmbeddingService(config?: Partial<OllamaEmbeddingConfig>): OllamaEmbeddingService {
