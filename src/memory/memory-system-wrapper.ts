...
export class SafeMemorySystemWrapper {
  ...
  async initialize(): Promise<boolean> {
    ...
    while (this.initializationAttempts < (this.config.maxRetries || 3)) {
      try {
        ...
        this.config.logger.info(`Initializing memory system (attempt ${this.initializationAttempts})`);
        ...
        this.config.logger.error(`Memory system initialization failed (attempt ${this.initializationAttempts}):`, error);
  ...
  async storeMemory(
    ...
    try {
      return await this.memorySystem!.storeMemory(serviceId, memoryType, content, metadata, keywords);
    } catch (error) {
