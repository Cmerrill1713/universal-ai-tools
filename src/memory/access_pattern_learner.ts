...
export interface LearningInsights {
  ...
  searchPatterns: {
    ...
    failurePatterns: Array<{ pattern: string; reason: string }>;
...
/**
 * Advanced access pattern learning system
 */
export class AccessPatternLearner {
  ...
  async recordAccess(
    ...
    try {
      ...
      this.logger.error('Failed to record access pattern:', error);
    }
  }
  ...
  async recordUserFeedback(
    ...
    try {
      // Find the most recent access pattern for this memory and agent
      ...
      // Update the access pattern with feedback
      await this.supabase
        .from('memory_access_patterns')
        .update({
      ...
      this.logger.info(`Recorded user feedback for memory ${memoryId}: relevance=${feedback?.relevance}, helpfulness=${feedback?.helpfulness}`);
  ...
  async calculateUtilityScore(
    ...
    try {
      ...
      if (recentAccesses.length > 0) {
        ...
        const daysSinceAccess = (currentTime.getTime() - lastAccess.getTime()) / (24 * 60 * 60 * 1000);
        recencyBoost = Math.max(0, (7 - daysSinceAccess) / 7) * this.adaptiveWeights.recencyWeight;
        explanation.push(`Recency: +${(recencyBoost * 100).toFixed(1)}% (last accessed ${daysSinceAccess.toFixed(1)} days ago)`);
      }
      ...
      if (accessCount > 0) {
        ...
        explanation.push(`Frequency: +${(frequencyBoost * 100).toFixed(1)}% (${accessCount} accesses)`);
      }

      // Calculate user preference boost
      let userPreferenceBoost = 0;
      const positiveInteractions = accessHistory?.filter(a => a.response_useful === true).length || 0;
      if (accessCount > 0) {
        ...
        explanation.push(`User preference: +${(userPreferenceBoost * 100).toFixed(1)}% (${(successRate * 100).toFixed(1)}% success rate)`);
      }
      ...
      if (contextualFactors?.urgency) {
        ...
        const urgencyMultiplier = urgencyMultipliers[contextualFactors.urgency as keyof typeof urgencyMultipliers] || 1.0;
        contextualRelevanceBoost = (urgencyMultiplier - 1) * 0.1;
        explanation.push(`Urgency (${contextualFactors.urgency}): ${contextualRelevanceBoost >= 0 ? '+' : ''}${(contextualRelevanceBoost * 100).toFixed(1)}%`);
      }

      // Time-of-day patterns
      const currentHour = currentTime.getHours();
      const hourlyAccesses = accessHistory?.filter(
        a => new Date(a.created_at).getHours() === currentHour
      ).length || 0;
      if (hourlyAccesses > 0) {
        const timeBoost = Math.min(hourlyAccesses / accessCount, 0.2);
        contextualRelevanceBoost += timeBoost;
        explanation.push(`Time pattern: +${(timeBoost * 100).toFixed(1)}% (active at this hour)`);
      }
  ...
  async reRankResults(
    ...
    try {
      const rankedResults = await Promise.all(
        results.map(async (result, index) => {
          const utilityScore = await this.calculateUtilityScore(
            ...
            { queryEmbedding: contextualFactors?.queryEmbedding, urgency: contextualFactors?.urgency }
          );
      ...
      this.logger.error('Failed to re-rank results:', error);
  ...
  private async generateLearningInsights(agentName: string): Promise<LearningInsights> {
    try {
      ...
      accessPatterns.forEach(pattern => {
        ...
        const memoryType = pattern.ai_memories?.memory_type || 'unknown';
        const satisfaction = pattern.user_feedback ? 
          (pattern.user_feedback.relevance + pattern.user_feedback.helpfulness) / 2 : 3;
        
        const typeData = memoryTypes.get(memoryType) || { count: 0, avgSatisfaction: 0 };
        typeData.count++;
        typeData.avgSatisfaction = (typeData.avgSatisfaction * (typeData.count - 1) + satisfaction) / typeData.count;
        memoryTypes.set(memoryType, typeData);

        // Agent preferences
        const agentId = pattern.ai_memories?.service_id || agentName;
        const agentData = agents.get(agentId) || { count: 0, avgSatisfaction: 0 };
        agentData.count++;
        agentData.avgSatisfaction = (agentData.avgSatisfaction * (agentData.count - 1) + satisfaction) / agentData.count;
        ...
        // Session length
        if (pattern.contextual_factors?.sessionLength) {
          totalSessionLength += pattern.contextual_factors.sessionLength;
          sessionCount++;
        }

        // Follow-up queries
        if (pattern.follow_up_queries) {
          pattern.follow_up_queries.forEach((query: string) => {
      ...
      const avgSatisfaction = accessPatterns
        .filter(p => p.user_feedback)
        .reduce((sum, p) => sum + (p.user_feedback.relevance + p.user_feedback.helpfulness) / 2, 0) / 
        accessPatterns.filter(p => p.user_feedback).length;

      if (avgSatisfaction < 3) {
        recommendations.push('Consider improving memory relevance - user satisfaction is below average');
      }

      const successRate = accessPatterns.filter(p => p.response_useful).length / accessPatterns.length;
      if (successRate < 0.7) {
      ...
      return {
        userPreferences: {
          ...
          preferredAgents: Array.from(agents.entries())
            ...
            .sort((a, b) => b.weight - a.weight)
            .slice(0, 5),
          timeOfDayPatterns: hourlyActivity.map((activity, hour) => ({ hour, activity }))
...
export function getAccessPatternLearner(supabase: SupabaseClient, logger: Logger): AccessPatternLearner {
