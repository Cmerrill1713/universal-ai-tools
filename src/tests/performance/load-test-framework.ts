...
export class LoadTestFramework extends EventEmitter {
  ...
  private async simulateUser(initialDelay: number, duration: number): Promise<void> {
    ...
    while (Date.now() < endTime && this.isRunning) {
      try {
        ...
        // Error already logged in individual request handlers
      }
  ...
  private async executeRandomRequest(): Promise<void> {
    ...
    await this.executeRequest(endpoint.path, endpoint.method, endpoint.payload, endpoint.expectedStatus);
  }
  ...
  private async executeRequest(
    ...
    try {
      ...
      if (!metrics.success) {
        metrics.error = `Unexpected status code: ${response.status}`;
      }
...
export function createApiLoadTest(baseUrl: string): LoadTestConfig {
  return {
    ...
    endpoints: [
      ...
      { path: '/api/memories', method: 'POST', weight: 25, 
        payload: { content: 'Test memory', type: 'user' }, expectedStatus: 201 },
...
export function createDatabaseLoadTest(baseUrl: string): LoadTestConfig {
  return {
    ...
    rampUpTime: 20,
    endpoints: [
      { path: '/api/memories/search', method: 'POST', weight: 40,
        payload: { query: 'test search', limit: 10 }, expectedStatus: 200 },
      { path: '/api/memories', method: 'POST', weight: 30,
        payload: { content: 'Load test memory', type: 'system' }, expectedStatus: 201 },
  ...
  return [
    {
      ...
      steps: [
        ...
        { endpoint: '/api/realtime/connect', method: 'POST', 
          payload: { type: 'test_client' }, expectedStatus: 200, delay: 1000 },
        { endpoint: '/api/realtime/disconnect', method: 'POST', 
          payload: { type: 'test_client' }, expectedStatus: 200, delay: 5000 }
      ]
    }
  ];
}