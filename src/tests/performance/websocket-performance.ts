...
export class WebSocketPerformanceTester extends EventEmitter {
  ...
  public async runWebSocketPerformanceTest(options: {
    ...
    try {
      ...
      logger.error('WebSocket performance test failed:', error);
  ...
  private async executeWebSocketTest(options: any): Promise<void> {
    ...
    logger.info(`Running test with ${this.clients.length} connections for ${options.test_duration} seconds`);
    await new Promise(resolve => setTimeout(resolve, options.test_duration * 1000));
  }
  ...
  private startMessageTesting(client: Socket, connectionId: number, options: any): void {
    ...
    const sendMessage = () => {
      ...
      if (messageType < 0.5) {
        // Request-response pattern with acknowledgment
  ...
  private analyzeWebSocketResults(testDuration: number): WebSocketPerformanceResult {
    ...
    const connection_stats = {
      ...
      average_connection_time: this.calculateAverage(connectionMetrics.filter(m => m.success).map(m => m.connection_time)),
      max_concurrent_connections: Math.max(...this.metrics.map(m => m.concurrent_connections)),
      connection_success_rate: (connectionMetrics.filter(m => m.success).length / connectionMetrics.length) * 100 || 0
    };
    ...
    const message_stats = {
      ...
      message_success_rate: (messageMetrics.filter(m => m.success).length / messageMetrics.length) * 100 || 0
    };
    ...
    const stability_metrics = {
      disconnection_rate: (disconnectionMetrics.length / connection_stats.total_connections) * 100 || 0,
      ...
      connection_stability_score: this.calculateStabilityScore(connection_stats, message_stats, disconnectionMetrics.length)
    };
