...
import { LoadTestFramework, createApiLoadTest, createCacheLoadTest, createDatabaseLoadTest } from './load-test-framework';
import { DatabasePerformanceTester, testBackupPerformance, testMigrationPerformance } from './database-performance';
...
export class PerformanceTestRunner {
  ...
  public async runComprehensivePerformanceTests(options: {
    ...
    try {
      ...
      logger.error('Performance test suite error:', error);
      throw error;
    }
  }
  ...
  private async runApiPerformanceTests(options: any): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.dim(`  • ${results.totalRequests} requests, ${results.requestsPerSecond.toFixed(1)} req/s`));
      console.log(chalk.dim(`  • ${results.averageResponseTime.toFixed(1)}ms avg response time`));
      console.log(chalk.dim(`  • ${results.errorRate.toFixed(1)}% error rate`));

    } catch (error) {
      this.testsFailed++;
      spinner.fail(chalk.red('API performance tests failed'));
      logger.error('API performance test error:', error);
      this.testResults.api_performance = { error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }
  ...
  private async runDatabasePerformanceTests(options: any): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.dim(`  • ${results.totalQueries} queries, ${results.aggregatedMetrics.queriesPerSecond.toFixed(1)} q/s`));
      console.log(chalk.dim(`  • ${results.aggregatedMetrics.averageQueryTime.toFixed(1)}ms avg query time`));
      console.log(chalk.dim(`  • ${results.aggregatedMetrics.successRate.toFixed(1)}% success rate`));

    } catch (error) {
      this.testsFailed++;
      spinner.fail(chalk.red('Database performance tests failed'));
      logger.error('Database performance test error:', error);
      this.testResults.database_performance = { error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }
  ...
  private async runCachePerformanceTests(options: any): Promise<void> {
    ...
    try {
      ...
      const results = await cacheTester.runPerformanceTest({
        ...
        dataSize: options.data_size,
        keyCount: options.data_size === 'large' ? 10000 : options.data_size === 'medium' ? 5000 : 1000
      });
      ...
      console.log(chalk.dim(`  • ${results.aggregatedMetrics.totalOperations} operations, ${results.aggregatedMetrics.operationsPerSecond.toFixed(1)} ops/s`));
      console.log(chalk.dim(`  • ${results.aggregatedMetrics.hitRate.toFixed(1)}% hit rate`));
      console.log(chalk.dim(`  • ${results.aggregatedMetrics.averageResponseTime.toFixed(2)}ms avg response time`));

    } catch (error) {
      this.testsFailed++;
      spinner.fail(chalk.red('Cache performance tests failed'));
      logger.error('Cache performance test error:', error);
      this.testResults.cache_performance = { error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }
  ...
  private async runResourceManagementTests(options: any): Promise<void> {
    ...
    try {
      ...
      const results = await resourceTester.runResourceStressTest({
        ...
        memory_stress_mb: options.data_size === 'large' ? 1024 : options.data_size === 'medium' ? 512 : 256,
      ...
      logger.error('Resource management test error:', error);
      this.testResults.resource_management = { error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }
  ...
  private async runWebSocketPerformanceTests(options: any): Promise<void> {
    ...
    try {
      ...
      const results = await wsTester.runWebSocketPerformanceTest({
        ...
        message_size: options.data_size === 'large' ? 1024 : options.data_size === 'medium' ? 512 : 256,
      ...
      console.log(chalk.dim(`  • ${results.message_stats.average_latency.toFixed(1)}ms avg latency`));

    } catch (error) {
      this.testsFailed++;
      spinner.fail(chalk.red('WebSocket performance tests failed'));
      logger.error('WebSocket performance test error:', error);
      this.testResults.websocket_performance = { error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }
  ...
  private async runAIServicePerformanceTests(options: any): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.dim(`  • ${results.system_performance.throughput_requests_per_second.toFixed(2)} req/s`));
      console.log(chalk.dim(`  • Primary bottleneck: ${results.bottleneck_analysis.primary_bottleneck}`));

    } catch (error) {
      this.testsFailed++;
      spinner.fail(chalk.red('AI service performance tests failed'));
      logger.error('AI service performance test error:', error);
      this.testResults.ai_service_performance = { error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }
  ...
  private calculateOverallScore(): number {
    ...
    if (this.testResults.api_performance && !this.testResults.api_performance.error) {
    ...
    if (this.testResults.database_performance && !this.testResults.database_performance.error) {
    ...
    if (this.testResults.cache_performance && !this.testResults.cache_performance.error) {
    ...
    if (this.testResults.resource_management && !this.testResults.resource_management.error) {
      totalScore += this.testResults.resource_management.stability_score * 0.15;
      componentCount += 0.15;
    }

    // WebSocket Performance Score (5% weight)
    if (this.testResults.websocket_performance && !this.testResults.websocket_performance.error) {
    ...
    if (this.testResults.ai_service_performance && !this.testResults.ai_service_performance.error) {
    ...
    if (this.testResults.api_performance && !this.testResults.api_performance.error) {
      ...
      if (api.errorRate > 5) {
        bottlenecks.push({
          ...
          severity: 'high',
          description: `High error rate: ${api.errorRate.toFixed(1)}%`,
          recommendation: 'Check error logs, increase timeout values, or scale API servers'
        });
      }
  ...
  private async generatePerformanceReport(result: ComprehensivePerformanceResult, format: string): Promise<void> {
  ...
  private generateConsoleReport(result: ComprehensivePerformanceResult): void {
    ...
    summaryTable.push(
      ...
      ['Tests Failed', result.test_summary.tests_failed > 0 ? chalk.red(result.test_summary.tests_failed.toString()) : '0'],
    ...
    if (result.bottlenecks.length > 0) {
      ...
      result.bottlenecks.forEach((bottleneck, index) => {
        const severityColor = bottleneck.severity === 'critical' ? chalk.red :
                             bottleneck.severity === 'high' ? chalk.hex('#FFA500') :
                             bottleneck.severity === 'medium' ? chalk.yellow : chalk.blue;
        
        console.log(`${index + 1}. ${severityColor(bottleneck.severity.toUpperCase())} - ${bottleneck.component}`);
  ...
  .action(async (options) => {
    ...
    try {
      await runner.runComprehensivePerformanceTests({
        ...
        concurrent_users: parseInt(options.concurrent),
