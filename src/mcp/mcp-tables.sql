-- MCP Context Storage Tables
-- These tables support the Universal AI Tools MCP server for context persistence and pattern learning
...
CREATE POLICY "Allow all for service role" ON mcp_context
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow all for service role" ON mcp_code_patterns
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow all for service role" ON mcp_task_progress
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow all for service role" ON mcp_error_analysis
    FOR ALL USING (auth.role() = 'service_role');
