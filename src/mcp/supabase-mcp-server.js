...
exports.SupabaseMCPServer = void 0;
...
class SupabaseMCPServer {
            ...
            try {
                switch (name) {
                    ...
                    case 'get_recent_context':
                        return await this.getRecentContext(args);
                    case 'save_code_pattern':
                        return await this.saveCodePattern(args);
                    case 'get_code_patterns':
                        return await this.getCodePatterns(args);
                    case 'propose_migration':
                        return await this.proposeMigration(args);
                    case 'save_task_progress':
                        return await this.saveTaskProgress(args);
                    case 'get_task_history':
                        return await this.getTaskHistory(args);
                    case 'analyze_errors':
                        return await this.analyzeErrors(args);
                    // Memory Layer Tools
                    case 'store_memory':
                    ...
                    case 'auto_generate_memory':
                        return await this.autoGenerateMemory(args);
    ...
    async proposeMigration(params) {
        ...
        try {
            if (lmStudioBase) {
                const res = await ((globalThis.fetch) || fetch)(`${lmStudioBase}/v1/chat/completions`, {
                    method: 'POST',
            ...
            const res = await ((globalThis.fetch) || fetch)(`${ollamaBase}/api/generate`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    model,
    ...
    async storeMemory(args) {
        try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            return { content: [{ type: 'text', text: `Error storing memory: ${msg}` }], isError: true };
        }
    }
    async retrieveMemories(args) {
        try {
                ...
                content: [
                    {
                        ...
                        text: `🔍 Found ${memories.length} relevant memories:\n\n` +
                            memories.map((m, i) => `${i + 1}. **${m.context_type}** (${m.programming_language})\n` +
                                `   Agent: ${m.agent_name}\n` +
                                `   Content: ${m.content}\n` +
            ...
            const msg = error instanceof Error ? error.message : String(error);
            return { content: [{ type: 'text', text: `Error retrieving memories: ${msg}` }], isError: true };
        }
    }
    async autoGenerateMemory(args) {
        try {
            if (!args.success) {
                return {
                    content: [
            ...
            const msg = error instanceof Error ? error.message : String(error);
            return { content: [{ type: 'text', text: `Error auto-generating memory: ${msg}` }], isError: true };
        }
    }
    async getMemoryStats() {
        try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            return { content: [{ type: 'text', text: `Error getting memory stats: ${msg}` }], isError: true };
        }
    }
