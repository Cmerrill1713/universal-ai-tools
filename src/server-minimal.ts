  ...
  try {
    ...
    const ollamaResponse = await fetch(`${supabaseUrl}/functions/v1/ollama-assistant`, {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        stream: false,
        system: 'You are Sweet Athena, a helpful and caring AI assistant. Respond in a warm, friendly manner.'
      })
    });
    ...
    logger.info(`Chat request processed: ${message.substring(0, 50)}...`);
