...
export class DynamicContextManager {
  ...
  private contextWindows: Map<string, ContextWindow> = new Map([
    ...
    ['large', { modelSize: '14B-34B', minContext: 32768, maxContext: 131072, optimalContext: 65536 }],
    ['xlarge', { modelSize: '70B+', minContext: 65536, maxContext: 262144, optimalContext: 131072 }]
  ]);
  ...
  public async optimizeContext(
    ...
    logger.info(`✅ Context optimized in ${Date.now() - startTime}ms: ${originalTokens} → ${optimizedTokens} tokens`);
  ...
  private selectStrategy(
    ...
    if (taskType === 'code_generation' || taskType === 'analysis') {
      return {
  ...
  private inferModelSize(modelName: string): string {
    ...
    if (name.includes('70b') || name.includes('175b') || name.includes('xlarge')) {
      return 'xlarge';
    } else if (name.includes('13b') || name.includes('14b') || name.includes('34b') || name.includes('large')) {
      return 'large';
    } else if (name.includes('7b') || name.includes('8b') || name.includes('9b') || name.includes('medium')) {
      return 'medium';
    } else if (name.includes('mini') || name.includes('2b') || name.includes('3b') || name.includes('small')) {
      return 'small';
  ...
  public getStats() {
    return {
      ...this.contextStats,
      savingsPercentage: 
        (this.contextStats.totalTokensSaved / this.contextStats.totalTokensProcessed * 100).toFixed(2)
    };
  }
   ...
   */
  public getContextRecommendations(modelName: string, taskType?: string): {
    recommended: number;
    minimum: number;
    maximum: number;
    strategy: string;
