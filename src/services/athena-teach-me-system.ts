...
export interface TeachingSession {
  ...
  teachingType: 'new_capability' | 'tool_usage' | 'domain_knowledge' | 'personal_preference' | 'workflow_pattern';
  subject: string;
...
export interface LearningCapability {
  ...
  name: string;
  description: string;
  category: 'database' | 'api' | 'analysis' | 'automation' | 'communication' | 'organization';
...
export class AthenaTeachMeSystem {
  ...
  async processTeachingInteraction(
    ...
    try {
      ...
      return {
        content: "I'm sorry, I had trouble learning from that. Could you try teaching me again? I really want to understand! 🌸",
        personalityMood: 'shy',
  ...
  private detectTeachingIntent(message: string): any {
    ...
    for (const pattern of this.learningPatterns.capability_indicators) {
    ...
    for (const pattern of this.learningPatterns.correction_indicators) {
      if (pattern.test(message)) {
        return {
          type: 'correction',
          confidence: 0.9,
    ...
    for (const pattern of this.learningPatterns.reinforcement_indicators) {
      if (pattern.test(message)) {
        return {
          type: 'reinforcement',
          confidence: 0.7,
    ...
    if (lowerMessage.includes('example') || lowerMessage.includes('like this') || lowerMessage.includes('for instance')) {
      return {
        type: 'demonstration',
        confidence: 0.8,
  ...
  private async handleDemonstration(
    ...
    return {
      content: `I love learning from examples! Let me see if I understand: ${understanding.summary}. Is that right? I'll practice this pattern so I can help you better! ✨`,
   ...
   */
  private async createTeachingSession(sessionData: Partial<TeachingSession>): Promise<TeachingSession> {
    ...
    try {
      await this.supabase
        .from('athena_conversational_development')
        .insert({
   ...
   */
  private async processLearning(session: TeachingSession): Promise<any> {
    try {
      // Analyze the learning content
      const analysis = this.analyzeLearningContent(session.learnedContent);
      
      // Generate understanding
      const understanding = this.generateUnderstanding(session, analysis);
  ...
  private categorizeTeaching(message: string): TeachingSession['teachingType'] {
    ...
    } else if (lowerMessage.includes('workflow') || lowerMessage.includes('process') || lowerMessage.includes('steps')) {
      return 'workflow_pattern';
    } else if (lowerMessage.includes('tool') || lowerMessage.includes('function') || lowerMessage.includes('feature')) {
      return 'tool_usage';
    } else if (lowerMessage.includes('database') || lowerMessage.includes('data') || lowerMessage.includes('information')) {
  ...
  /**
   * Analyze learning content to determine if it's implementable
   */
  private analyzeLearningContent(content: string): any {
    const lowerContent = content.toLowerCase();
    
    const implementableIndicators = [
      'create', 'build', 'make', 'generate', 'calculate', 'process', 'analyze', 'organize'
    ];
    ...
    return {
      ...
      requiresExternalData: lowerContent.includes('api') || lowerContent.includes('fetch') || lowerContent.includes('external')
    };
  }
   ...
   */
  private generateUnderstanding(session: TeachingSession, analysis: any): string {
   ...
   */
  private async createNewCapability(session: TeachingSession, understanding: string): Promise<LearningCapability> {
    ...
    try {
      await this.supabase
        .from('athena_learned_capabilities')
        .insert({
  ...
  private categorizeContent(content: string): string {
    ...
    if (lowerContent.includes('database') || lowerContent.includes('table') || lowerContent.includes('data')) {
      return 'database';
    } else if (lowerContent.includes('api') || lowerContent.includes('request') || lowerContent.includes('call')) {
      return 'api';
    } else if (lowerContent.includes('analyze') || lowerContent.includes('calculate') || lowerContent.includes('process')) {
      return 'analysis';
    } else if (lowerContent.includes('automate') || lowerContent.includes('schedule') || lowerContent.includes('trigger')) {
      return 'automation';
    }
  ...
  private assessComplexity(content: string): 'simple' | 'moderate' | 'complex' {
    const steps = content.split(/(?:then|next|after|and)/i).length;
    if (steps <= 2) return 'simple';
    if (steps <= 4) return 'moderate';
    return 'complex';
  }

  private mapToCapabilityCategory(teachingType: TeachingSession['teachingType']): LearningCapability['category'] {
...
function ${this.sanitizeIdentifier(session.subject)}(input) {
  ...
  private async findRecentCapabilityForCorrection(userId: string, subject: string): Promise<any> {
    // Would search for recent capabilities that might need correction
    return null;
  }

  private async findRecentSuccessForReinforcement(userId: string): Promise<any> {
    // Would find recent successful capability usage
    return { id: 'recent_success' };
  }

  private async updateCapabilityWithCorrection(capabilityId: string, correction: any): Promise<void> {
    ...
    try {
      await this.supabase
        ...
        .insert({
          ...
          memory_content: `Learned about ${session.subject}: ${understanding}`,
