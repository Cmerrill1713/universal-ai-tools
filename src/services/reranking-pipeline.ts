...
import { type RerankingMetrics, type RerankingOptions, RerankingService, type SearchResult } from './reranking-service';
import { type MemorySearchResult, type MultiStageSearchOptions, MultiStageSearchSystem } from '../memory/multi_stage_search';

export interface PipelineConfig {
  // Reranking configuration
  enableReranking: boolean;
  rerankingMethod: 'cross_encoder' | 'llm_judge' | 'hybrid' | 'feature_based' | 'learned' | 'adaptive';
...
export class RerankingPipeline {
  ...
  private configHistory: { config: PipelineConfig; timestamp: number; performance: AdaptiveMetrics }[] = [];
  ...
  async search(
    ...
    this.logger.debug(`Starting pipeline search with strategy: ${config.searchStrategy}, reranking: ${config.rerankingMethod}`);
    ...
    try {
      ...
      this.logger.info(`Pipeline search completed in ${totalTime}ms: ${finalResults.length} results, quality: ${qualityScore.toFixed(3)}`);
  ...
  async searchKnowledgeEntities(
    ...
    try {
      ...
      const qualityScore = this.calculateQualityScore(results, {
        ...
        method: config.rerankingMethod,
        cacheHit: false,
        averageScoreImprovement: results.reduce((sum: number, r: any) => sum + (r.rerankScore - r.similarity), 0) / results.length,
  ...
  analyzePerformance(): {
    ...
    const trends = {
      ...
      rerankingEffectiveness: this.getTrend(current.rerankingEffectiveness, previous.rerankingEffectiveness),
    ...
    if (trends.latency === 'declining') {
      recommendations.push('Response time is increasing - reduce result limits or disable features');
  ...
  private applyQualityFilters(results: MemorySearchResult[], config: PipelineConfig): MemorySearchResult[] {
  ...
  private calculateQualityScore(results: any[], rerankingMetrics: RerankingMetrics, config: PipelineConfig): number {
  ...
  private calculateAveragePerformance(metrics: AdaptiveMetrics[]): AdaptiveMetrics {
    ...
    return {
      ...
      rerankingEffectiveness: metrics.reduce((sum, m) => sum + m.rerankingEffectiveness, 0) / metrics.length,
  ...
  getStatistics(): {
    ...
    return {
      ...
      averageLatency: analysis.currentPerformance.latency,
