...
import { createKnowledgeFeedbackService, KnowledgeFeedbackService } from './knowledge-feedback-service';
import { createKnowledgeUpdateAutomation, KnowledgeUpdateAutomationService } from './knowledge-update-automation';
...
export class ContinuousLearningService extends EventEmitter {
  ...
  private async checkSystemHealth(): Promise<void> {
    try {
      ...
      const automationStatus = automationStats.recentFailures > automationStats.recentCompletions * 0.5
        ? 'unhealthy' : automationStats.activeJobs > 10 ? 'degraded' : 'healthy';
  ...
  private async identifySourcesForCollection(): Promise<any[]> {
    ...
    return KNOWLEDGE_SOURCES
      .filter(s => s.enabled && s.priority === 'high')
      .slice(0, 3); // Limit to prevent overload
  }

  private async getSourceConfig(sourceId: string): Promise<any> {
    const { KNOWLEDGE_SOURCES } = await import('../config/knowledge-sources');
  ...
  private async getScraperHealth(): Promise<any> {
  ...
  private async getValidationHealth(): Promise<any> {
    ...
    const status = backlog > 100 ? 'degraded' : backlog > 500 ? 'unhealthy' : 'healthy';
  ...
  private async getFeedbackHealth(): Promise<any> {
    const patterns = this.feedbackService.getPatterns();
    const insights = this.feedbackService.getInsights();
  ...
  private calculateOverallHealth(): {
    ...
    const status = unhealthyServices.length === 0 ? 'healthy' :
                   unhealthyServices.length <= 2 ? 'degraded' : 'unhealthy';
   ...
   */
  async getLearningHistory(limit: number = 10): Promise<any[]> {
