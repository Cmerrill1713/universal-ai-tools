...
export class EmbeddedModelManager extends EventEmitter {
  ...
  private async checkMLXAvailability(): Promise<void> {
    try {
      ...
      console.log(`MLX available: ${stdout.trim()}`);
    } catch (error) {
  ...
  async embedModel(modelName: string, config: EmbedConfig = {}): Promise<MLXModel> {
    try {
      ...
      this.emit('model-embedded', { model: modelName, memoryMB: model.memoryFootprint / 1024 / 1024 });
  ...
  private async downloadAndConvertModel(modelName: string, outputPath: string): Promise<void> {
    console.log(`Downloading and converting ${modelName} to MLX format...`);
...
`;
  ...
  async generate(modelName: string, prompt: string, maxTokens = 100): Promise<string> {
    ...
    try {
      ...
      metrics.averageLatency = (metrics.averageLatency * (metrics.inferenceCount - 1) + latency) / metrics.inferenceCount;
      
      this.modelMetrics.set(modelName, metrics);
      return response;
      
    } catch (error) {
      metrics.errorRate = (metrics.errorRate * metrics.inferenceCount + 1) / (metrics.inferenceCount + 1);
      this.modelMetrics.set(modelName, metrics);
      throw error;
    }
  }
   ...
   * Run MLX inference
   */
  private async runMLXInference(modelName: string, prompt: string, maxTokens: number): Promise<string> {
...
`;
   ...
   * Fallback to Ollama
   */
  private async runOllamaFallback(modelName: string, prompt: string, maxTokens: number): Promise<string> {
