  ...
  private async initializeProviders() {
    ...
    for (const [provider] of this.providerClients) {
      this.circuitBreakers.set(provider, new CircuitBreaker(provider, {
        failureThreshold: 5, // Increased threshold to be more forgiving
        successThreshold: 2,
        timeout: 30000, // Increased to 30s for reliable AI model responses
        rollingWindow: 60000
      }));
    }
  }
    ...
    try {
      ...
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Model timeout')), request.maxLatencyMs || 30000) // Increased to 30s for reliable execution
        );
      ...
      for (const fallbackModel of routing.fallbacks) {
        try {
          ...
          const timeoutPromise = new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('Fallback model timeout')), request.maxLatencyMs || 30000) // Increased to 30s for reliable execution
          );
