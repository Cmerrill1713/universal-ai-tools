...
export class MetricsCollectionService extends EventEmitter {
  ...
  private collectSystemMetrics(): void {
    try {
      ...
      // Check for concerning metrics - only warn about actual memory issues (>500MB used)
      const processMemoryPercent = (memUsage.heapUsed / memUsage.heapTotal) * 100;
      const heapUsedMB = memUsage.heapUsed / 1024 / 1024;
      
      // Only warn if we're actually using a lot of memory (>500MB), not percentage
      if (heapUsedMB > 500) {
        log.warn('High Node.js memory usage detected', LogContext.MONITORING, {
          heapPercent: processMemoryPercent.toFixed(2),
          heapUsedMB: heapUsedMB.toFixed(2),
          heapTotalMB: (memUsage.heapTotal / 1024 / 1024).toFixed(2)
        });
      }
      
      // Only warn about system memory if it's extremely high and we're in a container
      if (process.env.DOCKER_ENV && systemMetric.memory.percentage > 95) {
        log.warn('Container memory limit approaching', LogContext.MONITORING, {
