...
export class ResourceManager extends EventEmitter {
      ...
      try {
        ...
        if (process.platform === 'linux') {
          ...
          return new Promise((resolve) => {
            exec(`lsof -p ${pid} | wc -l`, (error, stdout) => {
              if (error) {
                resolve(0);
              } else {
  ...
  private checkResourceLimits() {
    ...
    // Check connections
    const connectionUsage = (this.usage.connections.total / this.config.limits.maxConnections) * 100;
  ...
  public getHealthStatus(): 'healthy' | 'degraded' | 'critical' {
    ...
    const connectionsOk = (this.usage.connections.total / this.config.limits.maxConnections) * 100 < 
