...
export class CircuitBreakerService extends EventEmitter {
   ...
   */
  getBreaker(name: string, options?: Partial<CircuitBreakerOptions>): OpossumCircuitBreaker<any, any> {
  ...
  private setupEventListeners(breaker: OpossumCircuitBreaker<any, any>, name: string): void {
    ...
    breaker.on('failure', (error) => {
      ...
      metrics.failures++;
      metrics.requests++;
      logger.warn(`Circuit breaker ${name}: Failure`, LogContext.SYSTEM, { error: error.message });
    });
  ...
  async httpRequest(
    ...
    const breaker = this.getBreaker(name, {
      fallback: () => {
        logger.warn(`HTTP request fallback for ${name}`, LogContext.API);
...
export function httpWithCircuitBreaker(
  ...
  return circuitBreaker.httpRequest(name, {
    url,
    ...config
  }, options);
}
