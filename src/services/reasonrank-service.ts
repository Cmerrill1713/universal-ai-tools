...
export class ReasonRankService extends EventEmitter {
  ...
  private goldStandardCache = new Map<string, string[]>();
  ...
  private initializeGoldStandards(): void {
    // Initialize with common test queries for calibration
    this.goldStandardCache.set('what is machine learning', [
      'ml_definition', 'ml_intro', 'ml_basics'
    ]);
    this.goldStandardCache.set('how does neural network work', [
      'nn_architecture', 'nn_layers', 'nn_forward_prop'
    ]);
    this.goldStandardCache.set('explain deep learning', [
      'dl_intro', 'dl_vs_ml', 'dl_applications'
    ]);
  }
  ...
  async rankPassages(
    ...
    try {
      ...
      this.updatePerformanceMetrics(totalTime, passages.length, sortedResults);

      // Calculate accuracy if gold standard exists
      const accuracy = this.calculateAccuracyForQuery(query.query, sortedResults);
      if (accuracy !== null) {
        this.updateRankingAccuracy(accuracy);
      }
      ...
      // Emit ranking metrics event
      this.emit('rankingCompleted', {
        query: query.query,
        passageCount: passages.length,
        resultCount: sortedResults.length,
        totalTime,
        accuracy,
        avgConfidence: this.calculateAverageConfidence(sortedResults),
      });
...
Return only the numerical score:`;
  ...
  private updatePerformanceMetrics(
    ...
    // Update confidence scores tracking
    if (results) {
      const confidences = results.map(r => r.confidence).filter(c => c !== undefined);
      this.performanceMetrics.confidenceScores.push(...confidences);
      // Keep only last 1000 confidence scores
      if (this.performanceMetrics.confidenceScores.length > 1000) {
        this.performanceMetrics.confidenceScores = 
          this.performanceMetrics.confidenceScores.slice(-1000);
      }
    }
  }

  /**
   * Calculate accuracy for a specific query
   */
  private calculateAccuracyForQuery(
    query: string, 
    results: RankingResult[]
  ): number | null {
    const normalizedQuery = query.toLowerCase().trim();
    const goldStandard = this.goldStandardCache.get(normalizedQuery);
    
    if (!goldStandard || goldStandard.length === 0) {
      return null;
    }

    const resultIds = results.map(r => r.passageId);
    const k = Math.min(3, results.length); // Precision@K where K=3
    const topKResults = resultIds.slice(0, k);
    
    // Calculate how many of the top K results are in the gold standard
    const correctCount = topKResults.filter(id => 
      goldStandard.some(goldId => id.includes(goldId) || goldId.includes(id))
    ).length;
  ...
  /**
   * Update ranking accuracy metric
   */
  private updateRankingAccuracy(accuracy: number): void {
    const currentAccuracy = this.performanceMetrics.rankingAccuracy;
    const totalQueries = this.performanceMetrics.totalQueriesProcessed;
    
    // Moving average of accuracy
    this.performanceMetrics.rankingAccuracy = 
      (currentAccuracy * (totalQueries - 1) + accuracy) / totalQueries;
    
    // Track successful rankings (accuracy > 0.5)
    if (accuracy > 0.5) {
      this.performanceMetrics.successfulRankings++;
    }

    // Update reasoning quality score based on accuracy trend
    if (totalQueries > 10) {
      this.performanceMetrics.reasoningQualityScore = 
        Math.min(1, this.performanceMetrics.rankingAccuracy * 1.2);
    }
  }
   ...
   */
  private calculateAverageConfidence(results: RankingResult[]): number {
    const confidences = results.map(r => r.confidence).filter(c => c !== undefined);
