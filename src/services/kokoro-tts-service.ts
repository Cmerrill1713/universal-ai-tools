...
export class KokoroTTSService {
  ...
  private async startTTSServer(): Promise<void> {
    ...
    const pythonExe = existsSync(join(aiEnvPath, 'bin', 'python')) ? 
      join(aiEnvPath, 'bin', 'python') : 
      (process.env.VIRTUAL_ENV ? 
        join(process.env.VIRTUAL_ENV, 'bin', 'python') : 
        'python3');

    log.info('🐍 Using Python interpreter', LogContext.AI, { pythonExe });

    // Start Python process with AI environment
    this.pythonProcess = spawn(pythonExe, [scriptPath], {
      stdio: ['pipe', 'pipe', 'pipe'],
      cwd: this.kokoroPath,
      env: {
        ...process.env,
        VIRTUAL_ENV: aiEnvPath,
        PATH: `${join(aiEnvPath, 'bin')}:${process.env.PATH}`,
        PYTHONPATH: `${aiEnvPath}/lib/python3.13/site-packages:${process.cwd()}:${process.env.PYTHONPATH || ''}`,
      },
    });
