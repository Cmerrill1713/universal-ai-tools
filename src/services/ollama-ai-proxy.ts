...
app.post('/api/ai/sql', async (req, res) => {
  try {
    ...
    logger.error('SQL generation failed', LogContext.API, { error: error instanceof Error ? error.message : String(error) });
...
app.post('/api/ai/explain', async (req, res) => {
  try {
    ...
    logger.error('SQL explanation failed', LogContext.API, { error: error instanceof Error ? error.message : String(error) });
    res.status(500).json({ error: error instanceof Error ? error.message : String(error) });
  }
});
