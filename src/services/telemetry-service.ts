...
export class TelemetryService extends EventEmitter {
  ...
  constructor() {
    ...
    this.on('error', (error) => {
      logger.error('Telemetry service error', LogContext.TELEMETRY, { error });
    });
  }
  ...
  async initialize(options: TelemetryServiceOptions = {}): Promise<void> {
    ...
    try {
      ...
      const resource = Resource.default().merge(
        new Resource({
          [SemanticResourceAttributes.SERVICE_NAME]: mergedOptions.serviceName || 'universal-ai-tools',
          [SemanticResourceAttributes.SERVICE_VERSION]: mergedOptions.serviceVersion || process.env.npm_package_version || '1.0.0',
          [SemanticResourceAttributes.DEPLOYMENT_ENVIRONMENT]: mergedOptions.environment || process.env.NODE_ENV || 'development',
      ...
      if (mergedOptions.enablePrometheus) {
        ...
        this.metricReaders.push(new PeriodicExportingMetricReader({
              ...
              requestHook: (span, request) => {
                if ('headers' in request) {
                  span.setAttributes({
                    ...
                    'ai.service': request.headers['x-ai-service'] || 'unknown'
                  });
                }
              },
      ...
      logger.error('Failed to initialize telemetry service', LogContext.SYSTEM, { error });
      throw error;
    }
  }
  ...
  async shutdown(): Promise<void> {
    if (this.sdk) {
      try {
        ...
        logger.error('Error shutting down telemetry service', LogContext.SYSTEM, { error });
      }
    }
  }
  ...
  startAIOperation(operationName: string, attributes?: Record<string, any>): Span {
  ...
  createSpan(spanData: CustomSpanData): Span | null {
    ...
    try {
      ...
      logger.error('Failed to create custom span', LogContext.TELEMETRY, { error, span_name: spanData.name });
      return null;
    }
  }
   ...
   */
  endSpan(span: Span, metadata?: { success?: boolean; error?: Error; attributes?: Record<string, any> }): void {
    if (!span) return;
    ...
    try {
      ...
      logger.error('Failed to end span', LogContext.TELEMETRY, { error });
    }
  }
  ...
  getServiceMetrics(): {
    ...
    const averageResponseTime = totalTraces > 0 
