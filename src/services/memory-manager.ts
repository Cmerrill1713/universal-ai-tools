...
export class MemoryManager extends EventEmitter {
    ...
    if (level === 'critical') {
      ...
      this.memoryPressureCallbacks.forEach(callback => {
        try {
          ...
          logger.error('Error in memory pressure callback', LogContext.PERFORMANCE, { error });
        }
      });
  ...
  public forceGC() {
    if (global.gc) {
      ...
      logger.info(`Forced GC completed in ${duration.toFixed(2)}ms, freed ${(freed / 1024 / 1024).toFixed(2)} MB`, LogContext.PERFORMANCE);
      ...
      logger.warn('Garbage collection not exposed. Run with --expose-gc flag', LogContext.PERFORMANCE);
    }
  }
      ...
      spaces.forEach(space => {
        ...
        if (spaceSamples.length >= 5) {
          ...
          if (growthRate > 0.1) { // 10% growth rate threshold
            ...
            logger.warn(`Potential memory leak detected in ${key}: ${(growthRate * 100).toFixed(1)}% growth rate`, LogContext.PERFORMANCE);
  ...
  public addCacheEntry(cacheName: string, key: string, size: number, priority = 1) {
    ...
    const heapUsedPercent = (process.memoryUsage().heapUsed / v8.getHeapStatistics().heap_size_limit) * 100;
  ...
  private evictCacheEntries(cacheName: string, targetSize: number) {
    ...
    for (const entry of entries) {
      ...
      logger.debug(`Evicted cache entry: ${entry.key} (${entry.size} bytes)`, LogContext.PERFORMANCE);
    }
  ...
  private evictOldCacheEntries() {
    ...
    this.caches.forEach((cache, cacheName) => {
      ...
      if (toEvict.length > 0) {
        logger.info(`Evicted ${toEvict.length} old entries from cache ${cacheName}`, LogContext.PERFORMANCE);
      }
    });
  }
  ...
  public async takeHeapSnapshot(): Promise<string> {
    ...
    try {
      ...
      logger.error('Failed to take heap snapshot', LogContext.PERFORMANCE, { error });
      throw error;
    }
  }
  ...
  public async storeAIMemory(context: string, response: any, metadata: any = {}): Promise<void> {
    try {
      ...
      this.addCacheEntry('ai_memories', 
      ...
      logger.error('Failed to store AI memory', LogContext.PERFORMANCE, { error });
    }
  }
  ...
  public retrieveAIMemory(context: string): any | null {
    try {
      ...
      logger.error('Failed to retrieve AI memory', LogContext.PERFORMANCE, { error });
      return null;
    }
  }
  ...
  public optimizeForAI(): void {
    ...
    this.onMemoryPressure(() => {
      ...
      this.evictCacheEntries('orchestration_results', this.getCacheSize('orchestration_results') * 0.3);
