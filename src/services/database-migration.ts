...
export class DatabaseMigrationService {
  ...
  async runPendingMigrations(): Promise<number> {
    try {
      ...
      if (status.conflicts.length > 0) {
        throw new Error(`Migration conflicts detected: ${status.conflicts.map(m => m.name).join(', ')}`);
      }
  ...
  async validate(): Promise<boolean> {
    try {
      ...
      const ids = [...status.applied, ...status.pending]
        .map(m => m.id)
        .sort();

      for (let i = 1; i < ids.length; i++) {
        const prev = parseInt(ids[i - 1]);
        const curr = parseInt(ids[i]);
