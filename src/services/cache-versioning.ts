...
export class CacheVersioningService {
  ...
  registerMigration(
    ...
    if (!semver.lt(migration.from, migration.to)) {
      throw new Error(
        `Invalid migration: ${migration.from} must be less than ${migration.to}`
      );
    }
  ...
  async set<T>(
    ...
    try {
      ...
      await this.redis.hincrby(
        `${this.SCHEMA_KEY_PREFIX}${schemaName}:usage`,
        version,
        1
      );
    } catch (error) {
  ...
  async migrate<TFrom = any, TTo = any>(
    ...
    try {
      ...
      if (!path.length) {
        logger.warn(
          `No migration path from ${fromVersion} to ${toVersion} for ${schemaName}`
        );
        return null;
      }
      ...
      for (const migration of path) {
        logger.info(
          `Applying migration ${migration.from} -> ${migration.to} for ${schemaName}`
        );
  ...
  async rollback<T>(
    ...
    try {
      ...
      if (semver.gte(versioned.version, toVersion)) {
        logger.warn(
          `Cannot rollback from ${versioned.version} to ${toVersion}`
        );
        return null;
      }
      ...
      // Find rollback path
      const rollbackPath = this.findRollbackPath(
        migrations,
        versioned.version,
        toVersion
      );
      ...
      if (!rollbackPath.length) {
        logger.warn(
          `No rollback path from ${versioned.version} to ${toVersion}`
        );
        return null;
      }
      ...
      for (const migration of rollbackPath) {
        if (!migration.rollback) {
          throw new Error(
            `No rollback function for ${migration.from} -> ${migration.to}`
          );
        }
  ...
  async getVersionUsage(schemaName: string): Promise<Record<string, number>> {
    const usage = await this.redis.hgetall(
      `${this.SCHEMA_KEY_PREFIX}${schemaName}:usage`
    );
    
    const result: Record<string, number> = {};
    for (const [version, count] of Object.entries(usage)) {
      result[version] = parseInt(count);
    }
  ...
  async cleanupOldVersions(
    ...
    try {
      ...
      const pattern = `${this.VERSION_KEY_PREFIX}*`;
      ...
      for (const key of keys) {
        ...
        if (
          versioned.schema === schemaName &&
          !keepVersions.includes(versioned.version)
        ) {
      ...
      logger.info(
        `Cleaned up ${cleaned} old cache entries for schema ${schemaName}`
      );
