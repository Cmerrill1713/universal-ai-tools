...
interface LearningInsight {
  type: 'usage_pattern' | 'performance_trend' | 'relationship_discovery' | 'quality_issue';
  title: string;
  description: string;
...
export class KnowledgeFeedbackService extends EventEmitter {
  ...
  private groupAnalyticsByKnowledge(
    analytics: any[]
  ): Map<string, any[]> {
  ...
  private async calculateKnowledgeMetrics(
    key: string,
    analytics: any[]
  ): Promise<void> {
    ...
    for (let i = 0; i < analytics.length; i++) {
      for (let j = i + 1; j < analytics.length; j++) {
        const timeDiff = Math.abs(
          new Date(analytics[i].created_at).getTime() -
          new Date(analytics[j].created_at).getTime()
        );
  ...
  private async generatePatternInsights(): Promise<void> {
    ...
    const failurePatterns = Array.from(this.patterns.entries())
      ...
      .filter(([_, pattern]) => pattern.confidence > 0.5);
  ...
  private async checkImmediateInsights(analytics: UsageAnalytics): Promise<void> {
    ...
    if (analytics.actionType === 'failed' && analytics.performanceScore === 0) {
      const key = `${analytics.knowledgeId}:${analytics.knowledgeType}`;
      const recentFailures = this.usageCache.get(key)?.filter(
        a => a.actionType === 'failed' && 
  ...
  private async calculateNewRankings(
    knowledgeItems: any[]
    ...
    for (const [knowledgeId, stats] of knowledgeStats) {
      ...
      const avgPerformance = stats.performanceCount > 0 
        ? stats.performanceSum / stats.performanceCount 
        : 0.5;
  ...
  private async applyRankingUpdate(
    ...
    await supabase
      .from('knowledge_reranking_history')
      .insert({
