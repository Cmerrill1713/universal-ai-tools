...
AGENT: ${agentName}
PERFORMANCE DATA: - Success, Rate: ${performanceData.successRate * 100}%
...
USER FEEDBACK: ${performanceData.userFeedback.map((f) => `Rating: ${f.rating}/5 - ${f.comment}`).join('n')}'
...
CURRENT, SYSTEM: - 4-tier LLM architecture (LFM2-1.2B, Gemma/Phi, Qwen/DeepSeek, Devstral)
- Agent-to-agent communication mesh
- DSPy optimization pipeline
- Voice integration with Kokoro-82M
- Multi-modal capabilities

PERFORMANCE METRICS: ${JSON.stringify(await this.getSystemMetrics())}

EVOLUTION GOALS: - Improve overall response time
...
Extract patterns and improvement opportunities: {
  ...
  private parseArchitecturalEvolution(response: string): unknown {
    try {
  ...
  public getEvolutionStatus(): {
    ...
    const lastEvolution =;
      this.evolutionHistory.length > 0
        ? this.evolutionHistory[this.evolutionHistory.length - 1]?.timestamp || null: null;
