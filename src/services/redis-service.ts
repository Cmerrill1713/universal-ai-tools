...
export class RedisService {
  ...
  private setupEventListeners(client?: Redis): void {
    ...
    targetClient.on('error', (error) => {
      ...
      logger.error('❌ Redis error', LogContext.CACHE, { 
        error: error.message,
        stack: error.stack 
      });
    });
  ...
  private setupClusterEventListeners(): void {
    ...
    this.clusterClient.on('error', (error) => {
      this.isConnected = false;
      logger.error('❌ Redis cluster error', LogContext.CACHE, { 
        error: error.message 
      });
    });
    ...
    this.clusterClient.on('node error', (error, address) => {
      logger.error('❌ Redis cluster node error', LogContext.CACHE, { 
        error: error.message,
        address 
      });
    });
  ...
  async lpush(key: string, ...values: string[]): Promise<number> {
    return circuitBreaker.redisOperation(
      ...
      {
        fallback: () => {
          logger.warn('⚠️ List operations not supported in fallback cache', LogContext.CACHE, { key });
          return 0;
        }
      }
    );
