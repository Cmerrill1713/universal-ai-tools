...
export class VoiceProfileService {
  ...
  private personalities: Record<string, PersonalityProfile> = {
    ...
    confident: {
      ...
      base_profile: {
        ...
        style: 0.7,
        use_speaker_boost: true,
        description: 'A clear, confident voice that commands attention while remaining approachable'
      },
      sweetness_modifiers: {
  ...
  getVoiceProfile(personality: string, sweetnessLevel = 0.7): VoiceProfile {
    ...
    const adjustedProfile: VoiceProfile = {
      ...
      speaking_rate: (baseProfile.speaking_rate || 1.0) + (modifiers.speaking_rate_adjustment * sweetnessLevel),
      stability: baseProfile.stability || 0.75,
      similarity_boost: baseProfile.similarity_boost || 0.8,
      style: baseProfile.style || 0.6,
      use_speaker_boost: baseProfile.use_speaker_boost !== undefined ? baseProfile.use_speaker_boost : true,
    ...
    logger.info(`Generated voice profile for ${personality} with sweetness ${sweetnessLevel}`, LogContext.AVATAR, {
  ...
  async updateVoiceConfiguration(
    ...
    if (settings && this.personalities[personality]) {
      ...
      if (settings.pitch_adjustment !== undefined) {
        profile.base_profile.pitch = (profile.base_profile.pitch || 1.0) + settings.pitch_adjustment;
      }
      ...
      if (settings.volume_gain_db !== undefined) {
        ...
        profile.base_profile.style = Math.max(0, Math.min(1, 
          (profile.base_profile.style || 0.5) + (settings.volume_gain_db / 40)
        ));
      }
    }
  ...
  validateVoiceConfig(config: any): {
    ...
    if (config.sweetness_level !== undefined) {
      if (typeof config.sweetness_level !== 'number' || 
    ...
    if (config.voice_settings) {
      ...
      if (settings.similarity_boost !== undefined && 
          (typeof settings.similarity_boost !== 'number' || settings.similarity_boost < 0 || settings.similarity_boost > 1)) {
