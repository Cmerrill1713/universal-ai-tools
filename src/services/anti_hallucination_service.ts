...
export class AntiHallucinationService {
   ...
   */
  private async checkCitations(facts: Fact[], memories: MemoryModel[]): Promise<VerificationResult> {
    if (facts.length === 0) {
      return {
    ...
    const citedFacts = facts.filter(fact => {
      ...
      return memories.some(memory => {
        ...
        // General matching
        return contentLower.includes(claimLower) || 
               this.semanticSimilarity(fact.claim, memory.content) > 0.7;
      });
    });

    const citations = memories
      .filter(m => facts.some(f => m.content.toLowerCase().includes(f.claim.toLowerCase())))
      .map(m => `Memory ${m.id}: ${m.content.substring(0, 100)}...`);
  ...
  private async validateConfidence(response: string): Promise<VerificationResult> {
    const uncertaintyMarkers = [
      'might', 'maybe', 'possibly', 'could be', 'I think', 'probably',
      'it seems', 'appears to be', 'likely', 'uncertain'
    ];
    
    const certaintyMarkers = [
      'definitely', 'certainly', 'absolutely', 'clearly', 'obviously',
      'without doubt', 'proven', 'confirmed', 'verified'
    ];
  ...
  private calculateTruthScore(
    ...
    const groundedFacts = facts.filter(f => {
      return memories.some(m => {
        ...
        // General matching
        return contentLower.includes(claimLower) || 
               this.semanticSimilarity(f.claim, m.content) > 0.7;
      });
    }).length;
    ...
    return {
      score: avgConfidence,
      confidence: verifications.length > 0 ? Math.min(...verifications.map(v => v.confidence)) : 0,
      verifications,
      groundedFacts,
      totalFacts: facts.length,
  ...
  private async generateWithCitations(
    ...
    const citations = memories.map((m, i) => `[${i + 1}] Memory ${m.id}: ${m.content.substring(0, 50)}...`);
  ...
  private isClaim(sentence: string): boolean {
    ...
    const uncertaintyStarters = ['i think', 'i believe', 'maybe', 'perhaps', 'possibly', 'might be'];
    if (uncertaintyStarters.some(starter => lowerSentence.startsWith(starter))) {
      return false;
    }
    
    // Skip questions and suggestions
    if (sentence.includes('?') || lowerSentence.includes('should we') || lowerSentence.includes('we should')) {
      return false;
    }
  ...
  private assessClaimConfidence(claim: string): number {
    ...
    const hasFactualIndicators = /\b(is|are|was|were|capital|population|located|founded|built)\b/i.test(claim);
