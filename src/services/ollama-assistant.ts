...
type ToolDescriptionKey = 'trading_data_provider' | 'database_connector' | 'memory_store' | 'context_store' | 'web_scraper' | 'api_integrator' | 'file_processor' | 'notification_system' | 'ai_model_connector' | 'workflow_orchestrator' | 'security_scanner' | 'performance_monitor' | 'backup_manager' | 'deployment_manager';
...
export class OllamaAssistant {
  ...
  /**
   * Analyze a request and suggest appropriate tools
   */
  async suggestTools(userRequest: string, availableTools: Tool[]): Promise<any> {
    try {
      ...
      const prompt = `You are an expert AI assistant specializing in tool selection and system integration. Analyze the user's request and provide intelligent tool recommendations.
...
RELEVANT CONTEXT:
${context.memories ? `Previous Experience: ${context.memories.slice(0, 3).map((m: Memory) => m.content).join('; ')}` : 'No previous experience found'}
${context.knowledge ? `Knowledge Base: ${context.knowledge.slice(0, 2).map((k: Knowledge) => `${k.title}: ${k.content.substring(0, 100)}`).join('; ')}` : 'No relevant knowledge found'}

INTELLIGENT ANALYSIS:
Based on the request analysis, determine:
...
Respond with a JSON object containing:
{
  ...
  /**
   * Analyze request intent and characteristics
   */
  private async analyzeRequestIntent(request: string): Promise<any> {
    try {
      const model = await this.ensureModel();
...
Respond with JSON: {"intent": "...", "domain": "...", "complexity": "...", "actionType": "..."}`;
  ...
  private async buildToolsList(availableTools: Tool[]): Promise<string> {
    ...
    const toolDescriptions: Record<ToolDescriptionKey, string> = {
      ...
      'web_scraper': 'Web content extraction and monitoring',
    ...
    return `${availableTools.map(tool => {
      const enhanced = toolDescriptions[tool.tool_name as ToolDescriptionKey] || tool.description;
   ...
   */
  private async fallbackToolSuggestion(request: string, availableTools: any[]): Promise<any> {
    const requestLower = request.toLowerCase();
    
    // Basic keyword matching
    const suggestions = [];
    
    if (requestLower.includes('trading') || requestLower.includes('bot') || requestLower.includes('market')) {
    ...
    if (requestLower.includes('web') || requestLower.includes('scraping') || requestLower.includes('api')) {
      suggestions.push('web_scraper', 'api_integrator');
    }
    
    if (requestLower.includes('ai') || requestLower.includes('model') || requestLower.includes('llm')) {
    ...
    return {
      ...
      reasoning: "Basic analysis based on keywords in your request. For more detailed suggestions, please ensure Ollama is running.",
      setup_steps: [
...
Provide clean, production-ready code with comments.`;
   ...
   */
  async analyzeIntegrationPoints(codeStructure: any): Promise<any> {
