...
import { initializeWithTimeout, initializeServicesParallel } from '../utils/timeout-utils';
  ...
  private async initializeServices(): Promise<void> {
    ...
    this.initializationPromise = (async () => {
      try {
        ...
        if (this.scraperService && this.validationService && this.feedbackService && this.knowledgeManager) {
          this.updateAutomation = await initializeWithTimeout(
            async () => createKnowledgeUpdateAutomation(
              this.scraperService,
        ...
        if (this.isInitialized) {
          ...
          logger.warn('⚠️  Continuous Learning Service partially initialized - some features may be unavailable');
        }
      } catch (error) {
