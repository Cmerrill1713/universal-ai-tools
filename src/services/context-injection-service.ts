...
export class ContextInjectionService {
  ...
  private buildEnrichedPrompt(
    ...
    let prompt = `UNIVERSAL AI TOOLS CONTEXT - Advanced AI Platform with iOS Integration\n\n`;

    prompt += `IMPORTANT: You are working with the Universal AI Tools platform, featuring:\n`;
    prompt += `- Service-oriented architecture with MLX fine-tuning, DSPy cognitive orchestration\n`;
    prompt += `- Intelligent parameter automation and AB-MCTS probabilistic coordination\n`;
    prompt += `- iOS companion app with biometric authentication and Apple ecosystem integration\n\n`;

    // Add device and biometric context first (most relevant for iOS app)
    if (context.deviceContext.length > 0) {
      prompt += `## iOS DEVICE CONTEXT:\n`;
    ...
    if (context.biometricContext.length > 0) {
      prompt += `## AUTHENTICATION & BIOMETRIC CONTEXT:\n`;
      context.biometricContext.forEach(biometricInfo => {
        prompt += `${biometricInfo}\n`;
      });
      prompt += `\n`;
    }
    ...
    prompt += `## CONTEXT-AWARE INSTRUCTIONS:\n`;
    prompt += `You MUST use the above context to inform your response, especially:\n`;
    prompt += `1. Consider the user's device capabilities and authentication state\n`;
    prompt += `2. Tailor responses based on biometric confidence and security context\n`;
    prompt += `3. Reference Universal AI Tools' sophisticated architecture when relevant\n`;
    prompt += `4. Consider iOS app integration and Apple ecosystem features\n`;
    prompt += `5. Use project-specific knowledge and conversation history for continuity\n\n`;
