...
import { KnowledgeSource, CONTENT_VALIDATION_RULES } from '../config/knowledge-sources';
...
export class KnowledgeValidationService {
  ...
  constructor() {
    ...
    const basicWordList = ['the', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'from', 'up', 'about', 'into', 'through', 'during', 'before', 'after', 'above', 'below', 'between', 'among', 'until', 'without', 'within'];
  ...
  async validateScrapedKnowledge(
    ...
    try {
      ...
      logger.error(`Failed to validate knowledge ${scrapedId}:`, error);
    }
  ...
  private async validateSourceCredibility(source: KnowledgeSource): Promise<ValidationResult> {
    ...
    if (updateFrequencyHours > 168) { // More than a week
  ...
  private async validateContentQuality(
    ...
    if (tokenCount > 8000) {
      ...
      suggestions.push('Summarize or split content for better processing');
      score *= 0.7;
    }
    ...
    try {
      ...
      if (factCheckResult.success) {
        ...
        if (validationData.incorrect_claims) {
          issues.push(...validationData.incorrect_claims.map((claim: any) => 
            `Incorrect claim: "${claim.claim}" - ${claim.issue}`
          ));
        }

        if (validationData.unsupported_claims) {
          issues.push(...validationData.unsupported_claims.map((claim: any) =>
            `Unsupported claim: "${claim.claim}"`
          ));
          suggestions.push('Provide references or sources for unsupported claims');
        }

        if (validationData.outdated_claims) {
          issues.push(...validationData.outdated_claims.map((claim: any) =>
            `Outdated information: "${claim.claim}"`
          ));
  ...
  private async detectDeprecation(
    ...
    const deprecationKeywords = [
      'deprecated', 'obsolete', 'outdated', 'no longer supported',
      'will be removed', 'legacy', 'old version', 'end of life',
      'discontinued', 'replaced by', 'use instead'
    ];

    const lowerContent = content.toLowerCase();
    const foundKeywords = deprecationKeywords.filter(keyword => 
    ...
    const yearMatches = content.match(datePattern);
    ...
    return {
      ...
      metadata: {
        ...
        deprecationDatabase: deprecationDatabase
      }
    };
  }
  ...
  private async validateTechnicalAccuracy(
    ...
    for (const codeBlock of codeBlocks) {
      ...
      if (!syntaxValidation.isValid) {
        issues.push(`Syntax error in ${codeBlock.language} code: ${syntaxValidation.error}`);
    ...
    if (metadata.category === 'api' || content.includes('endpoint')) {
  ...
  private async checkDomainReputation(url: string): Promise<number> {
    try {
      ...
      const trustedDomains = [
        'supabase.com', 'github.com', 'stackoverflow.com', 'openai.com',
        'arxiv.org', 'acm.org', 'ieee.org', 'microsoft.com', 'google.com',
        'apollographql.com', 'langchain.com', 'huggingface.co'
      ];
  ...
  private async checkGrammarAndSpelling(content: string): Promise<string[]> {
    ...
    for (const word of words) {
      if (word.length > 2 && !this.isValidWord(word)) {
        issues.push(`Possible spelling error: ${word}`);
      }
    }
  ...
  private validateContentStructure(content: string, metadata: Record<string, any>): number {
    let score = 1.0;
  ...
  private async validateCodeSyntax(
  ...
  private checkCodeSecurity(codeBlock: { code: string; language: string }): string[] {
    ...
    const securityPatterns = [
      ...
      { pattern: /dangerouslySetInnerHTML/, message: 'Potential XSS vulnerability' },
      { pattern: /process\.env\.\w+\s*[^|]/, message: 'Hardcoded environment variables' },
  ...
  private async checkCodeBestPractices(
    codeBlock: { code: string; language: string }
  ): Promise<string[]> {
    const issues: string[] = [];
  ...
  private async validateAPIUsage(content: string): Promise<{
    ...
    const mentionedMethods = httpMethods.filter(method => 
      content.includes(method)
    );
    
    if (mentionedMethods.length === 0 && content.includes('endpoint')) {
  ...
  private async updateScrapedKnowledgeStatus(
    ...
    const status = overallScore >= 0.7 ? 'validated' : 
                   overallScore >= 0.5 ? 'needs_review' : 'rejected';
    ...
    if (error) {
      logger.error('Failed to update scraped knowledge status:', error);
    }
  }
