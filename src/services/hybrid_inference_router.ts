...
export class HybridInferenceRouter extends EventEmitter {
  ...
  constructor(
    embeddedManager?: EmbeddedModelManager,
  ...
  private async analyzeRequest(request: InferenceRequest): Promise<RoutingDecision> {
    ...
    const needsSpeed = request.priority === 'critical' || (request.timeout !== undefined && request.timeout < 5000);
    const needsStreaming = request.streaming || false;
    const isMultimodal = this.detectMultimodal(request.prompt);
    const modelSize = this.estimateRequiredModelSize(complexity, request.prompt);
   ...
   * MLX inference
   */
  private async mlxInference(request: InferenceRequest, routing: RoutingDecision): Promise<InferenceResponse> {
    const startTime = Date.now();
   ...
   * Ollama inference
   */
  private async ollamaInference(request: InferenceRequest, routing: RoutingDecision): Promise<InferenceResponse> {
    const startTime = Date.now();
    
    // Use lifecycle manager to ensure model is ready
    await this.lifecycleManager.predictAndWarm({ userRequest: request.prompt });
   ...
   */
  private async hybridInference(request: InferenceRequest, routing: RoutingDecision): Promise<InferenceResponse> {
    const startTime = Date.now();
    ...
    const plan = await this.embeddedManager.generate(
      planningModel,
      `Plan approach for: ${request.prompt}`,
      50
    );
   ...
   */
  private async selectOptimalEngine(request: InferenceRequest, routing: RoutingDecision): Promise<InferenceResponse> {
  ...
  /**
   * Detect if request needs multimodal capabilities
   */
  ...
  private buildOllamaCommand(model: string, request: InferenceRequest): string {
    ...
    return `echo "${request.prompt.replace(/"/g, '\\"')}" | ${args}`;
  }
  ...
  private generateCacheKey(request: InferenceRequest): string {
    const key = `${request.prompt.substring(0, 50)}_${request.maxTokens}_${request.streaming}`;
   ...
   */
  private updatePerformanceStats(engine: 'mlx' | 'ollama' | 'hybrid', latencyMs: number, success: boolean): void {
    ...
    stats.totalRequests++;
    stats.averageLatency = (stats.averageLatency * (stats.totalRequests - 1) + latencyMs) / stats.totalRequests;
