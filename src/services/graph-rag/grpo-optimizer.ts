...
export class GRPOOptimizer extends EventEmitter {
  ...
  private computePolicyGradients(
    ...
    for (let i = 0; i < transitions.length; i++) {
      ...
      if (actionGrad) {
        for (let j = 0; j < stateFeatures.length; j++) {
          actionGrad[j] = (actionGrad[j] || 0) + advantage * (stateFeatures[j] || 0) * transition.action.confidence;
        }
      }
      ...
      for (const [action, prob] of actionProbs) {
        ...
        if (grad) {
          ...
          for (let j = 0; j < stateFeatures.length; j++) {
            grad[j] = (grad[j] || 0) + this.entropyCoefficient * entropy * (stateFeatures[j] || 0);
          }
        }
      }
    }
    ...
    for (const [action, grad] of gradients) {
      for (let i = 0; i < grad.length; i++) {
        grad[i] = (grad[i] || 0) / transitions.length;
      }
    }
    ...
    for (const [action, gradient] of gradients) {
      ...
      if (weights) {
        ...
        for (let i = 0; i < weights.length; i++) {
          weights[i] = (weights[i] || 0) + this.learningRate * (gradient[i] || 0) * scale;
        }
  ...
  private updateValueFunction(
    ...
    for (let i = 0; i < transitions.length; i++) {
      ...
      const target = returns[i]; if (target === undefined) continue;
  ...
  private applyAccumulatedGradients(): void {
    for (const [action, gradients] of this.gradientAccumulator) {
      ...
      if (weights) {
        ...
        let gradNorm = 0;
        for (let i = 0; i < gradients.length; i++) { if (gradients[i] === undefined) continue
          gradNorm += (gradients[i] || 0) * (gradients[i] || 0);
        }
        ...
        for (let i = 0; i < weights.length; i++) {
          weights[i] = (weights[i] || 0) + this.learningRate * (gradients[i] || 0) * scale;
        }
      }
    }
  }
  ...
  private pruneValueCache(): void {
    ...
    for (let i = 0; i < toRemove; i++) {
      ...
      const entry = entries[randomIndex]; if (entry) this.valueWeights.delete(entry[0]);
    }
  }
