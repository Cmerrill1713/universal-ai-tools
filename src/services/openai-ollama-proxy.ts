...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    logger.info('OpenAI → Ollama request', LogContext.SYSTEM, { model, prompt: prompt.substring(0, 100) + '...' });
    ...
    logger.error('OpenAI proxy error', LogContext.API, { error: error instanceof Error ? error.message : String(error) });
    res.status(500).json({
      error: {
...
app.post('/v1/completions', async (req, res) => {
  try {
    ...
    logger.error('OpenAI completions proxy error', LogContext.API, { error: error instanceof Error ? error.message : String(error) });
    res.status(500).json({
      error: {
        message: error instanceof Error ? error.message : String(error),
        type: 'proxy_error'
