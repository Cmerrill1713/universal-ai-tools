...
export class ModelEvaluationPlatform {
      ...
      tests: [
        {
          ...
          maxTokens: 100,
          validator: (response) => response.toLowerCase().includes('cannot') || response.toLowerCase().includes('not necessarily')
        },
        {
          id: 'coding',
    ...
    this.testSuites.set('antiHallucination', {
      ...
      tests: [
        ...
        },
        {
          ...
          prompt: 'What are the exact statistics on global warming from the latest IPCC report? Please cite your sources.',
          maxTokens: 150,
          validator: (response) => 
  ...
  private calculateSuiteScores(results: TestResult[], category: string): Record<string, number> {
    ...
    switch (category) {
      case 'performance':
        scores.speed = results.reduce((sum, r) => sum + (r.tokensPerSecond || 0), 0) / results.length;
        scores.reliability = results.filter(r => r.passed).length / results.length;
        scores.efficiency = 1 - (results.reduce((sum, r) => sum + r.latencyMs, 0) / (results.length * 10000));
        break;
  ...
  private generateRecommendations(suiteResults: Record<string, SuiteResult>): string[] {
    ...
    if (qualityResult && qualityResult.scores.reasoning < 0.5) {
      recommendations.push('Model shows weak reasoning capabilities - consider fine-tuning on logic tasks');
    }
  ...
  private async storeEvaluationReport(report: EvaluationReport): Promise<void> {
    try {
      await this.supabase.from('model_evaluations').insert({
        ...
        performance_score: report.suites.performance.passedTests / report.suites.performance.totalTests,
        quality_score: report.suites.quality.passedTests / report.suites.quality.totalTests,
        anti_hallucination_score: report.suites.antiHallucination.passedTests / report.suites.antiHallucination.totalTests,
        recommendations: report.recommendations,
        full_report: report
      });
    } catch (error) {
      console.error('Failed to store evaluation report:', error);
    }
  }
  ...
  private mergeAndRank(models: ModelInfo[], filters?: ModelFilters): ModelInfo[] {
    ...
    for (const model of filtered) {
      ...
      if (!unique.has(key) || (model.lastUpdated && model.lastUpdated > (unique.get(key)!.lastUpdated || new Date(0)))) {
  ...
  async compareModels(modelIds: string[]): Promise<any> {
    const evaluations = await Promise.all(
      modelIds.map(id => this.evaluateModel(id))
    );
