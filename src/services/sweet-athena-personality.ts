...
export interface SweetMemory {
  id: string;
  memoryType: 'personal_preference' | 'sweet_moment' | 'accomplishment' | 'learning_together' | 'gentle_correction' | 'encouragement_given';
  content: string;
...
export class SweetAthenaPersonality {
  ...
  private async createSweetResponse(
    ...
    const responseTemplates = this.sweetResponses[responseCategory]?.[personalityMode] || 
                             this.sweetResponses.helping.sweet;
  ...
  private async adjustPersonalityForContext(
    ...
    if (context.relationshipDepth === 'new') {
      this.personalityState.confidenceLevel = Math.max(4, this.personalityState.confidenceLevel - 1);
      this.personalityState.currentMood = 'shy';
    } else if (context.relationshipDepth === 'trusted') {
      this.personalityState.confidenceLevel = Math.min(9, this.personalityState.confidenceLevel + 1);
    ...
    if (userEmotion === 'frustrated' || userEmotion === 'sad') {
      ...
      this.personalityState.interactionComfort = Math.min(10, this.personalityState.interactionComfort + 1);
  ...
  private detectUserEmotion(message: string): string {
    ...
    if (lowerMessage.includes('help') || lowerMessage.includes('please') || lowerMessage.includes('stuck')) {
      return 'need_help';
    } else if (lowerMessage.includes('thank') || lowerMessage.includes('great') || lowerMessage.includes('perfect')) {
      return 'happy';
    } else if (lowerMessage.includes('frustrated') || lowerMessage.includes('problem') || lowerMessage.includes('wrong')) {
      return 'frustrated';
  ...
  private selectPersonalityMode(context: ConversationContext, message: string): string {
    ...
    // If it's a complex technical request, be purposeful
  ...
  private categorizeResponse(message: string, intent?: any): string {
    ...
    if (lowerMessage.includes('hello') || lowerMessage.includes('hi') || lowerMessage.includes('hey')) {
      ...
      return 'apologizing';
    } else if (lowerMessage.includes('can you') || lowerMessage.includes('help') || lowerMessage.includes('create')) {
      return 'helping';
   ...
   */
  private async personalizeResponse(baseResponse: string, context: ConversationContext): Promise<string> {
    ...
    if (context.personalMemories.length > 0) {
      ...
      if (recentMemory.emotionalContext === 'joyful') {
        baseResponse = baseResponse.replace('!', '! I remember how happy you were last time we worked together.');
      }
    }
  ...
  private determineMemoryType(message: string, response: AthenaResponse): SweetMemory['memoryType'] {
  ...
  private createFallbackResponse(): AthenaResponse {
    return {
      content: "I'm sorry, I'm having a little trouble right now, but I'm still here to help you. Could you try asking me again? 🌸",
      personalityMood: 'shy',
