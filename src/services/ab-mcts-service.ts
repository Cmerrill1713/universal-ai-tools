...
export class ABMCTSService {
  ...
  private readonly maxExecutionHistorySize = 1000; // Prevent unbounded growth
  ...
  /**
   * Main search entry point - supports both 2 and 3 parameter signatures for backwards compatibility
   */
  async search(
    initialContext: AgentContext,
    availableAgentsOrOptions?: string[] | Partial<ABMCTSExecutionOptions>,
    options?: Partial<ABMCTSExecutionOptions>
  ): Promise<ABMCTSSearchResult> {
    // Handle backward compatibility: if second parameter is options object, use default agents
    let availableAgents: string[];
    let fullOptions: ABMCTSExecutionOptions;

    if (Array.isArray(availableAgentsOrOptions)) {
      // 3-parameter signature: (context, agents, options)
      availableAgents = availableAgentsOrOptions;
      fullOptions = {
        useCache: true,
      ...
      // 2-parameter signature: (context, options) - use default agents
      availableAgents = ['planner', 'retriever', 'synthesizer', 'orchestrator', 'reasoning', 'cognitive'];
      fullOptions = {
        useCache: true,
        enableParallelism: true,
        collectFeedback: true,
    ...
    if (!this.root || !this.isSameContext(this.root.state, initialContext)) {
      ...
      if (fullOptions.useCache && treeStorage.isAvailable() && initialContext.requestId) {
        try {
          ...
          if (savedResult?.bestPath && savedResult.bestPath.length > 0) {
  ...
  private async simulateAgentExecution(
    ...
    // Simulate response with timeout tracking
   ...
   */
  reset(): void {
    log.info('🔄 Resetting AB-MCTS Service', LogContext.AI);
    ...
    // Reset Thompson selector
    this.thompsonSelector = new ThompsonSelector();
    this.adaptiveExplorer = new AdaptiveExplorer();
    
    log.info('✅ AB-MCTS Service reset completed', LogContext.AI);
  }
  ...
  async processFeedback(feedback: ABMCTSFeedback): Promise<void> {
    ...
    // Store feedback with bounds checking
    if (!this.executionHistory.has(feedback.nodeId)) {
      this.executionHistory.set(feedback.nodeId, []);
    }
    
    const feedbackList = this.executionHistory.get(feedback.nodeId)!;
    feedbackList.push(feedback);
    
    // Prevent unbounded growth of execution history
    if (feedbackList.length > 50) {
      feedbackList.shift(); // Remove oldest feedback
    }
    
    // Limit total execution history size
    if (this.executionHistory.size > this.maxExecutionHistorySize) {
  ...
  private pruneExecutionHistory(): void {
    // Remove oldest entries first (by timestamp in feedback)
    const entries = Array.from(this.executionHistory.entries());
    entries.sort(([, feedbackA], [, feedbackB]) => {
      const timestampA = feedbackA[0]?.reward?.metadata?.executionTime || 0;
      const timestampB = feedbackB[0]?.reward?.metadata?.executionTime || 0;
    ...
    for (let i = 0; i < toRemove; i++) {
      ...
      if (entry && entry[0]) {
        this.executionHistory.delete(entry[0]);
      }
    }
  }

  /**
   * Comprehensive shutdown method to prevent memory leaks
   */
  async shutdown(): Promise<void> {
    log.info('Shutting down AB-MCTS Service...', LogContext.AI);
    ...
    try {
      ...
      log.debug('Clearing active timeouts', LogContext.AI, { count: this.activeTimeouts.size });
      for (const timeout of this.activeTimeouts) {
        clearTimeout(timeout);
      }
      this.activeTimeouts.clear();
      ...
      // Clear node cache
      log.debug('Clearing node cache', LogContext.AI, { nodes: this.nodeCache.size });
      this.nodeCache.clear();
      
      // Clear execution history
      log.debug('Clearing execution history', LogContext.AI, { entries: this.executionHistory.size });
      this.executionHistory.clear();
      ...
      log.info('AB-MCTS Service shutdown completed', LogContext.AI);
    } catch (error) {
      log.error('Error during AB-MCTS Service shutdown', LogContext.AI, {
        error: error instanceof Error ? error.message : String(error)
      });
      throw error;
    }
...
class ABMCTSOrchestrator {
  ...
  async processUserFeedback(id: string, rating: number, comment?: string): Promise<void> {
    log.debug(`Mock feedback processed: ${id}, rating: ${rating}`, LogContext.AGENT);
  }
  ...
  reset(): void {
    log.info('AB-MCTS orchestrator reset', LogContext.SERVICE);
  }

  async shutdown(): Promise<void> {
    log.info('Shutting down AB-MCTS Orchestrator...', LogContext.SERVICE);
    await this.service.shutdown();
    log.info('AB-MCTS Orchestrator shutdown completed', LogContext.SERVICE);
  }
}
