...
export class ConnectionPoolManager extends EventEmitter {
  ...
  public async getSupabaseConnection(
    ...
    if (pool.length < this.config.database.max) {
      try {
        ...
        logger.error(`Failed to create Supabase connection for pool ${poolName}:`, error);
        throw error;
      }
    }
  ...
  public async getRedisConnection(poolName = 'default'): Promise<RedisClientType> {
    ...
    if (pool.length < this.config.redis.max) {
      try {
        ...
        logger.error(`Failed to create Redis connection for pool ${poolName}:`, error);
        throw error;
      }
    }
  ...
  private async createRedisConnection(poolName: string): Promise<RedisClientType> {
    ...
    client.on('error', (err) => {
      logger.error(`Redis connection error in pool ${poolName}:`, err);
      pooledConn.errors++;
      this.updateMetrics(poolName, 'redis', 'error');
    });
  ...
  private async checkConnectionHealth<T>(
    poolName: string,
    ...
    // Recycle connections based on age, idle time, or error count
    const shouldRecycle = 
      age > 3600000 || // 1 hour
  ...
  private async recycleConnection<T>(
    poolName: string,
    pooledConn: PooledConnection<T>
  ) {
