...
export class MLXService {
  ...
  constructor(config: MLXConfig = {}) {
    // Use our AI virtual environment Python interpreter
    const aiEnvPath = '/Users/christianmerrill/Desktop/universal-ai-tools/ai-env';
    const aiPython = existsSync(join(aiEnvPath, 'bin', 'python')) ? 
      join(aiEnvPath, 'bin', 'python') : 'python3';
    
    this.config = {
      pythonPath: config.pythonPath || aiPython,
    ...
    // Set default model path to our converted LFM2-1.2B MLX model
    this.defaultModelPath = '/Users/christianmerrill/Desktop/universal-ai-tools/models/agents/LFM2-1.2B-mlx';
  ...
  private async startMLXBridge(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.pythonProcess = spawn(this.config.pythonPath!, [scriptPath], {
        ...
        env: {
          ...process.env,
          VIRTUAL_ENV: '/Users/christianmerrill/Desktop/universal-ai-tools/ai-env',
          PATH: `/Users/christianmerrill/Desktop/universal-ai-tools/ai-env/bin:${process.env.PATH}`,
          PYTHONPATH: `/Users/christianmerrill/Desktop/universal-ai-tools/ai-env/lib/python3.13/site-packages:${join(__dirname, '..', '..')}:${process.env.PYTHONPATH || ''}`,
        },
      });
...
`;
  ...
  /**
   * Get the default LFM2-1.2B MLX model path
   */
   ...
   */
  public async runDefaultInference(prompt: string, parameters?: any): Promise<any> {
  ...
  public async healthCheck(): Promise<any> {
    try {
      // Check environment override for forced mock mode
      ...
      if (!this.isInitialized && this.pythonProcess === null) {
        return {
          status: 'initializing',
          healthy: true, // Still healthy, just initializing
          mode: 'initializing',
          message: 'MLX service initializing - will fall back to mock mode if needed',
          retryAfter: 5000,
          timestamp: new Date().toISOString(),
        };
      }
      ...
      if (!this.isInitialized) {
        return {
          status: 'healthy',
          healthy: true, // Healthy in mock mode
          mode: 'mock',
          message: 'MLX not available - running in mock mode for development',
          fallbackReason: 'MLX framework not found or failed to initialize',
          suggestion: 'Install MLX with: pip install mlx-lm',
          timestamp: new Date().toISOString(),
        };
      }
      ...
      return {
        ...
        healthy: true,
        mode: 'native',
        message: 'MLX service running with native Apple Silicon acceleration',
      ...
      return {
        status: 'healthy', // Still healthy in fallback mode
        healthy: true,
        mode: 'mock',
        message: 'MLX service error - running in mock mode',
        error: error instanceof Error ? error.message : String(error),
        timestamp: new Date().toISOString(),
      };
    }
