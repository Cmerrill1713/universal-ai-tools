...
export class SecretsManager {
  ...
  /**
   * Get a secret from Vault with enhanced security and fallback
   */
  public async getSecret(name: string): Promise<string | null> {
    if (!this.initialized) {
      // For critical secrets like JWT, fallback to environment
      if (name === 'jwt_secret') {
        const envSecret = process.env.JWT_SECRET;
        if (envSecret && envSecret.length >= 32) {
          log.info('🔐 Using JWT secret from environment (Vault not initialized)', LogContext.SYSTEM);
          return envSecret;
        }
      }
      return null;
    ...
    try {
      ...
      // Validate secret quality for security-critical secrets
      if (data && name === 'jwt_secret') {
        if (data.length < 32) {
          log.error('🔐 JWT secret from Vault is too short (< 32 chars)', LogContext.SYSTEM);
          return null;
        }
        log.info('🔐 JWT secret retrieved from Vault', LogContext.SYSTEM, {
          source: 'vault',
          length: data.length
        });
      }
      ...
      // Enhanced fallback for JWT secret
      if (name === 'jwt_secret') {
        const envSecret = process.env.JWT_SECRET;
        if (envSecret && envSecret.length >= 32) {
          log.warn('🔐 Fallback to environment JWT secret (Vault failed)', LogContext.SYSTEM, {
  ...
  /**
   * Get API key with fallback to environment variable and security validation
   */
  public async getApiKeyWithFallback(service: string, envVar: string): Promise<string | null> {
    ...
    if (vaultKey) {
      ...
      if (vaultKey.length < 16) {
        log.warn(`⚠️ API key for ${service} is suspiciously short`, LogContext.SYSTEM);
        return null;
      }
      return vaultKey;
    }
    ...
    if (envKey && envKey !== `your-${service}-key-here`) {
      // Validate key format for security
      if (envKey.length < 16) {
        log.warn(`⚠️ Environment API key for ${service} is too short`, LogContext.SYSTEM);
        return null;
      }
   ...
   */
  public async getJwtSecret(): Promise<string | null> {
    const secret = await this.getSecret('jwt_secret');
    
    if (!secret) {
      log.error('🔐 JWT secret not found in Vault or environment', LogContext.SYSTEM);
      return null;
    }
    
    // Enhanced security validation
    if (secret.length < 32) {
      log.error('🔐 JWT secret must be at least 32 characters for security', LogContext.SYSTEM, {
    ...
    if (isProduction && weakPatterns.some(pattern => pattern.test(secret))) {
      log.error('🔐 JWT secret appears to be a weak/default value in production', LogContext.SYSTEM);
      return null;
    }
