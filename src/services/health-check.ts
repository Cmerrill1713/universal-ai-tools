  ...
  private registerHealthChecks() {
      ...
      try {
        ...
        try {
          ...
          return {
            ...
            details: `${error.message  } (Auth fallback also failed: ${  authError.message  })`
          };
        }
      }
    });
    ...
    this.healthChecks.set('disk', async () => {
      try {
        ...
        if (isWindows) {
          ...
          const totalSpace = parseInt(dataLine[2]) || 1;
          ...
          const percentUsed = parseInt(stats[4]?.replace('%', '')) || 0;
    ...
    this.healthChecks.set('circuitBreakers', () => {
      ...
      return Promise.resolve({
        ...
        message: cbHealth.openCircuits.length > 0 
