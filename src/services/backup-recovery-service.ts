...
async function loadAwsSdk(): Promise<boolean> {
  ...
  try {
    ...
    awsSdkError = error instanceof Error ? error.message : 'Unknown error loading AWS SDK';
...
export class BackupRecoveryService {
  ...
  private initializeEncryption() {
    if (this.config.encryption.enabled) {
      ...
      if (!password) {
        logger.warn('Backup encryption enabled but BACKUP_ENCRYPTION_PASSWORD not set', LogContext.DATABASE);
  ...
  private async ensureS3Initialized(): Promise<boolean> {
    if (this.config.storage.s3.enabled) {
      ...
      if (!sdkLoaded) {
        logger.warn('AWS SDK not available - S3 backup storage disabled', LogContext.DATABASE, getAwsSdkInstallationHelp());
        this.config.storage.s3.enabled = false;
        return false;
      }
  ...
  async createBackup(options: {
    ...
    try {
      ...
      logger.error(`Backup ${backupId} failed: ${error.message}`, LogContext.DATABASE, { error });
  ...
  private async exportTables(tables: string[]): Promise<{
    ...
    for (const table of tables) {
      try {
        ...
        logger.error(`Failed to export table ${table}: ${error.message}`, LogContext.DATABASE, { error });
  ...
  private async storeLocalBackup(
    ...
    const backupDir = path.join(this.config.storage.local.path, 
  ...
  private async storeS3Backup(
    ...
    try {
      ...
      if (!s3Ready) {
        throw new Error(`S3 upload failed: ${JSON.stringify(getAwsSdkInstallationHelp(), null, 2)}`);
      }
      
      await this.s3Client.send(new PutObjectCommand(uploadParams));
      
      logger.debug(`Stored S3 backup: s3://${this.config.storage.s3.bucket}/${key}`, LogContext.DATABASE);
   ...
   */
  private async loadSupabaseBackup(
    backupId: string,
    metadata: BackupMetadata
  ): Promise<any> {
    const date = metadata.timestamp.toISOString().split('T')[0];
  ...
  private async loadS3Backup(
    ...
    try {
      ...
      const s3Ready = await this.ensureS3Initialized();
      if (!s3Ready) {
        throw new Error(`S3 download failed: ${JSON.stringify(getAwsSdkInstallationHelp(), null, 2)}`);
      }
   ...
   * Delete local backup
   */
  private async deleteLocalBackup(
    backupId: string,
    metadata: BackupMetadata
  ): Promise<void> {
    const date = metadata.timestamp.toISOString().split('T')[0];
   ...
   */
  private async deleteSupabaseBackup(
    backupId: string,
    metadata: BackupMetadata
  ): Promise<void> {
    const date = metadata.timestamp.toISOString().split('T')[0];
  ...
  private async deleteS3Backup(
    ...
    try {
      ...
      const s3Ready = await this.ensureS3Initialized();
      if (!s3Ready) {
        throw new Error(`S3 delete failed: ${JSON.stringify(getAwsSdkInstallationHelp(), null, 2)}`);
      }
      
      await this.s3Client.send(new DeleteObjectCommand(deleteParams));
      
      logger.debug(`Deleted S3 backup: s3://${this.config.storage.s3.bucket}/${key}`, LogContext.DATABASE);
  ...
  private calculateNextBackupTime(): Date | null {
    ...
    // For now, return tomorrow at 2 AM
