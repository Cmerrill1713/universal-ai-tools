...
export class SupabaseDocsScraper {
  ...
  async scrapeAndStore(): Promise<void> {
    ...
    try {
      ...
      const features = [
        // Core Features
        { category: 'Database', url: '/guides/database', subcategories: ['Tables', 'RLS', 'Triggers', 'Functions'] },
        { category: 'Auth', url: '/guides/auth', subcategories: ['Email', 'Social', 'Phone', 'MFA'] },
        { category: 'Storage', url: '/guides/storage', subcategories: ['Uploads', 'Downloads', 'Policies', 'CDN'] },
        { category: 'Realtime', url: '/guides/realtime', subcategories: ['Broadcast', 'Presence', 'Postgres Changes'] },
        { category: 'Edge Functions', url: '/guides/functions', subcategories: ['Deploy', 'Secrets', 'CORS', 'Webhooks'] },
        
        // Extensions
        { category: 'Vector/Embeddings', url: '/guides/ai', subcategories: ['pgvector', 'OpenAI', 'Similarity Search'] },
        { category: 'GraphQL', url: '/guides/graphql', subcategories: ['pg_graphql', 'Queries', 'Mutations', 'Subscriptions'] },
        { category: 'Vault', url: '/guides/vault', subcategories: ['Encryption', 'Key Management', 'Secrets'] },
        { category: 'Cron', url: '/guides/cron', subcategories: ['pg_cron', 'Scheduled Jobs', 'Maintenance'] },
        
        // Advanced Features
        { category: 'Webhooks', url: '/guides/webhooks', subcategories: ['Database Webhooks', 'HTTP Triggers'] },
        { category: 'Wrappers', url: '/guides/wrappers', subcategories: ['Foreign Data', 'External APIs'] },
        { category: 'Analytics', url: '/guides/analytics', subcategories: ['BigQuery', 'Iceberg', 'Data Export'] }
      ];
   ...
   */
  private async processFeature(feature: { category: string; url: string; subcategories: string[] }): Promise<void> {
  ...
  private getFeatureDescription(category: string): string {
    const descriptions: Record<string, string> = {
      'Database': 'Supabase provides a full Postgres database with automatic APIs, real-time subscriptions, and Row Level Security.',
      ...
      'Storage': 'S3-compatible object storage with CDN, automatic image optimization, and fine-grained access controls.',
      'Realtime': 'WebSocket-based real-time updates for database changes, broadcast messages, and presence tracking.',
      ...
      'Vault': 'Postgres extension for managing secrets and encryption keys directly in your database.',
...
COMMENT ON COLUMN posts.content IS E'@graphql({"description": "Post content"})';
...
WHERE hour > (
  ...
  private getPrerequisites(category: string): string[] {
    const prerequisites: Record<string, string[]> = {
      ...
      'Wrappers': [
        'SQL knowledge',
        'Understanding of foreign data',
        'API integration experience'
      ],
      'Analytics': [
        'Data warehouse concepts',
        'ETL understanding',
        'SQL aggregation knowledge'
      ]
    };
  ...
  private async storeDocumentation(): Promise<void> {
    ...
    for (const [key, doc] of this.docsCache) {
      try {
        ...
        const { error: kbError } = await this.supabase
        ...
        if (validatedDoc.examples) {
          for (const example of validatedDoc.examples) {
            const { error: exampleError } = await this.supabase
