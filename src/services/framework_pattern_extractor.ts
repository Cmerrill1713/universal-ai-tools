 ...
 * Supports pattern-based code generation and best practices enforcement
 */
...
export class FrameworkPatternExtractor {
    ...
    const extractFromDirectory = async (dirPath: string, depth = 0) => {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const filePatterns = await this.extractPatternsFromFile(
            fullPath, 
            rootPath, 
            framework
          );
  ...
  private extractReactPatterns(content: string, filePath: string): FrameworkPattern[] {
    ...
    if (functionComponentMatch) {
      patterns.push(this.createReactFunctionComponentPattern(
    ...
    // Context Pattern
    const contextMatch = content.match(/(?:const|let)\s+(\w+Context)\s*=\s*(?:React\.)?createContext/);
    if (contextMatch) {
      patterns.push(this.createReactContextPattern(contextMatch[1], content, filePath));
    }

    // HOC Pattern
    const hocMatch = content.match(/(?:const|function)\s+(with\w+).*?=.*?(?:Component|WrappedComponent)/);
    if (hocMatch) {
  ...
  private extractVuePatterns(content: string, filePath: string): FrameworkPattern[] {
    ...
    if (content.includes('setup()') || content.includes('<script setup>')) {
      patterns.push(this.createVueCompositionPattern(content, filePath));
    }

    // Composable Pattern
    const composableMatch = content.match(/(?:export\s+)?(?:const|function)\s+(use\w+)/);
  ...
  private extractNextPatterns(content: string, filePath: string): FrameworkPattern[] {
    ...
    if (content.includes('use server') || filePath.includes('.server.')) {
  ...
  private extractGenericPatterns(content: string, filePath: string): FrameworkPattern[] {
    ...
    // Singleton Pattern
    if (content.match(/class\s+\w+\s*{[\s\S]*?static\s+instance/)) {
      patterns.push(this.createSingletonPattern(content, filePath));
    }

    // Factory Pattern
    if (content.match(/(?:create|make|build)\w+\s*\(/)) {
      patterns.push(this.createFactoryPattern(content, filePath));
    }

    // Observer Pattern
    if (content.match(/(?:subscribe|observe|listen|on)\s*\(/)) {
  ...
  /**
   * Create pattern objects for different framework patterns
   */
  ...
  private createReactContextPattern(
    ...
    return {
      ...
      structure: {
        ...
        relationships: [{
          pattern: 'React Function Component',
          relationship: 'uses',
          optional: false
        }]
      },
    ...
    return {
      ...
      structure: {
        files: [{
          ...
          type: 'component',
          template: isAppDir ? this.generateNextAppRouteTemplate() : this.generateNextPageTemplate(),
          required: true
        }],
        imports: this.extractImports(content),
        exports: this.extractExports(content),
        conventions: [{
          type: 'file',
          pattern: isAppDir ? /^page\.(tsx?|jsx?)$/ : /^[a-z\-]+\.(tsx?|jsx?)$/,
  ...
  private createNextAPIPattern(content: string, filePath: string): FrameworkPattern {
    ...
    return {
      ...
      structure: {
        ...
        conventions: [{
          type: 'file',
          pattern: isAppDir ? /^route\.(ts|js)$/ : /^[a-z\-]+\.(ts|js)$/,
...
export default async function ServerComponent() {
  ...
  private createFactoryPattern(content: string, filePath: string): FrameworkPattern {
    return {
      ...
      structure: {
        ...
        conventions: [{
          type: 'function',
          pattern: /^(create|make|build)[A-Z][a-zA-Z0-9]*$/,
  ...
  private createObserverPattern(content: string, filePath: string): FrameworkPattern {
    return {
      ...
      structure: {
        ...
        conventions: [{
          type: 'class',
          pattern: /^[A-Z][a-zA-Z0-9]*(Observer|Subject|EventEmitter)$/,
  ...
  /**
   * Generate generic pattern templates
   */
...
export class EventEmitter<T = any> {
    ...
    while ((match = importRegex.exec(content)) !== null) {
      ...
      imports.push({
        ...
        imports: namespace ? [namespace] : 
                ...
                namedImports ? namedImports.replace(/[{}]/g, '').split(',').map(i => i.trim()) : [],
  ...
  private consolidatePatterns(patterns: FrameworkPattern[]): FrameworkPattern[] {
    ...
    for (const pattern of patterns) {
      const key = `${pattern.framework}-${pattern.name}`;
      ...
      if (consolidated.has(key)) {
        ...
        pattern.examples.forEach(e => uniqueExamples.set(e.title, e));
   ...
   * Framework detection
   */
  private async detectFramework(rootPath: string): Promise<{ name: string; version: string } | null> {
    try {
  ...
  private generateStatistics(patterns: FrameworkPattern[]): AnalysisResult['statistics'] {
    ...
    patterns.forEach(pattern => {
      ...
      byComplexity[pattern.metadata.complexity] = (byComplexity[pattern.metadata.complexity] || 0) + 1;
    });
  ...
  private generateRecommendations(
    ...
    if (!hasRouting) {
      recommendations.push('Consider implementing routing patterns for better navigation structure');
    }
    
    const hasStateManagement = patterns.some(p => p.category === 'state');
    if (!hasStateManagement && patterns.length > 10) {
      recommendations.push('Large application detected - consider adding state management patterns');
    }
    
    // Check for anti-patterns
    const hasCircularDeps = patterns.some(p => 
      p.structure.imports.some(imp => 
        imp.isRelative && imp.source.includes('../')
      )
    );
    if (hasCircularDeps) {
      ...
      const { error } = await this.supabase
        ...
        .upsert(
          patterns.map(pattern => ({
            ...
            best_practices: pattern.bestPractices,
  ...
  async generateFromPattern(
    ...
    for (const file of pattern.structure.files) {
      if (!file.required && !options.variables?.includeOptional) continue;
      
      const fileName = file.name.replace(/\[(\w+)\]/g, (_, key) => 
        options.variables?.[key] || options.name
      );
