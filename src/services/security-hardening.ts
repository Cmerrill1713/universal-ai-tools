...
export class SecurityHardeningService {
  ...
  async checkApiKeyRotation(): Promise<ApiKeyRotationStatus[]> {
    ...
    try {
      ...
      logger.error('Failed to check API key rotation:', error);
    }
  ...
  private generateRecommendations(
    ...
    if (expiredKeys.length > 0) {
      ...
      expiredKeys.forEach(k => {
        recommendations.push(`- ${k.keyName}: Last rotated ${Math.floor((Date.now() - k.lastRotated.getTime()) / (24 * 60 * 60 * 1000))} days ago`);
      });
    }
  ...
  async rotateApiKey(keyName: string): Promise<string> {
    try {
      ...
      logger.error(`Failed to rotate API key ${keyName}:`, error);
      throw error;
    }
  }
  ...
  async fixVulnerabilities(dryRun = false): Promise<{
    ...
    try {
      ...
      logger.error('Failed to fix vulnerabilities:', error);
...
export const securityHardeningService = {
  ...
  checkCommonVulnerabilities: async () => getSecurityHardeningService().checkCommonVulnerabilities(),
  sanitizeInput: (input: any) => getSecurityHardeningService().sanitizeInput(input),
  sanitizeSQL: (query: string, params?: any[]) => getSecurityHardeningService().sanitizeSQL(query, params),
  fixVulnerabilities: async (dryRun = false) => getSecurityHardeningService().fixVulnerabilities(dryRun)
};