...
EXAMPLES: ${examples.map((ex) => `Input: ${ex.input}nExpected: ${ex.expectedOutput}nScore: ${ex.score}`).join('n\n')}'''
...
TRAINING EXAMPLES: ${examples.map((ex) => `Q: ${ex.input}nA: ${ex.expectedOutput}`).join('nn')}'''
