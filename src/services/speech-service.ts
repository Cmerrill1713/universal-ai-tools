...
export class SpeechService {
  ...
  async transcribeAudio(
    ...
    try {
      ...
      logger.error('Transcription error', LogContext.AVATAR, { error });
  ...
  private async transcribeWithOpenAI(
    ...
    try {
      ...
      logger.error('OpenAI Whisper error', LogContext.AVATAR, { error });
  ...
  private async transcribeWithLocalWhisper(
    ...
    try {
      ...
      logger.error('Local Whisper error', LogContext.AVATAR, { error });
  ...
  async synthesizeSpeech(options: SynthesisOptions): Promise<AudioResult> {
    try {
      ...
      logger.error('Synthesis error', LogContext.AVATAR, { error });
  ...
  private async synthesizeWithElevenLabs(options: SynthesisOptions): Promise<AudioResult> {
    ...
    try {
      const response = await axios.post(
        url,
        {
          ...
          voice_settings: {
            ...
            similarity_boost: options.voiceSettings?.similarity_boost || options.voiceProfile.similarity_boost,
            style: options.voiceSettings?.style || options.voiceProfile.style,
            use_speaker_boost: options.voiceSettings?.use_speaker_boost || options.voiceProfile.use_speaker_boost
          }
        },
        {
          headers: {
      ...
      if (!validation.isValid) {
        logger.warn('ElevenLabs audio validation issues', LogContext.AVATAR, { errors: validation.errors });
      }
      ...
      logger.error('ElevenLabs synthesis error', LogContext.AVATAR, { error });
  ...
  private async synthesizeWithOpenAI(options: SynthesisOptions): Promise<AudioResult> {
    ...
    try {
      ...
      const validation = await audioHandler.validateAudioBuffer(buffer, options.format);
      if (!validation.isValid) {
        logger.warn('OpenAI audio validation issues', LogContext.AVATAR, { errors: validation.errors });
      }
      ...
      logger.error('OpenAI TTS error', LogContext.AVATAR, { error });
  ...
  private async synthesizeWithLocalTTS(options: SynthesisOptions): Promise<AudioResult> {
    try {
      ...
      logger.error('Kokoro TTS failed', LogContext.AVATAR, { error });
  ...
  private async synthesizeWithKokoro(options: SynthesisOptions): Promise<AudioResult> {
    try {
      ...
      if (audioProcessingResult.warnings.length > 0) {
        logger.warn('Audio processing warnings', LogContext.AVATAR, { warnings: audioProcessingResult.warnings });
      }
      ...
      logger.error('Kokoro TTS synthesis error', LogContext.AVATAR, { error });
  ...
  async getAvailableVoices(): Promise<any[]> {
    ...
    try {
      const kokoroProfiles = kokoroTTS.getVoiceProfiles();
      voices.push(...kokoroProfiles.map(profile => ({
        id: profile.id,
        name: profile.name,
        provider: 'kokoro',
      ...
      logger.error('Failed to fetch Kokoro voices', LogContext.AVATAR, { error });
    }
    ...
    if (this.elevenLabsApiKey) {
      try {
        ...
        voices.push(...response.data.voices.map((voice: any) => ({
          id: voice.voice_id,
          name: voice.name,
          provider: 'elevenlabs',
        ...
        logger.error('Failed to fetch ElevenLabs voices', LogContext.AVATAR, { error });
      }
    }
    ...
    if (this.openai) {
      voices.push(
        { id: 'nova', name: 'Nova (Sweet)', provider: 'openai', description: 'Warm and friendly', local: false },
        { id: 'shimmer', name: 'Shimmer (Shy)', provider: 'openai', description: 'Soft and gentle', local: false },
        { id: 'alloy', name: 'Alloy (Confident)', provider: 'openai', description: 'Clear and assured', local: false },
        { id: 'echo', name: 'Echo (Caring)', provider: 'openai', description: 'Soothing and nurturing', local: false },
        { id: 'fable', name: 'Fable (Playful)', provider: 'openai', description: 'Expressive and lively', local: false }
      );
    }
  ...
  async testKokoroVoice(voiceId: string, sampleText?: string): Promise<Buffer> {
    try {
      ...
      logger.error('Kokoro voice test failed', LogContext.AVATAR, { error });
      throw error;
    }
  }
  ...
  async getServiceHealth(): Promise<{
    ...
    try {
      ...
      health.details.openai = { available: false, error: error instanceof Error ? error.message : 'Unknown error' };
    }
    ...
    try {
      ...
      health.details.elevenlabs = { available: false, error: error instanceof Error ? error.message : 'Unknown error' };
    }
    ...
    try {
      ...
      health.details.kokoro = { available: false, error: error instanceof Error ? error.message : 'Unknown error' };
    }
    ...
    try {
      ...
      health.details.whisper = { available: false, error: error instanceof Error ? error.message : 'Unknown error' };
    }
    ...
    try {
      ...
      health.details.audioProcessing = { available: false, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  ...
  async clearAllCaches(): Promise<void> {
    try {
      await Promise.all([
        kokoroTTS.clearCache(),
        audioHandler.clearCache()
      ]);
      logger.info('All speech service caches cleared', LogContext.AVATAR);
    } catch (error) {
      logger.error('Error clearing speech service caches', LogContext.AVATAR, { error });
    }
  }
  ...
  async synthesizeSpeechWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      try {
        ...
        lastError = error instanceof Error ? error : new Error('Unknown synthesis error');
        logger.warn(`Speech synthesis attempt ${attempt} failed: ${lastError.message}`, LogContext.AVATAR);
  ...
  async estimateAudioDuration(text: string, voiceProfile?: VoiceProfile): Promise<number> {
    try {
      ...
      logger.error('Error estimating audio duration', LogContext.AVATAR, { error });
