...
import { AutofixMemoryService, type AutofixMemory } from './autofix-memory-service';
...
export class AdaptiveAutofixService {
  ...
  async applyFixWithFeedback(
    ...
    const newErrors = afterValidation.errors.filter(error => 
      !beforeValidation.errors.some(oldError => 
        oldError.line === error.line && oldError.message === error.message
      )
    );
    ...
    const validation: FixValidation = {
      ...
      validation_time_ms: Date.now() - parseInt(fixId.split('_')[1])
    };
    ...
    logger.info(`📊 Fix ${validation.success ? 'succeeded' : 'failed'}: ${improvement} errors fixed, ${newErrors.length} new errors`);
  ...
  private async learnFromFix(fix: AutofixMemory, validation: FixValidation): Promise<string[]> {
    ...
    if (validation.success && validation.improvement_score > 0.5) {
      adjustments.push(`✅ ${fix.fix_type} is highly effective for ${path.extname(fix.file_path)} files`);
    ...
    if (!validation.success) {
      adjustments.push(`❌ ${fix.fix_type} may need refinement - introduced ${validation.new_errors_introduced.length} new errors`);
   ...
   */
  async getAdaptiveRecommendations(filePath: string, currentErrors: string[]): Promise<{
    ...
    const failurePatterns = similarFixes
      .filter(fix => !fix.success)
      .map(fix => fix.fix_type);

    // Get file-type specific insights
    const fileTypeInsights = this.learningInsights.filter(insight => 
      insight.file_type_effectiveness[fileExtension] > 0.6
    );
  ...
  async runAdaptiveLearningLoop(): Promise<void> {
    ...
    for (const pattern of decliningPatterns) {
      await this.adjustFixStrategy(pattern.pattern, 'reduce_confidence');
    }

    for (const pattern of improvingPatterns) {
      await this.adjustFixStrategy(pattern.pattern, 'increase_priority');
    }
    ...
    await this.storeLearningUpdate({
      session_id: this.sessionId,
      declining_patterns: decliningPatterns.map(p => p.pattern),
      improving_patterns: improvingPatterns.map(p => p.pattern),
      new_variations: newVariations,
      timestamp: new Date().toISOString()
    });

    logger.info(`📈 Learning loop complete: ${improvingPatterns.length} improving, ${decliningPatterns.length} declining patterns`);
  }

  /**
   * Validate file and get error details
   */
  ...
  private async storeSuccessPattern(fix: AutofixMemory): Promise<void> {
    ...
    const content = `Successful fix pattern: ${fix.fix_type} in ${fix.file_path}`;
      ...
      await this.supabase
        ...
        .insert({
          ...
          metadata: {
            ...
            tags: ['autofix', 'success_pattern', fix.fix_type]
          },
          user_id: 'claude-autofix'
        });
    } catch (error) {
      logger.warn('Failed to store success pattern:', error);
    }
  }
  ...
  private async analyzeFailure(fix: AutofixMemory, validation: FixValidation): Promise<string[]> {
    ...
    if (validation.new_errors_introduced.length > 0) {
      analysis.push(`Fix introduced ${validation.new_errors_introduced.length} new errors`);
      
      // Common failure patterns
      const newErrors = validation.new_errors_introduced.join(' ');
      if (newErrors.includes('is not defined')) {
        analysis.push('Consider checking imports and variable declarations');
      }
      if (newErrors.includes('Cannot find module')) {
        analysis.push('Fix may have broken import paths');
      }
      if (newErrors.includes('Type')) {
        analysis.push('Type-related fix may need more specific typing');
      }
    }
  ...
  private prioritizeFixTypes(successPatterns: string[], insights: LearningInsight[]): string[] {
    ...
    successPatterns.forEach(pattern => {
      priorities.set(pattern, (priorities.get(pattern) || 0) + 1);
    });

    insights.forEach(insight => {
      priorities.set(insight.pattern, (priorities.get(insight.pattern) || 0) + insight.success_rate);
    });
  ...
  private async updateLearningInsights(fix: AutofixMemory, validation: FixValidation): Promise<void> {
    // This would update the internal learning insights array
    // and periodically sync with Supabase
  }

  private async adjustFixStrategy(pattern: string, adjustment: 'reduce_confidence' | 'increase_priority'): Promise<void> {
    // Update fix strategy based on learning
    logger.info(`🎯 Adjusting strategy for ${pattern}: ${adjustment}`);
  }
  ...
  private async generateFixVariations(patterns: LearningInsight[]): Promise<string[]> {
    ...
    return patterns.map(p => `${p.pattern}_enhanced`);
  }

  private async storeLearningUpdate(update: any): Promise<void> {
    try {
      const content = `Learning update: ${update.improving_patterns.length} improving patterns`;
      ...
      logger.warn('Failed to store learning update:', error);
    }
  }
}