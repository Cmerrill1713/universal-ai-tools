...
export class CacheConsistencyService extends EventEmitter {
  ...
  private initializeSubscriptions(): void {
    ...
    this.subClient.on('message', async (channel, message) => {
      if (channel === this.INVALIDATION_CHANNEL) {
        const { pattern, tags, version } = JSON.parse(message);
        await this.handleRemoteInvalidation(pattern, tags, version);
      }
    });
  }
  ...
  async invalidate(pattern?: string, tags?: string[], version?: string): Promise<void> {
    ...
    try {
      ...
      await this.pubClient.publish(
        ...
        JSON.stringify({ pattern, tags, version })
      );
  ...
  private async handleRemoteInvalidation(
    ...
    await this.invalidate(pattern, tags, version);
  }
  ...
  async getStats(key?: string): Promise<CacheStats | Map<string, CacheStats>> {
    if (key) {
      ...
      if (persisted && Object.keys(persisted).length > 0) {
        return {
          hits: parseInt(persisted.hits || '0'),
          misses: parseInt(persisted.misses || '0'),
          evictions: parseInt(persisted.evictions || '0'),
