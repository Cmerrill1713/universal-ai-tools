...
export class RerankingEvaluationService {
  ...
  async evaluateReranking(
    ...
    try {
      ...
      const relevanceImprovement = this.calculateRelevanceImprovement(
        originalResults, rerankedResults, relevanceMap
      );
      ...
      this.logger.debug(`Reranking evaluation completed: NDCG=${ndcg.toFixed(3)}, MAP=${map.toFixed(3)}`);
  ...
  async runABTest(
    ...
    methodB: string,
    testQueries: string[],
    evaluationFunction: (query: string, method: string) => Promise<Array<{ id: string; score: number; rank: number }>>
  ): Promise<ABTestResult> {
    
    this.logger.info(`Starting A/B test: ${methodA} vs ${methodB} on ${testQueries.length} queries`);
    ...
    this.logger.info(`A/B test completed: ${winner} wins with ${significance.toFixed(3)} significance`);
  ...
  async generateEvaluationReport(
    ...
    detailed_analysis: {
      ...
      query_difficulty_analysis: Array<{ difficulty: 'easy' | 'medium' | 'hard'; count: number; avg_performance: number }>;
      failure_analysis: Array<{ issue: string; frequency: number; impact: number }>;
    };
    recommendations: string[];
  }> {
    
    const filteredResults = this.evaluationResults.filter(result => 
      result.latency >= timeRange.start.getTime() && 
      result.latency <= timeRange.end.getTime()
    );
    ...
    return {
      ...
      detailed_analysis: {
        ...
        query_difficulty_analysis: difficultyAnalysis,
  ...
  private calculateNDCG(results: Array<{ id: string; score: number }>, relevanceMap: Map<string, number>, k = 10): number {
    ...
    const sortedRelevances = Array.from(relevanceMap.values()).sort((a, b) => b - a).slice(0, k);
    let idcg = 0;
  ...
  private calculateMAP(results: Array<{ id: string; score: number }>, relevanceMap: Map<string, number>): number {
    let sumPrecision = 0;
    let relevantCount = 0;
    let totalRelevant = 0;
  ...
  private calculateMRR(results: Array<{ id: string; score: number }>, relevanceMap: Map<string, number>): number {
    for (let i = 0; i < results.length; i++) {
      const relevance = relevanceMap.get(results[i].id) || 0;
      if (relevance > 2) {
        return 1 / (i + 1);
  ...
  private calculateDiversityScore(results: Array<{ id: string; score: number }>): number {
    // Simplified diversity calculation - in practice would use content analysis
  ...
  private calculateErrorRate(
    ...
    for (const result of results) {
      const relevance = relevanceMap.get(result.id) || 0;
      // Consider it an error if high-scored result has low relevance or vice versa
  ...
  private async calculateStabilityScore(query: string, results: Array<{ id: string; score: number }>): Promise<number> {
  ...
  private diversifyQueries(searchHistory: any[], sampleSize: number, threshold: number): Array<{ query: string }> {
  ...
  private async generateExpectedResults(query: string): Promise<Array<{ id: string; relevanceScore: number; idealRank: number }>> {
  ...
  private calculateAverageMetrics(results: EvaluationMetrics[]): EvaluationMetrics {
    ...
    const sum = results.reduce((acc, curr) => ({
      ...
      precision_at_k: Object.fromEntries(
        Object.entries(acc.precision_at_k).map(([k, v]) => [k, v + (curr.precision_at_k[parseInt(k)] || 0)])
      ),
      recall_at_k: Object.fromEntries(
        Object.entries(acc.recall_at_k).map(([k, v]) => [k, v + (curr.recall_at_k[parseInt(k)] || 0)])
      ),
      ...
      confidence_distribution: acc.confidence_distribution.map((v, i) => v + curr.confidence_distribution[i]),
  ...
  private analyzeTrends(results: EvaluationMetrics[]): Record<string, 'improving' | 'stable' | 'declining'> {
  ...
  private async analyzeBestMethods(): Promise<Array<{ method: string; avg_ndcg: number; avg_map: number }>> {
  ...
  private analyzeQueryDifficulty(results: EvaluationMetrics[]): Array<{ difficulty: 'easy' | 'medium' | 'hard'; count: number; avg_performance: number }> {
    // Simplified difficulty analysis based on performance
    return [
  ...
  private analyzeFailures(results: EvaluationMetrics[]): Array<{ issue: string; frequency: number; impact: number }> {
    ...
    return [
      { issue: 'High error rate', frequency: highErrorResults.length / results.length, impact: 0.8 },
  ...
  private generateRecommendations(
    ...
    if (avgMetrics.ndcg < 0.7) {
      recommendations.push('Consider switching to more effective reranking method (hybrid or cross-encoder)');
    }
    
    if (avgMetrics.latency > 500) {
      recommendations.push('Optimize for performance - consider feature-based reranking or caching');
    }
  ...
  private compareMetrics(resultsA: EvaluationMetrics[], resultsB: EvaluationMetrics[]): Record<string, any> {
  ...
  private determineWinner(comparison: Record<string, any>): 'a' | 'b' | 'tie' {
    let scoreA = 0;
    let scoreB = 0;
  ...
  private calculateSignificance(resultsA: EvaluationMetrics[], resultsB: EvaluationMetrics[]): number {
  ...
  exportEvaluationData(): {
    ...
    return {
      ...
      summary_statistics: this.evaluationResults.length > 0 
