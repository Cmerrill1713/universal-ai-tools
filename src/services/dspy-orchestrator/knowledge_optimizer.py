...
class OptimizedKnowledgeEvolver(Module):
    ...
    def forward(self, existing: Dict[str, Any], new_info: Dict[str, Any], 
    ...
    def forward(self, knowledge: Dict[str, Any], context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            result = self.validate(
                knowledge_item=json.dumps(knowledge),
...
class KnowledgeOptimizer:
    ...
    def optimize_with_examples(self, examples: List[Dict[str, Any]], 
        ...
        try:
            ...
            optimized_extractor = optimizer.compile(
                self.extractor,
                trainset=trainset.get("extraction", [])
            )
            ...
            optimized_evolver = optimizer.compile(
                self.evolver,
                trainset=trainset.get("evolution", [])
            )
            ...
            optimized_validator = optimizer.compile(
                self.validator,
                trainset=trainset.get("validation", [])
            )
    ...
    def _track_optimization(self, results: Dict[str, Any]):
        ...
        self.optimization_history.append({
            "timestamp": datetime.now().isoformat(),
            "results": results,
            "extractor_metrics": self.extractor.performance_history[-1] if self.extractor.performance_history else {},
    ...
    def _calculate_improvement(self) -> float:
        ...
        if "extractor_metrics" in current_metrics and "confidence" in current_metrics["extractor_metrics"]:
            scores.append(current_metrics["extractor_metrics"]["confidence"])
        
        if "searcher_metrics" in current_metrics and "average_confidence" in current_metrics["searcher_metrics"]:
            scores.append(current_metrics["searcher_metrics"]["average_confidence"])
        
        if "validator_stats" in current_metrics and "average_score" in current_metrics["validator_stats"]:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "extractor": {
                ...
                "latest": self.extractor.performance_history[-1] if self.extractor.performance_history else None
            },
            "searcher": self.searcher.search_metrics.copy(),
            "evolver": {
                "history_length": len(self.evolver.evolution_history),
                "latest": self.evolver.evolution_history[-1] if self.evolver.evolution_history else None
            },
