...
class ModelManager:
    ...
    async def recommend_model(self, task_type: str, constraints: Dict[str, Any] = None) -> Optional[str]:
        ...
        for provider, models in all_models.items():
            for model in models:
                ...
                if task_type == "coding" and "coding" in (model.capabilities or []):
                    score += 10
                elif task_type == "reasoning" and any(c in (model.capabilities or []) 
                                                     for c in ["reasoning", "advanced_reasoning"]):
                    score += 10
