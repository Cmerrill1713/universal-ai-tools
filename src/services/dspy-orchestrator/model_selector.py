...
class ModelSelector:
    ...
    def analyze_task(
        ...
        logger.info(
            f"Task analysis: complexity={complexity.value}, capabilities={[c.value for c in capabilities]}"
        )
    ...
    def select_model_for_task(
        ...
        for key, profile in self.available_models.items():
            ...
            if (
                complexity == TaskComplexity.CRITICAL
                and profile.quality_score < 0.8
                or complexity == TaskComplexity.COMPLEX
                and profile.quality_score < 0.7
            ):
                continue
        ...
        for score, key, profile in scored_models:
            ...
            logger.info(
                f"🎯 Trying {model_name} (score: {score:.2f}, quality: {profile.quality_score:.1f}, speed: {profile.speed_score:.1f})"
            )
            ...
            try:
                ...
                if max_response_time_ms and profile.response_time_ms > max_response_time_ms:
                    logger.info(
                        f"⏱️ {model_name} too slow ({profile.response_time_ms:.0f}ms > {max_response_time_ms}ms)"
                    )
                    continue
    ...
    def escalate_to_larger_model(
        ...
        for key, profile in self.available_models.items():
            if (
                profile.quality_score > current_quality
                and profile.quality_score >= min_quality_score
            ):
...
if __name__ == "__main__":
    ...
    for task, context in test_tasks:
        ...
        kwargs = {
            k: v for k, v in context.items() if k in ["max_response_time_ms", "prefer_quality"]
        }
