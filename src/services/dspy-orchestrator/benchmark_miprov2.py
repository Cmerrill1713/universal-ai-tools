            ...
            examples.append(
                {
                    ...
                    "expected_output": {"relevance": query_template["expected_relevance"]},
                }
            )
        ...
        knowledge_items = [
            {
                "knowledge_item": {
                    ...
                    "content": {
                        "algorithm": "MIPROv2",
                        "benefits": ["better optimization", "faster convergence"],
                    },
                    "confidence": 0.95,
                },
            ...
            examples.append(
                {
                    ...
                    "expected_output": {"is_valid": template["expected_valid"]},
                }
            )
...
class MIPROv2Benchmark:
    ...
    def __init__(self):
        ...
        self.results = {"before_optimization": {}, "after_optimization": {}, "improvement": {}}
    ...
    async def run_benchmark(self):
        ...
        self.results["before_optimization"] = await self._benchmark_modules(
            extraction_examples[:10], search_examples[:10], validation_examples[:10]
        )
    ...
    async def _benchmark_modules(
        ...
        for example in extraction_examples:
            ...
            result = self.optimizer.extractor(
                raw_content=example["raw_content"], context=example["context"]
            )
        ...
        for example in search_examples:
            ...
            result = self.optimizer.searcher(
                query=example["query"], context=example["search_context"]
            )
        ...
        for example in validation_examples:
            ...
            result = self.optimizer.validator(
                knowledge=example["knowledge_item"], context=example["validation_context"]
            )
    ...
    def _print_results(self):
        ...
        avg_conf_improvement = np.mean(
            [imp.get("confidence_improvement", 0) for imp in self.results["improvement"].values()]
        )
        avg_time_improvement = np.mean(
            [imp.get("time_improvement", 0) for imp in self.results["improvement"].values()]
        )
