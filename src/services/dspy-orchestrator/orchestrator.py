...
# HRM Integration Signatures
class HRMPreprocessorSignature(dspy.Signature):
    """Fast preprocessing with HRM for template-based reasoning."""
    
    request = dspy.InputField(desc="User request for preprocessing")
    template_hint = dspy.InputField(desc="Suggested reasoning template")
    preprocessed = dspy.OutputField(desc="HRM preprocessed reasoning")
    confidence = dspy.OutputField(desc="Confidence score")
    template_used = dspy.OutputField(desc="Template actually used")


class HRMEnhancedSignature(dspy.Signature):
    """Combine HRM preprocessing with LLM generation."""
    
    hrm_reasoning = dspy.InputField(desc="HRM preprocessed reasoning")
    original_request = dspy.InputField(desc="Original user request")
    enhanced_response = dspy.OutputField(desc="Enhanced final response")
...
class CognitiveReasoningChain(dspy.Module):
    ...
    def __init__(self, use_hrm_preprocessing: bool = True):
        super().__init__()
        ...
        # HRM preprocessing (optional)
        self.use_hrm = use_hrm_preprocessing
        if self.use_hrm:
            self.hrm_preprocessor = dspy.Predict(HRMPreprocessorSignature)
            self.hrm_enhancer = dspy.ChainOfThought(HRMEnhancedSignature)
    ...
    def forward(self, request: str, context: Optional[dict[str, Any]] = None) -> dict[str, Any]:
        """Execute full cognitive reasoning chain with optional HRM preprocessing."""
        context = context or {}
        hrm_data = {}

        # Phase 0: HRM Preprocessing (if enabled)
        if self.use_hrm:
            # Determine template hint based on request
            template_hint = self._determine_template_hint(request)
            hrm_result = self.hrm_preprocessor(
                request=request,
                template_hint=template_hint
            )
            hrm_data = {
                "hrm_preprocessed": hrm_result.preprocessed,
                "hrm_confidence": float(hrm_result.confidence),
                "hrm_template": hrm_result.template_used,
            }
            # Enhance the request with HRM reasoning
            request = f"{request}\n\nInitial Analysis: {hrm_result.preprocessed}"
        ...
        result = {
            ...
            "metadata": {
                ...
                "reasoning_mode": "cognitive_hrm_enhanced" if self.use_hrm else "cognitive",
                "agent_count": 10,
                "hrm_enabled": self.use_hrm,
            },
        }
        
        # Include HRM data if available
        if hrm_data:
            result["hrm_preprocessing"] = hrm_data
            
        return result

    def _determine_template_hint(self, request: str) -> str:
        """Determine the best reasoning template based on request content."""
        request_lower = request.lower()
        
        if any(word in request_lower for word in ["analyze", "impact", "evaluate", "assess"]):
            return "analytical"
        elif any(word in request_lower for word in ["solve", "fix", "optimize", "debug"]):
            return "problem_solving"
        elif any(word in request_lower for word in ["explain", "describe", "how", "why"]):
            return "explanatory"
        elif any(word in request_lower for word in ["compare", "versus", "vs", "difference"]):
            return "comparative"
        elif any(word in request_lower for word in ["create", "design", "innovate", "imagine"]):
            return "creative"
        else:
