...
# Check if HRM integration is requested
USE_HRM_INTEGRATION = os.environ.get("DSPY_USE_HRM", "true").lower() == "true"

# First try internal LLM relay with HRM if enabled
try:
    relay_lm = get_best_available_lm(use_hrm=USE_HRM_INTEGRATION)
    if relay_lm:
        if USE_HRM_INTEGRATION:
            logger.info("✅ Using HRM-enhanced Internal LLM Relay for hybrid reasoning")
        else:
            logger.info("✅ Using Internal LLM Relay for optimal local model support")
    else:
...
class DSPyServer:
    ...
    async def process_request(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            elif method == "hrm_enhanced_orchestrate":
                # HRM-enhanced orchestration
                result = self._hrm_enhanced_orchestrate(
                    params.get("userRequest"),
                    params.get("context", {}),
                    params.get("template_hint", "analytical")
                )
                return {"requestId": request_id, "success": True, "data": result}
            
            elif method == "hrm_preprocess":
                # Pure HRM preprocessing
                result = self._hrm_preprocess(
                    params.get("userRequest"),
                    params.get("template_hint", "analytical"),
                    params.get("max_steps", 7)
                )
                return {"requestId": request_id, "success": True, "data": result}
    ...
    def _hrm_enhanced_orchestrate(
        self, request: str, context: dict[str, Any], template_hint: str
    ) -> dict[str, Any]:
        """Orchestrate with HRM preprocessing for fast hybrid reasoning."""
        # Create HRM-enhanced cognitive chain
        enhanced_chain = CognitiveReasoningChain(use_hrm_preprocessing=True)
        
        # Add template hint to context
        context["hrm_template_hint"] = template_hint
        
        # Execute enhanced orchestration
        result = enhanced_chain(request=request, context=context)
        
        # Add HRM metadata
        result["hrm_integration"] = {
            "enabled": True,
    ...
    def _hrm_preprocess(self, request: str, template_hint: str, max_steps: int) -> dict[str, Any]:
        """Pure HRM preprocessing without LLM generation."""
        import httpx
        
        try:
            # Call HRM service directly
            client = httpx.Client(timeout=5.0)
            response = client.post(
                "http://localhost:8085/reasoning",
                json={
                    "prompt": request,
                    "template_hint": template_hint,
                    "max_steps": max_steps,
            ...
            if response.status_code == 200:
                ...
                if data.get("success"):
                    return {
                        "success": True,
                        "preprocessing": data.get("result", {}),
                        "inference_time_ms": data.get("inference_time_ms", 0),
                        "template_used": data.get("result", {}).get("template_used", template_hint),
                    }
            
            return {
                "success": False,
                "error": f"HRM service returned {response.status_code}",
            }
            
        except Exception as e:
            logger.error("HRM preprocessing failed: %s", e)
            return {
                "success": False,
                "error": str(e),
            }
