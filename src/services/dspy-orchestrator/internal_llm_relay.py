...
class InternalLLMRelay(LM):
    ...
    def basic_request(self, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            raise Exception(f"Internal LLM relay request failed: {e}")
...
def configure_dspy_with_internal_relay(prefer_local: bool = True) -> Optional[LM]:
    ...
    try:
        ...
        if test_response:
            ...
            print(f"✅ DSPy configured with Internal LLM Relay")
    ...
    if current_lm:
        ...
        if relay_lm.providers:
            ...
            if active_providers and any(p in ["mlx", "lfm2"] for p in active_providers):
                print(f"🔄 Switching to Internal LLM Relay for better local model support")
                dspy.configure(lm=relay_lm)
                return relay_lm
    else:
