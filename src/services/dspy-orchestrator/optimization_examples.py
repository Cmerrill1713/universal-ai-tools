...
class OptimizationExamples:
    ...
    async def example_knowledge_search(self, websocket):
        ...
        if response["success"]:
            ...
            if "metrics" in result:
                ...
                logger.info(
                    f"Success rate: {metrics.get('successful_searches', 0) / max(1, metrics.get('total_searches', 1)) * 100:.1f}%"
                )
        else:
    ...
    async def example_knowledge_evolution(self, websocket):
        ...
        request = {
            ...
            "params": {
                ...
                "data": {
                    ...
                    "context": {"source": "benchmark_results", "reliability": 0.95},
                },
            },
        }
        ...
        if response["success"]:
            ...
            logger.info(
                f"Evolved knowledge: {json.dumps(result.get('evolved_knowledge', {}), indent=2)}"
            )
        else:
    ...
    async def example_knowledge_validation(self, websocket):
        ...
        if response["success"]:
            ...
            if "stats" in result:
                ...
                logger.info(
                    f"Valid items rate: {stats.get('valid_items', 0) / max(1, stats.get('total_validations', 1)) * 100:.1f}%"
                )
        else:
    ...
    async def example_module_optimization(self, websocket):
        ...
        request = {
            ...
            "params": {"examples": training_examples, "iterations": 5},  # Fewer iterations for demo
        }
        ...
        if response["success"]:
            ...
            if "extractor" in metrics:
                ...
                if metrics["extractor"].get("latest"):
                    ...
                    logger.info(
                        f"  Latest extraction time: {latest.get('extraction_time', 0):.3f}s"
                    )
