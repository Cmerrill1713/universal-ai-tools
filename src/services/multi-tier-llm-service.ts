...
export class MultiTierLLMService {
  ...
  constructor() {
    ...
    // Don't start background monitoring in test mode
    ...
    try {
      if (plan.tier === 1) {
        ...
        const ollamaResponse = await ollamaService.generateResponse(
          ...
          this.getOptimalTemperature(classification.domain),
          maxTokens
        );
...
Respond with JSON:
{
  ...
  private getFallbackModels(tier: number): string[] {
    ...
    if (currentTier?.models && currentTier.models.length > 1) {
  ...
  private heuristicClassification(userRequest: string): TaskClassification {
    ...
    if (length < 50) {complexity = 'simple';} else if (length > 200 || complexKeywords.some((k) => userRequest.toLowerCase().includes(k))) {complexity = 'complex';}
