...
export class EnhancedContextService {
  ...
  async searchAcrossDomains(
    ...
    try {
      ...
      this.logger.error('Cross-domain search failed', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async searchKnowledgeGraph(
    ...
    try {
      ...
      this.logger.error('Knowledge graph search failed', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async discoverLearningPaths(
    ...
    try {
      ...
      this.logger.error('Learning path discovery failed', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async getKnowledgeClusters(primaryCluster?: string, complexityLevel?: string) {
    try {
      ...
      this.logger.error('Failed to get knowledge clusters', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async getTechnologyCrossReferences(domain?: string) {
    try {
      ...
      const { data, error } = await query.order('connection_count', { ascending: false });

      if (error) throw error;

      return data || [];
    } catch (error) {
      this.logger.error('Failed to get technology cross-references', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async getMemoryRelationships(
    ...
    try {
      ...
      this.logger.error('Failed to get memory relationships', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async initializeSystem(): Promise<{
    ...
    try {
      ...
      this.logger.error('Failed to initialize enhanced context system', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
   ...
   */
  async getKnowledgeUsagePatterns(
    options?: {
    ...
    try {
      ...
      const { data, error } = await query.order('current_relevance', { ascending: false });

      if (error) throw error;

      return data || [];
    } catch (error) {
      this.logger.error('Failed to get knowledge usage patterns', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async findKnowledgeGaps() {
    try {
      ...
      this.logger.error('Failed to find knowledge gaps', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async buildComprehensiveContext(
    ...
    try {
      ...
      this.logger.error('Failed to build comprehensive context', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
}
...
export class EnhancedContextExamples {
  static async debuggingScenario(service: EnhancedContextService) {
    ...
    const results = await service.searchAcrossDomains(
      'supabase realtime connection error WebSocket',
      {
        intent: 'debugging',
    ...
    const context = await service.buildComprehensiveContext(
      'supabase realtime connection error',
      {
        intent: 'debugging',
        includeRelated: true
      }
  ...
  static async learningScenario(service: EnhancedContextService) {
    ...
    // Get beginner-friendly content first
    const beginnerContent = await service.searchAcrossDomains(
      'GraphQL Supabase basics',
      {
        intent: 'learning',
        maxResults: 10
      }
  ...
  static async optimizationScenario(service: EnhancedContextService) {
    ...
    const optimizations = await service.searchAcrossDomains(
    ...
    const paths = await service.searchKnowledgeGraph(
