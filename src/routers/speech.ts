...
export function SpeechRouter(supabase: SupabaseClient) {
  ...
  router.post('/synthesize/kokoro', validateRequest(VoiceSynthesizeSchema), async (req: any, res) => {
    const startTime = Date.now();
    try {
      const data = req.validatedData;
      
      // Get Kokoro voice profile
      const voiceProfile = kokoroTTS.getVoiceProfile(data.voiceId) || 
        kokoroTTS.getVoiceProfile('athena-sweet')!; // Default to sweet voice
  ...
  router.post('/estimate-duration', async (req: any, res) => {
    try {
      ...
      const estimatedDuration = await speechService.estimateAudioDuration(
        data.text,
        voiceProfile
      );
