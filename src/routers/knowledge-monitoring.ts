 ...
 */
  ...
  try {
    ...
    const patternArray = Array.from(patterns.entries()).map(([key, pattern]) => ({
      id: key,
      ...pattern
    }));
    ...
    res.json({
      ...
      summary: {
        ...
        recentlyActive: patternArray.filter(p =>
          new Date(p.lastSeen).getTime() > Date.now() - 24 * 60 * 60 * 1000
        ).length
      }
    });
  ...
  try {
    ...
    res.json({
      ...
      health: {
        ...
        successRate: statistics.recentCompletions /
    ...
    res.json({
      graph: {
        nodes: Array.from(nodes).map(id => ({
          id,
          label: relationships?.find(r =>
            r.source_knowledge_id === id || r.target_knowledge_id === id
          )?.source?.title || id
        })),
        edges
      },
...
async function getOverviewMetrics(since: Date) {
  const [
    ...
    supabase.from('knowledge_monitoring_alerts')
      .select('id', { count: 'exact' })
      .eq('status', 'active'),
    supabase.from('scraped_knowledge')
      .select('id', { count: 'exact' })
      .gte('scraped_at', since.toISOString()),
    supabase.from('scraped_knowledge')
...
async function getValidationMetrics(since: Date) {
  ...
  const byType = validations?.reduce((acc, val) => {
...
async function getUsageAnalytics(since: Date) {
  ...
  const performanceByAction = Object.entries(actionCounts).reduce((acc, [action, items]) => {
    acc[action] = {
      count: items.length,
