...
const widgetGenerationRequestSchema = z.object({
  ...
  styling: z.enum(['mui', 'tailwind', 'css-modules', 'styled-components']).optional().default('mui')
});
...
export function DSPyWidgetsRouter(supabase: SupabaseClient) {
  ...
  router.post('/generate', async (req: any, res) => {
    try {
      ...
      const generationPromise = dspyWidgetOrchestrator.generateWidget(
        data.description,
        {
          ...data.context,
          functionality: data.functionality,
      ...
      generationPromise
        .then(async (widget) => {
          ...
          await supabase.from('ai_widget_generations').update({
            status: 'completed',
            widget_data: widget,
      ...
      logger.error('Widget generation request error:', LogContext.API, { 
        error: error instanceof Error ? error.message : String(error) 
      });
  ...
  router.post('/improve', async (req: any, res) => {
    try {
      ...
      logger.error('Widget improvement error:', LogContext.API, { 
        error: error instanceof Error ? error.message : String(error) 
      });
  ...
  router.get('/progress/:widgetId', async (req: any, res) => {
    try {
      ...
      // Check if this is a generation request ID
      ...
      logger.error('Progress check error:', LogContext.API, { 
        error: error instanceof Error ? error.message : String(error) 
      });
  ...
  router.get('/', async (req: any, res) => {
    try {
      ...
      logger.error('Widget list error:', LogContext.API, { 
        error: error instanceof Error ? error.message : String(error) 
      });
  ...
  router.get('/:widgetId', async (req: any, res) => {
    try {
      ...
      logger.error('Widget retrieval error:', LogContext.API, { 
        error: error instanceof Error ? error.message : String(error) 
      });
  ...
  router.delete('/:widgetId', async (req: any, res) => {
    try {
      ...
      logger.error('Widget deletion error:', LogContext.API, { 
        error: error instanceof Error ? error.message : String(error) 
      });
  ...
  router.get('/status/active', async (req: any, res) => {
    try {
      ...
      logger.error('Active generations error:', LogContext.API, { 
        error: error instanceof Error ? error.message : String(error) 
      });
  ...
  router.get('/status/health', async (req: any, res) => {
    try {
      ...
      logger.error('Health check error:', LogContext.API, { 
        error: error instanceof Error ? error.message : String(error) 
      });
