...
export function ToolRouter(supabase: SupabaseClient) {
  ...
  router.post('/execute', async (req: any, res) => {
    ...
    try {
      ...
      const { tool_name, parameters } = schema.parse(req.body);
      ...
      switch (tool.implementation_type) {
        ...
        case 'function':
          ...
          const func = new Function('parameters', 'supabase', tool.implementation);
          result = await func(parameters, supabase);
          break;

        case 'api':
          // Make API call
          const response = await fetch(tool.implementation, {
            method: 'POST',
  ...
  router.post('/execute/builtin/:toolName', async (req: any, res) => {
    try {
