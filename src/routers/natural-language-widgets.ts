...
export function NaturalLanguageWidgetsRouter() {
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        logger.info(`Generating widget from ${validatedData.inputType} input`, {
          userId,
        ...
        logger.error('Natural language widget generation error:', error);
        res.status(500).json({
          success: false,
    ...
    async (req: any, res: Response) => {
      try {
        ...
        await fs.unlink(req.file.path).catch(err => 
          logger.error('Failed to delete temp voice file:', err)
        );
  ...
  router.post(
    ...
    [
      ...
      body('editRequest')
        ...
        .trim()
        .notEmpty()
        .withMessage('Edit request is required')
        .isLength({ min: 10, max: 500 })
        .withMessage('Edit request must be between 10 and 500 characters')
    ],
    validateInput,
    async (req: Request, res: Response) => {
      try {
        ...
        // Edit the widget
        const result = await nlGenerator.editWidget(
          id,
          validatedData.editRequest,
          userId
        );
        ...
        logger.error('Widget edit error:', error);
        res.status(500).json({
          success: false,
  ...
  router.get(
    ...
    [
      ...
      query('interactive')
        .optional()
        .isBoolean()
        .withMessage('Interactive must be boolean'),
      query('mockData')
        .optional()
        .isBoolean()
        .withMessage('Mock data must be boolean')
    ],
    validateInput,
    async (req: Request, res: Response) => {
      try {
  ...
  router.post(
    ...
    [
      ...
      body('requests.*.input')
        ...
        .notEmpty()
        .isLength({ min: 10, max: 1000 })
        .withMessage('Each input must be between 10 and 1000 characters')
    ],
    validateInput,
    async (req: Request, res: Response) => {
      try {
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const suggestions = await nlGenerator.getWidgetSuggestions(
          context as string,
          userId
        );
  ...
  router.get('/patterns', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Patterns retrieval error:', error);
      res.status(500).json({
        success: false,
  ...
  router.post(
    ...
    [
      body('widgetId')
        .isUUID()
        .withMessage('Valid widget ID is required'),
      body('rating')
        .isInt({ min: 1, max: 5 })
