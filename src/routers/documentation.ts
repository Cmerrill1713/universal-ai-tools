...
export function DocumentationRouter(supabase: SupabaseClient) {
  ...
  router.get('/integration-patterns', validateRequest(GetIntegrationPatternsSchema), async (req: any, res) => {
    try {
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          frameworks: ['React', 'Vue', 'Angular', 'Next.js', 'Nuxt', 'SvelteKit', 'Flutter', 'React Native']
        },
        metadata: {
          apiVersion: 'v1',
          timestamp: new Date().toISOString()
