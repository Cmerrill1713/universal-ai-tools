...
router.post('/orchestrate', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const // TODO: Refactor nested ternary
      result = await abMCTSOrchestrator.orchestrate(agentContext, executionOptions);
...
router.post('/orchestrate/batch', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const // TODO: Refactor nested ternary
      validation = batchOrchestrationSchema.safeParse(req.body);
    if (!validation.success) {
      return sendError(res, 'VALIDATION_ERROR', 'Invalid request', 400, validation.error.errors);
    }
    ...
    const // TODO: Refactor nested ternary
      results = await abMCTSOrchestrator.orchestrateParallel(contexts, executionOptions);
...
router.get('/metrics', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const // TODO: Refactor nested ternary
      orchestratorStats = abMCTSOrchestrator.getStatistics();
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const // TODO: Refactor nested ternary
        visualization = await abMCTSOrchestrator.getVisualization(orchestrationId!);
...
router.get('/health', async (req: Request, res: Response) => {
  try {
    const // TODO: Refactor nested ternary
      stats = abMCTSOrchestrator.getStatistics();
...
router.get('/auto-pilot/status', async (req: Request, res: Response) => {
  try {
    const // TODO: Refactor nested ternary
      isActive = abMCTSAutoPilot.isActive();
      ...
      res,
      {
        ...
        recommendations: [
          metrics.successfulRequests / metrics.totalRequests < 0.8
            ...
            : null,
          queueStatus.queued > 10 ? 'High queue size - consider increasing batch size' : null,
