  ...
  router.get('/', async (req: Request, res: Response) => {
    try {
      ...
      sendError(
        ...
        'Health check failed',
        500,
        errorinstanceof Error ? errormessage : String(error
      );
    }
  });
...
router.post('/csp-report', (req: Request, res: Response) => {
  try {
    ...
    if (config.server.isProduction) {
      ...
      if (isCritical) {
        logger.error(Critical CSP Violation Detected', LogContext.SECURITY, {
    ...
    logger.error(Error processing CSP report', LogContext.SECURITY, {
      error errorinstanceof Error ? errormessage : String(error,
      body: req.body,
    });
...
router.post('/cors-report', (req: Request, res: Response) => {
  try {
    ...
    logger.error(Error processing CORS report', LogContext.SECURITY, {
      error errorinstanceof Error ? errormessage : String(error,
      body: req.body,
    });
...
router.post('/security-report', (req: Request, res: Response) => {
  try {
    ...
    logger.error(Error processing security report', LogContext.SECURITY, {
      error errorinstanceof Error ? errormessage : String(error,
      body: req.body,
    });
