...
router.get('/agents/optimal', async (req, res) => {'
  try {
    ...
    const // TODO: Refactor nested ternary;
      capabilityList = Array.isArray(capabilities)
...
router.get('/agents/team', async (req, res) => {'
  try {
    const {
      capabilities, // TODO: Refactor nested ternary
      size = 3,
    } = req.query;
...
router.get('/mesh/status', async (req, res) => {'
  try {
    const // TODO: Refactor nested ternary;
      meshStatus = agentRegistry.getMeshStatus();
    const agentConnections = a2aMesh.getAgentConnections();
    ...
    const averageTrustLevel =;
      agentConnections.length > 0
        ? agentConnections.reduce((sum, conn) => sum + conn.trustLevel, 0) / agentConnections.length: 0;
