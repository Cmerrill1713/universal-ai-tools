...
export function BackupRouter(supabase: SupabaseClient) {
  ...
  router.get('/help/aws-sdk', async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        system_message: sdkStatus === 'available' 
      ...
      logger.error('Error checking AWS SDK status:', LogContext.SYSTEM, { error: error instanceof Error ? error.message : String(error) });
      res.status(500).json({ 
  ...
  router.post('/estimate', async (req, res) => {
    try {
      ...
      const totalSize = data?.reduce((sum: number, t: any) => sum + (t.estimated_size || 0), 0) || 0;
