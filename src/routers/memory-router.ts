export function MemoryRouter(supabase: SupabaseClient) {
  ...
  // Enhanced validation middleware with proper errorresponses
  ...
  router.post('/', validateMemoryStore, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      sendError(
        ...
        'An unexpected erroroccurred while storing memory',
        500,
        errormessage
      );
    }
  ...
  router.get('/', async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (error {
        logger.error(Failed to retrieve memories', LogContext.API, { error errormessage });
        return sendError(
          res,
          'MEMORY_STORAGE_ERROR' as ErrorCode,
      ...
      sendError(
        res,
        'INTERNAL_SERVER_ERROR' as ErrorCode,
        'An unexpected erroroccurred while retrieving memories',
        500,
        errormessage
      );
    }
