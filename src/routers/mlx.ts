...
import { codeParametersMiddleware,
...
router.get()
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Apply filters
      if (type && type !== 'all') {'
        models = models.filter((model: unknown) => model.type === type);
      }

      if (available !== undefined) {
        models = models.filter((model: unknown) => model.available === available);
      }
...
router.post()
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Run inference
      const // TODO: Refactor nested ternary;
        result = await mlxService.runInference(inferenceRequest);
...
router.post()
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const fineTuningRequest: //, TODO: Refactor nested ternary;
      FineTuningRequest = req.body;
      ...
      setImmediate(async () => {
        try {
          ...
          const // TODO: Refactor nested ternary;
            jobEntry = fineTuningJobs.get(jobId);
          if (jobEntry) {
...
router.post()
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const {
        modelName, // TODO: Refactor nested ternary
        source = 'huggingface','
        url,
      } = req.body;
      ...
      // In a real implementation, you would: // 1. Download the model from HuggingFace or other source
      // 2. Convert it to MLX format if needed
      // 3. Store it in the models directory
      // 4. Update the models registry

      const // TODO: Refactor nested ternary;
        downloadId = `download_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
