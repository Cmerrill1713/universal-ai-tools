...
export class BaseModel {
   ...
   */
  static fromDict<T extends BaseModel>(this: new () => T, data: Record<string, any>): T {
...
export class MemoryModel extends BaseModel {
  ...
  constructor(content?: string, serviceId?: string, memoryType?: MemoryType, importanceScore?: number) {
    super();
...
export class ModelUtils {
   ...
   */
  static validateWithZod<T>(schema: z.ZodSchema<T>, data: unknown): { success: boolean; data?: T; error?: string } {
    try {
      const result = schema.parse(data);
      return { success: true, data: result };
    } catch (error) {
      return { success: false, error: error instanceof z.ZodError ? error.message : 'Validation failed' };
    }
  }
  ...
  static createMemory(data: Partial<MemoryData>): MemoryModel {
    ...
    if (!validation.success) {
      throw new Error(`Invalid memory data: ${validation.error}`);
    }
  ...
  static createSearchOptions(data: Partial<SearchOptionsData>): SearchOptions {
    ...
    if (!validation.success) {
      throw new Error(`Invalid search options: ${validation.error}`);
    }
  ...
  static createEntityExtraction(data: Partial<EntityExtractionData>): EntityExtraction {
    ...
    if (!validation.success) {
      throw new Error(`Invalid entity extraction data: ${validation.error}`);
    }
  ...
  /**
   * Create concept analysis with validation
   */
  static createConceptAnalysis(data: Partial<ConceptAnalysisData>): ConceptAnalysis {
    ...
    if (!validation.success) {
      throw new Error(`Invalid concept analysis data: ${validation.error}`);
    }
    
    const concept = new ConceptAnalysis(
  ...
  static createSearchResult(data: Partial<SearchResultData>): SearchResult {
    ...
    if (!validation.success) {
      throw new Error(`Invalid search result data: ${validation.error}`);
    }
  ...
  static createEmbeddingConfig(data: Partial<EmbeddingConfigData>): EmbeddingConfig {
    ...
    if (!validation.success) {
      throw new Error(`Invalid embedding config data: ${validation.error}`);
    }
  ...
  static createSystemHealth(data: Partial<SystemHealthData>): SystemHealth {
    ...
    if (!validation.success) {
      throw new Error(`Invalid system health data: ${validation.error}`);
    }
  ...
  static createUserFeedback(data: Partial<UserFeedbackData>): UserFeedback {
    ...
    if (!validation.success) {
      throw new Error(`Invalid user feedback data: ${validation.error}`);
    }
