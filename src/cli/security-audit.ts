...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console.log(`Overall Score: ${getScoreColor(result.overallScore)(`${result.overallScore}/100`)}\n`);
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      // Group by severity
      const bySeverity = vulnerabilities.reduce((acc: Record<string, number>, vuln: any) => {
...
<body>
  ...
  ${audit.vulnerabilities.length === 0 ? '<p class="success">✓ No vulnerabilities found</p>' : `
  <table>
    <tr><th>Severity</th><th>Package</th><th>Vulnerability</th><th>Fix Available</th></tr>
    ${audit.vulnerabilities.map((vuln: any) => `
    <tr>
  ...
  <table>
    ...
    ${audit.securityHeaders.map((header: any) => `
    <tr>
      <td>${header.header}</td>
  ...
  <table>
    ...
    ${audit.apiKeyStatus.map((key: any) => `
    <tr>
      <td>${key.keyName}</td>
