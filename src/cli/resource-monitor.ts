...
program
  ...
  .action(async (options) => {
    await startProfiling(parseInt(options.duration), options.output);
  });
...
async function startDashboard(interval: number) {
  ...
  const update = () => {
    ...
    const tableData = allocations.slice(0, 10).map(a => [
      a.type,
      a.owner,
      a.amount.toString(),
  ...
  try {
    ...
    const report = {
      ...
      allocations: {
        ...
        byType: allocations.reduce((acc, a) => {
...
async function checkHealth(verbose: boolean) {
  ...
  try {
    ...
    const statusColor = health === 'healthy' ? chalk.green : 
    ...
    if (verbose) {
      ...
      // CPU status
      const cpuStatus = usage.cpu.percentage < 60 ? 'OK' : 
                       usage.cpu.percentage < 80 ? 'WARNING' : 'CRITICAL';
      table.push(['CPU', cpuStatus, `${usage.cpu.percentage.toFixed(1)}% (${usage.cpu.cores} cores)`]);

      // Memory status
      table.push(['Memory', memoryCheck.status.toUpperCase(), 
      ...
      table.push(['Connections', connStatus, 
