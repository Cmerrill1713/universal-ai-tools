...
export class UniversalAgentRegistry extends EventEmitter {
    ...
    const cognitiveAgents: AgentDefinition[] = [
      {
        ...
        dependencies: [],
        capabilities: ['task_planning', 'goal_decomposition', 'strategy_design', 'memory_based_optimization'],
        memoryEnabled: true,
        maxLatencyMs: 100,
        retryAttempts: 3,
      },
    ...
    const personalAgents: AgentDefinition[] = [
      {
        ...
        dependencies: ['calendar_agent', 'photo_organizer', 'file_manager', 'code_assistant', 'system_control', 'web_scraper', 'tool_maker'],
        capabilities: ['comprehensive_assistance', 'smart_planning', 'proactive_assistance', 'memory_driven_intelligence'],
        memoryEnabled: true,
        maxLatencyMs: 10000,
        retryAttempts: 3,
      },
   ...
   */
  private async importAgentClass(definition: AgentDefinition): Promise<any> {
    try {
   ...
   */
  async processRequest(agentName: string, context: any): Promise<any> {
  ...
  async unloadUnusedAgents(maxIdleMinutes = 30): Promise<void> {
    ...
    for (const [agentName, lastUsed] of this.agentUsage.entries()) {
      if (this.loadedAgents.has(agentName)) {
        ...
        const definition = this.agentDefinitions.get(agentName);
        if (definition && definition.category !== AgentCategory.CORE && idleMinutes > maxIdleMinutes) {
