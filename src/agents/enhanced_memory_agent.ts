...
interface LearningInsight {
  ...
  timestamp: Date;
  agentName: string;
  category: 'performance' | 'pattern' | 'error' | 'optimization' | 'ethics_improvement' | 'reflection';
  insight: string;
  confidence: number;
...
export abstract class EnhancedMemoryAgent extends BaseAgent {
   ...
   */
  private trackPerformance(context: AgentContext, response: AgentResponse, executionTime: number): void {
   ...
   */
  private async extractLearningInsights(context: AgentContext, response: AgentResponse): Promise<void> {
    ...
    if (response.success && response.confidence > 0.8) {
      await this.addLearningInsight({
        ...
        insight: `Successful execution pattern: ${context.userRequest.substring(0, 100)}`,
  ...
  private async performMemoryDistillation(): Promise<void> {
    ...
    for (const pattern of patterns) {
      await this.storeSemanticMemory(pattern.concept, pattern.knowledge);
    }
  ...
  private extractPatternsFromEpisodes(): any[] {
    ...
    // Extract patterns for each request type
    for (const [type, episodes] of Array.from(requestTypes.entries())) {
      if (episodes.length >= 3) { // Need multiple examples
        patterns.push({
  ...
  private findCommonElements(episodes: any[]): any {
    ...
    return {
      averageExecutionTime: episodes.reduce((sum, ep) => sum + (ep.executionTime || 0), 0) / episodes.length,
  ...
  private calculateMemoryUtilization(): number {
    const totalCapacity = this.memoryConfig.workingMemorySize + 
                         this.memoryConfig.episodicMemoryLimit + 
                         1000; // Semantic + procedural estimate
    
    const totalUsed = this.workingMemory.size + 
  ...
  getMemoryStats(): any {
    return {
      ...
      performance: {
        ...
        averageConfidence: this.performanceHistory.length > 0 
          ? this.performanceHistory.reduce((sum, m) => sum + m.confidenceLevel, 0) / this.performanceHistory.length 
          : 0,
        learningEffectiveness: this.calculateLearningEffectiveness()
      },
      insights: {
