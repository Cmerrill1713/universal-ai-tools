...
export class ResourceManagerAgent extends EnhancedMemoryAgent {
  ...
  async processInput(input: string, context: AgentContext): Promise<PartialAgentResponse> {
    try {
      // Save input for strategy selection
      this.lastInput = input;
      
      // Parse resource management request
      const requestType = this.parseRequestType(input);
      
      switch (requestType) {
        case 'allocate':
          return await this.handleAllocationRequest(input, context);
        case 'release':
          return await this.handleReleaseRequest(input, context);
        case 'optimize':
          return await this.handleOptimizationRequest(input, context);
        case 'status':
          return await this.handleStatusRequest(input, context);
        case 'forecast':
          return await this.handleForecastRequest(input, context);
        default:
          return await this.handleGeneralResourceQuery(input, context);
      }
    } catch (error) {
      return this.handleResourceError(error, input, context);
    }
  }
  ...
  private initializeOptimizationStrategies(): void {
    ...
    this.optimizationStrategies.set('priority-based', {
      name: 'priority-based',
      description: 'Allocate based on request priority',
      applicability: (resources, allocations) => {
  ...
  private parseRequestType(input: string): string {
    if (input.match(/allocate|request|need|require/i)) return 'allocate';
    if (input.match(/release|free|deallocate|return/i)) return 'release';
    if (input.match(/optimize|rebalance|improve/i)) return 'optimize';
    if (input.match(/status|usage|utilization|available/i)) return 'status';
    if (input.match(/forecast|predict|estimate|project/i)) return 'forecast';
    return 'query';
  }

  private async handleAllocationRequest(input: string, context: AgentContext): Promise<PartialAgentResponse> {
    const request = this.parseAllocationRequest(input, context);
  ...
  private parseAllocationRequest(input: string, context: AgentContext): ResourceRequest {
    // Extract resource requirements from input
    const amountMatch = input.match(/(\d+)\s*(\w+)/);
    ...
    if (input.match(/memory|mb|gb|ram/i)) resourceType = 'memory';
    else if (input.match(/storage|disk/i)) resourceType = 'storage';
    else if (input.match(/api|calls/i)) resourceType = 'api_calls';
    else if (input.match(/token/i)) resourceType = 'tokens';
    else if (input.match(/compute|core|cpu/i)) resourceType = 'compute';
    else {
      const typeMatch = input.match(/(?:of\s+)?(\w+)\s+(?:resource|capacity|power)/i);
      resourceType = typeMatch ? typeMatch[1].toLowerCase() : 'compute';
    }
    
    const priorityMatch = input.match(/(?:priority|urgent|high|low)\s*(?:priority)?/i);
    const priority = priorityMatch?.toString().includes('high') || priorityMatch?.toString().includes('urgent') ? 5 : 3;
    
    return {
      consumerId: context.requestId || 'anonymous',
      resourceType,
      amount,
      priority,
  ...
  private extractDuration(input: string): number | undefined {
    const durationMatch = input.match(/(?:for\s+)?(\d+)\s*(millisecond|second|minute|hour|day)/i);
  ...
  private extractConstraints(input: string): ResourceRequest['constraints'] {
    ...
    if (input.includes('exclusive')) {
      constraints.exclusiveAccess = true;
    }
    
    const minMatch = input.match(/at\s+least\s+(\d+)/i);
    if (minMatch) {
      constraints.minAmount = parseInt(minMatch[1]);
    }
    
    const waitMatch = input.match(/within\s+(\d+)\s*(second|minute)/i);
    if (waitMatch) {
    ...
    const fromMatch = input.match(/from\s+([A-Za-z\s]+?)(?:\s|$)/i);
    if (fromMatch) {
  ...
  private findAvailableResource(request: ResourceRequest): Resource | null {
    const candidateResources = Array.from(this.resources.values())
      .filter(r => r.type === request.resourceType || r.type === 'compute') // fallback to compute
      .filter(r => r.available >= (request.constraints?.minAmount || request.amount))
      .sort((a, b) => {
    ...
    if (request.constraints?.preferredResources) {
      for (const resource of candidateResources) {
        const isPreferred = request.constraints.preferredResources.some(pref => 
          resource.name.includes(pref) || resource.id === pref
        );
        if (isPreferred) {
          if (request.constraints?.exclusiveAccess) {
            const hasActiveAllocations = Array.from(this.allocations.values())
              .some(a => a.resourceId === resource.id && a.status === 'active');
  ...
  private createAllocation(request: ResourceRequest, resource: Resource): ResourceAllocation {
    return {
      ...
      metadata: {
        ...
        cost: resource.cost * request.amount
      }
    };
  }
  ...
  private async handleReleaseRequest(input: string, context: AgentContext): Promise<PartialAgentResponse> {
    const allocationId = this.extractAllocationId(input);
    
    if (!allocationId) {
      // Try to find by consumer
      const consumerId = context.requestId || this.extractConsumerId(input);
      ...
      if (!allocation) {
        return {
          ...
          confidence: 0.9,
          reasoning: 'Searched for allocation by ID and consumer. No matching active allocations found'
        };
      }
  ...
  private extractAllocationId(input: string): string | null {
    const match = input.match(/alloc-[\w-]+/);
    return match ? match[0] : null;
  }

  private extractConsumerId(input: string): string {
    const match = input.match(/(?:for|by|from)\s+(\w+)/i);
    return match ? match[1] : 'unknown';
  }

  private findAllocationByConsumer(consumerId: string): ResourceAllocation | null {
    // Try exact match first
    let allocation = Array.from(this.allocations.values())
      .find(a => a.consumerId === consumerId && a.status === 'active');
    
    // If no exact match, try partial match (for "test-agent" in consumer IDs)
    if (!allocation) {
      allocation = Array.from(this.allocations.values())
        .find(a => a.consumerId.includes(consumerId) && a.status === 'active');
    }
  ...
  private releaseAllocation(allocationId: string): PartialAgentResponse {
    ...
    return {
      ...
      confidence: 1.0,
      reasoning: 'Found and validated allocation. Released resources back to pool. Updated resource availability. Processed pending requests'
    };
  }

  private async handleOptimizationRequest(input: string, context: AgentContext): Promise<PartialAgentResponse> {
  ...
  private async handleStatusRequest(input: string, context: AgentContext): Promise<PartialAgentResponse> {
    ...
    return {
      ...
      confidence: 1.0,
      reasoning: 'Collected resource utilization data. Calculated metrics and statistics. Generated comprehensive status report',
      metadata: {
        timestamp: new Date(),
  ...
  private async handleForecastRequest(input: string, context: AgentContext): Promise<PartialAgentResponse> {
    const forecast = this.generateResourceForecast(input);
    ...
    return {
      ...
      confidence: 0.8,
      reasoning: 'Analyzed historical allocation patterns. Projected future resource needs. Identified potential bottlenecks. Generated recommendations',
      metadata: {
        forecastBasis: {
  ...
  private async handleGeneralResourceQuery(input: string, context: AgentContext): Promise<PartialAgentResponse> {
    // Check if this is a utilization query
    if (input.match(/utilization/i)) {
    ...
    const queryAnalysis = this.analyzeResourceQuery(input);
    ...
    const response = await this.generateQueryResponse(queryAnalysis, context);
  ...
  private firstFitOptimization(resources: Resource[], allocations: ResourceAllocation[]): ResourceAllocation[] {
    // Simple first-fit doesn't change existing allocations
    return allocations;
  }

  private bestFitOptimization(resources: Resource[], allocations: ResourceAllocation[]): ResourceAllocation[] {
  ...
  private priorityBasedOptimization(resources: Resource[], allocations: ResourceAllocation[]): ResourceAllocation[] {
    // Ensure high-priority allocations get best resources
    return allocations.sort((a, b) => b.priority - a.priority);
  }

  private costOptimizedAllocation(resources: Resource[], allocations: ResourceAllocation[]): ResourceAllocation[] {
  ...
  private loadBalancedOptimization(resources: Resource[], allocations: ResourceAllocation[]): ResourceAllocation[] {
    ...
    const optimized = [...allocations];
    const avgLoad = Array.from(resourceLoads.values()).reduce((a, b) => a + b, 0) / resourceLoads.size;
  ...
  private generateResourceForecast(input: string): any {
    const horizon = this.extractForecastHorizon(input);
  ...
  private extractForecastHorizon(input: string): number {
    const match = input.match(/(\d+)\s*(hour|day|week)/i);
  ...
  private generateForecastRecommendations(historicalData: any): string[] {
    ...
    if (historicalData.patterns.peakHours.length > 0) {
      recommendations.push(`Schedule non-critical tasks outside peak hours: ${historicalData.patterns.peakHours.join(', ')}`);
    }
  ...
  private async performAutomaticOptimization(): Promise<void> {
    ...
    const strategy = this.selectOptimizationStrategy();
    const optimized = strategy.optimize(
      Array.from(this.resources.values()),
      activeAllocations
    );
  ...
  private updateReleaseMetrics(allocation: ResourceAllocation): void {
    ...
    if (allocation.duration) {
      ...
      typeMetrics.allocationEfficiency = 
        (typeMetrics.allocationEfficiency * 0.9) + (efficiency * 0.1);
    }
  ...
  private createPendingResponse(request: ResourceRequest): PartialAgentResponse {
    return {
      ...
      message: 'Resource request queued - no resources currently available',
      confidence: 0.9,
  ...
  private estimateWaitTime(request: ResourceRequest): number {
    ...
    const similarAllocations = Array.from(this.allocations.values())
      ...
      .filter(a => {
        ...
        return resource && resource.type === request.resourceType;
      });
  ...
  private createAllocationResponse(allocation: ResourceAllocation, resource: Resource): PartialAgentResponse {
    return {
      success: true,
      data: {
  ...
  private analyzeResourceQuery(input: string): any {
    return {
      intent: 'query',
      topics: this.extractQueryTopics(input),
  ...
  private extractQueryTopics(input: string): string[] {
    ...
    if (input.match(/cost|price|expense/i)) topics.push('cost');
    if (input.match(/performance|speed|latency/i)) topics.push('performance');
    if (input.match(/capacity|limit|maximum/i)) topics.push('capacity');
    if (input.match(/trend|pattern|history/i)) topics.push('trends');
    
    return topics;
  }

  private extractTimeframe(input: string): string {
    const match = input.match(/(?:last|past|previous)\s+(\d+)\s*(hour|day|week)/i);
    if (match) {
  ...
  private async generateQueryResponse(analysis: any, context: AgentContext): Promise<PartialAgentResponse> {
    const insights = [];
    ...
    return {
      ...
      confidence: 0.9,
      reasoning: `Analyzed query topics: ${analysis.topics.join(', ')}. Generated ${insights.length} insights. Compiled comprehensive response`
    };
  }
  ...
  private getTotalCapacity(): Record<string, any> {
  ...
  private handleResourceError(error: any, input: string, context: AgentContext): PartialAgentResponse {
    console.error('Resource management error:', error);
    
    return {
      success: false,
      data: null,
      message: `Resource management error: ${error.message}`,
      confidence: 0,
