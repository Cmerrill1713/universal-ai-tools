...
export class DevilsAdvocateAgent extends RealCognitiveAgent {
  ...
  protected async selectCapability(context: AgentContext): Promise<any> {
    ...
    if (request.includes('risk') || request.includes('security') || request.includes('vulnerable')) {
...
My role is to identify problems before they occur, ensuring robust and reliable implementations.`;
      ...
      return `I conducted comprehensive stress testing with ${Object.keys(stress.stressScenarios).length} scenarios. Overall resilience score: ${(stress.overallResilienceScore * 100).toFixed(1)}%.
...
The system shows ${stress.overallResilienceScore > HIGH_CONFIDENCE ? 'strong' : stress.overallResilienceScore > MODERATE_CONFIDENCE ? 'moderate' : 'weak'} resilience against failures.`;
  ...
  private async executeCriticalAnalysis(input: string, context: AgentContext): Promise<CritiqueReport> {
  ...
  private async executeStressTesting(input: string, context: AgentContext): Promise<StressTestReport> {
...
Format as JSON with the structure:
{
  ...
  private performPatternBasedAnalysis(setup: string, context: AgentContext): any {
    ...
    if (setupLower.includes('trading') || setupLower.includes('bot')) {
      ...
      analysis.security_vulnerabilities.push(
  ...
  private async generateCritiqueReport(setup: string, analysis: any): Promise<CritiqueReport> {
    ...
    return {
      ...
      actionableItems: analysis.improvement_suggestions || [],
    };
  }
  ...
  private createStructuredFindings(analysis: any): Finding[] {
    ...
    (analysis.key_weaknesses || []).forEach((weakness: string) => {
      findings.push({
    ...
    (analysis.security_vulnerabilities || []).forEach((vuln: string) => {
      findings.push({
  ...
  private calculateSeverity(analysis: any): 'low' | 'medium' | 'high' | 'critical' {
    const weaknessCount = (analysis.key_weaknesses || []).length;
    const riskCount = (analysis.risk_factors || []).length;
    const securityCount = (analysis.security_vulnerabilities || []).length;
  ...
  private async assessRisks(setup: string, context: AgentContext): Promise<any> {
    const risks = {
  ...
  private async assessOperationalRisks(setup: string): Promise<string[]> {
    return [
  ...
  private calculateOverallRisk(risks: any): 'low' | 'medium' | 'high' | 'critical' {
    ...
    if (totalRisks >= 15) return 'critical';
  ...
  private async generateStressScenarios(setup: string, context: AgentContext): Promise<StressScenario[]> {
  ...
  private async runStressTests(setup: string, scenarios: StressScenario[]): Promise<{ [scenarioName: string]: StressTestResult }> {
  ...
  private async generateStressReport(setup: string, stressResults: { [scenarioName: string]: StressTestResult }): Promise<StressTestReport> {
  ...
  private calculateResilienceScore(stressResults: { [scenarioName: string]: StressTestResult }): number {
  ...
  private createStressFindings(stressResults: { [scenarioName: string]: StressTestResult }): Finding[] {
    const findings: Finding[] = [];
  ...
  private generateStressRecommendations(stressResults: { [scenarioName: string]: StressTestResult }): string[] {
    return [
