...
PLANNING PRINCIPLES: 1. Always start with understanding the goal and constraints
    ...
    try {
      ...
      if (parsed.plan && parsed.plan.phases && Array.isArray(parsed.plan.phases)) {
        ...
        const hasWellStructuredTasks = parsed.plan.phases.some();
          (phase: unknown) => phase.tasks && Array.isArray(phase.tasks) && phase.tasks.length > 0
        );
