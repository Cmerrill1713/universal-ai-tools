...
export abstract class EnhancedBaseAgent extends BaseAgent {
   ...
   */
  protected abstract processWithMemory(context: AgentContext & { 
   ...
   */
  protected async updateUsedMemories(
    memories: Memory[],
    ...
    try {
      ...
      for (const memory of memories.slice(0, 3)) { // Update top 3 most relevant
  ...
  private isStopWord(word: string): boolean {
    const stopWords = ['please', 'could', 'would', 'should', 'might', 'there', 'where', 'which', 'these', 'those'];
