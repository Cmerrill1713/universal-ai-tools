...
export class WebScraperAgent extends BaseAgent {
      ...
      capabilities: [
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              extractType: { type: 'string', enum: ['text', 'html', 'links', 'images', 'structured'] },
        ...
        {
          name: 'api_request',
          description: 'Make intelligent API requests with authentication and error handling',
          inputSchema: {
            type: 'object',
            properties: {
              url: { type: 'string' },
  ...
  protected async process(context: AgentContext & { memoryContext?: any }): Promise<AgentResponse> {
    ...
    try {
      ...
      return {
        success: false,
        data: null,
        reasoning: `Web scraping failed: ${(error as Error).message}`,
        confidence: 0.1,
        latencyMs: Date.now() - startTime,
        agentId: this.config.name,
   ...
   */
  private async parseScrapingIntent(request: string): Promise<any> {
...
Respond with JSON: {
  ...
  private async scrapeWebsite(intent: any): Promise<WebData> {
    ...
    try {
      ...
      switch (extractType) {
        case 'text':
          extractedData = this.extractText(content, selector);
          break;
        case 'html':
          extractedData = this.extractHTML(content, selector);
          break;
        case 'links':
          extractedData = this.extractLinks(content, url);
          break;
        case 'images':
          extractedData = this.extractImages(content, url);
          break;
        case 'structured':
          extractedData = await this.extractStructuredDataFromHTML(content, url);
          break;
        default:
   ...
   */
  private async scrapeWithBrowser(url: string, selector?: string, extractType = 'text'): Promise<WebData> {
   ...
   */
  private async monitorWebsite(intent: any): Promise<any> {
    const {url} = intent;
   ...
   */
  private async makeAPIRequest(intent: any): Promise<any> {
    const {url} = intent;
   ...
   */
  private async extractStructuredDataFromHTML(html: string, url: string): Promise<any> {
...
Return as JSON with clear structure.`;
  ...
  private fallbackScrapingIntentParsing(request: string): any {
  ...
  private async storeAPIResponse(url: string, method: string, data: any, status: number): Promise<void> {
    try {
      await this.supabase
        .from('ai_memories')
        .insert({
  ...
  private async extractStructuredData(intent: any): Promise<any> {
    return { structured: true };
  }

  private async batchScrape(intent: any): Promise<any> {
    return { batch: true };
  }

  private async handleGeneralWebQuery(request: string): Promise<any> {
