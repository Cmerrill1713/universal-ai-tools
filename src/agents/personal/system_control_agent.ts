...
export class SystemControlAgent extends BaseAgent {
  ...
  protected async process(context: AgentContext & { memoryContext?: any }): Promise<AgentResponse> {
    ...
    try {
      ...
      this.logger.error('SystemControlAgent processing error:', error);
      return {
        success: false,
        data: null,
   ...
   */
  private async parseSystemIntent(request: string): Promise<any> {
...
Respond with JSON: {
   ...
   */
  private async getSystemStatus(intent: any): Promise<any> {
   ...
   */
  private async controlApplication(intent: any): Promise<any> {
    const {action} = intent;
   ...
   */
  private async manageSystemPreferences(intent: any): Promise<any> {
  ...
  private async getCPUMetrics(): Promise<SystemMetrics['cpu']> {
    try {
      ...
      this.logger.error('Failed to get CPU metrics:', (error as Error).message);
      return {
        usage: 0,
        cores: os.cpus().length,
        model: 'Unknown'
   ...
   * Launch application
   */
  private async launchApplication(appName: string, args: string[] = []): Promise<any> {
    try {
  ...
  private async focusApplication(appName: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Failed to focus ${appName}:`, error);
      return false;
    }
  }
  ...
  private async hideApplication(appName: string): Promise<boolean> {
    try {
      execSync(`osascript -e 'tell application "System Events" to set visible of process "${appName}" to false'`);
      return true;
    } catch (error) {
  ...
  private async storeSystemMetrics(metrics: SystemMetrics): Promise<void> {
    try {
      ...
      this.logger.error('Failed to store system metrics:', (error as Error).message);
    }
  }
  ...
  private async optimizeSystem(intent: any): Promise<any> {
    return { optimized: true };
  }

  private async monitorResource(intent: any): Promise<any> {
    return { monitoring: true };
  }

  private async manageNetwork(intent: any): Promise<any> {
    return { network: 'managed' };
  }

  private async executeAutomation(intent: any): Promise<any> {
    return { executed: true };
  }

  private async manageBackup(intent: any): Promise<any> {
    return { backup: 'managed' };
  }

  private async handleGeneralSystemQuery(request: string): Promise<any> {
