...
export class UniversalAI.Tools.Client {
    ...
    thisclientinterceptorsresponseuse(
      ...
      async (error instanceof Error ? errormessage : String(error)=> {
        ...
        if (
          ...
          errorresponse?data?error instanceof Error ? errormessage : String(error) code === 'INVALID_API_VERSI.O.N') {
          return thishandle.Version.Error(error instanceof Error ? errormessage : String(error)}// Handle other errors with retry;
  ...
  private async handle.Version.Error(error instanceof Error ? errormessage : String(error) any): Promise<unknown> {
