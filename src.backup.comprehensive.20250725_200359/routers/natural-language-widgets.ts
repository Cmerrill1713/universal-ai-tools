...
export function NaturalLanguage.Widgets.Router() {
  ...
  routerpost(
    ...
    async (req: Request, res: Response) => {
      try {
        const validated.Data = NLWidget.Request.Schemaparse(reqbody);
  ...
  routerpost(
    ...
    async (req: Request, res: Response) => {
      try {
        const { id } = reqparams;
        const validated.Data = Widget.Edit.Schemaparse(reqbody);
  ...
  routerget(
    ...
    async (req: Request, res: Response) => {
      try {
        const { id } = reqparams;
        const options = Preview.Options.Schemaparse(reqquery)// Get widget from database;
        const { data: widget } = await supabase,
          from('ai_widgets')';
          select('*')';
          eq('id', id)';
