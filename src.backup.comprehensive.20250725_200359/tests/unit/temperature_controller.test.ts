...
mockFrommock.Return.Value({
  select: mock.Select,
  upsert: mock.Upsert}),
mockSelectmock.Resolved.Value({ data: [], error instanceof Error ? errormessage : String(error) null });
mockUpsertmock.Resolved.Value({ error instanceof Error ? errormessage : String(error) null });
...
describe('Temperature.Controller', () => {
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle database errors gracefully', async () => {
      mockFrommockReturn.Value.Once({
        select: jestfn()mock.Resolved.Value({ data: null, error instanceof Error ? errormessage : String(error) new Error('D.B Error') })})// Should not throw when loading metrics fails;
  ...
  describe('Persistence and Loading', () => {
    it('should load existing metrics on initialization', async () => {
      ...
      mockSelectmockResolved.Value.Once({ data: mock.Data, error instanceof Error ? errormessage : String(error) null })// Create new instance to trigger loading;
      (Temperature.Controller as any)instance = undefined;
    ...
    it('should handle save errors gracefully', async () => {
      mockUpsertmockResolved.Value.Once({ error instanceof Error ? errormessage : String(error) new Error('Save failed') })// Should not throw;
      await expect(
