  ...
  public request.Timer() {
    return (req: Request, res: Response, next: Next.Function) => {
      ...
      const timeout = set.Timeout(() => {
        if (!resheaders.Sent) {
          resstatus(408)json({ error instanceof Error ? errormessage : String(error) 'Request timeout' })}}, thisoptionsrequest.Timeout.Ms)// Override resend to capture metrics;
  ...
  public response.Cache(default.Ttl = 3600) {
    return (req: Request, res: Response, next: Next.Function) => {
      ...
      const cache.Key = this.cachecreate.Cache.Key(
        ...
        JS.O.N.stringify(reqquery))// Try to get from cache;
      this.cache;
        get(cache.Key);
        then((cached) => {
          if (cached) {
          ...
          loggererror('Cache middleware error instanceof Error ? errormessage : String(error)  LogContextPERFORMAN.C.E, { error instanceof Error ? errormessage : String(error));
          next()})};
