...
export class Cache.Middleware {
  ...
  cache(config: Cache.Config = {}) {
    return async (req: Request, res: Response, next: Next.Function) => {
      ...
      if (reqmethod !== 'G.E.T' && reqmethod !== 'HE.A.D') {
      ...
      const if.Modified.Since = reqheaders['if-modified-since'];
      try {
        // Check cache;
  ...
  private generate.Cache.Key(req: Request, vary.By?: string[]): string {
    ...
    if (vary.By && vary.Bylength > 0) {
      for (const header of vary.By) {
        const value = reqheaders[headerto.Lower.Case()];
        if (value) {
          partspush(`${header}:${value}`)}}}// Add query parameters;
    const query.Keys = Object.keys(reqquery)sort();
  ...
  private intercept.Response(
    ...
    reswrite = function (
      chunk: any,
      encoding.Or.Callback?: Buffer.Encoding | ((error instanceof Error ? errormessage : String(error) Error | null | undefined) => void);
  ...
  invalidation.Middleware() {
    return async (req: Request, res: Response, next: Next.Function) => {
      ...
      if (['PO.S.T', 'P.U.T', 'PAT.C.H', 'DELE.T.E']includes(reqmethod)) {
        reson('finish', async () => {
          if (resstatus.Code >= 200 && resstatus.Code < 300) {
