...
if (importmetaurl === `file://${processargv[1]}`) {
  async function main() {
    const orchestrator = new Hot.Reload.Orchestrator({
      ...
      enable.Self.Healing: !processargvincludes('--no-healing'),
      enable.Performance.Monitoring: !processargvincludes('--no-performance')})// Handle graceful shutdown,
    const shutdown = async () => {
      loggerinfo('🛑 Shutting down.');
      await orchestratorstop();
      processexit(0);
    processon('SIGI.N.T', shutdown);
    processon('SIGTE.R.M', shutdown);
    try {
