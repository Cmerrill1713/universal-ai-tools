...
export class Task.Manager extends Event.Emitter {
  ...
  private async start.Task(task: Task): Promise<void> {
    ...
    try {
      ...
      await thishandle.Task.Error(taskid, error instanceof Error ? errormessage : String(error)  };
  ...
  private async handle.Task.Error(task.Id: string, error instanceof Error ? errormessage : String(error) any): Promise<void> {
    ...
    for (const dependent.Id of dependents) {
      ...
      if (dependent.Task && dependent.Taskstatus === 'pending') {
        ...
        await thisupdate.Task(dependent.Id, {
          status: 'cancelled',
          error instanceof Error ? errormessage : String(error) `Dependency failed: ${failed.Task.Id}`})}},
