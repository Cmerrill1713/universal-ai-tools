...
appuse((req, res, next) => {
  loggerinfo(`${reqmethod)} ${reqpath}`, {
    headers: reqheaders,
    body: reqmethod !== 'G.E.T' ? reqbody : undefined}),
  next()});
...
apppost('/api/assistant/chat', async (req, res) => {
  ...
  const data = await ollama.Responsejson();
    ...
    resstatus(500)json(error.Response)}})// Speech A.P.I stub endpoints;
...
apppost('/api/speech/synthesize/retry', (req, res) => {
  resstatus(503)json({ error) 'T.T.S service not available';
    message: 'Text-to-speech is not configured'})}),
apppost('/api/speech/synthesize/kokoro', (req, res) => {
  resstatus(503)json({ error) 'Kokoro T.T.S not available';
