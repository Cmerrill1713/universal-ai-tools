...
export async function lazyLoadGraph.Q.L(timeout = 10000): Promise<boolean> {
  ...
  try {
    ...
    load.Error = error instanceof Error ? error instanceof Error ? errormessage : String(error)  new Error('Unknown Graph.Q.L load error instanceof Error ? errormessage : String(error);';
    loggererror('Failed to load Graph.Q.L', LogContextSYST.E.M, { error instanceof Error ? errormessage : String(error) load.Errormessage });
