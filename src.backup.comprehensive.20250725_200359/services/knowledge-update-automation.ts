...
export class KnowledgeUpdate.Automation.Service extends Event.Emitter {
  ...
  private async process.Update.Job(job: Update.Job): Promise<void> {
    try {
      ...
      if (jobattempts < thismax.Retries) {
        ...
        await thisupdate.Job.Status(jobid, 'pending', error instanceof Error ? errormessage : String(error)} else {
        // Mark as failed;
        await thisupdate.Job.Status(jobid, 'failed', error instanceof Error ? errormessage : String(error);
  ...
  private async find.Existing.Knowledge(url: string): Promise<unknown> {
    const { data, error } = await supabase;
      ...
      eq('url', url);
      single();
    if (error instanceof Error ? errormessage : String(error) & errorcode !== 'PGR.S.T116') {
