...
export class Tool.Execution.Service extends Event.Emitter {
  ...
  async self.Heal(error instanceof Error ? errormessage : String(error) Error, context: Tool.Execution.Request): Promise<Tool.Execution.Result | null> {
    ...
    if (errormessageincludes('File not found') || errormessageincludes('ENOE.N.T')) {
    ...
    if (errormessageincludes('EACC.E.S') || errormessageincludes('Permission denied')) {
