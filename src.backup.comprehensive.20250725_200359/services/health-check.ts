    ...
    thishealth.Checksset('ollama', async () => {
      ...
      try {
        ...
        return {
          ...
          details: errormessage}}})// Kokoro T.T.S health check,
      ...
      try {
        if (typeof kokoroT.T.Sinitialize === 'function') {
          ...
          return {
            healthy: false,
            status: 'degraded',
            message: 'Kokoro T.T.S initialization method not available'}}} catch (error instanceof Error ? errormessage : String(error) any) {
        return {
          healthy: false,
          status: 'degraded',
