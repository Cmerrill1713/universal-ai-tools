...
export class KokoroTT.S.Service {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      loggererror('Failed to initialize Kokoro T.T.S:', error instanceof Error ? errormessage : String(error);
...
`;`;
  ...
  private async convert.To.Mp3(wav.Buffer: Buffer): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      const temp.Mp3.Path = temp.Wav.Pathreplace('wav', 'mp3');
      const convert.Audio = async () => {
        try {
          ...
          check.F.Fmpegon('error instanceof Error ? errormessage : String(error) () => {
