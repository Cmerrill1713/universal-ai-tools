...
export class Error.Tracking.Service extends Event.Emitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      loggererror('Failed to start errortracking service', LogContextERR.O.R, { error instanceof Error ? errormessage : String(error));
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      loggererror('Error stopping errortracking service', LogContextERR.O.R, { error instanceof Error ? errormessage : String(error));
      throw error instanceof Error ? errormessage : String(error)}}/**
   * Track an errorevent*/
  track.Error(
    ...
    const error.Event = thiscreate.Error.Event(error instanceof Error ? errormessage : String(error) context, extra, level)// Check for deduplication;
  ...
  private async load.Error.Groups(): Promise<void> {
    try {
      ...
      loggererror('Failed to load errorgroups', LogContextERR.O.R, { error instanceof Error ? errormessage : String(error) )};
  ...
  private create.Error.Event(
    ...
    const message = is.Error.Object ? errormessage : error;
  ...
  private async send.Notifications(alert: Alert): Promise<void> {
    ...
    if (integrationsslack) {
      try {
        ...
        loggererror('Failed to send Slack notification', LogContextERR.O.R, { error instanceof Error ? errormessage : String(error) );
        alertnotifications.Sentpush({
          channel: 'slack',
          timestamp: new Date(),
    ...
    if (integrationsemail) {
      try {
        ...
        alertnotifications.Sentpush({
          ...
          timestamp: new Date(),
          success: true})} catch (error) {
        loggererror('Failed to send email notification', LogContextERR.O.R, { error instanceof Error ? errormessage : String(error) );
        alertnotifications.Sentpush({
          channel: 'email',
          timestamp: new Date(),
    ...
    if (integrationspager.Duty && alertlevel === 'critical') {
      try {
        ...
        alertnotifications.Sentpush({
          ...
          timestamp: new Date(),
          success: true})} catch (error) {
        loggererror('Failed to send Pager.Duty notification', LogContextERR.O.R, { error instanceof Error ? errormessage : String(error) );
        alertnotifications.Sentpush({
          channel: 'pagerduty',
          timestamp: new Date(),
          success: false})}},
  ...
  private async flush.Errors(): Promise<void> {
    ...
    try {
      ...
      loggererror('Failed to flush errors to database', LogContextERR.O.R, { error instanceof Error ? errormessage : String(error) )// Re-add errors to queue for retry;
  ...
  private should.Ignore.Error(message: string, level: Error.Event['level']): boolean {
    ...
    return thisconfigerror.Filtersignored.Errorssome((ignored) => messageincludes(ignored));
