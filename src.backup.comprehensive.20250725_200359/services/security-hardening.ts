...
export class Security.Hardening.Service {
  ...
  async checkApi.Key.Rotation(): Promise<ApiKey.Rotation.Status[]> {
    ...
    try {
      ...
      loggererror('Failed to check A.P.I key rotation:', error instanceof Error ? errormessage : String(error)  ;
  ...
  async rotate.Api.Key(key.Name: string): Promise<string> {
    try {
      ...
      loggererror`Failed to rotate A.P.I key ${key.Name}:`, error instanceof Error ? errormessage : String(error);
