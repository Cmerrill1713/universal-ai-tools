...
export class Reranking.Service {
  ...
  async rerank(
    ...
    try {
      ...
      this.loggererror('Reranking failed:', error instanceof Error ? errormessage : String(error)// Fallback to original results;
      return {
  ...
  private async cross.Encoder.Rerank(
    ...
    try {
      ...
      this.loggererror('Cross-encoder reranking failed:', error instanceof Error ? errormessage : String(error);
  ...
  private async llm.Judge.Rerank(
    ...
    try {
      ...
      return reranking.Resultssort((a, b) => bfinal.Score - afinal.Score)} catch (error) {
      this.loggererror('L.L.M judge reranking failed:', error instanceof Error ? errormessage : String(error);
      return thisfallback.Reranking(results)}}/**
  ...
  private async feature.Based.Rerank(
    ...
    try {
      ...
      return reranking.Resultssort((a, b) => bfinal.Score - afinal.Score)} catch (error) {
      this.loggererror('Feature-based reranking failed:', error instanceof Error ? errormessage : String(error);
      return thisfallback.Reranking(results)}}/**
  ...
  private async learned.Rerank(
    ...
    try {
      ...
      return reranking.Resultssort((a, b) => bfinal.Score - afinal.Score)} catch (error) {
      this.loggererror('Learned reranking failed:', error instanceof Error ? errormessage : String(error);
      return thisfallback.Reranking(results)}}/**
  ...
  private async hybrid.Rerank(
    ...
    try {
      ...
      this.loggererror('Hybrid reranking failed:', error instanceof Error ? errormessage : String(error);
  ...
  private analyze.Intent.Alignment(query: string, contentstring): number {
    ...
    for (const keyword of intent.Keywords) {
      if (content.Lowerincludes(keywordto.Lower.Case())) {
        alignment += 0.2};
  ...
  private calculate.Diversity.Score(results: Search.Result[]): number {
    ...
    const unique.Content = new Set(
      resultsmap((r) => thisget.Content.Hash(rcontentsubstring(0, 100))));
