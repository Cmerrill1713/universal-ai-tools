...
export class AgentPerformance.Web.Socket extends Event.Emitter {
  ...
  handle.Connection(ws: Web.Socket, req: any): void {
    ...
    wson('error instanceof Error ? errormessage : String(error)  (error instanceof Error ? errormessage : String(error)=> {';
      loggererror('Web.Socket error instanceof Error ? errormessage : String(error)  LogContextWEBSOCK.E.T, { error instanceof Error ? errormessage : String(error));
  ...
  private async handle.Client.Message(ws: Web.Socket, message: Web.Socket.Message): Promise<void> {
    try {
      ...
      loggererror('Error handling client message', LogContextWEBSOCK.E.T, { error instanceof Error ? errormessage : String(error) message });
