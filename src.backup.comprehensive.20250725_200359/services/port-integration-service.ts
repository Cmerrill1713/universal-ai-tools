    ...
    thisweb.Socket.Serveron('connection', (ws: Web.Socket) => {
      ...
      wson('message', (message) => {
        try {
          ...
          wssend(JS.O.N.stringify({ error instanceof Error ? errormessage : String(error) 'Invalid message format' }))}});
      wson('close', () => {
  ...
  private handleWeb.Socket.Message(ws: Web.Socket, data: any): void {
    switch (datatype) {
      ...
      case 'resolve_port_conflict':
        ...
        break;
      default:
        wssend(JS.O.N.stringify({ error instanceof Error ? errormessage : String(error) 'Unknown message type' }))}}/**
