...
export interface Utility.Score {
  ...
  user.Preference.Boost: number,
  contextual.Relevance.Boost: number,
  final.Score: number,
  explanation: string[],
}
export interface Learning.Insights {
  user.Preferences: {
    preferred.Memory.Types: Array<{ type: string, weight: number }>
    preferred.Agents: Array<{ agent: string, weight: number }>
    timeOf.Day.Patterns: Array<{ hour: number, activity: number }>
    average.Session.Length: number,
}  search.Patterns: {
...
export class Access.Pattern.Learner {
  private supabase: Supabase.Client,
  private logger: Logger,
  private learning.Cache = new Map<string, any>();
  private readonly CACHE_T.T.L = 30 * 60 * 1000// 30 minutes// Learning parameters;
  ...
  async record.Access(
    ...
    try {
      ...
      await thisupdate.Memory.Stats(memory.Id, optionsresponse.Useful)// Invalidate learning cache for this agent;
      thislearning.Cachedelete(`insights:${agent.Name}`)} catch (error) {
      this.loggererror('Failed to record access _pattern', error instanceof Error ? errormessage : String(error)  }}/**
  ...
  async record.User.Feedback(
    ...
    try {
      ...
      await thisupdate.Adaptive.Weights(agent.Name, feedback);
      this.loggerinfo(
    ...
    try {
      ...
      const recent.Accesses =
        access.History?filter(
          (a) => new Date(acreated_at)get.Time() > current.Timeget.Time() - 7 * 24 * 60 * 60 * 1000) || []// Calculate recency boost;
      let recency.Boost = 0;
      if (recent.Accesseslength > 0) {
        const last.Access = new Date(recent.Accesses[0]created_at);
        const days.Since.Access =
          (current.Timeget.Time() - last.Accessget.Time()) / (24 * 60 * 60 * 1000);
        recency.Boost = Math.max(0, (7 - days.Since.Access) / 7) * thisadaptive.Weightsrecency.Weight;
        explanationpush(
          `Recency: +${(recency.Boost * 100)to.Fixed(1)}% (last accessed ${daysSince.Accessto.Fixed(1)} days ago)`)}// Calculate frequency boost,
      let frequency.Boost = 0;
      if (access.Count > 0) {
        const frequency.Score = Math.min(access.Count / 10, 1)// Normalize to 0-1;
        frequency.Boost = frequency.Score * thisadaptive.Weightsfrequency.Weight;
        explanationpush(
          `Frequency: +${(frequency.Boost * 100)to.Fixed(1)}% (${access.Count} accesses)`)}// Calculate user preference boost,
      let user.Preference.Boost = 0;
      ...
      if (access.Count > 0) {
        ...
        user.Preference.Boost = success.Rate * thisadaptiveWeightsuser.Feedback.Weight;
        explanationpush(
          `User preference: +${(user.Preference.Boost * 100)to.Fixed(1)}% (${(success.Rate * 100)to.Fixed(1)}% success rate)`)}// Calculate contextual relevance boost,
      let contextual.Relevance.Boost = 0;
      if (contextual.Factors?urgency) {
        ...
        contextual.Relevance.Boost = (urgency.Multiplier - 1) * 0.1;
        explanationpush(
          `Urgency (${contextual.Factorsurgency}): ${contextual.Relevance.Boost >= 0 ? '+' : ''}${(contextual.Relevance.Boost * 100)to.Fixed(1)}%`)}// Time-of-day patterns;
      const current.Hour = current.Timeget.Hours();
      ...
      if (hourly.Accesses > 0) {
        ...
        contextual.Relevance.Boost += time.Boost;
      ...
      const final.Score = Math.min(
        ...
        Math.max(
          0.0;
          base.Score + recency.Boost + frequency.Boost + user.Preference.Boost + contextual.Relevance.Boost));
      return {
        base.Score;
        recency.Boost;
        frequency.Boost;
    ...
    try {
      const ranked.Results = await Promiseall(
        resultsmap(async (result, index) => {
          const utility.Score = await thiscalculate.Utility.Score(
            resultid;
            agent.Name;
          ...
          return {
            ...
            original.Rank: index,
            utility.Score;
            final.Score: utility.Scorefinal.Score,
      ...
      this.loggererror('Failed to re-rank results:', error instanceof Error ? errormessage : String(error)// Return original results with utility scores of 0;
  ...
  async get.Learning.Insights(agent.Name: string): Promise<Learning.Insights> {
    try {
      ...
      const insights = await thisgenerate.Learning.Insights(agent.Name);
  ...
  private async update.Adaptive.Weights(
    agent.Name: string,
    ...
    try {
      ...
      let response.Useful.Correlation = 0;
      let total.Samples = 0;
      recent.Feedbackfor.Each((item) => {
        if (itemuser_feedback) {
          ...
          if (itemresponse_useful !== null) {
            response.Useful.Correlation += (itemresponse_useful ? 1 : 0) * (satisfaction - 3);
}
          total.Samples++}})// Adjust weights based on correlations;
      if (total.Samples > 0) {
        similarity.Correlation /= total.Samples;
        response.Useful.Correlation /= total.Samples// Gradually adjust weights (learning rate = 0.1);
        const learning.Rate = 0.1;
        if (similarity.Correlation > 0.1) {
          thisadaptive.Weightssimilarity.Weight += learning.Rate * 0.05;
          thisadaptive.Weightsfrequency.Weight -= learning.Rate * 0.025} else if (similarity.Correlation < -0.1) {
          thisadaptive.Weightssimilarity.Weight -= learning.Rate * 0.05;
          thisadaptive.Weightsfrequency.Weight += learning.Rate * 0.025}// Normalize weights to sum to 1;
        const total.Weight = Objectvalues(thisadaptive.Weights)reduce(
          (sum, weight) => sum + weight;
          0);
        Object.keys(thisadaptive.Weights)for.Each((key) => {
          thisadaptive.Weights[key as keyof typeof thisadaptive.Weights] /= total.Weight});
        this.loggerdebug(`Updated adaptive weights for ${agent.Name}:`, thisadaptive.Weights)}} catch (error) {
      this.loggererror('Failed to update adaptive weights:', error instanceof Error ? errormessage : String(error)  }}/**
   * Generate comprehensive learning insights*/
  private async generate.Learning.Insights(agent.Name: string): Promise<Learning.Insights> {
    try {
      ...
      const { data: access.Patterns } = await thissupabase,
        ...
        ``);
        eq('agent_name', agent.Name);
        gte('created_at', thirtyDaysAgotoIS.O.String());
      if (!access.Patterns || access.Patternslength === 0) {
        return thisget.Default.Insights()}// Analyze user preferences;
      ...
      let total.Session.Length = 0;
      let session.Count = 0;
      access.Patternsfor.Each((_pattern => {
        ...
        type.Datacount++
        type.Dataavg.Satisfaction =
          (type.Dataavg.Satisfaction * (type.Datacount - 1) + satisfaction) / type.Datacount;
        ...
        agent.Datacount++
        agent.Dataavg.Satisfaction =
          (agent.Dataavg.Satisfaction * (agent.Datacount - 1) + satisfaction) / agent.Datacount;
        ...
        if (_patterncontextual_factors?session.Length) {
          total.Session.Length += _patterncontextual_factorssession.Length;
      ...
      return {
        user.Preferences: {
          preferred.Memory.Types: Arrayfrom(memory.Typesentries()),
          ...
          preferred.Agents: Arrayfrom(agentsentries()),
            ...
            sort((a, b) => bweight - aweight);
            slice(0, 5);
          timeOf.Day.Patterns: hourly.Activity,
            map((activity, hour) => ({ hour, activity }));
            filter((item) => itemactivity > 0);
          average.Session.Length: session.Count > 0 ? total.Session.Length / session.Count : 0,
}        search.Patterns: {
          ...
          success.Factors: [
            { factor: 'High similarity score', impact: thisadaptive.Weightssimilarity.Weight ,
            { factor: 'Recent access', impact: thisadaptive.Weightsrecency.Weight ,
            { factor: 'Frequent use', impact: thisadaptive.Weightsfrequency.Weight }],
        adaptive.Weights: { .thisadaptive.Weights ,
        recommendations}} catch (error) {
      this.loggererror('Failed to generate learning insights:', error instanceof Error ? errormessage : String(error);
      return thisget.Default.Insights()};

  private async update.Memory.Stats(memory.Id: string, response.Useful?: boolean): Promise<void> {
    try {
      ...
      this.loggerwarn('Failed to update memory stats:', error instanceof Error ? errormessage : String(error)  };

  private get.Default.Insights(): Learning.Insights {
    return {
      user.Preferences: {
  ...
  get.Adaptive.Weights(): typeof thisadaptive.Weights {
    return { .thisadaptive.Weights }}}// Singleton instance;
let global.Access.Learner: Access.Pattern.Learner | null = null,
export function getAccess.Pattern.Learner(
  supabase: Supabase.Client,
  logger: Logger): Access.Pattern.Learner {
  if (!global.Access.Learner) {
    global.Access.Learner = new Access.Pattern.Learner(supabase, logger);
  return global.Access.Learner;

export function resetAccess.Pattern.Learner(): void {
  global.Access.Learner = null;
