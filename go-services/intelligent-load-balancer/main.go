...
func (lb *IntelligentLoadBalancer) makeRoutingDecision(requestType string, context map[string]string, requirements map[string]float64) RoutingDecision {
	...
	if bestService == "" && len(availableServices) > 0 {
		...
		n, err := rand.Int(rand.Reader, big.NewInt(int64(len(availableServices))))
		if err != nil {
			// Fallback to first service if crypto/rand fails
			bestService = availableServices[0]
		} else {
			bestService = availableServices[n.Int64()]
		}
