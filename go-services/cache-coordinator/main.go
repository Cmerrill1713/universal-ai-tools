...
type CacheCoordinator struct {
	...
	resilienceMgr  *shared.ResilienceManager
...
func (cc *CacheCoordinator) Start() error {
	...
	router.GET("/resilience", cc.resilienceMetricsHandler)
	...
	cc.logger.Info("Cache coordinator started successfully",
		zap.String("port", cc.config.HTTP.Port),
		zap.String("redis_addr", cc.config.Redis.Addr))

	// Wait for shutdown signal
	sig := <-shutdownChan
	cc.logger.Info("Shutdown signal received", zap.String("signal", sig.String()))
	...
	if shutdownErr != nil {
		cc.logger.Error("HTTP server shutdown error", zap.Error(shutdownErr))
	}

	// Close Redis connection
	if err := cc.redisClient.Close(); err != nil {
		cc.logger.Error("Redis connection close error", zap.Error(err))
	}
	...
	case <-shutdownComplete:
		cc.logger.Info("Graceful shutdown completed successfully")
	case <-ctx.Done():
		cc.logger.Warn("Shutdown timeout reached, some resources may not have been cleaned up")
	}
...
func (cc *CacheCoordinator) getHandler(c *gin.Context) {
	...
	// Try Redis with resilience patterns
	ctx := context.Background()
	result, err := cc.resilienceMgr.Execute(ctx, func() (interface{}, error) {
		return cc.redisClient.Get(ctx, cc.buildRedisKey(key, service)).Bytes()
	})
...
func (cc *CacheCoordinator) setHandler(c *gin.Context) {
	...
	// Store in Redis with resilience patterns
	ctx := context.Background()
	redisKey := cc.buildRedisKey(key, service)
	_, err := cc.resilienceMgr.Execute(ctx, func() (interface{}, error) {
		return nil, cc.redisClient.Set(ctx, redisKey, value, ttl).Err()
	})
...
func (cc *CacheCoordinator) resilienceMetricsHandler(c *gin.Context) {
	if cc.resilienceMgr == nil {
	...
	metrics := cc.resilienceMgr.GetMetrics()
	circuitStats := cc.resilienceMgr.GetCircuitBreakerStats()

	response := gin.H{
		"service": "cache-coordinator",
		"resilience_metrics": gin.H{
			...
			"bulkhead_rejections":   metrics.BulkheadRejections,
		},
