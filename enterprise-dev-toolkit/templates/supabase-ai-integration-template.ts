...
export class SupabaseAIClient {
  ...
  // Session Management
  async createSession(userId: string, agentId: string, context?: Record<string, any>): Promise<AgentSession> {
    const session = {
      user_id: userId,
      agent_id: agentId,
  ...
  async getAgentAnalytics(agentId: string, timeRange: string = '24h'): Promise<any> {
    const { data, error } = await this.serviceClient?.rpc('get_agent_analytics', {
      agent_id: agentId,
  ...
  async cleanupOldMemories(agentId: string, retentionDays: number = 30): Promise<number> {
    ...
    if (error) throw new Error(`Failed to cleanup memories: ${error.message}`);
    return count || 0;
  }
...
export class SupabaseAIBatch {
  ...
  async execute(concurrency: number = 5): Promise<any[]> {
    const results = [];
