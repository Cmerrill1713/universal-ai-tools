...
class IntegratedCircuitBreakerTests {
  ...
  setupMonitoring() {
    ...
    this.circuitBreaker.on('circuit-open', (data) => {
      ...
      console.log(`🔴 CRITICAL ALERT: Circuit ${data.name} opened at ${alert.timestamp.toISOString()}`);
    });
  ...
  async testMonitoringSystem() {
    ...
    await this.runTest('Metrics Collection and Aggregation', async () => {
      ...
      const metricsValid = allMetrics.every(metric => 
        metric.name && 
    ...
    await this.runTest('Alert System Validation', async () => {
      ...
      const alertStructureValid = this.alertsTriggered.every(alert => 
        alert.timestamp && alert.type && alert.circuit && alert.severity
      );
    ...
    await this.runTest('Health Check System', async () => {
      ...
      const logicallyConsistent = healthCheck.openCircuits.length === 0 ? 
        healthCheck.healthy : 
        !healthCheck.healthy;
  ...
  generateComprehensiveReport() {
    ...
    console.log(`  Critical Alerts (Circuit Open): ${this.alertsTriggered.filter(a => a.type === 'CIRCUIT_OPEN').length}`);
    console.log(`  Recovery Alerts (Circuit Close): ${this.alertsTriggered.filter(a => a.type === 'CIRCUIT_CLOSE').length}`);

    // Recent alerts
    console.log(`\n📋 Recent Alert History:`);
    this.alertsTriggered.slice(-5).forEach(alert => {
      const severity = alert.severity === 'CRITICAL' ? '🔴' : alert.severity === 'WARNING' ? '🟡' : '🟢';
      console.log(`  ${severity} ${alert.type}: ${alert.circuit} at ${alert.timestamp.toLocaleTimeString()}`);
    });
    ...
    console.log(`  Overall Failure Rate: ${totalRequests > 0 ? ((totalFailures/totalRequests)*100).toFixed(1) : 0}%`);
