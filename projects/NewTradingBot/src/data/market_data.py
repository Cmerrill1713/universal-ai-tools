    ...
    async def get_historical_data(
        ...
        try:
            ...
            since = int(
                ...
                1000)  # CCXT expects milliseconds
...
class MarketDataManager:
    ...
    async def get_current_prices(
            self, symbols: List[str]) -> Dict[str, Decimal]:
        """
        ...
        for symbol in symbols:
            for provider in self.providers.values():
                try:
                    ...
                    logger.error(
                        f"Error getting price for {symbol} from {
                            provider.source}: {e}")
    ...
    def unsubscribe_from_data(self,
                              symbol: str,
                              data_type: DataType,
                              callback: Callable[[MarketDataPoint],
                                                 None]) -> None:
                ...
                if symbols:
                    ...
                    for symbol, price in prices.items():
                        data_point = MarketDataPoint(
                            ...
                            source=list(
                                self.providers.keys())[0] if self.providers else DataSource.BINANCE,
                        ...
                        self._notify_subscribers(
                            symbol, DataType.TICKER, data_point)
        ...
        while self.is_running:
            try:
                ...
                for symbol, cache in self.data_cache.items():
                    while cache and current_time - \
                            cache[0].timestamp > timedelta(hours=1):
                        cache.popleft()
    ...
    def _notify_subscribers(
            ...
            symbol: str,
            data_type: DataType,
            data_point: MarketDataPoint) -> None:
    ...
    def get_data_quality_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'active_providers': sum(
                1 for p in self.providers.values() if p.is_connected),
            'cached_symbols': len(
                self.data_cache),
            'total_cached_points': sum(
                len(cache) for cache in self.data_cache.values()),
            'subscribers': len(
                self.subscribers),
            'data_sources': [
                source.value for source in self.providers.keys()],
