...
from .market_data import MarketDataPoint, DataType
...
class DataCleaner:
    ...
    def __init__(self):
        ...
        self.custom_validators: List[Callable[[pd.DataFrame], List[QualityIssue]]] = []
        
        # Default thresholds
        self.price_outlier_std_threshold = 3.0  # Standard deviations for outlier detection
        ...
        self.volatility_threshold = 5.0  # Maximum daily volatility (as z-score)
    ...
    def add_custom_validator(self, validator: Callable[[pd.DataFrame], List[QualityIssue]]) -> None:
        """Add a custom validation function."""
        self.custom_validators.append(validator)
    
    def clean_ohlcv_data(self, df: pd.DataFrame, symbol: str = None) -> Tuple[pd.DataFrame, CleaningResult]:
        """
        ...
        for rule in self.cleaning_rules:
            ...
            relevant_issues = [issue for issue in issues_found if issue.issue_type == rule.issue_type]
            
            if relevant_issues:
                try:
                    cleaned_df, rule_actions = self._apply_cleaning_rule(cleaned_df, rule, relevant_issues)
        ...
        result = CleaningResult(
            ...
            metadata={
                'symbol': symbol,
                'data_reduction_pct': (1 - len(cleaned_df) / original_count) * 100 if original_count > 0 else 0
            }
        )
        
        return cleaned_df, result
    
    def clean_data_points(self, data_points: List[MarketDataPoint]) -> Tuple[List[MarketDataPoint], CleaningResult]:
        """
        ...
        for idx, row in cleaned_df.iterrows():
            ...
            original_point = next((p for p in data_points if p.timestamp == idx), data_points[0])
    ...
    def _check_data_quality(self, df: pd.DataFrame) -> List[QualityIssue]:
        ...
        for column, count in missing_counts.items():
            if count > 0:
                ...
                issues.append(QualityIssue(
                    issue_type=DataQualityIssue.MISSING_VALUES,
                    severity=severity,
                    description=f"Missing values in {column}: {count} ({severity*100:.1f}%)",
        ...
        for col in price_columns:
            if col in df.columns:
                ...
                if invalid_count > 0:
                    issues.append(QualityIssue(
                        issue_type=DataQualityIssue.INVALID_PRICES,
                        severity=min(1.0, invalid_count / len(df)),
        ...
        if all(col in df.columns for col in price_columns):
            ...
            if inconsistent_count > 0:
                issues.append(QualityIssue(
                    issue_type=DataQualityIssue.INCONSISTENT_OHLC,
                    severity=min(1.0, inconsistent_count / len(df)),
        ...
        if 'close' in df.columns:
            outliers = self._detect_outliers(df['close'].dropna(), method='iqr')
            if len(outliers) > 0:
                issues.append(QualityIssue(
                    issue_type=DataQualityIssue.PRICE_OUTLIERS,
        ...
        if 'volume' in df.columns:
            ...
            if len(volume_data) > 0:
                ...
                if len(outliers) > 0:
                    issues.append(QualityIssue(
                        issue_type=DataQualityIssue.VOLUME_OUTLIERS,
                        severity=min(0.5, len(outliers) / len(df)),  # Lower severity for volume
                        description=f"Volume outliers detected: {len(outliers)}",
                        affected_records=len(outliers),
                        suggested_action=CleaningAction.CAP,
                        metadata={'outlier_indices': outliers.tolist()}
                    ))
        ...
        if 'close' in df.columns and len(df) > 1:
            ...
            if len(returns) > 10:
                ...
                if extreme_count > 0:
                    issues.append(QualityIssue(
                        issue_type=DataQualityIssue.EXTREME_VOLATILITY,
                        severity=min(0.8, extreme_count / len(returns)),
                        description=f"Extreme volatility periods: {extreme_count}",
                        affected_records=extreme_count,
                        suggested_action=CleaningAction.FLAG,
                        metadata={'avg_volatility': volatility}
                    ))
        
        # Check for time gaps
        if hasattr(df.index, 'to_series'):
            time_diffs = df.index.to_series().diff()
            large_gaps = time_diffs > timedelta(minutes=self.max_time_gap_minutes)
    ...
    def _apply_cleaning_rule(self, df: pd.DataFrame, rule: CleaningRule, 
                           issues: List[QualityIssue]) -> Tuple[pd.DataFrame, List[str]]:
            ...
            if rule.action == CleaningAction.REMOVE:
                ...
                if rule.issue_type == DataQualityIssue.INVALID_PRICES:
                    ...
                    for col in price_cols:
                        if col in df.columns:
                            min_price = rule.parameters.get('min_price', 0.0001)
                            max_price = rule.parameters.get('max_price', 1000000)
                ...
                if removed_count > 0:
                    actions_taken.append(f"Removed {removed_count} records ({rule.rule_name})")
            
            elif rule.action == CleaningAction.CAP:
                if rule.issue_type in [DataQualityIssue.PRICE_OUTLIERS, DataQualityIssue.VOLUME_OUTLIERS]:
                    # Cap outliers
                    ...
                    if column in df.columns:
                        ...
                        if capped_count > 0:
                            actions_taken.append(f"Capped {capped_count} outliers in {column}")
                ...
                if rule.issue_type == DataQualityIssue.MISSING_VALUES:
                    ...
                    if original_nulls > remaining_nulls:
                        actions_taken.append(f"Interpolated {original_nulls - remaining_nulls} missing values")
            
            elif rule.action == CleaningAction.SMOOTH:
                # Smooth extreme values (example implementation)
                if 'close' in df.columns:
                    window = rule.parameters.get('window', 3)
                    df['close'] = df['close'].rolling(window=window, center=True).mean()
                    actions_taken.append(f"Applied smoothing with window {window}")
            
            elif rule.action == CleaningAction.FLAG:
                # Flag issues without modifying data
                issue_count = sum(issue.affected_records for issue in issues if issue.issue_type == rule.issue_type)
                actions_taken.append(f"Flagged {issue_count} records for {rule.issue_type.value}")
    ...
    def _cap_outliers(self, df: pd.DataFrame, column: str, method: str = 'iqr') -> int:
    ...
    def generate_quality_report(self, df: pd.DataFrame, result: CleaningResult) -> Dict[str, Any]:
        ...
        report = {
            'summary': {
                ...
                'removal_percentage': ((result.original_count - result.cleaned_count) / result.original_count * 100) if result.original_count > 0 else 0,
        ...
        if result.quality_score < 0.7:
            report['recommendations'].append("Data quality is below acceptable threshold. Consider additional cleaning or data source validation.")
        
        if result.original_count - result.cleaned_count > result.original_count * 0.2:
            report['recommendations'].append("More than 20% of data was removed. Review data source quality.")
        
        high_severity_issues = [issue for issue in result.issues_found if issue.severity > 0.5]
        if high_severity_issues:
            report['recommendations'].append("High severity issues detected. Consider investigating data source.")
