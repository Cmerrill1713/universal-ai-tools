...
class StreamBuffer:
    ...
    def get_data(
            self,
            since: datetime = None,
            limit: int = None) -> List[MarketDataPoint]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            prices = [float(point.close_price)
                      for point in self.data if point.close_price]
            volumes = [float(point.volume)
                       for point in self.data if point.volume]
...
class StreamProcessor:
    ...
    def add_data_point(self, data_point: MarketDataPoint) -> None:
        ...
        if symbol not in self.buffers:
            if len(self.buffers) >= self.max_symbols:
                ...
                oldest_symbol = min(self.buffers.keys(), key=lambda s: self.buffers[s].get_latest(
                ).timestamp if self.buffers[s].get_latest() else datetime.min)
    ...
    def remove_filter(self, filter_type: FilterType) -> None:
        ...
        self.filters = [
            f for f in self.filters if f.filter_type != filter_type]
    ...
    def add_event_handler(
            self, handler: Callable[[StreamEvent], None]) -> None:
    ...
    def _apply_single_filter(
            ...
            stream_filter: StreamFilter) -> bool:
        ...
        try:
            if stream_filter.filter_type == FilterType.PRICE_RANGE:
                if data_point.close_price:
                    ...
                    max_price = stream_filter.parameters.get(
                        'max_price', float('inf'))
                ...
                now = datetime.utcnow()
                max_age = timedelta(
                    seconds=stream_filter.parameters.get(
    ...
    async def _process_aggregation(
            self, aggregation: StreamAggregation) -> None:
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if aggregation.aggregation_type in ['price_mean', 'price_std']:
                    values = [float(point.close_price)
                              for point in data_points if point.close_price]
                elif aggregation.aggregation_type in ['volume_sum', 'volume_mean']:
                    values = [float(point.volume)
                              for point in data_points if point.volume]
                else:
    ...
    async def _detect_patterns_for_symbol(self, symbol: str) -> None:
        ...
        for pattern_name, detector_func in self.pattern_detectors.items():
            try:
                ...
                logger.error(
                    f"Error in pattern detector {pattern_name} for {symbol}: {e}")

    def _detect_volume_spike(
            self,
            symbol: str,
            buffer: StreamBuffer) -> Optional[StreamEvent]:
        ...
        volumes = [float(point.volume)
                   for point in recent_data if point.volume]
        if len(volumes) < 10:
            return None
    ...
    def _detect_price_breakout(
        ...
        prices = [float(point.close_price)
                  for point in recent_data if point.close_price]
        if len(prices) < 20:
            return None

        # Calculate support/resistance levels
        highs = [float(point.high_price)
                 for point in recent_data[-20:-1] if point.high_price]
        lows = [float(point.low_price)
                for point in recent_data[-20:-1] if point.low_price]
    ...
    def _detect_volatility_change(
        ...
        # Annualized (assuming 1-min data)
        recent_volatility = np.std(returns[-20:]) * np.sqrt(1440)
        ...
        volatility_ratio = recent_volatility / \
            historical_volatility if historical_volatility > 0 else 1
