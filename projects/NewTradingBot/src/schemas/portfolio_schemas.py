...
class PositionCreateSchema(BaseModel):
    ...
    side: TradeSideSchema = Field(..., description="Position side (long/short)")
...
class PortfolioCreateSchema(BaseModel):
    ...
    base_currency: str = Field(default="USDT", min_length=1, description="Base currency for valuation")
    initial_balance: Optional[Decimal] = Field(None, gt=0, description="Initial balance")
...
class AllocationRequestSchema(BaseModel):
    ...
    max_slippage: float = Field(default=0.5, ge=0, le=10, description="Maximum allowed slippage %")
...
class RiskMetricsSchema(BaseModel):
    ...
    expected_shortfall: Optional[Decimal] = Field(None, description="Expected Shortfall (CVaR)")
...
class PortfolioOptimizationRequestSchema(BaseModel):
    ...
    optimization_type: str = Field(..., description="Type of optimization (mean_variance, risk_parity, etc.)")
    risk_tolerance: float = Field(default=0.5, ge=0, le=1, description="Risk tolerance (0=conservative, 1=aggressive)")
    target_return: Optional[float] = Field(None, gt=0, description="Target return percentage")
    max_weight_per_asset: float = Field(default=0.3, gt=0, le=1, description="Maximum weight per asset")
    min_weight_per_asset: float = Field(default=0.01, ge=0, lt=1, description="Minimum weight per asset")
    excluded_symbols: List[str] = Field(default_factory=list, description="Symbols to exclude")
    included_symbols: Optional[List[str]] = Field(None, description="Only consider these symbols")
    rebalance_frequency: str = Field(default="weekly", description="Rebalancing frequency")
    
    @validator('optimization_type')
    def validate_optimization_type(cls, v):
        """Validate optimization type."""
        valid_types = ['mean_variance', 'risk_parity', 'min_variance', 'max_sharpe', 'equal_weight']
