...
class OrderCreateSchema(BaseModel):
    ...
    time_in_force: TimeInForceSchema = Field(
        default=TimeInForceSchema.GTC,
        description="Time in force")
    client_order_id: Optional[str] = Field(
        None, max_length=100, description="Client-provided order ID")
    strategy_id: Optional[str] = Field(
        None, description="Strategy that created this order")
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional order metadata")
    expires_at: Optional[datetime] = Field(
        None, description="Order expiration time (for GTD orders)")
    ...
    def validate_order_requirements(self):
        ...
        # Price requirements
        if order_type in [
                OrderTypeSchema.LIMIT,
                OrderTypeSchema.STOP_LIMIT] and not price:
            raise ValueError(f'{order_type} orders require a price')

        if order_type in [
                OrderTypeSchema.STOP,
                OrderTypeSchema.STOP_LIMIT,
                OrderTypeSchema.TRAILING_STOP] and not stop_price:
...
class OrderFilterSchema(BaseModel):
    ...
    active_only: bool = Field(default=False,
                              description="Filter only active orders")
...
class OrderBatchCreateSchema(BaseModel):
    ...
    def validate_order_count(cls, v):
        ...
        if len(v) > 50:
            raise ValueError(
                'Cannot create more than 50 orders in a single batch')
        return v
        ...
        if entry_price and take_profit_price and stop_loss_price:
            if side == OrderSideSchema.BUY:
                if take_profit_price <= entry_price:
                    raise ValueError(
                        'Take profit price must be higher than entry price for buy orders')
                if stop_loss_price >= entry_price:
                    raise ValueError(
                        'Stop loss price must be lower than entry price for buy orders')
            else:  # SELL
                if take_profit_price >= entry_price:
                    raise ValueError(
                        'Take profit price must be lower than entry price for sell orders')
                if stop_loss_price <= entry_price:
                    raise ValueError(
                        'Stop loss price must be higher than entry price for sell orders')
...
class AlgoOrderSchema(BaseModel):
    ...
    symbol: str = Field(..., min_length=1)
    order_type: OrderTypeSchema = Field(...,
                                        description="Must be TWAP or VWAP")
    side: OrderSideSchema
    amount: Decimal = Field(..., gt=0)
    ...
    slice_count: int = Field(
        ...
        ge=2,
        le=100,
        description="Number of order slices")
    max_participation_rate: float = Field(
        default=0.1, gt=0, le=1, description="Max % of volume to consume")
    start_time: Optional[datetime] = Field(
        None, description="When to start execution")
    price_limit: Optional[Decimal] = Field(
        None, gt=0, description="Maximum price limit")
    strategy_id: Optional[str] = None
