...
class TradeCreateSchema(BaseModel):
    ...
    trade_type: TradeTypeSchema = Field(..., description="Type of trade (buy/sell)")
    ...
    strategy_id: Optional[str] = Field(None, description="Strategy that generated this trade")
    ...
    def validate_symbol(cls, v):
        ...
        if not base or not quote:
            raise ValueError('Both base and quote currencies must be specified')
        return v.upper()
...
class TradeBatchCreateSchema(BaseModel):
    ...
    def validate_trades_limit(cls, v):
        ...
        if len(v) > 100:
            raise ValueError('Cannot create more than 100 trades in a single batch')
        return v
