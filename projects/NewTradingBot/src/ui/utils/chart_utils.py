...
class ChartBuilder:
    ...
    def create_line_chart(
        ...
        colors = ['#1E88E5', '#FFC107', '#4CAF50', '#F44336', '#9C27B0', '#00BCD4']
    ...
    @staticmethod
    def _add_indicator(fig: go.Figure, df: pd.DataFrame, indicator: Dict[str, Any]):
    ...
    def prepare_ohlcv_data(raw_data: List[Dict]) -> pd.DataFrame:
        ...
        for col in required_columns:
            if col not in df.columns:
                if col == 'time':
                    df['time'] = pd.date_range(start='now', periods=len(df), freq='1min')
                else:
                    df[col] = 0
