...
class DataFormatter:
    ...
    @staticmethod
    def format_number(value: float, decimals: int = 2, use_suffix: bool = True) -> str:
    ...
    def highlight_max(s):
        ...
        is_max = s == s.max()
        return ['background-color: rgba(76, 175, 80, 0.3)' if v else '' for v in is_max]
    
    @staticmethod
    def highlight_min(s):
        """Highlight minimum value in a series"""
        is_min = s == s.min()
        return ['background-color: rgba(244, 67, 54, 0.3)' if v else '' for v in is_min]
...
class TableFormatter:
    ...
    def format_positions_table(positions: List[dict]) -> pd.DataFrame:
        ...
        if 'current_price' in df.columns and 'entry_price' in df.columns:
            df['pnl'] = (df['current_price'] - df['entry_price']) * df.get('quantity', 1)
            df['pnl_percentage'] = ((df['current_price'] / df['entry_price']) - 1) * 100
...
class MetricsFormatter:
    ...
    def format_portfolio_metrics(metrics: dict) -> dict:
        ...
        currency_fields = ['total_value', 'cash_balance', 'positions_value', 
                          'daily_pnl', 'total_pnl', 'realized_pnl', 'unrealized_pnl']
        
        for field in currency_fields:
            if field in metrics:
                formatted[field] = DataFormatter.format_currency(metrics[field])
        
        # Format percentage values
        percentage_fields = ['daily_return', 'total_return', 'win_rate', 'sharpe_ratio']
        
        for field in percentage_fields:
            if field in metrics:
                formatted[field] = DataFormatter.format_percentage(metrics[field])
        
        # Format number values
        number_fields = ['total_trades', 'winning_trades', 'losing_trades', 'open_positions']
        
        for field in number_fields:
            if field in metrics:
                formatted[field] = DataFormatter.format_number(metrics[field], decimals=0, use_suffix=False)
    ...
    def format_risk_metrics(metrics: dict) -> dict:
        ...
        if 'value_at_risk' in metrics:
            formatted['value_at_risk'] = DataFormatter.format_currency(metrics['value_at_risk'])
        
        if 'max_drawdown' in metrics:
            formatted['max_drawdown'] = DataFormatter.format_percentage(metrics['max_drawdown'])
        
        if 'exposure' in metrics:
            formatted['exposure'] = DataFormatter.format_percentage(metrics['exposure'])
