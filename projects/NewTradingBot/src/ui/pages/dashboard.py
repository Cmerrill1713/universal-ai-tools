...
from ..components.trading_chart import TradingChart, MiniChart
...
class DashboardPage:
    ...
    def _render_header(self):
        ...
        with col1:
            ...
            st.caption(f"Connected to: {st.session_state.dashboard['selected_exchange'].upper()} | "
        ...
        with st.container():
            ...
            with col1:
                ...
                st.session_state.dashboard['selected_symbol'] = st.selectbox(
                    "Symbol",
                    symbols,
                    index=symbols.index(st.session_state.dashboard['selected_symbol']),
            ...
            with col2:
                ...
                st.session_state.dashboard['selected_exchange'] = st.selectbox(
                    "Exchange",
                    exchanges,
                    index=exchanges.index(st.session_state.dashboard['selected_exchange']),
                    label_visibility="collapsed"
                )
    ...
    def _render_order_section(self):
        ...
        with st.container():
            ...
            current_price = self._get_current_price(st.session_state.dashboard['selected_symbol'])
    ...
    def _render_watchlist(self):
        ...
        with st.expander("👁️ Watchlist", expanded=True):
            ...
            with col2:
                if st.button("Add", use_container_width=True, key="add_watchlist_btn"):
                    if new_symbol and new_symbol not in st.session_state.dashboard['watchlist']:
                        st.session_state.dashboard['watchlist'].append(new_symbol)
            ...
            for symbol in st.session_state.dashboard['watchlist']:
                ...
                with col3:
                    color = "green" if change >= 0 else "red"
                    st.markdown(f"<span style='color: {color}'>{change:+.2f}%</span>", 
                              unsafe_allow_html=True)
                
                with col4:
                    if st.button("Trade", key=f"trade_{symbol}", use_container_width=True):
    ...
    def _render_active_orders(self):
        ...
        with st.expander("📋 Active Orders", expanded=True):
            ...
            if orders:
                ...
                with col3:
                    if st.button("Refresh", use_container_width=True, key="refresh_orders"):
                        st.rerun()
            else:
            ...
            if strategies:
                for strategy in strategies:
                    ...
                    with col1:
                        ...
                        st.caption(f"{strategy['symbol']} | {strategy['timeframe']}")
                    ...
                    with col3:
                        if strategy['status'] == 'Running':
                            if st.button("Stop", key=f"stop_{strategy['name']}", use_container_width=True):
                                st.info(f"Stopping {strategy['name']}")
                        else:
                            if st.button("Start", key=f"start_{strategy['name']}", use_container_width=True):
    ...
    def _render_bottom_section(self):
        ...
        tabs = st.tabs(["📊 Portfolio", "📈 Performance", "📜 Recent Trades", "📰 News"])
        ...
        with tabs[1]:
            # Performance chart
            performance_data = [100 + i * 0.5 + np.random.randn() * 2 for i in range(30)]
            MiniChart.render_sparkline(performance_data, "30-Day Performance", "#1E88E5")
    ...
    def _handle_order_submission(self, order_data: Dict[str, Any]):
        ...
        st.session_state.dashboard['notifications'].append({
            'type': 'success',
            'message': f"Order placed: {order_data['side']} {order_data['quantity']} {order_data['symbol']} at {order_data['price']}"
        })
    ...
    def _get_market_news(self) -> List[Dict[str, Any]]:
        ...
        return [
            {
                'title': 'Bitcoin Breaks $47,500 Resistance',
                'source': 'CoinDesk',
                'time': '1 hour ago',
