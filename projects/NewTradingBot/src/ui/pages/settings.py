...
class SettingsPage:
    ...
    def _render_exchange_settings(self):
        ...
        with col2:
            api_secret = st.text_input(
                "API Secret",
                value=st.session_state.settings['exchange'].get('api_secret', ''),
                type="password",
        ...
        with col1:
            st.session_state.settings['exchange']['testnet'] = st.checkbox(
                "Use Testnet",
                value=st.session_state.settings['exchange'].get('testnet', False),
                help="Use testnet for paper trading"
            )
            ...
            st.session_state.settings['exchange']['rate_limit'] = st.number_input(
                ...
                min_value=1,
                max_value=100,
                value=st.session_state.settings['exchange'].get('rate_limit', 10)
            )
        ...
        with col2:
            ...
            st.session_state.settings['exchange']['retry_count'] = st.number_input(
                ...
                min_value=0,
                max_value=10,
                value=st.session_state.settings['exchange'].get('retry_count', 3)
            )
    ...
    def _render_trading_settings(self):
        ...
        with col1:
            st.session_state.settings['trading']['max_position_size'] = st.slider(
                ...
                min_value=1,
                max_value=100,
                value=st.session_state.settings['trading'].get('max_position_size', 10),
            ...
            st.session_state.settings['trading']['max_daily_loss'] = st.slider(
                ...
                min_value=1,
                max_value=50,
                value=st.session_state.settings['trading'].get('max_daily_loss', 5),
        ...
        with col2:
            st.session_state.settings['trading']['default_stop_loss'] = st.slider(
                ...
                min_value=0.1,
                max_value=10.0,
                value=st.session_state.settings['trading'].get('default_stop_loss', 2.0),
                step=0.1,
            ...
            st.session_state.settings['trading']['default_take_profit'] = st.slider(
                ...
                min_value=0.1,
                max_value=20.0,
                value=st.session_state.settings['trading'].get('default_take_profit', 5.0),
                step=0.1,
        ...
        with col1:
            st.session_state.settings['trading']['default_order_type'] = st.selectbox(
                ...
                index=['Market', 'Limit', 'Stop', 'Stop Limit'].index(
                    st.session_state.settings['trading'].get('default_order_type', 'Limit')
                )
            )
            ...
            st.session_state.settings['trading']['time_in_force'] = st.selectbox(
                ...
                ['GTC', 'IOC', 'FOK', 'Day'],
                index=['GTC', 'IOC', 'FOK', 'Day'].index(
                    st.session_state.settings['trading'].get('time_in_force', 'GTC')
                )
            )
        ...
        with col2:
            st.session_state.settings['trading']['post_only'] = st.checkbox(
                "Post Only Orders",
                value=st.session_state.settings['trading'].get('post_only', False),
                help="Ensure orders are maker orders"
            )
            
            st.session_state.settings['trading']['reduce_only'] = st.checkbox(
                "Reduce Only Mode",
                value=st.session_state.settings['trading'].get('reduce_only', False),
                help="Only allow orders that reduce position size"
            )
    ...
    def _render_strategy_settings(self):
        ...
        strategies = ['Mean Reversion', 'Momentum', 'Arbitrage', 'Market Making', 'Grid Trading']
    ...
    def _render_notification_settings(self):
        ...
        with col1:
            st.session_state.settings['notifications']['email'] = st.checkbox(
                "Email Notifications",
                value=st.session_state.settings['notifications'].get('email', False)
            )
            
            if st.session_state.settings['notifications']['email']:
                st.text_input(
                    "Email Address",
                    value=st.session_state.settings['notifications'].get('email_address', ''),
                    key="email_address"
                )
            
            st.session_state.settings['notifications']['telegram'] = st.checkbox(
                "Telegram Notifications",
                value=st.session_state.settings['notifications'].get('telegram', False)
            )
            
            if st.session_state.settings['notifications']['telegram']:
                st.text_input(
                    "Telegram Bot Token",
                    value=st.session_state.settings['notifications'].get('telegram_token', ''),
                    type="password",
                    key="telegram_token"
                )
        ...
        with col2:
            st.session_state.settings['notifications']['discord'] = st.checkbox(
                ...
                value=st.session_state.settings['notifications'].get('discord', False)
            )
            
            if st.session_state.settings['notifications']['discord']:
                st.text_input(
                    "Discord Webhook URL",
                    value=st.session_state.settings['notifications'].get('discord_webhook', ''),
                    type="password",
                    key="discord_webhook"
                )
            ...
            st.session_state.settings['notifications']['push'] = st.checkbox(
                "Push Notifications",
                value=st.session_state.settings['notifications'].get('push', True)
            )
        ...
        selected_types = st.multiselect(
            ...
            default=[notification_types[k] for k in st.session_state.settings['notifications'].get('types', [])]
        )
    ...
    def _render_appearance_settings(self):
        ...
        with col1:
            st.session_state.settings['appearance']['theme'] = st.selectbox(
                ...
                index=['Dark', 'Light', 'Auto'].index(
                    st.session_state.settings['appearance'].get('theme', 'Dark')
                )
            )
            ...
            st.session_state.settings['appearance']['chart_style'] = st.selectbox(
                ...
                index=['Candlestick', 'Line', 'Area', 'Bars'].index(
                    st.session_state.settings['appearance'].get('chart_style', 'Candlestick')
                )
            )
        ...
        with col2:
            st.session_state.settings['appearance']['decimal_places'] = st.number_input(
                ...
                min_value=0,
                max_value=8,
                value=st.session_state.settings['appearance'].get('decimal_places', 2)
            )
        ...
        with col1:
            st.color_picker(
                "Profit Color",
                value=st.session_state.settings['appearance'].get('profit_color', '#4CAF50'),
                key="profit_color"
            )
        ...
        with col2:
            st.color_picker(
                "Loss Color",
                value=st.session_state.settings['appearance'].get('loss_color', '#F44336'),
                key="loss_color"
            )
        ...
        with col3:
            st.color_picker(
                "Accent Color",
                value=st.session_state.settings['appearance'].get('accent_color', '#1E88E5'),
                key="accent_color"
            )
    ...
    def _render_security_settings(self):
        ...
        st.session_state.settings['security']['2fa_enabled'] = st.checkbox(
            "Enable 2FA",
            value=st.session_state.settings['security'].get('2fa_enabled', False),
        ...
        if st.session_state.settings['security']['2fa_enabled']:
            ...
            with col1:
                st.text_input(
                    "Phone Number",
                    value=st.session_state.settings['security'].get('phone', ''),
                    help="Phone number for SMS authentication"
                )
        ...
        with col1:
            st.session_state.settings['security']['session_timeout'] = st.number_input(
                ...
                min_value=5,
                max_value=1440,
                value=st.session_state.settings['security'].get('session_timeout', 60),
        ...
        with col2:
            st.session_state.settings['security']['ip_whitelist'] = st.checkbox(
                "IP Whitelist",
                value=st.session_state.settings['security'].get('ip_whitelist', False),
                help="Only allow access from whitelisted IPs"
            )
        ...
        st.session_state.settings['security']['api_key_expiry'] = st.number_input(
            ...
            min_value=1,
            max_value=365,
            value=st.session_state.settings['security'].get('api_key_expiry', 90),
            help="Automatically expire API keys after specified days"
        )
        
        st.session_state.settings['security']['encrypt_keys'] = st.checkbox(
            "Encrypt API Keys",
            value=st.session_state.settings['security'].get('encrypt_keys', True),
            help="Store API keys in encrypted format"
        )
    ...
    def _render_api_settings(self):
        ...
        st.session_state.settings['api']['webhook_enabled'] = st.checkbox(
            "Enable Webhooks",
            value=st.session_state.settings['api'].get('webhook_enabled', False)
        )
        ...
        if st.session_state.settings['api']['webhook_enabled']:
            ...
            st.text_input(
                "Webhook Secret",
                value=st.session_state.settings['api'].get('webhook_secret', ''),
                type="password",
        ...
        with col1:
            st.text_input(
                ...
                value=st.session_state.settings['api'].get('alpha_vantage_key', ''),
                type="password",
