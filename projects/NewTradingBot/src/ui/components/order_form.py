...
class OrderForm:
    ...
    def render(self, available_balance: float = 10000.0, current_price: float = 100.0):
        """
        ...
        with st.container():
            ...
            # Advanced settings
            if st.checkbox("Advanced Settings", value=st.session_state.order_form['advanced_settings']):
    ...
    def _render_quantity_input(self, available_balance: float, current_price: float):
    ...
    def _render_price_inputs(self, current_price: float):
        ...
        if order_type in [OrderType.STOP.value, OrderType.STOP_LIMIT.value, OrderType.TRAILING_STOP.value]:
    ...
    def _render_time_in_force(self):
        ...
        st.session_state.order_form['time_in_force'] = st.selectbox(
            "Time in Force",
            tif_options,
            index=tif_options.index(st.session_state.order_form['time_in_force']),
    ...
    def _render_order_summary(self, current_price: float, available_balance: float):
        ...
        is_maker = order_type == OrderType.LIMIT.value and st.session_state.order_form['post_only']
        ...
        with col3:
            remaining_balance = available_balance - order_value - fees if side == OrderSide.BUY.value else available_balance
            st.metric(
                "Remaining Balance",
                self.formatter.format_currency(remaining_balance),
                delta=self.formatter.format_currency(remaining_balance - available_balance)
            )
        ...
        if quantity > 0:
            ...
            with col1:
                ...
                st.info(f"{risk_color} Position Size: {position_size_pct:.1f}% of balance")
            
            with col2:
                leverage = order_value / (available_balance * 0.1)  # Assuming 10% margin
    ...
    def _render_submit_buttons(self):
        ...
        with col1:
            side = st.session_state.order_form['side']
            button_label = f"{'🟢 BUY' if side == OrderSide.BUY.value else '🔴 SELL'} {st.session_state.order_form['symbol']}"
        ...
        with col3:
            if st.button("Templates", use_container_width=True, key="order_templates"):
    ...
    def _validate_order(self):
        ...
        if st.session_state.order_form['type'] in [OrderType.LIMIT.value, OrderType.STOP_LIMIT.value]:
            if st.session_state.order_form['price'] <= 0:
                errors.append("Price must be greater than 0")
        
        # Check stop price for stop orders
        if st.session_state.order_form['type'] in [OrderType.STOP.value, OrderType.STOP_LIMIT.value]:
