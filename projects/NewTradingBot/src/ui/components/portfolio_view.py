...
from ..utils.formatters import DataFormatter, TableFormatter, MetricsFormatter
...
class PortfolioView:
    ...
    def render(self, portfolio_data: Optional[Dict[str, Any]] = None):
        ...
        tabs = st.tabs(["📊 Positions", "📈 Performance", "📜 History", "⚠️ Risk"])
    ...
    def _render_portfolio_header(self, portfolio_data: Dict[str, Any]):
        ...
        with col3:
            ...
            st.session_state.portfolio_view['time_range'] = st.selectbox(
                "Time Range",
                time_ranges,
                index=time_ranges.index(st.session_state.portfolio_view['time_range']),
                label_visibility="collapsed"
            )
    ...
    def _render_portfolio_metrics(self, metrics: Dict[str, Any]):
        ...
        formatted_metrics = self.metrics_formatter.format_portfolio_metrics(metrics)
    ...
    def _render_positions_tab(self, positions: List[Dict[str, Any]]):
        ...
        # Apply filters
        filtered_positions = self._filter_positions(positions, position_filter, search)
        ...
        if st.session_state.portfolio_view['selected_position']:
            self._render_position_details(st.session_state.portfolio_view['selected_position'])
    ...
    def _render_position_card(self, position: Dict[str, Any]):
        ...
        with st.container():
            ...
            with col1:
                if st.button("Details", key=f"details_{position['symbol']}", use_container_width=True):
                    st.session_state.portfolio_view['selected_position'] = position
            
            with col2:
                if st.button("Add", key=f"add_{position['symbol']}", use_container_width=True):
                    st.info(f"Add to {position['symbol']} position")
            
            with col3:
                if st.button("Close", key=f"close_{position['symbol']}", use_container_width=True):
    ...
    def _render_performance_tab(self, performance: Dict[str, Any]):
        ...
        st.plotly_chart(
            self._create_monthly_returns_heatmap(performance['monthly_returns']),
            use_container_width=True
        )
    ...
    def _render_risk_tab(self, risk: Dict[str, Any]):
        ...
        with col2:
            ...
            st.metric(
                "Expected Shortfall",
                self.formatter.format_currency(risk.get('expected_shortfall', 0)),
    ...
    def _render_position_details(self, position: Dict[str, Any]):
        ...
        with st.expander(f"📊 Position Details: {position['symbol']}", expanded=True):
            ...
            with col2:
                ...
                st.markdown(
                    f"<h3 style='color: {color};'>{self.formatter.format_currency(pnl)}</h3>",
                    unsafe_allow_html=True
                )
                st.markdown(
                ...
                st.text(f"Position Value: {self.formatter.format_currency(position['current_value'])}")
                st.text(f"Daily Change: {self.formatter.format_currency(position.get('daily_change', 0))}")
    ...
    def _filter_positions(self, positions: List[Dict[str, Any]], 
                         filter_type: str, search: str) -> List[Dict[str, Any]]:
        ...
        if filter_type == "Profitable":
            ...
            filtered = [p for p in filtered 
                       if datetime.fromisoformat(p['entry_time']).date() == today]
    ...
    def _create_returns_distribution(self, returns: List[float]) -> go.Figure:
        ...
        y = y * len(returns) * (max(returns) - min(returns)) / 30  # Scale to histogram
    ...
    def _create_monthly_returns_heatmap(self, monthly_returns: Dict[str, List[float]]) -> go.Figure:
