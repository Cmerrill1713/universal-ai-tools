...
class TradingChart:
    ...
    def _render_chart_header(self, symbol: str, exchange: str):
        ...
        with col1:
            ...
            if not st.session_state.price_data.empty:
                ...
                prev_price = st.session_state.price_data['close'].iloc[-2] if len(
                    st.session_state.price_data) > 1 else current_price
                ...
                st.markdown(
                    f"<h2 style='color: {color};'>${
                        current_price:.2f} " f"<small>{arrow} ${
                        abs(change):.2f} ({
                        change_pct:+.2f}%)</small></h2>",
                    unsafe_allow_html=True)

        with col2:
            timeframes = ['1m', '5m', '15m', '30m', '1h', '4h', '1d', '1w']
            st.session_state.chart_settings['timeframe'] = st.selectbox(
                "Timeframe", timeframes, index=timeframes.index(
                    st.session_state.chart_settings['timeframe']))

        with col3:
            chart_types = ['candlestick', 'line', 'area', 'bars']
            st.session_state.chart_settings['chart_type'] = st.selectbox(
                "Chart Type", chart_types, index=chart_types.index(
                    st.session_state.chart_settings['chart_type']))
    ...
    def _create_line_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        self.chart_builder._apply_theme(
            fig, st.session_state.chart_settings['theme'])
        return fig
