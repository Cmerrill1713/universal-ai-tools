...
class TradingBotApp:
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            ...
            st.caption(
                f"v1.0 | {
                    st.session_state.start_time.strftime('%Y-%m-%d')}")

            # Connection status
            status_color = "🟢" if st.session_state.trading_state['connected'] else "🔴"
            mode = "Paper Trading" if st.session_state.trading_state[
                'paper_trading'] else "Live Trading"
            ...
            with col2:
                st.session_state.user_preferences['notifications_enabled'] = st.checkbox(
                    "Notifications", value=st.session_state.user_preferences['notifications_enabled'])
