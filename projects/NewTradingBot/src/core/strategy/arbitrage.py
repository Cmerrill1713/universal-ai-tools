...
class ArbitrageConfig(StrategyConfig):
    ...
    def __init__(self, **kwargs):
        ...
        self.min_spread_threshold = kwargs.get('min_spread_threshold', 0.3)  # Minimum 0.3% spread
        self.max_execution_time_ms = kwargs.get('max_execution_time_ms', 500)  # Max execution time
        self.min_profit_threshold = kwargs.get('min_profit_threshold', Decimal('10'))  # Min $10 profit
        
        # Fee considerations
        self.trading_fee_percentage = kwargs.get('trading_fee_percentage', 0.1)  # 0.1% trading fee
        self.withdrawal_fee = kwargs.get('withdrawal_fee', Decimal('5'))  # $5 withdrawal fee
        self.slippage_tolerance = kwargs.get('slippage_tolerance', 0.1)  # 0.1% slippage tolerance
        
        # Exchange configuration
        self.supported_exchanges = kwargs.get('supported_exchanges', ['binance', 'coinbase', 'kraken'])
        ...
        self.triangular_currencies = kwargs.get('triangular_currencies', ['BTC', 'ETH', 'USDT', 'USD'])
        ...
        # Risk management
        self.max_position_per_exchange = kwargs.get('max_position_per_exchange', Decimal('1000'))
        self.max_total_arbitrage_exposure = kwargs.get('max_total_arbitrage_exposure', Decimal('5000'))
        self.emergency_exit_threshold = kwargs.get('emergency_exit_threshold', 2.0)  # Exit if spread narrows by 2%
...
class ArbitrageStrategy(BaseStrategy):
    ...
    def __init__(self, config: ArbitrageConfig = None):
        ...
        self.exchange_volumes: Dict[str, Dict[str, Decimal]] = {}  # exchange -> symbol -> volume
        self.exchange_timestamps: Dict[str, datetime] = {}  # exchange -> last_update
        
        # Statistical arbitrage data
        self.correlation_matrix: Optional[pd.DataFrame] = None
        self.price_spreads: Dict[Tuple[str, str], List[float]] = {}  # (symbol1, symbol2) -> spread history
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        for exchange in exchanges:
            ...
            indicators[f'{exchange}_price'] = exchange_data['close'].iloc[-1] if len(exchange_data) > 0 else None
            indicators[f'{exchange}_volume'] = exchange_data['volume'].sum()
            indicators[f'{exchange}_volatility'] = exchange_data['close'].pct_change().std() * np.sqrt(24)  # Daily volatility
            indicators[f'{exchange}_spread'] = ((exchange_data['high'] - exchange_data['low']) / exchange_data['close']).mean()
            ...
            for i, exchange1 in enumerate(exchanges):
                for exchange2 in exchanges[i+1:]:
                    ...
                    if price1 and price2 and price1 > 0 and price2 > 0:
                        spread_pct = abs(float(price1 - price2)) / float((price1 + price2) / 2) * 100
        ...
        if len(data) >= self.config.statistical_lookback:
            price_matrix = data.pivot_table(index=data.index, columns='exchange', values='close')
    ...
    def detect_spatial_arbitrage(self, exchange_data: Dict[str, Dict[str, Any]]) -> List[ArbitrageOpportunity]:
        """
        ...
        for symbol in all_symbols:
            ...
            # Calculate spread
            spread_pct = float((highest_price - lowest_price) / lowest_price * 100)
            
            if spread_pct >= self.config.min_spread_threshold:
                # Calculate potential profit
                trade_amount = min(lowest_volume, highest_volume) * Decimal('0.1')  # Use 10% of lower volume
                trade_amount = min(trade_amount, self.config.max_position_per_exchange)
                
                gross_profit = (highest_price - lowest_price) * trade_amount
                
                # Calculate fees
                buy_fee = lowest_price * trade_amount * Decimal(str(self.config.trading_fee_percentage / 100))
                sell_fee = highest_price * trade_amount * Decimal(str(self.config.trading_fee_percentage / 100))
                ...
                if net_profit >= self.config.min_profit_threshold:
                    opportunity = ArbitrageOpportunity(
                        ...
                        max_trade_amount=min(lowest_volume, highest_volume),
                        execution_time_ms=max(
                            self.config.exchange_latency.get(lowest_exchange, 100),
                            self.config.exchange_latency.get(highest_exchange, 100)
                        ),
                        confidence=min(0.95, spread_pct / self.config.min_spread_threshold * 0.8),
    ...
    def detect_triangular_arbitrage(self, exchange_data: Dict[str, Any]) -> List[ArbitrageOpportunity]:
        """
        ...
        for i, curr_a in enumerate(currencies):
            for j, curr_b in enumerate(currencies):
                ...
                for k, curr_c in enumerate(currencies):
                    ...
                    price_ab = prices.get(pair_ab) or (1 / prices.get(pair_ba, Decimal('0')) if prices.get(pair_ba, Decimal('0')) > 0 else Decimal('0'))
                    price_bc = prices.get(pair_bc) or (1 / prices.get(pair_cb, Decimal('0')) if prices.get(pair_cb, Decimal('0')) > 0 else Decimal('0'))
                    price_ca = prices.get(pair_ca) or (1 / prices.get(pair_ac, Decimal('0')) if prices.get(pair_ac, Decimal('0')) > 0 else Decimal('0'))
                    ...
                    if price_ab > 0 and price_bc > 0 and price_ca > 0:
                        ...
                        if profit_percentage >= self.config.min_spread_threshold:
                            ...
                            trade_value = Decimal('1000')  # Start with $1000 equivalent
                            
                            # Three trades, each with fees
                            fee_rate = Decimal(str(self.config.trading_fee_percentage / 100))
                            ...
                            if net_profit >= self.config.min_profit_threshold:
                                opportunity = ArbitrageOpportunity(
                                    ...
                                    symbol_2=pair_bc,
                                    symbol_3=pair_ca,
                                    exchange_1=list(exchange_data.keys())[0] if exchange_data else "",
                                    price_1=price_ab,
                                    price_2=price_bc,
                                    price_3=price_ca,
                                    ...
                                    confidence=min(0.9, profit_percentage / self.config.min_spread_threshold * 0.7),
                                    fees_total=total_fees,
                                    net_profit=net_profit
                                )
                                opportunities.append(opportunity)
    ...
    def detect_statistical_arbitrage(self, data: pd.DataFrame) -> List[ArbitrageOpportunity]:
        """
        ...
        if not self.config.enable_statistical or len(data) < self.config.statistical_lookback:
            return opportunities
        ...
        for symbol in symbols:
            ...
            if len(symbol_data) >= self.config.statistical_lookback:
                price_data[symbol] = symbol_data.tail(self.config.statistical_lookback)
            ...
            for symbol2 in symbols[i+1:]:
                ...
                if abs(correlation) >= self.config.correlation_threshold:
                    ...
                    if abs(z_score) >= self.config.mean_reversion_zscore:
                        ...
                        profit_potential = abs(expected_reversion) * Decimal('1000')  # Scale by position size
                        
                        # Estimate fees (two positions)
                        fee_rate = Decimal(str(self.config.trading_fee_percentage / 100))
                        total_fees = Decimal('2000') * fee_rate  # $2000 total position value
    ...
    def analyze_market(self, data: pd.DataFrame) -> List[TradingSignal]:
        ...
        if 'exchange' in data.columns:
            ...
            spatial_opportunities = self.detect_spatial_arbitrage(exchange_data)
            
            # Detect triangular arbitrage for each exchange
            for exchange, ex_data in exchange_data.items():
                triangular_opportunities = self.detect_triangular_arbitrage({exchange: ex_data})
            ...
            if opp.net_profit >= self.config.min_profit_threshold:
                ...
                if opp.arbitrage_type == ArbitrageType.SPATIAL:
                    signal = TradingSignal(
                        ...
                        symbol=opp.symbol_1,
                        strength=min(1.0, opp.spread_percentage / 5.0),  # Scale to 5% max
                        confidence=opp.confidence,
                        price_target=opp.price_1,  # Buy at lower price
                        stop_loss=opp.price_1 * Decimal('0.99'),  # 1% stop loss
                        take_profit=opp.price_2 * Decimal('0.99'),  # Sell slightly below high price
                        indicators={
                    ...
                    for i, (symbol, price) in enumerate([(opp.symbol_1, opp.price_1), 
                        ...
                        signal = TradingSignal(
                            ...
                            indicators={
                                ...
                                'total_legs': 3,
                                'spread_percentage': opp.spread_percentage,
                                'net_profit': float(opp.net_profit / 3)  # Divide profit among legs
                            },
                            metadata={
                                'strategy': 'arbitrage',
