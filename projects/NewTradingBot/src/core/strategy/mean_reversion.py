...
class MeanReversionConfig(StrategyConfig):
    ...
    def __init__(self, **kwargs):
        ...
        self.quick_exit_threshold = kwargs.get(
            'quick_exit_threshold',
            0.5)  # Quick exit if trade goes against us
...
class MeanReversionStrategy(BaseStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        bb_upper = ta.volatility.bollinger_hband(
            close, window=self.config.bb_period, window_dev=self.config.bb_std_dev)
        bb_lower = ta.volatility.bollinger_lband(
            close, window=self.config.bb_period, window_dev=self.config.bb_std_dev)
        bb_middle = ta.volatility.bollinger_mavg(
            close, window=self.config.bb_period)
        ...
        # Volume analysis
        volume_sma = ta.volume.volume_sma(
            close, volume, window=self.config.mean_lookback)
    ...
    def detect_bullish_divergence(self, data: pd.DataFrame,
        ...
        price_data = data['close'].iloc[current_index -
                                        lookback_period:current_index + 1]
        rsi_data = indicators['rsi'].iloc[current_index -
                                          lookback_period:current_index + 1]
    ...
    def calculate_reversion_probability(self, indicators: Dict[str, Any],
        ...
        if not pd.isna(z_score):
            # Normalize to 3 standard deviations
        ...
        if not pd.isna(rsi):
            if rsi <= self.config.rsi_oversold:
                rsi_factor = 0.8 + (self.config.rsi_oversold -
                                    rsi) / self.config.rsi_oversold * 0.2
            elif rsi >= self.config.rsi_overbought:
                rsi_factor = 0.8 + (rsi - self.config.rsi_overbought) / \
                    (100 - self.config.rsi_overbought) * 0.2
            else:
    ...
    def analyze_market(self, data: pd.DataFrame) -> List[TradingSignal]:
        ...
        reversion_probability = self.calculate_reversion_probability(
        ...
        if sum(oversold_conditions) >= len(
                oversold_conditions) * 0.7:  # 70% of conditions met
            confidence = min(0.95, 0.5 + reversion_probability * 0.4)
            # Strength based on deviation from mean
            ...
            # Target mean or modest profit
            take_profit = min(mean_price, current_close + (atr * 2))
        ...
        # Generate sell signal (overbought reversal)
            ...
            stop_loss = current_close + (atr * 1.5)
            # Target mean or modest profit
            take_profit = max(mean_price, current_close - (atr * 2))
