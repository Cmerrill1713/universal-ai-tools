...
class StrategyConfig:
    ...
    # Risk management
    max_position_size: Decimal = field(default_factory=lambda: Decimal('0.1'))  # 10% of portfolio
    ...
    # Market conditions
    min_volume_threshold: Decimal = field(default_factory=lambda: Decimal('10000'))
...
class BaseStrategy(ABC):
    ...
    def __init__(self, config: StrategyConfig):
        ...
        self.active_positions: Dict[str, Dict[str, Any]] = {}  # Symbol -> position data
    ...
    def can_generate_signal(self, symbol: str) -> bool:
        ...
        time_since_last_signal = datetime.utcnow() - self.last_signal_time[symbol]
    ...
    def calculate_position_size(self, signal: TradingSignal, 
        ...
        if self.config.position_sizing_method == "fixed_percentage":
            ...
            volatility = signal.indicators.get('atr', current_price * Decimal('0.02'))
            risk_per_trade = portfolio_value * Decimal('0.02')  # 2% risk per trade
    ...
    def calculate_stop_loss(self, entry_price: Decimal, 
        ...
        Returns:
            Stop loss price
        """
        stop_loss_multiplier = Decimal(str(1 - self.config.stop_loss_percentage / 100))
    ...
    def calculate_take_profit(self, entry_price: Decimal, 
        ...
        Returns:
            Take profit price
        """
        take_profit_multiplier = Decimal(str(1 + self.config.take_profit_percentage / 100))
    ...
    def create_orders_from_signal(self, signal: TradingSignal,
        ...
        position_value = self.calculate_position_size(signal, portfolio_value, current_price)
        ...
        if self.config.stop_loss_percentage > 0:
            stop_loss_price = signal.stop_loss or self.calculate_stop_loss(current_price, signal.signal_type)
        ...
        if self.config.take_profit_percentage > 0:
            take_profit_price = signal.take_profit or self.calculate_take_profit(current_price, signal.signal_type)
    ...
    def update_performance_metrics(self, trade: Trade) -> None:
        ...
        self.performance_metrics['win_rate'] = successful_signals / total_signals if total_signals > 0 else 0
    ...
    def __str__(self) -> str:
        ...
        return f"{self.config.name} ({self.state.value}): {len(self.active_positions)} positions"
    
    def __repr__(self) -> str:
        """Detailed string representation of the strategy."""
        return f"BaseStrategy(id='{self.id}', name='{self.config.name}', state={self.state.value})"