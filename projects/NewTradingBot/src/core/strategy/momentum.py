...
class MomentumConfig(StrategyConfig):
    ...
    def __init__(self, **kwargs):
        ...
        self.volume_threshold_multiplier = kwargs.get('volume_threshold_multiplier', 1.5)
        self.momentum_lookback = kwargs.get('momentum_lookback', 10)
        
        # Momentum strength thresholds
        self.strong_momentum_threshold = kwargs.get('strong_momentum_threshold', 0.8)
        self.weak_momentum_threshold = kwargs.get('weak_momentum_threshold', 0.3)
        
        # Trend confirmation requirements
        self.require_volume_confirmation = kwargs.get('require_volume_confirmation', True)
        self.require_multiple_timeframes = kwargs.get('require_multiple_timeframes', False)
...
class MomentumStrategy(BaseStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        macd_signal = ta.trend.macd_signal(close, window_slow=self.config.macd_slow,
                                          window_fast=self.config.macd_fast,
        ...
        # Moving Averages
        indicators['sma_short'] = ta.trend.sma_indicator(close, window=self.config.sma_short)
        indicators['sma_long'] = ta.trend.sma_indicator(close, window=self.config.sma_long)
        ...
        # Price momentum
        indicators['momentum'] = close.pct_change(self.config.momentum_lookback)
        indicators['price_rate_of_change'] = ta.momentum.roc(close, window=self.config.momentum_lookback)
    ...
    def analyze_momentum_strength(self, indicators: Dict[str, Any], 
        ...
        close_price = indicators.get('close', pd.Series()).iloc[index] if 'close' in indicators else None
    ...
    def analyze_market(self, data: pd.DataFrame) -> List[TradingSignal]:
        ...
        if any(pd.isna(val) for val in [rsi_current, macd_current, macd_signal_current, 
                                       sma_short_current, sma_long_current]):
            return signals
        
        # Calculate momentum strength
        momentum_strength = self.analyze_momentum_strength(indicators, current_index)
        ...
        if all(buy_conditions):
            ...
            signal_type = SignalType.STRONG_SELL if momentum_strength < (1 - self.config.strong_momentum_threshold) else SignalType.SELL
