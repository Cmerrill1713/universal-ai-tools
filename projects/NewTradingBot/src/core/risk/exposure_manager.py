...
class ExposureManager:
    ...
    def add_exposure_limit(self, limit: ExposureLimit) -> None:
        ...
        self.exposure_limits = [l for l in self.exposure_limits if not (
            l.exposure_type == limit.exposure_type and l.category == limit.category)]
    ...
    def classify_asset(self, symbol: str, classification_type: str) -> str:
        ...
        """
        classifications = self.asset_classifications.get(
            classification_type, {})
        return classifications.get(symbol, 'other')

    def calculate_exposure_by_type(self,
                                   portfolio: Portfolio,
                                   exposure_type: ExposureType) -> Dict[str,
                                                                        ExposureMetrics]:
        """
        ...
        for category, positions in category_positions.items():
            ...
            if abs(deviation) > 5.0:
                if deviation > 0:
                    recommendation = f"REDUCE: {
                        abs(deviation):.1f}% over target"
                else:
                    recommendation = f"INCREASE: {
                        abs(deviation):.1f}% under target"
            else:
    ...
    def identify_correlation_clusters(
        ...
        available_symbols = [
            s for s in symbols if s in correlation_matrix.index]
        ...
        for i, symbol1 in enumerate(available_symbols):
            ...
            for j, symbol2 in enumerate(available_symbols):
                if i != j and symbol2 not in clustered_symbols:
                    if (symbol1 in correlation_matrix.index and
                        ...
                        correlation = abs(
                            correlation_matrix.loc[symbol1, symbol2])
            ...
            if len(cluster_symbols) > 1:
                ...
                total_cluster_value = sum(
                    pos.market_value for pos in cluster_positions)
                cluster_exposure_pct = float(
                    total_cluster_value /
                    portfolio.total_value *
                    100) if portfolio.total_value > 0 else 0
    ...
    def assess_concentration_risk(
            self, portfolio: Portfolio) -> Dict[str, Any]:
        """
        ...
        if n > 1:
            ...
            gini = (2 * sum((i + 1) * weight for i, weight in enumerate(weights_sorted))
                    ) / (n * sum(weights_sorted)) - (n + 1) / n
        else:
            gini = 0

        # Overall concentration score (0-1)
        hhi_normalized = (hhi - 1 / n) / (1 - 1 / n) if n > 1 else 0
        concentration_score = (hhi_normalized * 0.4 +
                               gini * 0.4 + min(1.0, top_5_pct / 50.0) * 0.2)
            ...
            for i, pos_data in enumerate(position_data[:3]):
                if pos_data['weight_pct'] > 15.0:
                    recommendations.append(
                        f"Reduce {
                            pos_data['symbol']} position ({
                            pos_data['weight_pct']:.1f}% of portfolio)")
    ...
    def validate_trade_exposure(
            self, trade: Trade, portfolio: Portfolio) -> Tuple[bool, List[str]]:
        """
        ...
        for exposure_type in ExposureType:
            try:
                exposures = self.calculate_exposure_by_type(
                    simulated_portfolio, exposure_type)
    ...
    def generate_rebalancing_recommendations(
        ...
        if self.correlation_matrix is not None:
            clusters = self.identify_correlation_clusters(
                portfolio, self.correlation_matrix)
            high_risk_clusters = [
                c for c in clusters if c.total_exposure_pct > 20.0]
    ...
    def get_exposure_summary(self, portfolio: Portfolio) -> Dict[str, Any]:
        ...
        if summary['violations']:
            summary['recommendations'].append(
                "Address exposure limit violations")
        ...
        if not summary['recommendations']:
            summary['recommendations'].append(
                "Exposure profile is within acceptable limits")
