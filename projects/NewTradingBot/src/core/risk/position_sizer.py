...
class PositionSizer:
    ...
    def calculate_fixed_amount(self, amount: Decimal, current_price: Decimal) -> PositionSizeResult:
        """
        ...
        """
        size_shares = amount / current_price if current_price > 0 else Decimal('0')
    ...
    def calculate_fixed_percentage(self, portfolio_value: Decimal, 
                                 percentage: float, current_price: Decimal) -> PositionSizeResult:
        """
        ...
        size_shares = size / current_price if current_price > 0 else Decimal('0')
    ...
    def calculate_volatility_adjusted(self, portfolio_value: Decimal,
        ...
        # Cap the percentage
        adjusted_percentage = min(adjusted_percentage, self.risk_limits.max_position_size_pct)
        
        size = portfolio_value * Decimal(str(adjusted_percentage / 100))
        size_shares = size / current_price if current_price > 0 else Decimal('0')
        
        # Risk amount based on volatility
        risk_amount = size * Decimal(str(asset_volatility * 2))  # 2x daily volatility as risk
        risk_percentage = float(risk_amount / portfolio_value * 100) if portfolio_value > 0 else 0
        
        constraints = []
        if adjusted_percentage != base_percentage * vol_adjustment:
            constraints.append(f"Capped at max position size {self.risk_limits.max_position_size_pct}%")
    ...
    def calculate_kelly_criterion(self, portfolio_value: Decimal,
                                current_price: Decimal,
                                win_rate: float = None,
                                avg_win: float = None,
        ...
        kelly_fraction = min(kelly_fraction, self.kelly_max_fraction)  # Cap at maximum
        ...
        constraints = []
        if kelly_fraction <= 0:
            constraints.append("Negative Kelly fraction - no position recommended")
        elif kelly_fraction == self.kelly_max_fraction:
            constraints.append(f"Capped at maximum Kelly fraction {self.kelly_max_fraction}")
        
        confidence = min(0.9, kelly_fraction / 0.2) if kelly_fraction > 0 else 0.1
    ...
    def calculate_atr_based(self, portfolio_value: Decimal,
                          current_price: Decimal,
                          atr: Decimal,
        ...
        max_size = portfolio_value * Decimal(str(self.risk_limits.max_position_size_pct / 100))
        constraints = []
        
        if size > max_size:
            size = max_size
            size_shares = size / current_price if current_price > 0 else Decimal('0')
            constraints.append(f"Position capped at {self.risk_limits.max_position_size_pct}% of portfolio")
    ...
    def calculate_risk_parity(self, portfolio_value: Decimal,
                            current_price: Decimal,
                            asset_volatility: float,
        ...
        position_volatility_adjusted = risk_budget / Decimal(str(asset_volatility))
    ...
    def calculate_optimal_position_size(self, 
        ...
        if method == PositionSizingMethod.FIXED_AMOUNT:
            ...
            result = self.calculate_fixed_percentage(portfolio_value, 10.0, current_price)
        
        elif method == PositionSizingMethod.VOLATILITY_ADJUSTED:
            result = self.calculate_volatility_adjusted(portfolio_value, current_price, volatility)
        
        elif method == PositionSizingMethod.KELLY_CRITERION:
            win_rate = signal_data.get('historical_win_rate', self.kelly_win_rate)
            avg_win = signal_data.get('avg_win', self.kelly_avg_win)
            avg_loss = signal_data.get('avg_loss', self.kelly_avg_loss)
            result = self.calculate_kelly_criterion(portfolio_value, current_price, win_rate, avg_win, avg_loss)
        
        elif method == PositionSizingMethod.ATR_BASED:
            result = self.calculate_atr_based(portfolio_value, current_price, atr)
        
        elif method == PositionSizingMethod.RISK_PARITY:
            result = self.calculate_risk_parity(portfolio_value, current_price, volatility)
        ...
        result = self._apply_signal_adjustments(result, signal_strength, signal_confidence)
    ...
    def _apply_signal_adjustments(self, result: PositionSizeResult, 
        ...
        adjustment_factor = Decimal(str(0.5 + signal_quality * 0.5))  # Range: 0.5 to 1.0
        ...
        if adjustment_factor < Decimal('1.0'):
            result.constraints_applied.append(f"Position reduced by {(1-float(adjustment_factor))*100:.1f}% due to signal quality")
    ...
    def _apply_portfolio_constraints(self, result: PositionSizeResult, 
        ...
        max_position_value = portfolio_value * Decimal(str(self.risk_limits.max_position_size_pct / 100))
        ...
        if total_position_size > max_position_value:
            ...
            if available_size < result.size:
                reduction_factor = available_size / result.size if result.size > 0 else Decimal('0')
                ...
                result.constraints_applied.append(f"Position size reduced to stay within {self.risk_limits.max_position_size_pct}% limit")
        ...
        if len(portfolio.active_positions) >= 20:  # Arbitrary limit
            ...
            result.constraints_applied.append("Position reduced due to high number of existing positions")
        
        # Update risk percentage
        result.risk_percentage = float(result.risk_amount / portfolio_value * 100) if portfolio_value > 0 else 0
    ...
    def get_sizing_recommendation(self, 
        ...
        for method, result in valid_results.items():
            ...
            score = result.confidence * (1 - len(result.constraints_applied) * 0.1)
