...
class RiskCalculator:
    ...
    def calculate_var(self, returns: List[float], confidence_level: float = 0.95) -> float:
        ...
        # VaR is the negative of the percentile (since we want losses as positive)
    ...
    def calculate_drawdown(self, portfolio_values: List[Decimal]) -> Tuple[float, float]:
        """
        ...
        current_drawdown = (current_peak - current_value) / current_peak * 100 if current_peak > 0 else 0.0
    ...
    def calculate_concentration_risk(self, portfolio: Portfolio) -> float:
        ...
        concentration_score = (hhi - min_hhi) / (1.0 - min_hhi) if min_hhi < 1.0 else 0.0
        
        return min(1.0, concentration_score)
    
    def calculate_correlation_risk(self, portfolio: Portfolio, correlation_matrix: pd.DataFrame = None) -> float:
        """
        ...
        weights = [float(pos.market_value / portfolio.total_value) for pos in portfolio.active_positions]
    ...
    def calculate_liquidity_score(self, portfolio: Portfolio, volume_data: Dict[str, Decimal] = None) -> float:
        """
        ...
        for position in portfolio.active_positions:
            ...
            if volume_data and position.symbol in volume_data:
                ...
                position_volume_ratio = position.market_value / (daily_volume * position.last_price) if daily_volume > 0 else 1.0
                volume_liquidity = max(0.0, 1.0 - float(position_volume_ratio) * 0.1)  # Penalty for large positions
            ...
            position_liquidity = np.mean(liquidity_factors) if liquidity_factors else 0.7
    ...
    def calculate_leverage_ratio(self, portfolio: Portfolio) -> float:
        ...
        total_position_value = sum(pos.market_value for pos in portfolio.active_positions)
    ...
    def assess_portfolio_risk(self, portfolio: Portfolio, 
                            historical_data: Dict[str, Any] = None) -> RiskMetrics:
        """
        ...
        returns = historical_data.get('returns', []) if historical_data else self.historical_returns
        portfolio_values = historical_data.get('values', []) if historical_data else self.historical_values
        market_returns = historical_data.get('market_returns', []) if historical_data else self.market_returns
        correlation_matrix = historical_data.get('correlation_matrix') if historical_data else None
        volume_data = historical_data.get('volume_data', {}) if historical_data else {}
        
        # Calculate VaR and Expected Shortfall
        if len(returns) >= 30:
            metrics.var_95 = Decimal(str(self.calculate_var(returns, 0.95))) * portfolio.total_value
            metrics.var_99 = Decimal(str(self.calculate_var(returns, 0.99))) * portfolio.total_value
            metrics.expected_shortfall = Decimal(str(self.calculate_expected_shortfall(returns, 0.95))) * portfolio.total_value
        ...
        if len(returns) >= 2:
            metrics.volatility_annual = self.calculate_volatility(returns, annualize=True)
        ...
        metrics.concentration_risk = self.calculate_concentration_risk(portfolio)
        metrics.correlation_risk = self.calculate_correlation_risk(portfolio, correlation_matrix)
        metrics.liquidity_score = self.calculate_liquidity_score(portfolio, volume_data)
    ...
    def validate_trade_risk(self, trade: Trade, portfolio: Portfolio, 
                           current_metrics: RiskMetrics = None) -> Tuple[bool, List[str]]:
        """
        ...
        if portfolio_value > 0:
            ...
            if position_pct > self.risk_limits.max_position_size_pct:
                violations.append(f"Position size {position_pct:.1f}% exceeds limit of {self.risk_limits.max_position_size_pct}%")
        ...
        if portfolio_value > 0:
            new_concentration_pct = float(new_position_value / portfolio_value * 100)
            if new_concentration_pct > self.risk_limits.max_position_size_pct:
                violations.append(f"Total position concentration {new_concentration_pct:.1f}% exceeds limit")
        
        # Check current risk metrics against limits
        if current_metrics:
            if current_metrics.leverage_ratio > self.risk_limits.max_leverage_ratio:
                violations.append(f"Leverage ratio {current_metrics.leverage_ratio:.2f} exceeds limit of {self.risk_limits.max_leverage_ratio}")
            
            if current_metrics.current_drawdown > self.risk_limits.max_drawdown_pct:
                violations.append(f"Current drawdown {current_metrics.current_drawdown:.1f}% exceeds limit")
            
            if current_metrics.liquidity_score < self.risk_limits.min_liquidity_score:
                violations.append(f"Liquidity score {current_metrics.liquidity_score:.2f} below minimum")
            
            var_pct = float(current_metrics.var_95 / portfolio_value * 100) if portfolio_value > 0 else 0
            if var_pct > self.risk_limits.max_portfolio_var_pct:
                violations.append(f"Portfolio VaR {var_pct:.1f}% exceeds limit")
    ...
    def calculate_optimal_position_size(self, signal_strength: float, 
        ...
        max_position = portfolio.total_value * Decimal(str(self.risk_limits.max_position_size_pct / 100))
        position_size = min(position_size, max_position)
        
        return position_size
    
    def generate_risk_report(self, metrics: RiskMetrics, portfolio: Portfolio) -> Dict[str, Any]:
        """
    ...
    def _generate_risk_recommendations(self, metrics: RiskMetrics, portfolio: Portfolio) -> List[str]:
        """Generate risk management recommendations."""
        recommendations = []
        
        if metrics.risk_level == RiskLevel.EXTREME:
            recommendations.append("URGENT: Risk level is EXTREME. Consider reducing positions immediately.")
        elif metrics.risk_level == RiskLevel.HIGH:
            recommendations.append("WARNING: High risk level detected. Review and reduce risk exposure.")
        
        if metrics.concentration_risk > 0.7:
            recommendations.append("High concentration risk. Consider diversifying positions across more assets.")
        
        if metrics.correlation_risk > 0.8:
            recommendations.append("High correlation risk. Positions are highly correlated, reducing diversification benefits.")
        
        if metrics.liquidity_score < self.risk_limits.min_liquidity_score:
            recommendations.append("Low liquidity score. Consider reducing positions in illiquid assets.")
        
        if metrics.leverage_ratio > self.risk_limits.max_leverage_ratio:
            recommendations.append(f"Leverage ratio {metrics.leverage_ratio:.2f} exceeds safe limits. Reduce leverage.")
        
        if metrics.current_drawdown > self.risk_limits.max_drawdown_pct:
            recommendations.append("Current drawdown exceeds limits. Consider defensive positioning.")
        
        if metrics.volatility_annual > self.risk_limits.max_portfolio_volatility_pct:
            recommendations.append("Portfolio volatility is high. Consider adding less volatile assets.")
        
        if not recommendations:
            recommendations.append("Risk profile is within acceptable limits. Continue monitoring.")
