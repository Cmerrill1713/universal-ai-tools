...
export const CACHE_BACKENDS = {;
  redis: {;
    ...
    db: parseInt(processenvREDIS_DB || '0', 10);
    keyPrefix: processenvREDIS_KEY_PREFIX || 'uai:';
    enableOfflineQueue: true;
    maxRetriesPerRequest: 3;
    retryStrategy: (times: number) => {;
      const delay = Mathmin(times * 50, 2000);
      return delay;
    };
  };
} as const;
...
export const getCacheConfig = (;
  env: string = processenvNODE_ENV || 'development';
): CacheConfig => {;
  const configs: Record<string, CacheConfig> = {;
    development: {;
      backend: 'redis';
      defaultTTL: CACHE_TTLDEFAULT;
      evictionPolicy: EvictionPolicyLRU;
      consistencyStrategy: ConsistencyStrategyEVENTUAL;
    ...
    test: {;
      ...
      defaultTTL: 60;
      evictionPolicy: EvictionPolicyLRU;
      consistencyStrategy: ConsistencyStrategySTRONG;
      enableCompression: false;
      enableDistributed: false;
      enableMetrics: false;
      enableWarmup: false;
  ...
  return configs[env] || configsdevelopment;
};
// Helper function to get Redis URL;
export const getRedisUrl = (): string => {;
  const { host, port, password, db } = CACHE_BACKENDSredis;
  if (password) {;
...
function createHash(inputstring): string {;
  ...
  return cryptocreateHash('sha256')update(inputdigest('hex')substring(0, 16););
};
