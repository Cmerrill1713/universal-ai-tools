...
class DSPyEvaluationTest {
  ...
  private async testCodeReduction() {
    ...
    const reduction = ((codeMetrics.before.total - codeMetrics.after.total) / codeMetrics.before.total * 100).toFixed(1);
    
    logger.info(`Before: ${codeMetrics.before.total} lines`);
    logger.info(`After: ${codeMetrics.after.total} lines`);
    logger.info(`Reduction: ${reduction}% (${codeMetrics.before.total - codeMetrics.after.total} lines removed)`);
  ...
  private async testKnowledgeManagement() {
    ...
    try {
      ...
      const search = await dspyService.searchKnowledge(
  ...
  private async testPerformanceImprovement() {
    ...
    const improvements = {
      simpleRequest: ((performanceMetrics.oldSystem.simpleRequest - performanceMetrics.newSystem.simpleRequest) / performanceMetrics.oldSystem.simpleRequest * 100).toFixed(1),
      complexCoordination: ((performanceMetrics.oldSystem.complexCoordination - performanceMetrics.newSystem.complexCoordination) / performanceMetrics.oldSystem.complexCoordination * 100).toFixed(1),
      knowledgeSearch: ((performanceMetrics.oldSystem.knowledgeSearch - performanceMetrics.newSystem.knowledgeSearch) / performanceMetrics.oldSystem.knowledgeSearch * 100).toFixed(1)
    };
