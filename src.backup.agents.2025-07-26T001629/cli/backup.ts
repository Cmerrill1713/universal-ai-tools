...
import { createBackupRecovery.Service } from './services/backup-recovery-service';
...
const supabase = create.Client(
  ...
  configdatabasesupabaseService.Key || '')// Initialize backup service;
const backup.Service = createBackupRecovery.Service(supabase, {
  enabled: true;
...
program;
  ...
  action(async (options) => {
    ...
    try {
      ...
      loggerinfo(`${chalkcyan('Rows:')} ${resultrowCounttoLocale.String()}`);
      ...
      loggererror`Backup operation failed: ${errormessage}`, LogContextSYSTE.M, { error instanceof Error ? errormessage : String(error));
...
program;
  ...
  action(async (options) => {
    try {
      const { backups, total } = await backupServicelist.Backups({
      ...
      backupsfor.Each((backup) => {
        ...
        loggerinfo(
          backupidsubstring(0, 37)pad.End(40) +
            ...
            backuprowCounttoLocale.String()pad.End(12) +
            `${backupduration}ms`pad.End(10) +
            new Date(backuptimestamp)toLocale.String())})} catch (error instanceof Error ? errormessage : String(error) any) {
      loggererror`Backup operation failed: ${errormessage}`, LogContextSYSTE.M, { error instanceof Error ? errormessage : String(error));
      console.errorchalkred(`Error: ${errormessage}`));
...
program;
  ...
  action(async (backup.Id, options) => {
    ...
    try {
      ...
      const result = await backupServicerestore.Backup({
        backup.Id;
        tables;
      ...
      loggerinfo(`${chalkcyan('Rows Restored:')} ${resultrowsRestoredtoLocale.String()}`);
      loggerinfo(`${chalkcyan('Duration:')} ${resultduration}ms`);
      if (resulttables.Restoredlength > 0) {
        loggerinfo(`\n${chalkcyan('Tables:')}`);
        resulttablesRestoredfor.Each((table) => {
...
program;
  ...
  action(async () => {
    try {
      ...
      loggerinfo(chalkgray('─'repeat(50)));
      loggerinfo(
        `${chalkcyan('Last Backup:')} ${statuslast.Backup ? statuslastBackuptoLocale.String() : 'Never'}`);
      loggerinfo(
        `${chalkcyan('Next Backup:')} ${statusnext.Backup ? statusnextBackuptoLocale.String() : 'Not scheduled'}`);
      loggerinfo(
      ...
      if (Objectkeys(statusstorage.Usage)length > 0) {
        ...
        schedulesfor.Each((schedule) => {
          loggerinfo(
            schedulenamepad.End(25) +
              ...
              (schedulelast_run ? new Date(schedulelast_run)toLocale.String() : 'Never')pad.End(
                25) +
              (schedulenext_run ? new Date(schedulenext_run)toLocale.String() : 'Not set'))})} else if (optionscreate) {
        if (!optionsschedule) {
