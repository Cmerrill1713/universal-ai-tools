...
import { EnhancedMemory.System } from './enhanced_memory_system';
...
export interface MemorySystem.Config {
  ...
  async initialize(): Promise<boolean> {
    ...
    while (thisinitialization.Attempts < (thisconfigmax.Retries || 3)) {
      try {
        ...
        thismemory.System = new EnhancedMemory.System(
          thisconfigsupabase;
          thisconfiglogger;
  ...
  async update.Memory(memory.Id: string, updates: any): Promise<unknown> {
    ...
    try {
      ...
      if (updatesimportance.Boost) {
        await thismemory.System!updateMemory.Importance(memory.Id, updatesimportance.Boost)};
