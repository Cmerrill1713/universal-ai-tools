...
import { circuitBreaker, withCircuitBreaker, httpWithCircuitBreaker } from './src/services/circuit-breaker.ts';
...
class RealCircuitBreakerDemo {
  ...
  async demoHttpCircuitBreaker() {
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      if (metrics) {
        console.log(`    📊 Circuit State: ${metrics.state}, Failures: ${metrics.failures}, Requests: ${metrics.requests}`);
      }
  ...
  async demoMonitoringAndMetrics() {
    ...
    if (allMetrics.length === 0) {
      ...
      allMetrics.forEach(metric => {
        ...
        console.log(`       Failure Rate: ${metric.requests > 0 ? ((metric.failures / metric.requests) * 100).toFixed(1) : 0}%`);
        console.log('');
      });
    }
  ...
  generateDemoReport() {
    ...
    console.log(`  Overall Success Rate: ${totalRequests > 0 ? ((totalSuccesses / totalRequests) * 100).toFixed(1) : 0}%`);
