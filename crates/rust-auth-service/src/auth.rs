...
impl AuthService {
    pub fn new(jwt_secret: String) -> Self {
        Self {
            jwt_secret,
            token_expiry_hours: 1, // Default 1 hour (15 minutes = 0.25 hours)
        }
    }
    ...
    /// Generate secure refresh token with proper rotation
    pub fn generate_refresh_token(&self, user_id: &str) -> Result<String, AuthServiceError> {
        use uuid::Uuid;
        ...
        // Create refresh token claims with 7-day expiry
        let refresh_claims = RefreshClaims {
            sub: user_id.to_string(),
            jti: token_id,
            exp: timestamp + (7 * 24 * 3600), // 7 days
            iat: timestamp,
        };
