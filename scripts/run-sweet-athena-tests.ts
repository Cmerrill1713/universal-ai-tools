...
class SweetAthenaTestRunner {
  ...
  async runTests(): Promise<void> {
    process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && console.log('🚀 Starting Sweet Athena Engineering Tests...\n');
  ...
  private parseTestOutput(output: string): void {
    ...
    try {
      ...
      process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && console.warn('Could not parse test output as JSON, using fallback parsing');
      this.parseFallback(output);
    }
  }

  private processTestData(data: unknown): void {
