    ...
    def print_failures(self):
        ...
        for i, failure in enumerate(failures, 1):
            ...
            if "Connection error" in str(failure['error']):
                print("   💡 Suggestion: Server may not be running. Check docker logs or start server.")
            elif failure.get('actual_status') == 404:
                print("   💡 Suggestion: Endpoint not found. Check router registration in api/app.py")
            elif failure.get('actual_status') == 500:
                print("   💡 Suggestion: Server error. Check module imports: python -c \"import api; print('ok')\"")
