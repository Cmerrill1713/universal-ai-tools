...
async function runInteractiveDemo() {
  console.clear();
  console.log('🤖 Personal AI Assistant - Interactive Demo');
  ...
  try {
    ...
    switch (intent.primaryAgent) {
      ...
      case 'multi':
        ...
        break;
      default:
        console.log("I'm not sure how to help with that. Try asking about calendar, files, system, or tools.");
    }
...
async function analyzeIntent(request) {
  ...
  if (requestLower.includes('schedule') || requestLower.includes('meeting') || requestLower.includes('calendar')) {
    return { primaryAgent: 'calendar', action: 'schedule' };
  }
  if (requestLower.includes('file') || requestLower.includes('download') || requestLower.includes('organize')) {
    return { primaryAgent: 'files', action: 'analyze' };
  }
  if (requestLower.includes('system') || requestLower.includes('performance') || requestLower.includes('app')) {
    return { primaryAgent: 'system', action: 'check' };
  }
  if (requestLower.includes('scrape') || requestLower.includes('website') || requestLower.includes('web')) {
...
async function handleSystemRequest(request, intent) {
  ...
  try {
    const cpu = execSync("ps -A -o %cpu | awk '{s+=$1} END {print s}'", { encoding: 'utf8' }).trim();
    ...
    // Get top apps
    const apps = execSync("ps aux | sort -k3 -r | head -5 | awk '{print $11}'", { encoding: 'utf8' })
      .trim()
      .split('\n')
      .slice(1); // Skip header
