...
async function demoSystemControlAgent() {
  try {
    // Get system metrics
    const cpuUsage = execSync("ps -A -o %cpu | awk '{s+=$1} END {print s}'", { encoding: 'utf8' }).trim();
    const memoryInfo = execSync("vm_stat | grep 'Pages free' | awk '{print $3}'", { encoding: 'utf8' }).trim();
    ...
    // Get running apps
    const apps = execSync("osascript -e 'tell application \"System Events\" to get name of every process whose background only is false'", { encoding: 'utf8' })
      .trim()
      .split(', ')
      .slice(0, 5);
