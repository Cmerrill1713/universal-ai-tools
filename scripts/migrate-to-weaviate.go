...
func main() {
	...
	case "memories":
		...
		// Verify migration
		if err := verifyMigration(client, "Memory"); err != nil {
			log.Printf("Warning: Migration verification failed: %v", err)
		}
	case "documents":
		migrateDocuments(db, client, stats)
		// Verify migration
		if err := verifyMigration(client, "Document"); err != nil {
			log.Printf("Warning: Migration verification failed: %v", err)
		}
	case "all":
		migrateMemories(db, client, stats)
		if err := verifyMigration(client, "Memory"); err != nil {
			log.Printf("Warning: Memory migration verification failed: %v", err)
		}
		migrateDocuments(db, client, stats)
		if err := verifyMigration(client, "Document"); err != nil {
			log.Printf("Warning: Document migration verification failed: %v", err)
		}
	default:
