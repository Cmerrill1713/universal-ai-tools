...
console.log('🖥️  Testing Resource Monitoring...\n');
...
function testMemoryMonitoring() {
  ...
  console.log(`  ✓ System Memory: ${(usedMem / 1024 / 1024 / 1024).toFixed(2)}GB / ${(totalMem / 1024 / 1024 / 1024).toFixed(2)}GB (${memoryPercentage.toFixed(1)}%)`);
...
function testNetworkMonitoring() {
  ...
  const interfaces = Object.keys(networkInterfaces).filter(name => 
    !name.startsWith('lo') && networkInterfaces[name].some(iface => !iface.internal)
  );
...
function testSystemHealth() {
  ...
  console.log(`  ${memoryHealthy ? '✅' : '⚠️ '} Memory: ${memPercentage.toFixed(1)}% ${memoryHealthy ? 'OK' : 'HIGH'}`);
  ...
  console.log(`  ${cpuHealthy ? '✅' : '⚠️ '} CPU Load: ${(normalizedLoad * 100).toFixed(1)}% ${cpuHealthy ? 'OK' : 'HIGH'}`);
...
function testConnectionMonitoring() {
  ...
  Object.entries(connections).forEach(([name, conn]) => {
    console.log(`  ${conn.healthy ? '✅' : '❌'} ${name}: ${conn.active} active, ${conn.idle} idle (${conn.total} total)`);
  });
