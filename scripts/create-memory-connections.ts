...
const connectionRules: ConnectionRule[] = [
  {
    ...
    condition: (mem1, mem2) => {
      ...
      return tech1.length > 0 && tech2.length > 0 && tech1.some((t) => tech2.includes(t));
    },
  ...
  },
  {
    ...
    condition: (mem1, mem2) => {
      const isImplementation1 =
        mem1.memory_type.includes('implementation') ||
        mem1.content.toLowerCase().includes('implement');
      const isImplementation2 =
        mem2.memory_type.includes('implementation') ||
        mem2.content.toLowerCase().includes('implement');
      return isImplementation1 && isImplementation2 && shareKeywords(mem1, mem2, 2);
    },
  ...
  },
  {
    name: 'Best Practices',
    condition: (mem1, mem2) => {
      const isBestPractices1 =
        mem1.memory_type.includes('best_practices') ||
        mem1.content.toLowerCase().includes('best practice');
      const isBestPractices2 =
        mem2.memory_type.includes('best_practices') ||
        mem2.content.toLowerCase().includes('best practice');
      return isBestPractices1 && isBestPractices2 && shareKeywords(mem1, mem2, 1);
    },
  ...
  },
  {
    ...
    condition: (mem1, mem2) => {
      const isOrchestration1 =
        mem1.memory_type.includes('agent_orchestration') ||
        mem1.service_id === 'agent-orchestration-system';
      const isOrchestration2 =
        mem2.memory_type.includes('agent_orchestration') ||
  ...
  },
  {
    ...
    condition: (mem1, mem2) => {
      ...
      const hasFramework1 = frameworks.some(
        (fw) =>
          mem1.memory_type.toLowerCase().includes(fw) || mem1.content.toLowerCase().includes(fw)
      );
      const hasFramework2 = frameworks.some(
        (fw) =>
          mem2.memory_type.toLowerCase().includes(fw) || mem2.content.toLowerCase().includes(fw)
      );
      return hasFramework1 && hasFramework2 && getCommonFramework(mem1, mem2) !== null;
    },
...
function getCommonFramework(mem1: MemoryItem, mem2: MemoryItem): string | null {
  ...
  for (const framework of frameworks) {
    const hasFramework1 =
      mem1.memory_type.toLowerCase().includes(framework) ||
      mem1.content.toLowerCase().includes(framework);
    const hasFramework2 =
      mem2.memory_type.toLowerCase().includes(framework) ||
...
async function loadMemories(): Promise<MemoryItem[]> {
  process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && console.log('📥 Loading memories from database...');
...
async function createConnection(
  ...
  try {
    const { error } = await supabase.from('memory_connections').insert({
...
function getConnectionContext(mem1: MemoryItem, mem2: MemoryItem): unknown {
  return {
...
async function generateConnectionReport(): Promise<void> {
  ...
  if (!hubError && hubMemories) {
    ...
    hubMemories.forEach((hub: unknown, index: number) => {
  ...
  const totalMemories =
    (await supabase.from('ai_memories').select('id', { count: 'exact' })).count || 0;
