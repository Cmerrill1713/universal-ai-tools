...
(function () {
  process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && console.log('🔧 Patching Supabase Studio for Ollama...');
  ...
  window.fetch = async function (url, options = {}) {
    ...
    if (
      actualUrl.includes('api.openai.com') ||
      ...
      try {
        ...
        process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && console.error('❌ Error processing request:', error);
  ...
  function addWorkingAIButton() {
    const checkForSQLEditor = setInterval(() => {
      ...
      if (editor && toolbar && !document.querySelector('#ollama-ai-btn')) {
        ...
        button.className =
          'bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm flex items-center gap-1';
...
- Original AI button should now use Ollama
  ...
  fetch('http://localhost:8080/api/tags')
    ...
    .catch(() => process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && console.warn('🔴 Ollama not accessible'));
})();
