...
class ClaudeContextGenerator {
  ...
  async findMockImplementations() {
    ...
    this.log(`  ✓ Found ${this.context.mockImplementations.length} files with mock implementations`, colors.green);
  }
  ...
  async scanSecurityVulnerabilities() {
    ...
    this.log(`  ⚠️  Found ${this.context.securityVulnerabilities.length} potential security issues`, colors.yellow);
  }
  ...
  async analyzeDatabaseMigrations() {
    ...
    try {
      ...
      if (this.context.databaseMigrations.potentialDuplicates.length > 0) {
        this.log(`  ⚠️  ${this.context.databaseMigrations.potentialDuplicates.length} potential duplicate migrations`, colors.yellow);
      }
    } catch (error) {
  ...
  async analyzeTestCoverage() {
    ...
    try {
      const coverageExists = await fs.access(path.join(rootDir, 'coverage')).then(() => true).catch(() => false);
      ...
      if (this.context.testCoverage.untestedDirectories.length > 0) {
        this.log(`  ⚠️  ${this.context.testCoverage.untestedDirectories.length} directories without tests`, colors.yellow);
      }
    } catch (error) {
  ...
  generateMarkdown() {
    ...
    md.push(`- **Database Migrations**: ${this.context.databaseMigrations.total} (${this.context.databaseMigrations.potentialDuplicates.length} potential duplicates)`);
    ...
    if (this.context.mockImplementations.length > 0) {
      ...
      this.context.mockImplementations.forEach(mock => {
        md.push(`- \`${mock.file}\`: ${mock.imports} imports, ${mock.functions} functions, ${mock.todos} TODOs`);
      });
      md.push('');
    }
    ...
    md.push(`- Test files: ${this.context.testCoverage.testFiles}`);
    md.push(`- Coverage report available: ${this.context.testCoverage.coverageAvailable ? 'Yes' : 'No'}`);
  ...
  async saveContext() {
    ...
    await fs.writeFile(path.join(contextDir, `context-${timestamp}.json`), JSON.stringify(this.context, null, 2));
