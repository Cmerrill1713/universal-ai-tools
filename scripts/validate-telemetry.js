...
class TelemetryValidator {
  ...
  async validateTracing() {
    ...
    const tests = [
      {
        ...
        test: async () => {
          ...
          return {
            ...
            message: result.status === 200 ? 'Jaeger UI accessible' : `Jaeger UI error: ${result.status}`,
      ...
      },
      {
        ...
        test: async () => {
          ...
          return {
            passed: result.status === 200,
            message: result.status === 200 ? 'OTLP Collector healthy' : `OTLP Collector error: ${result.status}`,
      ...
      },
      {
        ...
        test: async () => {
          ...
          const jaegerResult = await this.makeRequest(`${JAEGER_URL}/api/traces?service=universal-ai-tools&limit=10`);
    ...
    this.results.tracing.status = passedTests === tests.length ? 'healthy' : 
  ...
  async validateMetrics() {
    ...
    const tests = [
      {
        ...
        test: async () => {
          ...
          return {
            passed: result.status === 200,
            message: result.status === 200 ? 'Prometheus UI accessible' : `Prometheus error: ${result.status}`,
            details: result.error
          };
        }
      },
      {
        ...
        test: async () => {
          ...
          return {
            ...
            message: result.status === 200 ? 'Application metrics available' : 'Application metrics unavailable',
            details: result.status === 200 ? `Metrics size: ${result.data?.length || 0} bytes` : result.error
          };
        }
      },
      {
        ...
        test: async () => {
          ...
          return {
            ...
            details: { found: foundMetrics, missing: customMetrics.filter(m => !foundMetrics.includes(m)) }
          };
        }
      },
      {
        ...
        test: async () => {
          ...
          return {
            ...
            message: queryResult.status === 200 ? 'Metrics accurately reflecting requests' : 'Metrics not updating',
    ...
    this.results.metrics.status = passedTests === tests.length ? 'healthy' : 
                                  passedTests > 0 ? 'degraded' : 'unhealthy';
  }
  ...
  async validateHealthChecks() {
    ...
    const tests = [
      {
        ...
        test: async () => {
          ...
          return {
            passed: result.status === 200,
            message: result.status === 200 ? 'Health endpoint responsive' : `Health endpoint error: ${result.status}`,
            details: result.data || result.error
          };
        }
      },
      {
        ...
        test: async () => {
          ...
          return {
            ...
            message: result.status === 200 ? 'Detailed health report available' : 'Detailed health report unavailable',
    ...
    this.results.healthChecks.status = passedTests === tests.length ? 'healthy' : 
                                       passedTests > 0 ? 'degraded' : 'unhealthy';
  }
  ...
  async validateResourceMonitoring() {
        ...
        test: async () => {
          return new Promise((resolve) => {
            ...
            cmd.on('close', (code) => {
              resolve({
                passed: code === 0,
                message: code === 0 ? 'Connection pool monitoring functional' : 'Connection pool monitoring failed',
                details: output
              });
            });
    ...
    this.results.resourceMonitoring.status = passedTests === tests.length ? 'healthy' : 
                                            passedTests > 0 ? 'degraded' : 'unhealthy';
  }
  ...
  async validateDashboards() {
    ...
    const tests = [
      {
        ...
        test: async () => {
          ...
          return {
            passed: result.status === 200,
            message: result.status === 200 ? 'Grafana accessible' : `Grafana error: ${result.status}`,
            details: result.data || result.error
          };
        }
      },
      {
        ...
        test: async () => {
          ...
          return {
            ...
            message: result.status === 200 ? `Found ${result.data?.length || 0} dashboards` : 'Cannot access dashboards',
            details: result.data || result.error
          };
        }
      },
      {
        ...
        test: async () => {
          ...
          return {
            ...
            message: hasPrometheus ? 'Prometheus data source configured' : 'Prometheus data source missing',
    ...
    this.results.dashboards.status = passedTests === tests.length ? 'healthy' : 
                                     passedTests > 0 ? 'degraded' : 'unhealthy';
  }
  ...
  generateRecommendations() {
    ...
    this.results.overall.status = this.results.overall.score >= 80 ? 'healthy' :
                                  this.results.overall.score >= 60 ? 'degraded' : 'unhealthy';
    ...
    if (this.results.overall.score < 70) {
      this.results.recommendations.push({
        ...
        recommendation: 'Consider reviewing monitoring infrastructure and addressing critical issues',
  ...
  getRecommendation(category, testName) {
    const recommendations = {
      tracing: {
        'Jaeger UI Accessibility': 'Ensure Jaeger container is running and accessible on port 16686',
        'OTLP Collector Health': 'Check OpenTelemetry Collector configuration and ensure it\'s running',
        'Trace Collection': 'Verify trace instrumentation is properly configured in the application',
      ...
      metrics: {
        ...
        'Custom Metrics Validation': 'Check custom metrics are being properly registered and exported',
    ...
    return recommendations[category]?.[testName] || 'Review configuration and logs for this component';
  }
