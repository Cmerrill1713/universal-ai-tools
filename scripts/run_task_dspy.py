...
def main():
    ...
    print(f"🚀 Processing task with DSPy: {task}")

    # Use DSPy cognitive chain for planning
    print("🧠 DSPy cognitive reasoning...")
    dspy_result = call_dspy_pipeline(task)

    if "error" in dspy_result:
        print(f"❌ DSPy pipeline failed: {dspy_result['error']}")
        sys.exit(1)
    ...
    if not patches:
        print("❌ No patches generated by DSPy")
        sys.exit(1)

    # Apply patches from DSPy implementer
    print(f"⚙️ Applying {len(patches)} patches...")
    try:
    ...
    while not ok and attempts > 0:
        print(f"🔧 DSPy auto-fixing (attempts left: {attempts})...")
        ...
        if fix_patches:
            try:
                ...
                print(f"❌ Failed to apply fix patches: {e}")
                break

        attempts -= 1

    if not ok:
        print("❌ Validation still failing after DSPy fixes")
    ...
    if review_result.get("decision") != "approve":
        print(f"❌ DSPy reviewer rejected: {review_result.get('notes', 'No notes')}")
        sys.exit(2)

    print("✅ All checks passed and DSPy reviewer approved!")
    ...
    print(f"📊 Learning example stored: {json.dumps(learning_example, indent=2)}")
