...
result = model_selector.select_model_for_task(
    "Complex reasoning and programming tasks", {"complexity": "complex", "prefer_quality": True}
)
if result:
    lm, profile = result
        ...
        for problem_name, problem_data in TEST_PROBLEMS.items():
            ...
            try:
                ...
                results_table.add_row(
                    "Basic Predict", self._truncate(basic_result.answer, 100), f"{basic_score:.0%}"
                )
            ...
            try:
                ...
                cot_score = self._score_response(
                    f"{cot_result.reasoning}\n{cot_result.answer}", problem_data["type"]
                )
                results_table.add_row(
                    "Chain of Thought",
            ...
            if problem_data["type"] == "reasoning":
                try:
                    ...
                    results_table.add_row(
                        "ReAct", self._truncate(react_result.answer, 100), f"{react_score:.0%}"
                    )
                except Exception as e:
...
async def test_mipro_optimization():
    ...
    optimizer = MIPROv2(metric=metric, num_iterations=5, temperature_range=(0.5, 1.0))

    optimized = optimizer.compile(dspy.ChainOfThought(MathSolver), trainset=trainset)
    ...
    comparison_table.add_row(
        "Approach",
        (
            result_before.approach[:50] + "..."
            if len(result_before.approach) > 50
            else result_before.approach
        ),
        (
            result_after.approach[:50] + "..."
            if len(result_after.approach) > 50
            else result_after.approach
        ),
    )
    ...
    comparison_table.add_row(
        "Solution",
        (
            result_before.solution[:50] + "..."
            if len(result_before.solution) > 50
            else result_before.solution
        ),
        (
            result_after.solution[:50] + "..."
            if len(result_after.solution) > 50
            else result_after.solution
        ),
    )

    comparison_table.add_row("Answer", result_before.answer, result_after.answer)
