...
const log = (color, message) => process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && console.log(`${colors[color]}${message}${colors.reset}`);
...
class UniversalAITestSuite {
  ...
  async run() {
    try {
      ...
      process.env.NODE_ENV === 'development' && process.env.NODE_ENV === 'development' && console.error(error);
    } finally {
      await this.cleanup();
    }
  }
  ...
  async testBackendAPI() {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        log(success ? 'green' : 'red', `${success ? '✅' : '❌'} ${endpoint.name}: ${status}`);
      } catch (error) {
        this.results.backend.push({
          name: endpoint.name,
          success: false,
  ...
  async testSweetAthenaIntegration() {
    ...
    try {
      ...
      const chatExists =
        (await this.page
          .locator('[class*="sweet-athena"], [class*="SweetAthena"], [data-testid*="athena"]')
          .count()) > 0;
      ...
      log(
        chatExists ? 'green' : 'yellow',
        `${chatExists ? '✅' : '⚠️'} Sweet Athena Chat: ${chatExists ? 'Found' : 'Not visible'}`
      );

      // Look for AI Assistant Avatar
      const avatarExists =
        (await this.page.locator('[class*="avatar"], [class*="Assistant"], canvas').count()) > 0;
      this.results.sweetAthena.push({
        name: 'Avatar Component',
        success: avatarExists,
      });
      log(
        avatarExists ? 'green' : 'yellow',
      ...
      if (inputExists) {
        ...
        const sendButton = this.page
          .locator('button[type="submit"], button:has-text("Send"), button:has-text("►")')
          .first();
  ...
  async generateReport() {
    ...
    const report = {
      ...
      summary: {
        backend:
          this.results.backend.filter((r) => r.success).length + '/' + this.results.backend.length,
        frontend:
          this.results.frontend.filter((r) => r.success).length +
          '/' +
          this.results.frontend.length,
        sweetAthena:
          this.results.sweetAthena.filter((r) => r.success).length +
          '/' +
          this.results.sweetAthena.length,
        integration:
          this.results.integration.filter((r) => r.success).length +
          '/' +
