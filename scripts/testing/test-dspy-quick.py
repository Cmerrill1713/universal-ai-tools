...
configs_to_test = [
    {
        ...
        "config": lambda: dspy.LM(
            "openai/qwen2.5-coder-14b-instruct-mlx",
    ...
    },
    {
        "name": "Ollama (Stock)",
        "config": lambda: dspy.LM(
            "ollama_chat/llama3.2:3b", api_base="http://localhost:11434", api_key=""
        ),
    },
    {
        "name": "Ollama (Docker Proxy)",
        "config": lambda: dspy.LM(
            "ollama_chat/llama3.2:3b", api_base="http://localhost:8080", api_key=""
        ),
    },
    {
        "name": "LM Studio",
        "config": lambda: dspy.LM(
            "openai/local-model", api_base="http://localhost:1234/v1", api_key="lm-studio"
        ),
    },
]
...
for config in configs_to_test:
    try:
        ...
        print("✅ DSPy Signature Test Successful!")
        print("Question: What is the purpose of MIPRO in DSPy?")
