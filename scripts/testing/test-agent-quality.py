...
class QualityEvaluator:
    ...
    def evaluate_response(self, response: str, criteria: List[str]) -> Dict[str, Any]:
        ...
        scores["overall"] = (
            scores["length"] * 0.3 + scores["criteria_score"] * 0.5 + scores["structure"] * 0.2
        )

        return scores


async def test_model_quality(
    provider: str, model_name: str, test_name: str, test_data: Dict
) -> Dict[str, Any]:
    ...
    for test_name, test_data in QUALITY_TESTS.items():
        ...
        for provider, model_name in test_models:
            ...
            if result["success"]:
                ...
                results_table.add_row(
                    model_name, "N/A", "0", "0%", f"Error: {result['error'][:50]}..."
                )
...
async def main():
    ...
    for rank, (model, avg_score) in enumerate(ranked_models, 1):
        ...
        summary_table.add_row(str(rank), model, f"{avg_score:.0%}", best_for)
