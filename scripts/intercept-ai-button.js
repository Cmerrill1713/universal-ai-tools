...
(function() {
  console.log('🔍 Setting up AI Button interceptor...');
    ...
    if (typeof url === 'string') {
      ...
      if (url.includes('openai.com') || 
        ...
        try {
          ...
          const sql = ollamaData.response?.replace(/```sql\n?/gi, '').replace(/```\n?/gi, '').trim() || '';
  ...
  function findAIButton() {
    ...
    for (const button of buttons) {
      ...
      const hasAIText = text.includes('AI') || text.includes('Assistant') || text.includes('Generate');
      const hasAIIcon = button.querySelector('[data-icon="sparkles"]') || 
