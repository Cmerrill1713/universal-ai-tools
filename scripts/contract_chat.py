...
def test_chat_contract(base_url: str):
    ...
    try:
        ...
        if not has_id:
            print("❌ /chat missing 'id' field in response")
            print(f"   Response: {json.dumps(j, indent=2)[:400]}")
            return False

        if not has_message:
            print("❌ /chat missing 'message', 'content', or 'response' field")
            print(f"   Response: {json.dumps(j, indent=2)[:400]}")
            return False
        ...
        if not has_metadata:
            print("⚠️  /chat missing optional metadata (tokens/latency/model)")
