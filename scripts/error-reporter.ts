...
class ErrorReporter {
  ...
  private parseTypeScriptErrors(): ErrorInfo[] {
    try {
      ...
      console.error('Error parsing TypeScript errors:', error);
      return [];
    }
  }
  ...
  private printSummary(report: any): void {
    ...
    const priorityGroups = [1, 2, 3, 4, 5, 6, 7].map(priority => {
      const files = report.filesByPriority.filter((f: any) => f.priority === priority);
      const totalErrors = files.reduce((sum: number, f: any) => sum + f.errorCount, 0);
