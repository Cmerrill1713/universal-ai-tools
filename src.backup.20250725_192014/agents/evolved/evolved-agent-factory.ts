...
export class EvolvedAgent.Factory {
  ...
  static createEvolved.Agent(
    ...
    class DynamicEvolved.Agent extends EvolvedBaseAgent implements EvolvedAgent.Wrapper {
      public wrapped.Agent: BaseAgent;
      constructor() {
  ...
  static async evolve.Registry(
    ...
    for (const agent.Name of agent.Names) {
      try {
        ...
        const original.Agent = await registrygetAgent(agent.Name);
        if (!original.Agent) {
  ...
  static createSpecializedEvolved.Agent(
    ...
    return class SpecializedEvolved.Agent extends EvolvedBaseAgent {
      ...
      private apply.Specialization(context: AgentContext): AgentContext {
        return {
          .context;
          metadata: {
            .contextmetadata;
