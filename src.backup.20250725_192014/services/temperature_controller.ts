...
export class Temperature.Controller {
  ...
  public async getOptimal.Params(
    ...
    const profile = thisgetTemperature.Profile(task.Type);
    ...
    if (context?complexity) {
      const complexity.Adjustment = thisgetComplexity.Adjustment(contextcomplexity, task.Type);
    ...
    if (context?user.Preference !== undefined) {
      const user.Adjustment = thisgetUserPreference.Adjustment(contextuser.Preference, profile);
    ...
    if (context?previous.Attempts && contextprevious.Attempts > 0) {
      const retry.Adjustment = thisgetRetry.Adjustment(contextprevious.Attempts);
    ...
    if (context?quality.Requirement) {
      const quality.Adjustment = thisgetQuality.Adjustment(contextquality.Requirement, task.Type);
      temperature += quality.Adjustmentadjustment;
      adjustmentspush(quality.Adjustment)}// Apply learned optimizations;
    const learned.Temp = await thisgetLearned.Temperature(task.Type);
    ...
    if (thisshouldRunAB.Test()) {
      const variation = thisgetABTest.Variation(temperature, profile);
      adjustmentspush({
        factor: 'ab_testing';
    ...
    const params = thiscalculateComplementary.Params(temperature, task.Type);
    loggerinfo(
...
;
  ...
  private getUserPreference.Adjustment(
    user.Preference: number;
  ...
  private getRetry.Adjustment(attempts: number): Temperature.Adjustment {
...
;
  ...
  private async getLearned.Temperature(task.Type: string): Promise<number | null> {
  ...
  private calculateComplementary.Params(temperature: number, task.Type: string): Generation.Params {
...
;
  ...
  private shouldRunAB.Test(): boolean {
    return thisabTest.Configsenabled && Mathrandom() < thisabTestConfigssample.Rate}/**
   * Get A/B test temperature variation*/
  private getABTest.Variation(base.Temp: number, profile: Temperature.Profile): number {
    const variation = (Mathrandom() - 0.5) * 2 * thisabTestConfigsvariation.Range;
  ...
  public async record.Result(
    ...
    const profile.Key = thisgetTemperature.Profile(task.Type)task.Type;
    ...
    if (quality.Score !== undefined) {
      ...
      metricsavgQuality.Score = alpha * quality.Score + (1 - alpha) * metricsavgQuality.Score}// Update optimal temperature using gradient descent;
    if (success && quality.Score !== undefined) {
      const learning.Rate = 0.01;
      const gradient =
        (quality.Score - metricsavgQuality.Score) * (temperature - metricsoptimal.Temp);
      ...
      if (metrics) {
        ...
        if (total > 0) {
          recommendationperformance = {
            ...
            avg.Quality: metricsavgQuality.Score;
...
;
  ...
  private async loadTask.Metrics(): Promise<void> {
    try {
      ...
      if (data) {
        datafor.Each((record) => {
          thistask.Metricsset(recordtask_type, {
            ...
            avgQuality.Score: recordavg_quality_score;
  ...
  private async saveTask.Metrics(): Promise<void> {
    try {
      const records = Arrayfrom(thistask.Metricsentries())map(([_, metrics]) => ({
        ...
        avg_quality_score: metricsavgQuality.Score;
        optimal_temp: metricsoptimal.Temp;
        last_updated: metricslastUpdatedtoISO.String()}));
      ...
      if (error instanceof Error ? errormessage : String(error){
        loggererror('Failed to save temperature metrics:', error instanceof Error ? errormessage : String(error)  }} catch (error) {
