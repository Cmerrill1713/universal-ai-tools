...
import { SweetAthenaIntegration.Service } from './sweet-athena-integration';
...
export interface AuthenticatedWebSocket extends Web.Socket {
  user.Id?: string;
  is.Authenticated?: boolean;
  last.Ping?: number;
  sweetAthena.Service?: SweetAthenaIntegration.Service;
};
...
export interface SweetAthenaWSConfig {
  ...
  auth.Timeout?: number;
  messageRate.Limit?: number;
};

export class SweetAthenaWebSocket.Service extends Event.Emitter {
  private wss: WebSocket.Server | null = null;
  private clients: Map<string, AuthenticatedWeb.Socket> = new Map();
  private user.Connections: Map<string, Set<AuthenticatedWeb.Socket>> = new Map();
  private config: Required<SweetAthenaWS.Config>
  private ping.Interval: NodeJS.Timeout | null = null;
  private isRunning = false// Rate limiting;
  private message.Counts: Map<string, { count: number, reset.Time: number }> = new Map();
  constructor(config: SweetAthenaWS.Config = {}) {
    super();
    thisconfig = {
      ...
      messageRate.Limit: configmessageRate.Limit || 60, // messages per minute}}/**
  ...
  async start(server?: any): Promise<void> {
    try {
      ...
      thiswss = server? new WebSocket.Server({ server, path: '/api/sweet-athena/ws' }): new WebSocket.Server({ port: thisconfigport })// Setup connection handler;
      thiswsson('connection', thishandle.Connectionbind(this))// Setup ping interval;
      thisstartPing.Interval();
      thisisRunning = true;
      thisemit('started');
  ...
  async stop(): Promise<void> {
    try {
      ...
      thisisRunning = false// Stop ping interval;
  ...
  private async handle.Connection(ws: AuthenticatedWeb.Socket, req: any): Promise<void> {
    const client.Id = thisgenerateClient.Id();
  ...
  private async handle.Message(
    ws: AuthenticatedWeb.Socket;
    client.Id: string;
    data: WebSocketRaw.Data): Promise<void> {
    try {
      // Check rate limiting;
      if (!thischeckRate.Limit(wsuser.Id || client.Id)) {
        thissend.Error(ws, 'Rate limit exceeded');
        return}// Parse message;
      const message: WS.Message = JSO.N.parse(datato.String());
      ...
      switch (messagetype) {
        ...
        case 'personality_change':
          await thishandlePersonality.Change(ws, message);
          break;
        case 'clothing_update':
          await thishandleClothing.Update(ws, message);
          break;
        case 'state_change':
          await thishandleState.Change(ws, message);
          break;
        case 'voice_interaction':
          await thishandleVoice.Interaction(ws, message);
          break;
        default:
  ...
  private async handle.Auth(
    ws: AuthenticatedWeb.Socket;
    client.Id: string;
    message: WS.Message): Promise<void> {
    try {
      const { token } = messagedata || {};
      if (!token) {
        thissend.Error(ws, 'Authentication token required');
        return}// Verify JW.T token;
      const decoded = jwtverify(token, process.envJWT_SECRE.T!) as any;
      ...
      try {
        wssweetAthena.Service = new SweetAthenaIntegration.Service(supabase);
        await wssweetAthena.Serviceinitialize(user.Id)// Setup service event handlers;
        thissetupServiceEvent.Handlers(ws)} catch (service.Error) {
        loggererror('Failed to initialize Sweet Athena service:', undefined, service.Error);
        wssweetAthena.Service = undefined}// Track user connections;
      ...
      thissend.Message(ws, {
        ...
        data: {
          ...
          user.Id;
          sweetAthena.Enabled: !!wssweetAthena.Service;
          current.State: wssweetAthena.Service?getCurrent.State();
        }});
  ...
  private setupServiceEvent.Handlers(ws: AuthenticatedWeb.Socket): void {
    if (!wssweetAthena.Service) return;
    wssweetAthena.Serviceon('personality.Changed', (data) => {
      thissend.Message(ws, {
    ...
    wssweetAthena.Serviceon('clothing.Changed', (data) => {
      thissend.Message(ws, {
    ...
    wssweetAthena.Serviceon('avatarState.Changed', (state) => {
      thissend.Message(ws, {
        type: 'state_change';
        data: {
          state;
    ...
    wssweetAthena.Serviceon('avatar.Connected', () => {
      thissend.Message(ws, {
    ...
    wssweetAthena.Serviceon('avatar.Disconnected', () => {
      thissend.Message(ws, {
        type: 'avatar_response';
        data: {
          event: 'disconnected';
   ...
   * Handle ping message*/
  private async handle.Ping(ws: AuthenticatedWeb.Socket, message: WS.Message): Promise<void> {
    wslast.Ping = Date.now();
    thissend.Message(ws, {
      type: 'pong';
      id: messageid;
  ...
  private async handlePersonality.Change(
    ws: AuthenticatedWeb.Socket;
    message: WS.Message): Promise<void> {
    try {
      ...
      if (!wssweetAthena.Service) {
        thissend.Error(ws, 'Sweet Athena service not available');
        return};

      await wssweetAthenaServiceset.Personality(personality as Personality.Mode);
      thissend.Message(ws, {
        type: 'success';
        id: messageid;
        data: {
  ...
  private async handleClothing.Update(
    ws: AuthenticatedWeb.Socket;
    message: WS.Message): Promise<void> {
    try {
      ...
      await wssweetAthenaServicesetClothing.Level(level);
      thissend.Message(ws, {
        type: 'success';
        id: messageid;
        data: {
  ...
  private async handleState.Change(ws: AuthenticatedWeb.Socket, message: WS.Message): Promise<void> {
    try {
      ...
      if (interaction?mode) {
        await wssweetAthenaServicesetInteraction.Mode(interactionmode, interactioncontext || '')}// Update user engagement;
      if (interaction?user.Engagement !== undefined) {
        wssweetAthenaServiceupdateUser.Engagement(interactionuser.Engagement)};

      const current.State = wssweetAthenaServicegetCurrent.State();
      thissend.Message(ws, {
        type: 'success';
        id: messageid;
        data: {
  ...
  private async handleVoice.Interaction(
    ws: AuthenticatedWeb.Socket;
    message: WS.Message): Promise<void> {
    try {
      ...
      if (text) {
        ...
        const response = {
          ...
          data: {
            ...
            personality: wssweetAthenaServicegetCurrent.State()personalitymode;
            timestamp: new Date()toISO.String();
          }};
  ...
  private handle.Error(ws: AuthenticatedWeb.Socket, client.Id: string, error instanceof Error ? errormessage : String(error) Error): void {
  ...
  private cleanup(ws: AuthenticatedWeb.Socket, client.Id: string): void {
    ...
    if (wssweetAthena.Service) {
      wssweetAthena.Servicedestroy()}}/**
   * Send message to Web.Socket*/
  private send.Message(ws: AuthenticatedWeb.Socket, message: WS.Message): void {
    if (wsready.State === WebSocketOPE.N) {
      messagetimestamp = messagetimestamp || new Date()toISO.String();
      wssend(JSO.N.stringify(message));
    }}/**
   * Send errormessage*/
  private send.Error(ws: AuthenticatedWeb.Socket, error.Message: string, message.Id?: string): void {
    thissend.Message(ws, {
  ...
  private broadcastTo.User(
    user.Id: string;
    message: WS.Message;
    exclude.Ws?: AuthenticatedWeb.Socket): void {
    const user.Connections = thisuser.Connectionsget(user.Id);
    if (!user.Connections) return;
    for (const ws of user.Connections) {
      if (ws !== exclude.Ws && wsready.State === WebSocketOPE.N) {
        thissend.Message(ws, message)}}}/**
   * Broadcast message to all authenticated connections*/
  public broadcast(message: WS.Message): void {
    for (const [client.Id, ws] of thisclients) {
      if (wsis.Authenticated && wsready.State === WebSocketOPE.N) {
        thissend.Message(ws, message)}}}/**
   * Close connection with code and reason*/
  private close.Connection(ws: AuthenticatedWeb.Socket, code: number, reason: string): void {
    if (wsready.State === WebSocketOPE.N || wsready.State === WebSocketCONNECTIN.G) {
      wsclose(code, reason)}}/**
   * Generate unique client I.D*/
  private generateClient.Id(): string {
    return `client_${Date.now()}_${Mathrandom()to.String(36)substr(2, 9)}`}/**
   * Check rate limiting*/
  private checkRate.Limit(identifier: string): boolean {
    const now = Date.now();
    ...
    if (currentcount >= thisconfigmessageRate.Limit) {
      return false};

    currentcount++
    return true}/**
   * Start ping interval to keep connections alive*/
  private startPing.Interval(): void {
      ...
      for (const [client.Id, ws] of thisclients) {
        if (wslast.Ping && wslast.Ping < stale.Threshold) {
          loggerwarn('Closing stale Web.Socket connection', undefined, {
            client.Id;
            user.Id: wsuser.Id});
          thisclose.Connection(ws, 1001, 'Connection stale')} else if (wsready.State === WebSocketOPE.N) {
  ...
  public get.Stats(): any {
    return {
      ...
      uptime: thisisRunning ? Date.now() : 0;
    }}};

export default SweetAthenaWebSocket.Service;