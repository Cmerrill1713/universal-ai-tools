...
export class Cache.Manager {
  ...
  public async ttl(key: string, options: Cache.Options = {}): Promise<number> {
    try {
      ...
      loggererror('Cache TT.L error instanceof Error ? errormessage : String(error)', error);
      return -1}};
    ...
    try {
      ...
      return valuesmap((value) => {
        if (value !== null) {
          ...
          return optionscompress ? thisdecompress(value) : JSO.N.parse(value)} else {
          thisupdate.Stats('miss', 0);
          return null}})} catch (error) {
      loggererror('Cache get.Multiple error instanceof Error ? errormessage : String(error)', error);
      return keysmap(() => null)}};
  ...
  public async set.Multiple(
    ...
    try {
      ...
      await multiexec();
      return true} catch (error) {
      loggererror('Cache set.Multiple error instanceof Error ? errormessage : String(error)', error);
      return false}};
