...
export class Auth.Router {
  ...
  private supabase;
  private jwt.Service: JWTAuth.Service;
  constructor() {
    thisrouter = Router();
    thissupabase = create.Client(configsupabaseurl, configsupabaseservice.Key);
    thisjwt.Service = new JWTAuth.Service(thissupabase);
    thissetup.Routes();
    thissetupRate.Limiting()};

  private setupRate.Limiting() {
    ...
    const auth.Limiter = rate.Limit({
      ...
      skip: (req) => {
        ...
        return securityConfigrate.Limitingwhitelistincludes(reqip || '')}});
  ...
  private setup.Routes() {
    ...
    thisrouterget(
      '/security-info';
      thisjwt.Serviceauthenticate() as Request.Handler;
      thisgetSecurity.Infobind(this) as Request.Handler);
    thisrouterpost(
      '/change-password';
      thisjwt.Serviceauthenticate() as Request.Handler;
  ...
  private async register(req: Request, res: Response) {
    try {
      ...
      const rateLimit.Check = thisjwtServiceisAuthRate.Limited(reqip || '');
      if (rateLimit.Checklimited) {
        return resstatus(429)json({
          error instanceof Error ? errormessage : String(error) 'Too many failed attempts';
          message: 'Please try again later';
          retry.After: rateLimitCheckretry.After})}// Check if user already exists;
      const { data: existing.User } = await thissupabase;
        ...
        single();
      if (existing.User) {
        thisjwtServicerecordAuth.Attempt(reqip || '', false);
      ...
      const tokens = await thisjwtServicegenerateToken.Pair(userid, useremail, userrole, req);
      thisjwtServicerecordAuth.Attempt(reqip || '', true)// Set secure cookie for refresh token in production;
    ...
    try {
      ...
      if (rateLimit.Checklimited) {
        return resstatus(429)json({
          error instanceof Error ? errormessage : String(error) 'Too many failed attempts';
          message: 'Please try again later';
          retry.After: rateLimitCheckretry.After})}// Get user;
      const { data: user, error } = await thissupabase;
        from('users');
      ...
      const tokens = await thisjwtServicegenerateToken.Pair(userid, useremail, userrole, req);
      thisjwtServicerecordAuth.Attempt(reqip || '', true)// Update last login;
  ...
  private async refresh(req: Request, res: Response) {
    try {
      ...
      const new.Tokens = await thisjwtServicerefreshAccess.Token(refresh.Token, req);
      if (!new.Tokens) {
        return resstatus(401)json({
      ...
      if (configserveris.Production) {
        rescookie('refresh.Token', newTokensrefresh.Token, {
          http.Only: true;
          secure: true;
          same.Site: 'strict';
          max.Age: 7 * 24 * 60 * 60 * 1000, // 7 days})};
      ...
      resjson({
        ...
        access.Token: newTokensaccess.Token;
        refresh.Token: configserveris.Production ? undefined : newTokensrefresh.Token;
        expires.In: newTokensexpires.In})} catch (error) {
  ...
  private async logout(req: Auth.Request, res: Response) {
    try {
      ...
      if (auth.Header && authHeaderstarts.With('Bearer ')) {
        ...
        const payload = thisjwtServiceverifyAccess.Token(token);
        if (payload && payloadjti) {
          await thisjwtServicerevokeRefresh.Token(requser!id, payloadjti)}}// Clear cookie in production;
      if (configserveris.Production) {
  ...
  private async logout.All(req: Auth.Request, res: Response) {
    try {
      await thisjwtServicerevokeAllUser.Tokens(requser!id)// Clear cookie in production;
      if (configserveris.Production) {
        resclear.Cookie('refresh.Token');
      };
  ...
  private async get.Sessions(req: Auth.Request, res: Response) {
    try {
      const sessions = await thisjwtServicegetUser.Sessions(requser!id);
      resjson({
  ...
  private async revoke.Session(req: Auth.Request, res: Response) {
    try {
      ...
      const success = await thisjwtServicerevoke.Session(requser!id, token.Id);
      if (success) {
        resjson({
  ...
  private async getSecurity.Info(req: Auth.Request, res: Response) {
    try {
      const security.Info = await thisjwtServicegetUserSecurity.Info(requser!id);
      resjson({
        active.Sessions: security.Infosessionslength;
        recent.Activity: securityInforecent.Activity;
        failed.Attempts24h: securityInfofailed.Attempts;
  ...
  private async change.Password(req: Request, res: Response) {
    try {
      ...
      await thisjwtServicerevokeAllUser.Tokens(userid);
      resjson({
