...
export class EnhancedMemorySystem {
    ...
    useOllama = true;
    constructor(supabase, logger, embeddingConfig, cacheConfig, options) {
        ...
        if (this.useOllama) {
            ...
            // Set dimensions based on model
            const dimensions = openaiConfig?.dimensions ||
     ...
     */
    async storeMemory(serviceId, memoryType, content, metadata = {}, keywords) {
        try {
            // Perform contextual enrichment
            const enrichmentResult = this.contextualEnricher.enrichMemory(content, serviceId, memoryType, metadata);
            this.logger.debug(`Contextual enrichment extracted ${enrichmentResult.enrichment.entities.length} entities and ${enrichmentResult.enrichment.concepts.length} concepts`);
            ...
            const enrichedImportanceScore = this.calculateEnrichedImportance(enrichmentResult.enrichment, metadata.importance || 0.5);
        ...
        this.cacheSystem.cacheEmbedding(contentembedding);
        ...
        this.logger.error('Failed to store enriched memory:', error);
        throw error;
        ;
    }
}
...
if (options.enableUtilityRanking && formattedResults.length > 0) {
    const reRankedResults = await this.accessLearner.reRankResults(formattedResults.map((memory) => ({
        ...memory,
        similarityScore: data.find((d) => d.memory_id === memory.id)?.similarity || 0,
    })), options.agentFilter || 'unknown', {
    ...
    map((result) => {
        const { originalRank, newRank, utilityScore, finalScore, ...memory } = result;
        return memory;
    });
 ...
 */
async;
multiStageSearchMemories(options, MemorySearchOptions);
Promise < {
    results: Memory[],
...
const { results: searchResults, metrics } = await this.multiStageSearch.search(embedding, multiStageOptions);
;
...
try { }
    ...
    this.logger.error('Failed to perform multi-stage search:', error);
    throw error;
    ;
}
/**;
...
try { }
    ...
    this.logger.error('Failed to perform cross-agent search:', error);
    throw error;
    ;
}
/**;
...
Promise < Memory[] > {
    try: {
        ...
        if(error) { }, throw: error,
        return: data.map((memory) => this.formatMemory(memory))
    }, catch(error) {
        this.logger.error('Failed to find connected memories:', error);
        throw error;
        ;
    }
};
...
Promise < Memory[] > {
    try: {
        ...
        this.logger.error('Failed to get memory recommendations:', error);
        throw error;
        ;
    }
};
 ...
 */
async;
updateMemoryImportance(memoryId, string, boost = 0.1);
Promise < void  > {
    try: {
...
Promise < void  > {
     ...
     * Pre-warm embedding cache with common terms
     */
    async preWarmEmbeddingCache(commonTexts) {
     ...
     * Pre-warm memory cache with frequently accessed memories
     */
    async preWarmMemoryCache(limit = 100) {
        try {
...
calculateEnrichedImportance(enrichment, any, baseImportance, number);
number;
{
...
const hasImportantEntities = enrichment.entities.some((e) => importantEntityTypes.includes(e.type));
;
if (hasImportantEntities) {
    adjustedImportance += 0.1;
}
// Boost importance for technical content
if (enrichment.complexity.technicalLevel === 'expert' || )
    ;
...
const highRelevanceConcepts = enrichment.concepts.filter((c) => c.relevance > 0.8);
adjustedImportance += highRelevanceConcepts.length * 0.05;
return Math.min(1.0, Math.max(0.0, adjustedImportance));
extractKeywordsFromEnrichment(enrichment, any);
string[];
{
    const keywords = [];
    ...
    enrichment.entities.forEach((entity) => {
    ...
    enrichment.concepts;
    filter((concept) => concept.relevance > 0.5);
    slice(0, 10);
    forEach((concept) => {
...
categorizeMemoryFromEnrichment(enrichment, any, memoryType, string);
string;
{
...
const actionConcepts = enrichment.concepts.filter((c) => c.category === 'action');
const technicalConcepts = enrichment.concepts.filter((c) => c.category === 'technical');
const temporalConcepts = enrichment.concepts.filter((c) => c.category === 'temporal');
...
const hasPersonEntities = enrichment.entities.some((e) => e.type === 'person');
const hasOrgEntities = enrichment.entities.some((e) => e.type === 'organization');
if (hasPersonEntities || hasOrgEntities) {
    return 'social';
}
// Fallback to memory type or general
if (memoryType === 'consolidated')
    return 'consolidated';
return 'general';
/**;
...
Promise < {
    ...
    try: {
        ...
        const: queryEnrichment = this.contextualEnricher.enrichMemory(query, options.agentFilter || 'system', 'search_query'),
        this: .logger.debug()
    ...
    // Use contextual _contentfor embedding generation
...
if (queryEnrichment.enrichment.entities.length > 2) {
...
try { }
    ...
    this.logger.error('Failed to perform contextual search:', error);
    throw error;
    ;
}
extractKeywords(contentstring);
string[];
{
...
extractEntities(contentstring);
any[];
{
    // Simple entity extraction - in production, use NER
    const entities = [];
    // Extract emails
    const emails = _contentmatch(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g);
    if (emails) {
        entities.push(...emails.map((email) => ({ type: 'email', value: email })));
    }
    // Extract URLs
    const urls = _contentmatch(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/g);
    ;
    if (urls) {
...
categorizeMemory(memoryType, string, contentstring);
string;
{
    // Simple categorization logic
    if (memoryType === 'consolidated')
        return 'consolidated';
    if (_contenttoLowerCase().includes('task') || _contenttoLowerCase().includes('todo'))
        return 'task';
    if (_contenttoLowerCase().includes('meeting') || _contenttoLowerCase().includes('appointment'))
        return 'calendar';
    if (_contenttoLowerCase().includes('code') || _contenttoLowerCase().includes('function'))
        return 'technical';
    return 'general';
}
async;
createSimilarityConnections(memoryId, string, embedding, number[]);
Promise < void  > {
    try: {
...
string,
    accessType;
'search' | 'direct' | 'related' | 'contextual',
    options;
{
 ...
 */
async;
getMemoryStats(agentName ?  : string);
Promise < unknown > {
    try: {
    ...
    async getClusterStatistics() {
        try {
            ...
            this.logger.error('Failed to get cluster statistics:', error);
            throw error;
            ;
        }
    }
    ...
    async refreshSemanticClusters() {
        try {
            ...
            this.logger.error('Failed to refresh semantic clusters:', error);
            throw error;
            ;
        }
    }
     ...
     * Analyze search performance and get optimization recommendations
     */
    analyzeSearchPerformance(searchMetrics) {
     ...
     * Download/pull an embedding model (Ollama only)
     */
    async pullEmbeddingModel(model) {
        if (this.useOllama && this.embeddingService instanceof OllamaEmbeddingService) {
     ...
     * Switch between Ollama and OpenAI embedding services
     */
    switchEmbeddingService(useOllama, config) {
     ...
     * Get learning insights for an agent
     */
    async getLearningInsights(agentName) {
        try {
     ...
     * Calculate utility score for a memory
     */
    async calculateMemoryUtilityScore(memoryId, agentName, baseScore, contextualFactors) {
        try {
            ...
            this.logger.error('Failed to calculate utility score:', error);
            throw error;
            ;
        }
    }
     ...
     * Perform intelligent search with all enhancements enabled
     */
    async intelligentSearch(query, agentName, options = {}) {
        try {
            ...
            this.logger.error('Failed to perform intelligent search:', error);
            throw error;
            ;
        }
    }
