...
class SecurityTestSuite {
  ...
  async testSecurityHeaders() {
    ...
    try {
      ...
      for (const header of requiredHeaders) {
        ...
        const passed = present && (header.expectedValue ? headerValue.includes(header.expectedValue) : true);
  ...
  async testRateLimiting() {
    ...
    try {
      ...
      console.log(`  ${passed ? '✅' : '❌'} Rate limiting: ${rateLimitedResponses.length}/10 requests blocked`);
      
      // Check rate limit headers
      const firstResponse = responses[0];
      const hasRateLimitHeaders = firstResponse.headers.get('x-ratelimit-limit') ||
    ...
    const systemTests = [
      ...
      },
      {
        ...
        test: async () => {
          ...
          const hasSystemInfo = text.includes('Error:') ||
      ...
      },
      {
        ...
        test: async () => {
          ...
          const exposesSecrets = JSON.stringify(data).includes('password') ||
                                JSON.stringify(data).includes('secret');
  ...
  async testCSRFProtection() {
    ...
    try {
      ...
      if (tokenData.token) {
        ...
        console.log(`  ${csrfProtected ? '✅' : '⚠️'} CSRF protection: ${csrfProtected ? 'Active' : 'Not enforced'}`);
  ...
  async testSQLInjectionPrevention() {
    ...
    for (const payload of sqlPayloads) {
      try {
        ...
        console.log(`  ${passed ? '✅' : '❌'} SQL injection blocked: ${payload.substring(0, 20)}...`);
        
      } catch (error) {
        console.log(`  ✅ SQL injection blocked (connection error): ${payload.substring(0, 20)}...`);
      }
    }
    console.log();
  }
  ...
  async testXSSPrevention() {
    ...
    for (const payload of xssPayloads) {
      try {
        ...
        console.log(`  ${sanitized ? '✅' : '❌'} XSS payload sanitized: ${payload.substring(0, 20)}...`);
  ...
  async testPathTraversalPrevention() {
    ...
    for (const payload of pathPayloads) {
      try {
        const response = await fetch(`${BASE_URL}/api/v1/context/file/${encodeURIComponent(payload)}`, {
          method: 'GET',
          headers: {
            'X-API-Key': TEST_API_KEY,
            'X-AI-Service': TEST_SERVICE
        ...
        const blocked = response.status === 400 || response.status === 403 || response.status === 404;
        console.log(`  ${blocked ? '✅' : '❌'} Path traversal blocked: ${payload.substring(0, 20)}...`);
  ...
  generateReport() {
    ...
    console.log(`Overall Security Score: ${this.getScoreColor(this.results.overallScore)}${this.results.overallScore}/100\x1b[0m\n`);
    ...
    console.log('📊 SUMMARY:');
    console.log(`  Tests Run: ${this.results.inputValidation.length + this.results.authentication.length + this.results.securityHeaders.length + this.results.apiSecurity.length + this.results.systemSecurity.length}`);
    console.log(`  Tests Passed: ${[...this.results.inputValidation, ...this.results.authentication, ...this.results.securityHeaders, ...this.results.apiSecurity, ...this.results.systemSecurity].filter(t => t.passed).length}`);
  ...
  async saveReportToFile() {
    const reportData = {
      ...
      summary: {
        totalTests: this.results.inputValidation.length + this.results.authentication.length + 
                   this.results.securityHeaders.length + this.results.apiSecurity.length + 
                   this.results.systemSecurity.length,
        passedTests: [...this.results.inputValidation, ...this.results.authentication, 
                     ...this.results.securityHeaders, ...this.results.apiSecurity, 
                     ...this.results.systemSecurity].filter(t => t.passed).length,
