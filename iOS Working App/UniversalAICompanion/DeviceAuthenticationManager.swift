...
class DeviceAuthenticationManager: ObservableObject, ProximityDetectionDelegate, WatchConnectivityDelegate {
    ...
    deinit {
        Task { @MainActor in
            ...
            self.proximityService?.stopProximityDetection()
    ...
    func registerDevice() async {
        log("Attempting device registration", level: .info)
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                log("Registration response received: \(httpResponse.statusCode)", level: .debug)
                ...
                if httpResponse.statusCode == 200 {
                    if let responseData = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
                       ...
                       success {
                        ...
                        log("✅ Device registered successfully", level: .info)
                    } else {
            ...
            log("Registration failed", level: .error, error: error)
    ...
    func authenticateWithBiometrics() async {
        log("Starting biometric authentication", level: .info)
        ...
        do {
            ...
            log("Biometric authentication successful", level: .debug)
            ...
            log("Full authentication process completed successfully", level: .info)

        } catch {
            log("Authentication failed", level: .error, error: error)
            authenticationState = .unauthenticated
    ...
    private func setError(_ error: AuthError) async {
        ...
        ErrorLogger.shared.captureAndLogError(
            error,
            context: [
                "deviceId": deviceId,
                "deviceName": deviceName,
                "deviceType": deviceType.rawValue,
                "registrationState": registrationState.description,
                "authenticationState": authenticationState.description
            ],
...
extension AuthError {
    var logLevel: LogLevel {
        ...
        case .notRegistered, .registrationFailed:
            return .warning
        case .keyGenerationFailed, .biometricNotAvailable:
            return .critical
        case .biometricFailed, .challengeRequestFailed, .signatureFailed:
            return .error
        case .verificationFailed, .authenticationFailed:
            return .critical
        case .networkError:
            return .error
        }
