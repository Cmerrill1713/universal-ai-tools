 ...
 * @returns true if running in browser, false if in Nodejs;
 */;
...
/**;
 * Check if the code is running in Nodejs environment;
 * @returns true if running in Nodejs, false if in browser;
 */;
 ...
 */;
export function withBrowserContext<T>(callback: () => T, fallback?: T): T | undefined {;
  if (isBrowser()) {;
    try {;
      return callback();
    } catch (error) {;
      loggererror('Error accessing browser API:', error);
      return fallback;
    };
  };
  return fallback;
...
/**;
 * Execute code only in Nodejs context;
 * @param callback Function to execute in Nodejs;
 */;
