...
async function testAuthMiddleware() {
  console.log('🔐 Direct Authentication Middleware Test');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`   Valid DEV_API_KEY: ${validTest.allowed ? '✅ ALLOWED' : '❌ DENIED'} (${validTest.reason})`);
    
    // Test old hardcoded key
    const hardcodedTest = testAuth('local-dev-key', 'local-ui', process.env.DEV_API_KEY);
    console.log(`   Old hardcoded key: ${hardcodedTest.allowed ? '❌ ALLOWED (SECURITY ISSUE!)' : '✅ DENIED'} (${hardcodedTest.reason})`);
    
    // Test invalid key
    const invalidTest = testAuth('invalid-key', 'local-ui', process.env.DEV_API_KEY);
    console.log(`   Invalid key: ${invalidTest.allowed ? '❌ ALLOWED (SECURITY ISSUE!)' : '✅ DENIED'} (${invalidTest.reason})`);
    
    // Test no key
    const noKeyTest = testAuth(undefined, 'local-ui', process.env.DEV_API_KEY);
    console.log(`   No key: ${noKeyTest.allowed ? '❌ ALLOWED (SECURITY ISSUE!)' : '✅ DENIED'} (${noKeyTest.reason})`);
    ...
    console.log(`   Authenticated request: ${authTest.status || 'ERROR'} ${authTest.statusText || authTest.error || ''}`);
    console.log(`   Unauthenticated request: ${noAuthTest.status || 'ERROR'} ${noAuthTest.statusText || noAuthTest.error || ''}`);
