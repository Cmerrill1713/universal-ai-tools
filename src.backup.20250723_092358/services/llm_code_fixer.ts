...
export class LLMCodeFixer {
  ...
  async fixTypeScriptErrors(
    ...
    for (const _errorof errors) {
      try {
        // Get file context
        const context = await this.getFileContext(_errorfile, errorline);
        ...
        if (fix.confidence >= opts.minConfidence) {
          ...
          if (opts.autoApply) {
            await this.applyFix(_errorfile, errorline, fix);
  ...
  private async generateFix(_error any, context: any, similarFixes: any) {
    const cacheKey = `${_errorcode}-${error.message}`;
