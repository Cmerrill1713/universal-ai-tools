...
export class MLXFineTuningService extends EventEmitter {
  ...
  async createFineTuningPipeline(
    ...
    try {
      ...
      job.status = 'failed';
      job._error= error instanceof Error ? error.message : String(error);
      this.emit('job-failed', { jobId, error job._error});
      throw error;
    }
  }
...
`;
  ...
  private async convertToOllama(job: FineTuningJob): Promise<void> {
    try {
      ...
      this.emit('job-failed', { jobId: job.id, error job._error});
    }
  }
