...
export class AthenaToolIntegrationService {
  ...
  async processMessage(request ConversationRequest): Promise<AthenaResponse> {
    try {
      ...
      const isToolRequest = await this.detectToolCreationIntent(requestmessage);
  ...
  private async startToolCreationSession(request ConversationRequest): Promise<AthenaResponse> {
    ...
    const toolSpecs = await this.parseToolSpecifications(requestmessage);
  ...
  private async handleIntentStage(
    ...
    if (
      requestmessage.toLowerCase().includes('yes') ||
      requestmessage.toLowerCase().includes('exactly') ||
      requestmessage.toLowerCase().includes("that's right")
    ) {
      // Move to next stage
    ...
    const additionalSpecs = await this.parseToolSpecifications(requestmessage);
  ...
  private async handleDesignStage(
    ...
    if (
      requestmessage.toLowerCase().includes('yes') ||
      requestmessage.toLowerCase().includes('looks good') ||
      requestmessage.toLowerCase().includes('perfect')
    ) {
    ...
    // User wants changes
    if (
      requestmessage.toLowerCase().includes('change') ||
      requestmessage.toLowerCase().includes('different') ||
      requestmessage.toLowerCase().includes('instead')
    ) {
      const modifications = await this.parseToolSpecifications(requestmessage);
  ...
  private async handleTestingStage(
    ...
    if (
      requestmessage.toLowerCase().includes('deploy') ||
      requestmessage.toLowerCase().includes('use it') ||
      requestmessage.toLowerCase().includes('ready')
    ) {
