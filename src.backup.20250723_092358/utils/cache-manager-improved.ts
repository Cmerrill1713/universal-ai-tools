...
export class ImprovedCacheManager {
  ...
  private setupEventListeners(): void {
    ...
    this.redis.on('_error, (error => {
      logger.error('Redis error', LogContext.CACHE, { _error});
  ...
  public async get<T = any>(key: string, options: CacheOptions = {}): Promise<T | null> {
    ...
    try {
      ...
      logger.error('Cache get error', LogContext.CACHE, { _error});
      this.handleConnectionError();
  ...
  public async set(key: string, value: any, options: CacheOptions = {}): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Cache set error', LogContext.CACHE, { _error});
      this.handleConnectionError();
  ...
  public async del(key: string, options: CacheOptions = {}): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Cache delete error', LogContext.CACHE, { _error});
      this.handleConnectionError();
  ...
  public async getStats(): Promise<CacheStats> {
    try {
      ...
      logger.error('Cache stats error', LogContext.CACHE, { _error});
      this.handleConnectionError();
