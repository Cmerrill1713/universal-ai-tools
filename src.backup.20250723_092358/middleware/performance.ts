  ...
  public requestTimer() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      res.end = function (this: Response, ...args: any[]) {
        ...
        if (isError) {
          logger.error
            `Request _error ${req.method} ${req.url} - Status: ${res.statusCode} - Response time: ${responseTime}ms`,
  ...
  public responseCache(defaultTtl = 3600) {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      this.cache
        ...
        .catch((error => {
          logger.error('Cache middleware _error, LogContext.PERFORMANCE, { _error});
          next();
        });
    };
  }
