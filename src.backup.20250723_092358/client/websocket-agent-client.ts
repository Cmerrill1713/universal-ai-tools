...
export interface AgentMessage {
  type: 'request | 'response' | 'event' | '_error);
  requestId?: string;
  method?: string;
  params?: any;
  data?: any;
...
export class WebSocketAgentClient extends EventEmitter {
      ...
      try {
        ...
        this.ws.on('message', (data: WebSocket.Data) => {
          try {
            ...
            console._error'Failed to parse WebSocket message:', error);
          }
        });
        ...
        this.ws.on('_error, (_error Error) => {
          console._error'WebSocket _error', error);
  ...
  async orchestrate(request OrchestrationRequest): Promise<unknown> {
    return this.request'orchestrate', request;
  }

  /**
   * Coordinate specific agents
   */
  async coordinateAgents(request AgentCoordinationRequest): Promise<unknown> {
    return this.request'coordinate_agents', request;
  }
  ...
  async getModelInfo(): Promise<unknown> {
    return this.request'get_model_info', {});
  }
  ...
  async escalateModel(minQualityScore = 0.8): Promise<unknown> {
    return this.request'escalate_model', { min_quality_score: minQualityScore });
  }
  ...
  private attemptReconnect(): void {
    ...
    setTimeout(() => {
      this.connect().catch((error => {
        console._error'Reconnection failed:', error);
      });
