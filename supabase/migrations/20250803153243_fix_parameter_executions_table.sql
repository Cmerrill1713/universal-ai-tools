-- Parameter Analytics Tables Migration
-- Fixes missing parameter_executions table causing database errors
-- Based on archived migration 033_parameter_analytics_tables.sql
...
DROP TRIGGER IF EXISTS trigger_update_parameter_effectiveness ON parameter_executions;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON parameter_executions TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON parameter_effectiveness TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON parameter_insights TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON user_parameter_preferences TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON parameter_experiments TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON parameter_dashboard_metrics TO authenticated;

GRANT SELECT ON parameter_performance_realtime TO authenticated;
GRANT SELECT ON parameter_optimization_opportunities TO authenticated;

GRANT EXECUTE ON FUNCTION calculate_parameter_score TO authenticated;
GRANT EXECUTE ON FUNCTION update_parameter_effectiveness TO authenticated;

-- Comments for Documentation
COMMENT ON TABLE parameter_executions IS 'Detailed tracking of every parameter execution with outcomes - fixes missing table causing log errors';
...
-- Insert some initial test data to validate the setup
INSERT INTO parameter_executions (
    id, task_type, user_input, parameters, model, provider, user_id, request_id,
    execution_time, token_usage, response_length, success, endpoint
) VALUES (
    ...
    'ollama',
    null,
    'migration-test-' || gen_random_uuid()::text,
    1000,
