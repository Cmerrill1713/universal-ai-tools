-- Migration: Add comprehensive TypeScript agent orchestration and DSPy patterns to memory system
-- Description: Stores advanced agent orchestration patterns, DSPy framework concepts, and production-ready implementations

-- DSPy Framework Fundamentals
INSERT INTO ai_memories (service_id, memory_type, content, metadata, importance, keywords, created_at, last_accessed)
VALUES (
  'agent-orchestration-system',
  'dspy_framework_fundamentals',
  '# DSPy Framework Fundamentals
...
- Evolve system behavior over time',
  jsonb_build_object(
    ...
    'importance', 10,
    'tags', ARRAY['dspy', 'framework', 'declarative', 'prompting'],
    'implementation_patterns', true
  ),
  10,
  ARRAY['dspy', 'declarative', 'signatures', 'modules', 'optimization', 'self-improvement'],
  NOW(),
  NOW()
);

-- TypeScript DSPy Implementation Patterns
INSERT INTO ai_memories (service_id, memory_type, content, metadata, importance, keywords, created_at, last_accessed)
VALUES (
  'agent-orchestration-system',
  'typescript_dspy_implementation',
  '# TypeScript DSPy Implementation Patterns
...
```',
  jsonb_build_object(
    ...
    'importance', 10,
    'tags', ARRAY['typescript', 'dspy', 'ax-framework', 'implementation'],
    'code_examples', true
  ),
  10,
  ARRAY['typescript', 'dspy', 'ax', 'signature', 'module', 'predict', 'optimizer'],
  NOW(),
  NOW()
);
...
VALUES (
  ...
  '# TypeScript Agent Orchestration Patterns
...
```',
  jsonb_build_object(
    ...
    'importance', 10,
    'tags', ARRAY['orchestration', 'multi-agent', 'coordination', 'typescript'],
    'production_ready', true
  ),
  10,
  ARRAY['orchestration', 'multi-agent', 'registry', 'workflow', 'communication', 'state'],
  NOW(),
  NOW()
);
...
```',
  jsonb_build_object(
    ...
    'importance', 10,
    'tags', ARRAY['production', 'lifecycle', 'monitoring', 'resources', 'error-handling'],
    'production_patterns', true
  ),
  10,
  ARRAY['lifecycle', 'monitoring', 'observability', 'resources', 'load-balancing', 'circuit-breaker', 'error-handling'],
  NOW(),
  NOW()
);
...
```',
  jsonb_build_object(
    ...
    'importance', 10,
    'tags', ARRAY['hierarchical', 'swarm', 'self-organizing', 'adaptive', 'learning'],
    'advanced_techniques', true
  ),
  10,
  ARRAY['hierarchical', 'swarm', 'self-organizing', 'adaptive', 'q-learning', 'gossip', 'aco'],
  NOW(),
  NOW()
);

-- Practical Implementation Examples (Part 1)
INSERT INTO ai_memories (service_id, memory_type, content, metadata, importance, keywords, created_at, last_accessed)
VALUES (
  'agent-orchestration-system', 
  'practical_implementation_examples_part1',
  '# Practical Agent Orchestration Examples - Part 1
...
```',
  jsonb_build_object(
    ...
    'importance', 10,
    'tags', ARRAY['examples', 'document-processing', 'data-streaming', 'ocr', 'nlp'],
    'real_world_applications', true
  ),
  10,
  ARRAY['document-processing', 'ocr', 'nlp', 'data-streaming', 'swarm', 'load-balancing'],
  NOW(),
  NOW()
);

-- Practical Implementation Examples (Part 2)
INSERT INTO ai_memories (service_id, memory_type, content, metadata, importance, keywords, created_at, last_accessed)
VALUES (
  'agent-orchestration-system', 
  'practical_implementation_examples_part2',
  '# Practical Agent Orchestration Examples - Part 2
...
```',
  jsonb_build_object(
    'category', 'practical_examples',
    'importance', 10,
    'tags', ARRAY['customer-support', 'trading', 'consensus', 'escalation', 'financial'],
    'real_world_applications', true
  ),
  10,
  ARRAY['customer-support', 'trading', 'consensus', 'escalation', 'l1-support', 'l3-expert', 'risk-management'],
  NOW(),
  NOW()
);

-- Add index for efficient memory retrieval
CREATE INDEX IF NOT EXISTS idx_ai_memories_service_type ON ai_memories(service_id, memory_type);
CREATE INDEX IF NOT EXISTS idx_ai_memories_keywords ON ai_memories USING GIN(keywords);
...
SET access_count = access_count + 1, 
    last_accessed = NOW() 
WHERE service_id = 'agent-orchestration-system';