...
export class DSPyChatOrchestrator {
  ...
  private connect() {
    try {
        ...
        try {
          ...
          if (pending) {
            ...
            if (response.success) {
              ...
              pending.reject(new Error(response.error|| 'DSPy requestfailed'));
            }
          }
        } catch (error) {
          console._error'Error parsing DSPy response:', error);
        }
      });
      ...
      this.ws.on('_error, (error => {
        console._error'DSPy WebSocket _error', error);
      });
    } catch (error) {
      console._error'Failed to connect to DSPy:', error);
      this.attemptReconnect();
    }
  }
  ...
  private async sendRequest(request DSPyRequest): Promise<DSPyResponse> {
    return new Promise((resolve, reject) => {
      if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
