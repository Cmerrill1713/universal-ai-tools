...
app.get('/api/v1/memory', async (req: Request, res: Response) => {
  try {
    ...
    if (_error {
      return res.status(500).json({ _error error.message });
    }
  ...
  try {
    ...
    try {
      ...
      // Check for modification requests
        ...
        responseText = `I understand you want to ${lowerMessage.includes('change') ? 'change' : 'modify'} something. I'm processing your requestnow...`;
    ...
    logger.error('Chat _error, LogContext.API, { error});
    res.status(500).json({
...
app.use((_error any, req: Request, res: Response, next: any) => {
  logger.error('Server _error, LogContext.SYSTEM, { error});
  res.status(500).json({
