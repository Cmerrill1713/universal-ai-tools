...
export function createDSPyToolsRouter() {
  ...
  router.post('/execute', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to execute DSPy tool', LogContext.DSPY, { error, tool: req.body.tool_name });
      res.status(500).json({
        success: false,
  ...
  router.post('/pipeline', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to execute DSPy pipeline', LogContext.DSPY, { error, tools: req.body.tools });
      res.status(500).json({
        success: false,
  ...
  router.post('/recommend', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get tool recommendations', LogContext.DSPY, { error, task: req.body.task });
      res.status(500).json({
        success: false,
