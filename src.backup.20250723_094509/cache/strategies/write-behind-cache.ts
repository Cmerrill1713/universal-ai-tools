...
export class WriteBehindCache<T = any> extends EventEmitter {
  ...
  private startFlushTimer(): void {
    ...
    this.flushTimer = setInterval(() => {
      this.flushBatch().catch((error => {
        logger.error('Write-behind cache flush _error', error);
  ...
  private queueWrite(key: string, value: T, ttl: number): void {
    ...
    if (this.writeQueue.size >= this.batchSize) {
      this.flushBatch().catch((error => {
        logger.error('Write-behind cache immediate flush _error', error);
        this.emit('_error, error);
      });
    }
  }
  ...
  private async flushBatch(): Promise<void> {
    ...
    try {
      ...
      logger.error('Write-behind cache batch write _error', error);
  ...
  async clear(): Promise<void> {
    try {
      ...
      logger.error('Write-behind cache clear _error', error);
      this.emit('_error, error);
    }
  }
  ...
  async warmup(keys: string[]): Promise<void> {
    ...
    try {
      ...
      logger.error('Write-behind cache warmup _error', error);
      this.emit('_error, error);
    }
  }
