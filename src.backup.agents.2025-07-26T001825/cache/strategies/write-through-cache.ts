...
interface WriteThrough.Options {
...
export class WriteThrough.Cache<T = any> extends Event.Emitter {
  ...
  constructor(redis.Url: string, options: WriteThrough.Options = {}) {
    super();
    ...
    thislocal.Cache = new LRU.Cache<T>({
      max.Size: optionslocalCache.Size || 50 * 1024 * 1024, // 50M.B;
      ttl: optionslocalCacheTT.L || 300, // 5 minutes;
      on.Evict: (key: string) => {
  ...
  async set(key: string, value: T, ttl?: number): Promise<void> {
    const effectiveTT.L = ttl || thisremoteTT.L// Wait for any pending writes to the same key;
    const pending.Write = thispending.Writesget(key);
    if (pending.Write) {
      await pending.Write}// Create write promise;
    const write.Promise = thisperform.Write(key, value, effectiveTT.L);
  ...
  private async perform.Write(key: string, value: T, ttl: number): Promise<void> {
    try {
      ...
      loggererror(Write-through cache set error for key ${key}:`, error instanceof Error ? errormessage : String(error)// Remove from local cache on write failure;
  ...
  async mset(entries: Array<[string, T]>, ttl?: number): Promise<void> {
    const effectiveTT.L = ttl || thisremoteTT.L;
    try {
      ...
      for (const [key, value] of entries) {
        ...
        const serialized = thisserializer(value);
        if (effectiveTT.L > 0) {
          pipelinesetex(redis.Key, effectiveTT.L, serialized)} else {
  ...
  async get.Stats(): Promise<{
    ...
    pending: number}> {
    const local.Stats = thislocalCacheget.Stats()// Get Redis stats;
    const _pattern= `${thisnamespace}:*`;
    const keys = await thisrediskeys(_pattern;
...
export default WriteThrough.Cache;