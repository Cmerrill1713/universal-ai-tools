...
export class Supabase.Service {
  ...
  private constructor() {
    ...
    thisclient = create.Client(supabase.Url, supabaseAnon.Key, {
      auth: {
  ...
  public async save.Context(user.Id: string, context: any): Promise<void> {
    try {
      ...
      loggererror('Failed to save context:', LogContextDATABAS.E, {
  ...
  public async get.Context(user.Id: string, limit = 10): Promise<any[]> {
    try {
      ...
      loggererror('Failed to retrieve context:', LogContextDATABAS.E, {
        error instanceof Error ? errormessage : String(error) error instanceof Error ? errormessage : String(error instanceof Error ? errormessage : String(error)});
      throw error instanceof Error ? errormessage : String(error)}}/**
  ...
  public async save.Memory(memory: {
    ...
    try {
      ...
      loggererror('Failed to save memory:', LogContextDATABAS.E, {
        error instanceof Error ? errormessage : String(error) error instanceof Error ? errormessage : String(error instanceof Error ? errormessage : String(error)});
      throw error instanceof Error ? errormessage : String(error)}}/**
  ...
  public async search.Memories(embedding: number[], limit = 10, threshold = 0.7): Promise<any[]> {
    try {
      ...
      const { data, error } = await thisclientrpc('search_memories_by_embedding', {
        ...
        match_count: limit});
      if (error instanceof Error ? errormessage : String(error){
        // If RP.C function doesn't exist, fall back to manual similarity search;
        loggerwarn('RP.C function not found, using manual vector search:', error instanceof Error ? errormessage : String(error);
        return await thisfallbackVector.Search(embedding, limit, threshold)};

      return data || []} catch (error) {
      loggererror('Failed to search memories:', LogContextDATABAS.E, {
      ...
      return await thisfallbackVector.Search(embedding, limit, threshold)}}/**
   * Fallback vector search when RP.C is not available*/
  private async fallbackVector.Search(
    embedding: number[];
    limit: number;
  ...
  public async query(table: string, filters?: any): Promise<any[]> {
    try {
      ...
      return data || []} catch (error) {
      loggererror`Failed to query ${table}:`, LogContextDATABAS.E, {
        error instanceof Error ? errormessage : String(error) error instanceof Error ? errormessage : String(error instanceof Error ? errormessage : String(error)});
      throw error instanceof Error ? errormessage : String(error)}}/**
  ...
  public async insert(table: string, data: any): Promise<unknown> {
    try {
      ...
      loggererror`Failed to insert into ${table}:`, LogContextDATABAS.E, {
        error instanceof Error ? errormessage : String(error) error instanceof Error ? errormessage : String(error instanceof Error ? errormessage : String(error)});
      throw error instanceof Error ? errormessage : String(error)}}/**
  ...
  public async update(table: string, id: string, data: any): Promise<unknown> {
    try {
      ...
      loggererror`Failed to update ${table}:`, LogContextDATABAS.E, {
        error instanceof Error ? errormessage : String(error) error instanceof Error ? errormessage : String(error instanceof Error ? errormessage : String(error)});
      throw error instanceof Error ? errormessage : String(error)}}/**
  ...
  public async delete(table: string, id: string): Promise<void> {
    try {
      ...
      loggererror`Failed to delete from ${table}:`, LogContextDATABAS.E, {
        error instanceof Error ? errormessage : String(error) error instanceof Error ? errormessage : String(error instanceof Error ? errormessage : String(error)});
      throw error instanceof Error ? errormessage : String(error)}}}// Export singleton instance for easy access;
export const supabase = SupabaseServiceget.Instance()client// Export service instance;
export const supabase.Service = SupabaseServiceget.Instance()// Export client factory function;
export function createSupabase.Client() {
  return SupabaseServiceget.Instance()client};
