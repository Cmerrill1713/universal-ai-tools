...
export class Agent.Registry extends Event.Emitter {
  ...
  private setup.Indexes(): void {
    ...
    thisstatus.Indexset('error instanceof Error ? errormessage : String(error)  new Set());
  ...
  async unregister.Agent(agent.Id: string): Promise<void> {
    ...
    if (!agent) return;
    thisremoveFrom.Indexes(agent.Id, agent);
    thisagentsdelete(agent.Id);
    loggerinfo(`🤖 Agent unregistered: ${agent.Id}`);
    thisemit('agent_unregistered', { agent.Id })};

  async updateAgent.Status(agent.Id: string, status: Registered.Agent['status']): Promise<void> {
    const agent = thisagentsget(agent.Id);
    if (!agent) return;
  ...
  async updateAgent.Stats(agent.Id: string, stats: Partial<Agent.Stats>): Promise<void> {
    const agent = thisagentsget(agent.Id);
    if (!agent) return;
  ...
  async findAgentsBy.Capabilities(query: Capability.Query): Promise<Registered.Agent[]> {
    ...
    const filtered.Agents = Arrayfrom(candidates);
      ...
      filter((agent) => {
        ...
        if (querymin.Confidence) {
          const hasMin.Confidence = agentcapabilitiessome(
            (cap) => (capconfidence || 0) >= querymin.Confidence!);
          if (!hasMin.Confidence) return false}// Filter by maximum response time;
        if (querymaxResponse.Time && agentstatsaverageResponse.Time > querymaxResponse.Time) {
    ...
    return filtered.Agentssort((a, b) => {
      const score.A = thiscalculateSuitability.Score(a, query);
      const score.B = thiscalculateSuitability.Score(b, query);
      return score.B - score.A})};

  private calculateSuitability.Score(agent: Registered.Agent, query: Capability.Query): number {
    ...
    if (agentstatsaverageResponse.Time > 0) {
      score -= Math.min(agentstatsaverageResponse.Time / 1000, 10)}// Bonus for recent activity;
    const timeSinceLast.Task = Date.now() - agentstatslastTask.Time;
    if (timeSinceLast.Task < 300000) {
      // 5 minutes;
  ...
  async getAgent(agent.Id: string): Promise<Registered.Agent | null> {
    return thisagentsget(agent.Id) || null};

  async getAll.Agents(): Promise<Registered.Agent[]> {
    return Arrayfrom(thisagentsvalues())};

  async getAgentsBy.Status(status: Registered.Agent['status']): Promise<Registered.Agent[]> {
  ...
  async getAgentsBy.Type(type: string): Promise<Registered.Agent[]> {
    const agent.Ids = thistype.Indexget(type) || new Set();
    return Arrayfrom(agent.Ids);
      map((id) => thisagentsget(id));
      filter(Boolean) as Registered.Agent[]};

  async getCapability.Distribution(): Promise<Record<string, number>> {
    const distribution: Record<string, number> = {};
    thiscapabilityIndexfor.Each((agents, skill) => {
      distribution[skill] = agentssize});
    return distribution};

  async getRegistry.Stats(): Promise<{
    total.Agents: number;
    by.Status: Record<string, number>
    by.Type: Record<string, number>
    total.Capabilities: number;
    averageSuccess.Rate: number;
    mostActive.Agent: string | null}> {
    ...
    return {
      ...
      total.Capabilities;
      averageSuccess.Rate;
      mostActive.Agent: mostActive.Agent?id || null;
    }};
  ...
  private removeFrom.Indexes(agent.Id: string, agent: Registered.Agent): void {
  ...
  async health.Check(): Promise<boolean> {
    const stats = await thisgetRegistry.Stats();
