...
import { DynamicContext.Manager } from '././services/dynamic_context_managerjs'// Mock dependencies;
...
jestmock('././services/model_lifecycle_managerjs', () => ({
  ModelLifecycle.Manager: jestfn()mock.Implementation(() => ({
    getModel.Info: jestfn()mockReturn.Value({
      name: 'test-model';
      size: 'medium';
      context.Window: 8192});
    load.Model: jestfn();
    unload.Model: jestfn();
    get.Instance: jestfn()}))}));
describe('DynamicContext.Manager', () => {
  let context.Manager: DynamicContext.Manager;
  before.Each(() => {
    jestclearAll.Mocks()// Reset singleton instance if it exists;
    if ('instance' in DynamicContext.Manager) {
      (DynamicContext.Manager as any)instance = undefined};
    context.Manager = DynamicContextManagerget.Instance()});
  describe('Context Window Configuration', () => {
    it('should return correct context windows for different model sizes', () => {
      ...
      testCasesfor.Each(({ model, expected.Optimal }) => {
        const config = contextManagergetOptimal.Context(model);
        expect(configoptimal.Context)to.Be(expected.Optimal);
        expect(configmin.Context)toBeLessThanOr.Equal(configoptimal.Context);
        expect(configmax.Context)toBeGreaterThanOr.Equal(configoptimal.Context)})});
    it('should default to medium context for unknown models', () => {
      const config = contextManagergetOptimal.Context('unknown-model');
  ...
  describe('Context Optimization', () => {
    ...
    it('should apply sliding window strategy for conversations', async () => {
      ...
      const optimized = await contextManageroptimize.Context(
        messages;
        'llama-3.2-1b';
      ...
      expect(optimizedlength)toBeLess.Than(messageslength);
    ...
    it('should apply importance-based selection for code generation', async () => {
      ...
      expect(code.Messageslength)toBeGreater.Than(0)});
    it('should optimize context based on strategy', async () => {
      ...
      expect(optimized.Tokens)toBeLess.Than(original.Tokens);
      expect(optimizedlength)toBeLess.Than(messageslength)// Should keep system message;
  ...
  describe('Context Statistics', () => {
    it('should track token usage statistics', async () => {
      ...
      await contextManageroptimize.Context(messages, 'llama-3.1-8b', 'general');
      const stats = contextManagerget.Stats();
      expect(statstotalTokens.Processed)toBeGreater.Than(0);
      expect(statscompression.Ratio)toBeGreaterThanOr.Equal(1);
      expect(statssavings.Percentage)toBe.Defined()})});
  describe('Context Recommendations', () => {
    it('should provide appropriate recommendations for different tasks', () => {
      ...
      testCasesfor.Each(({ model, task, expected.Strategy }) => {
        const recommendations = contextManagergetContext.Recommendations(model, task);
        expect(recommendationsstrategy)to.Be(expected.Strategy);
        expect(recommendationsrecommended)toBeGreater.Than(0);
        expect(recommendationsminimum)toBeLessThanOr.Equal(recommendationsrecommended);
        expect(recommendationsmaximum)toBeGreaterThanOr.Equal(recommendationsrecommended)})})})});