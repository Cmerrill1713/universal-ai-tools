...
export function createAsync.Handler<T.Body = any, T.Query = any, T.Params = any>(
    ...
    try {
      ...
      if (!resheaders.Sent) {
        ...
        resstatus(status.Code)json({
          success: false;
          error instanceof Error ? errormessage : String(error) error instanceof Error ? errormessage : 'Internal server error'.(process.envNODE_EN.V === 'development' && {
...
export function asyncError.Handler(
  ...
  return async (err: Error, req: Request, res: Response, next: Next.Function) => {
    try {
      ...
      loggererror('Error in error handler', LogContextSYSTE.M, { error });
      if (!resheaders.Sent) {
        resstatus(500)json({
          success: false;
