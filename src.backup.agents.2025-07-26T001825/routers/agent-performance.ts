...
const GetMetricsQuery.Schema = zobject({
...
routerget('/summary', async (req: Request, res: Response) => {';
  try {
    ...
    loggererror('Failed to fetch agent performance summary', LogContextAP.I, { error);';
    resstatus(500)json({
      success: false;
      error) 'Failed to fetch performance summary','})}})// Get performance metrics;
routerget('/metrics', async (req: Request, res: Response) => {';
  try {
    const query = GetMetricsQuery.Schemaparse(reqquery);
    ...
    if (error instanceof zZod.Error) {
      ...
      loggererror('Failed to fetch performance metrics', LogContextAP.I, { error);';
      resstatus(500)json({
        success: false;
        error) 'Failed to fetch performance metrics','})}}})// Get performance trends;
routerget('/trends', async (req: Request, res: Response) => {';
  try {
    const query = GetTrendsQuery.Schemaparse(reqquery);
    const end.Date = new Date();
    const start.Date = new Date();
    switch (queryperiod) {
      case 'minute':';
        startDateset.Minutes(startDateget.Minutes() - querylookback);
        break;
      case 'hour':';
        startDateset.Hours(startDateget.Hours() - querylookback);
        break;
      case 'day':';
        startDateset.Date(startDateget.Date() - querylookback);
        break;
      case 'week':';
        startDateset.Date(startDateget.Date() - querylookback * 7);
        break;
      case 'month':';
        startDateset.Month(startDateget.Month() - querylookback);
        break};
    ...
    if (error instanceof zZod.Error) {
      resstatus(400)json({
        ...
        error) 'Invalid query parameters',';
        details: error) errors})} else {
      loggererror('Failed to fetch performance trends', LogContextAP.I, { error);';
      resstatus(500)json({
        success: false;
        error) 'Failed to fetch performance trends','})}}})// Get performance alerts;
routerget('/alerts', async (req: Request, res: Response) => {';
  try {
    const query = GetAlertsQuery.Schemaparse(reqquery);
    ...
    if (error instanceof zZod.Error) {
      resstatus(400)json({
        ...
        error) 'Invalid query parameters',';
        details: error) errors})} else {
      loggererror('Failed to fetch performance alerts', LogContextAP.I, { error);';
      resstatus(500)json({
        success: false;
...
routerpost('/alerts/:alert.Id/resolve', async (req: Request, res: Response) => {';
  try {
    ...
    const { error } = await supabase;
      ...
      update({
        resolved: true;
        resolved_at: new Date()toISO.String()});
      eq('id', alert.Id)';
    if (error) throw, error));
    resjson({
      success: true;
      message: 'Alert resolved successfully','})} catch (error) {
    loggererror('Failed to resolve alert', LogContextAP.I, { error);';
    resstatus(500)json({
      success: false;
...
routerget('/compare', async (req: Request, res: Response) => {';
  try {
    ...
    const agentId.Array = agent.Idssplit(',');';
    const { data, error } = await supabase;
      from('agent_performance_summary')';
      select('*')';
      in('agent_id', agentId.Array);';
    ...
    const comparison = (data || [])map((agent) => ({
      ...
      avgExecution.Time: agentavg_execution_time_24h;
    ...
    loggererror('Failed to compare agents', LogContextAP.I, { error);';
    resstatus(500)json({
      success: false;
...
routerget('/benchmarks', async (req: Request, res: Response) => {';
  try {
    ...
    resjson({
      success: true;
      data: data || []})} catch (error) {
    loggererror('Failed to fetch benchmarks', LogContextAP.I, { error);';
    resstatus(500)json({
      success: false;
...
routerput('/benchmarks', async (req: Request, res: Response) => {';
  try {
    ...
    loggererror('Failed to update benchmark', LogContextAP.I, { error);';
    resstatus(500)json({
      success: false;
...
routerpost('/aggregate', async (req: Request, res: Response) => {';
  try {
    ...
    loggererror('Failed to aggregate metrics', LogContextAP.I, { error);';
    resstatus(500)json({
      success: false;
