/**
 * Knowledge Monitoring Router* AP.I endpoints for knowledge base health monitoring and management*/
...
import { createKnowledgeFeedback.Service } from './services/knowledge-feedback-service';
import { knowledgeScraper.Service } from './services/knowledge-scraper-service';
import { createKnowledgeUpdate.Automation } from './services/knowledge-update-automation';
import { knowledgeValidation.Service } from './services/knowledge-validation-service';
...
export default function createKnowledgeMonitoring.Router(supabase: Supabase.Client) {
  ...
  const feedback.Service = createKnowledgeFeedback.Service(supabase, logger);
  const update.Automation = createKnowledgeUpdate.Automation(
  ...
  routerget('/dashboard', async (req: Request, res: Response) => {';
    try {
      ...
      const [
        ...
        getOverview.Metrics(since);
        getSourceHealth.Metrics();
        getValidation.Metrics(since);
        getUsage.Analytics(since);
        getPerformance.Metrics(since);
        getActive.Alerts();
        getUpdateQueue.Status();
        feedbackServiceget.Insights()]);
      resjson({
   ...
   * GE.T /api/knowledge-monitoring/sources* Get detailed source status*/
  routerget('/sources', async (_req, res) => {';
    try {
      const sources = await Promiseall(
        KNOWLEDGE_SOURCE.Smap(async (source) => {
          const [last.Scrape, item.Count, quality.Score, issues] = await Promiseall([
            getLastScrape.Time(sourceid);
            getSourceItem.Count(sourceid);
            getSourceQuality.Score(sourceid);
            getSource.Issues(sourceid)]);
          return {
            id: sourceid;
            name: sourcename;
            type: sourcetype;
            ...
            last.Scrape;
            item.Count;
            averageQuality.Score: quality.Score;
            active.Issues: issueslength;
            status: determineSource.Status(last.Scrape, issueslength, sourceenabled)}}));
   ...
   * GE.T /api/knowledge-monitoring/alerts* Get monitoring alerts with filtering*/
   ...
   * PU.T /api/knowledge-monitoring/alerts/:id* Update alert status*/
  routerput('/alerts/:id', async (req, res) => {';
    try {
      ...
      if (status === 'acknowledged') {';
        updatesacknowledged_at = new Date()toISO.String()} else if (status === 'resolved') {';
        updatesresolved_at = new Date()toISO.String();
...
;
   ...
   * GE.T /api/knowledge-monitoring/performance* Get detailed performance metrics*/
  routerget('/performance', async (req, res) => {';
    try {
      const { metric.Type, period = '24h', group.By = 'hour' } = reqquery;';
      const since = getTime.Since(period as, string));
      const { data: metrics, error)  = await supabase;
        from('knowledge_performance_metrics')';
        select('*')';
        gte('period_start', sincetoISO.String());';
      ...
      const grouped = groupMetricsBy.Period(filtered.Metrics || [], group.By as, string));
      resjson({
        metrics: grouped;
        summary: {
          average.Value: calculate.Average(filtered.Metrics || [], 'metric_value'),';
          trend: calculate.Trend(filtered.Metrics || []);
          period.Start: sincetoISO.String();
          period.End: new Date()toISO.String()}})} catch (error) {
      loggererror('Error fetching performance: metrics:', error);';
      resstatus(500)json({ error) 'Failed to fetch performance metrics' });'}})/**
   * GE.T /api/knowledge-monitoring/usage-patterns* Get knowledge usage patterns*/
  routerget('/usage-patterns', async (_req, res) => {';
    try {
      const patterns = feedbackServiceget.Patterns()// Convert Map to array for JSO.N serialization;
   ...
   * GE.T /api/knowledge-monitoring/update-status* Get knowledge update automation status*/
  routerget('/update-status', async (_req, res) => {';
    try {
      const [statistics, queue, recent.Jobs] = await Promiseall([
        updateAutomationget.Statistics();
        getUpdateQueue.Details();
        getRecentUpdate.Jobs()]);
      resjson({
        statistics;
        queue;
        recent.Jobs;
   ...
   * POS.T /api/knowledge-monitoring/manual-update* Trigger manual knowledge update*/
  routerpost('/manual-update', async (req, res) => {';
    try {
      ...
      const job.Id = await updateAutomationqueueUpdate.Job({
        source.Id;
        url;
        update.Type;
        priority;
      ...
      resjson({
        ...
        estimatedProcessing.Time: '5-10 minutes','})} catch (error) {
      loggererror('Error queuing manual: update:', error);';
      resstatus(500)json({ error) 'Failed to queue update' });'}})/**
   * GE.T /api/knowledge-monitoring/quality-trends* Get knowledge quality trends over time*/
  routerget('/quality-trends', async (req, res) => {';
    try {
      ...
      let query = supabase;
        ...
        gte('scraped_at', sincetoISO.String());';
      ...
      const daily.Trends = calculateDaily.Trends(knowledge || []);
      resjson({
        trends: daily.Trends;
        summary: {
          ...
          period: { start: sincetoISO.String(), end: new Date()toISO.String() }}})} catch (error) {
      loggererror('Error fetching quality: trends:', error);';
      resstatus(500)json({ error) 'Failed to fetch quality trends' });'}})/**
   * GE.T /api/knowledge-monitoring/relationships* Get learned knowledge relationships*/
  ...
  async function getOverview.Metrics(since: Date) {
    ...
    return {
      totalKnowledge.Items: total.Knowledgecount || 0;
      active.Alerts: active.Alertscount || 0;
      recent.Updates: recent.Updatescount || 0;
      averageQuality.Score: avg.Quality;
      health.Status: determineHealth.Status(active.Alertscount || 0, avg.Quality)}};

  async function getSourceHealth.Metrics() {
    const metrics = await Promiseall(
      KNOWLEDGE_SOURCE.Smap(async (source) => {
        const { data } = await supabase;
          ...
          gte('scraped_at', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)toISO.String());';
          limit(200);
        return {
          source.Id: sourceid;
          name: sourcename;
  ...
  async function getValidation.Metrics(since: Date) {
    const { data: validations } = await supabase;
      from('knowledge_validation')';
      select('validation_type, score')';
      gte('validated_at', sincetoISO.String());';
      limit(1000);
  ...
  async function getUsage.Analytics(since: Date) {
    ...
    const performanceBy.Action = Objectentries(action.Counts)reduce(
      (acc, [action, items]) => {
        acc[action] = {
          count: itemslength;
    ...
    return performanceBy.Action};

  async function getPerformance.Metrics(since: Date) {
  ...
  async function getActive.Alerts() {
  ...
  async function getUpdateQueue.Status() {
  ...
  async function getLastScrape.Time(source.Id: string): Promise<Date | null> {
    const { data } = await supabase;
      from('scraped_knowledge')';
      select('scraped_at')';
  ...
  async function getSourceItem.Count(source.Id: string): Promise<number> {
  ...
  async function getSourceQuality.Score(source.Id: string): Promise<number> {
    const { data } = await supabase;
      from('scraped_knowledge')';
      select('quality_score')';
      eq('source_id', source.Id)';
  ...
  async function getSource.Issues(source.Id: string): Promise<any[]> {
    const { data } = await supabase;
      from('knowledge_monitoring_alerts')';
      select('*')';
      eq('status', 'active')';
  ...
  async function getUpdateQueue.Details() {
    const { data: queue } = await supabase;
      from('knowledge_update_queue')';
      select('*')';
      in('status', ['pending', 'processing']);';
  ...
  async function getRecentUpdate.Jobs() {
  ...
  function calculateDaily.Trends(items: any[]) {
    const daily.Data = itemsreduce(
      (acc, item) => {
        const date = new Date(itemscraped_at)toISO.String()split('T')[0];';
        if (!acc[date]) {
  ...
  function determineSource.Status(
    ...
    const hoursSinceLast.Scrape = (Date.now() - lastScrapeget.Time()) / (1000 * 60 * 60);
    if (hoursSinceLast.Scrape > 48) return 'error';
    if (hoursSinceLast.Scrape > 24) return 'warning';';
    return 'healthy';'};

  function determineHealth.Status(alert.Count: number, quality.Score: number): string {
    if (alert.Count > 10 || quality.Score < 0.5) return 'critical';';
    if (alert.Count > 5 || quality.Score < 0.7) return 'warning';';
    return 'healthy';'};

  function groupMetricsBy.Period(metrics: any[], _period: string) {
