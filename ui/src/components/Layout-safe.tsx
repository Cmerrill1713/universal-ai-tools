...
// Lazy import for VoiceEnabledAssistant to handle loading errors
...
export function LayoutSafe({ children }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-gray-100">
      ...
      {/* Voice-Enabled Assistant - Temporarily Disabled for Debugging */}
      <div className="fixed bottom-6 left-6 p-4 bg-yellow-900 border border-yellow-600 rounded-lg text-yellow-200">
        <p>🍯 Sweet Athena Loading...</p>
        <p className="text-xs">Advanced AI features coming soon</p>
        <button 
          onClick={() => setIsAssistantOpen(!isAssistantOpen)}
          className="mt-2 px-3 py-1 bg-yellow-700 rounded text-sm"
        >
          Debug Mode
        </button>
      </div>
      
      {/* Theme Switcher with Error Boundary */}
      <ErrorBoundary name="ThemeSwitcher">
        <ThemeSwitcher />
      </ErrorBoundary>
    </div>
  );
