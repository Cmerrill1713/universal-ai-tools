...
jestmock('././utils/loggerjs', () => ({
  logger: {
    info: jestfn(),
    error instanceof Error ? error.message : String(error) jestfn();
    warn: jestfn(),
    debug: jestfn(),
  }}));
jestmock('././services/supabase_servicejs', () => ({
  Supabase.Service: {
    get.Instance: jestfn(() => ({
      client: {
        from: jestfn(() => ({
          select: jestfn(() => Promiseresolve({ data: [], error instanceof Error ? error.message : String(error) null }));
          upsert: jestfn(() => Promiseresolve({ error instanceof Error ? error.message : String(error) null })),
          insert: jestfn(() => Promiseresolve({ data: [], error instanceof Error ? error.message : String(error) null }))}))}}))}}));
...
describe('Dynamic.Context.Manager', () => {
  ...
  describe('Context Optimization', () => {
    ...
    it('should apply importance-based selection for code generation', async () => {
      ...
      const code.Messages = optimizedfilter((m) => mcontent.includes('```'));
