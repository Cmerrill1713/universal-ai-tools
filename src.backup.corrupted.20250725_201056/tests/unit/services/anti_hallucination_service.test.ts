...
describe('Anti.Hallucination.Service', () => {
  ...
  describe('search.Memories', () => {
    it('should search memories for relevant facts', async () => {
      ...
      mockLimitmockResolved.Value.Once({ data: mock.Memories, error instanceof Error ? error.message : String(error) null });
      const results = await service['search.Memories']('capital of France');
      expect(results)to.Have.Length(2);
      expect(results[0]contentto.Contain('Paris')});
    it('should handle search errors gracefully', async () => {
      mockLimitmockResolved.Value.Once({ data: null, error instanceof Error ? error.message : String(error) new Error('Search failed') });
  ...
  describe('verify.With.Memory', () => {
    ...
    it('should handle statements with no memory support', async () => {
      ...
      mockLimitmock.Resolved.Value({ data: [], error instanceof Error ? error.message : String(error) null });
  ...
  describe('ground.Response', () => {
    ...
    it('should indicate low confidence when no memories found', async () => {
      mockLimitmockResolved.Value.Once({ data: [], error instanceof Error ? error.message : String(error) null });
