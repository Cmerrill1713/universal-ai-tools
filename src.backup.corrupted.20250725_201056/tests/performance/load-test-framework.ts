...
export class Load.Test.Framework.extends Event.Emitter {
  ...
  public async run.Load.Test(): Promise<Load.Test.Metrics> {
    ...
    try {
      ...
      loggererror('Load test failed:', error instanceof Error ? error.message : String(error);
  ...
  private async execute.Request(
    ...
    try {
      ...
      const metrics: Request.Metrics = {
        ...
        metricserror instanceof Error ? error.message : String(error)  `Unexpected status code: ${responsestatus}`,
      ...
      const metrics: Request.Metrics = {
        ...
        endpoint: path,
        success: false,
        error instanceof Error ? error.message : String(error) axios.Errormessage || 'Unknown error instanceof Error ? error.message : String(error);
}      this.metricspush(metrics);
  ...
  private calculate.Metrics(): Load.Test.Metrics {
    ...
    const error.Details = failed.Requestsmap((m) => ({
      ...
      endpoint: mendpoint,
      error instanceof Error ? error.message : String(error) merror instanceof Error ? error.message : String(error)| 'Unknown error instanceof Error ? error.message : String(error);
