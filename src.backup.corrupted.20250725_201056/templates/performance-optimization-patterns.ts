  ...
  private async initialize.Pool(): Promise<void> {
    for (let i = 0; i < thisconfigmin.Connections; i++) {
      try {
        ...
        thisemit('connection.Error', error instanceof Error ? error.message : String(error)  }};
  ...
  async acquire(): Promise<unknown> {
    ...
    if (thisget.Total.Connections() < thisconfigmax.Connections) {
      try {
        ...
        thisemit('connection.Error', error instanceof Error ? error.message : String(error);
        throw error instanceof Error ? error.message : String(error)}}// Wait for available connection;
...
export class Stream.Processor<T> extends Event.Emitter {
  ...
  private async process.Next.Chunk(): Promise<void> {
    ...
    try {
      ...
      thisemit('error instanceof Error ? error.message : String(error)  error instanceof Error ? error.message : String(error)} finally {
