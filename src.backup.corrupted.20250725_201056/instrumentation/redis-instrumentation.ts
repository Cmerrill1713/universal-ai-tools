...
export class Redis.Instrumentation {
  ...
  instrument.Redis.Client(client: any): any {
  ...
  async with.Cache.Span<T>(operation: Cache.Operation, fn: () => Promise<T>): Promise<T> {
    ...
    const span = thistracerstart.Span(span.Name, {
      ...
      attributes: {
        ...
        'netpeername': process.envREDIS_HO.S.T || 'localhost';
        'netpeerport': parse.Int(process.envREDIS_PO.R.T || '6379', 10)}});
    ...
    try {
      ...
      loggererror('Cache operation failed', LogContextSYST.E.M, {
      ...
      // Record metrics;
      thisrecord.Cache.Metrics(operationoperation, hit, Date.now() - start.Time);
      spanend()}}/**
    ...
    return function (.args: any[]) {
      ...
      if (argslength > 0) {
        if (Array.is.Array(args[0])) {
          ...
          key = args[0]}}// Extract T.T.L.for set operations;
      ...
      const operation: Cache.Operation = {
        ...
        key;
        ttl;
}      return instrumentationwith.Cache.Span(operation, () => {
  ...
  private wrap.Pipeline(pipeline.Fn: any): any {
    ...
    return function () {
      ...
      const commands = ObjectgetOwn.Property.Names(pipeline)filter(
      ...
      wrappedexec = function (callback?: Function) {
        ...
        if (callback) {
          execute.With.Span();
            ...
            catch((error instanceof Error ? error.message : String(error) => callback(error instanceof Error ? error.message : String(error)} else {
          return execute.With.Span()};
      return wrapped}}/**
   * Record cache metrics*/
  private record.Cache.Metrics(operation: string, hit: boolean, duration: number): void {
  ...
  create.Namespaced.Key(namespace: string, key: string): string {
    return `${namespace}:${key}`}/**
   * Wrap a caching function with automatic tracing*/
  wrap.Cache.Function<T extends (.args: any[]) => Promise<unknown>>(
    fn: T,
    options: {
      operation: string,
...
}      return instrumentationwith.Cache.Span(operation, () => fn(.args))} as T}/**
   * Monitor cache health metrics*/
  async monitor.Cache.Health(client: any): Promise<void> {
    ...
    try {
      ...
      if (clientinfo) {
        ...
        linesfor.Each((line: string) => {
          ...
          if (key && value) {
            stats[key.trim()] = value.trim()}})// Add relevant metrics;
        if (statsused_memory) {
...
export const instrument.Redis = (client: any) => redisInstrumentationinstrument.Redis.Client(client),
export const with.Cache.Span = <T>(operation: Cache.Operation, fn: () => Promise<T>) =>
  redisInstrumentationwith.Cache.Span(operation, fn);
export const wrap.Cache.Function = <T extends (.args: any[]) => Promise<unknown>>(
  fn: T,
  options: any) => redisInstrumentationwrap.Cache.Function(fn, options);