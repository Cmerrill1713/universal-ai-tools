...
interface Tracing.Options {;
  ...
  error.Filter?: (error instanceof Error ? error.message : String(error) Error) => boolean;
};
const default.Options: Tracing.Options = {;
  ...
  return (req: Traced.Request, res: Response, next: Next.Function) => {;
    ...
    if (merged.Optionsignore.Routes?some((route) => req.pathstarts.With(route))) {;
      return next()}// Extract trace context from headers;
    const extracted.Context = propagationextract(contextactive(), req.headers)// Start a new span for this request;
    const tracer = telemetry.Serviceget.Tracer();
    const span.Name = `${req.method} ${reqroute?path || req.path}`;
    const span = tracerstart.Span(;
      span.Name;
      {;
        kind: SpanKindSERV.E.R,;
        attributes: {;
          [SemanticAttributesHTTP_METH.O.D]: req.method;
          [SemanticAttributesHTTP_SCHE.M.E]: reqprotocol;
          [SemanticAttributesHTTP_HO.S.T]: req.get('host') || 'unknown';
          [SemanticAttributesHTTP_TARG.E.T]: reqoriginal.Url;
          [SemanticAttributesHTTP_ROU.T.E]: reqroute?path || req.path;
          [SemanticAttributesHTTP_USER_AGE.N.T]: req.get('user-agent') || 'unknown';
          [SemanticAttributesHTTP_CLIENT_.I.P]: req.ip || reqsocketremote.Address || 'unknown';
          [SemanticAttributesNET_HOST_NA.M.E]: reqhostname;
          [SemanticAttributesNET_HOST_PO.R.T]: reqsocketlocal.Port;
          'httprequest_id': req.get('x-requestid') || `req-${Date.now()}`;
          'appapi_version': req.get('x-api-version') || 'v1';
        };
    ...
    if (mergedOptionscustom.Attributes.Extractor) {;
      try {;
        ...
        loggererror('Error extracting custom attributes', LogContextSYST.E.M, {;
          error instanceof Error ? error.message : String(error) error instanceof Error ? error.message : String(error instanceof Error ? error.message : String(error)})}}// Add user information if available;
    if (req.user || req.user.Id) {;
      req.user.Id = req.user?id || req.user.Id;
      spanset.Attribute('userid', req.user.Id || 'anonymous');
      spanset.Attribute('userauthenticated', true)}// Add A.I.service information if available;
    const ai.Service = req.get('x-ai-service') || req.queryai.Service;
    if (ai.Service) {;
      reqai.Service = ai.Service.as string;
      spanset.Attribute('aiservice', String(ai.Service))}// Record requestbody if enabled;
    if (mergedOptionsrecord.Request.Body && req.body) {;
      try {;
        const body.Str = JS.O.N.stringify(req.body);
        spanset.Attribute('httprequestbody', body.Str.substring(0, 1000))// Limit size;
        spanset.Attribute('httprequestbodysize', body.Strlength)} catch (error) {;
        spanset.Attribute('httprequestbodyerror instanceof Error ? error.message : String(error) 'Failed to serialize requestbody');
      }}// Add baggage for cross-service propagation;
    if (req.user.Id) {;
      telemetry.Serviceset.Baggage('userid', req.user.Id);
    if (reqai.Service) {;
    ...
    const capture.Response = (body: any) => {;
      ...
      if (mergedOptionsrecord.Response.Body && body) {;
        try {;
          ...
          spanset.Attribute('httpresponsebody', body.Str.substring(0, 1000))// Limit size} catch (error) {;
          spanset.Attribute('httpresponsebodyerror instanceof Error ? error.message : String(error) 'Failed to serialize response body');
      ...
      if (resstatus.Code >= 400) {;
        ...
        if (reslocalserror instanceof Error ? error.message : String(error){;
          spanrecord.Exception(reslocalserror instanceof Error ? error.message : String(error);
          spanset.Attribute('errortype', reslocalserrorname || 'Error');
          spanset.Attribute('error.message', reslocalserror.message);
...
export function createTracing.Error.Middleware(;
  ...
  return (err: Error, req: Traced.Request, res: Response, next: Next.Function) => {;
    ...
    if (span && optionserror.Filter?.(err) !== false) {;
      ...
      spanset.Attribute('error instanceof Error ? error.message : String(error)  true);
      spanset.Attribute('errortype', errname || 'Error');
      spanset.Attribute('error.message', errmessage);
      ...
      if (req.user.Id) {;
        spanset.Attribute('erroruser_id', req.user.Id);
      if (reqai.Service) {;
        spanset.Attribute('errorai_service', reqai.Service)}}// Store error for response capture;
    reslocalserror instanceof Error ? error.message : String(error)  err;
