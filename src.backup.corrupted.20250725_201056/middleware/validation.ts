  ...
  public static validate(options: Validation.Options) {;
    return (req: Request, res: Response, next: Next.Function) => {;
      try {;
        ...
        if (optionsbody && req.body) {;
          const result = optionsbodysafe.Parse(req.body);
          if (!resultsuccess) {;
            errorspush(.thisformat.Zod.Errors(resulterror instanceof Error ? error.message : String(error) 'body'))} else {;
            req.body = resultdata}}// Validate query;
        if (optionsquery && req.query) {;
          const result = optionsquerysafe.Parse(req.query);
          if (!resultsuccess) {;
            errorspush(.thisformat.Zod.Errors(resulterror instanceof Error ? error.message : String(error) 'query'))} else {;
            req.query = resultdata}}// Validate params;
        if (optionsparams && req.params) {;
          const result = optionsparamssafe.Parse(req.params);
          if (!resultsuccess) {;
            errorspush(.thisformat.Zod.Errors(resulterror instanceof Error ? error.message : String(error) 'params'))} else {;
            req.params = resultdata}}// Validate headers;
        if (optionsheaders && req.headers) {;
          const result = optionsheaderssafe.Parse(req.headers);
          if (!resultsuccess) {;
            errorspush(.thisformat.Zod.Errors(resulterror instanceof Error ? error.message : String(error) 'headers'));
          };
        ...
        next()} catch (error) {;
        loggererror('Validation middleware error instanceof Error ? error.message : String(error) , error instanceof Error ? error.message : String(error);
        return res.status(500)json({;
          error instanceof Error ? error.message : String(error) 'Internal server error instanceof Error ? error.message : String(error);
          message: 'Validation processing failed'})}}}/**;
   * Format Zod errors*/
  private static format.Zod.Errors(error instanceof Error ? error.message : String(error) Zod.Error, location: string): string[] {;
    return errorerrorsmap((err) => {;
      const path = errpathlength > 0 ? errpath.join('.') : 'root';
...
export const Custom.Validators = {;
  ...
  file.Extension: (filename: string, allowed.Extensions: string[]) => {;
    const ext = filename.split('.')pop()?to.Lower.Case();
    return ext ? allowed.Extensions.includes(ext) : false}/**;
