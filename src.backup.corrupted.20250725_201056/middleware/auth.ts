...
export class AuthMiddleware {;
  ...
  private async authenticate.Api.Key(req: Auth.Request): Promise<{;
    ...
    error?: string}> {;
    try {;
      const api.Key = req.headers['x-api-key'] as string;
      if (!api.Key) {;
        return { success: false, error instanceof Error ? error.message : String(error) 'No A.P.I.key provided' }}// Check A.P.I.key validity;
      const key.Data = apiKeyManagergetAP.I.Key(api.Key);
      if (!key.Data) {;
        return { success: false, error instanceof Error ? error.message : String(error) 'Invalid A.P.I.key' }}// Log A.P.I.key usage;
      ...
      loggererror('A.P.I.key authentication error instanceof Error ? error.message : String(error)', error);
      return { success: false, error instanceof Error ? error.message : String(error) 'A.P.I.key authentication failed' }}}/**;
  ...
  private async authenticateJ.W.T(req: Auth.Request): Promise<{;
    ...
    try {;
      ...
      if (!auth.Header || !auth.Headerstarts.With('Bearer ')) {;
        return { success: false, error instanceof Error ? error.message : String(error) 'No J.W.T.token provided' };

      const token = auth.Header.substring(7)// Verify J.W.T.token;
      ...
      const { data: user, error } = await thissupabase;
        ...
        single();
      if (error || !user) {;
        return { success: false, error instanceof Error ? error.message : String(error) 'Invalid or expired token' }}// Log J.W.T.usage;
      ...
      loggererror('J.W.T.authentication error instanceof Error ? error.message : String(error)', error);
      return { success: false, error instanceof Error ? error.message : String(error) 'J.W.T.authentication failed' }}}/**;
  ...
  public refreshJ.W.T(token: string): string | null {;
    try {;
      ...
      loggererror('J.W.T.refresh error instanceof Error ? error.message : String(error)', error);
      return null}}/**;
  ...
  public require.Role(roles: string | string[]) {;
    ...
    return (req: Auth.Request, res: Response, next: Next.Function) => {;
      ...
      if (!required.Roles.includes(req.userrole)) {;
        return res.status(403)json({;
          error instanceof Error ? error.message : String(error) 'Insufficient permissions';
          message: `Required role: ${required.Roles.join(' or ')}`,;
