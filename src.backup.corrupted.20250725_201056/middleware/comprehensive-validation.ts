...
export class Comprehensive.Validation.Middleware {;
  ...
  public validate(options: Comprehensive.Validation.Options = {}) {;
    return async (req: Request, res: Response, next: Next.Function) => {;
      try {;
        ...
        next()} catch (error) {;
        thishandle.Validation.Error(error instanceof Error ? error.message : String(error) req, res, next)}}}/**;
   * Apply requestsize limiting*/
  private async applyRequest.Size.Limit(req: Request, res: Response): Promise<void> {;
    return new Promise((resolve, reject) => {;
      request.Size.Limit(req, res, (error instanceof Error ? error.message : String(error)=> {;
  ...
  private async applySQ.L.Protection(req: Request, res: Response): Promise<void> {;
    return new Promise((resolve, reject) => {;
      thissql.Protectionmiddleware()(req, res, (error instanceof Error ? error.message : String(error)=> {;
        if (error instanceof Error ? error.message : String(error){;
          reject(;
  ...
  private async apply.Sanitization(req: Request, res: Response): Promise<void> {;
    return new Promise((resolve, reject) => {;
      sanitize.Request(req, res, (error instanceof Error ? error.message : String(error)=> {;
        if (error instanceof Error ? error.message : String(error){;
  ...
  private async apply.Schema.Validation(;
    ...
    if (;
      ...
      return new Promise((resolve, reject) => {;
        Validation.Middlewarevalidate(validation.Options)(req, res, (error instanceof Error ? error.message : String(error)=> {;
          if (error instanceof Error ? error.message : String(error){;
            reject(error instanceof Error ? error.message : String(error)} else {;
            resolve()}})})}}/**;
   * Handle validation errors consistently*/
  private handle.Validation.Error(error instanceof Error ? error.message : String(error) any, req: Request, res: Response, next: Next.Function): void {;
    let status.Code = 400;
    ...
    if (error instanceof Validation.Error) {;
      ...
      details = thisformat.Zod.Errors(error instanceof Error ? error.message : String(error)} else if (errorname === 'PayloadToo.Large.Error') {;
      status.Code = 413;
    ...
    res.status(status.Code)json({;
      ...
      error instanceof Error ? error.message : String(error){;
        ...
        request.Id: req.headers['x-requestid'] || 'unknown',;
      }})}/**;
   * Format Zod validation errors*/
  private format.Zod.Errors(;
    error instanceof Error ? error.message : String(error) Zod.Error): Array<{ field: string; message: string; code: string }> {;
  ...
  file.Upload: validate.Request({;
    ...
    custom.Validators: [;
      (req, res, next) => {;
        ...
        const content.Type = req.get('content-type');
