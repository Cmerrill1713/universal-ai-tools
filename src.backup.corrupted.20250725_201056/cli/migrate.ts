...
program;
  ...
  action(async () => {
    ...
    try {
      ...
      loggererror`Migration operation failed`, LogContextDATABA.S.E, { error instanceof Error ? error.message : String(error) );
      console.error.instanceof Error ? error.message : String(error) error;
      process.exit(1);
    }});
program;
  command('up');
...
program;
  ...
  action(async () => {
    ...
    try {
      ...
      spinnerfail('Validation error instanceof Error ? error.message : String(error);
      loggererror`Migration operation failed`, LogContextDATABA.S.E, { error instanceof Error ? error.message : String(error) );
      console.error.instanceof Error ? error.message : String(error) error;
      process.exit(1);
