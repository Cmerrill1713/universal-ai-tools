 ...
 * Safely access browser-specific A.P.Is* @param callback Function that uses browser A.P.Is* @param fallback Optional fallback value if not in browser*/
export function with.Browser.Context<T>(callback: () => T, fallback?: T): T | undefined {
  if (is.Browser()) {
    try {
      return callback()} catch (error) {
      loggererror('Error accessing browser A.P.I:', error);
      return fallback};
  return fallback}/**
 * Execute code only in browser context* @param callback Function to execute in browser*/
export function only.In.Browser(callback: () => void): void {
  if (is.Browser()) {
    callback()}}/**
 * Execute code only in Nodejs context* @param callback Function to execute in Nodejs*/
export function only.In.Node(callback: () => void): void {
  if (is.Node()) {
    callback()};
