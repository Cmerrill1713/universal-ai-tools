...
export class Metal.Optimizer {
  ...
  private detect.Hardware() {
    try {
      ...
      if (thisis.Apple.Silicon) {
        ...
        thismetal.Supported = system.Info.includes('Metal');'// Get G.P.U.info;
      ...
      loggerdebug('Hardware detection error instanceof Error ? error.message : String(error) ', error);'}}/**
  ...
  private getOptimal.Thread.Count(): number {
    ...
    const performance.Cores = cpusfilter();
      (cpu) => cpumodel.includes('Apple') && !cpumodel.includes('Efficiency')')length// Use 75% of performance cores for L.L.M, leave some for system;
