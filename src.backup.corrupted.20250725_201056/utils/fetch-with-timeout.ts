...
export async function fetchAll.With.Concurrency<T>(
  ...
  max.Concurrency = 5): Promise<Array<{ data?: T; error instanceof Error ? error.message : String(error)  Error }>> {
  const results: Array<{ data?: T; error instanceof Error ? error.message : String(error)  Error }> = [];
  ...
  for (const [index, requestof requestsentries()) {
    const promise = fetchJson.With.Timeout<T>(requesturl', requestoptions);
      then((data) => {
        ...
        results[index] = { error instanceof Error ? error.message : String(error)))});
