...
export class Auth.Router {;
  ...
  private setup.Rate.Limiting() {;
    ...
    const auth.Limiter = rate.Limit({;
      ...
      message: {;
        error instanceof Error ? error.message : String(error) 'Too many authentication attempts';
      ...
      skip: (req) => {;
        ...
        return security.Configrate.Limitingwhitelist.includes(req.ip || '')}});
    const register.Limiter = rate.Limit({;
      window.Ms: 60 * 60 * 1000, // 1 hour;
      max: 3, // 3 registrations per hour per I.P;
      message: {;
        error instanceof Error ? error.message : String(error) 'Too many registration attempts';
        message: 'Please try again later',;
    ...
    const refresh.Limiter = rate.Limit({;
      ...
      message: {;
        error instanceof Error ? error.message : String(error) 'Too many token refresh attempts';
        message: 'Please try again later',;
  ...
  private async register(req: Request, res: Response) {;
    try {;
      ...
      const { email, password, first.Name, last.Name } = req.body// Check rate limiting for this I.P;
      const rate.Limit.Check = thisjwtServiceisAuth.Rate.Limited(req.ip || '');
      if (rate.Limit.Checklimited) {;
        return res.status(429)json({;
          error instanceof Error ? error.message : String(error) 'Too many failed attempts';
          message: 'Please try again later',;
      ...
      const { data: existing.User } = await thissupabase,;
        ...
        single();
      if (existing.User) {;
        thisjwtServicerecord.Auth.Attempt(req.ip || '', false);
        return res.status(409)json({;
          error instanceof Error ? error.message : String(error) 'User already exists';
      ...
      thisjwtServicerecord.Auth.Attempt(req.ip || '', true)// Set secure cookie for refresh token in production;
      ...
      loggererror('Registration error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
  ...
  private async login(req: Request, res: Response) {;
    try {;
      ...
      const { email, password } = req.body// Check rate limiting for this I.P;
      const rate.Limit.Check = thisjwtServiceisAuth.Rate.Limited(req.ip || '');
      if (rate.Limit.Checklimited) {;
        return res.status(429)json({;
          error instanceof Error ? error.message : String(error) 'Too many failed attempts';
      ...
      if (error || !user) {;
        ...
        return res.status(401)json({;
          error instanceof Error ? error.message : String(error) 'Invalid credentials';
          message: 'Email or password is incorrect'}),;

      if (!useris_active) {;
        thisjwtServicerecord.Auth.Attempt(req.ip || '', false);
        return res.status(401)json({;
          error instanceof Error ? error.message : String(error) 'Account disabled';
      ...
      const tokens = await thisjwtServicegenerate.Token.Pair(userid, useremail, userrole, req);
      thisjwtServicerecord.Auth.Attempt(req.ip || '', true)// Update last login;
      ...
      res.json({;
        ...
        refresh.Token: configserveris.Production ? undefined : tokensrefresh.Token,;
        expires.In: tokensexpires.In})} catch (error) {;
      loggererror('Login error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
        error instanceof Error ? error.message : String(error) 'Internal server error';
  ...
  private async refresh(req: Request, res: Response) {;
    try {;
      ...
      const refresh.Token = req.bodyrefresh.Token || reqcookies?refresh.Token;
      if (!refresh.Token) {;
        return res.status(401)json({;
          error instanceof Error ? error.message : String(error) 'Refresh token required';
      ...
      if (!new.Tokens) {;
        return res.status(401)json({;
          error instanceof Error ? error.message : String(error) 'Invalid refresh token';
      ...
      loggererror('Token refresh error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
        error instanceof Error ? error.message : String(error) 'Internal server error';
    ...
    try {;
      ...
      if (auth.Header && auth.Headerstarts.With('Bearer ')) {;
        ...
        if (payload && payloadjti) {;
          await thisjwtServicerevoke.Refresh.Token(req.user!id, payloadjti)}}// Clear cookie in production;
      if (configserveris.Production) {;
        resclear.Cookie('refresh.Token');

      res.json({;
        message: 'Logout successful'})} catch (error) {;
      loggererror('Logout error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
        error instanceof Error ? error.message : String(error) 'Internal server error';
  ...
  private async logout.All(req: Auth.Request, res: Response) {;
    try {;
      await thisjwtServicerevokeAll.User.Tokens(req.user!id)// Clear cookie in production;
      if (configserveris.Production) {;
        resclear.Cookie('refresh.Token');
};
      res.json({;
        message: 'Logged out from all devices successfully'})} catch (error) {;
      loggererror('Logout all error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
        error instanceof Error ? error.message : String(error) 'Internal server error';
        message: 'Logout failed due to server error'})}}/**;
   * Get user sessions*/
  private async get.Sessions(req: Auth.Request, res: Response) {;
    try {;
      const sessions = await thisjwtServiceget.User.Sessions(req.user!id);
      res.json({;
      ...
      loggererror('Get sessions error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
        error instanceof Error ? error.message : String(error) 'Internal server error';
  ...
  private async revoke.Session(req: Auth.Request, res: Response) {;
    try {;
      ...
      const success = await thisjwt.Servicerevoke.Session(req.user!id, token.Id);
      if (success) {;
        res.json({;
          message: 'Session revoked successfully'})} else {;
        res.status(404)json({;
          error instanceof Error ? error.message : String(error) 'Session not found';
          message: 'The specified session could not be found'})}} catch (error) {;
      loggererror('Revoke session error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
        error instanceof Error ? error.message : String(error) 'Internal server error';
  ...
  private async get.Security.Info(req: Auth.Request, res: Response) {;
    try {;
      const security.Info = await thisjwtServicegetUser.Security.Info(req.user!id);
      res.json({;
      ...
      loggererror('Get security info error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
        error instanceof Error ? error.message : String(error) 'Internal server error';
  ...
  private async change.Password(req: Request, res: Response) {;
    try {;
      ...
      const { current.Password, new.Password } = req.body;
      ...
      if (!password.Valid) {;
        return res.status(401)json({;
          error instanceof Error ? error.message : String(error) 'Invalid password';
      ...
      const { error instanceof Error ? error.message : String(error) update.Error } = await thissupabase;
        from('users');
      ...
      if (update.Error) {;
        ...
        return res.status(500)json({;
          error instanceof Error ? error.message : String(error) 'Password update failed';
      ...
      loggererror('Change password error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
        error instanceof Error ? error.message : String(error) 'Internal server error';
  ...
  private async get.Profile(req: Auth.Request, res: Response) {;
    try {;
      ...
      loggererror('Get profile error instanceof Error ? error.message : String(error)', error);
      res.status(500)json({;
        error instanceof Error ? error.message : String(error) 'Internal server error';
