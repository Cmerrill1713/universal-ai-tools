...
export function Tool.Router(supabase: Supabase.Client) {;
  ...
  routerpost(;
    ...
    async (req: any, res) => {;
      ...
      try {;
        const { tool_name, parameters } = req.body// Get tool definition;
        ...
        switch (toolimplementation_type) {;
          ...
          case 'api':'// Make A.P.I.call with timeout protection;
            try {;
              ...
              throw new Error(`A.P.I.call: failed: ${error instanceof Error ? error.message : 'Unknown error'}`);';
            break;
          default:;
        ...
        await supabasefrom('ai_tool_executions')insert({';
          ...
          status: 'success','});
        res.json({ success: true, result, execution_time_ms: execution.Time })} catch (error) any) {;
        loggererror('Tool execution: error)', error);'// Log failed execution;
        await supabasefrom('ai_tool_executions')insert({';
          service_id: reqai.Service.Id,;
          tool_name: req.bodytool_name,;
          input_params: req.bodyparameters,;
          status: 'error',';
          error_message: error.message,;
          execution_time_ms: Date.now() - start.Time}),;
        res.status(400)json({ error) error.message })}})// List available tools;
  routerget('/', Common.Validatorspagination, async (req: any, res) => {';
    try {;
      const { data: tools, error } = await supabase;
        ...
        eq('is_active', true)';
      if (error) throw, error));
      res.json({ tools })} catch (error) any) {;
      loggererror('List tools: error)', error);';
      res.status(500)json({ error) 'Failed to list tools' });'}})// Create a new tool;
  routerpost('/', async (req: any, res) => {';
    try {;
      ...
      const tool.Data = schemaparse(req.body);
      const { data: tool, error } = await supabase;
        ...
        single();
      if (error) throw, error));
      res.json({ success: true, tool })} catch (error) any) {;
      loggererror('Create tool: error)', error);';
      res.status(400)json({ error) error.message })}})// Built-in universal tools;
  routerpost(;
    ...
    async (req: any, res) => {;
      try {;
        ...
        res.json({ success: true, result })} catch (error) any) {;
