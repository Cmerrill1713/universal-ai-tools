...
const storage = multerdisk.Storage({;
  destination: async (req, file, cb) => {;
    const upload.Dir = path.join(__dirname, '././uploads/audio');';
...
export function Speech.Router(supabase: Supabase.Client) {;
  ...
  routerpost('/transcribe', uploadsingle('audio'), async (req: any, res) => {';
    try {;
      ...
      const { conversation_id, context } = req.body// Transcribe the audio;
      ...
      res.status(500)json({;
        ...
        details: error.message})}})// Voice synthesis endpoint,;
  routerpost('/synthesize', async (req: any, res) => {';
    try {;
      ...
      const data = schemaparse(req.body)// Get voice profile based on personality;
      ...
      res.send(audio.Resultbuffer)} catch (error) any) {;
      loggererror('loggererror('Synthesis: error) , error);';
      res.status(500)json({;
        error) 'Failed to synthesize speech',';
        details: error.message})}})// Kokoro T.T.S.synthesis endpoint (high-quality local, T.T.S));
  routerpost(;
    '/synthesize/kokoro',';
    ...
    async (req: any, res) => {;
      ...
      try {;
        ...
        const audio.Buffer = await kokoroT.T.Ssynthesize({;
          ...
          temperature: 0.7,;
          token.Length: Math.min(200, datatext.split(/\s+/)length), // Optimal for Kokoro})// Set response headers;
        resset({;
        ...
        res.send(audio.Buffer)} catch (error) any) {;
  ...
  routerget('/voices', async (req: any, res) => {';
    try {;
      ...
      res.status(500)json({;
        ...
        details: error.message})}})// Voice configuration endpoint,;
    ...
    try {;
      const schema = zobject({;
        ...
        settings: z,;
          ...
          optional()});
      const data = schemaparse(req.body)// Update voice configuration;
      ...
      res.status(500)json({;
        ...
        details: error.message})}})// Get voice history endpoint,;
  routerget('/history/:conversation_id', async (req: any, res) => {';
    try {;
      ...
      res.status(500)json({;
        ...
        details: error.message})}})// Health check endpoint for speech services,;
  routerget('/health', async (req: any, res) => {';
    try {;
      const health = await speechServiceget.Service.Health();
      res.status(healthstatus === 'unhealthy' ? 503 : 200)json({';
        success: true.health,;
        timestamp: new Date()toIS.O.String()})} catch (error) any) {;
      ...
      res.status(500)json({;
        ...
        error) {;
          ...
          message: error.message}})}})// Test Kokoro voice endpoint,;
  routerpost('/test/kokoro/:voice.Id', async (req: any, res) => {';
    try {;
      const { voice.Id } = req.params;
      const { text } = req.body;
      ...
      res.status(500)json({;
        ...
        error) {;
          ...
          message: error.message}})}})// Clear caches endpoint,;
    ...
    try {;
      ...
      res.status(500)json({;
        ...
        error) {;
          ...
          message: error.message}})}})// Speech synthesis with retry endpoint,;
  routerpost('/synthesize/retry', async (req: any, res) => {';
    try {;
      ...
      res.status(500)json({;
        ...
        details: error.message})}})// Audio duration estimation endpoint,;
  routerpost('/estimate-duration', async (req: any, res) => {';
    try {;
      ...
      res.json({;
        ...
        word_count: datatext.trim()split(/\s+/)length,;
