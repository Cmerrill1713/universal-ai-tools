...
export function Chat.Router(supabase: Supabase.Client) {;
  ...
  routerpost('/', async (req: any, res) => {';
    try {;
      const data = chat.Request.Schemaparse(req.body);
  ...
  routerget('/history/:conversation.Id', async (req: any, res) => {';
    try {;
      ...
      const limit = parse.Int(req.querylimit) || 50;
      const offset = parse.Int(req.queryoffset) || 0// Search for conversation messages;
      const search.Options = {;
        query: '',';
        filters: {;
          conversation.Id;
  ...
  routerdelete('/history/:conversation.Id', async (req: any, res) => {';
    try {;
      const { conversation.Id } = req.params// Note: This would need to be implemented in the memory system// For now, we'll just log the request';
