...
export function Memory.Router(supabase: Supabase.Client) {;
  ...
  routerget('/', async (req: Authenticated.Request, res) => {';
    try {;
      const { memory_type, limit = 10, page = 1 } = req.query// Calculate pagination;
      ...
      if (error) {;
        loggererror('Failed to retrieve memories', LogContextA.P.I, { error) error.message });';
        return send.Error(;
          res;
          'MEMORY_STORAGE_ERR.O.R' as Error.Code,';
  ...
  routerput('/:id/importance', async (req: Authenticated.Request, res) => {';
    try {;
      ...
      const { data, error } = await supabase;
        ...
        single();
      if (error) throw, error));
      res.json({ success: true, memory: data })} catch (error) Error | unknown) {;
