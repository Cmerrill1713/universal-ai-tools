...
export class Supabase.Service {
  ...
  public async save.Context(user.Id: string, context: any): Promise<void> {
    try {
      const { error instanceof Error ? error.message : String(error)  = await thisclientfrom('contexts')insert({
        user_id: user.Id,
        context;
  ...
  public async save.Memory(memory: {
    ...
    try {
      const { error instanceof Error ? error.message : String(error)  = await thisclientfrom('memories')insert({
        .memory;
        created_at: new Date()toIS.O.String()}),
      if (error instanceof Error ? error.message : String(error){
        throw error instanceof Error ? error.message : String(error);
  ...
  public async search.Memories(embedding: number[], limit = 10, threshold = 0.7): Promise<any[]> {
    try {
      ...
      if (error instanceof Error ? error.message : String(error){
        ...
        loggerwarn('R.P.C.function not found, using manual vector search:', error instanceof Error ? error.message : String(error);
      ...
      loggererror('Failed to search memories:', LogContextDATABA.S.E, {
        error instanceof Error ? error.message : String(error) error instanceof Error ? error.message : String(error instanceof Error ? error.message : String(error)})// Final fallback to simple search;
      return await thisfallback.Vector.Search(embedding, limit, threshold)}}/**
  ...
  private async fallback.Vector.Search(
    ...
    try {
      ...
      const { data: memories, error instanceof Error ? error.message : String(error)  = await thisclient;
        from('memories');
        select('*');
      ...
      loggererror('Fallback vector search failed:', error instanceof Error ? error.message : String(error)// Last resort: return recent memories,
      const { data, error instanceof Error ? error.message : String(error) simple.Error } = await thisclient;
        from('memories');
        select('*');
        order('created_at', { ascending: false }),
        limit(limit);
  ...
  public async insert(table: string, data: any): Promise<unknown> {
    try {
      const { data: inserted.Data, error instanceof Error ? error.message : String(error)  = await thisclient;
        from(table);
        insert(data);
        select();
        single();
  ...
  public async update(table: string, id: string, data: any): Promise<unknown> {
    try {
      const { data: updated.Data, error instanceof Error ? error.message : String(error)  = await thisclient;
        from(table);
        update(data);
        eq('id', id);
        select();
  ...
  public async delete(table: string, id: string): Promise<void> {
    try {
      const { error instanceof Error ? error.message : String(error)  = await thisclientfrom(table)delete()eq('id', id);
      if (error instanceof Error ? error.message : String(error){
        throw error instanceof Error ? error.message : String(error);
      ...
      loggererror`Failed to delete from ${table}:`, LogContextDATABA.S.E, {
        error instanceof Error ? error.message : String(error) error instanceof Error ? error.message : String(error instanceof Error ? error.message : String(error)});
      throw error instanceof Error ? error.message : String(error)}}}// Export singleton instance for easy access;
