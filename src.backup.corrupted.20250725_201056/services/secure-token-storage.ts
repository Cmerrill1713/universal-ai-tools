...
class JWT.Token.Manager {
  ...
  private store.Refresh.Token(refresh.Token?: string) {
    ...
    try {
      ...
      console.error.instanceof Error ? error.message : String(error) Failed to store refresh token:', error instanceof Error ? error.message : String(error)  ;
  ...
  async refresh.Tokens(): Promise<boolean> {
    try {
      ...
      console.error.instanceof Error ? error.message : String(error) Token refresh failed:', error instanceof Error ? error.message : String(error);
      thisclear.Tokens();
      return false}}/**
