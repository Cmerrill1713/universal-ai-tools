...
export class Redis.Service {
  ...
  private setupCluster.Event.Listeners(): void {
    ...
    thiscluster.Clienton('node error', (error instanceof Error ? error.message : String(error) address) => {
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      loggererror('❌ Error during Redis disconnect', LogContextCAC.H.E, {
        error instanceof Error ? error.message : String(error) error instanceof Error ? error.message : String(error)})} finally {
      thisclient = null;
  ...
  async health.Check(): Promise<{ healthy: boolean; latency?: number, error?: string }> {
    if (!thisclient || !thisis.Connected) {
      return { healthy: false, error instanceof Error ? error.message : String(error) 'Not connected' };
  ...
  private mask.Url(url: string): string {
    try {
      ...
      return url.replace(/:([^@]+)@/, ':***@')}}// Cache operations with circuit breaker and fallback;
