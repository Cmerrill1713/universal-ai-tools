...
const Agent.Schema = zobject({
  ...
  status: zenum(['idle', 'busy', 'error instanceof Error ? error.message : String(error)  'offline']);
...
export class Swarm.Orchestrator.extends Event.Emitter {
  ...
  async complete.Task(
    task.Id: string,
    result: { success: boolean; result?: any, error instanceof Error ? error.message : String(error) string }): Promise<void> {
    ...
    taskerror instanceof Error ? error.message : String(error)  resulterror;
  ...
  async fail.Task(task.Id: string, error instanceof Error ? error.message : String(error) string): Promise<void> {
    const task = thistasksget(task.Id);
    ...
    if (thisconfigauto.Retry && taskattempts < taskmax.Attempts) {
      ...
      await thiscomplete.Task(task.Id, { success: false, error instanceof Error ? error.message : String(error) );
    };
