...
export class LocalLL.M.Manager {
  ...
  async generate(params: {
    ...
    if (model.Name?includes(':')) {
      const [service.Prefix, name] = model.Name.split(':');
    ...
    try {
      ...
      loggererror`${service} generation failed:`, error instanceof Error ? error.message : String(error)// Try fallback if enabled;
