...
export class Anti.Hallucination.Service {
  ...
  async ground.Response(prompt: string): Promise<{ response: string, citations: string[] }> {
    ...
    const { data: memories, error instanceof Error ? error.message : String(error)  = await thissupabase;
      from('ai_memories');
      select('*');
  ...
  private async search.Memories(response: string): Promise<Memory.Model[]> {
    ...
    const { data: memories, error instanceof Error ? error.message : String(error)  = await thissupabase;
      ...
      limit(20);
    if (error instanceof Error ? error.message : String(error) | !memories) {
      console.error.instanceof Error ? error.message : String(error) Error searching memories:', error instanceof Error ? error.message : String(error);
      return [];
  ...
  private async check.Citations(
    ...
    const cited.Facts = factsfilter((fact) => {
      ...
      return memoriessome((memory) => {
        ...
        if (claim.Lower.includes('capital') && content.Lower.includes('capital')) {
        ...
        return (
          content.Lower.includes(claim.Lower) || thissemantic.Similarity(factclaim, memorycontent> 0.7)})});
    const citations = memories;
      filter((m) => factssome((f) => mcontentto.Lower.Case()includes(fclaimto.Lower.Case())));
      map((m) => `Memory ${mid}: ${mcontent.substring(0, 100)}.`);
    return {
      ...
      return memoriessome((m) => {
        ...
        if (claim.Lower.includes('capital') && content.Lower.includes('capital')) {
          ...
          if (claim.Match && memory.Match) {
            ...
            return claim.Match[1] === memory.Match[1] && claim.Match[2] === memory.Match[2]}}// General matching;
        return (
          content.Lower.includes(claim.Lower) || thissemantic.Similarity(fclaim, mcontent> 0.7)})})length;
  ...
  private async generate.With.Citations(
    ...
    const citations = memoriesmap(
      (m, i) => `[${i + 1}] Memory ${mid}: ${mcontent.substring(0, 50)}.`);
  ...
  private is.Claim(sentence: string): boolean {
    ...
    if (
      sentence.includes('?') || lower.Sentence.includes('should we') || lower.Sentence.includes('we should')) {
