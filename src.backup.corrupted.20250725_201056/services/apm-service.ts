...
export interface Transaction {
  ...
  duration?: number;
  result: 'success' | 'error instanceof Error ? error.message : String(error) | 'timeout' | 'cancelled',
  spans: Span[],
...
export class AP.M.Service.extends Event.Emitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      loggererror('Failed to start A.P.M.service', LogContextPERFORMAN.C.E, { error instanceof Error ? error.message : String(error));
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      loggererror('Error stopping A.P.M.service', LogContextPERFORMAN.C.E, { error instanceof Error ? error.message : String(error));
      throw error instanceof Error ? error.message : String(error)}}/**
  ...
  record.Error(
    ...
    const fingerprint = thisgenerate.Error.Fingerprint(error instanceof Error ? error.message : String(error);
    ...
    if (transaction.Id) {
      const transaction = thistransactionsget(transaction.Id);
      if (transaction) {
        transactionresult = 'error instanceof Error ? error.message : String(error)  }}// Keep only recent errors (last 1000);
  ...
  generate.Report(duration.Minutes = 60): AP.M.Report {
    ...
    const top.Transactions = Arrayfrom(transaction.Groupsentries());
      map(([name, transactions]) => {
        ...
        const errors = transactionsfilter((t) => tresult === 'error instanceof Error ? error.message : String(error) length;
        return {
          name;
  ...
  private setup.Error.Handling(): void {
    ...
    process.on('uncaught.Exception', (error instanceof Error ? error.message : String(error)=> {
      thisrecord.Error(error instanceof Error ? error.message : String(error) { source: 'uncaught.Exception' })}),
    process.on('unhandled.Rejection', (reason) => {
      ...
      thisrecord.Error(error instanceof Error ? error.message : String(error) { source: 'unhandled.Rejection' })}),
  ...
  private async flush.Metrics(): Promise<void> {
    try {
      ...
      loggererror('Failed to flush A.P.M.metrics', LogContextPERFORMAN.C.E, { error instanceof Error ? error.message : String(error) );
    };
  ...
  private generate.Error.Fingerprint(error instanceof Error ? error.message : String(error) Error): string {
