...
export class Database.Migration.Service {
  ...
  async initialize(): Promise<void> {
    try {
      const { error instanceof Error ? error.message : String(error)  = await thissupabaserpc('create_migration_table', {
        sql: ``,
        ...
        `,`});
      if (error instanceof Error ? error.message : String(error) & !error.message.includes('already exists')) {
        throw error instanceof Error ? error.message : String(error);

      loggerinfo('Migration table initialized')} catch (error) {
      loggererror('Failed to initialize migration table:', error instanceof Error ? error.message : String(error);
      throw error instanceof Error ? error.message : String(error)}}/**
  ...
  async get.Migration.Files(): Promise<Migration[]> {
    try {
      ...
      loggererror('Failed to read migration files:', error instanceof Error ? error.message : String(error);
      throw error instanceof Error ? error.message : String(error)}}/**
  ...
  async get.Applied.Migrations(): Promise<Migration[]> {
    try {
      ...
      loggererror('Failed to get applied migrations:', error instanceof Error ? error.message : String(error);
      throw error instanceof Error ? error.message : String(error)}}/**
  ...
  async run.Migration(migration: Migration): Promise<void> {
    ...
    try {
      // Start transaction;
      const { error instanceof Error ? error.message : String(error) tx.Error } = await thissupabaserpc('begin_transaction');
      if (tx.Error) throw tx.Error;
      try {
        // Execute migration S.Q.L;
        const { error instanceof Error ? error.message : String(error) sql.Error } = await thissupabaserpc('execute_sql', {
          sql: migrationsql}),
        if (sql.Error) throw sql.Error// Record migration;
        const { error instanceof Error ? error.message : String(error) record.Error } = await thissupabasefrom(thismigration.Table)insert({
          id: migrationid,
          name: migrationname,
          checksum: migrationchecksum,
          execution_time_ms: Date.now() - start.Time}),
        if (record.Error) throw record.Error// Commit transaction;
        const { error instanceof Error ? error.message : String(error) commit.Error } = await thissupabaserpc('commit_transaction');
      ...
      loggererror`Migration failed: ${migrationname}`, error instanceof Error ? error.message : String(error);
      throw error instanceof Error ? error.message : String(error)}}/**
  ...
  async run.Pending.Migrations(): Promise<number> {
    try {
      ...
      loggererror('Failed to run migrations:', error instanceof Error ? error.message : String(error);
      throw error instanceof Error ? error.message : String(error)}}/**
  ...
  async rollback.Last(): Promise<void> {
    try {
      ...
      loggererror('Rollback failed:', error instanceof Error ? error.message : String(error);
      throw error instanceof Error ? error.message : String(error)}}/**
   * Validate migrations*/
  async validate(): Promise<boolean> {
    try {
      ...
      loggererror('Migration validation failed:', error instanceof Error ? error.message : String(error);
      return false}}/**
   * Generate checksum for migration content*/
  private generate.Checksum(contentstring): string {
    return cryptocreate.Hash('sha256')update(content.trim())digest('hex')}/**
