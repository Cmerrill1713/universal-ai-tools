...
export class SafeMemory.System.Wrapper {
    ...
    while (thisinitialization.Attempts < (thisconfigmax.Retries || 3)) {
      try {
        ...
        if (error instanceof Error ? error.message : String(error){
          throw new Error(`Supabase connection test failed: ${error.message}`)}// Create memory system with safe defaults,
  ...
  async store.Memory(
    ...
    try {
      ...
      thisconfigloggererror('Failed to store memory:', error instanceof Error ? error.message : String(error)// If it's a connection error instanceof Error ? error.message : String(error) try to reinitialize;
      if (thisshould.Reinitialize(error instanceof Error ? error.message : String(error) {
  ...
  async search.Memories(options: any): Promise<any[]> {
    ...
    try {
      ...
      thisconfigloggererror('Failed to search memories:', error instanceof Error ? error.message : String(error)// If it's a connection error instanceof Error ? error.message : String(error) try to reinitialize;
      if (thisshould.Reinitialize(error instanceof Error ? error.message : String(error) {
        thisis.Initialized = false;
        thisinitialization.Attempts = 0;
        const reinitialized = await thisinitialize();
  ...
  async update.Memory(memory.Id: string, updates: any): Promise<unknown> {
    ...
    try {
      ...
      const { data, error } = await thisconfigsupabase;
        ...
        select();
        single();
      if (error instanceof Error ? error.message : String(error) throw error instanceof Error ? error.message : String(error)// Update importance if needed;
      if (updatesimportance.Boost) {
        await thismemory.System!update.Memory.Importance(memory.Id, updatesimportance.Boost);

      return data} catch (error) {
      thisconfigloggererror('Failed to update memory:', error instanceof Error ? error.message : String(error);
      if (thisshould.Reinitialize(error instanceof Error ? error.message : String(error) {
        thisis.Initialized = false;
        thisinitialization.Attempts = 0;
        const reinitialized = await thisinitialize();
  ...
  async delete.Memory(memory.Id: string): Promise<boolean> {
    ...
    try {
      ...
      const { error instanceof Error ? error.message : String(error)  = await thisconfigsupabasefrom('ai_memories')delete()eq('id', memory.Id);
      if (error instanceof Error ? error.message : String(error) throw error instanceof Error ? error.message : String(error)// Also delete from memory connections;
      await thisconfigsupabase;
        from('memory_connections');
        delete();
        or(`source_memory_ideq.${memory.Id},target_memory_ideq.${memory.Id}`);
      return true} catch (error) {
      thisconfigloggererror('Failed to delete memory:', error instanceof Error ? error.message : String(error);
      if (thisshould.Reinitialize(error instanceof Error ? error.message : String(error) {
        thisis.Initialized = false;
        thisinitialization.Attempts = 0;
        const reinitialized = await thisinitialize();
  ...
  async generate.Embedding(text: string): Promise<number[]> {
    ...
    try {
      ...
      thisconfigloggererror('Failed to generate embedding:', error instanceof Error ? error.message : String(error);
      if (thisshould.Reinitialize(error instanceof Error ? error.message : String(error) {
        thisis.Initialized = false;
        thisinitialization.Attempts = 0;
        const reinitialized = await thisinitialize();
  ...
  private should.Reinitialize(error instanceof Error ? error.message : String(error) any): boolean {
    const error.Message = error instanceof Error ? error.message : String(error) message || '';
    const connection.Errors = [
      ...
      'Cannot read';
      'undefined'];
    return connection.Errorssome((keyword) => error.Message.includes(keyword));
  ...
  get.Status(): {
    ...
    return {
      ...
      error instanceof Error ? error.message : String(error) thisinitialization.Error?message || null;
    };
