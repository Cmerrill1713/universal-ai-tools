...
export class Production.Embedding.Service {;
  ...
  private get.Cache.Key(text: string): string {;
    const normalized = text.trim()to.Lower.Case();
    return crypto;
      create.Hash('md5');
    ...
    for (let attempt = 0; attempt < thisconfigretry.Attempts, attempt++) {;
      try {;
        ...
        if (error instanceof Error) {;
          if (;
            error.message.includes('rate limit') || error.message.includes('quota') || error.message.includes('invalid_api_key')) {;
            // Wait longer for rate limits;
            if (error.message.includes('rate limit')) {;
              await thissleep(thisconfigretry.Delay.Ms * (attempt + 1) * 2)} else {;
              throw error instanceof Error ? error.message : String(error) // Don't retry quota/auth errors}};
