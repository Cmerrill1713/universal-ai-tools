...
export class PatternMiningSystem extends EventEmitter {
  ...
  private async fetchData(dataSource: DataSource): Promise<any[]> {
    try {
      switch (dataSource?.type) {
        ...
        default: logger?.warn(`Unknown data source, type: ${dataSource?.type)}`, LogContext?.SYSTEM)
          return [];
      }
    } catch (error) {
  ...
  private generateNodeSignature(node: any, ts: any): string | null {
    try {
      ...
      switch (node?.kind) {
        case ts?.SyntaxKind?.IfStatement: return `if-statement`;
        case ts?.SyntaxKind?.ForStatement: return `for-loop`;
        case ts?.SyntaxKind?.WhileStatement: return `while-loop`;
        case ts?.SyntaxKind?.TryStatement: return `try-catch`;
        case ts?.SyntaxKind?.CallExpression: const expr = node?.expression;
