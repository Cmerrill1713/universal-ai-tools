    ...
    const status = {
      ...
      tools: {,
        ...
        averageSuccessRate: tools?.length > 0 
          ? tools?.reduce((sum: number, t: any) => sum + t?.performance?.successRate, 0) / tools?.length: 0,
      },
      performance: {,
        ...
        averageAgentSuccessRate: agents?.length > 0 
          ? agents?.reduce((sum: any, a: any) => sum + (a?.performance?.successRate || 0), 0) / agents?.length: 0,
      },
      evolution: {,
