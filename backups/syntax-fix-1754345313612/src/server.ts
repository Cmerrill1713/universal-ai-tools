...
class UniversalAIToolsServer {
  ...
  private setupMiddleware(): void {
    ...
    // Request timeout middleware (first middleware to ensure all requests are covered)
          ...
          try {
            ...
            const { data: memoryData, error: memoryError } = await this?.supabase;
              ...
              .insert({
                ...
                content_type: 'image',
                visual_embedding: (embeddingResult as unknown).data?.vector,
                image_metadata: {
      ...
      async (req, res) => {
        try {
          ...
          const agentContext = {
            ...
            userId: (req as unknown).user?.id || 'anonymous',
            ...context,
          };
    ...
    try {
      ...
      import('./services/device-auth-websocket')
        .then(({ deviceAuthWebSocket }) => {
          deviceAuthWebSocket?.initialize(this?.server as unknown, '/ws/device-auth');
      ...
      import('./services/athena-websocket')
        .then(({ athenaWebSocket, handleAthenaWebSocket }) => {
          ...
          this?.io?.of('/athena').on('connection', (socket) => {
            ...
            athenaWebSocket?.handleConnection(mockWs);
  ...
  private async loadAsyncRoutes(): Promise<void> {
    ...
    // Load Smart Auto-Healing routes;
    try {
      const smartHealingModule = await import('./routers/smart-healing');
      this?.app?.use('/api/v1/smart-healing', smartHealingModule?.default);
      log?.info('✅ Smart Auto-Healing routes loaded', LogContext?.SERVER);
      
      // Start the smart healing processor;
      const { smartAutoHealingProcessor } = await import('./services/smart-auto-healing-processor');
      await smartAutoHealingProcessor?.start();
      log?.info('✅ Smart Auto-Healing processor started', LogContext?.SERVER);
    } catch (error) {
      log?.warn('⚠️ Smart Auto-Healing routes failed to load', LogContext?.SERVER, {
        error: error instanceof Error ? error?.message : String(error),
      });
    }
    ...
    try {
      const visionDebugModule = await import('./routers/vision-debug');
    ...
    try {
      ...
      log?.info('✅ MLX fine-tuning routes loaded', LogContext?.SERVER);
    } catch (error) {
    ...
    try {
      log?.info('⚡ Loading fast coordinator router...', LogContext?.SERVER);
      ...
      log?.info('✅ Fast coordinator routes loaded', LogContext?.SERVER);
    } catch (error) {
      log?.error('❌ Failed to load fast coordinator router', LogContext?.SERVER, {
        error: error instanceof Error ? error?.message : String(error),
      });
    }

    try {
      log?.info('🏰 Loading memory palace router...', LogContext?.SERVER);
      const memoryPalaceModule = await import('./routers/memory-palace');
      this?.app?.use('/api/v1/memory-palace', memoryPalaceModule?.default);
      log?.info('✅ Memory palace routes loaded', LogContext?.SERVER);
    } catch (error) {
      log?.error('❌ Failed to load memory palace router', LogContext?.SERVER, {
        error: error instanceof Error ? error?.message : String(error),
      });
    }

    try {
      log?.info('🐝 Loading MALT swarm router...', LogContext?.SERVER);
      const maltSwarmModule = await import('./routers/malt-swarm');
      this?.app?.use('/api/v1/malt-swarm', maltSwarmModule?.default);
      log?.info('✅ MALT swarm routes loaded', LogContext?.SERVER);
    } catch (error) {
      log?.error('❌ Failed to load MALT swarm router', LogContext?.SERVER, {
        error: error instanceof Error ? error?.message : String(error),
      });
    }

    try {
      log?.info('📦 Loading sandboxed execution router...', LogContext?.SERVER);
      const sandboxModule = await import('./routers/sandboxed-execution');
      this?.app?.use('/api/v1/sandbox', sandboxModule?.default);
      log?.info('✅ Sandboxed execution routes loaded', LogContext?.SERVER);
    } catch (error) {
      log?.error('❌ Failed to load sandboxed execution router', LogContext?.SERVER, {
        error: error instanceof Error ? error?.message : String(error),
      });
    }

    try {
      log?.info('🚀 Loading speculative inference router...', LogContext?.SERVER);
      const speculativeModule = await import('./routers/speculative-inference');
      this?.app?.use('/api/v1/speculative', speculativeModule?.default);
      log?.info('✅ Speculative inference routes loaded', LogContext?.SERVER);
    } catch (error) {
      log?.error('❌ Failed to load speculative inference router', LogContext?.SERVER, {
        error: error instanceof Error ? error?.message : String(error),
      });
    }

    try {
      log?.info('📡 Loading event stream router...', LogContext?.SERVER);
      const eventStreamModule = await import('./routers/event-stream');
      this?.app?.use('/api/v1/events', eventStreamModule?.default);
      log?.info('✅ Event stream routes loaded', LogContext?.SERVER);
    } catch (error) {
      log?.error('❌ Failed to load event stream router', LogContext?.SERVER, {
        error: error instanceof Error ? error?.message : String(error),
      });
    }

    try {
      log?.info('⚙️ Loading parameters router...', LogContext?.SERVER);
      const parametersModule = await import('./routers/parameters');
      this?.app?.use('/api/v1/parameters', parametersModule?.default);
      log?.info('✅ Parameters routes loaded', LogContext?.SERVER);
    } catch (error) {
