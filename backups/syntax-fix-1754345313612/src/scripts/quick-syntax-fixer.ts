...
class QuickSyntaxFixer {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      // Fix implicitly has unknown type;
  ...
  private async fixAnyDotContent(): Promise<void> {
    ...
    const { stdout } = await execAsync('grep -r "as unknown\.content" src/ --include="*.ts" || true');
    ...
    if (stdout?.trim()) {
      ...
      for (const line of lines) {
        ...
        if (filePath) {
          await this?.fixFilePattern(filePath, /as unknown?.content/g, 'as (any).content');
        }
      }
    }
  }
  ...
  private async fixImplicitAnyType(): Promise<void> {
    ...
    const files = ['src/agents/enhanced-base-agent?.ts'];

    for (const filePath of files) {
      try {
        await this?.fixFilePattern(filePath, /protecteds+(w+);s*$/gm, 'protected $1: any;');
      } catch (error) {
  ...
  private async fixSpreadTypes(): Promise<void> {
    // Fix spread types issues;
    const files = ['src/agents/agent-registry?.ts'];
    ...
    for (const filePath of files) {
      try {
        ...
        newContent = newContent?.replace(/...(w+),?s*(=})/g, '...(($1 as unknown) || {})');
