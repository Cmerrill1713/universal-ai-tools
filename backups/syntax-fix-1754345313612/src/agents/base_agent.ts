...
export abstract class BaseAgent extends EventEmitter {
  ...
  async initialize(memoryCoordinator?: unknown): Promise<void> {
    try {
      ...
      (this as unknown).logger?.info(`✅ Agent ${(this as unknown).config?.name} initialized successfully`);
    } catch (error) {
      (this as unknown).logger?.error(
        `❌ Failed to initialize agent ${(this as unknown).config?.name}:`,
        error;
      );
      throw error;
    }
  ...
  async shutdown(): Promise<void> {
    try {
      ...
      (this as unknown).logger?.info(`✅ Agent ${(this as unknown).config?.name} shutdown complete`);
    } catch (error) {
      (this as unknown).logger?.error(`❌ Error during agent shutdown:`, error);
    }
  }
  ...
  protected async loadMemory(): Promise<void> {
    ...
    try {
      ...
      (this as unknown).logger?.debug(`📚 Loaded memory for agent ${(this as unknown).config?.name}`);
    } catch (error) {
      (this as unknown).logger?.warn(
        `⚠️ Failed to load memory for agent ${(this as unknown).config?.name}:`,
        error;
      );
    }
  }
  ...
  protected async retrieveMemory(context: AgentContext): Promise<any> {
    ...
    try {
      ...
      (this as unknown).logger?.warn(`⚠️ Failed to retrieve memory:`, error);
      return null;
    }
  }
  ...
  protected async storeMemory(context: AgentContext, result: PartialAgentResponse): Promise<void> {
    ...
    try {
      ...
      (this as unknown).logger?.warn(`⚠️ Failed to store memory:`, error);
    }
  }
  ...
  protected onRequestStarted(event: unknown): void {
    if (event && typeof event === 'object' && 'requestId' in event) {
      (this as unknown).logger?.debug(
        `🚀 Agent ${(this as unknown).config?.name} processing request ${event?.requestId}`
      );
    }
  }
  ...
  protected onRequestCompleted(event: unknown): void {
    if (event && typeof event === 'object' && 'requestId' in event) {
      (this as unknown).logger?.debug(
        `✅ Agent ${(this as unknown).config?.name} completed request ${event?.requestId}`
      );
    }
  }
