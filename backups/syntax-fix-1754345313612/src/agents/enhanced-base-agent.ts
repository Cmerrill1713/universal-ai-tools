...
export abstract class EnhancedBaseAgent {
  ...
  protected async processLLMResponse(
    ...
    try {
      ...
      let data: unknown = (llmResponse as unknown).content;
      let reasoning = (llmResponse as unknown).content;

      try {
        const parsed = JSON?.parse((llmResponse as unknown).content);
        if (parsed && typeof parsed === 'object') {
          data = parsed;
          reasoning = parsed?.reasoning || parsed?.explanation || (llmResponse as unknown).content;
        }
      } catch {
  ...
  public async executeValidated<T>(
    ...
    try {
      ...
      return this?.createValidatedSuccessResponse(
        response?.data as T,
        response?.message || 'Agent execution completed successfully',
        response?.confidence,
        response?.reasoning,
        dataSchema,
      );
  ...
  public getPerformanceMetrics() {
    ...
    if (this?.executionHistory?.length > 0) {
      ...
      const totalConfidence = recentExecutions?.reduce((sum, e) => sum + (e?.response?.confidence || 0), 0);
  ...
  protected calculateReward(
    ...
    const quality = response?.success ? (response?.confidence || 0) : 0;
    ...
    const tokensUsed = (response?.metadata?.tokens as unknown)?.total_tokens || 100;
