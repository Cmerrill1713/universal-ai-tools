...
export class AgentRegistry extends EventEmitter {
  ...
  private createAgentDefinition(partial: {
    name: string;
    ...
    return {
      ...
      type: partial?.category?.toLowerCase(),
      description: partial?.description,
      priority: partial?.priority,
      className: partial?.className,
      modulePath: partial?.modulePath,
      dependencies: partial?.dependencies,
      capabilities: partial?.capabilities,
      memoryEnabled: partial?.memoryEnabled,
  ...
  private registerBuiltInAgents(): void {
    // Core agents;
    this?.registerAgent(this?.createAgentDefinition({
      name: 'planner',
  ...
  public async processRequest(agentName: string, context: unknown): Promise<unknown> {
    ...
    // Process the request;
    const result = await agent?.execute(context as unknown);
    return result;
  }
  ...
  private async createTaskRecord(
    ...
    try {
      const { error } = await (this?.supabase as unknown).from('tasks').insert({
        id: taskId,
        agent_name: primaryAgent,
        supporting_agents: supportingAgents,
        user_request: (context as unknown).userRequest || 'No request specified',
        context,
        status: 'running',
        priority: 'medium',
  ...
  public async orchestrateAgents(
    ...
    const hasErrors = (primaryResult as unknown).error || supportingResults?.some((r) => r?.error);
