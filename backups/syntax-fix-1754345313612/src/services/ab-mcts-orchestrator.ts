...
export class ABMCTSOrchestrator {
  ...
  private calculateResourcesUsed(
    ...
    return {
      ...
      tokensUsed: (executionResult as unknown).response?.metadata?.tokens?.total_tokens || 0,
    };
  }
  ...
  async getVisualization(orchestrationId: string): Promise<ABMCTSVisualization | null> {
    ...
    return abMCTSService?.getVisualizationData() as unknown;
  }
  ...
  /**
   * Get orchestrator statistics (alias for getStatistics)
   */
  getStats() {
