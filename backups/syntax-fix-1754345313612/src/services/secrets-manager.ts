...
export class SecretsManager {
  ...
  public async storeSecret(config: SecretConfig): Promise<boolean> {
    ...
    try {
      ...
      const { data: existing } = await (this as unknown).supabase;
        .rpc('read_secret', { secret_name: config?.name })
        .single();

      if (existing) {
        // Update existing secret;
        const { error } = await (this as unknown).supabase?.rpc('update_secret', {
        ...
        // Create new secret;
        const { error } = await (this as unknown).supabase?.rpc('insert_secret', {
          name: config?.name,
          secret: config?.value,
      ...
      if (config?.service) {
        const { error } = await (this as unknown).supabase?.from('api_secrets').upsert(
          {
  ...
  public async getSecret(name: string): Promise<string | null> {
    ...
    try {
      const { data, error } = await (this as unknown).supabase;
  ...
  public async getMissingCredentials(): Promise<string[]> {
    ...
    try {
      const { data, error } = await (this as unknown).supabase?.rpc('get_missing_credentials').single();
