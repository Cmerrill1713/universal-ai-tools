...
export class ABMCTSAutoPilot extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    if (this?.config?.autoLearnThreshold > 0) {
      ...
      }, this?.config?.learningInterval) as unknown as NodeJS?.Timeout;
    }
  ...
  private async processTask(task: AutoPilotTask): Promise<any> {
    ...
    try {
      ...
      await this?.generateAutoFeedback(task, result as unknown as AutoPilotResult);
   ...
   */
  private async generateAutoFeedback(task: AutoPilotTask, result: AutoPilotResult): Promise<void> {
  ...
  private async performLearningCycle(): Promise<void> {
    try {
      ...
      const recentFeedback = feedbackCollector?.getRecentFeedback(50, Date?.now() - (30 * 60 * 1000)); // Last 30 minutes;
      ...
      const performanceAnalysis = this?.analyzePerformance(recentFeedback as unknown as FeedbackItem[]);

      // Optimize parameters if enabled;
      if (this?.config?.autoOptimizeParameters && (performanceAnalysis?.averageScore || 0) < 0?.8) {
        await this?.optimizeParameters(performanceAnalysis as unknown as PerformanceAnalysisData);
      }
   ...
   */
  private analyzePerformance(feedback: FeedbackItem[]): AnalysisResult {
    const scores = feedback?.map((f) => f?.score || (f?.reward as unknown).value || 0);
    ...
    const averageTime = times?.reduce((a: number, b: number) => a + b, 0) / times?.length;
  ...
  private async optimizeParameters(analysis: PerformanceAnalysisData): Promise<void> {
    try {
      ...
      for (const recommendation of recommendations?.slice(0, THREE)) {
        log?.info('📊 Applying recommendation', LogContext?.AI, recommendation as unknown);
        
        // Apply the recommendation using the parameter analytics service;
        const result = await parameterAnalyticsService?.applyRecommendation(recommendation);
        
        if (result?.success) {
          log?.info('✅ Recommendation applied successfully', LogContext?.AI, {
            message: result?.message,
            parameters: result?.appliedParameters,
          });
        } else {
          log?.warn('⚠️ Failed to apply recommendation', LogContext?.AI, {
            message: result?.message,
   ...
   * Update metrics;
   */
  private updateMetrics(update: UpdateData): void {
  ...
  private calculateDistribution(scores: number[]): Record<string, number> {
    ...
    return {
      ...
      median: sorted[Math?.floor(sorted?.length / TWO)] ?? 0,
  ...
  private calculateTrends(feedback: FeedbackItem[]): AnalysisResult {
    ...
    const recentAvg = (sorted as unknown).slice(-10).reduce((a: number, b: any) => a + (b?.score || 0), 0) / 10,
    const olderAvg = (sorted as unknown).slice(0, 10).reduce((a: number, b: any) => a + (b?.score || 0), 0) / 10,
    ...
    return {
      ...
      averageTime: sorted?.length > 0 ? 
        sorted?.reduce((sum, item) => sum + (item?.processingTime || 0), 0) / sorted?.length : 0,
