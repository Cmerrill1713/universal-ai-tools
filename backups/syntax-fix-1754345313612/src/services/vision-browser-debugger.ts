...
class VisionBrowserDebugger {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // Validate vision service connection;
      await this?.validateVisionService();
      ...
      setInterval(async () => {
        if (this?.isRunning) {
          try {
            ...
            console?.error('❌ Monitoring cycle failed:', error instanceof Error ? error?.message : String(error));
          }
        }
      ...
      console?.error('❌ Failed to start Vision Browser Debugger:', error instanceof Error ? error?.message : String(error));
...
- Suggested fixes for each issue`,
  ...
  private ensureDirectories(): void {
    [this?.screenshotsPath, 'logs', 'logs/screenshots/uploads'].forEach((dir) => {
  ...
  /**
   * Validate vision service connectivity;
   */
  private async validateVisionService(): Promise<void> {
    try {
      const response = await fetch(`${this?.visionServiceUrl}/health`, {
        method: 'GET',
        signal: AbortSignal?.timeout(5000),
      });

      if (!response?.ok) {
        console?.warn('⚠️ Vision service not available - fallback analysis will be used');
      } else {
        console?.log('✅ Vision service connection validated');
      }
    } catch (error) {
      console?.warn('⚠️ Vision service validation failed - fallback analysis will be used:', 
   ...
   */
  setScreenshotInterval(intervalMs: number): void {
    this?.screenshotInterval = Math?.max(10000, intervalMs); // Minimum 10 seconds;
    console?.log(`⏱️ Screenshot interval updated to ${this?.screenshotInterval}ms`);
  }

  /**
   * Configure vision service URL;
   */
  setVisionServiceUrl(url: string): void {
    this?.visionServiceUrl = url;
    console?.log(`🔗 Vision service URL updated to ${url}`);
  }
  ...
  /**
   * Test the vision debugging functionality;
   */
  async runDiagnosticTest(): Promise<{
    success: boolean;
    results: Array<{
      test: string;
      passed: boolean;
    ...
    const results = [];
    console?.log('🧪 Running Vision Browser Debugger diagnostic tests...');

    // Test 1: Directory creation;
    const dirTest = { test: 'Directory Creation', passed: false, message: '', duration: 0 };
    const startTime = Date?.now();
    try {
      this?.ensureDirectories();
      dirTest?.passed = fs?.existsSync(this?.screenshotsPath) && fs?.existsSync('logs');
      dirTest?.message = dirTest?.passed ? 'Directories created successfully' : 'Failed to create required directories';
    } catch (error) {
      dirTest?.message = `Directory creation failed: ${error instanceof Error ? error?.message : String(error)}`;
    }
    dirTest?.duration = Date?.now() - startTime;
    results?.push(dirTest);

    // Test 2: Vision service connectivity;
    const visionTest = { test: 'Vision Service', passed: false, message: '', duration: 0 };
    const visionStartTime = Date?.now();
    try {
      await this?.validateVisionService();
      visionTest?.passed = true;
      visionTest?.message = 'Vision service connectivity validated';
    } catch (error) {
      visionTest?.message = `Vision service test failed: ${error instanceof Error ? error?.message : String(error)}`;
    }
    visionTest?.duration = Date?.now() - visionStartTime;
    results?.push(visionTest);

    // Test 3: Screenshot capability;
    const screenshotTest = { test: 'Screenshot Capture', passed: false, message: '', duration: 0 };
    const screenshotStartTime = Date?.now();
    try {
      const testScreenshot = await this?.captureDevToolsScreenshot();
      screenshotTest?.passed = testScreenshot !== null;
      screenshotTest?.message = screenshotTest?.passed 
        ? `Screenshot captured successfully: ${testScreenshot}` 
        : 'Screenshot capture failed';
    } catch (error) {
      screenshotTest?.message = `Screenshot test failed: ${error instanceof Error ? error?.message : String(error)}`;
    }
    screenshotTest?.duration = Date?.now() - screenshotStartTime;
    results?.push(screenshotTest);

    // Test 4: Analysis functionality;
    const analysisTest = { test: 'Analysis Pipeline', passed: false, message: '', duration: 0 };
    const analysisStartTime = Date?.now();
    try {
      const mockAnalysis = this?.generateFallbackAnalysis('test-image?.png');
      analysisTest?.passed = mockAnalysis !== null && typeof mockAnalysis === 'object';
      analysisTest?.message = analysisTest?.passed 
        ? 'Analysis pipeline functional' 
        : 'Analysis pipeline failed';
    } catch (error) {
      analysisTest?.message = `Analysis test failed: ${error instanceof Error ? error?.message : String(error)}`;
    }
    analysisTest?.duration = Date?.now() - analysisStartTime;
    ...
    console?.log(`🧪 Diagnostic test ${overallSuccess ? '✅ PASSED' : '❌ FAILED'} (${totalDuration}ms)`);
   ...
   */
  exportAnalysisData(analysisId?: string): {
    analysisId: string;
    ...
    try {
      if (analysisId) {
        const analysis = this?.analysisResults?.find(a => a?.id === analysisId);
      ...
      console?.error('❌ Failed to export analysis data:', error instanceof Error ? error?.message : String(error));
      return null;
    }
  }
}
...
// Start if run directly (ES6 module compatibility)
