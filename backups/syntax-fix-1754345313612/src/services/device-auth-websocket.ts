...
export class DeviceAuthWebSocketService {
  ...
  private async verifyClient(
    ...
    try {
      ...
      const decoded = jwt?.verify(token, process?.env?.JWT_SECRET || 'device-auth-secret') as unknown;

      // Store user info in request for later use;
      (info?.req as unknown).userId = decoded?.userId;
      (info?.req as unknown).deviceId = decoded?.deviceId;
