...
export class ContextInjectionService {
  ...
  private securityFilters = {
    promptInjectionPatterns: [
      /ignores+previouss+instructions/gi,
    ...
    sensitiveDataPatterns: [
      ...
      /\bd{4}[-s]d{4}[-s]d{4}[-s]d{4}\b/g, // Credit cards;
  ...
  /**
   * Inject context method (for backward compatibility)
   */
  async injectContext(request: any, options: any = {}): Promise<any> {
    return this?.enrichWithContext(request?.userRequest || request?.query || '', options);
  }
}
