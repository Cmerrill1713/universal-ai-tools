...
class AdvancedHealingSystem {
    ...
    try {
      ...
      if (this?.networkHealingService) {
        try {
          ...
          console?.error('❌ Failed to start network healing service:', error instanceof Error ? error?.message : String(error));
        }
      }
      ...
      try {
        ...
        console?.error('❌ Failed to run initial diagnostic:', error instanceof Error ? error?.message : String(error));
      }
      ...
      setInterval(async () => {
        if (this?.isRunning) {
          try {
            ...
            console?.error('❌ Healing cycle failed:', error instanceof Error ? error?.message : String(error));
          }
        }
      }, this?.healingInterval);

    } catch (error) {
      console?.error('❌ Failed to start Advanced Healing System:', error instanceof Error ? error?.message : String(error));
  ...
  async healSyntaxIssues(task: HealingTask): Promise<HealingResult> {
    try {
      ...
      const stats = await healer?.healDirectory('/Users/christianmerrill/Desktop/universal-ai-tools/src');
