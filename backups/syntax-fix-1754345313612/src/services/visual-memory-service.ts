...
export class VisualMemoryService {
  ...
  private calculateLearningDelta(prediction: unknown, actual: unknown): any {
    return {
      added: this?.findDifferences(actual, prediction),
      removed: this?.findDifferences(prediction, actual),
      confidence_change: ((actual as unknown).confidence || 0) - ((prediction as unknown).confidence || 0),
    };
  }

  private findDifferences(a: any, b: any): any {
    const diff: any = {
      added: {},
      removed: {},
      modified: {}
    ...
    for (const key in a as Record<string, any>) {
      if (!(key in (b as Record<string, any>)) || JSON?.stringify((a as unknown)[key]) !== JSON?.stringify((b as unknown)[key])) {
        diff?.modified[key] = (a as unknown)[key];
      }
    }
    return diff;
  }
