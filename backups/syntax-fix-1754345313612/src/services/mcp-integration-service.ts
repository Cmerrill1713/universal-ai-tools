...
export class MCPIntegrationService extends EventEmitter {
  ...
  async start(): Promise<boolean> {
    ...
    try {
      ...
      if (this?.retryCount < this?.maxRetries) {
        this?.retryCount++;
        log?.info(
          `🔄 Retrying MCP server start (${this?.retryCount}/${this?.maxRetries})`,
          LogContext?.MCP;
        );
  ...
  private async fallbackOperation(method: string, params: unknown): Promise<unknown> {
    ...
    try {
      switch (method) {
        case 'save_context':
          return await this?.fallbackSaveContext(params as unknown);
        case 'search_context':
          return await this?.fallbackSearchContext(params as unknown);
        case 'get_recent_context':
          return await this?.fallbackGetRecentContext(params as unknown);
        case 'save_code_pattern':
          return await this?.fallbackSaveCodePattern(params as unknown);
        case 'get_code_patterns':
          return await this?.fallbackGetCodePatterns(params as unknown);
        default:
