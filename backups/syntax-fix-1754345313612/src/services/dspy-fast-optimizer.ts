...
Create an optimized prompt that generates higher quality responses for this task type.`;
  ...
  private async runDSPyOptimization(
    ...
    return {
      ...
      performanceGain: 15 + Math?.random() * 0?.2, // 15-35% improvement;
      confidence: 8 + Math?.random() * 0?.15, // 80-95% confidence,
  ...
  private generatePerformanceRecommendations(results: any): string[] {
    ...
    if ((results as unknown).lfm2?.avgResponseTime < (results as unknown).ollama?.avgResponseTime) {
      recommendations?.push('Use LFM2 for more simple tasks to improve speed');
    }

    if ((results as unknown).ollama?.accuracy > (results as unknown).lfm2?.accuracy) {
