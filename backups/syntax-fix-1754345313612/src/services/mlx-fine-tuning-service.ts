...
export class MLXFineTuningService extends EventEmitter {
  ...
  private async validateDataset(data: unknown[], format: string): Promise<DatasetValidationResult> {
    ...
    const sampleEntry = data[0] as unknown;
    ...
    for (const entry of data as unknown[]) {
      ...
      if (!(entry as unknown).input || !(entry as unknown).output) {
        malformed++;
        continue;
      }

      // Check for duplicates;
      const key = `${(entry as unknown).input}|${(entry as unknown).output}`;
      if (seen?.has(key)) {
        duplicates++;
      } else {
        seen?.add(key);
  ...
  public async createFineTuningJob(
    ...
    try {
      ...
      const job: FineTuningJob = {
        ...
        outputModelPath,
        datasetPath,
        datasetFormat: this?.detectDatasetFormat(datasetPath) as unknown,
        hyperparameters: {
          learningRate: 0001,
          batchSize: 4,
          epochs: 3,
