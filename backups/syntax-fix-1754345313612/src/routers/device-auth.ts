...
/**
 * Verify device signature using crypto module;
 * In production, this would use proper ECDSA or RSA signature verification;
 */
async function verifyDeviceSignature(
  ...
  try {
    // For test devices, allow any signature;
    if (publicKey?.includes('temp-key') || publicKey?.includes('test')) {
      return true;
    }

    // Simple HMAC-based verification for demo purposes;
    ...
    log?.error('Signature verification failed', LogContext?.AUTH, {
...
router?.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const isValidSignature = await verifyDeviceSignature(
        challenge?.challenge || '',
        signature,
        device?.publicKey;
      );
