  ...
  try {
    ...
    if (!keys || typeof keys !== 'object') {
      res?.status(400).json({
        ...
        details: 'Request body must contain a keys object'
      });
      return;
    }
    ...
    for (const [keyName, keyValue] of Object?.entries(keys)) {
      ...
      try {
        ...
        const existing = await secretsManager?.getSecret(vaultKeyName);
        if (existing) {
          // Update existing key;
          await secretsManager?.storeSecret({ name: vaultKeyName, value: keyValue as string, description: `MCP agent key for ${agentId}` });
        } else {
          // Create new key;
          await secretsManager?.storeSecret({ name: vaultKeyName, value: keyValue as string, description: `MCP agent key for ${agentId}` });
        }
