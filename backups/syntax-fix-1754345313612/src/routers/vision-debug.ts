...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file?.mimetype?.startsWith('image/')) {
      ...
      cb(new Error('Only image files are allowed') as unknown, false);
    }
  },
});
    ...
    try {
      ...
      return res?.json({
        ...
        meta: {
          ...
          totalAnalyses: (visionDebugger?.getStatus() as unknown).totalAnalyses,
        },
      });
    } catch (error) {
      return res?.status(500).json({
...
router?.get('/health', (req: Request, res: Response) => {
  try {
    const status = visionDebugger?.getStatus() as unknown;
...
/**
 * @route POST /api/v1/vision-debug/diagnostic-test;
 * @desc Run comprehensive diagnostic test;
 * @access Public;
 */
router?.post('/diagnostic-test', async (req: Request, res: Response) => {
  try {
    const testResults = await visionDebugger?.runDiagnosticTest();
    
    return res?.status(testResults?.success ? 200 : 503).json({
      success: testResults?.success,
      data: testResults,
      message: testResults?.success ? 'All diagnostic tests passed' : 'Some diagnostic tests failed',
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    return res?.status(500).json({
      success: false,
      error: 'Diagnostic test execution failed',
      details: error instanceof Error ? error?.message : 'Unknown error',
    });
  }
});
...
/**
 * @route GET /api/v1/vision-debug/export/:analysisId?
 * @desc Export analysis data;
 * @access Public;
 */
router?.get('/export/:analysisId?', (req: Request, res: Response) => {
  try {
    const { analysisId } = req?.params;
    const exportData = visionDebugger?.exportAnalysisData(analysisId);
