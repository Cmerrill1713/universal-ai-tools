...
// AB-MCTS status endpoint;
router?.get('/status', async (req: Request, res: Response) => {
  try {
    const stats = abMCTSOrchestrator?.getStats();
    const status = {
    ...
    log?.error('AB-MCTS status check failed', LogContext?.AGENT, {
      error: error instanceof Error ? error?.message : String(error),
    });
    sendError(res, 'AB_MCTS_STATUS_ERROR', 'Failed to check AB-MCTS status', 500);
  }
});
  ...
  try {
    ...
    sendSuccess(
      res,
      {
        taskId,
        status: 'queued',
        queuePosition: (abMCTSAutoPilot as unknown).getQueueStatus().queued,
        timestamp: Date?.now(),
      },
      200,
      ...
      res,
      {
        ...
        recommendations: [
          metrics?.successfulRequests / metrics?.totalRequests < 0?.8;
            ...
            : null,
          (queueStatus as unknown).queued > 10 ? 'High queue size - consider increasing batch size' : null,
