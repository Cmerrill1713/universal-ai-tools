  ...
  try {
    const capabilities = {
      ...
      features: {
        ...
        textExtraction: true,
        faceRecognition: false, // Not implemented yet;
      ...
      backends: {
        python: true,
        mlx: true,
        gpu: false, // Would detect actual GPU;
      },
      uptime: process?.uptime(),
      timestamp: new Date().toISOString(),
    };
    
    sendSuccess(res, capabilities, 200);
  } catch (error) {
    log?.error('Vision capabilities check failed', LogContext?.VISION, {
      error: error instanceof Error ? error?.message : String(error),
    });
    sendError(res, 'VISION_CAPABILITIES_ERROR', 'Failed to get vision capabilities', 500);
  }
});
...
router?.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    // Validate request - skip schema validation if file is uploaded;
    if (!req?.file) {
      try {
      ...
      if (refine?.enabled && result?.data?.base64) {
        try {
          ...
          if (refinement?.success) {
            ...
            (responseData as unknown).refinement_applied = true;
          }
        } catch (refineError) {
