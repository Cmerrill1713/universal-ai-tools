/**
 * AB-MCTS API Router - Fixed Version;
...
router?.post('/reset', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!(req as unknown).user?.isAdmin) {
...
/**
 * @route GET /api/v1/ab-mcts/status;
 * @desc Get AB-MCTS service status (alias for health)
 */
router?.get('/status', async (req: Request, res: Response) => {
  try {
    ...
    log?.error('AB-MCTS status check failed', LogContext?.API, { error });
    sendError(res, 'INTERNAL_ERROR', 'Status check failed', 500);
  }
});
