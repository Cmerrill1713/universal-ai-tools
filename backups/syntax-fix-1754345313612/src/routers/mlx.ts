...
router?.get('/status', async (req: Request, res: Response) => {
  try {
    const status = await mlxService?.healthCheck();
    sendSuccess(res, status, 200);
  } catch (error) {
    log?.error('MLX status check failed', LogContext?.MLX, {
      error: error instanceof Error ? error?.message : String(error),
    });
    sendError(res, 'MLX_STATUS_ERROR', 'Failed to check MLX status', 500);
  }
});
...
router?.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { type, available } = req?.query as unknown;
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      log?.info('🧠 MLX inference requested with intelligent parameters', LogContext?.API, {
        ...
        taskType: (req as unknown).taskContext?.type,
