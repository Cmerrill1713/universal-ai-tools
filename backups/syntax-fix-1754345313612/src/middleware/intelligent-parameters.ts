...
export function optimizeParameters(
  ...
  const result: OptimizedParameters & { 
    ...
    stopSequences: optimizedParams?.stopSequences,
    systemPrompt: optimizedParams?.systemPrompt,
    confidence: optimizedParams?.confidence || 0?.8,
    reasoning: optimizedParams?.reasoning || 'Auto-optimized parameters',
  };

  // Add extended properties for compatibility;
  (result as unknown).stop = optimizedParams?.stopSequences;
  (result as unknown).system = optimizedParams?.systemPrompt;
  (result as unknown).userPromptTemplate = optimizedParams?.userPromptTemplate;
  (result as unknown).taskType = taskContext?.type;
  (result as unknown).complexity = taskContext?.complexity;
