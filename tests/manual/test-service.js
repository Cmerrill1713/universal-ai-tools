...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      response:
        "I'm having trouble processing your message right now. Could you try rephrasing or asking something simpler?",
      type: 'error',
      error: error.message,
    });
  }
  ...
  try {
    const response = await axios.post(
      'http://localhost:11434/api/generate',
      {
        model: 'gemma3n:e2b',
        prompt:
          'You are an expert AI assistant specializing in software development, system setup, and technical guidance. You have access to advanced multi-agent orchestration capabilities and memory systems.\n\nPlease provide helpful, accurate, and detailed responses. If asked about system setup or technical implementation, provide step-by-step guidance.\n\nUser: ' +
          message +
...
async function generateIntelligentResponse(message, sessionId) {
  ...
  if (messageLower.includes('hello') || messageLower.includes('hi')) {
    return {
      message:
        'Hello! I am your Universal AI Tools assistant. I can help you set up systems, answer questions about development, or have a general conversation. What can I help you with today?',
      confidence: 0.9,
  ...
  if (messageLower.includes('how are you')) {
    return {
      message:
        'I am operating at full capacity with all my cognitive agents ready to assist! My enhanced multi-agent orchestration system is running smoothly. How can I help you today?',
      confidence: 0.9,
...
async function analyzeUserIntent(userRequest) {
  ...
  if (requestLower.includes('trading') || requestLower.includes('bot')) {
    ...
    requestLower.includes('mcp') ||
    requestLower.includes('claude desktop') ||
    requestLower.includes('model context protocol')
  ) {
    domain = 'mcp_setup';
    complexity = 'medium';
  }
...
async function performCriticalAnalysis(plan) {
  ...
  const severity =
    plan.complexity === 'high' ? 'high' : plan.complexity === 'medium' ? 'medium' : 'low';
...
async function validateSafety(solution) {
  return {
    ...
    safetyLevel: 'high',
    concerns: [],
    recommendations: ['Regular security audits', 'Monitor for unusual activity'],
    confidence: 0.9,
  };
}
...
function buildComprehensiveReasoning(analysis) {
  ...
  return (
    ...
    '% confidence)**\n' +
    'Identified your primary goal as "' +
    ...
    'Created a ' +
    plan.steps.length +
    '-step implementation plan optimized for ' +
    intent.domain +
    ' setups. Estimated completion time: ' +
    plan.estimatedTime +
    '.\n\n' +
    ...
    ' severity)**\n' +
    'Identified ' +
    Object.values(risks.risks).flat().length +
    ' potential risks across technical, operational, and security domains.\n\n' +
    '**Solution Synthesis (' +
    (solution.confidence * 100).toFixed(0) +
    '% confidence)**\n' +
    'Integrated insights from multiple cognitive agents to recommend: ' +
    solution.tools.join(', ') +
    '.\n\n' +
    '**Safety Validation (' +
    (safetyValidation.approved ? 'APPROVED' : 'REQUIRES REVIEW') +
    ')**\n' +
    (safetyValidation.approved
      ? 'Solution meets safety and security requirements'
