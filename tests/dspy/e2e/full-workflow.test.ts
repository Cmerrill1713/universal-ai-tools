...
testUserId = `test-user-${uuidv4()}`;
  ...
  describe('Code Development Workflow', () => {
    it('should complete a full code development workflow', async () => {
        ...
        },
        {
          ...
          action: async () => {
            ...
            return await dspyService.coordinateAgents(
              'Generate TypeScript code for todo API endpoints',
              agents,
              {
                projectPath,
      ...
      expect(workflow.totalDuration).toBeLessThan(30000); // Complete within 30 seconds
    });
  });
        ...
        },
        {
          ...
          action: async () => {
            ...
            const problematicRequest = {
              requestId: uuidv4(),
              userRequest: '', // Empty request to trigger error handling
              userId: testUserId,
          ...
          validate: (result) => {
            // Should either handle the error or succeed on retry
            expect(result).toBeTruthy();
      ...
      const workflow = await executeWorkflow('Full System Integration', [
        {
          ...
          action: async () => {
            // Initialize session with memory system
            const sessionData = {
              sessionId,
              userId: testUserId,
              startTime: new Date(),
