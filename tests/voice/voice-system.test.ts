...
// Mock classes for removed agent files
...
class VoiceServiceCircuitBreaker {
  execute = jest.fn();
...
jest.mock('../../src/services/secrets-manager', () => ({
  secretsManager: {
    getSecret: jest.fn(() => Promise.resolve('test-secret-key')),
    getAvailableServices: jest.fn(() => Promise.resolve([])),
    getServiceConfig: jest.fn(() => Promise.resolve(null)),
  },
}));
jest.mock('../../src/utils/api-response', () => ({
  sendError: jest.fn((res: any, type: string, message: string, status?: number) => {
...
jest.mock('../../src/middleware/auth', () => ({
  ...
  authenticateAPIKey: jest.fn(() => Promise.resolve(true)),
  requireAdmin: (req: any, res: any, next: any) => next(),
}));

// Mock the circuit breaker and cache managers
// Mock circuit breaker functionality inline to avoid missing module issues
...
// Mock cache functionality inline to avoid missing module issues
const mockVoiceCache = {
  synthesisCache: {
...
// Mock removed agent functionality inline rather than importing
const mockConversationalVoiceAgent = {
  handleVoiceInteraction: jest.fn(() => Promise.resolve({
    success: true,
  ...
  getCapabilities: jest.fn(() => ['conversation', 'commands'])
};
...
describe('Voice System Tests', () => {
  ...
  describe('Circuit Breaker', () => {
    ...
    it('should open circuit after failure threshold', async () => {
      const failingFunction = jest.fn(() => Promise.reject(new Error('Service error')));

      // Mock the execute method to throw errors and track state
      breaker.execute.mockImplementation(async (fn: any) => {
    ...
    it('should enter half-open state after timeout', async () => {
      ...
      const successFunction = jest.fn(() => Promise.resolve('success'));
    ...
    it('should track metrics correctly', async () => {
      const successFunction = jest.fn(() => Promise.resolve('success'));
      const failFunction = jest.fn(() => Promise.reject(new Error('fail')));
