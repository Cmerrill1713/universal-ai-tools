    ...
    describe('Security and Performance', () => {
        it('should rate limit knowledge creation', async () => {
            const promises = Array(10)
                .fill(null)
                .map(() => request(app).post('/api/knowledge').set('Authorization', 'Bearer valid-token').send({
        ...
        it('should sanitize search queries to prevent injection', async () => {
            ...
            expect([200, 400]).toContain(response.status);
            if (response.status === 200) {
                expect(mockSupabase.rpc).toHaveBeenCalledWith('search_knowledge', expect.not.objectContaining({ query: maliciousQuery.query }));
            }
        });
    });
});
