...
describe('Orchestration Router', () => {
    ...
    describe('POST /api/orchestration/orchestrate', () => {
        ...
        it('should sanitize task input', async () => {
            ...
            const response = await request(app)
                ...
                .send(maliciousTask);
            expect(response.status).toBe(200);
            expect(mockOrchestrator.orchestrate).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('POST /api/orchestration/coordinate', () => {
        ...
        it('should validate workflow dependencies', async () => {
            const invalidWorkflow = {
                ...
                workflow: {
                    type: 'sequential',
                    dependencies: ['agent-1 → non-existent-agent'],
                },
            };
            const response = await request(app)
                .post('/api/orchestration/coordinate')
    ...
    describe('POST /api/orchestration/knowledge/search', () => {
        ...
        it('should apply context-aware filtering', async () => {
            ...
            expect(response.status).toBe(200);
            expect(mockKnowledgeManager.search).toHaveBeenCalledWith(expect.objectContaining({
                ...
                optimized_prompts: [
                    {
                        ...
                        optimized: 'Generate a comprehensive API test plan including unit, integration, and security tests',
                        improvements: {
                            accuracy: 0.89,
                            efficiency: 0.84,
                            clarity: 0.92,
                    ...
                    },
                    {
                        ...
                        optimized: 'Conduct a thorough security code review focusing on authentication, authorization, and input validation',
                        improvements: {
                            accuracy: 0.91,
                            efficiency: 0.79,
                            clarity: 0.88,
