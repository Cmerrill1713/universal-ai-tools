      ...
      for (const page of pages) {
        await agentPool.executeOnAgent(agent.id, async (agent) => {
          if (agent.type === 'puppeteer') {
            await (agent.page as any).goto(`http://localhost:5173${page}`, { waitUntil: 'networkidle0' });
            const url = await (agent.page as any).url();
            expect(url).toContain(page === '/' ? 'localhost:5173' : page);
          } else {
            await (agent.page as any).goto(`http://localhost:5173${page}`, { waitUntil: 'networkidle' });
            const url = await (agent.page as any).url();
            expect(url).toContain(page === '/' ? 'localhost:5173' : page);
          }
        });
  ...
  test('should detect and interact with buttons', async () => {
    ...
    for (const agent of agents) {
      ...
      await agentPool.executeOnAgent(agent.id, async (agent) => {
        // Go to memory page
        if (agent.type === 'puppeteer') {
          await (agent.page as any).goto('http://localhost:5173/memory', { waitUntil: 'networkidle0' });
          ...
          for (const button of buttons.slice(0, 3)) { // Test first 3 buttons
            const isEnabled = await button.evaluate((btn: any) => !btn.disabled);
            expect(isEnabled).toBe(true);
          }
        } else {
          await (agent.page as any).goto('http://localhost:5173/memory', { waitUntil: 'networkidle' });
