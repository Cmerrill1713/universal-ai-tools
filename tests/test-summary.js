...
export class TestSummaryGenerator {
    ...
    determineQualityGate(coverage) {
        const avgCoverage = (coverage.lines.pct +
            coverage.statements.pct +
            coverage.functions.pct +
            coverage.branches.pct) / 4;
        if (avgCoverage >= 90)
            return 'excellent';
        if (avgCoverage >= 80)
            return 'good';
    ...
    calculateProductionReadiness(summary) {
        const blockers = [];
        const warnings = [];
        let score = 100;
        // Test results impact
        const requiredTests = this.testSuites.filter(s => s.required);
        const failedRequired = this.results.filter(r => r.status === 'failed' &&
        ...
        if (summary.coverage.overall) {
            const avgCoverage = (summary.coverage.overall.lines.pct +
                summary.coverage.overall.statements.pct +
                summary.coverage.overall.functions.pct +
                summary.coverage.overall.branches.pct) / 4;
            if (avgCoverage < 80) {
        ...
        const securityTests = this.results.filter(r => r.name.toLowerCase().includes('security') ||
        ...
        const performanceTests = this.results.filter(r => r.name.toLowerCase().includes('performance'));
    ...
    generateRecommendations(summary) {
        ...
        const securityTests = summary.testSuites.filter(t => t.name.toLowerCase().includes('security') ||
    ...
    async generateReport() {
        ...
        const requiredFailures = this.results.filter(r => r.status === 'failed' &&
            this.testSuites.find(s => s.name === r.name)?.required).length;
        const overallStatus = requiredFailures > 0 ? 'failed' :
