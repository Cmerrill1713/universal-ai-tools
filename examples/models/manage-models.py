...
class InteractiveModelManager:
    ...
    async def list_models(self):
        ...
        for model in all_models.get("ollama", []):
            ...
            table.add_row(status, model.name, size, "Ollama", model.description or "")
        ...
        for model_name, description, size in self.LIQUID_MODELS:
            ...
            liquid_table.add_row(status, model_name, f"{size:.0f}GB", description)
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    "http://localhost:11434/api/show", json={"name": model_name}, timeout=5.0
                )
